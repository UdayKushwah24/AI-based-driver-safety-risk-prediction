...
const source330 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 330) / 4) * 11;
  const drift = Math.cos((idx + 330) / 5) * 9;
  const edge = ((idx * 330) % 19) - 9;
...
function smooth330(input, radius) {
  const result = [];
...
function derive330(series, mode) {
  ...
  const spread = normalize330(peak - valley);
  const jitter = normalize330(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize330(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize330(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize330(avg),
    peak: normalize330(peak),
    valley: normalize330(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch330(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize330(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic330(initialSeed = 330) {
  ...
  useEffect(() => {
    ...
    fakeFetch330(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source330.map((value, idx) => {
      ...
      return normalize330(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth330(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize330(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive330(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize330(quality) });
    dispatch({ type: 'marker', payload: normalize330(flags.confidence * 0.7 + quality * 0.3) });
