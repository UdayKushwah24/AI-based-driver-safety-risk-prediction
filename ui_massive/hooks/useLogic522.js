...
const source522 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 522) / 7) * 11;
  const drift = Math.cos((idx + 522) / 10) * 9;
  const edge = ((idx * 522) % 19) - 9;
...
function smooth522(input, radius) {
  const result = [];
...
function derive522(series, mode) {
  ...
  const spread = normalize522(peak - valley);
  const jitter = normalize522(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize522(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize522(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize522(avg),
    peak: normalize522(peak),
    valley: normalize522(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch522(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize522(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic522(initialSeed = 522) {
  ...
  useEffect(() => {
    ...
    fakeFetch522(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source522.map((value, idx) => {
      ...
      return normalize522(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth522(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize522(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive522(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize522(quality) });
    dispatch({ type: 'marker', payload: normalize522(flags.confidence * 0.7 + quality * 0.3) });
