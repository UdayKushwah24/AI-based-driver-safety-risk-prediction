...
const source517 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 517) / 9) * 11;
  const drift = Math.cos((idx + 517) / 5) * 9;
  const edge = ((idx * 517) % 19) - 9;
...
function smooth517(input, radius) {
  const result = [];
...
function derive517(series, mode) {
  ...
  const spread = normalize517(peak - valley);
  const jitter = normalize517(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize517(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize517(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize517(avg),
    peak: normalize517(peak),
    valley: normalize517(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch517(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize517(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic517(initialSeed = 517) {
  ...
  useEffect(() => {
    ...
    fakeFetch517(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source517.map((value, idx) => {
      ...
      return normalize517(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth517(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize517(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive517(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize517(quality) });
    dispatch({ type: 'marker', payload: normalize517(flags.confidence * 0.7 + quality * 0.3) });
