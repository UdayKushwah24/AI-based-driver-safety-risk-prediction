...
const source414 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 414) / 4) * 11;
  const drift = Math.cos((idx + 414) / 12) * 9;
  const edge = ((idx * 414) % 19) - 9;
...
function smooth414(input, radius) {
  const result = [];
...
function derive414(series, mode) {
  ...
  const spread = normalize414(peak - valley);
  const jitter = normalize414(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize414(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize414(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize414(avg),
    peak: normalize414(peak),
    valley: normalize414(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch414(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize414(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic414(initialSeed = 414) {
  ...
  useEffect(() => {
    ...
    fakeFetch414(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source414.map((value, idx) => {
      ...
      return normalize414(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth414(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize414(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive414(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize414(quality) });
    dispatch({ type: 'marker', payload: normalize414(flags.confidence * 0.7 + quality * 0.3) });
