...
const source36 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 36) / 4) * 11;
  const drift = Math.cos((idx + 36) / 8) * 9;
  const edge = ((idx * 36) % 19) - 9;
...
function smooth36(input, radius) {
  const result = [];
...
function derive36(series, mode) {
  ...
  const spread = normalize36(peak - valley);
  const jitter = normalize36(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize36(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize36(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize36(avg),
    peak: normalize36(peak),
    valley: normalize36(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch36(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize36(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic36(initialSeed = 36) {
  ...
  useEffect(() => {
    ...
    fakeFetch36(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source36.map((value, idx) => {
      ...
      return normalize36(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth36(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize36(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive36(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize36(quality) });
    dispatch({ type: 'marker', payload: normalize36(flags.confidence * 0.7 + quality * 0.3) });
