...
const source66 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 66) / 6) * 11;
  const drift = Math.cos((idx + 66) / 5) * 9;
  const edge = ((idx * 66) % 19) - 9;
...
function smooth66(input, radius) {
  const result = [];
...
function derive66(series, mode) {
  ...
  const spread = normalize66(peak - valley);
  const jitter = normalize66(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize66(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize66(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize66(avg),
    peak: normalize66(peak),
    valley: normalize66(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch66(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize66(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic66(initialSeed = 66) {
  ...
  useEffect(() => {
    ...
    fakeFetch66(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source66.map((value, idx) => {
      ...
      return normalize66(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth66(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize66(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive66(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize66(quality) });
    dispatch({ type: 'marker', payload: normalize66(flags.confidence * 0.7 + quality * 0.3) });
