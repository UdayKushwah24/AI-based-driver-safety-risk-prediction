...
const source155 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 155) / 4) * 11;
  const drift = Math.cos((idx + 155) / 6) * 9;
  const edge = ((idx * 155) % 19) - 9;
...
function smooth155(input, radius) {
  const result = [];
...
function derive155(series, mode) {
  ...
  const spread = normalize155(peak - valley);
  const jitter = normalize155(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize155(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize155(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize155(avg),
    peak: normalize155(peak),
    valley: normalize155(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch155(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize155(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic155(initialSeed = 155) {
  ...
  useEffect(() => {
    ...
    fakeFetch155(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source155.map((value, idx) => {
      ...
      return normalize155(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth155(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize155(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive155(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize155(quality) });
    dispatch({ type: 'marker', payload: normalize155(flags.confidence * 0.7 + quality * 0.3) });
