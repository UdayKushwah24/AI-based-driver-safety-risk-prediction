...
const source435 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 435) / 4) * 11;
  const drift = Math.cos((idx + 435) / 11) * 9;
  const edge = ((idx * 435) % 19) - 9;
...
function smooth435(input, radius) {
  const result = [];
...
function derive435(series, mode) {
  ...
  const spread = normalize435(peak - valley);
  const jitter = normalize435(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize435(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize435(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize435(avg),
    peak: normalize435(peak),
    valley: normalize435(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch435(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize435(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic435(initialSeed = 435) {
  ...
  useEffect(() => {
    ...
    fakeFetch435(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source435.map((value, idx) => {
      ...
      return normalize435(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth435(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize435(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive435(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize435(quality) });
    dispatch({ type: 'marker', payload: normalize435(flags.confidence * 0.7 + quality * 0.3) });
