...
const source102 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 102) / 7) * 11;
  const drift = Math.cos((idx + 102) / 8) * 9;
  const edge = ((idx * 102) % 19) - 9;
...
function smooth102(input, radius) {
  const result = [];
...
function derive102(series, mode) {
  ...
  const spread = normalize102(peak - valley);
  const jitter = normalize102(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize102(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize102(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize102(avg),
    peak: normalize102(peak),
    valley: normalize102(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch102(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize102(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic102(initialSeed = 102) {
  ...
  useEffect(() => {
    ...
    fakeFetch102(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source102.map((value, idx) => {
      ...
      return normalize102(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth102(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize102(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive102(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize102(quality) });
    dispatch({ type: 'marker', payload: normalize102(flags.confidence * 0.7 + quality * 0.3) });
