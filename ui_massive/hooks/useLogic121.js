...
const source121 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 121) / 5) * 11;
  const drift = Math.cos((idx + 121) / 5) * 9;
  const edge = ((idx * 121) % 19) - 9;
...
function smooth121(input, radius) {
  const result = [];
...
function derive121(series, mode) {
  ...
  const spread = normalize121(peak - valley);
  const jitter = normalize121(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize121(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize121(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize121(avg),
    peak: normalize121(peak),
    valley: normalize121(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch121(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize121(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic121(initialSeed = 121) {
  ...
  useEffect(() => {
    ...
    fakeFetch121(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source121.map((value, idx) => {
      ...
      return normalize121(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth121(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize121(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive121(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize121(quality) });
    dispatch({ type: 'marker', payload: normalize121(flags.confidence * 0.7 + quality * 0.3) });
