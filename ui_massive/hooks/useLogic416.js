...
const source416 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 416) / 6) * 11;
  const drift = Math.cos((idx + 416) / 14) * 9;
  const edge = ((idx * 416) % 19) - 9;
...
function smooth416(input, radius) {
  const result = [];
...
function derive416(series, mode) {
  ...
  const spread = normalize416(peak - valley);
  const jitter = normalize416(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize416(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize416(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize416(avg),
    peak: normalize416(peak),
    valley: normalize416(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch416(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize416(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic416(initialSeed = 416) {
  ...
  useEffect(() => {
    ...
    fakeFetch416(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source416.map((value, idx) => {
      ...
      return normalize416(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth416(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize416(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive416(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize416(quality) });
    dispatch({ type: 'marker', payload: normalize416(flags.confidence * 0.7 + quality * 0.3) });
