...
const source81 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 81) / 7) * 11;
  const drift = Math.cos((idx + 81) / 9) * 9;
  const edge = ((idx * 81) % 19) - 9;
...
function smooth81(input, radius) {
  const result = [];
...
function derive81(series, mode) {
  ...
  const spread = normalize81(peak - valley);
  const jitter = normalize81(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize81(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize81(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize81(avg),
    peak: normalize81(peak),
    valley: normalize81(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch81(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize81(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic81(initialSeed = 81) {
  ...
  useEffect(() => {
    ...
    fakeFetch81(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source81.map((value, idx) => {
      ...
      return normalize81(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth81(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize81(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive81(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize81(quality) });
    dispatch({ type: 'marker', payload: normalize81(flags.confidence * 0.7 + quality * 0.3) });
