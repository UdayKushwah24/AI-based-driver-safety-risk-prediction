...
const source139 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 139) / 9) * 11;
  const drift = Math.cos((idx + 139) / 12) * 9;
  const edge = ((idx * 139) % 19) - 9;
...
function smooth139(input, radius) {
  const result = [];
...
function derive139(series, mode) {
  ...
  const spread = normalize139(peak - valley);
  const jitter = normalize139(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize139(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize139(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize139(avg),
    peak: normalize139(peak),
    valley: normalize139(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch139(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize139(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic139(initialSeed = 139) {
  ...
  useEffect(() => {
    ...
    fakeFetch139(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source139.map((value, idx) => {
      ...
      return normalize139(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth139(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize139(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive139(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize139(quality) });
    dispatch({ type: 'marker', payload: normalize139(flags.confidence * 0.7 + quality * 0.3) });
