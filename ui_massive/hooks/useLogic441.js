...
const source441 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 441) / 3) * 11;
  const drift = Math.cos((idx + 441) / 6) * 9;
  const edge = ((idx * 441) % 19) - 9;
...
function smooth441(input, radius) {
  const result = [];
...
function derive441(series, mode) {
  ...
  const spread = normalize441(peak - valley);
  const jitter = normalize441(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize441(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize441(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize441(avg),
    peak: normalize441(peak),
    valley: normalize441(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch441(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize441(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic441(initialSeed = 441) {
  ...
  useEffect(() => {
    ...
    fakeFetch441(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source441.map((value, idx) => {
      ...
      return normalize441(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth441(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize441(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive441(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize441(quality) });
    dispatch({ type: 'marker', payload: normalize441(flags.confidence * 0.7 + quality * 0.3) });
