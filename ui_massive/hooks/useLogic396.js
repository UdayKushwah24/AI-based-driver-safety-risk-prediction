...
const source396 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 396) / 7) * 11;
  const drift = Math.cos((idx + 396) / 5) * 9;
  const edge = ((idx * 396) % 19) - 9;
...
function smooth396(input, radius) {
  const result = [];
...
function derive396(series, mode) {
  ...
  const spread = normalize396(peak - valley);
  const jitter = normalize396(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize396(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize396(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize396(avg),
    peak: normalize396(peak),
    valley: normalize396(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch396(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize396(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic396(initialSeed = 396) {
  ...
  useEffect(() => {
    ...
    fakeFetch396(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source396.map((value, idx) => {
      ...
      return normalize396(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth396(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize396(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive396(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize396(quality) });
    dispatch({ type: 'marker', payload: normalize396(flags.confidence * 0.7 + quality * 0.3) });
