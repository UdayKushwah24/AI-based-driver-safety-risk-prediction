...
const source491 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 491) / 4) * 11;
  const drift = Math.cos((idx + 491) / 12) * 9;
  const edge = ((idx * 491) % 19) - 9;
...
function smooth491(input, radius) {
  const result = [];
...
function derive491(series, mode) {
  ...
  const spread = normalize491(peak - valley);
  const jitter = normalize491(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize491(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize491(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize491(avg),
    peak: normalize491(peak),
    valley: normalize491(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch491(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize491(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic491(initialSeed = 491) {
  ...
  useEffect(() => {
    ...
    fakeFetch491(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source491.map((value, idx) => {
      ...
      return normalize491(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth491(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize491(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive491(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize491(quality) });
    dispatch({ type: 'marker', payload: normalize491(flags.confidence * 0.7 + quality * 0.3) });
