...
const source538 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 538) / 9) * 11;
  const drift = Math.cos((idx + 538) / 15) * 9;
  const edge = ((idx * 538) % 19) - 9;
...
function smooth538(input, radius) {
  const result = [];
...
function derive538(series, mode) {
  ...
  const spread = normalize538(peak - valley);
  const jitter = normalize538(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize538(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize538(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize538(avg),
    peak: normalize538(peak),
    valley: normalize538(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch538(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize538(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic538(initialSeed = 538) {
  ...
  useEffect(() => {
    ...
    fakeFetch538(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source538.map((value, idx) => {
      ...
      return normalize538(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth538(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize538(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive538(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize538(quality) });
    dispatch({ type: 'marker', payload: normalize538(flags.confidence * 0.7 + quality * 0.3) });
