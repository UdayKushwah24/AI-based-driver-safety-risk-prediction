...
const source324 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 324) / 5) * 11;
  const drift = Math.cos((idx + 324) / 10) * 9;
  const edge = ((idx * 324) % 19) - 9;
...
function smooth324(input, radius) {
  const result = [];
...
function derive324(series, mode) {
  ...
  const spread = normalize324(peak - valley);
  const jitter = normalize324(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize324(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize324(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize324(avg),
    peak: normalize324(peak),
    valley: normalize324(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch324(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize324(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic324(initialSeed = 324) {
  ...
  useEffect(() => {
    ...
    fakeFetch324(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source324.map((value, idx) => {
      ...
      return normalize324(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth324(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize324(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive324(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize324(quality) });
    dispatch({ type: 'marker', payload: normalize324(flags.confidence * 0.7 + quality * 0.3) });
