...
const source166 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 166) / 8) * 11;
  const drift = Math.cos((idx + 166) / 6) * 9;
  const edge = ((idx * 166) % 19) - 9;
...
function smooth166(input, radius) {
  const result = [];
...
function derive166(series, mode) {
  ...
  const spread = normalize166(peak - valley);
  const jitter = normalize166(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize166(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize166(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize166(avg),
    peak: normalize166(peak),
    valley: normalize166(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch166(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize166(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic166(initialSeed = 166) {
  ...
  useEffect(() => {
    ...
    fakeFetch166(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source166.map((value, idx) => {
      ...
      return normalize166(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth166(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize166(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive166(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize166(quality) });
    dispatch({ type: 'marker', payload: normalize166(flags.confidence * 0.7 + quality * 0.3) });
