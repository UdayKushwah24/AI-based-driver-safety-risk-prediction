...
const source56 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 56) / 3) * 11;
  const drift = Math.cos((idx + 56) / 6) * 9;
  const edge = ((idx * 56) % 19) - 9;
...
function smooth56(input, radius) {
  const result = [];
...
function derive56(series, mode) {
  ...
  const spread = normalize56(peak - valley);
  const jitter = normalize56(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize56(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize56(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize56(avg),
    peak: normalize56(peak),
    valley: normalize56(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch56(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize56(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic56(initialSeed = 56) {
  ...
  useEffect(() => {
    ...
    fakeFetch56(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source56.map((value, idx) => {
      ...
      return normalize56(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth56(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize56(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive56(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize56(quality) });
    dispatch({ type: 'marker', payload: normalize56(flags.confidence * 0.7 + quality * 0.3) });
