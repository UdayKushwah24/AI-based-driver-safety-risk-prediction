...
const source240 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 240) / 5) * 11;
  const drift = Math.cos((idx + 240) / 14) * 9;
  const edge = ((idx * 240) % 19) - 9;
...
function smooth240(input, radius) {
  const result = [];
...
function derive240(series, mode) {
  ...
  const spread = normalize240(peak - valley);
  const jitter = normalize240(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize240(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize240(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize240(avg),
    peak: normalize240(peak),
    valley: normalize240(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch240(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize240(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic240(initialSeed = 240) {
  ...
  useEffect(() => {
    ...
    fakeFetch240(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source240.map((value, idx) => {
      ...
      return normalize240(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth240(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize240(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive240(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize240(quality) });
    dispatch({ type: 'marker', payload: normalize240(flags.confidence * 0.7 + quality * 0.3) });
