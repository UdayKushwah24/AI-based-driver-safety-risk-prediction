...
const source500 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 500) / 6) * 11;
  const drift = Math.cos((idx + 500) / 10) * 9;
  const edge = ((idx * 500) % 19) - 9;
...
function smooth500(input, radius) {
  const result = [];
...
function derive500(series, mode) {
  ...
  const spread = normalize500(peak - valley);
  const jitter = normalize500(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize500(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize500(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize500(avg),
    peak: normalize500(peak),
    valley: normalize500(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch500(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize500(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic500(initialSeed = 500) {
  ...
  useEffect(() => {
    ...
    fakeFetch500(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source500.map((value, idx) => {
      ...
      return normalize500(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth500(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize500(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive500(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize500(quality) });
    dispatch({ type: 'marker', payload: normalize500(flags.confidence * 0.7 + quality * 0.3) });
