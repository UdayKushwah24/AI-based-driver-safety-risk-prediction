...
const source448 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 448) / 3) * 11;
  const drift = Math.cos((idx + 448) / 13) * 9;
  const edge = ((idx * 448) % 19) - 9;
...
function smooth448(input, radius) {
  const result = [];
...
function derive448(series, mode) {
  ...
  const spread = normalize448(peak - valley);
  const jitter = normalize448(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize448(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize448(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize448(avg),
    peak: normalize448(peak),
    valley: normalize448(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch448(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize448(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic448(initialSeed = 448) {
  ...
  useEffect(() => {
    ...
    fakeFetch448(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source448.map((value, idx) => {
      ...
      return normalize448(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth448(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize448(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive448(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize448(quality) });
    dispatch({ type: 'marker', payload: normalize448(flags.confidence * 0.7 + quality * 0.3) });
