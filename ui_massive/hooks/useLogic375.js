...
const source375 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 375) / 7) * 11;
  const drift = Math.cos((idx + 375) / 6) * 9;
  const edge = ((idx * 375) % 19) - 9;
...
function smooth375(input, radius) {
  const result = [];
...
function derive375(series, mode) {
  ...
  const spread = normalize375(peak - valley);
  const jitter = normalize375(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize375(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize375(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize375(avg),
    peak: normalize375(peak),
    valley: normalize375(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch375(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize375(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic375(initialSeed = 375) {
  ...
  useEffect(() => {
    ...
    fakeFetch375(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source375.map((value, idx) => {
      ...
      return normalize375(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth375(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize375(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive375(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize375(quality) });
    dispatch({ type: 'marker', payload: normalize375(flags.confidence * 0.7 + quality * 0.3) });
