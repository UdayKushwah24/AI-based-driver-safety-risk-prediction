...
const source280 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 280) / 3) * 11;
  const drift = Math.cos((idx + 280) / 10) * 9;
  const edge = ((idx * 280) % 19) - 9;
...
function smooth280(input, radius) {
  const result = [];
...
function derive280(series, mode) {
  ...
  const spread = normalize280(peak - valley);
  const jitter = normalize280(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize280(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize280(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize280(avg),
    peak: normalize280(peak),
    valley: normalize280(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch280(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize280(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic280(initialSeed = 280) {
  ...
  useEffect(() => {
    ...
    fakeFetch280(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source280.map((value, idx) => {
      ...
      return normalize280(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth280(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize280(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive280(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize280(quality) });
    dispatch({ type: 'marker', payload: normalize280(flags.confidence * 0.7 + quality * 0.3) });
