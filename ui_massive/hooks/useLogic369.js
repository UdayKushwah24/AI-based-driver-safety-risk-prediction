...
const source369 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 369) / 8) * 11;
  const drift = Math.cos((idx + 369) / 11) * 9;
  const edge = ((idx * 369) % 19) - 9;
...
function smooth369(input, radius) {
  const result = [];
...
function derive369(series, mode) {
  ...
  const spread = normalize369(peak - valley);
  const jitter = normalize369(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize369(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize369(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize369(avg),
    peak: normalize369(peak),
    valley: normalize369(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch369(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize369(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic369(initialSeed = 369) {
  ...
  useEffect(() => {
    ...
    fakeFetch369(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source369.map((value, idx) => {
      ...
      return normalize369(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth369(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize369(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive369(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize369(quality) });
    dispatch({ type: 'marker', payload: normalize369(flags.confidence * 0.7 + quality * 0.3) });
