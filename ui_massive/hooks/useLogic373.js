...
const source373 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 373) / 5) * 11;
  const drift = Math.cos((idx + 373) / 15) * 9;
  const edge = ((idx * 373) % 19) - 9;
...
function smooth373(input, radius) {
  const result = [];
...
function derive373(series, mode) {
  ...
  const spread = normalize373(peak - valley);
  const jitter = normalize373(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize373(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize373(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize373(avg),
    peak: normalize373(peak),
    valley: normalize373(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch373(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize373(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic373(initialSeed = 373) {
  ...
  useEffect(() => {
    ...
    fakeFetch373(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source373.map((value, idx) => {
      ...
      return normalize373(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth373(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize373(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive373(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize373(quality) });
    dispatch({ type: 'marker', payload: normalize373(flags.confidence * 0.7 + quality * 0.3) });
