...
const source485 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 485) / 5) * 11;
  const drift = Math.cos((idx + 485) / 6) * 9;
  const edge = ((idx * 485) % 19) - 9;
...
function smooth485(input, radius) {
  const result = [];
...
function derive485(series, mode) {
  ...
  const spread = normalize485(peak - valley);
  const jitter = normalize485(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize485(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize485(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize485(avg),
    peak: normalize485(peak),
    valley: normalize485(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch485(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize485(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic485(initialSeed = 485) {
  ...
  useEffect(() => {
    ...
    fakeFetch485(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source485.map((value, idx) => {
      ...
      return normalize485(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth485(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize485(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive485(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize485(quality) });
    dispatch({ type: 'marker', payload: normalize485(flags.confidence * 0.7 + quality * 0.3) });
