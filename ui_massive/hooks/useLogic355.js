...
const source355 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 355) / 8) * 11;
  const drift = Math.cos((idx + 355) / 8) * 9;
  const edge = ((idx * 355) % 19) - 9;
...
function smooth355(input, radius) {
  const result = [];
...
function derive355(series, mode) {
  ...
  const spread = normalize355(peak - valley);
  const jitter = normalize355(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize355(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize355(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize355(avg),
    peak: normalize355(peak),
    valley: normalize355(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch355(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize355(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic355(initialSeed = 355) {
  ...
  useEffect(() => {
    ...
    fakeFetch355(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source355.map((value, idx) => {
      ...
      return normalize355(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth355(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize355(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive355(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize355(quality) });
    dispatch({ type: 'marker', payload: normalize355(flags.confidence * 0.7 + quality * 0.3) });
