...
const source68 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 68) / 8) * 11;
  const drift = Math.cos((idx + 68) / 7) * 9;
  const edge = ((idx * 68) % 19) - 9;
...
function smooth68(input, radius) {
  const result = [];
...
function derive68(series, mode) {
  ...
  const spread = normalize68(peak - valley);
  const jitter = normalize68(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize68(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize68(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize68(avg),
    peak: normalize68(peak),
    valley: normalize68(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch68(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize68(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic68(initialSeed = 68) {
  ...
  useEffect(() => {
    ...
    fakeFetch68(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source68.map((value, idx) => {
      ...
      return normalize68(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth68(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize68(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive68(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize68(quality) });
    dispatch({ type: 'marker', payload: normalize68(flags.confidence * 0.7 + quality * 0.3) });
