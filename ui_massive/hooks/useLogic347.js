...
const source347 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 347) / 7) * 11;
  const drift = Math.cos((idx + 347) / 11) * 9;
  const edge = ((idx * 347) % 19) - 9;
...
function smooth347(input, radius) {
  const result = [];
...
function derive347(series, mode) {
  ...
  const spread = normalize347(peak - valley);
  const jitter = normalize347(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize347(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize347(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize347(avg),
    peak: normalize347(peak),
    valley: normalize347(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch347(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize347(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic347(initialSeed = 347) {
  ...
  useEffect(() => {
    ...
    fakeFetch347(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source347.map((value, idx) => {
      ...
      return normalize347(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth347(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize347(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive347(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize347(quality) });
    dispatch({ type: 'marker', payload: normalize347(flags.confidence * 0.7 + quality * 0.3) });
