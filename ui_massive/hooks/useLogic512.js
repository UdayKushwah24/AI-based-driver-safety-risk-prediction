...
const source512 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 512) / 4) * 11;
  const drift = Math.cos((idx + 512) / 11) * 9;
  const edge = ((idx * 512) % 19) - 9;
...
function smooth512(input, radius) {
  const result = [];
...
function derive512(series, mode) {
  ...
  const spread = normalize512(peak - valley);
  const jitter = normalize512(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize512(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize512(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize512(avg),
    peak: normalize512(peak),
    valley: normalize512(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch512(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize512(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic512(initialSeed = 512) {
  ...
  useEffect(() => {
    ...
    fakeFetch512(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source512.map((value, idx) => {
      ...
      return normalize512(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth512(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize512(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive512(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize512(quality) });
    dispatch({ type: 'marker', payload: normalize512(flags.confidence * 0.7 + quality * 0.3) });
