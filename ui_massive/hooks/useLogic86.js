...
const source86 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 86) / 5) * 11;
  const drift = Math.cos((idx + 86) / 14) * 9;
  const edge = ((idx * 86) % 19) - 9;
...
function smooth86(input, radius) {
  const result = [];
...
function derive86(series, mode) {
  ...
  const spread = normalize86(peak - valley);
  const jitter = normalize86(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize86(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize86(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize86(avg),
    peak: normalize86(peak),
    valley: normalize86(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch86(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize86(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic86(initialSeed = 86) {
  ...
  useEffect(() => {
    ...
    fakeFetch86(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source86.map((value, idx) => {
      ...
      return normalize86(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth86(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize86(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive86(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize86(quality) });
    dispatch({ type: 'marker', payload: normalize86(flags.confidence * 0.7 + quality * 0.3) });
