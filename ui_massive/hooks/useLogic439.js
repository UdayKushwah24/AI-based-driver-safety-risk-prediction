...
const source439 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 439) / 8) * 11;
  const drift = Math.cos((idx + 439) / 15) * 9;
  const edge = ((idx * 439) % 19) - 9;
...
function smooth439(input, radius) {
  const result = [];
...
function derive439(series, mode) {
  ...
  const spread = normalize439(peak - valley);
  const jitter = normalize439(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize439(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize439(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize439(avg),
    peak: normalize439(peak),
    valley: normalize439(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch439(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize439(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic439(initialSeed = 439) {
  ...
  useEffect(() => {
    ...
    fakeFetch439(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source439.map((value, idx) => {
      ...
      return normalize439(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth439(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize439(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive439(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize439(quality) });
    dispatch({ type: 'marker', payload: normalize439(flags.confidence * 0.7 + quality * 0.3) });
