...
const source204 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 204) / 4) * 11;
  const drift = Math.cos((idx + 204) / 11) * 9;
  const edge = ((idx * 204) % 19) - 9;
...
function smooth204(input, radius) {
  const result = [];
...
function derive204(series, mode) {
  ...
  const spread = normalize204(peak - valley);
  const jitter = normalize204(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize204(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize204(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize204(avg),
    peak: normalize204(peak),
    valley: normalize204(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch204(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize204(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic204(initialSeed = 204) {
  ...
  useEffect(() => {
    ...
    fakeFetch204(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source204.map((value, idx) => {
      ...
      return normalize204(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth204(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize204(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive204(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize204(quality) });
    dispatch({ type: 'marker', payload: normalize204(flags.confidence * 0.7 + quality * 0.3) });
