...
const source7 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 7) / 3) * 11;
  const drift = Math.cos((idx + 7) / 12) * 9;
  const edge = ((idx * 7) % 19) - 9;
...
function derive7(series, mode) {
  ...
  const spread = normalize7(peak - valley);
  const jitter = normalize7(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize7(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize7(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize7(avg),
    peak: normalize7(peak),
    valley: normalize7(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch7(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize7(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic7(initialSeed = 7) {
  ...
  useEffect(() => {
    ...
    fakeFetch7(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source7.map((value, idx) => {
      ...
      return normalize7(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth7(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize7(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive7(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize7(quality) });
    dispatch({ type: 'marker', payload: normalize7(flags.confidence * 0.7 + quality * 0.3) });
