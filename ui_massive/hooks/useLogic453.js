...
const source453 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 453) / 8) * 11;
  const drift = Math.cos((idx + 453) / 7) * 9;
  const edge = ((idx * 453) % 19) - 9;
...
function smooth453(input, radius) {
  const result = [];
...
function derive453(series, mode) {
  ...
  const spread = normalize453(peak - valley);
  const jitter = normalize453(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize453(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize453(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize453(avg),
    peak: normalize453(peak),
    valley: normalize453(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch453(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize453(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic453(initialSeed = 453) {
  ...
  useEffect(() => {
    ...
    fakeFetch453(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source453.map((value, idx) => {
      ...
      return normalize453(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth453(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize453(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive453(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize453(quality) });
    dispatch({ type: 'marker', payload: normalize453(flags.confidence * 0.7 + quality * 0.3) });
