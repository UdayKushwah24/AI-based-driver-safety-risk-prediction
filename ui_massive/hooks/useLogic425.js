...
const source425 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 425) / 8) * 11;
  const drift = Math.cos((idx + 425) / 12) * 9;
  const edge = ((idx * 425) % 19) - 9;
...
function smooth425(input, radius) {
  const result = [];
...
function derive425(series, mode) {
  ...
  const spread = normalize425(peak - valley);
  const jitter = normalize425(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize425(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize425(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize425(avg),
    peak: normalize425(peak),
    valley: normalize425(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch425(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize425(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic425(initialSeed = 425) {
  ...
  useEffect(() => {
    ...
    fakeFetch425(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source425.map((value, idx) => {
      ...
      return normalize425(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth425(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize425(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive425(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize425(quality) });
    dispatch({ type: 'marker', payload: normalize425(flags.confidence * 0.7 + quality * 0.3) });
