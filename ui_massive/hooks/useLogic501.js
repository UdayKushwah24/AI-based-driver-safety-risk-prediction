...
const source501 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 501) / 7) * 11;
  const drift = Math.cos((idx + 501) / 11) * 9;
  const edge = ((idx * 501) % 19) - 9;
...
function smooth501(input, radius) {
  const result = [];
...
function derive501(series, mode) {
  ...
  const spread = normalize501(peak - valley);
  const jitter = normalize501(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize501(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize501(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize501(avg),
    peak: normalize501(peak),
    valley: normalize501(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch501(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize501(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic501(initialSeed = 501) {
  ...
  useEffect(() => {
    ...
    fakeFetch501(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source501.map((value, idx) => {
      ...
      return normalize501(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth501(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize501(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive501(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize501(quality) });
    dispatch({ type: 'marker', payload: normalize501(flags.confidence * 0.7 + quality * 0.3) });
