...
const source160 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 160) / 9) * 11;
  const drift = Math.cos((idx + 160) / 11) * 9;
  const edge = ((idx * 160) % 19) - 9;
...
function smooth160(input, radius) {
  const result = [];
...
function derive160(series, mode) {
  ...
  const spread = normalize160(peak - valley);
  const jitter = normalize160(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize160(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize160(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize160(avg),
    peak: normalize160(peak),
    valley: normalize160(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch160(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize160(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic160(initialSeed = 160) {
  ...
  useEffect(() => {
    ...
    fakeFetch160(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source160.map((value, idx) => {
      ...
      return normalize160(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth160(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize160(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive160(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize160(quality) });
    dispatch({ type: 'marker', payload: normalize160(flags.confidence * 0.7 + quality * 0.3) });
