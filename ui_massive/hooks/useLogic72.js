...
const source72 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 72) / 5) * 11;
  const drift = Math.cos((idx + 72) / 11) * 9;
  const edge = ((idx * 72) % 19) - 9;
...
function smooth72(input, radius) {
  const result = [];
...
function derive72(series, mode) {
  ...
  const spread = normalize72(peak - valley);
  const jitter = normalize72(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize72(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize72(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize72(avg),
    peak: normalize72(peak),
    valley: normalize72(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch72(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize72(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic72(initialSeed = 72) {
  ...
  useEffect(() => {
    ...
    fakeFetch72(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source72.map((value, idx) => {
      ...
      return normalize72(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth72(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize72(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive72(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize72(quality) });
    dispatch({ type: 'marker', payload: normalize72(flags.confidence * 0.7 + quality * 0.3) });
