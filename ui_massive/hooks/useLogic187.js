...
const source187 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 187) / 8) * 11;
  const drift = Math.cos((idx + 187) / 5) * 9;
  const edge = ((idx * 187) % 19) - 9;
...
function smooth187(input, radius) {
  const result = [];
...
function derive187(series, mode) {
  ...
  const spread = normalize187(peak - valley);
  const jitter = normalize187(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize187(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize187(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize187(avg),
    peak: normalize187(peak),
    valley: normalize187(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch187(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize187(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic187(initialSeed = 187) {
  ...
  useEffect(() => {
    ...
    fakeFetch187(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source187.map((value, idx) => {
      ...
      return normalize187(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth187(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize187(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive187(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize187(quality) });
    dispatch({ type: 'marker', payload: normalize187(flags.confidence * 0.7 + quality * 0.3) });
