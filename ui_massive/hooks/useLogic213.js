...
const source213 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 213) / 6) * 11;
  const drift = Math.cos((idx + 213) / 9) * 9;
  const edge = ((idx * 213) % 19) - 9;
...
function smooth213(input, radius) {
  const result = [];
...
function derive213(series, mode) {
  ...
  const spread = normalize213(peak - valley);
  const jitter = normalize213(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize213(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize213(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize213(avg),
    peak: normalize213(peak),
    valley: normalize213(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch213(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize213(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic213(initialSeed = 213) {
  ...
  useEffect(() => {
    ...
    fakeFetch213(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source213.map((value, idx) => {
      ...
      return normalize213(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth213(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize213(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive213(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize213(quality) });
    dispatch({ type: 'marker', payload: normalize213(flags.confidence * 0.7 + quality * 0.3) });
