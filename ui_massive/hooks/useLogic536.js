...
const source536 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 536) / 7) * 11;
  const drift = Math.cos((idx + 536) / 13) * 9;
  const edge = ((idx * 536) % 19) - 9;
...
function smooth536(input, radius) {
  const result = [];
...
function derive536(series, mode) {
  ...
  const spread = normalize536(peak - valley);
  const jitter = normalize536(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize536(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize536(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize536(avg),
    peak: normalize536(peak),
    valley: normalize536(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch536(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize536(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic536(initialSeed = 536) {
  ...
  useEffect(() => {
    ...
    fakeFetch536(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source536.map((value, idx) => {
      ...
      return normalize536(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth536(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize536(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive536(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize536(quality) });
    dispatch({ type: 'marker', payload: normalize536(flags.confidence * 0.7 + quality * 0.3) });
