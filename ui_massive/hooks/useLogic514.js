...
const source514 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 514) / 6) * 11;
  const drift = Math.cos((idx + 514) / 13) * 9;
  const edge = ((idx * 514) % 19) - 9;
...
function smooth514(input, radius) {
  const result = [];
...
function derive514(series, mode) {
  ...
  const spread = normalize514(peak - valley);
  const jitter = normalize514(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize514(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize514(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize514(avg),
    peak: normalize514(peak),
    valley: normalize514(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch514(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize514(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic514(initialSeed = 514) {
  ...
  useEffect(() => {
    ...
    fakeFetch514(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source514.map((value, idx) => {
      ...
      return normalize514(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth514(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize514(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive514(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize514(quality) });
    dispatch({ type: 'marker', payload: normalize514(flags.confidence * 0.7 + quality * 0.3) });
