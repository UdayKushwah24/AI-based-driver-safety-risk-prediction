...
const source442 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 442) / 4) * 11;
  const drift = Math.cos((idx + 442) / 7) * 9;
  const edge = ((idx * 442) % 19) - 9;
...
function smooth442(input, radius) {
  const result = [];
...
function derive442(series, mode) {
  ...
  const spread = normalize442(peak - valley);
  const jitter = normalize442(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize442(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize442(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize442(avg),
    peak: normalize442(peak),
    valley: normalize442(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch442(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize442(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic442(initialSeed = 442) {
  ...
  useEffect(() => {
    ...
    fakeFetch442(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source442.map((value, idx) => {
      ...
      return normalize442(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth442(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize442(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive442(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize442(quality) });
    dispatch({ type: 'marker', payload: normalize442(flags.confidence * 0.7 + quality * 0.3) });
