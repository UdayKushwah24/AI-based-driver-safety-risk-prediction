...
const source383 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 383) / 8) * 11;
  const drift = Math.cos((idx + 383) / 14) * 9;
  const edge = ((idx * 383) % 19) - 9;
...
function smooth383(input, radius) {
  const result = [];
...
function derive383(series, mode) {
  ...
  const spread = normalize383(peak - valley);
  const jitter = normalize383(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize383(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize383(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize383(avg),
    peak: normalize383(peak),
    valley: normalize383(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch383(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize383(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic383(initialSeed = 383) {
  ...
  useEffect(() => {
    ...
    fakeFetch383(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source383.map((value, idx) => {
      ...
      return normalize383(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth383(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize383(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive383(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize383(quality) });
    dispatch({ type: 'marker', payload: normalize383(flags.confidence * 0.7 + quality * 0.3) });
