...
const source31 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 31) / 6) * 11;
  const drift = Math.cos((idx + 31) / 14) * 9;
  const edge = ((idx * 31) % 19) - 9;
...
function smooth31(input, radius) {
  const result = [];
...
function derive31(series, mode) {
  ...
  const spread = normalize31(peak - valley);
  const jitter = normalize31(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize31(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize31(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize31(avg),
    peak: normalize31(peak),
    valley: normalize31(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch31(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize31(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic31(initialSeed = 31) {
  ...
  useEffect(() => {
    ...
    fakeFetch31(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source31.map((value, idx) => {
      ...
      return normalize31(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth31(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize31(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive31(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize31(quality) });
    dispatch({ type: 'marker', payload: normalize31(flags.confidence * 0.7 + quality * 0.3) });
