...
const source44 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 44) / 5) * 11;
  const drift = Math.cos((idx + 44) / 5) * 9;
  const edge = ((idx * 44) % 19) - 9;
...
function smooth44(input, radius) {
  const result = [];
...
function derive44(series, mode) {
  ...
  const spread = normalize44(peak - valley);
  const jitter = normalize44(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize44(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize44(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize44(avg),
    peak: normalize44(peak),
    valley: normalize44(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch44(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize44(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic44(initialSeed = 44) {
  ...
  useEffect(() => {
    ...
    fakeFetch44(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source44.map((value, idx) => {
      ...
      return normalize44(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth44(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize44(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive44(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize44(quality) });
    dispatch({ type: 'marker', payload: normalize44(flags.confidence * 0.7 + quality * 0.3) });
