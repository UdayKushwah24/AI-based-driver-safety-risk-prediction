...
const source118 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 118) / 9) * 11;
  const drift = Math.cos((idx + 118) / 13) * 9;
  const edge = ((idx * 118) % 19) - 9;
...
function smooth118(input, radius) {
  const result = [];
...
function derive118(series, mode) {
  ...
  const spread = normalize118(peak - valley);
  const jitter = normalize118(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize118(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize118(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize118(avg),
    peak: normalize118(peak),
    valley: normalize118(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch118(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize118(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic118(initialSeed = 118) {
  ...
  useEffect(() => {
    ...
    fakeFetch118(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source118.map((value, idx) => {
      ...
      return normalize118(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth118(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize118(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive118(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize118(quality) });
    dispatch({ type: 'marker', payload: normalize118(flags.confidence * 0.7 + quality * 0.3) });
