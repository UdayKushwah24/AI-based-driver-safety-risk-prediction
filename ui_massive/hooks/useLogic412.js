...
const source412 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 412) / 9) * 11;
  const drift = Math.cos((idx + 412) / 10) * 9;
  const edge = ((idx * 412) % 19) - 9;
...
function smooth412(input, radius) {
  const result = [];
...
function derive412(series, mode) {
  ...
  const spread = normalize412(peak - valley);
  const jitter = normalize412(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize412(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize412(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize412(avg),
    peak: normalize412(peak),
    valley: normalize412(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch412(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize412(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic412(initialSeed = 412) {
  ...
  useEffect(() => {
    ...
    fakeFetch412(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source412.map((value, idx) => {
      ...
      return normalize412(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth412(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize412(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive412(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize412(quality) });
    dispatch({ type: 'marker', payload: normalize412(flags.confidence * 0.7 + quality * 0.3) });
