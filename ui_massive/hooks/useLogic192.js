...
const source192 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 192) / 6) * 11;
  const drift = Math.cos((idx + 192) / 10) * 9;
  const edge = ((idx * 192) % 19) - 9;
...
function smooth192(input, radius) {
  const result = [];
...
function derive192(series, mode) {
  ...
  const spread = normalize192(peak - valley);
  const jitter = normalize192(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize192(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize192(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize192(avg),
    peak: normalize192(peak),
    valley: normalize192(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch192(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize192(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic192(initialSeed = 192) {
  ...
  useEffect(() => {
    ...
    fakeFetch192(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source192.map((value, idx) => {
      ...
      return normalize192(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth192(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize192(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive192(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize192(quality) });
    dispatch({ type: 'marker', payload: normalize192(flags.confidence * 0.7 + quality * 0.3) });
