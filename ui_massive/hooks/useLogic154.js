...
const source154 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 154) / 3) * 11;
  const drift = Math.cos((idx + 154) / 5) * 9;
  const edge = ((idx * 154) % 19) - 9;
...
function smooth154(input, radius) {
  const result = [];
...
function derive154(series, mode) {
  ...
  const spread = normalize154(peak - valley);
  const jitter = normalize154(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize154(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize154(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize154(avg),
    peak: normalize154(peak),
    valley: normalize154(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch154(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize154(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic154(initialSeed = 154) {
  ...
  useEffect(() => {
    ...
    fakeFetch154(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source154.map((value, idx) => {
      ...
      return normalize154(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth154(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize154(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive154(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize154(quality) });
    dispatch({ type: 'marker', payload: normalize154(flags.confidence * 0.7 + quality * 0.3) });
