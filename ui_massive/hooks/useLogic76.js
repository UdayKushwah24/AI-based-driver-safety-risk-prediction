...
const source76 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 76) / 9) * 11;
  const drift = Math.cos((idx + 76) / 15) * 9;
  const edge = ((idx * 76) % 19) - 9;
...
function smooth76(input, radius) {
  const result = [];
...
function derive76(series, mode) {
  ...
  const spread = normalize76(peak - valley);
  const jitter = normalize76(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize76(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize76(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize76(avg),
    peak: normalize76(peak),
    valley: normalize76(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch76(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize76(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic76(initialSeed = 76) {
  ...
  useEffect(() => {
    ...
    fakeFetch76(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source76.map((value, idx) => {
      ...
      return normalize76(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth76(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize76(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive76(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize76(quality) });
    dispatch({ type: 'marker', payload: normalize76(flags.confidence * 0.7 + quality * 0.3) });
