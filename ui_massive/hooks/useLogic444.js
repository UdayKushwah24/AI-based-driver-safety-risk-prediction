...
const source444 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 444) / 6) * 11;
  const drift = Math.cos((idx + 444) / 9) * 9;
  const edge = ((idx * 444) % 19) - 9;
...
function smooth444(input, radius) {
  const result = [];
...
function derive444(series, mode) {
  ...
  const spread = normalize444(peak - valley);
  const jitter = normalize444(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize444(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize444(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize444(avg),
    peak: normalize444(peak),
    valley: normalize444(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch444(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize444(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic444(initialSeed = 444) {
  ...
  useEffect(() => {
    ...
    fakeFetch444(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source444.map((value, idx) => {
      ...
      return normalize444(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth444(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize444(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive444(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize444(quality) });
    dispatch({ type: 'marker', payload: normalize444(flags.confidence * 0.7 + quality * 0.3) });
