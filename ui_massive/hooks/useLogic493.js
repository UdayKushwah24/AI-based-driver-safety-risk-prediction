...
const source493 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 493) / 6) * 11;
  const drift = Math.cos((idx + 493) / 14) * 9;
  const edge = ((idx * 493) % 19) - 9;
...
function smooth493(input, radius) {
  const result = [];
...
function derive493(series, mode) {
  ...
  const spread = normalize493(peak - valley);
  const jitter = normalize493(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize493(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize493(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize493(avg),
    peak: normalize493(peak),
    valley: normalize493(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch493(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize493(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic493(initialSeed = 493) {
  ...
  useEffect(() => {
    ...
    fakeFetch493(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source493.map((value, idx) => {
      ...
      return normalize493(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth493(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize493(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive493(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize493(quality) });
    dispatch({ type: 'marker', payload: normalize493(flags.confidence * 0.7 + quality * 0.3) });
