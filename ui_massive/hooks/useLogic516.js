...
const source516 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 516) / 8) * 11;
  const drift = Math.cos((idx + 516) / 15) * 9;
  const edge = ((idx * 516) % 19) - 9;
...
function smooth516(input, radius) {
  const result = [];
...
function derive516(series, mode) {
  ...
  const spread = normalize516(peak - valley);
  const jitter = normalize516(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize516(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize516(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize516(avg),
    peak: normalize516(peak),
    valley: normalize516(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch516(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize516(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic516(initialSeed = 516) {
  ...
  useEffect(() => {
    ...
    fakeFetch516(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source516.map((value, idx) => {
      ...
      return normalize516(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth516(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize516(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive516(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize516(quality) });
    dispatch({ type: 'marker', payload: normalize516(flags.confidence * 0.7 + quality * 0.3) });
