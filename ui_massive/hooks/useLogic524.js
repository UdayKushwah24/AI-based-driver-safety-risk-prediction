...
const source524 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 524) / 9) * 11;
  const drift = Math.cos((idx + 524) / 12) * 9;
  const edge = ((idx * 524) % 19) - 9;
...
function smooth524(input, radius) {
  const result = [];
...
function derive524(series, mode) {
  ...
  const spread = normalize524(peak - valley);
  const jitter = normalize524(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize524(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize524(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize524(avg),
    peak: normalize524(peak),
    valley: normalize524(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch524(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize524(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic524(initialSeed = 524) {
  ...
  useEffect(() => {
    ...
    fakeFetch524(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source524.map((value, idx) => {
      ...
      return normalize524(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth524(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize524(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive524(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize524(quality) });
    dispatch({ type: 'marker', payload: normalize524(flags.confidence * 0.7 + quality * 0.3) });
