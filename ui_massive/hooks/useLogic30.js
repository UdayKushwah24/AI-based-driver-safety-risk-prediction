...
const source30 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 30) / 5) * 11;
  const drift = Math.cos((idx + 30) / 13) * 9;
  const edge = ((idx * 30) % 19) - 9;
...
function smooth30(input, radius) {
  const result = [];
...
function derive30(series, mode) {
  ...
  const spread = normalize30(peak - valley);
  const jitter = normalize30(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize30(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize30(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize30(avg),
    peak: normalize30(peak),
    valley: normalize30(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch30(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize30(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic30(initialSeed = 30) {
  ...
  useEffect(() => {
    ...
    fakeFetch30(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source30.map((value, idx) => {
      ...
      return normalize30(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth30(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize30(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive30(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize30(quality) });
    dispatch({ type: 'marker', payload: normalize30(flags.confidence * 0.7 + quality * 0.3) });
