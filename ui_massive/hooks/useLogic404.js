...
const source404 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 404) / 8) * 11;
  const drift = Math.cos((idx + 404) / 13) * 9;
  const edge = ((idx * 404) % 19) - 9;
...
function smooth404(input, radius) {
  const result = [];
...
function derive404(series, mode) {
  ...
  const spread = normalize404(peak - valley);
  const jitter = normalize404(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize404(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize404(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize404(avg),
    peak: normalize404(peak),
    valley: normalize404(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch404(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize404(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic404(initialSeed = 404) {
  ...
  useEffect(() => {
    ...
    fakeFetch404(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source404.map((value, idx) => {
      ...
      return normalize404(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth404(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize404(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive404(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize404(quality) });
    dispatch({ type: 'marker', payload: normalize404(flags.confidence * 0.7 + quality * 0.3) });
