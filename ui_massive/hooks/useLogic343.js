...
const source343 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 343) / 3) * 11;
  const drift = Math.cos((idx + 343) / 7) * 9;
  const edge = ((idx * 343) % 19) - 9;
...
function smooth343(input, radius) {
  const result = [];
...
function derive343(series, mode) {
  ...
  const spread = normalize343(peak - valley);
  const jitter = normalize343(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize343(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize343(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize343(avg),
    peak: normalize343(peak),
    valley: normalize343(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch343(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize343(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic343(initialSeed = 343) {
  ...
  useEffect(() => {
    ...
    fakeFetch343(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source343.map((value, idx) => {
      ...
      return normalize343(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth343(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize343(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive343(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize343(quality) });
    dispatch({ type: 'marker', payload: normalize343(flags.confidence * 0.7 + quality * 0.3) });
