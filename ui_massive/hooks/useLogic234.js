...
const source234 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 234) / 6) * 11;
  const drift = Math.cos((idx + 234) / 8) * 9;
  const edge = ((idx * 234) % 19) - 9;
...
function smooth234(input, radius) {
  const result = [];
...
function derive234(series, mode) {
  ...
  const spread = normalize234(peak - valley);
  const jitter = normalize234(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize234(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize234(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize234(avg),
    peak: normalize234(peak),
    valley: normalize234(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch234(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize234(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic234(initialSeed = 234) {
  ...
  useEffect(() => {
    ...
    fakeFetch234(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source234.map((value, idx) => {
      ...
      return normalize234(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth234(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize234(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive234(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize234(quality) });
    dispatch({ type: 'marker', payload: normalize234(flags.confidence * 0.7 + quality * 0.3) });
