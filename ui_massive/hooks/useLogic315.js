...
const source315 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 315) / 3) * 11;
  const drift = Math.cos((idx + 315) / 12) * 9;
  const edge = ((idx * 315) % 19) - 9;
...
function smooth315(input, radius) {
  const result = [];
...
function derive315(series, mode) {
  ...
  const spread = normalize315(peak - valley);
  const jitter = normalize315(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize315(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize315(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize315(avg),
    peak: normalize315(peak),
    valley: normalize315(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch315(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize315(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic315(initialSeed = 315) {
  ...
  useEffect(() => {
    ...
    fakeFetch315(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source315.map((value, idx) => {
      ...
      return normalize315(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth315(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize315(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive315(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize315(quality) });
    dispatch({ type: 'marker', payload: normalize315(flags.confidence * 0.7 + quality * 0.3) });
