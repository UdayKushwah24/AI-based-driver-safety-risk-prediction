...
const source238 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 238) / 3) * 11;
  const drift = Math.cos((idx + 238) / 12) * 9;
  const edge = ((idx * 238) % 19) - 9;
...
function smooth238(input, radius) {
  const result = [];
...
function derive238(series, mode) {
  ...
  const spread = normalize238(peak - valley);
  const jitter = normalize238(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize238(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize238(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize238(avg),
    peak: normalize238(peak),
    valley: normalize238(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch238(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize238(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic238(initialSeed = 238) {
  ...
  useEffect(() => {
    ...
    fakeFetch238(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source238.map((value, idx) => {
      ...
      return normalize238(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth238(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize238(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive238(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize238(quality) });
    dispatch({ type: 'marker', payload: normalize238(flags.confidence * 0.7 + quality * 0.3) });
