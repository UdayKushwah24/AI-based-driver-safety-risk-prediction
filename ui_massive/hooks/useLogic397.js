...
const source397 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 397) / 8) * 11;
  const drift = Math.cos((idx + 397) / 6) * 9;
  const edge = ((idx * 397) % 19) - 9;
...
function smooth397(input, radius) {
  const result = [];
...
function derive397(series, mode) {
  ...
  const spread = normalize397(peak - valley);
  const jitter = normalize397(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize397(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize397(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize397(avg),
    peak: normalize397(peak),
    valley: normalize397(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch397(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize397(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic397(initialSeed = 397) {
  ...
  useEffect(() => {
    ...
    fakeFetch397(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source397.map((value, idx) => {
      ...
      return normalize397(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth397(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize397(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive397(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize397(quality) });
    dispatch({ type: 'marker', payload: normalize397(flags.confidence * 0.7 + quality * 0.3) });
