...
const source8 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 8) / 4) * 11;
  const drift = Math.cos((idx + 8) / 13) * 9;
  const edge = ((idx * 8) % 19) - 9;
...
function derive8(series, mode) {
  ...
  const spread = normalize8(peak - valley);
  const jitter = normalize8(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize8(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize8(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize8(avg),
    peak: normalize8(peak),
    valley: normalize8(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch8(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize8(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic8(initialSeed = 8) {
  ...
  useEffect(() => {
    ...
    fakeFetch8(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source8.map((value, idx) => {
      ...
      return normalize8(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth8(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize8(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive8(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize8(quality) });
    dispatch({ type: 'marker', payload: normalize8(flags.confidence * 0.7 + quality * 0.3) });
