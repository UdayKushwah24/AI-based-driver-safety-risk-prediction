...
const source107 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 107) / 5) * 11;
  const drift = Math.cos((idx + 107) / 13) * 9;
  const edge = ((idx * 107) % 19) - 9;
...
function smooth107(input, radius) {
  const result = [];
...
function derive107(series, mode) {
  ...
  const spread = normalize107(peak - valley);
  const jitter = normalize107(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize107(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize107(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize107(avg),
    peak: normalize107(peak),
    valley: normalize107(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch107(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize107(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic107(initialSeed = 107) {
  ...
  useEffect(() => {
    ...
    fakeFetch107(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source107.map((value, idx) => {
      ...
      return normalize107(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth107(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize107(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive107(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize107(quality) });
    dispatch({ type: 'marker', payload: normalize107(flags.confidence * 0.7 + quality * 0.3) });
