...
const source135 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 135) / 5) * 11;
  const drift = Math.cos((idx + 135) / 8) * 9;
  const edge = ((idx * 135) % 19) - 9;
...
function smooth135(input, radius) {
  const result = [];
...
function derive135(series, mode) {
  ...
  const spread = normalize135(peak - valley);
  const jitter = normalize135(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize135(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize135(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize135(avg),
    peak: normalize135(peak),
    valley: normalize135(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch135(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize135(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic135(initialSeed = 135) {
  ...
  useEffect(() => {
    ...
    fakeFetch135(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source135.map((value, idx) => {
      ...
      return normalize135(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth135(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize135(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive135(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize135(quality) });
    dispatch({ type: 'marker', payload: normalize135(flags.confidence * 0.7 + quality * 0.3) });
