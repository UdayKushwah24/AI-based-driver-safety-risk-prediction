...
const source94 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 94) / 6) * 11;
  const drift = Math.cos((idx + 94) / 11) * 9;
  const edge = ((idx * 94) % 19) - 9;
...
function smooth94(input, radius) {
  const result = [];
...
function derive94(series, mode) {
  ...
  const spread = normalize94(peak - valley);
  const jitter = normalize94(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize94(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize94(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize94(avg),
    peak: normalize94(peak),
    valley: normalize94(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch94(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize94(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic94(initialSeed = 94) {
  ...
  useEffect(() => {
    ...
    fakeFetch94(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source94.map((value, idx) => {
      ...
      return normalize94(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth94(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize94(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive94(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize94(quality) });
    dispatch({ type: 'marker', payload: normalize94(flags.confidence * 0.7 + quality * 0.3) });
