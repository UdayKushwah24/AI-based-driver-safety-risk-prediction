...
const source157 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 157) / 6) * 11;
  const drift = Math.cos((idx + 157) / 8) * 9;
  const edge = ((idx * 157) % 19) - 9;
...
function smooth157(input, radius) {
  const result = [];
...
function derive157(series, mode) {
  ...
  const spread = normalize157(peak - valley);
  const jitter = normalize157(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize157(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize157(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize157(avg),
    peak: normalize157(peak),
    valley: normalize157(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch157(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize157(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic157(initialSeed = 157) {
  ...
  useEffect(() => {
    ...
    fakeFetch157(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source157.map((value, idx) => {
      ...
      return normalize157(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth157(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize157(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive157(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize157(quality) });
    dispatch({ type: 'marker', payload: normalize157(flags.confidence * 0.7 + quality * 0.3) });
