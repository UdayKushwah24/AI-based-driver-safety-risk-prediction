...
const source389 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 389) / 7) * 11;
  const drift = Math.cos((idx + 389) / 9) * 9;
  const edge = ((idx * 389) % 19) - 9;
...
function smooth389(input, radius) {
  const result = [];
...
function derive389(series, mode) {
  ...
  const spread = normalize389(peak - valley);
  const jitter = normalize389(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize389(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize389(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize389(avg),
    peak: normalize389(peak),
    valley: normalize389(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch389(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize389(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic389(initialSeed = 389) {
  ...
  useEffect(() => {
    ...
    fakeFetch389(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source389.map((value, idx) => {
      ...
      return normalize389(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth389(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize389(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive389(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize389(quality) });
    dispatch({ type: 'marker', payload: normalize389(flags.confidence * 0.7 + quality * 0.3) });
