...
const source398 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 398) / 9) * 11;
  const drift = Math.cos((idx + 398) / 7) * 9;
  const edge = ((idx * 398) % 19) - 9;
...
function smooth398(input, radius) {
  const result = [];
...
function derive398(series, mode) {
  ...
  const spread = normalize398(peak - valley);
  const jitter = normalize398(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize398(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize398(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize398(avg),
    peak: normalize398(peak),
    valley: normalize398(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch398(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize398(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic398(initialSeed = 398) {
  ...
  useEffect(() => {
    ...
    fakeFetch398(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source398.map((value, idx) => {
      ...
      return normalize398(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth398(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize398(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive398(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize398(quality) });
    dispatch({ type: 'marker', payload: normalize398(flags.confidence * 0.7 + quality * 0.3) });
