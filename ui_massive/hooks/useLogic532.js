...
const source532 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 532) / 3) * 11;
  const drift = Math.cos((idx + 532) / 9) * 9;
  const edge = ((idx * 532) % 19) - 9;
...
function smooth532(input, radius) {
  const result = [];
...
function derive532(series, mode) {
  ...
  const spread = normalize532(peak - valley);
  const jitter = normalize532(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize532(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize532(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize532(avg),
    peak: normalize532(peak),
    valley: normalize532(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch532(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize532(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic532(initialSeed = 532) {
  ...
  useEffect(() => {
    ...
    fakeFetch532(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source532.map((value, idx) => {
      ...
      return normalize532(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth532(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize532(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive532(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize532(quality) });
    dispatch({ type: 'marker', payload: normalize532(flags.confidence * 0.7 + quality * 0.3) });
