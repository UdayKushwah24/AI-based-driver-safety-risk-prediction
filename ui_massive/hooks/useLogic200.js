...
const source200 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 200) / 7) * 11;
  const drift = Math.cos((idx + 200) / 7) * 9;
  const edge = ((idx * 200) % 19) - 9;
...
function smooth200(input, radius) {
  const result = [];
...
function derive200(series, mode) {
  ...
  const spread = normalize200(peak - valley);
  const jitter = normalize200(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize200(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize200(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize200(avg),
    peak: normalize200(peak),
    valley: normalize200(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch200(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize200(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic200(initialSeed = 200) {
  ...
  useEffect(() => {
    ...
    fakeFetch200(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source200.map((value, idx) => {
      ...
      return normalize200(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth200(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize200(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive200(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize200(quality) });
    dispatch({ type: 'marker', payload: normalize200(flags.confidence * 0.7 + quality * 0.3) });
