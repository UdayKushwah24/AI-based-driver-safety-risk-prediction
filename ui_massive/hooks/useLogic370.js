...
const source370 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 370) / 9) * 11;
  const drift = Math.cos((idx + 370) / 12) * 9;
  const edge = ((idx * 370) % 19) - 9;
...
function smooth370(input, radius) {
  const result = [];
...
function derive370(series, mode) {
  ...
  const spread = normalize370(peak - valley);
  const jitter = normalize370(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize370(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize370(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize370(avg),
    peak: normalize370(peak),
    valley: normalize370(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch370(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize370(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic370(initialSeed = 370) {
  ...
  useEffect(() => {
    ...
    fakeFetch370(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source370.map((value, idx) => {
      ...
      return normalize370(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth370(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize370(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive370(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize370(quality) });
    dispatch({ type: 'marker', payload: normalize370(flags.confidence * 0.7 + quality * 0.3) });
