...
const source158 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 158) / 7) * 11;
  const drift = Math.cos((idx + 158) / 9) * 9;
  const edge = ((idx * 158) % 19) - 9;
...
function smooth158(input, radius) {
  const result = [];
...
function derive158(series, mode) {
  ...
  const spread = normalize158(peak - valley);
  const jitter = normalize158(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize158(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize158(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize158(avg),
    peak: normalize158(peak),
    valley: normalize158(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch158(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize158(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic158(initialSeed = 158) {
  ...
  useEffect(() => {
    ...
    fakeFetch158(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source158.map((value, idx) => {
      ...
      return normalize158(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth158(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize158(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive158(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize158(quality) });
    dispatch({ type: 'marker', payload: normalize158(flags.confidence * 0.7 + quality * 0.3) });
