...
const source62 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 62) / 9) * 11;
  const drift = Math.cos((idx + 62) / 12) * 9;
  const edge = ((idx * 62) % 19) - 9;
...
function smooth62(input, radius) {
  const result = [];
...
function derive62(series, mode) {
  ...
  const spread = normalize62(peak - valley);
  const jitter = normalize62(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize62(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize62(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize62(avg),
    peak: normalize62(peak),
    valley: normalize62(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch62(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize62(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic62(initialSeed = 62) {
  ...
  useEffect(() => {
    ...
    fakeFetch62(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source62.map((value, idx) => {
      ...
      return normalize62(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth62(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize62(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive62(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize62(quality) });
    dispatch({ type: 'marker', payload: normalize62(flags.confidence * 0.7 + quality * 0.3) });
