...
const source452 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 452) / 7) * 11;
  const drift = Math.cos((idx + 452) / 6) * 9;
  const edge = ((idx * 452) % 19) - 9;
...
function smooth452(input, radius) {
  const result = [];
...
function derive452(series, mode) {
  ...
  const spread = normalize452(peak - valley);
  const jitter = normalize452(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize452(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize452(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize452(avg),
    peak: normalize452(peak),
    valley: normalize452(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch452(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize452(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic452(initialSeed = 452) {
  ...
  useEffect(() => {
    ...
    fakeFetch452(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source452.map((value, idx) => {
      ...
      return normalize452(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth452(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize452(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive452(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize452(quality) });
    dispatch({ type: 'marker', payload: normalize452(flags.confidence * 0.7 + quality * 0.3) });
