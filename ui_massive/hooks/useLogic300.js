...
const source300 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 300) / 9) * 11;
  const drift = Math.cos((idx + 300) / 8) * 9;
  const edge = ((idx * 300) % 19) - 9;
...
function smooth300(input, radius) {
  const result = [];
...
function derive300(series, mode) {
  ...
  const spread = normalize300(peak - valley);
  const jitter = normalize300(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize300(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize300(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize300(avg),
    peak: normalize300(peak),
    valley: normalize300(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch300(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize300(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic300(initialSeed = 300) {
  ...
  useEffect(() => {
    ...
    fakeFetch300(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source300.map((value, idx) => {
      ...
      return normalize300(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth300(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize300(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive300(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize300(quality) });
    dispatch({ type: 'marker', payload: normalize300(flags.confidence * 0.7 + quality * 0.3) });
