...
const source476 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 476) / 3) * 11;
  const drift = Math.cos((idx + 476) / 8) * 9;
  const edge = ((idx * 476) % 19) - 9;
...
function smooth476(input, radius) {
  const result = [];
...
function derive476(series, mode) {
  ...
  const spread = normalize476(peak - valley);
  const jitter = normalize476(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize476(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize476(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize476(avg),
    peak: normalize476(peak),
    valley: normalize476(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch476(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize476(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic476(initialSeed = 476) {
  ...
  useEffect(() => {
    ...
    fakeFetch476(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source476.map((value, idx) => {
      ...
      return normalize476(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth476(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize476(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive476(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize476(quality) });
    dispatch({ type: 'marker', payload: normalize476(flags.confidence * 0.7 + quality * 0.3) });
