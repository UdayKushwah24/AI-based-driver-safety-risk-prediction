...
const source3 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 3) / 6) * 11;
  const drift = Math.cos((idx + 3) / 8) * 9;
  const edge = ((idx * 3) % 19) - 9;
...
function derive3(series, mode) {
  ...
  const spread = normalize3(peak - valley);
  const jitter = normalize3(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize3(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize3(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize3(avg),
    peak: normalize3(peak),
    valley: normalize3(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch3(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize3(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic3(initialSeed = 3) {
  ...
  useEffect(() => {
    ...
    fakeFetch3(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source3.map((value, idx) => {
      ...
      return normalize3(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth3(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize3(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive3(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize3(quality) });
    dispatch({ type: 'marker', payload: normalize3(flags.confidence * 0.7 + quality * 0.3) });
