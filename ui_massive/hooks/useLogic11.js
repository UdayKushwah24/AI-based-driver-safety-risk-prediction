...
const source11 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 11) / 7) * 11;
  const drift = Math.cos((idx + 11) / 5) * 9;
  const edge = ((idx * 11) % 19) - 9;
...
function smooth11(input, radius) {
  const result = [];
...
function derive11(series, mode) {
  ...
  const spread = normalize11(peak - valley);
  const jitter = normalize11(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize11(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize11(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize11(avg),
    peak: normalize11(peak),
    valley: normalize11(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch11(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize11(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic11(initialSeed = 11) {
  ...
  useEffect(() => {
    ...
    fakeFetch11(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source11.map((value, idx) => {
      ...
      return normalize11(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth11(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize11(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive11(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize11(quality) });
    dispatch({ type: 'marker', payload: normalize11(flags.confidence * 0.7 + quality * 0.3) });
