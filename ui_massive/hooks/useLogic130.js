...
const source130 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 130) / 7) * 11;
  const drift = Math.cos((idx + 130) / 14) * 9;
  const edge = ((idx * 130) % 19) - 9;
...
function smooth130(input, radius) {
  const result = [];
...
function derive130(series, mode) {
  ...
  const spread = normalize130(peak - valley);
  const jitter = normalize130(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize130(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize130(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize130(avg),
    peak: normalize130(peak),
    valley: normalize130(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch130(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize130(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic130(initialSeed = 130) {
  ...
  useEffect(() => {
    ...
    fakeFetch130(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source130.map((value, idx) => {
      ...
      return normalize130(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth130(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize130(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive130(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize130(quality) });
    dispatch({ type: 'marker', payload: normalize130(flags.confidence * 0.7 + quality * 0.3) });
