...
const source284 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 284) / 7) * 11;
  const drift = Math.cos((idx + 284) / 14) * 9;
  const edge = ((idx * 284) % 19) - 9;
...
function smooth284(input, radius) {
  const result = [];
...
function derive284(series, mode) {
  ...
  const spread = normalize284(peak - valley);
  const jitter = normalize284(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize284(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize284(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize284(avg),
    peak: normalize284(peak),
    valley: normalize284(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch284(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize284(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic284(initialSeed = 284) {
  ...
  useEffect(() => {
    ...
    fakeFetch284(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source284.map((value, idx) => {
      ...
      return normalize284(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth284(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize284(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive284(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize284(quality) });
    dispatch({ type: 'marker', payload: normalize284(flags.confidence * 0.7 + quality * 0.3) });
