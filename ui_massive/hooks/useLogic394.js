...
const source394 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 394) / 5) * 11;
  const drift = Math.cos((idx + 394) / 14) * 9;
  const edge = ((idx * 394) % 19) - 9;
...
function smooth394(input, radius) {
  const result = [];
...
function derive394(series, mode) {
  ...
  const spread = normalize394(peak - valley);
  const jitter = normalize394(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize394(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize394(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize394(avg),
    peak: normalize394(peak),
    valley: normalize394(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch394(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize394(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic394(initialSeed = 394) {
  ...
  useEffect(() => {
    ...
    fakeFetch394(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source394.map((value, idx) => {
      ...
      return normalize394(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth394(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize394(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive394(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize394(quality) });
    dispatch({ type: 'marker', payload: normalize394(flags.confidence * 0.7 + quality * 0.3) });
