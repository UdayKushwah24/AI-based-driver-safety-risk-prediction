...
const source100 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 100) / 5) * 11;
  const drift = Math.cos((idx + 100) / 6) * 9;
  const edge = ((idx * 100) % 19) - 9;
...
function smooth100(input, radius) {
  const result = [];
...
function derive100(series, mode) {
  ...
  const spread = normalize100(peak - valley);
  const jitter = normalize100(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize100(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize100(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize100(avg),
    peak: normalize100(peak),
    valley: normalize100(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch100(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize100(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic100(initialSeed = 100) {
  ...
  useEffect(() => {
    ...
    fakeFetch100(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source100.map((value, idx) => {
      ...
      return normalize100(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth100(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize100(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive100(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize100(quality) });
    dispatch({ type: 'marker', payload: normalize100(flags.confidence * 0.7 + quality * 0.3) });
