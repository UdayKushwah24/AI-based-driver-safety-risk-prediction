...
const source272 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 272) / 9) * 11;
  const drift = Math.cos((idx + 272) / 13) * 9;
  const edge = ((idx * 272) % 19) - 9;
...
function smooth272(input, radius) {
  const result = [];
...
function derive272(series, mode) {
  ...
  const spread = normalize272(peak - valley);
  const jitter = normalize272(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize272(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize272(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize272(avg),
    peak: normalize272(peak),
    valley: normalize272(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch272(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize272(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic272(initialSeed = 272) {
  ...
  useEffect(() => {
    ...
    fakeFetch272(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source272.map((value, idx) => {
      ...
      return normalize272(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth272(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize272(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive272(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize272(quality) });
    dispatch({ type: 'marker', payload: normalize272(flags.confidence * 0.7 + quality * 0.3) });
