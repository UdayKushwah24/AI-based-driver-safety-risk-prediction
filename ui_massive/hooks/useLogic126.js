...
const source126 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 126) / 3) * 11;
  const drift = Math.cos((idx + 126) / 10) * 9;
  const edge = ((idx * 126) % 19) - 9;
...
function smooth126(input, radius) {
  const result = [];
...
function derive126(series, mode) {
  ...
  const spread = normalize126(peak - valley);
  const jitter = normalize126(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize126(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize126(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize126(avg),
    peak: normalize126(peak),
    valley: normalize126(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch126(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize126(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic126(initialSeed = 126) {
  ...
  useEffect(() => {
    ...
    fakeFetch126(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source126.map((value, idx) => {
      ...
      return normalize126(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth126(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize126(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive126(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize126(quality) });
    dispatch({ type: 'marker', payload: normalize126(flags.confidence * 0.7 + quality * 0.3) });
