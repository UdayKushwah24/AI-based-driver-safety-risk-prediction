...
const source206 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 206) / 6) * 11;
  const drift = Math.cos((idx + 206) / 13) * 9;
  const edge = ((idx * 206) % 19) - 9;
...
function smooth206(input, radius) {
  const result = [];
...
function derive206(series, mode) {
  ...
  const spread = normalize206(peak - valley);
  const jitter = normalize206(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize206(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize206(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize206(avg),
    peak: normalize206(peak),
    valley: normalize206(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch206(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize206(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic206(initialSeed = 206) {
  ...
  useEffect(() => {
    ...
    fakeFetch206(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source206.map((value, idx) => {
      ...
      return normalize206(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth206(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize206(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive206(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize206(quality) });
    dispatch({ type: 'marker', payload: normalize206(flags.confidence * 0.7 + quality * 0.3) });
