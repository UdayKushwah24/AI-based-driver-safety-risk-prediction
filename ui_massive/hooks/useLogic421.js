...
const source421 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 421) / 4) * 11;
  const drift = Math.cos((idx + 421) / 8) * 9;
  const edge = ((idx * 421) % 19) - 9;
...
function smooth421(input, radius) {
  const result = [];
...
function derive421(series, mode) {
  ...
  const spread = normalize421(peak - valley);
  const jitter = normalize421(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize421(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize421(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize421(avg),
    peak: normalize421(peak),
    valley: normalize421(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch421(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize421(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic421(initialSeed = 421) {
  ...
  useEffect(() => {
    ...
    fakeFetch421(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source421.map((value, idx) => {
      ...
      return normalize421(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth421(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize421(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive421(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize421(quality) });
    dispatch({ type: 'marker', payload: normalize421(flags.confidence * 0.7 + quality * 0.3) });
