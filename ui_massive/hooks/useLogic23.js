...
const source23 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 23) / 5) * 11;
  const drift = Math.cos((idx + 23) / 6) * 9;
  const edge = ((idx * 23) % 19) - 9;
...
function smooth23(input, radius) {
  const result = [];
...
function derive23(series, mode) {
  ...
  const spread = normalize23(peak - valley);
  const jitter = normalize23(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize23(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize23(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize23(avg),
    peak: normalize23(peak),
    valley: normalize23(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch23(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize23(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic23(initialSeed = 23) {
  ...
  useEffect(() => {
    ...
    fakeFetch23(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source23.map((value, idx) => {
      ...
      return normalize23(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth23(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize23(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive23(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize23(quality) });
    dispatch({ type: 'marker', payload: normalize23(flags.confidence * 0.7 + quality * 0.3) });
