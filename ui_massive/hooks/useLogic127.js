...
const source127 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 127) / 4) * 11;
  const drift = Math.cos((idx + 127) / 11) * 9;
  const edge = ((idx * 127) % 19) - 9;
...
function smooth127(input, radius) {
  const result = [];
...
function derive127(series, mode) {
  ...
  const spread = normalize127(peak - valley);
  const jitter = normalize127(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize127(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize127(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize127(avg),
    peak: normalize127(peak),
    valley: normalize127(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch127(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize127(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic127(initialSeed = 127) {
  ...
  useEffect(() => {
    ...
    fakeFetch127(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source127.map((value, idx) => {
      ...
      return normalize127(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth127(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize127(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive127(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize127(quality) });
    dispatch({ type: 'marker', payload: normalize127(flags.confidence * 0.7 + quality * 0.3) });
