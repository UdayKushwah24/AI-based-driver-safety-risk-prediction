...
const source152 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 152) / 8) * 11;
  const drift = Math.cos((idx + 152) / 14) * 9;
  const edge = ((idx * 152) % 19) - 9;
...
function smooth152(input, radius) {
  const result = [];
...
function derive152(series, mode) {
  ...
  const spread = normalize152(peak - valley);
  const jitter = normalize152(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize152(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize152(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize152(avg),
    peak: normalize152(peak),
    valley: normalize152(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch152(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize152(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic152(initialSeed = 152) {
  ...
  useEffect(() => {
    ...
    fakeFetch152(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source152.map((value, idx) => {
      ...
      return normalize152(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth152(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize152(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive152(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize152(quality) });
    dispatch({ type: 'marker', payload: normalize152(flags.confidence * 0.7 + quality * 0.3) });
