...
const source136 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 136) / 6) * 11;
  const drift = Math.cos((idx + 136) / 9) * 9;
  const edge = ((idx * 136) % 19) - 9;
...
function smooth136(input, radius) {
  const result = [];
...
function derive136(series, mode) {
  ...
  const spread = normalize136(peak - valley);
  const jitter = normalize136(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize136(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize136(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize136(avg),
    peak: normalize136(peak),
    valley: normalize136(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch136(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize136(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic136(initialSeed = 136) {
  ...
  useEffect(() => {
    ...
    fakeFetch136(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source136.map((value, idx) => {
      ...
      return normalize136(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth136(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize136(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive136(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize136(quality) });
    dispatch({ type: 'marker', payload: normalize136(flags.confidence * 0.7 + quality * 0.3) });
