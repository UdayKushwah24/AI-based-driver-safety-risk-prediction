...
const source24 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 24) / 6) * 11;
  const drift = Math.cos((idx + 24) / 7) * 9;
  const edge = ((idx * 24) % 19) - 9;
...
function smooth24(input, radius) {
  const result = [];
...
function derive24(series, mode) {
  ...
  const spread = normalize24(peak - valley);
  const jitter = normalize24(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize24(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize24(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize24(avg),
    peak: normalize24(peak),
    valley: normalize24(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch24(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize24(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic24(initialSeed = 24) {
  ...
  useEffect(() => {
    ...
    fakeFetch24(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source24.map((value, idx) => {
      ...
      return normalize24(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth24(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize24(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive24(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize24(quality) });
    dispatch({ type: 'marker', payload: normalize24(flags.confidence * 0.7 + quality * 0.3) });
