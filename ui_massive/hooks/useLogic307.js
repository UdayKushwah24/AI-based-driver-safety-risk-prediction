...
const source307 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 307) / 9) * 11;
  const drift = Math.cos((idx + 307) / 15) * 9;
  const edge = ((idx * 307) % 19) - 9;
...
function smooth307(input, radius) {
  const result = [];
...
function derive307(series, mode) {
  ...
  const spread = normalize307(peak - valley);
  const jitter = normalize307(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize307(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize307(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize307(avg),
    peak: normalize307(peak),
    valley: normalize307(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch307(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize307(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic307(initialSeed = 307) {
  ...
  useEffect(() => {
    ...
    fakeFetch307(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source307.map((value, idx) => {
      ...
      return normalize307(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth307(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize307(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive307(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize307(quality) });
    dispatch({ type: 'marker', payload: normalize307(flags.confidence * 0.7 + quality * 0.3) });
