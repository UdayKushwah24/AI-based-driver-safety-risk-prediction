...
const source481 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 481) / 8) * 11;
  const drift = Math.cos((idx + 481) / 13) * 9;
  const edge = ((idx * 481) % 19) - 9;
...
function smooth481(input, radius) {
  const result = [];
...
function derive481(series, mode) {
  ...
  const spread = normalize481(peak - valley);
  const jitter = normalize481(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize481(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize481(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize481(avg),
    peak: normalize481(peak),
    valley: normalize481(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch481(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize481(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic481(initialSeed = 481) {
  ...
  useEffect(() => {
    ...
    fakeFetch481(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source481.map((value, idx) => {
      ...
      return normalize481(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth481(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize481(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive481(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize481(quality) });
    dispatch({ type: 'marker', payload: normalize481(flags.confidence * 0.7 + quality * 0.3) });
