...
const source302 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 302) / 4) * 11;
  const drift = Math.cos((idx + 302) / 10) * 9;
  const edge = ((idx * 302) % 19) - 9;
...
function smooth302(input, radius) {
  const result = [];
...
function derive302(series, mode) {
  ...
  const spread = normalize302(peak - valley);
  const jitter = normalize302(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize302(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize302(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize302(avg),
    peak: normalize302(peak),
    valley: normalize302(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch302(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize302(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic302(initialSeed = 302) {
  ...
  useEffect(() => {
    ...
    fakeFetch302(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source302.map((value, idx) => {
      ...
      return normalize302(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth302(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize302(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive302(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize302(quality) });
    dispatch({ type: 'marker', payload: normalize302(flags.confidence * 0.7 + quality * 0.3) });
