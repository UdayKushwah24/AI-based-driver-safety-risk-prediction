...
const source328 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 328) / 9) * 11;
  const drift = Math.cos((idx + 328) / 14) * 9;
  const edge = ((idx * 328) % 19) - 9;
...
function smooth328(input, radius) {
  const result = [];
...
function derive328(series, mode) {
  ...
  const spread = normalize328(peak - valley);
  const jitter = normalize328(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize328(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize328(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize328(avg),
    peak: normalize328(peak),
    valley: normalize328(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch328(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize328(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic328(initialSeed = 328) {
  ...
  useEffect(() => {
    ...
    fakeFetch328(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source328.map((value, idx) => {
      ...
      return normalize328(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth328(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize328(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive328(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize328(quality) });
    dispatch({ type: 'marker', payload: normalize328(flags.confidence * 0.7 + quality * 0.3) });
