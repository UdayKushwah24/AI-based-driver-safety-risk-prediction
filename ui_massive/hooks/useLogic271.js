...
const source271 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 271) / 8) * 11;
  const drift = Math.cos((idx + 271) / 12) * 9;
  const edge = ((idx * 271) % 19) - 9;
...
function smooth271(input, radius) {
  const result = [];
...
function derive271(series, mode) {
  ...
  const spread = normalize271(peak - valley);
  const jitter = normalize271(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize271(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize271(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize271(avg),
    peak: normalize271(peak),
    valley: normalize271(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch271(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize271(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic271(initialSeed = 271) {
  ...
  useEffect(() => {
    ...
    fakeFetch271(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source271.map((value, idx) => {
      ...
      return normalize271(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth271(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize271(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive271(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize271(quality) });
    dispatch({ type: 'marker', payload: normalize271(flags.confidence * 0.7 + quality * 0.3) });
