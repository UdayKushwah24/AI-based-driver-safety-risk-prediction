...
const source478 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 478) / 5) * 11;
  const drift = Math.cos((idx + 478) / 10) * 9;
  const edge = ((idx * 478) % 19) - 9;
...
function smooth478(input, radius) {
  const result = [];
...
function derive478(series, mode) {
  ...
  const spread = normalize478(peak - valley);
  const jitter = normalize478(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize478(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize478(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize478(avg),
    peak: normalize478(peak),
    valley: normalize478(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch478(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize478(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic478(initialSeed = 478) {
  ...
  useEffect(() => {
    ...
    fakeFetch478(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source478.map((value, idx) => {
      ...
      return normalize478(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth478(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize478(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive478(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize478(quality) });
    dispatch({ type: 'marker', payload: normalize478(flags.confidence * 0.7 + quality * 0.3) });
