...
const source432 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 432) / 8) * 11;
  const drift = Math.cos((idx + 432) / 8) * 9;
  const edge = ((idx * 432) % 19) - 9;
...
function smooth432(input, radius) {
  const result = [];
...
function derive432(series, mode) {
  ...
  const spread = normalize432(peak - valley);
  const jitter = normalize432(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize432(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize432(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize432(avg),
    peak: normalize432(peak),
    valley: normalize432(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch432(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize432(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic432(initialSeed = 432) {
  ...
  useEffect(() => {
    ...
    fakeFetch432(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source432.map((value, idx) => {
      ...
      return normalize432(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth432(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize432(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive432(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize432(quality) });
    dispatch({ type: 'marker', payload: normalize432(flags.confidence * 0.7 + quality * 0.3) });
