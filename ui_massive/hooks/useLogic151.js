...
const source151 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 151) / 7) * 11;
  const drift = Math.cos((idx + 151) / 13) * 9;
  const edge = ((idx * 151) % 19) - 9;
...
function smooth151(input, radius) {
  const result = [];
...
function derive151(series, mode) {
  ...
  const spread = normalize151(peak - valley);
  const jitter = normalize151(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize151(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize151(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize151(avg),
    peak: normalize151(peak),
    valley: normalize151(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch151(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize151(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic151(initialSeed = 151) {
  ...
  useEffect(() => {
    ...
    fakeFetch151(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source151.map((value, idx) => {
      ...
      return normalize151(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth151(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize151(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive151(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize151(quality) });
    dispatch({ type: 'marker', payload: normalize151(flags.confidence * 0.7 + quality * 0.3) });
