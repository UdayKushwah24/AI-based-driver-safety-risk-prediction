...
const source470 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 470) / 4) * 11;
  const drift = Math.cos((idx + 470) / 13) * 9;
  const edge = ((idx * 470) % 19) - 9;
...
function smooth470(input, radius) {
  const result = [];
...
function derive470(series, mode) {
  ...
  const spread = normalize470(peak - valley);
  const jitter = normalize470(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize470(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize470(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize470(avg),
    peak: normalize470(peak),
    valley: normalize470(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch470(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize470(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic470(initialSeed = 470) {
  ...
  useEffect(() => {
    ...
    fakeFetch470(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source470.map((value, idx) => {
      ...
      return normalize470(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth470(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize470(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive470(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize470(quality) });
    dispatch({ type: 'marker', payload: normalize470(flags.confidence * 0.7 + quality * 0.3) });
