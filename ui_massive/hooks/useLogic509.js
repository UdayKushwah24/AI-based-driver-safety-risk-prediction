...
const source509 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 509) / 8) * 11;
  const drift = Math.cos((idx + 509) / 8) * 9;
  const edge = ((idx * 509) % 19) - 9;
...
function smooth509(input, radius) {
  const result = [];
...
function derive509(series, mode) {
  ...
  const spread = normalize509(peak - valley);
  const jitter = normalize509(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize509(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize509(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize509(avg),
    peak: normalize509(peak),
    valley: normalize509(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch509(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize509(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic509(initialSeed = 509) {
  ...
  useEffect(() => {
    ...
    fakeFetch509(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source509.map((value, idx) => {
      ...
      return normalize509(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth509(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize509(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive509(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize509(quality) });
    dispatch({ type: 'marker', payload: normalize509(flags.confidence * 0.7 + quality * 0.3) });
