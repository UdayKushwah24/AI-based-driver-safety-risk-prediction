...
const source360 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 360) / 6) * 11;
  const drift = Math.cos((idx + 360) / 13) * 9;
  const edge = ((idx * 360) % 19) - 9;
  return Number((48 + wave + drift + edge).toFixed(4));
});

const initialState360 = {
  mode: 'balanced',
  granularity: 6,
  tick: 0,
  status: 'idle',
...
function reducer360(state, action) {
...
function smooth360(input, radius) {
  const result = [];
...
function derive360(series, mode) {
  ...
  const spread = normalize360(peak - valley);
  const jitter = normalize360(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize360(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize360(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize360(avg),
    peak: normalize360(peak),
    valley: normalize360(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch360(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize360(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic360(initialSeed = 360) {
  const [seed, setSeed] = useState(initialSeed);
  const [state, dispatch] = useReducer(reducer360, initialState360);

  useEffect(() => {
    let live = true;
    dispatch({ type: 'status', payload: 'loading' });
    fakeFetch360(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source360.map((value, idx) => {
      ...
      return normalize360(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth360(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize360(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive360(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize360(quality) });
    dispatch({ type: 'marker', payload: normalize360(flags.confidence * 0.7 + quality * 0.3) });
