...
const source298 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 298) / 7) * 11;
  const drift = Math.cos((idx + 298) / 6) * 9;
  const edge = ((idx * 298) % 19) - 9;
...
function smooth298(input, radius) {
  const result = [];
...
function derive298(series, mode) {
  ...
  const spread = normalize298(peak - valley);
  const jitter = normalize298(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize298(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize298(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize298(avg),
    peak: normalize298(peak),
    valley: normalize298(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch298(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize298(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic298(initialSeed = 298) {
  ...
  useEffect(() => {
    ...
    fakeFetch298(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source298.map((value, idx) => {
      ...
      return normalize298(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth298(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize298(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive298(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize298(quality) });
    dispatch({ type: 'marker', payload: normalize298(flags.confidence * 0.7 + quality * 0.3) });
