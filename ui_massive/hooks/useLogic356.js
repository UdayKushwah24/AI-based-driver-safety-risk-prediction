...
const source356 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 356) / 9) * 11;
  const drift = Math.cos((idx + 356) / 9) * 9;
  const edge = ((idx * 356) % 19) - 9;
...
function smooth356(input, radius) {
  const result = [];
...
function derive356(series, mode) {
  ...
  const spread = normalize356(peak - valley);
  const jitter = normalize356(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize356(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize356(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize356(avg),
    peak: normalize356(peak),
    valley: normalize356(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch356(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize356(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic356(initialSeed = 356) {
  ...
  useEffect(() => {
    ...
    fakeFetch356(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source356.map((value, idx) => {
      ...
      return normalize356(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth356(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize356(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive356(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize356(quality) });
    dispatch({ type: 'marker', payload: normalize356(flags.confidence * 0.7 + quality * 0.3) });
