...
const source203 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 203) / 3) * 11;
  const drift = Math.cos((idx + 203) / 10) * 9;
  const edge = ((idx * 203) % 19) - 9;
...
function smooth203(input, radius) {
  const result = [];
...
function derive203(series, mode) {
  ...
  const spread = normalize203(peak - valley);
  const jitter = normalize203(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize203(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize203(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize203(avg),
    peak: normalize203(peak),
    valley: normalize203(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch203(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize203(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic203(initialSeed = 203) {
  ...
  useEffect(() => {
    ...
    fakeFetch203(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source203.map((value, idx) => {
      ...
      return normalize203(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth203(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize203(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive203(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize203(quality) });
    dispatch({ type: 'marker', payload: normalize203(flags.confidence * 0.7 + quality * 0.3) });
