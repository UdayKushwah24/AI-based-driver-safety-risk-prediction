...
const source546 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 546) / 3) * 11;
  const drift = Math.cos((idx + 546) / 12) * 9;
  const edge = ((idx * 546) % 19) - 9;
...
function smooth546(input, radius) {
  const result = [];
...
function derive546(series, mode) {
  ...
  const spread = normalize546(peak - valley);
  const jitter = normalize546(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize546(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize546(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize546(avg),
    peak: normalize546(peak),
    valley: normalize546(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch546(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize546(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic546(initialSeed = 546) {
  ...
  useEffect(() => {
    ...
    fakeFetch546(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source546.map((value, idx) => {
      ...
      return normalize546(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth546(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize546(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive546(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize546(quality) });
    dispatch({ type: 'marker', payload: normalize546(flags.confidence * 0.7 + quality * 0.3) });
