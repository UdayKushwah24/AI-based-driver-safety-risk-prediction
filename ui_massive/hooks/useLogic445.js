...
const source445 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 445) / 7) * 11;
  const drift = Math.cos((idx + 445) / 10) * 9;
  const edge = ((idx * 445) % 19) - 9;
...
function smooth445(input, radius) {
  const result = [];
...
function derive445(series, mode) {
  ...
  const spread = normalize445(peak - valley);
  const jitter = normalize445(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize445(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize445(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize445(avg),
    peak: normalize445(peak),
    valley: normalize445(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch445(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize445(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic445(initialSeed = 445) {
  ...
  useEffect(() => {
    ...
    fakeFetch445(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source445.map((value, idx) => {
      ...
      return normalize445(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth445(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize445(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive445(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize445(quality) });
    dispatch({ type: 'marker', payload: normalize445(flags.confidence * 0.7 + quality * 0.3) });
