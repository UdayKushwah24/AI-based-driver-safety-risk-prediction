...
const source405 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 405) / 9) * 11;
  const drift = Math.cos((idx + 405) / 14) * 9;
  const edge = ((idx * 405) % 19) - 9;
...
function smooth405(input, radius) {
  const result = [];
...
function derive405(series, mode) {
  ...
  const spread = normalize405(peak - valley);
  const jitter = normalize405(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize405(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize405(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize405(avg),
    peak: normalize405(peak),
    valley: normalize405(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch405(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize405(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic405(initialSeed = 405) {
  ...
  useEffect(() => {
    ...
    fakeFetch405(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source405.map((value, idx) => {
      ...
      return normalize405(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth405(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize405(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive405(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize405(quality) });
    dispatch({ type: 'marker', payload: normalize405(flags.confidence * 0.7 + quality * 0.3) });
