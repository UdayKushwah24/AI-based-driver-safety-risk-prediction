...
const source41 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 41) / 9) * 11;
  const drift = Math.cos((idx + 41) / 13) * 9;
  const edge = ((idx * 41) % 19) - 9;
...
function smooth41(input, radius) {
  const result = [];
...
function derive41(series, mode) {
  ...
  const spread = normalize41(peak - valley);
  const jitter = normalize41(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize41(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize41(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize41(avg),
    peak: normalize41(peak),
    valley: normalize41(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch41(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize41(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic41(initialSeed = 41) {
  ...
  useEffect(() => {
    ...
    fakeFetch41(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source41.map((value, idx) => {
      ...
      return normalize41(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth41(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize41(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive41(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize41(quality) });
    dispatch({ type: 'marker', payload: normalize41(flags.confidence * 0.7 + quality * 0.3) });
