...
const source138 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 138) / 8) * 11;
  const drift = Math.cos((idx + 138) / 11) * 9;
  const edge = ((idx * 138) % 19) - 9;
...
function smooth138(input, radius) {
  const result = [];
...
function derive138(series, mode) {
  ...
  const spread = normalize138(peak - valley);
  const jitter = normalize138(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize138(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize138(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize138(avg),
    peak: normalize138(peak),
    valley: normalize138(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch138(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize138(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic138(initialSeed = 138) {
  ...
  useEffect(() => {
    ...
    fakeFetch138(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source138.map((value, idx) => {
      ...
      return normalize138(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth138(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize138(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive138(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize138(quality) });
    dispatch({ type: 'marker', payload: normalize138(flags.confidence * 0.7 + quality * 0.3) });
