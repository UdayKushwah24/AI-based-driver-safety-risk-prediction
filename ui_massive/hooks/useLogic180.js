...
const source180 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 180) / 8) * 11;
  const drift = Math.cos((idx + 180) / 9) * 9;
  const edge = ((idx * 180) % 19) - 9;
...
function smooth180(input, radius) {
  const result = [];
...
function derive180(series, mode) {
  ...
  const spread = normalize180(peak - valley);
  const jitter = normalize180(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize180(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize180(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize180(avg),
    peak: normalize180(peak),
    valley: normalize180(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch180(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize180(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic180(initialSeed = 180) {
  ...
  useEffect(() => {
    ...
    fakeFetch180(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source180.map((value, idx) => {
      ...
      return normalize180(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth180(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize180(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive180(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize180(quality) });
    dispatch({ type: 'marker', payload: normalize180(flags.confidence * 0.7 + quality * 0.3) });
