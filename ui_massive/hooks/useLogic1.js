...
const source1 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 1) / 4) * 11;
  const drift = Math.cos((idx + 1) / 6) * 9;
  const edge = ((idx * 1) % 19) - 9;
...
function derive1(series, mode) {
  ...
  const spread = normalize1(peak - valley);
  const jitter = normalize1(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize1(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize1(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize1(avg),
    peak: normalize1(peak),
    valley: normalize1(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch1(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize1(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic1(initialSeed = 1) {
  ...
  useEffect(() => {
    ...
    fakeFetch1(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source1.map((value, idx) => {
      ...
      return normalize1(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth1(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize1(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive1(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize1(quality) });
    dispatch({ type: 'marker', payload: normalize1(flags.confidence * 0.7 + quality * 0.3) });
