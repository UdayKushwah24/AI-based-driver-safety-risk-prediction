...
const source251 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 251) / 9) * 11;
  const drift = Math.cos((idx + 251) / 14) * 9;
  const edge = ((idx * 251) % 19) - 9;
...
function smooth251(input, radius) {
  const result = [];
...
function derive251(series, mode) {
  ...
  const spread = normalize251(peak - valley);
  const jitter = normalize251(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize251(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize251(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize251(avg),
    peak: normalize251(peak),
    valley: normalize251(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch251(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize251(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic251(initialSeed = 251) {
  ...
  useEffect(() => {
    ...
    fakeFetch251(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source251.map((value, idx) => {
      ...
      return normalize251(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth251(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize251(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive251(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize251(quality) });
    dispatch({ type: 'marker', payload: normalize251(flags.confidence * 0.7 + quality * 0.3) });
