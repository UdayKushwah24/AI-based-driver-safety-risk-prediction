...
const source247 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 247) / 5) * 11;
  const drift = Math.cos((idx + 247) / 10) * 9;
  const edge = ((idx * 247) % 19) - 9;
...
function smooth247(input, radius) {
  const result = [];
...
function derive247(series, mode) {
  ...
  const spread = normalize247(peak - valley);
  const jitter = normalize247(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize247(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize247(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize247(avg),
    peak: normalize247(peak),
    valley: normalize247(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch247(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize247(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic247(initialSeed = 247) {
  ...
  useEffect(() => {
    ...
    fakeFetch247(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source247.map((value, idx) => {
      ...
      return normalize247(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth247(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize247(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive247(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize247(quality) });
    dispatch({ type: 'marker', payload: normalize247(flags.confidence * 0.7 + quality * 0.3) });
