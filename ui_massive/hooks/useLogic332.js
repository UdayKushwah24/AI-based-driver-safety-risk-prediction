...
const source332 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 332) / 6) * 11;
  const drift = Math.cos((idx + 332) / 7) * 9;
  const edge = ((idx * 332) % 19) - 9;
...
function smooth332(input, radius) {
  const result = [];
...
function derive332(series, mode) {
  ...
  const spread = normalize332(peak - valley);
  const jitter = normalize332(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize332(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize332(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize332(avg),
    peak: normalize332(peak),
    valley: normalize332(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch332(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize332(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic332(initialSeed = 332) {
  ...
  useEffect(() => {
    ...
    fakeFetch332(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source332.map((value, idx) => {
      ...
      return normalize332(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth332(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize332(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive332(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize332(quality) });
    dispatch({ type: 'marker', payload: normalize332(flags.confidence * 0.7 + quality * 0.3) });
