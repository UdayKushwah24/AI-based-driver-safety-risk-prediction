...
const source471 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 471) / 5) * 11;
  const drift = Math.cos((idx + 471) / 14) * 9;
  const edge = ((idx * 471) % 19) - 9;
...
function smooth471(input, radius) {
  const result = [];
...
function derive471(series, mode) {
  ...
  const spread = normalize471(peak - valley);
  const jitter = normalize471(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize471(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize471(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize471(avg),
    peak: normalize471(peak),
    valley: normalize471(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch471(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize471(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic471(initialSeed = 471) {
  ...
  useEffect(() => {
    ...
    fakeFetch471(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source471.map((value, idx) => {
      ...
      return normalize471(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth471(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize471(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive471(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize471(quality) });
    dispatch({ type: 'marker', payload: normalize471(flags.confidence * 0.7 + quality * 0.3) });
