...
const source261 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 261) / 5) * 11;
  const drift = Math.cos((idx + 261) / 13) * 9;
  const edge = ((idx * 261) % 19) - 9;
...
function smooth261(input, radius) {
  const result = [];
...
function derive261(series, mode) {
  ...
  const spread = normalize261(peak - valley);
  const jitter = normalize261(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize261(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize261(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize261(avg),
    peak: normalize261(peak),
    valley: normalize261(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch261(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize261(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic261(initialSeed = 261) {
  ...
  useEffect(() => {
    ...
    fakeFetch261(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source261.map((value, idx) => {
      ...
      return normalize261(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth261(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize261(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive261(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize261(quality) });
    dispatch({ type: 'marker', payload: normalize261(flags.confidence * 0.7 + quality * 0.3) });
