...
const source105 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 105) / 3) * 11;
  const drift = Math.cos((idx + 105) / 11) * 9;
  const edge = ((idx * 105) % 19) - 9;
...
function smooth105(input, radius) {
  const result = [];
...
function derive105(series, mode) {
  ...
  const spread = normalize105(peak - valley);
  const jitter = normalize105(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize105(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize105(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize105(avg),
    peak: normalize105(peak),
    valley: normalize105(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch105(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize105(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic105(initialSeed = 105) {
  ...
  useEffect(() => {
    ...
    fakeFetch105(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source105.map((value, idx) => {
      ...
      return normalize105(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth105(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize105(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive105(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize105(quality) });
    dispatch({ type: 'marker', payload: normalize105(flags.confidence * 0.7 + quality * 0.3) });
