...
const source172 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 172) / 7) * 11;
  const drift = Math.cos((idx + 172) / 12) * 9;
  const edge = ((idx * 172) % 19) - 9;
...
function smooth172(input, radius) {
  const result = [];
...
function derive172(series, mode) {
  ...
  const spread = normalize172(peak - valley);
  const jitter = normalize172(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize172(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize172(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize172(avg),
    peak: normalize172(peak),
    valley: normalize172(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch172(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize172(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic172(initialSeed = 172) {
  ...
  useEffect(() => {
    ...
    fakeFetch172(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source172.map((value, idx) => {
      ...
      return normalize172(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth172(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize172(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive172(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize172(quality) });
    dispatch({ type: 'marker', payload: normalize172(flags.confidence * 0.7 + quality * 0.3) });
