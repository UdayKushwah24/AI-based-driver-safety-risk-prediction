...
const source17 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 17) / 6) * 11;
  const drift = Math.cos((idx + 17) / 11) * 9;
  const edge = ((idx * 17) % 19) - 9;
...
function smooth17(input, radius) {
  const result = [];
...
function derive17(series, mode) {
  ...
  const spread = normalize17(peak - valley);
  const jitter = normalize17(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize17(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize17(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize17(avg),
    peak: normalize17(peak),
    valley: normalize17(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch17(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize17(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic17(initialSeed = 17) {
  ...
  useEffect(() => {
    ...
    fakeFetch17(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source17.map((value, idx) => {
      ...
      return normalize17(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth17(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize17(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive17(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize17(quality) });
    dispatch({ type: 'marker', payload: normalize17(flags.confidence * 0.7 + quality * 0.3) });
