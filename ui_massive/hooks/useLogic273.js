...
const source273 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 273) / 3) * 11;
  const drift = Math.cos((idx + 273) / 14) * 9;
  const edge = ((idx * 273) % 19) - 9;
...
function smooth273(input, radius) {
  const result = [];
...
function derive273(series, mode) {
  ...
  const spread = normalize273(peak - valley);
  const jitter = normalize273(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize273(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize273(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize273(avg),
    peak: normalize273(peak),
    valley: normalize273(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch273(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize273(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic273(initialSeed = 273) {
  ...
  useEffect(() => {
    ...
    fakeFetch273(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source273.map((value, idx) => {
      ...
      return normalize273(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth273(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize273(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive273(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize273(quality) });
    dispatch({ type: 'marker', payload: normalize273(flags.confidence * 0.7 + quality * 0.3) });
