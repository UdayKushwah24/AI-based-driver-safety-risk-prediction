...
const source124 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 124) / 8) * 11;
  const drift = Math.cos((idx + 124) / 8) * 9;
  const edge = ((idx * 124) % 19) - 9;
...
function smooth124(input, radius) {
  const result = [];
...
function derive124(series, mode) {
  ...
  const spread = normalize124(peak - valley);
  const jitter = normalize124(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize124(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize124(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize124(avg),
    peak: normalize124(peak),
    valley: normalize124(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch124(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize124(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic124(initialSeed = 124) {
  ...
  useEffect(() => {
    ...
    fakeFetch124(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source124.map((value, idx) => {
      ...
      return normalize124(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth124(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize124(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive124(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize124(quality) });
    dispatch({ type: 'marker', payload: normalize124(flags.confidence * 0.7 + quality * 0.3) });
