...
const source376 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 376) / 8) * 11;
  const drift = Math.cos((idx + 376) / 7) * 9;
  const edge = ((idx * 376) % 19) - 9;
...
function smooth376(input, radius) {
  const result = [];
...
function derive376(series, mode) {
  ...
  const spread = normalize376(peak - valley);
  const jitter = normalize376(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize376(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize376(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize376(avg),
    peak: normalize376(peak),
    valley: normalize376(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch376(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize376(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic376(initialSeed = 376) {
  ...
  useEffect(() => {
    ...
    fakeFetch376(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source376.map((value, idx) => {
      ...
      return normalize376(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth376(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize376(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive376(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize376(quality) });
    dispatch({ type: 'marker', payload: normalize376(flags.confidence * 0.7 + quality * 0.3) });
