...
const source137 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 137) / 7) * 11;
  const drift = Math.cos((idx + 137) / 10) * 9;
  const edge = ((idx * 137) % 19) - 9;
...
function smooth137(input, radius) {
  const result = [];
...
function derive137(series, mode) {
  ...
  const spread = normalize137(peak - valley);
  const jitter = normalize137(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize137(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize137(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize137(avg),
    peak: normalize137(peak),
    valley: normalize137(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch137(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize137(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic137(initialSeed = 137) {
  ...
  useEffect(() => {
    ...
    fakeFetch137(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source137.map((value, idx) => {
      ...
      return normalize137(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth137(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize137(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive137(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize137(quality) });
    dispatch({ type: 'marker', payload: normalize137(flags.confidence * 0.7 + quality * 0.3) });
