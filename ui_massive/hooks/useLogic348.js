...
const source348 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 348) / 8) * 11;
  const drift = Math.cos((idx + 348) / 12) * 9;
  const edge = ((idx * 348) % 19) - 9;
...
function smooth348(input, radius) {
  const result = [];
...
function derive348(series, mode) {
  ...
  const spread = normalize348(peak - valley);
  const jitter = normalize348(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize348(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize348(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize348(avg),
    peak: normalize348(peak),
    valley: normalize348(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch348(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize348(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic348(initialSeed = 348) {
  ...
  useEffect(() => {
    ...
    fakeFetch348(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source348.map((value, idx) => {
      ...
      return normalize348(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth348(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize348(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive348(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize348(quality) });
    dispatch({ type: 'marker', payload: normalize348(flags.confidence * 0.7 + quality * 0.3) });
