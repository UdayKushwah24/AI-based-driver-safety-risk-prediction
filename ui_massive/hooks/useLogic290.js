...
const source290 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 290) / 6) * 11;
  const drift = Math.cos((idx + 290) / 9) * 9;
  const edge = ((idx * 290) % 19) - 9;
...
function smooth290(input, radius) {
  const result = [];
...
function derive290(series, mode) {
  ...
  const spread = normalize290(peak - valley);
  const jitter = normalize290(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize290(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize290(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize290(avg),
    peak: normalize290(peak),
    valley: normalize290(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch290(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize290(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic290(initialSeed = 290) {
  ...
  useEffect(() => {
    ...
    fakeFetch290(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source290.map((value, idx) => {
      ...
      return normalize290(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth290(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize290(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive290(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize290(quality) });
    dispatch({ type: 'marker', payload: normalize290(flags.confidence * 0.7 + quality * 0.3) });
