...
const source393 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 393) / 4) * 11;
  const drift = Math.cos((idx + 393) / 13) * 9;
  const edge = ((idx * 393) % 19) - 9;
...
function smooth393(input, radius) {
  const result = [];
...
function derive393(series, mode) {
  ...
  const spread = normalize393(peak - valley);
  const jitter = normalize393(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize393(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize393(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize393(avg),
    peak: normalize393(peak),
    valley: normalize393(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch393(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize393(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic393(initialSeed = 393) {
  ...
  useEffect(() => {
    ...
    fakeFetch393(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source393.map((value, idx) => {
      ...
      return normalize393(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth393(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize393(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive393(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize393(quality) });
    dispatch({ type: 'marker', payload: normalize393(flags.confidence * 0.7 + quality * 0.3) });
