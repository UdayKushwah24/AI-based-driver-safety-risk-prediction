...
const source440 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 440) / 9) * 11;
  const drift = Math.cos((idx + 440) / 5) * 9;
  const edge = ((idx * 440) % 19) - 9;
...
function smooth440(input, radius) {
  const result = [];
...
function derive440(series, mode) {
  ...
  const spread = normalize440(peak - valley);
  const jitter = normalize440(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize440(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize440(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize440(avg),
    peak: normalize440(peak),
    valley: normalize440(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch440(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize440(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic440(initialSeed = 440) {
  ...
  useEffect(() => {
    ...
    fakeFetch440(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source440.map((value, idx) => {
      ...
      return normalize440(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth440(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize440(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive440(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize440(quality) });
    dispatch({ type: 'marker', payload: normalize440(flags.confidence * 0.7 + quality * 0.3) });
