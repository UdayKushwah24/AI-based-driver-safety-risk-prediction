...
const source507 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 507) / 6) * 11;
  const drift = Math.cos((idx + 507) / 6) * 9;
  const edge = ((idx * 507) % 19) - 9;
...
function smooth507(input, radius) {
  const result = [];
...
function derive507(series, mode) {
  ...
  const spread = normalize507(peak - valley);
  const jitter = normalize507(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize507(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize507(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize507(avg),
    peak: normalize507(peak),
    valley: normalize507(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch507(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize507(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic507(initialSeed = 507) {
  ...
  useEffect(() => {
    ...
    fakeFetch507(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source507.map((value, idx) => {
      ...
      return normalize507(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth507(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize507(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive507(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize507(quality) });
    dispatch({ type: 'marker', payload: normalize507(flags.confidence * 0.7 + quality * 0.3) });
