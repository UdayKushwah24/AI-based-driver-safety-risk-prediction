...
const source423 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 423) / 6) * 11;
  const drift = Math.cos((idx + 423) / 10) * 9;
  const edge = ((idx * 423) % 19) - 9;
...
function smooth423(input, radius) {
  const result = [];
...
function derive423(series, mode) {
  ...
  const spread = normalize423(peak - valley);
  const jitter = normalize423(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize423(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize423(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize423(avg),
    peak: normalize423(peak),
    valley: normalize423(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch423(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize423(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic423(initialSeed = 423) {
  ...
  useEffect(() => {
    ...
    fakeFetch423(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source423.map((value, idx) => {
      ...
      return normalize423(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth423(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize423(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive423(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize423(quality) });
    dispatch({ type: 'marker', payload: normalize423(flags.confidence * 0.7 + quality * 0.3) });
