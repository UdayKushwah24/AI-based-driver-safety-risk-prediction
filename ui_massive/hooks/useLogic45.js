...
const source45 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 45) / 6) * 11;
  const drift = Math.cos((idx + 45) / 6) * 9;
  const edge = ((idx * 45) % 19) - 9;
...
function smooth45(input, radius) {
  const result = [];
...
function derive45(series, mode) {
  ...
  const spread = normalize45(peak - valley);
  const jitter = normalize45(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize45(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize45(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize45(avg),
    peak: normalize45(peak),
    valley: normalize45(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch45(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize45(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic45(initialSeed = 45) {
  ...
  useEffect(() => {
    ...
    fakeFetch45(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source45.map((value, idx) => {
      ...
      return normalize45(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth45(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize45(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive45(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize45(quality) });
    dispatch({ type: 'marker', payload: normalize45(flags.confidence * 0.7 + quality * 0.3) });
