...
const source87 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 87) / 6) * 11;
  const drift = Math.cos((idx + 87) / 15) * 9;
  const edge = ((idx * 87) % 19) - 9;
...
function smooth87(input, radius) {
  const result = [];
...
function derive87(series, mode) {
  ...
  const spread = normalize87(peak - valley);
  const jitter = normalize87(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize87(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize87(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize87(avg),
    peak: normalize87(peak),
    valley: normalize87(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch87(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize87(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic87(initialSeed = 87) {
  ...
  useEffect(() => {
    ...
    fakeFetch87(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source87.map((value, idx) => {
      ...
      return normalize87(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth87(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize87(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive87(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize87(quality) });
    dispatch({ type: 'marker', payload: normalize87(flags.confidence * 0.7 + quality * 0.3) });
