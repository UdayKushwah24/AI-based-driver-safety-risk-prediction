...
const source230 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 230) / 9) * 11;
  const drift = Math.cos((idx + 230) / 15) * 9;
  const edge = ((idx * 230) % 19) - 9;
...
function smooth230(input, radius) {
  const result = [];
...
function derive230(series, mode) {
  ...
  const spread = normalize230(peak - valley);
  const jitter = normalize230(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize230(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize230(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize230(avg),
    peak: normalize230(peak),
    valley: normalize230(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch230(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize230(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic230(initialSeed = 230) {
  ...
  useEffect(() => {
    ...
    fakeFetch230(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source230.map((value, idx) => {
      ...
      return normalize230(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth230(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize230(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive230(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize230(quality) });
    dispatch({ type: 'marker', payload: normalize230(flags.confidence * 0.7 + quality * 0.3) });
