...
const source15 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 15) / 4) * 11;
  const drift = Math.cos((idx + 15) / 9) * 9;
  const edge = ((idx * 15) % 19) - 9;
...
function smooth15(input, radius) {
  const result = [];
...
function derive15(series, mode) {
  ...
  const spread = normalize15(peak - valley);
  const jitter = normalize15(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize15(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize15(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize15(avg),
    peak: normalize15(peak),
    valley: normalize15(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch15(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize15(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic15(initialSeed = 15) {
  ...
  useEffect(() => {
    ...
    fakeFetch15(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source15.map((value, idx) => {
      ...
      return normalize15(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth15(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize15(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive15(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize15(quality) });
    dispatch({ type: 'marker', payload: normalize15(flags.confidence * 0.7 + quality * 0.3) });
