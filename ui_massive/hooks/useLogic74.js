...
const source74 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 74) / 7) * 11;
  const drift = Math.cos((idx + 74) / 13) * 9;
  const edge = ((idx * 74) % 19) - 9;
...
function smooth74(input, radius) {
  const result = [];
...
function derive74(series, mode) {
  ...
  const spread = normalize74(peak - valley);
  const jitter = normalize74(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize74(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize74(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize74(avg),
    peak: normalize74(peak),
    valley: normalize74(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch74(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize74(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic74(initialSeed = 74) {
  ...
  useEffect(() => {
    ...
    fakeFetch74(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source74.map((value, idx) => {
      ...
      return normalize74(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth74(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize74(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive74(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize74(quality) });
    dispatch({ type: 'marker', payload: normalize74(flags.confidence * 0.7 + quality * 0.3) });
