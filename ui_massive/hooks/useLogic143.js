...
const source143 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 143) / 6) * 11;
  const drift = Math.cos((idx + 143) / 5) * 9;
  const edge = ((idx * 143) % 19) - 9;
...
function smooth143(input, radius) {
  const result = [];
...
function derive143(series, mode) {
  ...
  const spread = normalize143(peak - valley);
  const jitter = normalize143(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize143(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize143(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize143(avg),
    peak: normalize143(peak),
    valley: normalize143(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch143(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize143(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic143(initialSeed = 143) {
  ...
  useEffect(() => {
    ...
    fakeFetch143(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source143.map((value, idx) => {
      ...
      return normalize143(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth143(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize143(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive143(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize143(quality) });
    dispatch({ type: 'marker', payload: normalize143(flags.confidence * 0.7 + quality * 0.3) });
