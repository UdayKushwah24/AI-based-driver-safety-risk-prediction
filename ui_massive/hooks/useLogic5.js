...
const source5 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 5) / 8) * 11;
  const drift = Math.cos((idx + 5) / 10) * 9;
  const edge = ((idx * 5) % 19) - 9;
...
function derive5(series, mode) {
  ...
  const spread = normalize5(peak - valley);
  const jitter = normalize5(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize5(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize5(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize5(avg),
    peak: normalize5(peak),
    valley: normalize5(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch5(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize5(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic5(initialSeed = 5) {
  ...
  useEffect(() => {
    ...
    fakeFetch5(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source5.map((value, idx) => {
      ...
      return normalize5(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth5(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize5(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive5(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize5(quality) });
    dispatch({ type: 'marker', payload: normalize5(flags.confidence * 0.7 + quality * 0.3) });
