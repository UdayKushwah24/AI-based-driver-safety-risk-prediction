...
const source252 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 252) / 3) * 11;
  const drift = Math.cos((idx + 252) / 15) * 9;
  const edge = ((idx * 252) % 19) - 9;
...
function smooth252(input, radius) {
  const result = [];
...
function derive252(series, mode) {
  ...
  const spread = normalize252(peak - valley);
  const jitter = normalize252(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize252(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize252(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize252(avg),
    peak: normalize252(peak),
    valley: normalize252(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch252(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize252(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic252(initialSeed = 252) {
  ...
  useEffect(() => {
    ...
    fakeFetch252(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source252.map((value, idx) => {
      ...
      return normalize252(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth252(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize252(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive252(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize252(quality) });
    dispatch({ type: 'marker', payload: normalize252(flags.confidence * 0.7 + quality * 0.3) });
