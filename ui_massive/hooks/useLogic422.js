...
const source422 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 422) / 5) * 11;
  const drift = Math.cos((idx + 422) / 9) * 9;
  const edge = ((idx * 422) % 19) - 9;
...
function smooth422(input, radius) {
  const result = [];
...
function derive422(series, mode) {
  ...
  const spread = normalize422(peak - valley);
  const jitter = normalize422(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize422(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize422(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize422(avg),
    peak: normalize422(peak),
    valley: normalize422(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch422(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize422(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic422(initialSeed = 422) {
  ...
  useEffect(() => {
    ...
    fakeFetch422(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source422.map((value, idx) => {
      ...
      return normalize422(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth422(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize422(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive422(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize422(quality) });
    dispatch({ type: 'marker', payload: normalize422(flags.confidence * 0.7 + quality * 0.3) });
