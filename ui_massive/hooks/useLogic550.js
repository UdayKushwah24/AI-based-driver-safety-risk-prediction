...
const source550 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 550) / 7) * 11;
  const drift = Math.cos((idx + 550) / 5) * 9;
  const edge = ((idx * 550) % 19) - 9;
...
function smooth550(input, radius) {
  const result = [];
...
function derive550(series, mode) {
  ...
  const spread = normalize550(peak - valley);
  const jitter = normalize550(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize550(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize550(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize550(avg),
    peak: normalize550(peak),
    valley: normalize550(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch550(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize550(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic550(initialSeed = 550) {
  ...
  useEffect(() => {
    ...
    fakeFetch550(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source550.map((value, idx) => {
      ...
      return normalize550(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth550(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize550(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive550(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize550(quality) });
    dispatch({ type: 'marker', payload: normalize550(flags.confidence * 0.7 + quality * 0.3) });
