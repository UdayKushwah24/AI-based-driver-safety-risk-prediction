...
const source466 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 466) / 7) * 11;
  const drift = Math.cos((idx + 466) / 9) * 9;
  const edge = ((idx * 466) % 19) - 9;
...
function smooth466(input, radius) {
  const result = [];
...
function derive466(series, mode) {
  ...
  const spread = normalize466(peak - valley);
  const jitter = normalize466(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize466(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize466(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize466(avg),
    peak: normalize466(peak),
    valley: normalize466(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch466(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize466(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic466(initialSeed = 466) {
  ...
  useEffect(() => {
    ...
    fakeFetch466(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source466.map((value, idx) => {
      ...
      return normalize466(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth466(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize466(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive466(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize466(quality) });
    dispatch({ type: 'marker', payload: normalize466(flags.confidence * 0.7 + quality * 0.3) });
