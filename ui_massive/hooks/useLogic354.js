...
const source354 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 354) / 7) * 11;
  const drift = Math.cos((idx + 354) / 7) * 9;
  const edge = ((idx * 354) % 19) - 9;
...
function smooth354(input, radius) {
  const result = [];
...
function derive354(series, mode) {
  ...
  const spread = normalize354(peak - valley);
  const jitter = normalize354(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize354(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize354(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize354(avg),
    peak: normalize354(peak),
    valley: normalize354(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch354(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize354(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic354(initialSeed = 354) {
  ...
  useEffect(() => {
    ...
    fakeFetch354(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source354.map((value, idx) => {
      ...
      return normalize354(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth354(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize354(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive354(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize354(quality) });
    dispatch({ type: 'marker', payload: normalize354(flags.confidence * 0.7 + quality * 0.3) });
