...
const source48 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 48) / 9) * 11;
  const drift = Math.cos((idx + 48) / 9) * 9;
  const edge = ((idx * 48) % 19) - 9;
...
function smooth48(input, radius) {
  const result = [];
...
function derive48(series, mode) {
  ...
  const spread = normalize48(peak - valley);
  const jitter = normalize48(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize48(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize48(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize48(avg),
    peak: normalize48(peak),
    valley: normalize48(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch48(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize48(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic48(initialSeed = 48) {
  ...
  useEffect(() => {
    ...
    fakeFetch48(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source48.map((value, idx) => {
      ...
      return normalize48(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth48(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize48(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive48(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize48(quality) });
    dispatch({ type: 'marker', payload: normalize48(flags.confidence * 0.7 + quality * 0.3) });
