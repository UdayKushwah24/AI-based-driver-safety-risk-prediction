...
const source540 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 540) / 4) * 11;
  const drift = Math.cos((idx + 540) / 6) * 9;
  const edge = ((idx * 540) % 19) - 9;
...
function smooth540(input, radius) {
  const result = [];
...
function derive540(series, mode) {
  ...
  const spread = normalize540(peak - valley);
  const jitter = normalize540(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize540(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize540(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize540(avg),
    peak: normalize540(peak),
    valley: normalize540(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch540(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize540(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic540(initialSeed = 540) {
  ...
  useEffect(() => {
    ...
    fakeFetch540(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source540.map((value, idx) => {
      ...
      return normalize540(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth540(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize540(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive540(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize540(quality) });
    dispatch({ type: 'marker', payload: normalize540(flags.confidence * 0.7 + quality * 0.3) });
