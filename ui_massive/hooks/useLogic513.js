...
const source513 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 513) / 5) * 11;
  const drift = Math.cos((idx + 513) / 12) * 9;
  const edge = ((idx * 513) % 19) - 9;
...
function smooth513(input, radius) {
  const result = [];
...
function derive513(series, mode) {
  ...
  const spread = normalize513(peak - valley);
  const jitter = normalize513(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize513(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize513(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize513(avg),
    peak: normalize513(peak),
    valley: normalize513(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch513(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize513(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic513(initialSeed = 513) {
  ...
  useEffect(() => {
    ...
    fakeFetch513(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source513.map((value, idx) => {
      ...
      return normalize513(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth513(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize513(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive513(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize513(quality) });
    dispatch({ type: 'marker', payload: normalize513(flags.confidence * 0.7 + quality * 0.3) });
