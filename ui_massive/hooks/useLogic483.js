...
const source483 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 483) / 3) * 11;
  const drift = Math.cos((idx + 483) / 15) * 9;
  const edge = ((idx * 483) % 19) - 9;
...
function smooth483(input, radius) {
  const result = [];
...
function derive483(series, mode) {
  ...
  const spread = normalize483(peak - valley);
  const jitter = normalize483(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize483(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize483(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize483(avg),
    peak: normalize483(peak),
    valley: normalize483(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch483(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize483(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic483(initialSeed = 483) {
  ...
  useEffect(() => {
    ...
    fakeFetch483(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source483.map((value, idx) => {
      ...
      return normalize483(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth483(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize483(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive483(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize483(quality) });
    dispatch({ type: 'marker', payload: normalize483(flags.confidence * 0.7 + quality * 0.3) });
