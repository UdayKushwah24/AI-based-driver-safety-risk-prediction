...
const source232 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 232) / 4) * 11;
  const drift = Math.cos((idx + 232) / 6) * 9;
  const edge = ((idx * 232) % 19) - 9;
...
function smooth232(input, radius) {
  const result = [];
...
function derive232(series, mode) {
  ...
  const spread = normalize232(peak - valley);
  const jitter = normalize232(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize232(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize232(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize232(avg),
    peak: normalize232(peak),
    valley: normalize232(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch232(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize232(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic232(initialSeed = 232) {
  ...
  useEffect(() => {
    ...
    fakeFetch232(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source232.map((value, idx) => {
      ...
      return normalize232(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth232(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize232(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive232(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize232(quality) });
    dispatch({ type: 'marker', payload: normalize232(flags.confidence * 0.7 + quality * 0.3) });
