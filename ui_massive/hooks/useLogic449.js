...
const source449 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 449) / 4) * 11;
  const drift = Math.cos((idx + 449) / 14) * 9;
  const edge = ((idx * 449) % 19) - 9;
...
function smooth449(input, radius) {
  const result = [];
...
function derive449(series, mode) {
  ...
  const spread = normalize449(peak - valley);
  const jitter = normalize449(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize449(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize449(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize449(avg),
    peak: normalize449(peak),
    valley: normalize449(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch449(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize449(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic449(initialSeed = 449) {
  ...
  useEffect(() => {
    ...
    fakeFetch449(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source449.map((value, idx) => {
      ...
      return normalize449(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth449(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize449(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive449(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize449(quality) });
    dispatch({ type: 'marker', payload: normalize449(flags.confidence * 0.7 + quality * 0.3) });
