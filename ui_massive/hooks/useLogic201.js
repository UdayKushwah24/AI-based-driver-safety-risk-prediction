...
const source201 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 201) / 8) * 11;
  const drift = Math.cos((idx + 201) / 8) * 9;
  const edge = ((idx * 201) % 19) - 9;
...
function smooth201(input, radius) {
  const result = [];
...
function derive201(series, mode) {
  ...
  const spread = normalize201(peak - valley);
  const jitter = normalize201(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize201(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize201(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize201(avg),
    peak: normalize201(peak),
    valley: normalize201(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch201(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize201(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic201(initialSeed = 201) {
  ...
  useEffect(() => {
    ...
    fakeFetch201(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source201.map((value, idx) => {
      ...
      return normalize201(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth201(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize201(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive201(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize201(quality) });
    dispatch({ type: 'marker', payload: normalize201(flags.confidence * 0.7 + quality * 0.3) });
