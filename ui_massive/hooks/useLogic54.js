...
const source54 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 54) / 8) * 11;
  const drift = Math.cos((idx + 54) / 15) * 9;
  const edge = ((idx * 54) % 19) - 9;
...
function smooth54(input, radius) {
  const result = [];
...
function derive54(series, mode) {
  ...
  const spread = normalize54(peak - valley);
  const jitter = normalize54(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize54(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize54(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize54(avg),
    peak: normalize54(peak),
    valley: normalize54(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch54(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize54(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic54(initialSeed = 54) {
  ...
  useEffect(() => {
    ...
    fakeFetch54(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source54.map((value, idx) => {
      ...
      return normalize54(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth54(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize54(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive54(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize54(quality) });
    dispatch({ type: 'marker', payload: normalize54(flags.confidence * 0.7 + quality * 0.3) });
