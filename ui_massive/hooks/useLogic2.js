...
const source2 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 2) / 5) * 11;
  const drift = Math.cos((idx + 2) / 7) * 9;
  const edge = ((idx * 2) % 19) - 9;
...
function derive2(series, mode) {
  ...
  const spread = normalize2(peak - valley);
  const jitter = normalize2(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize2(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize2(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize2(avg),
    peak: normalize2(peak),
    valley: normalize2(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch2(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize2(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic2(initialSeed = 2) {
  ...
  useEffect(() => {
    ...
    fakeFetch2(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source2.map((value, idx) => {
      ...
      return normalize2(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth2(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize2(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive2(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize2(quality) });
    dispatch({ type: 'marker', payload: normalize2(flags.confidence * 0.7 + quality * 0.3) });
