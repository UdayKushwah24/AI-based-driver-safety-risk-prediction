...
const source112 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 112) / 3) * 11;
  const drift = Math.cos((idx + 112) / 7) * 9;
  const edge = ((idx * 112) % 19) - 9;
...
function smooth112(input, radius) {
  const result = [];
...
function derive112(series, mode) {
  ...
  const spread = normalize112(peak - valley);
  const jitter = normalize112(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize112(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize112(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize112(avg),
    peak: normalize112(peak),
    valley: normalize112(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch112(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize112(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic112(initialSeed = 112) {
  ...
  useEffect(() => {
    ...
    fakeFetch112(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source112.map((value, idx) => {
      ...
      return normalize112(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth112(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize112(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive112(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize112(quality) });
    dispatch({ type: 'marker', payload: normalize112(flags.confidence * 0.7 + quality * 0.3) });
