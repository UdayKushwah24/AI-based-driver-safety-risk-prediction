...
const source304 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 304) / 6) * 11;
  const drift = Math.cos((idx + 304) / 12) * 9;
  const edge = ((idx * 304) % 19) - 9;
...
function smooth304(input, radius) {
  const result = [];
...
function derive304(series, mode) {
  ...
  const spread = normalize304(peak - valley);
  const jitter = normalize304(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize304(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize304(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize304(avg),
    peak: normalize304(peak),
    valley: normalize304(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch304(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize304(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic304(initialSeed = 304) {
  ...
  useEffect(() => {
    ...
    fakeFetch304(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source304.map((value, idx) => {
      ...
      return normalize304(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth304(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize304(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive304(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize304(quality) });
    dispatch({ type: 'marker', payload: normalize304(flags.confidence * 0.7 + quality * 0.3) });
