...
const source95 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 95) / 7) * 11;
  const drift = Math.cos((idx + 95) / 12) * 9;
  const edge = ((idx * 95) % 19) - 9;
...
function smooth95(input, radius) {
  const result = [];
...
function derive95(series, mode) {
  ...
  const spread = normalize95(peak - valley);
  const jitter = normalize95(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize95(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize95(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize95(avg),
    peak: normalize95(peak),
    valley: normalize95(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch95(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize95(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic95(initialSeed = 95) {
  ...
  useEffect(() => {
    ...
    fakeFetch95(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source95.map((value, idx) => {
      ...
      return normalize95(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth95(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize95(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive95(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize95(quality) });
    dispatch({ type: 'marker', payload: normalize95(flags.confidence * 0.7 + quality * 0.3) });
