...
const source534 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 534) / 5) * 11;
  const drift = Math.cos((idx + 534) / 11) * 9;
  const edge = ((idx * 534) % 19) - 9;
...
function smooth534(input, radius) {
  const result = [];
...
function derive534(series, mode) {
  ...
  const spread = normalize534(peak - valley);
  const jitter = normalize534(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize534(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize534(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize534(avg),
    peak: normalize534(peak),
    valley: normalize534(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch534(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize534(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic534(initialSeed = 534) {
  ...
  useEffect(() => {
    ...
    fakeFetch534(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source534.map((value, idx) => {
      ...
      return normalize534(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth534(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize534(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive534(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize534(quality) });
    dispatch({ type: 'marker', payload: normalize534(flags.confidence * 0.7 + quality * 0.3) });
