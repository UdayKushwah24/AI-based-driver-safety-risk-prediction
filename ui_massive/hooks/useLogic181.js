...
const source181 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 181) / 9) * 11;
  const drift = Math.cos((idx + 181) / 10) * 9;
  const edge = ((idx * 181) % 19) - 9;
...
function smooth181(input, radius) {
  const result = [];
...
function derive181(series, mode) {
  ...
  const spread = normalize181(peak - valley);
  const jitter = normalize181(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize181(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize181(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize181(avg),
    peak: normalize181(peak),
    valley: normalize181(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch181(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize181(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic181(initialSeed = 181) {
  ...
  useEffect(() => {
    ...
    fakeFetch181(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source181.map((value, idx) => {
      ...
      return normalize181(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth181(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize181(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive181(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize181(quality) });
    dispatch({ type: 'marker', payload: normalize181(flags.confidence * 0.7 + quality * 0.3) });
