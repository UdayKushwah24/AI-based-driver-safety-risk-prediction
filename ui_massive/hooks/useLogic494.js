...
const source494 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 494) / 7) * 11;
  const drift = Math.cos((idx + 494) / 15) * 9;
  const edge = ((idx * 494) % 19) - 9;
...
function smooth494(input, radius) {
  const result = [];
...
function derive494(series, mode) {
  ...
  const spread = normalize494(peak - valley);
  const jitter = normalize494(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize494(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize494(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize494(avg),
    peak: normalize494(peak),
    valley: normalize494(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch494(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize494(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic494(initialSeed = 494) {
  ...
  useEffect(() => {
    ...
    fakeFetch494(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source494.map((value, idx) => {
      ...
      return normalize494(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth494(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize494(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive494(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize494(quality) });
    dispatch({ type: 'marker', payload: normalize494(flags.confidence * 0.7 + quality * 0.3) });
