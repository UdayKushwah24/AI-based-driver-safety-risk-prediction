...
const source125 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 125) / 9) * 11;
  const drift = Math.cos((idx + 125) / 9) * 9;
  const edge = ((idx * 125) % 19) - 9;
...
function smooth125(input, radius) {
  const result = [];
...
function derive125(series, mode) {
  ...
  const spread = normalize125(peak - valley);
  const jitter = normalize125(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize125(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize125(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize125(avg),
    peak: normalize125(peak),
    valley: normalize125(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch125(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize125(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic125(initialSeed = 125) {
  ...
  useEffect(() => {
    ...
    fakeFetch125(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source125.map((value, idx) => {
      ...
      return normalize125(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth125(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize125(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive125(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize125(quality) });
    dispatch({ type: 'marker', payload: normalize125(flags.confidence * 0.7 + quality * 0.3) });
