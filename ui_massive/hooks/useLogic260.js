...
const source260 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 260) / 4) * 11;
  const drift = Math.cos((idx + 260) / 12) * 9;
  const edge = ((idx * 260) % 19) - 9;
...
function smooth260(input, radius) {
  const result = [];
...
function derive260(series, mode) {
  ...
  const spread = normalize260(peak - valley);
  const jitter = normalize260(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize260(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize260(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize260(avg),
    peak: normalize260(peak),
    valley: normalize260(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch260(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize260(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic260(initialSeed = 260) {
  ...
  useEffect(() => {
    ...
    fakeFetch260(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source260.map((value, idx) => {
      ...
      return normalize260(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth260(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize260(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive260(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize260(quality) });
    dispatch({ type: 'marker', payload: normalize260(flags.confidence * 0.7 + quality * 0.3) });
