...
const source27 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 27) / 9) * 11;
  const drift = Math.cos((idx + 27) / 10) * 9;
  const edge = ((idx * 27) % 19) - 9;
...
function smooth27(input, radius) {
  const result = [];
...
function derive27(series, mode) {
  ...
  const spread = normalize27(peak - valley);
  const jitter = normalize27(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize27(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize27(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize27(avg),
    peak: normalize27(peak),
    valley: normalize27(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch27(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize27(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic27(initialSeed = 27) {
  ...
  useEffect(() => {
    ...
    fakeFetch27(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source27.map((value, idx) => {
      ...
      return normalize27(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth27(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize27(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive27(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize27(quality) });
    dispatch({ type: 'marker', payload: normalize27(flags.confidence * 0.7 + quality * 0.3) });
