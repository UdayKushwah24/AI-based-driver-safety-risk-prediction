...
const source311 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 311) / 6) * 11;
  const drift = Math.cos((idx + 311) / 8) * 9;
  const edge = ((idx * 311) % 19) - 9;
...
function smooth311(input, radius) {
  const result = [];
...
function derive311(series, mode) {
  ...
  const spread = normalize311(peak - valley);
  const jitter = normalize311(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize311(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize311(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize311(avg),
    peak: normalize311(peak),
    valley: normalize311(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch311(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize311(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic311(initialSeed = 311) {
  ...
  useEffect(() => {
    ...
    fakeFetch311(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source311.map((value, idx) => {
      ...
      return normalize311(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth311(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize311(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive311(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize311(quality) });
    dispatch({ type: 'marker', payload: normalize311(flags.confidence * 0.7 + quality * 0.3) });
