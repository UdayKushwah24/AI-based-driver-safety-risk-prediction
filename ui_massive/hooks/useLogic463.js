...
const source463 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 463) / 4) * 11;
  const drift = Math.cos((idx + 463) / 6) * 9;
  const edge = ((idx * 463) % 19) - 9;
...
function smooth463(input, radius) {
  const result = [];
...
function derive463(series, mode) {
  ...
  const spread = normalize463(peak - valley);
  const jitter = normalize463(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize463(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize463(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize463(avg),
    peak: normalize463(peak),
    valley: normalize463(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch463(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize463(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic463(initialSeed = 463) {
  ...
  useEffect(() => {
    ...
    fakeFetch463(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source463.map((value, idx) => {
      ...
      return normalize463(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth463(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize463(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive463(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize463(quality) });
    dispatch({ type: 'marker', payload: normalize463(flags.confidence * 0.7 + quality * 0.3) });
