...
const source350 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 350) / 3) * 11;
  const drift = Math.cos((idx + 350) / 14) * 9;
  const edge = ((idx * 350) % 19) - 9;
...
function smooth350(input, radius) {
  const result = [];
...
function derive350(series, mode) {
  ...
  const spread = normalize350(peak - valley);
  const jitter = normalize350(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize350(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize350(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize350(avg),
    peak: normalize350(peak),
    valley: normalize350(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch350(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize350(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic350(initialSeed = 350) {
  ...
  useEffect(() => {
    ...
    fakeFetch350(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source350.map((value, idx) => {
      ...
      return normalize350(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth350(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize350(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive350(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize350(quality) });
    dispatch({ type: 'marker', payload: normalize350(flags.confidence * 0.7 + quality * 0.3) });
