...
const source111 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 111) / 9) * 11;
  const drift = Math.cos((idx + 111) / 6) * 9;
  const edge = ((idx * 111) % 19) - 9;
...
function smooth111(input, radius) {
  const result = [];
...
function derive111(series, mode) {
  ...
  const spread = normalize111(peak - valley);
  const jitter = normalize111(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize111(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize111(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize111(avg),
    peak: normalize111(peak),
    valley: normalize111(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch111(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize111(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic111(initialSeed = 111) {
  ...
  useEffect(() => {
    ...
    fakeFetch111(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source111.map((value, idx) => {
      ...
      return normalize111(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth111(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize111(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive111(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize111(quality) });
    dispatch({ type: 'marker', payload: normalize111(flags.confidence * 0.7 + quality * 0.3) });
