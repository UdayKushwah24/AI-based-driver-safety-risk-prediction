...
const source171 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 171) / 6) * 11;
  const drift = Math.cos((idx + 171) / 11) * 9;
  const edge = ((idx * 171) % 19) - 9;
...
function smooth171(input, radius) {
  const result = [];
...
function derive171(series, mode) {
  ...
  const spread = normalize171(peak - valley);
  const jitter = normalize171(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize171(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize171(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize171(avg),
    peak: normalize171(peak),
    valley: normalize171(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch171(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize171(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic171(initialSeed = 171) {
  ...
  useEffect(() => {
    ...
    fakeFetch171(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source171.map((value, idx) => {
      ...
      return normalize171(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth171(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize171(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive171(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize171(quality) });
    dispatch({ type: 'marker', payload: normalize171(flags.confidence * 0.7 + quality * 0.3) });
