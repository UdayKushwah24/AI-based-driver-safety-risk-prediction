...
const source10 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 10) / 6) * 11;
  const drift = Math.cos((idx + 10) / 15) * 9;
  const edge = ((idx * 10) % 19) - 9;
...
function smooth10(input, radius) {
  const result = [];
...
function derive10(series, mode) {
  ...
  const spread = normalize10(peak - valley);
  const jitter = normalize10(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize10(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize10(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize10(avg),
    peak: normalize10(peak),
    valley: normalize10(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch10(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize10(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic10(initialSeed = 10) {
  ...
  useEffect(() => {
    ...
    fakeFetch10(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source10.map((value, idx) => {
      ...
      return normalize10(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth10(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize10(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive10(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize10(quality) });
    dispatch({ type: 'marker', payload: normalize10(flags.confidence * 0.7 + quality * 0.3) });
