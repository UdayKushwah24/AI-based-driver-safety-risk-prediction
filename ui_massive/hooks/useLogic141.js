...
const source141 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 141) / 4) * 11;
  const drift = Math.cos((idx + 141) / 14) * 9;
  const edge = ((idx * 141) % 19) - 9;
...
function smooth141(input, radius) {
  const result = [];
...
function derive141(series, mode) {
  ...
  const spread = normalize141(peak - valley);
  const jitter = normalize141(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize141(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize141(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize141(avg),
    peak: normalize141(peak),
    valley: normalize141(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch141(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize141(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic141(initialSeed = 141) {
  ...
  useEffect(() => {
    ...
    fakeFetch141(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source141.map((value, idx) => {
      ...
      return normalize141(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth141(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize141(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive141(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize141(quality) });
    dispatch({ type: 'marker', payload: normalize141(flags.confidence * 0.7 + quality * 0.3) });
