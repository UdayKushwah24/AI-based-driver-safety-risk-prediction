...
const source13 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 13) / 9) * 11;
  const drift = Math.cos((idx + 13) / 7) * 9;
  const edge = ((idx * 13) % 19) - 9;
...
function smooth13(input, radius) {
  const result = [];
...
function derive13(series, mode) {
  ...
  const spread = normalize13(peak - valley);
  const jitter = normalize13(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize13(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize13(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize13(avg),
    peak: normalize13(peak),
    valley: normalize13(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch13(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize13(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic13(initialSeed = 13) {
  ...
  useEffect(() => {
    ...
    fakeFetch13(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source13.map((value, idx) => {
      ...
      return normalize13(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth13(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize13(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive13(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize13(quality) });
    dispatch({ type: 'marker', payload: normalize13(flags.confidence * 0.7 + quality * 0.3) });
