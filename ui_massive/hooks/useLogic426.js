...
const source426 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 426) / 9) * 11;
  const drift = Math.cos((idx + 426) / 13) * 9;
  const edge = ((idx * 426) % 19) - 9;
...
function smooth426(input, radius) {
  const result = [];
...
function derive426(series, mode) {
  ...
  const spread = normalize426(peak - valley);
  const jitter = normalize426(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize426(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize426(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize426(avg),
    peak: normalize426(peak),
    valley: normalize426(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch426(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize426(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic426(initialSeed = 426) {
  ...
  useEffect(() => {
    ...
    fakeFetch426(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source426.map((value, idx) => {
      ...
      return normalize426(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth426(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize426(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive426(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize426(quality) });
    dispatch({ type: 'marker', payload: normalize426(flags.confidence * 0.7 + quality * 0.3) });
