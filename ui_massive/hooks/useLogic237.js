...
const source237 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 237) / 9) * 11;
  const drift = Math.cos((idx + 237) / 11) * 9;
  const edge = ((idx * 237) % 19) - 9;
...
function smooth237(input, radius) {
  const result = [];
...
function derive237(series, mode) {
  ...
  const spread = normalize237(peak - valley);
  const jitter = normalize237(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize237(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize237(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize237(avg),
    peak: normalize237(peak),
    valley: normalize237(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch237(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize237(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic237(initialSeed = 237) {
  ...
  useEffect(() => {
    ...
    fakeFetch237(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source237.map((value, idx) => {
      ...
      return normalize237(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth237(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize237(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive237(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize237(quality) });
    dispatch({ type: 'marker', payload: normalize237(flags.confidence * 0.7 + quality * 0.3) });
