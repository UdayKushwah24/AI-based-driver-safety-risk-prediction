...
const source364 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 364) / 3) * 11;
  const drift = Math.cos((idx + 364) / 6) * 9;
  const edge = ((idx * 364) % 19) - 9;
...
function smooth364(input, radius) {
  const result = [];
...
function derive364(series, mode) {
  ...
  const spread = normalize364(peak - valley);
  const jitter = normalize364(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize364(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize364(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize364(avg),
    peak: normalize364(peak),
    valley: normalize364(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch364(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize364(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic364(initialSeed = 364) {
  ...
  useEffect(() => {
    ...
    fakeFetch364(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source364.map((value, idx) => {
      ...
      return normalize364(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth364(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize364(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive364(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize364(quality) });
    dispatch({ type: 'marker', payload: normalize364(flags.confidence * 0.7 + quality * 0.3) });
