...
const source417 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 417) / 7) * 11;
  const drift = Math.cos((idx + 417) / 15) * 9;
  const edge = ((idx * 417) % 19) - 9;
...
function smooth417(input, radius) {
  const result = [];
...
function derive417(series, mode) {
  ...
  const spread = normalize417(peak - valley);
  const jitter = normalize417(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize417(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize417(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize417(avg),
    peak: normalize417(peak),
    valley: normalize417(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch417(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize417(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic417(initialSeed = 417) {
  ...
  useEffect(() => {
    ...
    fakeFetch417(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source417.map((value, idx) => {
      ...
      return normalize417(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth417(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize417(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive417(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize417(quality) });
    dispatch({ type: 'marker', payload: normalize417(flags.confidence * 0.7 + quality * 0.3) });
