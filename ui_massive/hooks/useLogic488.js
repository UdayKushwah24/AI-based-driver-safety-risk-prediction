...
const source488 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 488) / 8) * 11;
  const drift = Math.cos((idx + 488) / 9) * 9;
  const edge = ((idx * 488) % 19) - 9;
...
function smooth488(input, radius) {
  const result = [];
...
function derive488(series, mode) {
  ...
  const spread = normalize488(peak - valley);
  const jitter = normalize488(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize488(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize488(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize488(avg),
    peak: normalize488(peak),
    valley: normalize488(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch488(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize488(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic488(initialSeed = 488) {
  ...
  useEffect(() => {
    ...
    fakeFetch488(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source488.map((value, idx) => {
      ...
      return normalize488(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth488(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize488(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive488(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize488(quality) });
    dispatch({ type: 'marker', payload: normalize488(flags.confidence * 0.7 + quality * 0.3) });
