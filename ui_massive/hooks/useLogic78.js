...
const source78 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 78) / 4) * 11;
  const drift = Math.cos((idx + 78) / 6) * 9;
  const edge = ((idx * 78) % 19) - 9;
...
function smooth78(input, radius) {
  const result = [];
...
function derive78(series, mode) {
  ...
  const spread = normalize78(peak - valley);
  const jitter = normalize78(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize78(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize78(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize78(avg),
    peak: normalize78(peak),
    valley: normalize78(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch78(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize78(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic78(initialSeed = 78) {
  ...
  useEffect(() => {
    ...
    fakeFetch78(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source78.map((value, idx) => {
      ...
      return normalize78(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth78(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize78(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive78(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize78(quality) });
    dispatch({ type: 'marker', payload: normalize78(flags.confidence * 0.7 + quality * 0.3) });
