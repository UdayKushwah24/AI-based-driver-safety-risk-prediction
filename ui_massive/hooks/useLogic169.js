...
const source169 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 169) / 4) * 11;
  const drift = Math.cos((idx + 169) / 9) * 9;
  const edge = ((idx * 169) % 19) - 9;
...
function smooth169(input, radius) {
  const result = [];
...
function derive169(series, mode) {
  ...
  const spread = normalize169(peak - valley);
  const jitter = normalize169(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize169(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize169(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize169(avg),
    peak: normalize169(peak),
    valley: normalize169(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch169(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize169(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic169(initialSeed = 169) {
  ...
  useEffect(() => {
    ...
    fakeFetch169(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source169.map((value, idx) => {
      ...
      return normalize169(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth169(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize169(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive169(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize169(quality) });
    dispatch({ type: 'marker', payload: normalize169(flags.confidence * 0.7 + quality * 0.3) });
