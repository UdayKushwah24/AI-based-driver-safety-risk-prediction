...
const source77 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 77) / 3) * 11;
  const drift = Math.cos((idx + 77) / 5) * 9;
  const edge = ((idx * 77) % 19) - 9;
...
function smooth77(input, radius) {
  const result = [];
...
function derive77(series, mode) {
  ...
  const spread = normalize77(peak - valley);
  const jitter = normalize77(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize77(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize77(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize77(avg),
    peak: normalize77(peak),
    valley: normalize77(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch77(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize77(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic77(initialSeed = 77) {
  ...
  useEffect(() => {
    ...
    fakeFetch77(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source77.map((value, idx) => {
      ...
      return normalize77(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth77(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize77(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive77(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize77(quality) });
    dispatch({ type: 'marker', payload: normalize77(flags.confidence * 0.7 + quality * 0.3) });
