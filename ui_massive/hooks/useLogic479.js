...
const source479 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 479) / 6) * 11;
  const drift = Math.cos((idx + 479) / 11) * 9;
  const edge = ((idx * 479) % 19) - 9;
...
function smooth479(input, radius) {
  const result = [];
...
function derive479(series, mode) {
  ...
  const spread = normalize479(peak - valley);
  const jitter = normalize479(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize479(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize479(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize479(avg),
    peak: normalize479(peak),
    valley: normalize479(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch479(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize479(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic479(initialSeed = 479) {
  ...
  useEffect(() => {
    ...
    fakeFetch479(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source479.map((value, idx) => {
      ...
      return normalize479(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth479(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize479(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive479(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize479(quality) });
    dispatch({ type: 'marker', payload: normalize479(flags.confidence * 0.7 + quality * 0.3) });
