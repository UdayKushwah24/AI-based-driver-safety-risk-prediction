...
const source216 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 216) / 9) * 11;
  const drift = Math.cos((idx + 216) / 12) * 9;
  const edge = ((idx * 216) % 19) - 9;
...
function smooth216(input, radius) {
  const result = [];
...
function derive216(series, mode) {
  ...
  const spread = normalize216(peak - valley);
  const jitter = normalize216(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize216(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize216(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize216(avg),
    peak: normalize216(peak),
    valley: normalize216(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch216(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize216(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic216(initialSeed = 216) {
  ...
  useEffect(() => {
    ...
    fakeFetch216(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source216.map((value, idx) => {
      ...
      return normalize216(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth216(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize216(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive216(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize216(quality) });
    dispatch({ type: 'marker', payload: normalize216(flags.confidence * 0.7 + quality * 0.3) });
