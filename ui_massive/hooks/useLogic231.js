...
const source231 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 231) / 3) * 11;
  const drift = Math.cos((idx + 231) / 5) * 9;
  const edge = ((idx * 231) % 19) - 9;
...
function smooth231(input, radius) {
  const result = [];
...
function derive231(series, mode) {
  ...
  const spread = normalize231(peak - valley);
  const jitter = normalize231(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize231(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize231(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize231(avg),
    peak: normalize231(peak),
    valley: normalize231(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch231(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize231(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic231(initialSeed = 231) {
  ...
  useEffect(() => {
    ...
    fakeFetch231(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source231.map((value, idx) => {
      ...
      return normalize231(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth231(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize231(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive231(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize231(quality) });
    dispatch({ type: 'marker', payload: normalize231(flags.confidence * 0.7 + quality * 0.3) });
