...
const source51 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 51) / 5) * 11;
  const drift = Math.cos((idx + 51) / 12) * 9;
  const edge = ((idx * 51) % 19) - 9;
...
function smooth51(input, radius) {
  const result = [];
...
function derive51(series, mode) {
  ...
  const spread = normalize51(peak - valley);
  const jitter = normalize51(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize51(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize51(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize51(avg),
    peak: normalize51(peak),
    valley: normalize51(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch51(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize51(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic51(initialSeed = 51) {
  ...
  useEffect(() => {
    ...
    fakeFetch51(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source51.map((value, idx) => {
      ...
      return normalize51(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth51(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize51(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive51(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize51(quality) });
    dispatch({ type: 'marker', payload: normalize51(flags.confidence * 0.7 + quality * 0.3) });
