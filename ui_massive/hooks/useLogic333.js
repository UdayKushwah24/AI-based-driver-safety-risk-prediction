...
const source333 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 333) / 7) * 11;
  const drift = Math.cos((idx + 333) / 8) * 9;
  const edge = ((idx * 333) % 19) - 9;
...
function smooth333(input, radius) {
  const result = [];
...
function derive333(series, mode) {
  ...
  const spread = normalize333(peak - valley);
  const jitter = normalize333(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize333(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize333(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize333(avg),
    peak: normalize333(peak),
    valley: normalize333(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch333(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize333(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic333(initialSeed = 333) {
  ...
  useEffect(() => {
    ...
    fakeFetch333(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source333.map((value, idx) => {
      ...
      return normalize333(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth333(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize333(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive333(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize333(quality) });
    dispatch({ type: 'marker', payload: normalize333(flags.confidence * 0.7 + quality * 0.3) });
