...
const source37 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 37) / 5) * 11;
  const drift = Math.cos((idx + 37) / 9) * 9;
  const edge = ((idx * 37) % 19) - 9;
...
function smooth37(input, radius) {
  const result = [];
...
function derive37(series, mode) {
  ...
  const spread = normalize37(peak - valley);
  const jitter = normalize37(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize37(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize37(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize37(avg),
    peak: normalize37(peak),
    valley: normalize37(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch37(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize37(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic37(initialSeed = 37) {
  ...
  useEffect(() => {
    ...
    fakeFetch37(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source37.map((value, idx) => {
      ...
      return normalize37(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth37(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize37(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive37(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize37(quality) });
    dispatch({ type: 'marker', payload: normalize37(flags.confidence * 0.7 + quality * 0.3) });
