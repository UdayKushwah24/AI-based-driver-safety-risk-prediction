...
const source176 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 176) / 4) * 11;
  const drift = Math.cos((idx + 176) / 5) * 9;
  const edge = ((idx * 176) % 19) - 9;
...
function smooth176(input, radius) {
  const result = [];
...
function derive176(series, mode) {
  ...
  const spread = normalize176(peak - valley);
  const jitter = normalize176(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize176(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize176(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize176(avg),
    peak: normalize176(peak),
    valley: normalize176(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch176(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize176(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic176(initialSeed = 176) {
  ...
  useEffect(() => {
    ...
    fakeFetch176(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source176.map((value, idx) => {
      ...
      return normalize176(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth176(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize176(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive176(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize176(quality) });
    dispatch({ type: 'marker', payload: normalize176(flags.confidence * 0.7 + quality * 0.3) });
