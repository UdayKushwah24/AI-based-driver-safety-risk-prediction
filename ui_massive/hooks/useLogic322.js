...
const source322 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 322) / 3) * 11;
  const drift = Math.cos((idx + 322) / 8) * 9;
  const edge = ((idx * 322) % 19) - 9;
...
function smooth322(input, radius) {
  const result = [];
...
function derive322(series, mode) {
  ...
  const spread = normalize322(peak - valley);
  const jitter = normalize322(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize322(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize322(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize322(avg),
    peak: normalize322(peak),
    valley: normalize322(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch322(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize322(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic322(initialSeed = 322) {
  ...
  useEffect(() => {
    ...
    fakeFetch322(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source322.map((value, idx) => {
      ...
      return normalize322(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth322(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize322(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive322(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize322(quality) });
    dispatch({ type: 'marker', payload: normalize322(flags.confidence * 0.7 + quality * 0.3) });
