...
const source220 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 220) / 6) * 11;
  const drift = Math.cos((idx + 220) / 5) * 9;
  const edge = ((idx * 220) % 19) - 9;
...
function smooth220(input, radius) {
  const result = [];
...
function derive220(series, mode) {
  ...
  const spread = normalize220(peak - valley);
  const jitter = normalize220(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize220(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize220(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize220(avg),
    peak: normalize220(peak),
    valley: normalize220(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch220(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize220(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic220(initialSeed = 220) {
  ...
  useEffect(() => {
    ...
    fakeFetch220(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source220.map((value, idx) => {
      ...
      return normalize220(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth220(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize220(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive220(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize220(quality) });
    dispatch({ type: 'marker', payload: normalize220(flags.confidence * 0.7 + quality * 0.3) });
