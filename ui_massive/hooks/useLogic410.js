...
const source410 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 410) / 7) * 11;
  const drift = Math.cos((idx + 410) / 8) * 9;
  const edge = ((idx * 410) % 19) - 9;
...
function smooth410(input, radius) {
  const result = [];
...
function derive410(series, mode) {
  ...
  const spread = normalize410(peak - valley);
  const jitter = normalize410(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize410(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize410(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize410(avg),
    peak: normalize410(peak),
    valley: normalize410(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch410(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize410(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic410(initialSeed = 410) {
  ...
  useEffect(() => {
    ...
    fakeFetch410(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source410.map((value, idx) => {
      ...
      return normalize410(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth410(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize410(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive410(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize410(quality) });
    dispatch({ type: 'marker', payload: normalize410(flags.confidence * 0.7 + quality * 0.3) });
