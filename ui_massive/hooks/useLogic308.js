...
const source308 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 308) / 3) * 11;
  const drift = Math.cos((idx + 308) / 5) * 9;
  const edge = ((idx * 308) % 19) - 9;
...
function smooth308(input, radius) {
  const result = [];
...
function derive308(series, mode) {
  ...
  const spread = normalize308(peak - valley);
  const jitter = normalize308(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize308(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize308(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize308(avg),
    peak: normalize308(peak),
    valley: normalize308(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch308(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize308(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic308(initialSeed = 308) {
  ...
  useEffect(() => {
    ...
    fakeFetch308(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source308.map((value, idx) => {
      ...
      return normalize308(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth308(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize308(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive308(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize308(quality) });
    dispatch({ type: 'marker', payload: normalize308(flags.confidence * 0.7 + quality * 0.3) });
