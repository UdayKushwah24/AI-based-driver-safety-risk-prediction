...
const source26 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 26) / 8) * 11;
  const drift = Math.cos((idx + 26) / 9) * 9;
  const edge = ((idx * 26) % 19) - 9;
...
function smooth26(input, radius) {
  const result = [];
...
function derive26(series, mode) {
  ...
  const spread = normalize26(peak - valley);
  const jitter = normalize26(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize26(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize26(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize26(avg),
    peak: normalize26(peak),
    valley: normalize26(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch26(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize26(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic26(initialSeed = 26) {
  ...
  useEffect(() => {
    ...
    fakeFetch26(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source26.map((value, idx) => {
      ...
      return normalize26(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth26(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize26(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive26(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize26(quality) });
    dispatch({ type: 'marker', payload: normalize26(flags.confidence * 0.7 + quality * 0.3) });
