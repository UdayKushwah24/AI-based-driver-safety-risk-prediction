...
const source413 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 413) / 3) * 11;
  const drift = Math.cos((idx + 413) / 11) * 9;
  const edge = ((idx * 413) % 19) - 9;
...
function smooth413(input, radius) {
  const result = [];
...
function derive413(series, mode) {
  ...
  const spread = normalize413(peak - valley);
  const jitter = normalize413(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize413(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize413(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize413(avg),
    peak: normalize413(peak),
    valley: normalize413(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch413(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize413(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic413(initialSeed = 413) {
  ...
  useEffect(() => {
    ...
    fakeFetch413(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source413.map((value, idx) => {
      ...
      return normalize413(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth413(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize413(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive413(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize413(quality) });
    dispatch({ type: 'marker', payload: normalize413(flags.confidence * 0.7 + quality * 0.3) });
