...
const source22 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 22) / 4) * 11;
  const drift = Math.cos((idx + 22) / 5) * 9;
  const edge = ((idx * 22) % 19) - 9;
...
function smooth22(input, radius) {
  const result = [];
...
function derive22(series, mode) {
  ...
  const spread = normalize22(peak - valley);
  const jitter = normalize22(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize22(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize22(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize22(avg),
    peak: normalize22(peak),
    valley: normalize22(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch22(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize22(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic22(initialSeed = 22) {
  ...
  useEffect(() => {
    ...
    fakeFetch22(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source22.map((value, idx) => {
      ...
      return normalize22(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth22(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize22(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive22(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize22(quality) });
    dispatch({ type: 'marker', payload: normalize22(flags.confidence * 0.7 + quality * 0.3) });
