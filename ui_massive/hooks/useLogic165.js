...
const source165 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 165) / 7) * 11;
  const drift = Math.cos((idx + 165) / 5) * 9;
  const edge = ((idx * 165) % 19) - 9;
...
function smooth165(input, radius) {
  const result = [];
...
function derive165(series, mode) {
  ...
  const spread = normalize165(peak - valley);
  const jitter = normalize165(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize165(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize165(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize165(avg),
    peak: normalize165(peak),
    valley: normalize165(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch165(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize165(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic165(initialSeed = 165) {
  ...
  useEffect(() => {
    ...
    fakeFetch165(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source165.map((value, idx) => {
      ...
      return normalize165(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth165(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize165(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive165(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize165(quality) });
    dispatch({ type: 'marker', payload: normalize165(flags.confidence * 0.7 + quality * 0.3) });
