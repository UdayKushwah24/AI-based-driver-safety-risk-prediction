...
const source387 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 387) / 5) * 11;
  const drift = Math.cos((idx + 387) / 7) * 9;
  const edge = ((idx * 387) % 19) - 9;
...
function smooth387(input, radius) {
  const result = [];
...
function derive387(series, mode) {
  ...
  const spread = normalize387(peak - valley);
  const jitter = normalize387(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize387(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize387(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize387(avg),
    peak: normalize387(peak),
    valley: normalize387(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch387(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize387(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic387(initialSeed = 387) {
  ...
  useEffect(() => {
    ...
    fakeFetch387(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source387.map((value, idx) => {
      ...
      return normalize387(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth387(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize387(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive387(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize387(quality) });
    dispatch({ type: 'marker', payload: normalize387(flags.confidence * 0.7 + quality * 0.3) });
