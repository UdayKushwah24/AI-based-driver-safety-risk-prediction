...
const source221 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 221) / 7) * 11;
  const drift = Math.cos((idx + 221) / 6) * 9;
  const edge = ((idx * 221) % 19) - 9;
...
function smooth221(input, radius) {
  const result = [];
...
function derive221(series, mode) {
  ...
  const spread = normalize221(peak - valley);
  const jitter = normalize221(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize221(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize221(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize221(avg),
    peak: normalize221(peak),
    valley: normalize221(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch221(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize221(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic221(initialSeed = 221) {
  ...
  useEffect(() => {
    ...
    fakeFetch221(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source221.map((value, idx) => {
      ...
      return normalize221(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth221(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize221(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive221(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize221(quality) });
    dispatch({ type: 'marker', payload: normalize221(flags.confidence * 0.7 + quality * 0.3) });
