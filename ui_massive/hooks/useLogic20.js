...
const source20 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 20) / 9) * 11;
  const drift = Math.cos((idx + 20) / 14) * 9;
  const edge = ((idx * 20) % 19) - 9;
...
function smooth20(input, radius) {
  const result = [];
...
function derive20(series, mode) {
  ...
  const spread = normalize20(peak - valley);
  const jitter = normalize20(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize20(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize20(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize20(avg),
    peak: normalize20(peak),
    valley: normalize20(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch20(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize20(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic20(initialSeed = 20) {
  ...
  useEffect(() => {
    ...
    fakeFetch20(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source20.map((value, idx) => {
      ...
      return normalize20(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth20(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize20(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive20(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize20(quality) });
    dispatch({ type: 'marker', payload: normalize20(flags.confidence * 0.7 + quality * 0.3) });
