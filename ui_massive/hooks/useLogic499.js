...
const source499 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 499) / 5) * 11;
  const drift = Math.cos((idx + 499) / 9) * 9;
  const edge = ((idx * 499) % 19) - 9;
...
function smooth499(input, radius) {
  const result = [];
...
function derive499(series, mode) {
  ...
  const spread = normalize499(peak - valley);
  const jitter = normalize499(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize499(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize499(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize499(avg),
    peak: normalize499(peak),
    valley: normalize499(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch499(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize499(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic499(initialSeed = 499) {
  ...
  useEffect(() => {
    ...
    fakeFetch499(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source499.map((value, idx) => {
      ...
      return normalize499(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth499(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize499(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive499(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize499(quality) });
    dispatch({ type: 'marker', payload: normalize499(flags.confidence * 0.7 + quality * 0.3) });
