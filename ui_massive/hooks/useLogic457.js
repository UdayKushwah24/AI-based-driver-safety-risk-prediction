...
const source457 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 457) / 5) * 11;
  const drift = Math.cos((idx + 457) / 11) * 9;
  const edge = ((idx * 457) % 19) - 9;
...
function smooth457(input, radius) {
  const result = [];
...
function derive457(series, mode) {
  ...
  const spread = normalize457(peak - valley);
  const jitter = normalize457(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize457(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize457(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize457(avg),
    peak: normalize457(peak),
    valley: normalize457(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch457(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize457(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic457(initialSeed = 457) {
  ...
  useEffect(() => {
    ...
    fakeFetch457(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source457.map((value, idx) => {
      ...
      return normalize457(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth457(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize457(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive457(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize457(quality) });
    dispatch({ type: 'marker', payload: normalize457(flags.confidence * 0.7 + quality * 0.3) });
