...
const source365 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 365) / 4) * 11;
  const drift = Math.cos((idx + 365) / 7) * 9;
  const edge = ((idx * 365) % 19) - 9;
...
function smooth365(input, radius) {
  const result = [];
...
function derive365(series, mode) {
  ...
  const spread = normalize365(peak - valley);
  const jitter = normalize365(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize365(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize365(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize365(avg),
    peak: normalize365(peak),
    valley: normalize365(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch365(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize365(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic365(initialSeed = 365) {
  ...
  useEffect(() => {
    ...
    fakeFetch365(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source365.map((value, idx) => {
      ...
      return normalize365(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth365(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize365(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive365(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize365(quality) });
    dispatch({ type: 'marker', payload: normalize365(flags.confidence * 0.7 + quality * 0.3) });
