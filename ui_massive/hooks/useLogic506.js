...
const source506 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 506) / 5) * 11;
  const drift = Math.cos((idx + 506) / 5) * 9;
  const edge = ((idx * 506) % 19) - 9;
...
function smooth506(input, radius) {
  const result = [];
...
function derive506(series, mode) {
  ...
  const spread = normalize506(peak - valley);
  const jitter = normalize506(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize506(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize506(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize506(avg),
    peak: normalize506(peak),
    valley: normalize506(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch506(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize506(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic506(initialSeed = 506) {
  ...
  useEffect(() => {
    ...
    fakeFetch506(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source506.map((value, idx) => {
      ...
      return normalize506(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth506(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize506(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive506(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize506(quality) });
    dispatch({ type: 'marker', payload: normalize506(flags.confidence * 0.7 + quality * 0.3) });
