...
const source211 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 211) / 4) * 11;
  const drift = Math.cos((idx + 211) / 7) * 9;
  const edge = ((idx * 211) % 19) - 9;
...
function smooth211(input, radius) {
  const result = [];
...
function derive211(series, mode) {
  ...
  const spread = normalize211(peak - valley);
  const jitter = normalize211(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize211(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize211(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize211(avg),
    peak: normalize211(peak),
    valley: normalize211(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch211(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize211(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic211(initialSeed = 211) {
  ...
  useEffect(() => {
    ...
    fakeFetch211(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source211.map((value, idx) => {
      ...
      return normalize211(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth211(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize211(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive211(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize211(quality) });
    dispatch({ type: 'marker', payload: normalize211(flags.confidence * 0.7 + quality * 0.3) });
