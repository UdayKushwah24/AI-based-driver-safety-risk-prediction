...
const source428 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 428) / 4) * 11;
  const drift = Math.cos((idx + 428) / 15) * 9;
  const edge = ((idx * 428) % 19) - 9;
...
function smooth428(input, radius) {
  const result = [];
...
function derive428(series, mode) {
  ...
  const spread = normalize428(peak - valley);
  const jitter = normalize428(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize428(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize428(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize428(avg),
    peak: normalize428(peak),
    valley: normalize428(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch428(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize428(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic428(initialSeed = 428) {
  ...
  useEffect(() => {
    ...
    fakeFetch428(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source428.map((value, idx) => {
      ...
      return normalize428(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth428(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize428(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive428(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize428(quality) });
    dispatch({ type: 'marker', payload: normalize428(flags.confidence * 0.7 + quality * 0.3) });
