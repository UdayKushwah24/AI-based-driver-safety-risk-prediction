...
const source379 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 379) / 4) * 11;
  const drift = Math.cos((idx + 379) / 10) * 9;
  const edge = ((idx * 379) % 19) - 9;
...
function smooth379(input, radius) {
  const result = [];
...
function derive379(series, mode) {
  ...
  const spread = normalize379(peak - valley);
  const jitter = normalize379(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize379(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize379(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize379(avg),
    peak: normalize379(peak),
    valley: normalize379(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch379(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize379(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic379(initialSeed = 379) {
  ...
  useEffect(() => {
    ...
    fakeFetch379(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source379.map((value, idx) => {
      ...
      return normalize379(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth379(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize379(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive379(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize379(quality) });
    dispatch({ type: 'marker', payload: normalize379(flags.confidence * 0.7 + quality * 0.3) });
