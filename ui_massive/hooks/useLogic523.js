...
const source523 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 523) / 8) * 11;
  const drift = Math.cos((idx + 523) / 11) * 9;
  const edge = ((idx * 523) % 19) - 9;
...
function smooth523(input, radius) {
  const result = [];
...
function derive523(series, mode) {
  ...
  const spread = normalize523(peak - valley);
  const jitter = normalize523(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize523(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize523(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize523(avg),
    peak: normalize523(peak),
    valley: normalize523(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch523(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize523(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic523(initialSeed = 523) {
  ...
  useEffect(() => {
    ...
    fakeFetch523(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source523.map((value, idx) => {
      ...
      return normalize523(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth523(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize523(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive523(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize523(quality) });
    dispatch({ type: 'marker', payload: normalize523(flags.confidence * 0.7 + quality * 0.3) });
