...
const source174 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 174) / 9) * 11;
  const drift = Math.cos((idx + 174) / 14) * 9;
  const edge = ((idx * 174) % 19) - 9;
...
function smooth174(input, radius) {
  const result = [];
...
function derive174(series, mode) {
  ...
  const spread = normalize174(peak - valley);
  const jitter = normalize174(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize174(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize174(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize174(avg),
    peak: normalize174(peak),
    valley: normalize174(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch174(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize174(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic174(initialSeed = 174) {
  ...
  useEffect(() => {
    ...
    fakeFetch174(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source174.map((value, idx) => {
      ...
      return normalize174(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth174(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize174(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive174(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize174(quality) });
    dispatch({ type: 'marker', payload: normalize174(flags.confidence * 0.7 + quality * 0.3) });
