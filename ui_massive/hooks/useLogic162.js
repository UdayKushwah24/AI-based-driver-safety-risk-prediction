...
const source162 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 162) / 4) * 11;
  const drift = Math.cos((idx + 162) / 13) * 9;
  const edge = ((idx * 162) % 19) - 9;
...
function smooth162(input, radius) {
  const result = [];
...
function derive162(series, mode) {
  ...
  const spread = normalize162(peak - valley);
  const jitter = normalize162(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize162(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize162(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize162(avg),
    peak: normalize162(peak),
    valley: normalize162(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch162(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize162(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic162(initialSeed = 162) {
  ...
  useEffect(() => {
    ...
    fakeFetch162(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source162.map((value, idx) => {
      ...
      return normalize162(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth162(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize162(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive162(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize162(quality) });
    dispatch({ type: 'marker', payload: normalize162(flags.confidence * 0.7 + quality * 0.3) });
