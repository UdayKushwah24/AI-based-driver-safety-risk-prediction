...
const source21 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 21) / 3) * 11;
  const drift = Math.cos((idx + 21) / 15) * 9;
  const edge = ((idx * 21) % 19) - 9;
...
function smooth21(input, radius) {
  const result = [];
...
function derive21(series, mode) {
  ...
  const spread = normalize21(peak - valley);
  const jitter = normalize21(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize21(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize21(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize21(avg),
    peak: normalize21(peak),
    valley: normalize21(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch21(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize21(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic21(initialSeed = 21) {
  ...
  useEffect(() => {
    ...
    fakeFetch21(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source21.map((value, idx) => {
      ...
      return normalize21(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth21(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize21(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive21(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize21(quality) });
    dispatch({ type: 'marker', payload: normalize21(flags.confidence * 0.7 + quality * 0.3) });
