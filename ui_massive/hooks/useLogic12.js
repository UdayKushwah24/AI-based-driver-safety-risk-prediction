...
const source12 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 12) / 8) * 11;
  const drift = Math.cos((idx + 12) / 6) * 9;
  const edge = ((idx * 12) % 19) - 9;
...
function smooth12(input, radius) {
  const result = [];
...
function derive12(series, mode) {
  ...
  const spread = normalize12(peak - valley);
  const jitter = normalize12(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize12(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize12(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize12(avg),
    peak: normalize12(peak),
    valley: normalize12(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch12(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize12(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic12(initialSeed = 12) {
  ...
  useEffect(() => {
    ...
    fakeFetch12(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source12.map((value, idx) => {
      ...
      return normalize12(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth12(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize12(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive12(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize12(quality) });
    dispatch({ type: 'marker', payload: normalize12(flags.confidence * 0.7 + quality * 0.3) });
