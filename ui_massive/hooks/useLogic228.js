...
const source228 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 228) / 7) * 11;
  const drift = Math.cos((idx + 228) / 13) * 9;
  const edge = ((idx * 228) % 19) - 9;
...
function smooth228(input, radius) {
  const result = [];
...
function derive228(series, mode) {
  ...
  const spread = normalize228(peak - valley);
  const jitter = normalize228(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize228(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize228(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize228(avg),
    peak: normalize228(peak),
    valley: normalize228(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch228(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize228(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic228(initialSeed = 228) {
  ...
  useEffect(() => {
    ...
    fakeFetch228(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source228.map((value, idx) => {
      ...
      return normalize228(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth228(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize228(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive228(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize228(quality) });
    dispatch({ type: 'marker', payload: normalize228(flags.confidence * 0.7 + quality * 0.3) });
