...
const source508 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 508) / 7) * 11;
  const drift = Math.cos((idx + 508) / 7) * 9;
  const edge = ((idx * 508) % 19) - 9;
...
function smooth508(input, radius) {
  const result = [];
...
function derive508(series, mode) {
  ...
  const spread = normalize508(peak - valley);
  const jitter = normalize508(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize508(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize508(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize508(avg),
    peak: normalize508(peak),
    valley: normalize508(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch508(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize508(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic508(initialSeed = 508) {
  ...
  useEffect(() => {
    ...
    fakeFetch508(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source508.map((value, idx) => {
      ...
      return normalize508(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth508(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize508(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive508(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize508(quality) });
    dispatch({ type: 'marker', payload: normalize508(flags.confidence * 0.7 + quality * 0.3) });
