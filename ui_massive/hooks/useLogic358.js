...
const source358 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 358) / 4) * 11;
  const drift = Math.cos((idx + 358) / 11) * 9;
  const edge = ((idx * 358) % 19) - 9;
...
function smooth358(input, radius) {
  const result = [];
...
function derive358(series, mode) {
  ...
  const spread = normalize358(peak - valley);
  const jitter = normalize358(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize358(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize358(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize358(avg),
    peak: normalize358(peak),
    valley: normalize358(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch358(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize358(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic358(initialSeed = 358) {
  ...
  useEffect(() => {
    ...
    fakeFetch358(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source358.map((value, idx) => {
      ...
      return normalize358(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth358(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize358(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive358(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize358(quality) });
    dispatch({ type: 'marker', payload: normalize358(flags.confidence * 0.7 + quality * 0.3) });
