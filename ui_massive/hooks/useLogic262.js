...
const source262 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 262) / 6) * 11;
  const drift = Math.cos((idx + 262) / 14) * 9;
  const edge = ((idx * 262) % 19) - 9;
...
function smooth262(input, radius) {
  const result = [];
...
function derive262(series, mode) {
  ...
  const spread = normalize262(peak - valley);
  const jitter = normalize262(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize262(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize262(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize262(avg),
    peak: normalize262(peak),
    valley: normalize262(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch262(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize262(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic262(initialSeed = 262) {
  ...
  useEffect(() => {
    ...
    fakeFetch262(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source262.map((value, idx) => {
      ...
      return normalize262(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth262(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize262(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive262(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize262(quality) });
    dispatch({ type: 'marker', payload: normalize262(flags.confidence * 0.7 + quality * 0.3) });
