...
const source91 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 91) / 3) * 11;
  const drift = Math.cos((idx + 91) / 8) * 9;
  const edge = ((idx * 91) % 19) - 9;
...
function smooth91(input, radius) {
  const result = [];
...
function derive91(series, mode) {
  ...
  const spread = normalize91(peak - valley);
  const jitter = normalize91(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize91(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize91(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize91(avg),
    peak: normalize91(peak),
    valley: normalize91(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch91(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize91(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic91(initialSeed = 91) {
  ...
  useEffect(() => {
    ...
    fakeFetch91(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source91.map((value, idx) => {
      ...
      return normalize91(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth91(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize91(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive91(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize91(quality) });
    dispatch({ type: 'marker', payload: normalize91(flags.confidence * 0.7 + quality * 0.3) });
