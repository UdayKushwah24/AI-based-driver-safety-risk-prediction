...
const source101 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 101) / 6) * 11;
  const drift = Math.cos((idx + 101) / 7) * 9;
  const edge = ((idx * 101) % 19) - 9;
...
function smooth101(input, radius) {
  const result = [];
...
function derive101(series, mode) {
  ...
  const spread = normalize101(peak - valley);
  const jitter = normalize101(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize101(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize101(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize101(avg),
    peak: normalize101(peak),
    valley: normalize101(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch101(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize101(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic101(initialSeed = 101) {
  ...
  useEffect(() => {
    ...
    fakeFetch101(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source101.map((value, idx) => {
      ...
      return normalize101(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth101(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize101(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive101(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize101(quality) });
    dispatch({ type: 'marker', payload: normalize101(flags.confidence * 0.7 + quality * 0.3) });
