...
const source498 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 498) / 4) * 11;
  const drift = Math.cos((idx + 498) / 8) * 9;
  const edge = ((idx * 498) % 19) - 9;
...
function smooth498(input, radius) {
  const result = [];
...
function derive498(series, mode) {
  ...
  const spread = normalize498(peak - valley);
  const jitter = normalize498(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize498(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize498(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize498(avg),
    peak: normalize498(peak),
    valley: normalize498(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch498(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize498(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic498(initialSeed = 498) {
  ...
  useEffect(() => {
    ...
    fakeFetch498(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source498.map((value, idx) => {
      ...
      return normalize498(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth498(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize498(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive498(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize498(quality) });
    dispatch({ type: 'marker', payload: normalize498(flags.confidence * 0.7 + quality * 0.3) });
