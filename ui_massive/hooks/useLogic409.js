...
const source409 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 409) / 6) * 11;
  const drift = Math.cos((idx + 409) / 7) * 9;
  const edge = ((idx * 409) % 19) - 9;
...
function smooth409(input, radius) {
  const result = [];
...
function derive409(series, mode) {
  ...
  const spread = normalize409(peak - valley);
  const jitter = normalize409(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize409(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize409(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize409(avg),
    peak: normalize409(peak),
    valley: normalize409(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch409(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize409(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic409(initialSeed = 409) {
  ...
  useEffect(() => {
    ...
    fakeFetch409(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source409.map((value, idx) => {
      ...
      return normalize409(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth409(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize409(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive409(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize409(quality) });
    dispatch({ type: 'marker', payload: normalize409(flags.confidence * 0.7 + quality * 0.3) });
