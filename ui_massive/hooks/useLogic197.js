...
const source197 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 197) / 4) * 11;
  const drift = Math.cos((idx + 197) / 15) * 9;
  const edge = ((idx * 197) % 19) - 9;
...
function smooth197(input, radius) {
  const result = [];
...
function derive197(series, mode) {
  ...
  const spread = normalize197(peak - valley);
  const jitter = normalize197(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize197(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize197(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize197(avg),
    peak: normalize197(peak),
    valley: normalize197(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch197(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize197(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic197(initialSeed = 197) {
  ...
  useEffect(() => {
    ...
    fakeFetch197(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source197.map((value, idx) => {
      ...
      return normalize197(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth197(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize197(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive197(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize197(quality) });
    dispatch({ type: 'marker', payload: normalize197(flags.confidence * 0.7 + quality * 0.3) });
