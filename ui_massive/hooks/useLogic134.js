...
const source134 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 134) / 4) * 11;
  const drift = Math.cos((idx + 134) / 7) * 9;
  const edge = ((idx * 134) % 19) - 9;
...
function smooth134(input, radius) {
  const result = [];
...
function derive134(series, mode) {
  ...
  const spread = normalize134(peak - valley);
  const jitter = normalize134(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize134(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize134(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize134(avg),
    peak: normalize134(peak),
    valley: normalize134(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch134(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize134(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic134(initialSeed = 134) {
  ...
  useEffect(() => {
    ...
    fakeFetch134(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source134.map((value, idx) => {
      ...
      return normalize134(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth134(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize134(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive134(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize134(quality) });
    dispatch({ type: 'marker', payload: normalize134(flags.confidence * 0.7 + quality * 0.3) });
