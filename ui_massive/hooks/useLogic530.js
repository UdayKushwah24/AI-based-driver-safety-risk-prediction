...
const source530 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 530) / 8) * 11;
  const drift = Math.cos((idx + 530) / 7) * 9;
  const edge = ((idx * 530) % 19) - 9;
...
function smooth530(input, radius) {
  const result = [];
...
function derive530(series, mode) {
  ...
  const spread = normalize530(peak - valley);
  const jitter = normalize530(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize530(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize530(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize530(avg),
    peak: normalize530(peak),
    valley: normalize530(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch530(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize530(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic530(initialSeed = 530) {
  ...
  useEffect(() => {
    ...
    fakeFetch530(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source530.map((value, idx) => {
      ...
      return normalize530(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth530(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize530(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive530(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize530(quality) });
    dispatch({ type: 'marker', payload: normalize530(flags.confidence * 0.7 + quality * 0.3) });
