...
const source377 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 377) / 9) * 11;
  const drift = Math.cos((idx + 377) / 8) * 9;
  const edge = ((idx * 377) % 19) - 9;
...
function smooth377(input, radius) {
  const result = [];
...
function derive377(series, mode) {
  ...
  const spread = normalize377(peak - valley);
  const jitter = normalize377(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize377(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize377(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize377(avg),
    peak: normalize377(peak),
    valley: normalize377(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch377(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize377(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic377(initialSeed = 377) {
  ...
  useEffect(() => {
    ...
    fakeFetch377(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source377.map((value, idx) => {
      ...
      return normalize377(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth377(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize377(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive377(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize377(quality) });
    dispatch({ type: 'marker', payload: normalize377(flags.confidence * 0.7 + quality * 0.3) });
