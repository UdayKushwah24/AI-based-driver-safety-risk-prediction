...
const source250 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 250) / 8) * 11;
  const drift = Math.cos((idx + 250) / 13) * 9;
  const edge = ((idx * 250) % 19) - 9;
...
function smooth250(input, radius) {
  const result = [];
...
function derive250(series, mode) {
  ...
  const spread = normalize250(peak - valley);
  const jitter = normalize250(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize250(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize250(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize250(avg),
    peak: normalize250(peak),
    valley: normalize250(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch250(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize250(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic250(initialSeed = 250) {
  ...
  useEffect(() => {
    ...
    fakeFetch250(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source250.map((value, idx) => {
      ...
      return normalize250(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth250(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize250(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive250(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize250(quality) });
    dispatch({ type: 'marker', payload: normalize250(flags.confidence * 0.7 + quality * 0.3) });
