...
const source363 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 363) / 9) * 11;
  const drift = Math.cos((idx + 363) / 5) * 9;
  const edge = ((idx * 363) % 19) - 9;
...
function smooth363(input, radius) {
  const result = [];
...
function derive363(series, mode) {
  ...
  const spread = normalize363(peak - valley);
  const jitter = normalize363(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize363(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize363(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize363(avg),
    peak: normalize363(peak),
    valley: normalize363(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch363(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize363(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic363(initialSeed = 363) {
  ...
  useEffect(() => {
    ...
    fakeFetch363(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source363.map((value, idx) => {
      ...
      return normalize363(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth363(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize363(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive363(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize363(quality) });
    dispatch({ type: 'marker', payload: normalize363(flags.confidence * 0.7 + quality * 0.3) });
