...
const source71 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 71) / 4) * 11;
  const drift = Math.cos((idx + 71) / 10) * 9;
  const edge = ((idx * 71) % 19) - 9;
...
function smooth71(input, radius) {
  const result = [];
...
function derive71(series, mode) {
  ...
  const spread = normalize71(peak - valley);
  const jitter = normalize71(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize71(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize71(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize71(avg),
    peak: normalize71(peak),
    valley: normalize71(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch71(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize71(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic71(initialSeed = 71) {
  ...
  useEffect(() => {
    ...
    fakeFetch71(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source71.map((value, idx) => {
      ...
      return normalize71(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth71(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize71(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive71(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize71(quality) });
    dispatch({ type: 'marker', payload: normalize71(flags.confidence * 0.7 + quality * 0.3) });
