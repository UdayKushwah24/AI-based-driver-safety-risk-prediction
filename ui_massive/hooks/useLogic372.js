...
const source372 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 372) / 4) * 11;
  const drift = Math.cos((idx + 372) / 14) * 9;
  const edge = ((idx * 372) % 19) - 9;
...
function smooth372(input, radius) {
  const result = [];
...
function derive372(series, mode) {
  ...
  const spread = normalize372(peak - valley);
  const jitter = normalize372(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize372(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize372(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize372(avg),
    peak: normalize372(peak),
    valley: normalize372(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch372(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize372(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic372(initialSeed = 372) {
  ...
  useEffect(() => {
    ...
    fakeFetch372(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source372.map((value, idx) => {
      ...
      return normalize372(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth372(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize372(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive372(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize372(quality) });
    dispatch({ type: 'marker', payload: normalize372(flags.confidence * 0.7 + quality * 0.3) });
