...
const source28 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 28) / 3) * 11;
  const drift = Math.cos((idx + 28) / 11) * 9;
  const edge = ((idx * 28) % 19) - 9;
...
function smooth28(input, radius) {
  const result = [];
...
function derive28(series, mode) {
  ...
  const spread = normalize28(peak - valley);
  const jitter = normalize28(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize28(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize28(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize28(avg),
    peak: normalize28(peak),
    valley: normalize28(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch28(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize28(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic28(initialSeed = 28) {
  ...
  useEffect(() => {
    ...
    fakeFetch28(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source28.map((value, idx) => {
      ...
      return normalize28(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth28(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize28(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive28(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize28(quality) });
    dispatch({ type: 'marker', payload: normalize28(flags.confidence * 0.7 + quality * 0.3) });
