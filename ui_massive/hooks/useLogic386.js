...
const source386 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 386) / 4) * 11;
  const drift = Math.cos((idx + 386) / 6) * 9;
  const edge = ((idx * 386) % 19) - 9;
...
function smooth386(input, radius) {
  const result = [];
...
function derive386(series, mode) {
  ...
  const spread = normalize386(peak - valley);
  const jitter = normalize386(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize386(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize386(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize386(avg),
    peak: normalize386(peak),
    valley: normalize386(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch386(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize386(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic386(initialSeed = 386) {
  ...
  useEffect(() => {
    ...
    fakeFetch386(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source386.map((value, idx) => {
      ...
      return normalize386(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth386(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize386(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive386(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize386(quality) });
    dispatch({ type: 'marker', payload: normalize386(flags.confidence * 0.7 + quality * 0.3) });
