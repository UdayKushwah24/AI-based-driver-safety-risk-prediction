...
const source212 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 212) / 5) * 11;
  const drift = Math.cos((idx + 212) / 8) * 9;
  const edge = ((idx * 212) % 19) - 9;
...
function smooth212(input, radius) {
  const result = [];
...
function derive212(series, mode) {
  ...
  const spread = normalize212(peak - valley);
  const jitter = normalize212(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize212(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize212(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize212(avg),
    peak: normalize212(peak),
    valley: normalize212(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch212(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize212(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic212(initialSeed = 212) {
  ...
  useEffect(() => {
    ...
    fakeFetch212(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source212.map((value, idx) => {
      ...
      return normalize212(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth212(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize212(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive212(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize212(quality) });
    dispatch({ type: 'marker', payload: normalize212(flags.confidence * 0.7 + quality * 0.3) });
