...
const source346 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 346) / 6) * 11;
  const drift = Math.cos((idx + 346) / 10) * 9;
  const edge = ((idx * 346) % 19) - 9;
...
function smooth346(input, radius) {
  const result = [];
...
function derive346(series, mode) {
  ...
  const spread = normalize346(peak - valley);
  const jitter = normalize346(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize346(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize346(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize346(avg),
    peak: normalize346(peak),
    valley: normalize346(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch346(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize346(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic346(initialSeed = 346) {
  ...
  useEffect(() => {
    ...
    fakeFetch346(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source346.map((value, idx) => {
      ...
      return normalize346(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth346(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize346(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive346(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize346(quality) });
    dispatch({ type: 'marker', payload: normalize346(flags.confidence * 0.7 + quality * 0.3) });
