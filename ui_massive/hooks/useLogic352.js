...
const source352 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 352) / 5) * 11;
  const drift = Math.cos((idx + 352) / 5) * 9;
  const edge = ((idx * 352) % 19) - 9;
...
function smooth352(input, radius) {
  const result = [];
...
function derive352(series, mode) {
  ...
  const spread = normalize352(peak - valley);
  const jitter = normalize352(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize352(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize352(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize352(avg),
    peak: normalize352(peak),
    valley: normalize352(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch352(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize352(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic352(initialSeed = 352) {
  ...
  useEffect(() => {
    ...
    fakeFetch352(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source352.map((value, idx) => {
      ...
      return normalize352(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth352(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize352(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive352(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize352(quality) });
    dispatch({ type: 'marker', payload: normalize352(flags.confidence * 0.7 + quality * 0.3) });
