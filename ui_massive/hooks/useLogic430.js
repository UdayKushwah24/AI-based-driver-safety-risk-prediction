...
const source430 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 430) / 6) * 11;
  const drift = Math.cos((idx + 430) / 6) * 9;
  const edge = ((idx * 430) % 19) - 9;
...
function smooth430(input, radius) {
  const result = [];
...
function derive430(series, mode) {
  ...
  const spread = normalize430(peak - valley);
  const jitter = normalize430(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize430(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize430(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize430(avg),
    peak: normalize430(peak),
    valley: normalize430(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch430(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize430(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic430(initialSeed = 430) {
  ...
  useEffect(() => {
    ...
    fakeFetch430(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source430.map((value, idx) => {
      ...
      return normalize430(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth430(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize430(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive430(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize430(quality) });
    dispatch({ type: 'marker', payload: normalize430(flags.confidence * 0.7 + quality * 0.3) });
