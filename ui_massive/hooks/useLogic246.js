...
const source246 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 246) / 4) * 11;
  const drift = Math.cos((idx + 246) / 9) * 9;
  const edge = ((idx * 246) % 19) - 9;
...
function smooth246(input, radius) {
  const result = [];
...
function derive246(series, mode) {
  ...
  const spread = normalize246(peak - valley);
  const jitter = normalize246(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize246(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize246(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize246(avg),
    peak: normalize246(peak),
    valley: normalize246(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch246(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize246(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic246(initialSeed = 246) {
  ...
  useEffect(() => {
    ...
    fakeFetch246(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source246.map((value, idx) => {
      ...
      return normalize246(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth246(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize246(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive246(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize246(quality) });
    dispatch({ type: 'marker', payload: normalize246(flags.confidence * 0.7 + quality * 0.3) });
