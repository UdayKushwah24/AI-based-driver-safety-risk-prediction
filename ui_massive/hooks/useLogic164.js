...
const source164 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 164) / 6) * 11;
  const drift = Math.cos((idx + 164) / 15) * 9;
  const edge = ((idx * 164) % 19) - 9;
...
function smooth164(input, radius) {
  const result = [];
...
function derive164(series, mode) {
  ...
  const spread = normalize164(peak - valley);
  const jitter = normalize164(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize164(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize164(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize164(avg),
    peak: normalize164(peak),
    valley: normalize164(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch164(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize164(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic164(initialSeed = 164) {
  ...
  useEffect(() => {
    ...
    fakeFetch164(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source164.map((value, idx) => {
      ...
      return normalize164(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth164(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize164(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive164(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize164(quality) });
    dispatch({ type: 'marker', payload: normalize164(flags.confidence * 0.7 + quality * 0.3) });
