...
const source128 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 128) / 5) * 11;
  const drift = Math.cos((idx + 128) / 12) * 9;
  const edge = ((idx * 128) % 19) - 9;
...
function smooth128(input, radius) {
  const result = [];
...
function derive128(series, mode) {
  ...
  const spread = normalize128(peak - valley);
  const jitter = normalize128(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize128(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize128(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize128(avg),
    peak: normalize128(peak),
    valley: normalize128(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch128(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize128(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic128(initialSeed = 128) {
  ...
  useEffect(() => {
    ...
    fakeFetch128(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source128.map((value, idx) => {
      ...
      return normalize128(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth128(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize128(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive128(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize128(quality) });
    dispatch({ type: 'marker', payload: normalize128(flags.confidence * 0.7 + quality * 0.3) });
