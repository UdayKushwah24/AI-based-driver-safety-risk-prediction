...
const source336 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 336) / 3) * 11;
  const drift = Math.cos((idx + 336) / 11) * 9;
  const edge = ((idx * 336) % 19) - 9;
...
function smooth336(input, radius) {
  const result = [];
...
function derive336(series, mode) {
  ...
  const spread = normalize336(peak - valley);
  const jitter = normalize336(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize336(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize336(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize336(avg),
    peak: normalize336(peak),
    valley: normalize336(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch336(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize336(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic336(initialSeed = 336) {
  ...
  useEffect(() => {
    ...
    fakeFetch336(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source336.map((value, idx) => {
      ...
      return normalize336(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth336(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize336(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive336(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize336(quality) });
    dispatch({ type: 'marker', payload: normalize336(flags.confidence * 0.7 + quality * 0.3) });
