...
const source460 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 460) / 8) * 11;
  const drift = Math.cos((idx + 460) / 14) * 9;
  const edge = ((idx * 460) % 19) - 9;
...
function smooth460(input, radius) {
  const result = [];
...
function derive460(series, mode) {
  ...
  const spread = normalize460(peak - valley);
  const jitter = normalize460(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize460(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize460(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize460(avg),
    peak: normalize460(peak),
    valley: normalize460(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch460(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize460(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic460(initialSeed = 460) {
  ...
  useEffect(() => {
    ...
    fakeFetch460(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source460.map((value, idx) => {
      ...
      return normalize460(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth460(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize460(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive460(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize460(quality) });
    dispatch({ type: 'marker', payload: normalize460(flags.confidence * 0.7 + quality * 0.3) });
