...
const source477 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 477) / 4) * 11;
  const drift = Math.cos((idx + 477) / 9) * 9;
  const edge = ((idx * 477) % 19) - 9;
...
function smooth477(input, radius) {
  const result = [];
...
function derive477(series, mode) {
  ...
  const spread = normalize477(peak - valley);
  const jitter = normalize477(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize477(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize477(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize477(avg),
    peak: normalize477(peak),
    valley: normalize477(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch477(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize477(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic477(initialSeed = 477) {
  ...
  useEffect(() => {
    ...
    fakeFetch477(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source477.map((value, idx) => {
      ...
      return normalize477(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth477(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize477(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive477(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize477(quality) });
    dispatch({ type: 'marker', payload: normalize477(flags.confidence * 0.7 + quality * 0.3) });
