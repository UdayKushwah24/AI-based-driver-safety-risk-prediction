...
const source323 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 323) / 4) * 11;
  const drift = Math.cos((idx + 323) / 9) * 9;
  const edge = ((idx * 323) % 19) - 9;
...
function smooth323(input, radius) {
  const result = [];
...
function derive323(series, mode) {
  ...
  const spread = normalize323(peak - valley);
  const jitter = normalize323(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize323(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize323(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize323(avg),
    peak: normalize323(peak),
    valley: normalize323(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch323(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize323(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic323(initialSeed = 323) {
  ...
  useEffect(() => {
    ...
    fakeFetch323(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source323.map((value, idx) => {
      ...
      return normalize323(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth323(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize323(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive323(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize323(quality) });
    dispatch({ type: 'marker', payload: normalize323(flags.confidence * 0.7 + quality * 0.3) });
