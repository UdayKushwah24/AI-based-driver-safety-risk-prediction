...
const source351 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 351) / 4) * 11;
  const drift = Math.cos((idx + 351) / 15) * 9;
  const edge = ((idx * 351) % 19) - 9;
...
function smooth351(input, radius) {
  const result = [];
...
function derive351(series, mode) {
  ...
  const spread = normalize351(peak - valley);
  const jitter = normalize351(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize351(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize351(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize351(avg),
    peak: normalize351(peak),
    valley: normalize351(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch351(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize351(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic351(initialSeed = 351) {
  ...
  useEffect(() => {
    ...
    fakeFetch351(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source351.map((value, idx) => {
      ...
      return normalize351(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth351(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize351(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive351(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize351(quality) });
    dispatch({ type: 'marker', payload: normalize351(flags.confidence * 0.7 + quality * 0.3) });
