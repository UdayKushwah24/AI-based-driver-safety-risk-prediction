...
const source318 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 318) / 6) * 11;
  const drift = Math.cos((idx + 318) / 15) * 9;
  const edge = ((idx * 318) % 19) - 9;
...
function smooth318(input, radius) {
  const result = [];
...
function derive318(series, mode) {
  ...
  const spread = normalize318(peak - valley);
  const jitter = normalize318(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize318(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize318(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize318(avg),
    peak: normalize318(peak),
    valley: normalize318(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch318(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize318(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic318(initialSeed = 318) {
  ...
  useEffect(() => {
    ...
    fakeFetch318(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source318.map((value, idx) => {
      ...
      return normalize318(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth318(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize318(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive318(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize318(quality) });
    dispatch({ type: 'marker', payload: normalize318(flags.confidence * 0.7 + quality * 0.3) });
