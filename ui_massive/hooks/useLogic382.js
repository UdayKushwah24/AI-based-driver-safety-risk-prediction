...
const source382 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 382) / 7) * 11;
  const drift = Math.cos((idx + 382) / 13) * 9;
  const edge = ((idx * 382) % 19) - 9;
...
function smooth382(input, radius) {
  const result = [];
...
function derive382(series, mode) {
  ...
  const spread = normalize382(peak - valley);
  const jitter = normalize382(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize382(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize382(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize382(avg),
    peak: normalize382(peak),
    valley: normalize382(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch382(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize382(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic382(initialSeed = 382) {
  ...
  useEffect(() => {
    ...
    fakeFetch382(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source382.map((value, idx) => {
      ...
      return normalize382(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth382(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize382(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive382(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize382(quality) });
    dispatch({ type: 'marker', payload: normalize382(flags.confidence * 0.7 + quality * 0.3) });
