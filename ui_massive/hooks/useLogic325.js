...
const source325 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 325) / 6) * 11;
  const drift = Math.cos((idx + 325) / 11) * 9;
  const edge = ((idx * 325) % 19) - 9;
...
function smooth325(input, radius) {
  const result = [];
...
function derive325(series, mode) {
  ...
  const spread = normalize325(peak - valley);
  const jitter = normalize325(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize325(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize325(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize325(avg),
    peak: normalize325(peak),
    valley: normalize325(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch325(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize325(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic325(initialSeed = 325) {
  ...
  useEffect(() => {
    ...
    fakeFetch325(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source325.map((value, idx) => {
      ...
      return normalize325(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth325(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize325(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive325(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize325(quality) });
    dispatch({ type: 'marker', payload: normalize325(flags.confidence * 0.7 + quality * 0.3) });
