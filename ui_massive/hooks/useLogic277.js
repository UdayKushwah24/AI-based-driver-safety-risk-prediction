...
const source277 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 277) / 7) * 11;
  const drift = Math.cos((idx + 277) / 7) * 9;
  const edge = ((idx * 277) % 19) - 9;
...
function smooth277(input, radius) {
  const result = [];
...
function derive277(series, mode) {
  ...
  const spread = normalize277(peak - valley);
  const jitter = normalize277(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize277(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize277(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize277(avg),
    peak: normalize277(peak),
    valley: normalize277(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch277(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize277(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic277(initialSeed = 277) {
  ...
  useEffect(() => {
    ...
    fakeFetch277(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source277.map((value, idx) => {
      ...
      return normalize277(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth277(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize277(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive277(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize277(quality) });
    dispatch({ type: 'marker', payload: normalize277(flags.confidence * 0.7 + quality * 0.3) });
