...
const source64 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 64) / 4) * 11;
  const drift = Math.cos((idx + 64) / 14) * 9;
  const edge = ((idx * 64) % 19) - 9;
...
function smooth64(input, radius) {
  const result = [];
...
function derive64(series, mode) {
  ...
  const spread = normalize64(peak - valley);
  const jitter = normalize64(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize64(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize64(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize64(avg),
    peak: normalize64(peak),
    valley: normalize64(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch64(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize64(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic64(initialSeed = 64) {
  ...
  useEffect(() => {
    ...
    fakeFetch64(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source64.map((value, idx) => {
      ...
      return normalize64(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth64(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize64(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive64(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize64(quality) });
    dispatch({ type: 'marker', payload: normalize64(flags.confidence * 0.7 + quality * 0.3) });
