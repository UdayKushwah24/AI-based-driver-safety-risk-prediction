...
const source65 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 65) / 5) * 11;
  const drift = Math.cos((idx + 65) / 15) * 9;
  const edge = ((idx * 65) % 19) - 9;
...
function smooth65(input, radius) {
  const result = [];
...
function derive65(series, mode) {
  ...
  const spread = normalize65(peak - valley);
  const jitter = normalize65(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize65(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize65(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize65(avg),
    peak: normalize65(peak),
    valley: normalize65(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch65(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize65(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic65(initialSeed = 65) {
  ...
  useEffect(() => {
    ...
    fakeFetch65(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source65.map((value, idx) => {
      ...
      return normalize65(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth65(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize65(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive65(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize65(quality) });
    dispatch({ type: 'marker', payload: normalize65(flags.confidence * 0.7 + quality * 0.3) });
