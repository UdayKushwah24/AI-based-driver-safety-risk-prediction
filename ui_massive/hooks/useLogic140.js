...
const source140 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 140) / 3) * 11;
  const drift = Math.cos((idx + 140) / 13) * 9;
  const edge = ((idx * 140) % 19) - 9;
...
function smooth140(input, radius) {
  const result = [];
...
function derive140(series, mode) {
  ...
  const spread = normalize140(peak - valley);
  const jitter = normalize140(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize140(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize140(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize140(avg),
    peak: normalize140(peak),
    valley: normalize140(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch140(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize140(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic140(initialSeed = 140) {
  ...
  useEffect(() => {
    ...
    fakeFetch140(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source140.map((value, idx) => {
      ...
      return normalize140(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth140(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize140(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive140(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize140(quality) });
    dispatch({ type: 'marker', payload: normalize140(flags.confidence * 0.7 + quality * 0.3) });
