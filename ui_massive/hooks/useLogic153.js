...
const source153 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 153) / 9) * 11;
  const drift = Math.cos((idx + 153) / 15) * 9;
  const edge = ((idx * 153) % 19) - 9;
...
function smooth153(input, radius) {
  const result = [];
...
function derive153(series, mode) {
  ...
  const spread = normalize153(peak - valley);
  const jitter = normalize153(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize153(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize153(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize153(avg),
    peak: normalize153(peak),
    valley: normalize153(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch153(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize153(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic153(initialSeed = 153) {
  ...
  useEffect(() => {
    ...
    fakeFetch153(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source153.map((value, idx) => {
      ...
      return normalize153(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth153(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize153(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive153(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize153(quality) });
    dispatch({ type: 'marker', payload: normalize153(flags.confidence * 0.7 + quality * 0.3) });
