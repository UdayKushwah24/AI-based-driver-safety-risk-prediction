...
const source486 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 486) / 6) * 11;
  const drift = Math.cos((idx + 486) / 7) * 9;
  const edge = ((idx * 486) % 19) - 9;
...
function smooth486(input, radius) {
  const result = [];
...
function derive486(series, mode) {
  ...
  const spread = normalize486(peak - valley);
  const jitter = normalize486(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize486(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize486(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize486(avg),
    peak: normalize486(peak),
    valley: normalize486(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch486(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize486(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic486(initialSeed = 486) {
  ...
  useEffect(() => {
    ...
    fakeFetch486(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source486.map((value, idx) => {
      ...
      return normalize486(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth486(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize486(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive486(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize486(quality) });
    dispatch({ type: 'marker', payload: normalize486(flags.confidence * 0.7 + quality * 0.3) });
