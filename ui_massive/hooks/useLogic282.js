...
const source282 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 282) / 5) * 11;
  const drift = Math.cos((idx + 282) / 12) * 9;
  const edge = ((idx * 282) % 19) - 9;
...
function smooth282(input, radius) {
  const result = [];
...
function derive282(series, mode) {
  ...
  const spread = normalize282(peak - valley);
  const jitter = normalize282(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize282(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize282(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize282(avg),
    peak: normalize282(peak),
    valley: normalize282(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch282(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize282(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic282(initialSeed = 282) {
  ...
  useEffect(() => {
    ...
    fakeFetch282(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source282.map((value, idx) => {
      ...
      return normalize282(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth282(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize282(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive282(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize282(quality) });
    dispatch({ type: 'marker', payload: normalize282(flags.confidence * 0.7 + quality * 0.3) });
