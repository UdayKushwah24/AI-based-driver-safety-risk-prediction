...
const source454 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 454) / 9) * 11;
  const drift = Math.cos((idx + 454) / 8) * 9;
  const edge = ((idx * 454) % 19) - 9;
...
function smooth454(input, radius) {
  const result = [];
...
function derive454(series, mode) {
  ...
  const spread = normalize454(peak - valley);
  const jitter = normalize454(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize454(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize454(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize454(avg),
    peak: normalize454(peak),
    valley: normalize454(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch454(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize454(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic454(initialSeed = 454) {
  ...
  useEffect(() => {
    ...
    fakeFetch454(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source454.map((value, idx) => {
      ...
      return normalize454(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth454(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize454(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive454(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize454(quality) });
    dispatch({ type: 'marker', payload: normalize454(flags.confidence * 0.7 + quality * 0.3) });
