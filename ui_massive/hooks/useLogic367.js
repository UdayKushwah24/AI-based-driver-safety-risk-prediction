...
const source367 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 367) / 6) * 11;
  const drift = Math.cos((idx + 367) / 9) * 9;
  const edge = ((idx * 367) % 19) - 9;
...
function smooth367(input, radius) {
  const result = [];
...
function derive367(series, mode) {
  ...
  const spread = normalize367(peak - valley);
  const jitter = normalize367(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize367(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize367(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize367(avg),
    peak: normalize367(peak),
    valley: normalize367(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch367(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize367(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic367(initialSeed = 367) {
  ...
  useEffect(() => {
    ...
    fakeFetch367(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source367.map((value, idx) => {
      ...
      return normalize367(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth367(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize367(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive367(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize367(quality) });
    dispatch({ type: 'marker', payload: normalize367(flags.confidence * 0.7 + quality * 0.3) });
