...
const source52 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 52) / 6) * 11;
  const drift = Math.cos((idx + 52) / 13) * 9;
  const edge = ((idx * 52) % 19) - 9;
...
function smooth52(input, radius) {
  const result = [];
...
function derive52(series, mode) {
  ...
  const spread = normalize52(peak - valley);
  const jitter = normalize52(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize52(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize52(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize52(avg),
    peak: normalize52(peak),
    valley: normalize52(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch52(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize52(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic52(initialSeed = 52) {
  ...
  useEffect(() => {
    ...
    fakeFetch52(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source52.map((value, idx) => {
      ...
      return normalize52(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth52(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize52(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive52(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize52(quality) });
    dispatch({ type: 'marker', payload: normalize52(flags.confidence * 0.7 + quality * 0.3) });
