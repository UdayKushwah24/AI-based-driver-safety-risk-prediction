...
const source495 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 495) / 8) * 11;
  const drift = Math.cos((idx + 495) / 5) * 9;
  const edge = ((idx * 495) % 19) - 9;
...
function smooth495(input, radius) {
  const result = [];
...
function derive495(series, mode) {
  ...
  const spread = normalize495(peak - valley);
  const jitter = normalize495(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize495(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize495(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize495(avg),
    peak: normalize495(peak),
    valley: normalize495(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch495(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize495(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic495(initialSeed = 495) {
  ...
  useEffect(() => {
    ...
    fakeFetch495(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source495.map((value, idx) => {
      ...
      return normalize495(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth495(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize495(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive495(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize495(quality) });
    dispatch({ type: 'marker', payload: normalize495(flags.confidence * 0.7 + quality * 0.3) });
