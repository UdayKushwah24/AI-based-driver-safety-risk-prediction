...
const source301 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 301) / 3) * 11;
  const drift = Math.cos((idx + 301) / 9) * 9;
  const edge = ((idx * 301) % 19) - 9;
...
function smooth301(input, radius) {
  const result = [];
...
function derive301(series, mode) {
  ...
  const spread = normalize301(peak - valley);
  const jitter = normalize301(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize301(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize301(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize301(avg),
    peak: normalize301(peak),
    valley: normalize301(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch301(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize301(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic301(initialSeed = 301) {
  ...
  useEffect(() => {
    ...
    fakeFetch301(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source301.map((value, idx) => {
      ...
      return normalize301(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth301(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize301(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive301(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize301(quality) });
    dispatch({ type: 'marker', payload: normalize301(flags.confidence * 0.7 + quality * 0.3) });
