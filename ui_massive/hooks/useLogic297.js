...
const source297 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 297) / 6) * 11;
  const drift = Math.cos((idx + 297) / 5) * 9;
  const edge = ((idx * 297) % 19) - 9;
...
function smooth297(input, radius) {
  const result = [];
...
function derive297(series, mode) {
  ...
  const spread = normalize297(peak - valley);
  const jitter = normalize297(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize297(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize297(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize297(avg),
    peak: normalize297(peak),
    valley: normalize297(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch297(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize297(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic297(initialSeed = 297) {
  ...
  useEffect(() => {
    ...
    fakeFetch297(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source297.map((value, idx) => {
      ...
      return normalize297(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth297(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize297(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive297(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize297(quality) });
    dispatch({ type: 'marker', payload: normalize297(flags.confidence * 0.7 + quality * 0.3) });
