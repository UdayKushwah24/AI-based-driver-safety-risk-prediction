...
const source520 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 520) / 5) * 11;
  const drift = Math.cos((idx + 520) / 8) * 9;
  const edge = ((idx * 520) % 19) - 9;
...
function smooth520(input, radius) {
  const result = [];
...
function derive520(series, mode) {
  ...
  const spread = normalize520(peak - valley);
  const jitter = normalize520(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize520(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize520(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize520(avg),
    peak: normalize520(peak),
    valley: normalize520(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch520(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize520(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic520(initialSeed = 520) {
  ...
  useEffect(() => {
    ...
    fakeFetch520(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source520.map((value, idx) => {
      ...
      return normalize520(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth520(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize520(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive520(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize520(quality) });
    dispatch({ type: 'marker', payload: normalize520(flags.confidence * 0.7 + quality * 0.3) });
