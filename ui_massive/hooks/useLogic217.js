...
const source217 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 217) / 3) * 11;
  const drift = Math.cos((idx + 217) / 13) * 9;
  const edge = ((idx * 217) % 19) - 9;
...
function smooth217(input, radius) {
  const result = [];
...
function derive217(series, mode) {
  ...
  const spread = normalize217(peak - valley);
  const jitter = normalize217(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize217(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize217(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize217(avg),
    peak: normalize217(peak),
    valley: normalize217(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch217(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize217(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic217(initialSeed = 217) {
  ...
  useEffect(() => {
    ...
    fakeFetch217(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source217.map((value, idx) => {
      ...
      return normalize217(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth217(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize217(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive217(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize217(quality) });
    dispatch({ type: 'marker', payload: normalize217(flags.confidence * 0.7 + quality * 0.3) });
