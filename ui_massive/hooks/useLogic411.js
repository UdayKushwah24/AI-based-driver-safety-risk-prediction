...
const source411 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 411) / 8) * 11;
  const drift = Math.cos((idx + 411) / 9) * 9;
  const edge = ((idx * 411) % 19) - 9;
...
function smooth411(input, radius) {
  const result = [];
...
function derive411(series, mode) {
  ...
  const spread = normalize411(peak - valley);
  const jitter = normalize411(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize411(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize411(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize411(avg),
    peak: normalize411(peak),
    valley: normalize411(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch411(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize411(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic411(initialSeed = 411) {
  ...
  useEffect(() => {
    ...
    fakeFetch411(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source411.map((value, idx) => {
      ...
      return normalize411(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth411(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize411(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive411(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize411(quality) });
    dispatch({ type: 'marker', payload: normalize411(flags.confidence * 0.7 + quality * 0.3) });
