...
const source116 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 116) / 7) * 11;
  const drift = Math.cos((idx + 116) / 11) * 9;
  const edge = ((idx * 116) % 19) - 9;
...
function smooth116(input, radius) {
  const result = [];
...
function derive116(series, mode) {
  ...
  const spread = normalize116(peak - valley);
  const jitter = normalize116(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize116(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize116(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize116(avg),
    peak: normalize116(peak),
    valley: normalize116(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch116(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize116(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic116(initialSeed = 116) {
  ...
  useEffect(() => {
    ...
    fakeFetch116(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source116.map((value, idx) => {
      ...
      return normalize116(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth116(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize116(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive116(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize116(quality) });
    dispatch({ type: 'marker', payload: normalize116(flags.confidence * 0.7 + quality * 0.3) });
