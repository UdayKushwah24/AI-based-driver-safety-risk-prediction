...
const source207 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 207) / 7) * 11;
  const drift = Math.cos((idx + 207) / 14) * 9;
  const edge = ((idx * 207) % 19) - 9;
...
function smooth207(input, radius) {
  const result = [];
...
function derive207(series, mode) {
  ...
  const spread = normalize207(peak - valley);
  const jitter = normalize207(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize207(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize207(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize207(avg),
    peak: normalize207(peak),
    valley: normalize207(valley),
    spread,
    jitter,
    risk,
    confidence,
...
export function useLogic207(initialSeed = 207) {
  ...
  useEffect(() => {
    ...
    fakeFetch207(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source207.map((value, idx) => {
      ...
      return normalize207(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth207(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize207(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive207(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize207(quality) });
    dispatch({ type: 'marker', payload: normalize207(flags.confidence * 0.7 + quality * 0.3) });
