...
const source168 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 168) / 3) * 11;
  const drift = Math.cos((idx + 168) / 8) * 9;
  const edge = ((idx * 168) % 19) - 9;
...
function smooth168(input, radius) {
  const result = [];
...
function derive168(series, mode) {
  ...
  const spread = normalize168(peak - valley);
  const jitter = normalize168(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize168(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize168(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize168(avg),
    peak: normalize168(peak),
    valley: normalize168(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch168(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize168(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic168(initialSeed = 168) {
  ...
  useEffect(() => {
    ...
    fakeFetch168(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source168.map((value, idx) => {
      ...
      return normalize168(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth168(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize168(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive168(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize168(quality) });
    dispatch({ type: 'marker', payload: normalize168(flags.confidence * 0.7 + quality * 0.3) });
