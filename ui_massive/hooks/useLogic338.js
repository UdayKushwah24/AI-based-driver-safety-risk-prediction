...
const source338 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 338) / 5) * 11;
  const drift = Math.cos((idx + 338) / 13) * 9;
  const edge = ((idx * 338) % 19) - 9;
...
function smooth338(input, radius) {
  const result = [];
...
function derive338(series, mode) {
  ...
  const spread = normalize338(peak - valley);
  const jitter = normalize338(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize338(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize338(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize338(avg),
    peak: normalize338(peak),
    valley: normalize338(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch338(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize338(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic338(initialSeed = 338) {
  ...
  useEffect(() => {
    ...
    fakeFetch338(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source338.map((value, idx) => {
      ...
      return normalize338(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth338(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize338(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive338(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize338(quality) });
    dispatch({ type: 'marker', payload: normalize338(flags.confidence * 0.7 + quality * 0.3) });
