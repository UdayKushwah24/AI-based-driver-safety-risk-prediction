...
const source361 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 361) / 7) * 11;
  const drift = Math.cos((idx + 361) / 14) * 9;
  const edge = ((idx * 361) % 19) - 9;
...
function smooth361(input, radius) {
  const result = [];
...
function derive361(series, mode) {
  ...
  const spread = normalize361(peak - valley);
  const jitter = normalize361(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize361(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize361(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize361(avg),
    peak: normalize361(peak),
    valley: normalize361(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch361(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize361(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic361(initialSeed = 361) {
  ...
  useEffect(() => {
    ...
    fakeFetch361(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source361.map((value, idx) => {
      ...
      return normalize361(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth361(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize361(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive361(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize361(quality) });
    dispatch({ type: 'marker', payload: normalize361(flags.confidence * 0.7 + quality * 0.3) });
