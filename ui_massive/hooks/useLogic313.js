...
const source313 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 313) / 8) * 11;
  const drift = Math.cos((idx + 313) / 10) * 9;
  const edge = ((idx * 313) % 19) - 9;
...
function smooth313(input, radius) {
  const result = [];
...
function derive313(series, mode) {
  ...
  const spread = normalize313(peak - valley);
  const jitter = normalize313(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize313(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize313(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize313(avg),
    peak: normalize313(peak),
    valley: normalize313(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch313(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize313(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic313(initialSeed = 313) {
  ...
  useEffect(() => {
    ...
    fakeFetch313(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source313.map((value, idx) => {
      ...
      return normalize313(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth313(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize313(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive313(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize313(quality) });
    dispatch({ type: 'marker', payload: normalize313(flags.confidence * 0.7 + quality * 0.3) });
