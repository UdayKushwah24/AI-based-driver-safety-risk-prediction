...
const source144 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 144) / 7) * 11;
  const drift = Math.cos((idx + 144) / 6) * 9;
  const edge = ((idx * 144) % 19) - 9;
...
function smooth144(input, radius) {
  const result = [];
...
function derive144(series, mode) {
  ...
  const spread = normalize144(peak - valley);
  const jitter = normalize144(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize144(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize144(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize144(avg),
    peak: normalize144(peak),
    valley: normalize144(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch144(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize144(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic144(initialSeed = 144) {
  ...
  useEffect(() => {
    ...
    fakeFetch144(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source144.map((value, idx) => {
      ...
      return normalize144(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth144(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize144(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive144(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize144(quality) });
    dispatch({ type: 'marker', payload: normalize144(flags.confidence * 0.7 + quality * 0.3) });
