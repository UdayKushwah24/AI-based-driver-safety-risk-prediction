...
const source88 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 88) / 7) * 11;
  const drift = Math.cos((idx + 88) / 5) * 9;
  const edge = ((idx * 88) % 19) - 9;
...
function smooth88(input, radius) {
  const result = [];
...
function derive88(series, mode) {
  ...
  const spread = normalize88(peak - valley);
  const jitter = normalize88(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize88(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize88(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize88(avg),
    peak: normalize88(peak),
    valley: normalize88(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch88(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize88(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic88(initialSeed = 88) {
  ...
  useEffect(() => {
    ...
    fakeFetch88(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source88.map((value, idx) => {
      ...
      return normalize88(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth88(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize88(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive88(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize88(quality) });
    dispatch({ type: 'marker', payload: normalize88(flags.confidence * 0.7 + quality * 0.3) });
