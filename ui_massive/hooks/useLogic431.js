...
const source431 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 431) / 7) * 11;
  const drift = Math.cos((idx + 431) / 7) * 9;
  const edge = ((idx * 431) % 19) - 9;
...
function smooth431(input, radius) {
  const result = [];
...
function derive431(series, mode) {
  ...
  const spread = normalize431(peak - valley);
  const jitter = normalize431(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize431(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize431(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize431(avg),
    peak: normalize431(peak),
    valley: normalize431(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch431(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize431(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic431(initialSeed = 431) {
  ...
  useEffect(() => {
    ...
    fakeFetch431(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source431.map((value, idx) => {
      ...
      return normalize431(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth431(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize431(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive431(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize431(quality) });
    dispatch({ type: 'marker', payload: normalize431(flags.confidence * 0.7 + quality * 0.3) });
