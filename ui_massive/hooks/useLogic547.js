...
const source547 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 547) / 4) * 11;
  const drift = Math.cos((idx + 547) / 13) * 9;
  const edge = ((idx * 547) % 19) - 9;
...
function smooth547(input, radius) {
  const result = [];
...
function derive547(series, mode) {
  ...
  const spread = normalize547(peak - valley);
  const jitter = normalize547(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize547(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize547(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize547(avg),
    peak: normalize547(peak),
    valley: normalize547(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch547(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize547(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic547(initialSeed = 547) {
  ...
  useEffect(() => {
    ...
    fakeFetch547(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source547.map((value, idx) => {
      ...
      return normalize547(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth547(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize547(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive547(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize547(quality) });
    dispatch({ type: 'marker', payload: normalize547(flags.confidence * 0.7 + quality * 0.3) });
