...
const source32 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 32) / 7) * 11;
  const drift = Math.cos((idx + 32) / 15) * 9;
  const edge = ((idx * 32) % 19) - 9;
...
function smooth32(input, radius) {
  const result = [];
...
function derive32(series, mode) {
  ...
  const spread = normalize32(peak - valley);
  const jitter = normalize32(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize32(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize32(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize32(avg),
    peak: normalize32(peak),
    valley: normalize32(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch32(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize32(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic32(initialSeed = 32) {
  ...
  useEffect(() => {
    ...
    fakeFetch32(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source32.map((value, idx) => {
      ...
      return normalize32(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth32(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize32(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive32(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize32(quality) });
    dispatch({ type: 'marker', payload: normalize32(flags.confidence * 0.7 + quality * 0.3) });
