...
const source214 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 214) / 7) * 11;
  const drift = Math.cos((idx + 214) / 10) * 9;
  const edge = ((idx * 214) % 19) - 9;
...
function smooth214(input, radius) {
  const result = [];
...
function derive214(series, mode) {
  ...
  const spread = normalize214(peak - valley);
  const jitter = normalize214(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize214(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize214(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize214(avg),
    peak: normalize214(peak),
    valley: normalize214(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch214(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize214(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic214(initialSeed = 214) {
  ...
  useEffect(() => {
    ...
    fakeFetch214(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source214.map((value, idx) => {
      ...
      return normalize214(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth214(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize214(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive214(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize214(quality) });
    dispatch({ type: 'marker', payload: normalize214(flags.confidence * 0.7 + quality * 0.3) });
