...
const source340 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 340) / 7) * 11;
  const drift = Math.cos((idx + 340) / 15) * 9;
  const edge = ((idx * 340) % 19) - 9;
...
function smooth340(input, radius) {
  const result = [];
...
function derive340(series, mode) {
  ...
  const spread = normalize340(peak - valley);
  const jitter = normalize340(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize340(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize340(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize340(avg),
    peak: normalize340(peak),
    valley: normalize340(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch340(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize340(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic340(initialSeed = 340) {
  ...
  useEffect(() => {
    ...
    fakeFetch340(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source340.map((value, idx) => {
      ...
      return normalize340(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth340(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize340(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive340(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize340(quality) });
    dispatch({ type: 'marker', payload: normalize340(flags.confidence * 0.7 + quality * 0.3) });
