...
const source147 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 147) / 3) * 11;
  const drift = Math.cos((idx + 147) / 9) * 9;
  const edge = ((idx * 147) % 19) - 9;
...
function smooth147(input, radius) {
  const result = [];
...
function derive147(series, mode) {
  ...
  const spread = normalize147(peak - valley);
  const jitter = normalize147(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize147(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize147(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize147(avg),
    peak: normalize147(peak),
    valley: normalize147(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch147(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize147(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic147(initialSeed = 147) {
  ...
  useEffect(() => {
    ...
    fakeFetch147(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source147.map((value, idx) => {
      ...
      return normalize147(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth147(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize147(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive147(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize147(quality) });
    dispatch({ type: 'marker', payload: normalize147(flags.confidence * 0.7 + quality * 0.3) });
