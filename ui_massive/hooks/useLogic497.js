...
const source497 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 497) / 3) * 11;
  const drift = Math.cos((idx + 497) / 7) * 9;
  const edge = ((idx * 497) % 19) - 9;
...
function smooth497(input, radius) {
  const result = [];
...
function derive497(series, mode) {
  ...
  const spread = normalize497(peak - valley);
  const jitter = normalize497(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize497(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize497(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize497(avg),
    peak: normalize497(peak),
    valley: normalize497(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch497(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize497(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic497(initialSeed = 497) {
  ...
  useEffect(() => {
    ...
    fakeFetch497(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source497.map((value, idx) => {
      ...
      return normalize497(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth497(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize497(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive497(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize497(quality) });
    dispatch({ type: 'marker', payload: normalize497(flags.confidence * 0.7 + quality * 0.3) });
