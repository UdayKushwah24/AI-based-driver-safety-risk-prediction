...
const source399 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 399) / 3) * 11;
  const drift = Math.cos((idx + 399) / 8) * 9;
  const edge = ((idx * 399) % 19) - 9;
...
function smooth399(input, radius) {
  const result = [];
...
function derive399(series, mode) {
  ...
  const spread = normalize399(peak - valley);
  const jitter = normalize399(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize399(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize399(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize399(avg),
    peak: normalize399(peak),
    valley: normalize399(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch399(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize399(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic399(initialSeed = 399) {
  ...
  useEffect(() => {
    ...
    fakeFetch399(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source399.map((value, idx) => {
      ...
      return normalize399(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth399(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize399(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive399(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize399(quality) });
    dispatch({ type: 'marker', payload: normalize399(flags.confidence * 0.7 + quality * 0.3) });
