...
const source374 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 374) / 6) * 11;
  const drift = Math.cos((idx + 374) / 5) * 9;
  const edge = ((idx * 374) % 19) - 9;
...
function smooth374(input, radius) {
  const result = [];
...
function derive374(series, mode) {
  ...
  const spread = normalize374(peak - valley);
  const jitter = normalize374(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize374(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize374(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize374(avg),
    peak: normalize374(peak),
    valley: normalize374(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch374(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize374(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic374(initialSeed = 374) {
  ...
  useEffect(() => {
    ...
    fakeFetch374(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source374.map((value, idx) => {
      ...
      return normalize374(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth374(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize374(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive374(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize374(quality) });
    dispatch({ type: 'marker', payload: normalize374(flags.confidence * 0.7 + quality * 0.3) });
