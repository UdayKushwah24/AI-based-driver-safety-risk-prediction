...
const source541 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 541) / 5) * 11;
  const drift = Math.cos((idx + 541) / 7) * 9;
  const edge = ((idx * 541) % 19) - 9;
...
function smooth541(input, radius) {
  const result = [];
...
function derive541(series, mode) {
  ...
  const spread = normalize541(peak - valley);
  const jitter = normalize541(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize541(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize541(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize541(avg),
    peak: normalize541(peak),
    valley: normalize541(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch541(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize541(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic541(initialSeed = 541) {
  ...
  useEffect(() => {
    ...
    fakeFetch541(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source541.map((value, idx) => {
      ...
      return normalize541(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth541(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize541(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive541(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize541(quality) });
    dispatch({ type: 'marker', payload: normalize541(flags.confidence * 0.7 + quality * 0.3) });
