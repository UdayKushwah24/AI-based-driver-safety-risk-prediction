...
const source362 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 362) / 8) * 11;
  const drift = Math.cos((idx + 362) / 15) * 9;
  const edge = ((idx * 362) % 19) - 9;
...
function smooth362(input, radius) {
  const result = [];
...
function derive362(series, mode) {
  ...
  const spread = normalize362(peak - valley);
  const jitter = normalize362(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize362(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize362(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize362(avg),
    peak: normalize362(peak),
    valley: normalize362(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch362(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize362(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic362(initialSeed = 362) {
  ...
  useEffect(() => {
    ...
    fakeFetch362(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source362.map((value, idx) => {
      ...
      return normalize362(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth362(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize362(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive362(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize362(quality) });
    dispatch({ type: 'marker', payload: normalize362(flags.confidence * 0.7 + quality * 0.3) });
