...
const source146 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 146) / 9) * 11;
  const drift = Math.cos((idx + 146) / 8) * 9;
  const edge = ((idx * 146) % 19) - 9;
...
function smooth146(input, radius) {
  const result = [];
...
function derive146(series, mode) {
  ...
  const spread = normalize146(peak - valley);
  const jitter = normalize146(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize146(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize146(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize146(avg),
    peak: normalize146(peak),
    valley: normalize146(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch146(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize146(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic146(initialSeed = 146) {
  ...
  useEffect(() => {
    ...
    fakeFetch146(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source146.map((value, idx) => {
      ...
      return normalize146(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth146(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize146(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive146(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize146(quality) });
    dispatch({ type: 'marker', payload: normalize146(flags.confidence * 0.7 + quality * 0.3) });
