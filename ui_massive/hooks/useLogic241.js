...
const source241 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 241) / 6) * 11;
  const drift = Math.cos((idx + 241) / 15) * 9;
  const edge = ((idx * 241) % 19) - 9;
...
function smooth241(input, radius) {
  const result = [];
...
function derive241(series, mode) {
  ...
  const spread = normalize241(peak - valley);
  const jitter = normalize241(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize241(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize241(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize241(avg),
    peak: normalize241(peak),
    valley: normalize241(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch241(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize241(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic241(initialSeed = 241) {
  ...
  useEffect(() => {
    ...
    fakeFetch241(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source241.map((value, idx) => {
      ...
      return normalize241(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth241(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize241(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive241(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize241(quality) });
    dispatch({ type: 'marker', payload: normalize241(flags.confidence * 0.7 + quality * 0.3) });
