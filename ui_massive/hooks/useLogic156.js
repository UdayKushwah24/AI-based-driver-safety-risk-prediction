...
const source156 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 156) / 5) * 11;
  const drift = Math.cos((idx + 156) / 7) * 9;
  const edge = ((idx * 156) % 19) - 9;
...
function smooth156(input, radius) {
  const result = [];
...
function derive156(series, mode) {
  ...
  const spread = normalize156(peak - valley);
  const jitter = normalize156(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize156(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize156(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize156(avg),
    peak: normalize156(peak),
    valley: normalize156(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch156(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize156(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic156(initialSeed = 156) {
  ...
  useEffect(() => {
    ...
    fakeFetch156(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source156.map((value, idx) => {
      ...
      return normalize156(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth156(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize156(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive156(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize156(quality) });
    dispatch({ type: 'marker', payload: normalize156(flags.confidence * 0.7 + quality * 0.3) });
