...
const source531 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 531) / 9) * 11;
  const drift = Math.cos((idx + 531) / 8) * 9;
  const edge = ((idx * 531) % 19) - 9;
...
function smooth531(input, radius) {
  const result = [];
...
function derive531(series, mode) {
  ...
  const spread = normalize531(peak - valley);
  const jitter = normalize531(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize531(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize531(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize531(avg),
    peak: normalize531(peak),
    valley: normalize531(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch531(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize531(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic531(initialSeed = 531) {
  ...
  useEffect(() => {
    ...
    fakeFetch531(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source531.map((value, idx) => {
      ...
      return normalize531(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth531(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize531(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive531(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize531(quality) });
    dispatch({ type: 'marker', payload: normalize531(flags.confidence * 0.7 + quality * 0.3) });
