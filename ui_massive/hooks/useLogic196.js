...
const source196 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 196) / 3) * 11;
  const drift = Math.cos((idx + 196) / 14) * 9;
  const edge = ((idx * 196) % 19) - 9;
...
function smooth196(input, radius) {
  const result = [];
...
function derive196(series, mode) {
  ...
  const spread = normalize196(peak - valley);
  const jitter = normalize196(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize196(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize196(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize196(avg),
    peak: normalize196(peak),
    valley: normalize196(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch196(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize196(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic196(initialSeed = 196) {
  ...
  useEffect(() => {
    ...
    fakeFetch196(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source196.map((value, idx) => {
      ...
      return normalize196(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth196(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize196(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive196(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize196(quality) });
    dispatch({ type: 'marker', payload: normalize196(flags.confidence * 0.7 + quality * 0.3) });
