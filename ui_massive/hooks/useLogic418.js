...
const source418 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 418) / 8) * 11;
  const drift = Math.cos((idx + 418) / 5) * 9;
  const edge = ((idx * 418) % 19) - 9;
...
function smooth418(input, radius) {
  const result = [];
...
function derive418(series, mode) {
  ...
  const spread = normalize418(peak - valley);
  const jitter = normalize418(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize418(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize418(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize418(avg),
    peak: normalize418(peak),
    valley: normalize418(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch418(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize418(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic418(initialSeed = 418) {
  ...
  useEffect(() => {
    ...
    fakeFetch418(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source418.map((value, idx) => {
      ...
      return normalize418(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth418(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize418(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive418(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize418(quality) });
    dispatch({ type: 'marker', payload: normalize418(flags.confidence * 0.7 + quality * 0.3) });
