...
const source371 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 371) / 3) * 11;
  const drift = Math.cos((idx + 371) / 13) * 9;
  const edge = ((idx * 371) % 19) - 9;
...
function smooth371(input, radius) {
  const result = [];
...
function derive371(series, mode) {
  ...
  const spread = normalize371(peak - valley);
  const jitter = normalize371(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize371(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize371(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize371(avg),
    peak: normalize371(peak),
    valley: normalize371(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch371(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize371(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic371(initialSeed = 371) {
  ...
  useEffect(() => {
    ...
    fakeFetch371(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source371.map((value, idx) => {
      ...
      return normalize371(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth371(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize371(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive371(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize371(quality) });
    dispatch({ type: 'marker', payload: normalize371(flags.confidence * 0.7 + quality * 0.3) });
