...
const source286 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 286) / 9) * 11;
  const drift = Math.cos((idx + 286) / 5) * 9;
  const edge = ((idx * 286) % 19) - 9;
...
function smooth286(input, radius) {
  const result = [];
...
function derive286(series, mode) {
  ...
  const spread = normalize286(peak - valley);
  const jitter = normalize286(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize286(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize286(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize286(avg),
    peak: normalize286(peak),
    valley: normalize286(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch286(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize286(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic286(initialSeed = 286) {
  ...
  useEffect(() => {
    ...
    fakeFetch286(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source286.map((value, idx) => {
      ...
      return normalize286(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth286(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize286(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive286(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize286(quality) });
    dispatch({ type: 'marker', payload: normalize286(flags.confidence * 0.7 + quality * 0.3) });
