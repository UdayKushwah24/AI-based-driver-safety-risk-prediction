...
const source16 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 16) / 5) * 11;
  const drift = Math.cos((idx + 16) / 10) * 9;
  const edge = ((idx * 16) % 19) - 9;
...
function smooth16(input, radius) {
  const result = [];
...
function derive16(series, mode) {
  ...
  const spread = normalize16(peak - valley);
  const jitter = normalize16(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize16(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize16(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize16(avg),
    peak: normalize16(peak),
    valley: normalize16(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch16(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize16(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic16(initialSeed = 16) {
  ...
  useEffect(() => {
    ...
    fakeFetch16(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source16.map((value, idx) => {
      ...
      return normalize16(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth16(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize16(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive16(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize16(quality) });
    dispatch({ type: 'marker', payload: normalize16(flags.confidence * 0.7 + quality * 0.3) });
