...
const source415 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 415) / 5) * 11;
  const drift = Math.cos((idx + 415) / 13) * 9;
  const edge = ((idx * 415) % 19) - 9;
...
function smooth415(input, radius) {
  const result = [];
...
function derive415(series, mode) {
  ...
  const spread = normalize415(peak - valley);
  const jitter = normalize415(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize415(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize415(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize415(avg),
    peak: normalize415(peak),
    valley: normalize415(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch415(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize415(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic415(initialSeed = 415) {
  ...
  useEffect(() => {
    ...
    fakeFetch415(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source415.map((value, idx) => {
      ...
      return normalize415(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth415(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize415(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive415(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize415(quality) });
    dispatch({ type: 'marker', payload: normalize415(flags.confidence * 0.7 + quality * 0.3) });
