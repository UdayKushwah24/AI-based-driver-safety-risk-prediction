...
const source98 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 98) / 3) * 11;
  const drift = Math.cos((idx + 98) / 15) * 9;
  const edge = ((idx * 98) % 19) - 9;
...
function smooth98(input, radius) {
  const result = [];
...
function derive98(series, mode) {
  ...
  const spread = normalize98(peak - valley);
  const jitter = normalize98(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize98(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize98(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize98(avg),
    peak: normalize98(peak),
    valley: normalize98(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch98(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize98(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic98(initialSeed = 98) {
  ...
  useEffect(() => {
    ...
    fakeFetch98(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source98.map((value, idx) => {
      ...
      return normalize98(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth98(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize98(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive98(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize98(quality) });
    dispatch({ type: 'marker', payload: normalize98(flags.confidence * 0.7 + quality * 0.3) });
