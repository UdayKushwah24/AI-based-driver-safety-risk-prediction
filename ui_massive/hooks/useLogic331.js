...
const source331 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 331) / 5) * 11;
  const drift = Math.cos((idx + 331) / 6) * 9;
  const edge = ((idx * 331) % 19) - 9;
...
function smooth331(input, radius) {
  const result = [];
...
function derive331(series, mode) {
  ...
  const spread = normalize331(peak - valley);
  const jitter = normalize331(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize331(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize331(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize331(avg),
    peak: normalize331(peak),
    valley: normalize331(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch331(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize331(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic331(initialSeed = 331) {
  ...
  useEffect(() => {
    ...
    fakeFetch331(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source331.map((value, idx) => {
      ...
      return normalize331(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth331(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize331(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive331(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize331(quality) });
    dispatch({ type: 'marker', payload: normalize331(flags.confidence * 0.7 + quality * 0.3) });
