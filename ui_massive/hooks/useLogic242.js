...
const source242 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 242) / 7) * 11;
  const drift = Math.cos((idx + 242) / 5) * 9;
  const edge = ((idx * 242) % 19) - 9;
...
function smooth242(input, radius) {
  const result = [];
...
function derive242(series, mode) {
  ...
  const spread = normalize242(peak - valley);
  const jitter = normalize242(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize242(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize242(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize242(avg),
    peak: normalize242(peak),
    valley: normalize242(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch242(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize242(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic242(initialSeed = 242) {
  ...
  useEffect(() => {
    ...
    fakeFetch242(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source242.map((value, idx) => {
      ...
      return normalize242(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth242(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize242(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive242(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize242(quality) });
    dispatch({ type: 'marker', payload: normalize242(flags.confidence * 0.7 + quality * 0.3) });
