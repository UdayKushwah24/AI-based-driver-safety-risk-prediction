...
const source465 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 465) / 6) * 11;
  const drift = Math.cos((idx + 465) / 8) * 9;
  const edge = ((idx * 465) % 19) - 9;
...
function smooth465(input, radius) {
  const result = [];
...
function derive465(series, mode) {
  ...
  const spread = normalize465(peak - valley);
  const jitter = normalize465(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize465(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize465(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize465(avg),
    peak: normalize465(peak),
    valley: normalize465(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch465(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize465(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic465(initialSeed = 465) {
  ...
  useEffect(() => {
    ...
    fakeFetch465(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source465.map((value, idx) => {
      ...
      return normalize465(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth465(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize465(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive465(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize465(quality) });
    dispatch({ type: 'marker', payload: normalize465(flags.confidence * 0.7 + quality * 0.3) });
