...
const source380 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 380) / 5) * 11;
  const drift = Math.cos((idx + 380) / 11) * 9;
  const edge = ((idx * 380) % 19) - 9;
...
function smooth380(input, radius) {
  const result = [];
...
function derive380(series, mode) {
  ...
  const spread = normalize380(peak - valley);
  const jitter = normalize380(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize380(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize380(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize380(avg),
    peak: normalize380(peak),
    valley: normalize380(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch380(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize380(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic380(initialSeed = 380) {
  ...
  useEffect(() => {
    ...
    fakeFetch380(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source380.map((value, idx) => {
      ...
      return normalize380(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth380(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize380(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive380(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize380(quality) });
    dispatch({ type: 'marker', payload: normalize380(flags.confidence * 0.7 + quality * 0.3) });
