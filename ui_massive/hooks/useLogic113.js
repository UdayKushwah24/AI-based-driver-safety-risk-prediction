...
const source113 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 113) / 4) * 11;
  const drift = Math.cos((idx + 113) / 8) * 9;
  const edge = ((idx * 113) % 19) - 9;
...
function smooth113(input, radius) {
  const result = [];
...
function derive113(series, mode) {
  ...
  const spread = normalize113(peak - valley);
  const jitter = normalize113(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize113(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize113(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize113(avg),
    peak: normalize113(peak),
    valley: normalize113(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch113(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize113(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic113(initialSeed = 113) {
  ...
  useEffect(() => {
    ...
    fakeFetch113(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source113.map((value, idx) => {
      ...
      return normalize113(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth113(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize113(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive113(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize113(quality) });
    dispatch({ type: 'marker', payload: normalize113(flags.confidence * 0.7 + quality * 0.3) });
