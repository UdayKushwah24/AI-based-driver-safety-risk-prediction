...
const source511 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 511) / 3) * 11;
  const drift = Math.cos((idx + 511) / 10) * 9;
  const edge = ((idx * 511) % 19) - 9;
...
function smooth511(input, radius) {
  const result = [];
...
function derive511(series, mode) {
  ...
  const spread = normalize511(peak - valley);
  const jitter = normalize511(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize511(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize511(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize511(avg),
    peak: normalize511(peak),
    valley: normalize511(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch511(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize511(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic511(initialSeed = 511) {
  ...
  useEffect(() => {
    ...
    fakeFetch511(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source511.map((value, idx) => {
      ...
      return normalize511(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth511(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize511(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive511(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize511(quality) });
    dispatch({ type: 'marker', payload: normalize511(flags.confidence * 0.7 + quality * 0.3) });
