...
const source85 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 85) / 4) * 11;
  const drift = Math.cos((idx + 85) / 13) * 9;
  const edge = ((idx * 85) % 19) - 9;
...
function smooth85(input, radius) {
  const result = [];
...
function derive85(series, mode) {
  ...
  const spread = normalize85(peak - valley);
  const jitter = normalize85(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize85(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize85(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize85(avg),
    peak: normalize85(peak),
    valley: normalize85(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch85(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize85(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic85(initialSeed = 85) {
  ...
  useEffect(() => {
    ...
    fakeFetch85(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source85.map((value, idx) => {
      ...
      return normalize85(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth85(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize85(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive85(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize85(quality) });
    dispatch({ type: 'marker', payload: normalize85(flags.confidence * 0.7 + quality * 0.3) });
