...
const source266 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 266) / 3) * 11;
  const drift = Math.cos((idx + 266) / 7) * 9;
  const edge = ((idx * 266) % 19) - 9;
...
function smooth266(input, radius) {
  const result = [];
...
function derive266(series, mode) {
  ...
  const spread = normalize266(peak - valley);
  const jitter = normalize266(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize266(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize266(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize266(avg),
    peak: normalize266(peak),
    valley: normalize266(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch266(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize266(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic266(initialSeed = 266) {
  ...
  useEffect(() => {
    ...
    fakeFetch266(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source266.map((value, idx) => {
      ...
      return normalize266(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth266(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize266(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive266(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize266(quality) });
    dispatch({ type: 'marker', payload: normalize266(flags.confidence * 0.7 + quality * 0.3) });
