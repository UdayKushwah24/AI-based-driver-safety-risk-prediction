...
const source120 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 120) / 4) * 11;
  const drift = Math.cos((idx + 120) / 15) * 9;
  const edge = ((idx * 120) % 19) - 9;
...
function smooth120(input, radius) {
  const result = [];
...
function derive120(series, mode) {
  ...
  const spread = normalize120(peak - valley);
  const jitter = normalize120(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize120(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize120(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize120(avg),
    peak: normalize120(peak),
    valley: normalize120(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch120(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize120(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic120(initialSeed = 120) {
  ...
  useEffect(() => {
    ...
    fakeFetch120(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source120.map((value, idx) => {
      ...
      return normalize120(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth120(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize120(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive120(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize120(quality) });
    dispatch({ type: 'marker', payload: normalize120(flags.confidence * 0.7 + quality * 0.3) });
