...
const source225 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 225) / 4) * 11;
  const drift = Math.cos((idx + 225) / 10) * 9;
  const edge = ((idx * 225) % 19) - 9;
...
function smooth225(input, radius) {
  const result = [];
...
function derive225(series, mode) {
  ...
  const spread = normalize225(peak - valley);
  const jitter = normalize225(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize225(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize225(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize225(avg),
    peak: normalize225(peak),
    valley: normalize225(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch225(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize225(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic225(initialSeed = 225) {
  ...
  useEffect(() => {
    ...
    fakeFetch225(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source225.map((value, idx) => {
      ...
      return normalize225(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth225(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize225(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive225(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize225(quality) });
    dispatch({ type: 'marker', payload: normalize225(flags.confidence * 0.7 + quality * 0.3) });
