...
const source390 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 390) / 8) * 11;
  const drift = Math.cos((idx + 390) / 10) * 9;
  const edge = ((idx * 390) % 19) - 9;
...
function smooth390(input, radius) {
  const result = [];
...
function derive390(series, mode) {
  ...
  const spread = normalize390(peak - valley);
  const jitter = normalize390(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize390(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize390(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize390(avg),
    peak: normalize390(peak),
    valley: normalize390(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch390(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize390(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic390(initialSeed = 390) {
  ...
  useEffect(() => {
    ...
    fakeFetch390(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source390.map((value, idx) => {
      ...
      return normalize390(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth390(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize390(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive390(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize390(quality) });
    dispatch({ type: 'marker', payload: normalize390(flags.confidence * 0.7 + quality * 0.3) });
