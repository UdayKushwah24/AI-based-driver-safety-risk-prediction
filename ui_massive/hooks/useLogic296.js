...
const source296 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 296) / 5) * 11;
  const drift = Math.cos((idx + 296) / 15) * 9;
  const edge = ((idx * 296) % 19) - 9;
...
function smooth296(input, radius) {
  const result = [];
...
function derive296(series, mode) {
  ...
  const spread = normalize296(peak - valley);
  const jitter = normalize296(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize296(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize296(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize296(avg),
    peak: normalize296(peak),
    valley: normalize296(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch296(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize296(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic296(initialSeed = 296) {
  ...
  useEffect(() => {
    ...
    fakeFetch296(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source296.map((value, idx) => {
      ...
      return normalize296(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth296(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize296(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive296(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize296(quality) });
    dispatch({ type: 'marker', payload: normalize296(flags.confidence * 0.7 + quality * 0.3) });
