...
const source208 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 208) / 8) * 11;
  const drift = Math.cos((idx + 208) / 15) * 9;
  const edge = ((idx * 208) % 19) - 9;
...
function smooth208(input, radius) {
  const result = [];
...
function derive208(series, mode) {
  ...
  const spread = normalize208(peak - valley);
  const jitter = normalize208(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize208(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize208(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize208(avg),
    peak: normalize208(peak),
    valley: normalize208(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch208(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize208(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic208(initialSeed = 208) {
  ...
  useEffect(() => {
    ...
    fakeFetch208(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source208.map((value, idx) => {
      ...
      return normalize208(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth208(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize208(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive208(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize208(quality) });
    dispatch({ type: 'marker', payload: normalize208(flags.confidence * 0.7 + quality * 0.3) });
