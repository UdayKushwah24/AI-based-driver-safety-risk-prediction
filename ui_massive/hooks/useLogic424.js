...
const source424 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 424) / 7) * 11;
  const drift = Math.cos((idx + 424) / 11) * 9;
  const edge = ((idx * 424) % 19) - 9;
...
function smooth424(input, radius) {
  const result = [];
...
function derive424(series, mode) {
  ...
  const spread = normalize424(peak - valley);
  const jitter = normalize424(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize424(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize424(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize424(avg),
    peak: normalize424(peak),
    valley: normalize424(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch424(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize424(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic424(initialSeed = 424) {
  ...
  useEffect(() => {
    ...
    fakeFetch424(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source424.map((value, idx) => {
      ...
      return normalize424(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth424(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize424(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive424(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize424(quality) });
    dispatch({ type: 'marker', payload: normalize424(flags.confidence * 0.7 + quality * 0.3) });
