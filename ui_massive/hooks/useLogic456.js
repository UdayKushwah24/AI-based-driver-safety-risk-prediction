...
const source456 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 456) / 4) * 11;
  const drift = Math.cos((idx + 456) / 10) * 9;
  const edge = ((idx * 456) % 19) - 9;
...
function smooth456(input, radius) {
  const result = [];
...
function derive456(series, mode) {
  ...
  const spread = normalize456(peak - valley);
  const jitter = normalize456(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize456(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize456(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize456(avg),
    peak: normalize456(peak),
    valley: normalize456(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch456(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize456(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic456(initialSeed = 456) {
  ...
  useEffect(() => {
    ...
    fakeFetch456(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source456.map((value, idx) => {
      ...
      return normalize456(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth456(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize456(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive456(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize456(quality) });
    dispatch({ type: 'marker', payload: normalize456(flags.confidence * 0.7 + quality * 0.3) });
