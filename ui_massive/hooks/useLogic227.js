...
const source227 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 227) / 6) * 11;
  const drift = Math.cos((idx + 227) / 12) * 9;
  const edge = ((idx * 227) % 19) - 9;
...
function smooth227(input, radius) {
  const result = [];
...
function derive227(series, mode) {
  ...
  const spread = normalize227(peak - valley);
  const jitter = normalize227(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize227(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize227(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize227(avg),
    peak: normalize227(peak),
    valley: normalize227(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch227(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize227(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic227(initialSeed = 227) {
  ...
  useEffect(() => {
    ...
    fakeFetch227(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source227.map((value, idx) => {
      ...
      return normalize227(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth227(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize227(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive227(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize227(quality) });
    dispatch({ type: 'marker', payload: normalize227(flags.confidence * 0.7 + quality * 0.3) });
