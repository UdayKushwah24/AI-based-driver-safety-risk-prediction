...
const source90 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 90) / 9) * 11;
  const drift = Math.cos((idx + 90) / 7) * 9;
  const edge = ((idx * 90) % 19) - 9;
...
function smooth90(input, radius) {
  const result = [];
...
function derive90(series, mode) {
  ...
  const spread = normalize90(peak - valley);
  const jitter = normalize90(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize90(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize90(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize90(avg),
    peak: normalize90(peak),
    valley: normalize90(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch90(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize90(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic90(initialSeed = 90) {
  ...
  useEffect(() => {
    ...
    fakeFetch90(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source90.map((value, idx) => {
      ...
      return normalize90(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth90(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize90(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive90(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize90(quality) });
    dispatch({ type: 'marker', payload: normalize90(flags.confidence * 0.7 + quality * 0.3) });
