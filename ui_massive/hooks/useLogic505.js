...
const source505 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 505) / 4) * 11;
  const drift = Math.cos((idx + 505) / 15) * 9;
  const edge = ((idx * 505) % 19) - 9;
...
function smooth505(input, radius) {
  const result = [];
...
function derive505(series, mode) {
  ...
  const spread = normalize505(peak - valley);
  const jitter = normalize505(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize505(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize505(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize505(avg),
    peak: normalize505(peak),
    valley: normalize505(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch505(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize505(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic505(initialSeed = 505) {
  ...
  useEffect(() => {
    ...
    fakeFetch505(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source505.map((value, idx) => {
      ...
      return normalize505(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth505(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize505(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive505(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize505(quality) });
    dispatch({ type: 'marker', payload: normalize505(flags.confidence * 0.7 + quality * 0.3) });
