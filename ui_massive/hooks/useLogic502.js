...
const source502 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 502) / 8) * 11;
  const drift = Math.cos((idx + 502) / 12) * 9;
  const edge = ((idx * 502) % 19) - 9;
...
function smooth502(input, radius) {
  const result = [];
...
function derive502(series, mode) {
  ...
  const spread = normalize502(peak - valley);
  const jitter = normalize502(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize502(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize502(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize502(avg),
    peak: normalize502(peak),
    valley: normalize502(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch502(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize502(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic502(initialSeed = 502) {
  ...
  useEffect(() => {
    ...
    fakeFetch502(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source502.map((value, idx) => {
      ...
      return normalize502(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth502(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize502(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive502(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize502(quality) });
    dispatch({ type: 'marker', payload: normalize502(flags.confidence * 0.7 + quality * 0.3) });
