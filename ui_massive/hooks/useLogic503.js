...
const source503 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 503) / 9) * 11;
  const drift = Math.cos((idx + 503) / 13) * 9;
  const edge = ((idx * 503) % 19) - 9;
...
function smooth503(input, radius) {
  const result = [];
...
function derive503(series, mode) {
  ...
  const spread = normalize503(peak - valley);
  const jitter = normalize503(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize503(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize503(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize503(avg),
    peak: normalize503(peak),
    valley: normalize503(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch503(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize503(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic503(initialSeed = 503) {
  ...
  useEffect(() => {
    ...
    fakeFetch503(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source503.map((value, idx) => {
      ...
      return normalize503(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth503(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize503(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive503(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize503(quality) });
    dispatch({ type: 'marker', payload: normalize503(flags.confidence * 0.7 + quality * 0.3) });
