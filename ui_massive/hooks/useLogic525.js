...
const source525 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 525) / 3) * 11;
  const drift = Math.cos((idx + 525) / 13) * 9;
  const edge = ((idx * 525) % 19) - 9;
...
function smooth525(input, radius) {
  const result = [];
...
function derive525(series, mode) {
  ...
  const spread = normalize525(peak - valley);
  const jitter = normalize525(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize525(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize525(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize525(avg),
    peak: normalize525(peak),
    valley: normalize525(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch525(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize525(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic525(initialSeed = 525) {
  ...
  useEffect(() => {
    ...
    fakeFetch525(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source525.map((value, idx) => {
      ...
      return normalize525(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth525(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize525(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive525(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize525(quality) });
    dispatch({ type: 'marker', payload: normalize525(flags.confidence * 0.7 + quality * 0.3) });
