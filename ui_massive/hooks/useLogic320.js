...
const source320 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 320) / 8) * 11;
  const drift = Math.cos((idx + 320) / 6) * 9;
  const edge = ((idx * 320) % 19) - 9;
...
function smooth320(input, radius) {
  const result = [];
...
function derive320(series, mode) {
  ...
  const spread = normalize320(peak - valley);
  const jitter = normalize320(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize320(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize320(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize320(avg),
    peak: normalize320(peak),
    valley: normalize320(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch320(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize320(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic320(initialSeed = 320) {
  ...
  useEffect(() => {
    ...
    fakeFetch320(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source320.map((value, idx) => {
      ...
      return normalize320(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth320(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize320(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive320(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize320(quality) });
    dispatch({ type: 'marker', payload: normalize320(flags.confidence * 0.7 + quality * 0.3) });
