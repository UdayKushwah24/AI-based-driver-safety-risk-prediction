...
const source103 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 103) / 8) * 11;
  const drift = Math.cos((idx + 103) / 9) * 9;
  const edge = ((idx * 103) % 19) - 9;
...
function smooth103(input, radius) {
  const result = [];
...
function derive103(series, mode) {
  ...
  const spread = normalize103(peak - valley);
  const jitter = normalize103(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize103(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize103(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize103(avg),
    peak: normalize103(peak),
    valley: normalize103(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch103(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize103(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic103(initialSeed = 103) {
  ...
  useEffect(() => {
    ...
    fakeFetch103(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source103.map((value, idx) => {
      ...
      return normalize103(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth103(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize103(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive103(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize103(quality) });
    dispatch({ type: 'marker', payload: normalize103(flags.confidence * 0.7 + quality * 0.3) });
