...
const source341 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 341) / 8) * 11;
  const drift = Math.cos((idx + 341) / 5) * 9;
  const edge = ((idx * 341) % 19) - 9;
...
function smooth341(input, radius) {
  const result = [];
...
function derive341(series, mode) {
  ...
  const spread = normalize341(peak - valley);
  const jitter = normalize341(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize341(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize341(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize341(avg),
    peak: normalize341(peak),
    valley: normalize341(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch341(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize341(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic341(initialSeed = 341) {
  ...
  useEffect(() => {
    ...
    fakeFetch341(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source341.map((value, idx) => {
      ...
      return normalize341(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth341(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize341(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive341(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize341(quality) });
    dispatch({ type: 'marker', payload: normalize341(flags.confidence * 0.7 + quality * 0.3) });
