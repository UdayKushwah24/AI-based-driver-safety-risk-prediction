...
const source473 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 473) / 7) * 11;
  const drift = Math.cos((idx + 473) / 5) * 9;
  const edge = ((idx * 473) % 19) - 9;
...
function smooth473(input, radius) {
  const result = [];
...
function derive473(series, mode) {
  ...
  const spread = normalize473(peak - valley);
  const jitter = normalize473(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize473(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize473(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize473(avg),
    peak: normalize473(peak),
    valley: normalize473(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch473(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize473(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic473(initialSeed = 473) {
  ...
  useEffect(() => {
    ...
    fakeFetch473(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source473.map((value, idx) => {
      ...
      return normalize473(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth473(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize473(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive473(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize473(quality) });
    dispatch({ type: 'marker', payload: normalize473(flags.confidence * 0.7 + quality * 0.3) });
