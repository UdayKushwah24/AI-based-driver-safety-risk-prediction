...
const source446 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 446) / 8) * 11;
  const drift = Math.cos((idx + 446) / 11) * 9;
  const edge = ((idx * 446) % 19) - 9;
...
function smooth446(input, radius) {
  const result = [];
...
function derive446(series, mode) {
  ...
  const spread = normalize446(peak - valley);
  const jitter = normalize446(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize446(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize446(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize446(avg),
    peak: normalize446(peak),
    valley: normalize446(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch446(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize446(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic446(initialSeed = 446) {
  ...
  useEffect(() => {
    ...
    fakeFetch446(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source446.map((value, idx) => {
      ...
      return normalize446(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth446(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize446(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive446(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize446(quality) });
    dispatch({ type: 'marker', payload: normalize446(flags.confidence * 0.7 + quality * 0.3) });
