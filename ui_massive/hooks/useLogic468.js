...
const source468 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 468) / 9) * 11;
  const drift = Math.cos((idx + 468) / 11) * 9;
  const edge = ((idx * 468) % 19) - 9;
...
function smooth468(input, radius) {
  const result = [];
...
function derive468(series, mode) {
  ...
  const spread = normalize468(peak - valley);
  const jitter = normalize468(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize468(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize468(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize468(avg),
    peak: normalize468(peak),
    valley: normalize468(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch468(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize468(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic468(initialSeed = 468) {
  ...
  useEffect(() => {
    ...
    fakeFetch468(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source468.map((value, idx) => {
      ...
      return normalize468(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth468(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize468(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive468(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize468(quality) });
    dispatch({ type: 'marker', payload: normalize468(flags.confidence * 0.7 + quality * 0.3) });
