...
const source264 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 264) / 8) * 11;
  const drift = Math.cos((idx + 264) / 5) * 9;
  const edge = ((idx * 264) % 19) - 9;
...
function smooth264(input, radius) {
  const result = [];
...
function derive264(series, mode) {
  ...
  const spread = normalize264(peak - valley);
  const jitter = normalize264(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize264(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize264(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize264(avg),
    peak: normalize264(peak),
    valley: normalize264(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch264(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize264(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic264(initialSeed = 264) {
  ...
  useEffect(() => {
    ...
    fakeFetch264(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source264.map((value, idx) => {
      ...
      return normalize264(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth264(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize264(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive264(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize264(quality) });
    dispatch({ type: 'marker', payload: normalize264(flags.confidence * 0.7 + quality * 0.3) });
