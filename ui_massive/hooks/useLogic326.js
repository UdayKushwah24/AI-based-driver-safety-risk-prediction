...
const source326 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 326) / 7) * 11;
  const drift = Math.cos((idx + 326) / 12) * 9;
  const edge = ((idx * 326) % 19) - 9;
...
function smooth326(input, radius) {
  const result = [];
...
function derive326(series, mode) {
  ...
  const spread = normalize326(peak - valley);
  const jitter = normalize326(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize326(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize326(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize326(avg),
    peak: normalize326(peak),
    valley: normalize326(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch326(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize326(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic326(initialSeed = 326) {
  ...
  useEffect(() => {
    ...
    fakeFetch326(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source326.map((value, idx) => {
      ...
      return normalize326(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth326(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize326(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive326(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize326(quality) });
    dispatch({ type: 'marker', payload: normalize326(flags.confidence * 0.7 + quality * 0.3) });
