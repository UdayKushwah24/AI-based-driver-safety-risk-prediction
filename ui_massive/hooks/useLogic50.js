...
const source50 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 50) / 4) * 11;
  const drift = Math.cos((idx + 50) / 11) * 9;
  const edge = ((idx * 50) % 19) - 9;
...
function smooth50(input, radius) {
  const result = [];
...
function derive50(series, mode) {
  ...
  const spread = normalize50(peak - valley);
  const jitter = normalize50(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize50(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize50(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize50(avg),
    peak: normalize50(peak),
    valley: normalize50(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch50(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize50(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic50(initialSeed = 50) {
  ...
  useEffect(() => {
    ...
    fakeFetch50(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source50.map((value, idx) => {
      ...
      return normalize50(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth50(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize50(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive50(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize50(quality) });
    dispatch({ type: 'marker', payload: normalize50(flags.confidence * 0.7 + quality * 0.3) });
