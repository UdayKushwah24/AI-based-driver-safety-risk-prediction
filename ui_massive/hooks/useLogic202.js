...
const source202 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 202) / 9) * 11;
  const drift = Math.cos((idx + 202) / 9) * 9;
  const edge = ((idx * 202) % 19) - 9;
...
function smooth202(input, radius) {
  const result = [];
...
function derive202(series, mode) {
  ...
  const spread = normalize202(peak - valley);
  const jitter = normalize202(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize202(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize202(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize202(avg),
    peak: normalize202(peak),
    valley: normalize202(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch202(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize202(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic202(initialSeed = 202) {
  ...
  useEffect(() => {
    ...
    fakeFetch202(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source202.map((value, idx) => {
      ...
      return normalize202(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth202(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize202(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive202(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize202(quality) });
    dispatch({ type: 'marker', payload: normalize202(flags.confidence * 0.7 + quality * 0.3) });
