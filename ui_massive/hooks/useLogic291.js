...
const source291 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 291) / 7) * 11;
  const drift = Math.cos((idx + 291) / 10) * 9;
  const edge = ((idx * 291) % 19) - 9;
...
function smooth291(input, radius) {
  const result = [];
...
function derive291(series, mode) {
  ...
  const spread = normalize291(peak - valley);
  const jitter = normalize291(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize291(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize291(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize291(avg),
    peak: normalize291(peak),
    valley: normalize291(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch291(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize291(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic291(initialSeed = 291) {
  ...
  useEffect(() => {
    ...
    fakeFetch291(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source291.map((value, idx) => {
      ...
      return normalize291(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth291(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize291(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive291(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize291(quality) });
    dispatch({ type: 'marker', payload: normalize291(flags.confidence * 0.7 + quality * 0.3) });
