...
const source368 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 368) / 7) * 11;
  const drift = Math.cos((idx + 368) / 10) * 9;
  const edge = ((idx * 368) % 19) - 9;
...
function smooth368(input, radius) {
  const result = [];
...
function derive368(series, mode) {
  ...
  const spread = normalize368(peak - valley);
  const jitter = normalize368(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize368(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize368(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize368(avg),
    peak: normalize368(peak),
    valley: normalize368(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch368(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize368(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic368(initialSeed = 368) {
  ...
  useEffect(() => {
    ...
    fakeFetch368(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source368.map((value, idx) => {
      ...
      return normalize368(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth368(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize368(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive368(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize368(quality) });
    dispatch({ type: 'marker', payload: normalize368(flags.confidence * 0.7 + quality * 0.3) });
