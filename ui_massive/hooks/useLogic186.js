...
const source186 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 186) / 7) * 11;
  const drift = Math.cos((idx + 186) / 15) * 9;
  const edge = ((idx * 186) % 19) - 9;
...
function smooth186(input, radius) {
  const result = [];
...
function derive186(series, mode) {
  ...
  const spread = normalize186(peak - valley);
  const jitter = normalize186(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize186(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize186(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize186(avg),
    peak: normalize186(peak),
    valley: normalize186(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch186(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize186(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic186(initialSeed = 186) {
  ...
  useEffect(() => {
    ...
    fakeFetch186(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source186.map((value, idx) => {
      ...
      return normalize186(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth186(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize186(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive186(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize186(quality) });
    dispatch({ type: 'marker', payload: normalize186(flags.confidence * 0.7 + quality * 0.3) });
