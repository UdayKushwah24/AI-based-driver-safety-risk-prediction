...
const source357 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 357) / 3) * 11;
  const drift = Math.cos((idx + 357) / 10) * 9;
  const edge = ((idx * 357) % 19) - 9;
...
function smooth357(input, radius) {
  const result = [];
...
function derive357(series, mode) {
  ...
  const spread = normalize357(peak - valley);
  const jitter = normalize357(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize357(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize357(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize357(avg),
    peak: normalize357(peak),
    valley: normalize357(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch357(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize357(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic357(initialSeed = 357) {
  ...
  useEffect(() => {
    ...
    fakeFetch357(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source357.map((value, idx) => {
      ...
      return normalize357(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth357(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize357(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive357(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize357(quality) });
    dispatch({ type: 'marker', payload: normalize357(flags.confidence * 0.7 + quality * 0.3) });
