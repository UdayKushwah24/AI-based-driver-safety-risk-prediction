...
const source472 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 472) / 6) * 11;
  const drift = Math.cos((idx + 472) / 15) * 9;
  const edge = ((idx * 472) % 19) - 9;
...
function smooth472(input, radius) {
  const result = [];
...
function derive472(series, mode) {
  ...
  const spread = normalize472(peak - valley);
  const jitter = normalize472(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize472(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize472(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize472(avg),
    peak: normalize472(peak),
    valley: normalize472(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch472(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize472(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic472(initialSeed = 472) {
  ...
  useEffect(() => {
    ...
    fakeFetch472(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source472.map((value, idx) => {
      ...
      return normalize472(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth472(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize472(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive472(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize472(quality) });
    dispatch({ type: 'marker', payload: normalize472(flags.confidence * 0.7 + quality * 0.3) });
