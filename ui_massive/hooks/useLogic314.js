...
const source314 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 314) / 9) * 11;
  const drift = Math.cos((idx + 314) / 11) * 9;
  const edge = ((idx * 314) % 19) - 9;
...
function smooth314(input, radius) {
  const result = [];
...
function derive314(series, mode) {
  ...
  const spread = normalize314(peak - valley);
  const jitter = normalize314(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize314(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize314(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize314(avg),
    peak: normalize314(peak),
    valley: normalize314(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch314(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize314(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic314(initialSeed = 314) {
  ...
  useEffect(() => {
    ...
    fakeFetch314(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source314.map((value, idx) => {
      ...
      return normalize314(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth314(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize314(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive314(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize314(quality) });
    dispatch({ type: 'marker', payload: normalize314(flags.confidence * 0.7 + quality * 0.3) });
