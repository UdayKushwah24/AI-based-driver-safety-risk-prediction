...
const source378 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 378) / 3) * 11;
  const drift = Math.cos((idx + 378) / 9) * 9;
  const edge = ((idx * 378) % 19) - 9;
...
function smooth378(input, radius) {
  const result = [];
...
function derive378(series, mode) {
  ...
  const spread = normalize378(peak - valley);
  const jitter = normalize378(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize378(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize378(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize378(avg),
    peak: normalize378(peak),
    valley: normalize378(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch378(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize378(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic378(initialSeed = 378) {
  ...
  useEffect(() => {
    ...
    fakeFetch378(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source378.map((value, idx) => {
      ...
      return normalize378(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth378(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize378(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive378(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize378(quality) });
    dispatch({ type: 'marker', payload: normalize378(flags.confidence * 0.7 + quality * 0.3) });
