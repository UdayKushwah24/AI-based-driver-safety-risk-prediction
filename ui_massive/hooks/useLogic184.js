...
const source184 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 184) / 5) * 11;
  const drift = Math.cos((idx + 184) / 13) * 9;
  const edge = ((idx * 184) % 19) - 9;
...
function smooth184(input, radius) {
  const result = [];
...
function derive184(series, mode) {
  ...
  const spread = normalize184(peak - valley);
  const jitter = normalize184(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize184(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize184(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize184(avg),
    peak: normalize184(peak),
    valley: normalize184(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch184(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize184(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic184(initialSeed = 184) {
  ...
  useEffect(() => {
    ...
    fakeFetch184(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source184.map((value, idx) => {
      ...
      return normalize184(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth184(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize184(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive184(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize184(quality) });
    dispatch({ type: 'marker', payload: normalize184(flags.confidence * 0.7 + quality * 0.3) });
