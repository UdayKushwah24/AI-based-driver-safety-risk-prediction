...
const source475 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 475) / 9) * 11;
  const drift = Math.cos((idx + 475) / 7) * 9;
  const edge = ((idx * 475) % 19) - 9;
...
function smooth475(input, radius) {
  const result = [];
...
function derive475(series, mode) {
  ...
  const spread = normalize475(peak - valley);
  const jitter = normalize475(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize475(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize475(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize475(avg),
    peak: normalize475(peak),
    valley: normalize475(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch475(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize475(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic475(initialSeed = 475) {
  ...
  useEffect(() => {
    ...
    fakeFetch475(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source475.map((value, idx) => {
      ...
      return normalize475(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth475(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize475(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive475(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize475(quality) });
    dispatch({ type: 'marker', payload: normalize475(flags.confidence * 0.7 + quality * 0.3) });
