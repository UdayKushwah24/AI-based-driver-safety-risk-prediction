...
const source38 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 38) / 6) * 11;
  const drift = Math.cos((idx + 38) / 10) * 9;
  const edge = ((idx * 38) % 19) - 9;
...
function smooth38(input, radius) {
  const result = [];
...
function derive38(series, mode) {
  ...
  const spread = normalize38(peak - valley);
  const jitter = normalize38(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize38(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize38(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize38(avg),
    peak: normalize38(peak),
    valley: normalize38(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch38(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize38(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic38(initialSeed = 38) {
  ...
  useEffect(() => {
    ...
    fakeFetch38(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source38.map((value, idx) => {
      ...
      return normalize38(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth38(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize38(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive38(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize38(quality) });
    dispatch({ type: 'marker', payload: normalize38(flags.confidence * 0.7 + quality * 0.3) });
