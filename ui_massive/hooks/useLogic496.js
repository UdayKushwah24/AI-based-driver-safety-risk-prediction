...
const source496 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 496) / 9) * 11;
  const drift = Math.cos((idx + 496) / 6) * 9;
  const edge = ((idx * 496) % 19) - 9;
...
function smooth496(input, radius) {
  const result = [];
...
function derive496(series, mode) {
  ...
  const spread = normalize496(peak - valley);
  const jitter = normalize496(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize496(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize496(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize496(avg),
    peak: normalize496(peak),
    valley: normalize496(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch496(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize496(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic496(initialSeed = 496) {
  ...
  useEffect(() => {
    ...
    fakeFetch496(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source496.map((value, idx) => {
      ...
      return normalize496(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth496(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize496(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive496(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize496(quality) });
    dispatch({ type: 'marker', payload: normalize496(flags.confidence * 0.7 + quality * 0.3) });
