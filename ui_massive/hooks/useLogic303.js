...
const source303 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 303) / 5) * 11;
  const drift = Math.cos((idx + 303) / 11) * 9;
  const edge = ((idx * 303) % 19) - 9;
...
function smooth303(input, radius) {
  const result = [];
...
function derive303(series, mode) {
  ...
  const spread = normalize303(peak - valley);
  const jitter = normalize303(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize303(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize303(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize303(avg),
    peak: normalize303(peak),
    valley: normalize303(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch303(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize303(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic303(initialSeed = 303) {
  ...
  useEffect(() => {
    ...
    fakeFetch303(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source303.map((value, idx) => {
      ...
      return normalize303(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth303(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize303(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive303(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize303(quality) });
    dispatch({ type: 'marker', payload: normalize303(flags.confidence * 0.7 + quality * 0.3) });
