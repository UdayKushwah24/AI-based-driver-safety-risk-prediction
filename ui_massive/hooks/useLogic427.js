...
const source427 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 427) / 3) * 11;
  const drift = Math.cos((idx + 427) / 14) * 9;
  const edge = ((idx * 427) % 19) - 9;
...
function smooth427(input, radius) {
  const result = [];
...
function derive427(series, mode) {
  ...
  const spread = normalize427(peak - valley);
  const jitter = normalize427(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize427(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize427(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize427(avg),
    peak: normalize427(peak),
    valley: normalize427(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch427(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize427(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic427(initialSeed = 427) {
  ...
  useEffect(() => {
    ...
    fakeFetch427(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source427.map((value, idx) => {
      ...
      return normalize427(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth427(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize427(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive427(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize427(quality) });
    dispatch({ type: 'marker', payload: normalize427(flags.confidence * 0.7 + quality * 0.3) });
