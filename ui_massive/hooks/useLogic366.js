...
const source366 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 366) / 5) * 11;
  const drift = Math.cos((idx + 366) / 8) * 9;
  const edge = ((idx * 366) % 19) - 9;
...
function smooth366(input, radius) {
  const result = [];
...
function derive366(series, mode) {
  ...
  const spread = normalize366(peak - valley);
  const jitter = normalize366(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize366(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize366(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize366(avg),
    peak: normalize366(peak),
    valley: normalize366(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch366(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize366(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic366(initialSeed = 366) {
  ...
  useEffect(() => {
    ...
    fakeFetch366(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source366.map((value, idx) => {
      ...
      return normalize366(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth366(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize366(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive366(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize366(quality) });
    dispatch({ type: 'marker', payload: normalize366(flags.confidence * 0.7 + quality * 0.3) });
