...
const source281 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 281) / 4) * 11;
  const drift = Math.cos((idx + 281) / 11) * 9;
  const edge = ((idx * 281) % 19) - 9;
...
function smooth281(input, radius) {
  const result = [];
...
function derive281(series, mode) {
  ...
  const spread = normalize281(peak - valley);
  const jitter = normalize281(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize281(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize281(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize281(avg),
    peak: normalize281(peak),
    valley: normalize281(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch281(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize281(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic281(initialSeed = 281) {
  ...
  useEffect(() => {
    ...
    fakeFetch281(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source281.map((value, idx) => {
      ...
      return normalize281(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth281(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize281(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive281(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize281(quality) });
    dispatch({ type: 'marker', payload: normalize281(flags.confidence * 0.7 + quality * 0.3) });
