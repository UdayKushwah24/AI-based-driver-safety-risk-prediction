...
const source353 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 353) / 6) * 11;
  const drift = Math.cos((idx + 353) / 6) * 9;
  const edge = ((idx * 353) % 19) - 9;
...
function smooth353(input, radius) {
  const result = [];
...
function derive353(series, mode) {
  ...
  const spread = normalize353(peak - valley);
  const jitter = normalize353(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize353(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize353(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize353(avg),
    peak: normalize353(peak),
    valley: normalize353(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch353(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize353(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic353(initialSeed = 353) {
  ...
  useEffect(() => {
    ...
    fakeFetch353(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source353.map((value, idx) => {
      ...
      return normalize353(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth353(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize353(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive353(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize353(quality) });
    dispatch({ type: 'marker', payload: normalize353(flags.confidence * 0.7 + quality * 0.3) });
