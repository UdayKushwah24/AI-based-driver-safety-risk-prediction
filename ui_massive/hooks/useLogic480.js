...
const source480 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 480) / 7) * 11;
  const drift = Math.cos((idx + 480) / 12) * 9;
  const edge = ((idx * 480) % 19) - 9;
...
function smooth480(input, radius) {
  const result = [];
...
function derive480(series, mode) {
  ...
  const spread = normalize480(peak - valley);
  const jitter = normalize480(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize480(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize480(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize480(avg),
    peak: normalize480(peak),
    valley: normalize480(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch480(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize480(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic480(initialSeed = 480) {
  ...
  useEffect(() => {
    ...
    fakeFetch480(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source480.map((value, idx) => {
      ...
      return normalize480(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth480(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize480(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive480(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize480(quality) });
    dispatch({ type: 'marker', payload: normalize480(flags.confidence * 0.7 + quality * 0.3) });
