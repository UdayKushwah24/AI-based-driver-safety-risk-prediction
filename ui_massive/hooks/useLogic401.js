...
const source401 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 401) / 5) * 11;
  const drift = Math.cos((idx + 401) / 10) * 9;
  const edge = ((idx * 401) % 19) - 9;
...
function smooth401(input, radius) {
  const result = [];
...
function derive401(series, mode) {
  ...
  const spread = normalize401(peak - valley);
  const jitter = normalize401(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize401(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize401(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize401(avg),
    peak: normalize401(peak),
    valley: normalize401(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch401(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize401(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic401(initialSeed = 401) {
  ...
  useEffect(() => {
    ...
    fakeFetch401(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source401.map((value, idx) => {
      ...
      return normalize401(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth401(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize401(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive401(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize401(quality) });
    dispatch({ type: 'marker', payload: normalize401(flags.confidence * 0.7 + quality * 0.3) });
