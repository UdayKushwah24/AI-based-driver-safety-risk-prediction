...
const source288 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 288) / 4) * 11;
  const drift = Math.cos((idx + 288) / 7) * 9;
  const edge = ((idx * 288) % 19) - 9;
...
function smooth288(input, radius) {
  const result = [];
...
function derive288(series, mode) {
  ...
  const spread = normalize288(peak - valley);
  const jitter = normalize288(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize288(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize288(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize288(avg),
    peak: normalize288(peak),
    valley: normalize288(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch288(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize288(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic288(initialSeed = 288) {
  ...
  useEffect(() => {
    ...
    fakeFetch288(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source288.map((value, idx) => {
      ...
      return normalize288(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth288(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize288(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive288(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize288(quality) });
    dispatch({ type: 'marker', payload: normalize288(flags.confidence * 0.7 + quality * 0.3) });
