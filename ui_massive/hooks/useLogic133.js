...
const source133 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 133) / 3) * 11;
  const drift = Math.cos((idx + 133) / 6) * 9;
  const edge = ((idx * 133) % 19) - 9;
...
function smooth133(input, radius) {
  const result = [];
...
function derive133(series, mode) {
  ...
  const spread = normalize133(peak - valley);
  const jitter = normalize133(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize133(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize133(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize133(avg),
    peak: normalize133(peak),
    valley: normalize133(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch133(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize133(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic133(initialSeed = 133) {
  ...
  useEffect(() => {
    ...
    fakeFetch133(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source133.map((value, idx) => {
      ...
      return normalize133(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth133(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize133(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive133(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize133(quality) });
    dispatch({ type: 'marker', payload: normalize133(flags.confidence * 0.7 + quality * 0.3) });
