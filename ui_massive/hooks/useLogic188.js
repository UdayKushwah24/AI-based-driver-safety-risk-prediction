...
const source188 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 188) / 9) * 11;
  const drift = Math.cos((idx + 188) / 6) * 9;
  const edge = ((idx * 188) % 19) - 9;
...
function smooth188(input, radius) {
  const result = [];
...
function derive188(series, mode) {
  ...
  const spread = normalize188(peak - valley);
  const jitter = normalize188(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize188(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize188(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize188(avg),
    peak: normalize188(peak),
    valley: normalize188(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch188(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize188(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic188(initialSeed = 188) {
  ...
  useEffect(() => {
    ...
    fakeFetch188(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source188.map((value, idx) => {
      ...
      return normalize188(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth188(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize188(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive188(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize188(quality) });
    dispatch({ type: 'marker', payload: normalize188(flags.confidence * 0.7 + quality * 0.3) });
