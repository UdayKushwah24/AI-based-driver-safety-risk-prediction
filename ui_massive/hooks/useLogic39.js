...
const source39 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 39) / 7) * 11;
  const drift = Math.cos((idx + 39) / 11) * 9;
  const edge = ((idx * 39) % 19) - 9;
...
function smooth39(input, radius) {
  const result = [];
...
function derive39(series, mode) {
  ...
  const spread = normalize39(peak - valley);
  const jitter = normalize39(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize39(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize39(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize39(avg),
    peak: normalize39(peak),
    valley: normalize39(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch39(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize39(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic39(initialSeed = 39) {
  ...
  useEffect(() => {
    ...
    fakeFetch39(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source39.map((value, idx) => {
      ...
      return normalize39(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth39(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize39(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive39(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize39(quality) });
    dispatch({ type: 'marker', payload: normalize39(flags.confidence * 0.7 + quality * 0.3) });
