...
const source474 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 474) / 8) * 11;
  const drift = Math.cos((idx + 474) / 6) * 9;
  const edge = ((idx * 474) % 19) - 9;
...
function smooth474(input, radius) {
  const result = [];
...
function derive474(series, mode) {
  ...
  const spread = normalize474(peak - valley);
  const jitter = normalize474(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize474(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize474(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize474(avg),
    peak: normalize474(peak),
    valley: normalize474(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch474(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize474(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic474(initialSeed = 474) {
  ...
  useEffect(() => {
    ...
    fakeFetch474(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source474.map((value, idx) => {
      ...
      return normalize474(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth474(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize474(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive474(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize474(quality) });
    dispatch({ type: 'marker', payload: normalize474(flags.confidence * 0.7 + quality * 0.3) });
