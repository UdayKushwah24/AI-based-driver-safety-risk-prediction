...
const source528 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 528) / 6) * 11;
  const drift = Math.cos((idx + 528) / 5) * 9;
  const edge = ((idx * 528) % 19) - 9;
...
function smooth528(input, radius) {
  const result = [];
...
function derive528(series, mode) {
  ...
  const spread = normalize528(peak - valley);
  const jitter = normalize528(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize528(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize528(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize528(avg),
    peak: normalize528(peak),
    valley: normalize528(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch528(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize528(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic528(initialSeed = 528) {
  ...
  useEffect(() => {
    ...
    fakeFetch528(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source528.map((value, idx) => {
      ...
      return normalize528(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth528(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize528(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive528(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize528(quality) });
    dispatch({ type: 'marker', payload: normalize528(flags.confidence * 0.7 + quality * 0.3) });
