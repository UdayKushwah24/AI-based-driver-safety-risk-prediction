...
const source450 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 450) / 5) * 11;
  const drift = Math.cos((idx + 450) / 15) * 9;
  const edge = ((idx * 450) % 19) - 9;
...
function smooth450(input, radius) {
  const result = [];
...
function derive450(series, mode) {
  ...
  const spread = normalize450(peak - valley);
  const jitter = normalize450(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize450(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize450(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize450(avg),
    peak: normalize450(peak),
    valley: normalize450(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch450(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize450(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic450(initialSeed = 450) {
  ...
  useEffect(() => {
    ...
    fakeFetch450(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source450.map((value, idx) => {
      ...
      return normalize450(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth450(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize450(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive450(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize450(quality) });
    dispatch({ type: 'marker', payload: normalize450(flags.confidence * 0.7 + quality * 0.3) });
