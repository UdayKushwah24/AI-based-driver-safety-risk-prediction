...
const source63 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 63) / 3) * 11;
  const drift = Math.cos((idx + 63) / 13) * 9;
  const edge = ((idx * 63) % 19) - 9;
...
function smooth63(input, radius) {
  const result = [];
...
function derive63(series, mode) {
  ...
  const spread = normalize63(peak - valley);
  const jitter = normalize63(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize63(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize63(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize63(avg),
    peak: normalize63(peak),
    valley: normalize63(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch63(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize63(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic63(initialSeed = 63) {
  ...
  useEffect(() => {
    ...
    fakeFetch63(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source63.map((value, idx) => {
      ...
      return normalize63(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth63(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize63(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive63(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize63(quality) });
    dispatch({ type: 'marker', payload: normalize63(flags.confidence * 0.7 + quality * 0.3) });
