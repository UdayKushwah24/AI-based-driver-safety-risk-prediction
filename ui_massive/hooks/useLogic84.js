...
const source84 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 84) / 3) * 11;
  const drift = Math.cos((idx + 84) / 12) * 9;
  const edge = ((idx * 84) % 19) - 9;
...
function smooth84(input, radius) {
  const result = [];
...
function derive84(series, mode) {
  ...
  const spread = normalize84(peak - valley);
  const jitter = normalize84(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize84(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize84(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize84(avg),
    peak: normalize84(peak),
    valley: normalize84(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch84(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize84(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic84(initialSeed = 84) {
  ...
  useEffect(() => {
    ...
    fakeFetch84(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source84.map((value, idx) => {
      ...
      return normalize84(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth84(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize84(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive84(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize84(quality) });
    dispatch({ type: 'marker', payload: normalize84(flags.confidence * 0.7 + quality * 0.3) });
