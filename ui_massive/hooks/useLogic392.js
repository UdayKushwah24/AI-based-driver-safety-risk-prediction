...
const source392 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 392) / 3) * 11;
  const drift = Math.cos((idx + 392) / 12) * 9;
  const edge = ((idx * 392) % 19) - 9;
...
function smooth392(input, radius) {
  const result = [];
...
function derive392(series, mode) {
  ...
  const spread = normalize392(peak - valley);
  const jitter = normalize392(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize392(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize392(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize392(avg),
    peak: normalize392(peak),
    valley: normalize392(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch392(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize392(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic392(initialSeed = 392) {
  ...
  useEffect(() => {
    ...
    fakeFetch392(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source392.map((value, idx) => {
      ...
      return normalize392(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth392(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize392(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive392(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize392(quality) });
    dispatch({ type: 'marker', payload: normalize392(flags.confidence * 0.7 + quality * 0.3) });
