...
const source312 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 312) / 7) * 11;
  const drift = Math.cos((idx + 312) / 9) * 9;
  const edge = ((idx * 312) % 19) - 9;
...
function smooth312(input, radius) {
  const result = [];
...
function derive312(series, mode) {
  ...
  const spread = normalize312(peak - valley);
  const jitter = normalize312(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize312(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize312(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize312(avg),
    peak: normalize312(peak),
    valley: normalize312(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch312(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize312(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic312(initialSeed = 312) {
  ...
  useEffect(() => {
    ...
    fakeFetch312(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source312.map((value, idx) => {
      ...
      return normalize312(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth312(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize312(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive312(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize312(quality) });
    dispatch({ type: 'marker', payload: normalize312(flags.confidence * 0.7 + quality * 0.3) });
