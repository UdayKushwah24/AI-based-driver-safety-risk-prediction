...
const source345 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 345) / 5) * 11;
  const drift = Math.cos((idx + 345) / 9) * 9;
  const edge = ((idx * 345) % 19) - 9;
...
function smooth345(input, radius) {
  const result = [];
...
function derive345(series, mode) {
  ...
  const spread = normalize345(peak - valley);
  const jitter = normalize345(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize345(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize345(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize345(avg),
    peak: normalize345(peak),
    valley: normalize345(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch345(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize345(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic345(initialSeed = 345) {
  ...
  useEffect(() => {
    ...
    fakeFetch345(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source345.map((value, idx) => {
      ...
      return normalize345(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth345(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize345(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive345(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize345(quality) });
    dispatch({ type: 'marker', payload: normalize345(flags.confidence * 0.7 + quality * 0.3) });
