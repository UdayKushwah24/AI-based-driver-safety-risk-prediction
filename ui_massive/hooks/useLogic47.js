...
const source47 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 47) / 8) * 11;
  const drift = Math.cos((idx + 47) / 8) * 9;
  const edge = ((idx * 47) % 19) - 9;
...
function smooth47(input, radius) {
  const result = [];
...
function derive47(series, mode) {
  ...
  const spread = normalize47(peak - valley);
  const jitter = normalize47(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize47(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize47(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize47(avg),
    peak: normalize47(peak),
    valley: normalize47(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch47(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize47(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic47(initialSeed = 47) {
  ...
  useEffect(() => {
    ...
    fakeFetch47(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source47.map((value, idx) => {
      ...
      return normalize47(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth47(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize47(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive47(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize47(quality) });
    dispatch({ type: 'marker', payload: normalize47(flags.confidence * 0.7 + quality * 0.3) });
