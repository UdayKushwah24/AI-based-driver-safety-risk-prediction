...
const source150 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 150) / 6) * 11;
  const drift = Math.cos((idx + 150) / 12) * 9;
  const edge = ((idx * 150) % 19) - 9;
...
function smooth150(input, radius) {
  const result = [];
...
function derive150(series, mode) {
  ...
  const spread = normalize150(peak - valley);
  const jitter = normalize150(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize150(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize150(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize150(avg),
    peak: normalize150(peak),
    valley: normalize150(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch150(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize150(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic150(initialSeed = 150) {
  ...
  useEffect(() => {
    ...
    fakeFetch150(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source150.map((value, idx) => {
      ...
      return normalize150(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth150(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize150(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive150(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize150(quality) });
    dispatch({ type: 'marker', payload: normalize150(flags.confidence * 0.7 + quality * 0.3) });
