...
const source70 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 70) / 3) * 11;
  const drift = Math.cos((idx + 70) / 9) * 9;
  const edge = ((idx * 70) % 19) - 9;
...
function smooth70(input, radius) {
  const result = [];
...
function derive70(series, mode) {
  ...
  const spread = normalize70(peak - valley);
  const jitter = normalize70(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize70(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize70(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize70(avg),
    peak: normalize70(peak),
    valley: normalize70(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch70(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize70(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic70(initialSeed = 70) {
  ...
  useEffect(() => {
    ...
    fakeFetch70(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source70.map((value, idx) => {
      ...
      return normalize70(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth70(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize70(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive70(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize70(quality) });
    dispatch({ type: 'marker', payload: normalize70(flags.confidence * 0.7 + quality * 0.3) });
