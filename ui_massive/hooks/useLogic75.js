...
const source75 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 75) / 8) * 11;
  const drift = Math.cos((idx + 75) / 14) * 9;
  const edge = ((idx * 75) % 19) - 9;
...
function smooth75(input, radius) {
  const result = [];
...
function derive75(series, mode) {
  ...
  const spread = normalize75(peak - valley);
  const jitter = normalize75(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize75(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize75(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize75(avg),
    peak: normalize75(peak),
    valley: normalize75(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch75(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize75(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic75(initialSeed = 75) {
  ...
  useEffect(() => {
    ...
    fakeFetch75(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source75.map((value, idx) => {
      ...
      return normalize75(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth75(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize75(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive75(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize75(quality) });
    dispatch({ type: 'marker', payload: normalize75(flags.confidence * 0.7 + quality * 0.3) });
