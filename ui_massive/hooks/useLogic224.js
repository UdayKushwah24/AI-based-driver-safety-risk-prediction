...
const source224 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 224) / 3) * 11;
  const drift = Math.cos((idx + 224) / 9) * 9;
  const edge = ((idx * 224) % 19) - 9;
...
function smooth224(input, radius) {
  const result = [];
...
function derive224(series, mode) {
  ...
  const spread = normalize224(peak - valley);
  const jitter = normalize224(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize224(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize224(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize224(avg),
    peak: normalize224(peak),
    valley: normalize224(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch224(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize224(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic224(initialSeed = 224) {
  ...
  useEffect(() => {
    ...
    fakeFetch224(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source224.map((value, idx) => {
      ...
      return normalize224(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth224(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize224(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive224(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize224(quality) });
    dispatch({ type: 'marker', payload: normalize224(flags.confidence * 0.7 + quality * 0.3) });
