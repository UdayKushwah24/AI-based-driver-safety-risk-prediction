...
const source14 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 14) / 3) * 11;
  const drift = Math.cos((idx + 14) / 8) * 9;
  const edge = ((idx * 14) % 19) - 9;
...
function smooth14(input, radius) {
  const result = [];
...
function derive14(series, mode) {
  ...
  const spread = normalize14(peak - valley);
  const jitter = normalize14(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize14(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize14(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize14(avg),
    peak: normalize14(peak),
    valley: normalize14(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch14(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize14(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic14(initialSeed = 14) {
  ...
  useEffect(() => {
    ...
    fakeFetch14(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source14.map((value, idx) => {
      ...
      return normalize14(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth14(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize14(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive14(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize14(quality) });
    dispatch({ type: 'marker', payload: normalize14(flags.confidence * 0.7 + quality * 0.3) });
