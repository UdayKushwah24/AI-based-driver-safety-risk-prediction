...
const source487 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 487) / 7) * 11;
  const drift = Math.cos((idx + 487) / 8) * 9;
  const edge = ((idx * 487) % 19) - 9;
...
function smooth487(input, radius) {
  const result = [];
...
function derive487(series, mode) {
  ...
  const spread = normalize487(peak - valley);
  const jitter = normalize487(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize487(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize487(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize487(avg),
    peak: normalize487(peak),
    valley: normalize487(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch487(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize487(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic487(initialSeed = 487) {
  ...
  useEffect(() => {
    ...
    fakeFetch487(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source487.map((value, idx) => {
      ...
      return normalize487(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth487(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize487(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive487(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize487(quality) });
    dispatch({ type: 'marker', payload: normalize487(flags.confidence * 0.7 + quality * 0.3) });
