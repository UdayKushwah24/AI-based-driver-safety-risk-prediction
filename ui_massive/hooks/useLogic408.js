...
const source408 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 408) / 5) * 11;
  const drift = Math.cos((idx + 408) / 6) * 9;
  const edge = ((idx * 408) % 19) - 9;
...
function smooth408(input, radius) {
  const result = [];
...
function derive408(series, mode) {
  ...
  const spread = normalize408(peak - valley);
  const jitter = normalize408(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize408(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize408(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize408(avg),
    peak: normalize408(peak),
    valley: normalize408(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch408(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize408(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic408(initialSeed = 408) {
  ...
  useEffect(() => {
    ...
    fakeFetch408(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source408.map((value, idx) => {
      ...
      return normalize408(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth408(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize408(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive408(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize408(quality) });
    dispatch({ type: 'marker', payload: normalize408(flags.confidence * 0.7 + quality * 0.3) });
