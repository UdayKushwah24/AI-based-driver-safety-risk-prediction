...
const source53 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 53) / 7) * 11;
  const drift = Math.cos((idx + 53) / 14) * 9;
  const edge = ((idx * 53) % 19) - 9;
...
function smooth53(input, radius) {
  const result = [];
...
function derive53(series, mode) {
  ...
  const spread = normalize53(peak - valley);
  const jitter = normalize53(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize53(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize53(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize53(avg),
    peak: normalize53(peak),
    valley: normalize53(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch53(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize53(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic53(initialSeed = 53) {
  ...
  useEffect(() => {
    ...
    fakeFetch53(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source53.map((value, idx) => {
      ...
      return normalize53(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth53(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize53(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive53(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize53(quality) });
    dispatch({ type: 'marker', payload: normalize53(flags.confidence * 0.7 + quality * 0.3) });
