...
const source400 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 400) / 4) * 11;
  const drift = Math.cos((idx + 400) / 9) * 9;
  const edge = ((idx * 400) % 19) - 9;
...
function smooth400(input, radius) {
  const result = [];
...
function derive400(series, mode) {
  ...
  const spread = normalize400(peak - valley);
  const jitter = normalize400(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize400(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize400(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize400(avg),
    peak: normalize400(peak),
    valley: normalize400(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch400(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize400(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic400(initialSeed = 400) {
  ...
  useEffect(() => {
    ...
    fakeFetch400(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source400.map((value, idx) => {
      ...
      return normalize400(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth400(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize400(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive400(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize400(quality) });
    dispatch({ type: 'marker', payload: normalize400(flags.confidence * 0.7 + quality * 0.3) });
