...
const source198 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 198) / 5) * 11;
  const drift = Math.cos((idx + 198) / 5) * 9;
  const edge = ((idx * 198) % 19) - 9;
...
function smooth198(input, radius) {
  const result = [];
...
function derive198(series, mode) {
  ...
  const spread = normalize198(peak - valley);
  const jitter = normalize198(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize198(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize198(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize198(avg),
    peak: normalize198(peak),
    valley: normalize198(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch198(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize198(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic198(initialSeed = 198) {
  ...
  useEffect(() => {
    ...
    fakeFetch198(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source198.map((value, idx) => {
      ...
      return normalize198(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth198(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize198(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive198(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize198(quality) });
    dispatch({ type: 'marker', payload: normalize198(flags.confidence * 0.7 + quality * 0.3) });
