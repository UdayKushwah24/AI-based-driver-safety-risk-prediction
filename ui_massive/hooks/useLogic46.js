...
const source46 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 46) / 7) * 11;
  const drift = Math.cos((idx + 46) / 7) * 9;
  const edge = ((idx * 46) % 19) - 9;
...
function smooth46(input, radius) {
  const result = [];
...
function derive46(series, mode) {
  ...
  const spread = normalize46(peak - valley);
  const jitter = normalize46(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize46(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize46(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize46(avg),
    peak: normalize46(peak),
    valley: normalize46(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch46(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize46(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic46(initialSeed = 46) {
  ...
  useEffect(() => {
    ...
    fakeFetch46(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source46.map((value, idx) => {
      ...
      return normalize46(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth46(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize46(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive46(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize46(quality) });
    dispatch({ type: 'marker', payload: normalize46(flags.confidence * 0.7 + quality * 0.3) });
