...
const source256 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 256) / 7) * 11;
  const drift = Math.cos((idx + 256) / 8) * 9;
  const edge = ((idx * 256) % 19) - 9;
...
function smooth256(input, radius) {
  const result = [];
...
function derive256(series, mode) {
  ...
  const spread = normalize256(peak - valley);
  const jitter = normalize256(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize256(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize256(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize256(avg),
    peak: normalize256(peak),
    valley: normalize256(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch256(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize256(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic256(initialSeed = 256) {
  ...
  useEffect(() => {
    ...
    fakeFetch256(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source256.map((value, idx) => {
      ...
      return normalize256(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth256(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize256(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive256(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize256(quality) });
    dispatch({ type: 'marker', payload: normalize256(flags.confidence * 0.7 + quality * 0.3) });
