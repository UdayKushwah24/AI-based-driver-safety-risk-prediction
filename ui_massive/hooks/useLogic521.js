...
const source521 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 521) / 6) * 11;
  const drift = Math.cos((idx + 521) / 9) * 9;
  const edge = ((idx * 521) % 19) - 9;
...
function smooth521(input, radius) {
  const result = [];
...
function derive521(series, mode) {
  ...
  const spread = normalize521(peak - valley);
  const jitter = normalize521(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize521(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize521(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize521(avg),
    peak: normalize521(peak),
    valley: normalize521(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch521(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize521(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic521(initialSeed = 521) {
  ...
  useEffect(() => {
    ...
    fakeFetch521(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source521.map((value, idx) => {
      ...
      return normalize521(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth521(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize521(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive521(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize521(quality) });
    dispatch({ type: 'marker', payload: normalize521(flags.confidence * 0.7 + quality * 0.3) });
