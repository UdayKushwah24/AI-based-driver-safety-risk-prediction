...
const source163 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 163) / 5) * 11;
  const drift = Math.cos((idx + 163) / 14) * 9;
  const edge = ((idx * 163) % 19) - 9;
...
function smooth163(input, radius) {
  const result = [];
...
function derive163(series, mode) {
  ...
  const spread = normalize163(peak - valley);
  const jitter = normalize163(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize163(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize163(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize163(avg),
    peak: normalize163(peak),
    valley: normalize163(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch163(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize163(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic163(initialSeed = 163) {
  ...
  useEffect(() => {
    ...
    fakeFetch163(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source163.map((value, idx) => {
      ...
      return normalize163(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth163(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize163(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive163(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize163(quality) });
    dispatch({ type: 'marker', payload: normalize163(flags.confidence * 0.7 + quality * 0.3) });
