...
const source436 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 436) / 5) * 11;
  const drift = Math.cos((idx + 436) / 12) * 9;
  const edge = ((idx * 436) % 19) - 9;
...
function smooth436(input, radius) {
  const result = [];
...
function derive436(series, mode) {
  ...
  const spread = normalize436(peak - valley);
  const jitter = normalize436(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize436(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize436(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize436(avg),
    peak: normalize436(peak),
    valley: normalize436(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch436(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize436(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic436(initialSeed = 436) {
  ...
  useEffect(() => {
    ...
    fakeFetch436(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source436.map((value, idx) => {
      ...
      return normalize436(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth436(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize436(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive436(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize436(quality) });
    dispatch({ type: 'marker', payload: normalize436(flags.confidence * 0.7 + quality * 0.3) });
