...
const source287 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 287) / 3) * 11;
  const drift = Math.cos((idx + 287) / 6) * 9;
  const edge = ((idx * 287) % 19) - 9;
...
function smooth287(input, radius) {
  const result = [];
...
function derive287(series, mode) {
  ...
  const spread = normalize287(peak - valley);
  const jitter = normalize287(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize287(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize287(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize287(avg),
    peak: normalize287(peak),
    valley: normalize287(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch287(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize287(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic287(initialSeed = 287) {
  ...
  useEffect(() => {
    ...
    fakeFetch287(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source287.map((value, idx) => {
      ...
      return normalize287(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth287(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize287(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive287(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize287(quality) });
    dispatch({ type: 'marker', payload: normalize287(flags.confidence * 0.7 + quality * 0.3) });
