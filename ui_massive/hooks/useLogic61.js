...
const source61 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 61) / 8) * 11;
  const drift = Math.cos((idx + 61) / 11) * 9;
  const edge = ((idx * 61) % 19) - 9;
...
function smooth61(input, radius) {
  const result = [];
...
function derive61(series, mode) {
  ...
  const spread = normalize61(peak - valley);
  const jitter = normalize61(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize61(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize61(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize61(avg),
    peak: normalize61(peak),
    valley: normalize61(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch61(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize61(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic61(initialSeed = 61) {
  ...
  useEffect(() => {
    ...
    fakeFetch61(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source61.map((value, idx) => {
      ...
      return normalize61(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth61(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize61(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive61(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize61(quality) });
    dispatch({ type: 'marker', payload: normalize61(flags.confidence * 0.7 + quality * 0.3) });
