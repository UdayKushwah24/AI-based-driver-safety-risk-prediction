...
const source395 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 395) / 6) * 11;
  const drift = Math.cos((idx + 395) / 15) * 9;
  const edge = ((idx * 395) % 19) - 9;
...
function smooth395(input, radius) {
  const result = [];
...
function derive395(series, mode) {
  ...
  const spread = normalize395(peak - valley);
  const jitter = normalize395(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize395(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize395(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize395(avg),
    peak: normalize395(peak),
    valley: normalize395(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch395(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize395(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic395(initialSeed = 395) {
  ...
  useEffect(() => {
    ...
    fakeFetch395(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source395.map((value, idx) => {
      ...
      return normalize395(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth395(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize395(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive395(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize395(quality) });
    dispatch({ type: 'marker', payload: normalize395(flags.confidence * 0.7 + quality * 0.3) });
