...
const source434 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 434) / 3) * 11;
  const drift = Math.cos((idx + 434) / 10) * 9;
  const edge = ((idx * 434) % 19) - 9;
...
function smooth434(input, radius) {
  const result = [];
...
function derive434(series, mode) {
  ...
  const spread = normalize434(peak - valley);
  const jitter = normalize434(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize434(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize434(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize434(avg),
    peak: normalize434(peak),
    valley: normalize434(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch434(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize434(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic434(initialSeed = 434) {
  ...
  useEffect(() => {
    ...
    fakeFetch434(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source434.map((value, idx) => {
      ...
      return normalize434(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth434(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize434(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive434(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize434(quality) });
    dispatch({ type: 'marker', payload: normalize434(flags.confidence * 0.7 + quality * 0.3) });
