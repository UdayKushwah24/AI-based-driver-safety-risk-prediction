...
const source429 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 429) / 5) * 11;
  const drift = Math.cos((idx + 429) / 5) * 9;
  const edge = ((idx * 429) % 19) - 9;
...
function smooth429(input, radius) {
  const result = [];
...
function derive429(series, mode) {
  ...
  const spread = normalize429(peak - valley);
  const jitter = normalize429(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize429(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize429(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize429(avg),
    peak: normalize429(peak),
    valley: normalize429(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch429(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize429(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic429(initialSeed = 429) {
  ...
  useEffect(() => {
    ...
    fakeFetch429(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source429.map((value, idx) => {
      ...
      return normalize429(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth429(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize429(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive429(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize429(quality) });
    dispatch({ type: 'marker', payload: normalize429(flags.confidence * 0.7 + quality * 0.3) });
