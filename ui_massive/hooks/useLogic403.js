...
const source403 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 403) / 7) * 11;
  const drift = Math.cos((idx + 403) / 12) * 9;
  const edge = ((idx * 403) % 19) - 9;
...
function smooth403(input, radius) {
  const result = [];
...
function derive403(series, mode) {
  ...
  const spread = normalize403(peak - valley);
  const jitter = normalize403(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize403(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize403(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize403(avg),
    peak: normalize403(peak),
    valley: normalize403(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch403(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize403(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic403(initialSeed = 403) {
  ...
  useEffect(() => {
    ...
    fakeFetch403(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source403.map((value, idx) => {
      ...
      return normalize403(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth403(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize403(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive403(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize403(quality) });
    dispatch({ type: 'marker', payload: normalize403(flags.confidence * 0.7 + quality * 0.3) });
