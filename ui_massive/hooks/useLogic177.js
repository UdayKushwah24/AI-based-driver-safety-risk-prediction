...
const source177 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 177) / 5) * 11;
  const drift = Math.cos((idx + 177) / 6) * 9;
  const edge = ((idx * 177) % 19) - 9;
...
function smooth177(input, radius) {
  const result = [];
...
function derive177(series, mode) {
  ...
  const spread = normalize177(peak - valley);
  const jitter = normalize177(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize177(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize177(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize177(avg),
    peak: normalize177(peak),
    valley: normalize177(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch177(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize177(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic177(initialSeed = 177) {
  ...
  useEffect(() => {
    ...
    fakeFetch177(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source177.map((value, idx) => {
      ...
      return normalize177(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth177(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize177(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive177(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize177(quality) });
    dispatch({ type: 'marker', payload: normalize177(flags.confidence * 0.7 + quality * 0.3) });
