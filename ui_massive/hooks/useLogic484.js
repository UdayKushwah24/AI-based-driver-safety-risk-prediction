...
const source484 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 484) / 4) * 11;
  const drift = Math.cos((idx + 484) / 5) * 9;
  const edge = ((idx * 484) % 19) - 9;
...
function smooth484(input, radius) {
  const result = [];
...
function derive484(series, mode) {
  ...
  const spread = normalize484(peak - valley);
  const jitter = normalize484(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize484(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize484(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize484(avg),
    peak: normalize484(peak),
    valley: normalize484(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch484(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize484(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic484(initialSeed = 484) {
  ...
  useEffect(() => {
    ...
    fakeFetch484(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source484.map((value, idx) => {
      ...
      return normalize484(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth484(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize484(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive484(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize484(quality) });
    dispatch({ type: 'marker', payload: normalize484(flags.confidence * 0.7 + quality * 0.3) });
