...
const source236 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 236) / 8) * 11;
  const drift = Math.cos((idx + 236) / 10) * 9;
  const edge = ((idx * 236) % 19) - 9;
...
function smooth236(input, radius) {
  const result = [];
...
function derive236(series, mode) {
  ...
  const spread = normalize236(peak - valley);
  const jitter = normalize236(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize236(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize236(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize236(avg),
    peak: normalize236(peak),
    valley: normalize236(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch236(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize236(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic236(initialSeed = 236) {
  ...
  useEffect(() => {
    ...
    fakeFetch236(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source236.map((value, idx) => {
      ...
      return normalize236(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth236(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize236(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive236(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize236(quality) });
    dispatch({ type: 'marker', payload: normalize236(flags.confidence * 0.7 + quality * 0.3) });
