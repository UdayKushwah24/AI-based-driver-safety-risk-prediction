...
const source334 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 334) / 8) * 11;
  const drift = Math.cos((idx + 334) / 9) * 9;
  const edge = ((idx * 334) % 19) - 9;
...
function smooth334(input, radius) {
  const result = [];
...
function derive334(series, mode) {
  ...
  const spread = normalize334(peak - valley);
  const jitter = normalize334(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize334(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize334(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize334(avg),
    peak: normalize334(peak),
    valley: normalize334(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch334(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize334(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic334(initialSeed = 334) {
  ...
  useEffect(() => {
    ...
    fakeFetch334(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source334.map((value, idx) => {
      ...
      return normalize334(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth334(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize334(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive334(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize334(quality) });
    dispatch({ type: 'marker', payload: normalize334(flags.confidence * 0.7 + quality * 0.3) });
