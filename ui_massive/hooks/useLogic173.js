...
const source173 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 173) / 8) * 11;
  const drift = Math.cos((idx + 173) / 13) * 9;
  const edge = ((idx * 173) % 19) - 9;
...
function smooth173(input, radius) {
  const result = [];
...
function derive173(series, mode) {
  ...
  const spread = normalize173(peak - valley);
  const jitter = normalize173(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize173(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize173(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize173(avg),
    peak: normalize173(peak),
    valley: normalize173(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch173(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize173(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic173(initialSeed = 173) {
  ...
  useEffect(() => {
    ...
    fakeFetch173(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source173.map((value, idx) => {
      ...
      return normalize173(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth173(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize173(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive173(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize173(quality) });
    dispatch({ type: 'marker', payload: normalize173(flags.confidence * 0.7 + quality * 0.3) });
