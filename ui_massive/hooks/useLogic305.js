...
const source305 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 305) / 7) * 11;
  const drift = Math.cos((idx + 305) / 13) * 9;
  const edge = ((idx * 305) % 19) - 9;
...
function smooth305(input, radius) {
  const result = [];
...
function derive305(series, mode) {
  ...
  const spread = normalize305(peak - valley);
  const jitter = normalize305(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize305(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize305(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize305(avg),
    peak: normalize305(peak),
    valley: normalize305(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch305(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize305(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic305(initialSeed = 305) {
  ...
  useEffect(() => {
    ...
    fakeFetch305(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source305.map((value, idx) => {
      ...
      return normalize305(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth305(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize305(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive305(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize305(quality) });
    dispatch({ type: 'marker', payload: normalize305(flags.confidence * 0.7 + quality * 0.3) });
