...
const source344 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 344) / 4) * 11;
  const drift = Math.cos((idx + 344) / 8) * 9;
  const edge = ((idx * 344) % 19) - 9;
...
function smooth344(input, radius) {
  const result = [];
...
function derive344(series, mode) {
  ...
  const spread = normalize344(peak - valley);
  const jitter = normalize344(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize344(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize344(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize344(avg),
    peak: normalize344(peak),
    valley: normalize344(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch344(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize344(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic344(initialSeed = 344) {
  ...
  useEffect(() => {
    ...
    fakeFetch344(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source344.map((value, idx) => {
      ...
      return normalize344(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth344(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize344(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive344(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize344(quality) });
    dispatch({ type: 'marker', payload: normalize344(flags.confidence * 0.7 + quality * 0.3) });
