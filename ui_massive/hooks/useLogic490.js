...
const source490 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 490) / 3) * 11;
  const drift = Math.cos((idx + 490) / 11) * 9;
  const edge = ((idx * 490) % 19) - 9;
...
function smooth490(input, radius) {
  const result = [];
...
function derive490(series, mode) {
  ...
  const spread = normalize490(peak - valley);
  const jitter = normalize490(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize490(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize490(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize490(avg),
    peak: normalize490(peak),
    valley: normalize490(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch490(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize490(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic490(initialSeed = 490) {
  ...
  useEffect(() => {
    ...
    fakeFetch490(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source490.map((value, idx) => {
      ...
      return normalize490(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth490(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize490(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive490(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize490(quality) });
    dispatch({ type: 'marker', payload: normalize490(flags.confidence * 0.7 + quality * 0.3) });
