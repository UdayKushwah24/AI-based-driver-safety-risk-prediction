...
const source381 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 381) / 6) * 11;
  const drift = Math.cos((idx + 381) / 12) * 9;
  const edge = ((idx * 381) % 19) - 9;
...
function smooth381(input, radius) {
  const result = [];
...
function derive381(series, mode) {
  ...
  const spread = normalize381(peak - valley);
  const jitter = normalize381(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize381(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize381(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize381(avg),
    peak: normalize381(peak),
    valley: normalize381(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch381(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize381(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic381(initialSeed = 381) {
  ...
  useEffect(() => {
    ...
    fakeFetch381(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source381.map((value, idx) => {
      ...
      return normalize381(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth381(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize381(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive381(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize381(quality) });
    dispatch({ type: 'marker', payload: normalize381(flags.confidence * 0.7 + quality * 0.3) });
