...
const source122 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 122) / 6) * 11;
  const drift = Math.cos((idx + 122) / 6) * 9;
  const edge = ((idx * 122) % 19) - 9;
...
function smooth122(input, radius) {
  const result = [];
...
function derive122(series, mode) {
  ...
  const spread = normalize122(peak - valley);
  const jitter = normalize122(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize122(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize122(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize122(avg),
    peak: normalize122(peak),
    valley: normalize122(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch122(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize122(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic122(initialSeed = 122) {
  ...
  useEffect(() => {
    ...
    fakeFetch122(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source122.map((value, idx) => {
      ...
      return normalize122(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth122(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize122(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive122(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize122(quality) });
    dispatch({ type: 'marker', payload: normalize122(flags.confidence * 0.7 + quality * 0.3) });
