...
const source437 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 437) / 6) * 11;
  const drift = Math.cos((idx + 437) / 13) * 9;
  const edge = ((idx * 437) % 19) - 9;
...
function smooth437(input, radius) {
  const result = [];
...
function derive437(series, mode) {
  ...
  const spread = normalize437(peak - valley);
  const jitter = normalize437(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize437(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize437(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize437(avg),
    peak: normalize437(peak),
    valley: normalize437(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch437(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize437(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic437(initialSeed = 437) {
  ...
  useEffect(() => {
    ...
    fakeFetch437(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source437.map((value, idx) => {
      ...
      return normalize437(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth437(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize437(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive437(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize437(quality) });
    dispatch({ type: 'marker', payload: normalize437(flags.confidence * 0.7 + quality * 0.3) });
