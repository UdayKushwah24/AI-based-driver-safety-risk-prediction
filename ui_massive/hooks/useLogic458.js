...
const source458 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 458) / 6) * 11;
  const drift = Math.cos((idx + 458) / 12) * 9;
  const edge = ((idx * 458) % 19) - 9;
...
function smooth458(input, radius) {
  const result = [];
...
function derive458(series, mode) {
  ...
  const spread = normalize458(peak - valley);
  const jitter = normalize458(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize458(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize458(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize458(avg),
    peak: normalize458(peak),
    valley: normalize458(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch458(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize458(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic458(initialSeed = 458) {
  ...
  useEffect(() => {
    ...
    fakeFetch458(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source458.map((value, idx) => {
      ...
      return normalize458(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth458(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize458(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive458(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize458(quality) });
    dispatch({ type: 'marker', payload: normalize458(flags.confidence * 0.7 + quality * 0.3) });
