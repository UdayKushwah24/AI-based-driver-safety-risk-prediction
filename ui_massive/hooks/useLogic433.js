...
const source433 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 433) / 9) * 11;
  const drift = Math.cos((idx + 433) / 9) * 9;
  const edge = ((idx * 433) % 19) - 9;
...
function smooth433(input, radius) {
  const result = [];
...
function derive433(series, mode) {
  ...
  const spread = normalize433(peak - valley);
  const jitter = normalize433(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize433(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize433(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize433(avg),
    peak: normalize433(peak),
    valley: normalize433(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch433(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize433(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic433(initialSeed = 433) {
  ...
  useEffect(() => {
    ...
    fakeFetch433(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source433.map((value, idx) => {
      ...
      return normalize433(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth433(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize433(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive433(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize433(quality) });
    dispatch({ type: 'marker', payload: normalize433(flags.confidence * 0.7 + quality * 0.3) });
