...
const source25 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 25) / 7) * 11;
  const drift = Math.cos((idx + 25) / 8) * 9;
  const edge = ((idx * 25) % 19) - 9;
...
function smooth25(input, radius) {
  const result = [];
...
function derive25(series, mode) {
  ...
  const spread = normalize25(peak - valley);
  const jitter = normalize25(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize25(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize25(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize25(avg),
    peak: normalize25(peak),
    valley: normalize25(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch25(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize25(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic25(initialSeed = 25) {
  ...
  useEffect(() => {
    ...
    fakeFetch25(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source25.map((value, idx) => {
      ...
      return normalize25(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth25(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize25(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive25(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize25(quality) });
    dispatch({ type: 'marker', payload: normalize25(flags.confidence * 0.7 + quality * 0.3) });
