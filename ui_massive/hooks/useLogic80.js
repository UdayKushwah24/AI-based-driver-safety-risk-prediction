...
const source80 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 80) / 6) * 11;
  const drift = Math.cos((idx + 80) / 8) * 9;
  const edge = ((idx * 80) % 19) - 9;
...
function smooth80(input, radius) {
  const result = [];
...
function derive80(series, mode) {
  ...
  const spread = normalize80(peak - valley);
  const jitter = normalize80(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize80(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize80(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize80(avg),
    peak: normalize80(peak),
    valley: normalize80(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch80(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize80(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic80(initialSeed = 80) {
  ...
  useEffect(() => {
    ...
    fakeFetch80(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source80.map((value, idx) => {
      ...
      return normalize80(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth80(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize80(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive80(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize80(quality) });
    dispatch({ type: 'marker', payload: normalize80(flags.confidence * 0.7 + quality * 0.3) });
