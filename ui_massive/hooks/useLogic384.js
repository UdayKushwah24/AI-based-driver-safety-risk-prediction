...
const source384 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 384) / 9) * 11;
  const drift = Math.cos((idx + 384) / 15) * 9;
  const edge = ((idx * 384) % 19) - 9;
...
function smooth384(input, radius) {
  const result = [];
...
function derive384(series, mode) {
  ...
  const spread = normalize384(peak - valley);
  const jitter = normalize384(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize384(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize384(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize384(avg),
    peak: normalize384(peak),
    valley: normalize384(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch384(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize384(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic384(initialSeed = 384) {
  ...
  useEffect(() => {
    ...
    fakeFetch384(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source384.map((value, idx) => {
      ...
      return normalize384(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth384(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize384(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive384(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize384(quality) });
    dispatch({ type: 'marker', payload: normalize384(flags.confidence * 0.7 + quality * 0.3) });
