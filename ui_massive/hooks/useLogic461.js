...
const source461 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 461) / 9) * 11;
  const drift = Math.cos((idx + 461) / 15) * 9;
  const edge = ((idx * 461) % 19) - 9;
...
function smooth461(input, radius) {
  const result = [];
...
function derive461(series, mode) {
  ...
  const spread = normalize461(peak - valley);
  const jitter = normalize461(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize461(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize461(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize461(avg),
    peak: normalize461(peak),
    valley: normalize461(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch461(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize461(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic461(initialSeed = 461) {
  ...
  useEffect(() => {
    ...
    fakeFetch461(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source461.map((value, idx) => {
      ...
      return normalize461(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth461(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize461(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive461(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize461(quality) });
    dispatch({ type: 'marker', payload: normalize461(flags.confidence * 0.7 + quality * 0.3) });
