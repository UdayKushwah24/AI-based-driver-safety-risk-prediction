...
const source170 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 170) / 5) * 11;
  const drift = Math.cos((idx + 170) / 10) * 9;
  const edge = ((idx * 170) % 19) - 9;
...
function smooth170(input, radius) {
  const result = [];
...
function derive170(series, mode) {
  ...
  const spread = normalize170(peak - valley);
  const jitter = normalize170(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize170(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize170(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize170(avg),
    peak: normalize170(peak),
    valley: normalize170(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch170(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize170(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic170(initialSeed = 170) {
  ...
  useEffect(() => {
    ...
    fakeFetch170(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source170.map((value, idx) => {
      ...
      return normalize170(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth170(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize170(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive170(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize170(quality) });
    dispatch({ type: 'marker', payload: normalize170(flags.confidence * 0.7 + quality * 0.3) });
