...
const source327 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 327) / 8) * 11;
  const drift = Math.cos((idx + 327) / 13) * 9;
  const edge = ((idx * 327) % 19) - 9;
...
function smooth327(input, radius) {
  const result = [];
...
function derive327(series, mode) {
  ...
  const spread = normalize327(peak - valley);
  const jitter = normalize327(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize327(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize327(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize327(avg),
    peak: normalize327(peak),
    valley: normalize327(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch327(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize327(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic327(initialSeed = 327) {
  ...
  useEffect(() => {
    ...
    fakeFetch327(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source327.map((value, idx) => {
      ...
      return normalize327(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth327(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize327(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive327(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize327(quality) });
    dispatch({ type: 'marker', payload: normalize327(flags.confidence * 0.7 + quality * 0.3) });
