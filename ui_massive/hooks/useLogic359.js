...
const source359 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 359) / 5) * 11;
  const drift = Math.cos((idx + 359) / 12) * 9;
  const edge = ((idx * 359) % 19) - 9;
...
function smooth359(input, radius) {
  const result = [];
...
function derive359(series, mode) {
  ...
  const spread = normalize359(peak - valley);
  const jitter = normalize359(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize359(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize359(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize359(avg),
    peak: normalize359(peak),
    valley: normalize359(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch359(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize359(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic359(initialSeed = 359) {
  ...
  useEffect(() => {
    ...
    fakeFetch359(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source359.map((value, idx) => {
      ...
      return normalize359(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth359(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize359(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive359(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize359(quality) });
    dispatch({ type: 'marker', payload: normalize359(flags.confidence * 0.7 + quality * 0.3) });
