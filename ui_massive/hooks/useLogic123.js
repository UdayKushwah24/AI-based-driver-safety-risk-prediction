...
const source123 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 123) / 7) * 11;
  const drift = Math.cos((idx + 123) / 7) * 9;
  const edge = ((idx * 123) % 19) - 9;
...
function smooth123(input, radius) {
  const result = [];
...
function derive123(series, mode) {
  ...
  const spread = normalize123(peak - valley);
  const jitter = normalize123(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize123(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize123(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize123(avg),
    peak: normalize123(peak),
    valley: normalize123(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch123(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize123(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic123(initialSeed = 123) {
  ...
  useEffect(() => {
    ...
    fakeFetch123(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source123.map((value, idx) => {
      ...
      return normalize123(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth123(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize123(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive123(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize123(quality) });
    dispatch({ type: 'marker', payload: normalize123(flags.confidence * 0.7 + quality * 0.3) });
