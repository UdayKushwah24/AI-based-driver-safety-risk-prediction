...
const source114 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 114) / 5) * 11;
  const drift = Math.cos((idx + 114) / 9) * 9;
  const edge = ((idx * 114) % 19) - 9;
...
function smooth114(input, radius) {
  const result = [];
...
function derive114(series, mode) {
  ...
  const spread = normalize114(peak - valley);
  const jitter = normalize114(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize114(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize114(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize114(avg),
    peak: normalize114(peak),
    valley: normalize114(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch114(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize114(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic114(initialSeed = 114) {
  ...
  useEffect(() => {
    ...
    fakeFetch114(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source114.map((value, idx) => {
      ...
      return normalize114(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth114(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize114(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive114(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize114(quality) });
    dispatch({ type: 'marker', payload: normalize114(flags.confidence * 0.7 + quality * 0.3) });
