...
const source388 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 388) / 6) * 11;
  const drift = Math.cos((idx + 388) / 8) * 9;
  const edge = ((idx * 388) % 19) - 9;
...
function smooth388(input, radius) {
  const result = [];
...
function derive388(series, mode) {
  ...
  const spread = normalize388(peak - valley);
  const jitter = normalize388(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize388(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize388(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize388(avg),
    peak: normalize388(peak),
    valley: normalize388(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch388(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize388(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic388(initialSeed = 388) {
  ...
  useEffect(() => {
    ...
    fakeFetch388(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source388.map((value, idx) => {
      ...
      return normalize388(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth388(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize388(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive388(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize388(quality) });
    dispatch({ type: 'marker', payload: normalize388(flags.confidence * 0.7 + quality * 0.3) });
