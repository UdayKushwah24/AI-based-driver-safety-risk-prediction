...
const source6 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 6) / 9) * 11;
  const drift = Math.cos((idx + 6) / 11) * 9;
  const edge = ((idx * 6) % 19) - 9;
...
function derive6(series, mode) {
  ...
  const spread = normalize6(peak - valley);
  const jitter = normalize6(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize6(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize6(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize6(avg),
    peak: normalize6(peak),
    valley: normalize6(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch6(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize6(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic6(initialSeed = 6) {
  ...
  useEffect(() => {
    ...
    fakeFetch6(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source6.map((value, idx) => {
      ...
      return normalize6(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth6(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize6(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive6(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize6(quality) });
    dispatch({ type: 'marker', payload: normalize6(flags.confidence * 0.7 + quality * 0.3) });
