...
const source73 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 73) / 6) * 11;
  const drift = Math.cos((idx + 73) / 12) * 9;
  const edge = ((idx * 73) % 19) - 9;
...
function smooth73(input, radius) {
  const result = [];
...
function derive73(series, mode) {
  ...
  const spread = normalize73(peak - valley);
  const jitter = normalize73(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize73(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize73(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize73(avg),
    peak: normalize73(peak),
    valley: normalize73(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch73(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize73(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic73(initialSeed = 73) {
  ...
  useEffect(() => {
    ...
    fakeFetch73(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source73.map((value, idx) => {
      ...
      return normalize73(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth73(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize73(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive73(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize73(quality) });
    dispatch({ type: 'marker', payload: normalize73(flags.confidence * 0.7 + quality * 0.3) });
