...
const source104 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 104) / 9) * 11;
  const drift = Math.cos((idx + 104) / 10) * 9;
  const edge = ((idx * 104) % 19) - 9;
...
function smooth104(input, radius) {
  const result = [];
...
function derive104(series, mode) {
  ...
  const spread = normalize104(peak - valley);
  const jitter = normalize104(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize104(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize104(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize104(avg),
    peak: normalize104(peak),
    valley: normalize104(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch104(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize104(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic104(initialSeed = 104) {
  ...
  useEffect(() => {
    ...
    fakeFetch104(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source104.map((value, idx) => {
      ...
      return normalize104(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth104(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize104(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive104(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize104(quality) });
    dispatch({ type: 'marker', payload: normalize104(flags.confidence * 0.7 + quality * 0.3) });
