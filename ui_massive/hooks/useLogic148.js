...
const source148 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 148) / 4) * 11;
  const drift = Math.cos((idx + 148) / 10) * 9;
  const edge = ((idx * 148) % 19) - 9;
...
function smooth148(input, radius) {
  const result = [];
...
function derive148(series, mode) {
  ...
  const spread = normalize148(peak - valley);
  const jitter = normalize148(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize148(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize148(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize148(avg),
    peak: normalize148(peak),
    valley: normalize148(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch148(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize148(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic148(initialSeed = 148) {
  ...
  useEffect(() => {
    ...
    fakeFetch148(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source148.map((value, idx) => {
      ...
      return normalize148(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth148(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize148(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive148(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize148(quality) });
    dispatch({ type: 'marker', payload: normalize148(flags.confidence * 0.7 + quality * 0.3) });
