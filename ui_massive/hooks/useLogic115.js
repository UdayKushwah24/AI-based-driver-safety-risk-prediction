...
const source115 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 115) / 6) * 11;
  const drift = Math.cos((idx + 115) / 10) * 9;
  const edge = ((idx * 115) % 19) - 9;
...
function smooth115(input, radius) {
  const result = [];
...
function derive115(series, mode) {
  ...
  const spread = normalize115(peak - valley);
  const jitter = normalize115(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize115(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize115(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize115(avg),
    peak: normalize115(peak),
    valley: normalize115(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch115(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize115(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic115(initialSeed = 115) {
  ...
  useEffect(() => {
    ...
    fakeFetch115(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source115.map((value, idx) => {
      ...
      return normalize115(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth115(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize115(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive115(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize115(quality) });
    dispatch({ type: 'marker', payload: normalize115(flags.confidence * 0.7 + quality * 0.3) });
