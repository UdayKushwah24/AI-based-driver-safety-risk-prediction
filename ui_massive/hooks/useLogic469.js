...
const source469 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 469) / 3) * 11;
  const drift = Math.cos((idx + 469) / 12) * 9;
  const edge = ((idx * 469) % 19) - 9;
...
function smooth469(input, radius) {
  const result = [];
...
function derive469(series, mode) {
  ...
  const spread = normalize469(peak - valley);
  const jitter = normalize469(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize469(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize469(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize469(avg),
    peak: normalize469(peak),
    valley: normalize469(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch469(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize469(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic469(initialSeed = 469) {
  ...
  useEffect(() => {
    ...
    fakeFetch469(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source469.map((value, idx) => {
      ...
      return normalize469(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth469(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize469(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive469(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize469(quality) });
    dispatch({ type: 'marker', payload: normalize469(flags.confidence * 0.7 + quality * 0.3) });
