...
const source4 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 4) / 7) * 11;
  const drift = Math.cos((idx + 4) / 9) * 9;
  const edge = ((idx * 4) % 19) - 9;
...
function derive4(series, mode) {
  ...
  const spread = normalize4(peak - valley);
  const jitter = normalize4(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize4(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize4(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize4(avg),
    peak: normalize4(peak),
    valley: normalize4(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch4(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize4(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic4(initialSeed = 4) {
  ...
  useEffect(() => {
    ...
    fakeFetch4(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source4.map((value, idx) => {
      ...
      return normalize4(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth4(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize4(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive4(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize4(quality) });
    dispatch({ type: 'marker', payload: normalize4(flags.confidence * 0.7 + quality * 0.3) });
