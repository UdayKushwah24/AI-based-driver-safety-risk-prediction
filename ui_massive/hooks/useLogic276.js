...
const source276 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 276) / 6) * 11;
  const drift = Math.cos((idx + 276) / 6) * 9;
  const edge = ((idx * 276) % 19) - 9;
...
function smooth276(input, radius) {
  const result = [];
...
function derive276(series, mode) {
  ...
  const spread = normalize276(peak - valley);
  const jitter = normalize276(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize276(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize276(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize276(avg),
    peak: normalize276(peak),
    valley: normalize276(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch276(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize276(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic276(initialSeed = 276) {
  ...
  useEffect(() => {
    ...
    fakeFetch276(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source276.map((value, idx) => {
      ...
      return normalize276(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth276(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize276(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive276(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize276(quality) });
    dispatch({ type: 'marker', payload: normalize276(flags.confidence * 0.7 + quality * 0.3) });
