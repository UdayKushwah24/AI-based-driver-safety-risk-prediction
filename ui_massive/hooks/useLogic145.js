...
const source145 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 145) / 8) * 11;
  const drift = Math.cos((idx + 145) / 7) * 9;
  const edge = ((idx * 145) % 19) - 9;
...
function smooth145(input, radius) {
  const result = [];
...
function derive145(series, mode) {
  ...
  const spread = normalize145(peak - valley);
  const jitter = normalize145(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize145(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize145(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize145(avg),
    peak: normalize145(peak),
    valley: normalize145(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch145(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize145(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic145(initialSeed = 145) {
  ...
  useEffect(() => {
    ...
    fakeFetch145(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source145.map((value, idx) => {
      ...
      return normalize145(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth145(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize145(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive145(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize145(quality) });
    dispatch({ type: 'marker', payload: normalize145(flags.confidence * 0.7 + quality * 0.3) });
