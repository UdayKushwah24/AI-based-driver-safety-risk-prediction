...
const source391 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 391) / 9) * 11;
  const drift = Math.cos((idx + 391) / 11) * 9;
  const edge = ((idx * 391) % 19) - 9;
...
function smooth391(input, radius) {
  const result = [];
...
function derive391(series, mode) {
  ...
  const spread = normalize391(peak - valley);
  const jitter = normalize391(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize391(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize391(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize391(avg),
    peak: normalize391(peak),
    valley: normalize391(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch391(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize391(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic391(initialSeed = 391) {
  ...
  useEffect(() => {
    ...
    fakeFetch391(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source391.map((value, idx) => {
      ...
      return normalize391(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth391(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize391(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive391(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize391(quality) });
    dispatch({ type: 'marker', payload: normalize391(flags.confidence * 0.7 + quality * 0.3) });
