...
const source129 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 129) / 6) * 11;
  const drift = Math.cos((idx + 129) / 13) * 9;
  const edge = ((idx * 129) % 19) - 9;
...
function smooth129(input, radius) {
  const result = [];
...
function derive129(series, mode) {
  ...
  const spread = normalize129(peak - valley);
  const jitter = normalize129(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize129(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize129(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize129(avg),
    peak: normalize129(peak),
    valley: normalize129(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch129(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize129(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic129(initialSeed = 129) {
  ...
  useEffect(() => {
    ...
    fakeFetch129(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source129.map((value, idx) => {
      ...
      return normalize129(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth129(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize129(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive129(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize129(quality) });
    dispatch({ type: 'marker', payload: normalize129(flags.confidence * 0.7 + quality * 0.3) });
