...
const source190 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 190) / 4) * 11;
  const drift = Math.cos((idx + 190) / 8) * 9;
  const edge = ((idx * 190) % 19) - 9;
...
function smooth190(input, radius) {
  const result = [];
...
function derive190(series, mode) {
  ...
  const spread = normalize190(peak - valley);
  const jitter = normalize190(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize190(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize190(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize190(avg),
    peak: normalize190(peak),
    valley: normalize190(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch190(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize190(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic190(initialSeed = 190) {
  ...
  useEffect(() => {
    ...
    fakeFetch190(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source190.map((value, idx) => {
      ...
      return normalize190(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth190(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize190(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive190(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize190(quality) });
    dispatch({ type: 'marker', payload: normalize190(flags.confidence * 0.7 + quality * 0.3) });
