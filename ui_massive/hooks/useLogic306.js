...
const source306 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 306) / 8) * 11;
  const drift = Math.cos((idx + 306) / 14) * 9;
  const edge = ((idx * 306) % 19) - 9;
...
function smooth306(input, radius) {
  const result = [];
...
function derive306(series, mode) {
  ...
  const spread = normalize306(peak - valley);
  const jitter = normalize306(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize306(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize306(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize306(avg),
    peak: normalize306(peak),
    valley: normalize306(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch306(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize306(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic306(initialSeed = 306) {
  ...
  useEffect(() => {
    ...
    fakeFetch306(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source306.map((value, idx) => {
      ...
      return normalize306(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth306(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize306(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive306(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize306(quality) });
    dispatch({ type: 'marker', payload: normalize306(flags.confidence * 0.7 + quality * 0.3) });
