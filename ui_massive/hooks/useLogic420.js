...
const source420 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 420) / 3) * 11;
  const drift = Math.cos((idx + 420) / 7) * 9;
  const edge = ((idx * 420) % 19) - 9;
...
function smooth420(input, radius) {
  const result = [];
...
function derive420(series, mode) {
  ...
  const spread = normalize420(peak - valley);
  const jitter = normalize420(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize420(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize420(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize420(avg),
    peak: normalize420(peak),
    valley: normalize420(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch420(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize420(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic420(initialSeed = 420) {
  ...
  useEffect(() => {
    ...
    fakeFetch420(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source420.map((value, idx) => {
      ...
      return normalize420(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth420(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize420(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive420(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize420(quality) });
    dispatch({ type: 'marker', payload: normalize420(flags.confidence * 0.7 + quality * 0.3) });
