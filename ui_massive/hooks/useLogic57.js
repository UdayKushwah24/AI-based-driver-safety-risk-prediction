...
const source57 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 57) / 4) * 11;
  const drift = Math.cos((idx + 57) / 7) * 9;
  const edge = ((idx * 57) % 19) - 9;
...
function smooth57(input, radius) {
  const result = [];
...
function derive57(series, mode) {
  ...
  const spread = normalize57(peak - valley);
  const jitter = normalize57(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize57(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize57(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize57(avg),
    peak: normalize57(peak),
    valley: normalize57(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch57(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize57(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic57(initialSeed = 57) {
  ...
  useEffect(() => {
    ...
    fakeFetch57(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source57.map((value, idx) => {
      ...
      return normalize57(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth57(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize57(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive57(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize57(quality) });
    dispatch({ type: 'marker', payload: normalize57(flags.confidence * 0.7 + quality * 0.3) });
