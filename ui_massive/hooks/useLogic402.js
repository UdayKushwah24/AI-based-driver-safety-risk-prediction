...
const source402 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 402) / 6) * 11;
  const drift = Math.cos((idx + 402) / 11) * 9;
  const edge = ((idx * 402) % 19) - 9;
...
function smooth402(input, radius) {
  const result = [];
...
function derive402(series, mode) {
  ...
  const spread = normalize402(peak - valley);
  const jitter = normalize402(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize402(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize402(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize402(avg),
    peak: normalize402(peak),
    valley: normalize402(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch402(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize402(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic402(initialSeed = 402) {
  ...
  useEffect(() => {
    ...
    fakeFetch402(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source402.map((value, idx) => {
      ...
      return normalize402(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth402(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize402(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive402(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize402(quality) });
    dispatch({ type: 'marker', payload: normalize402(flags.confidence * 0.7 + quality * 0.3) });
