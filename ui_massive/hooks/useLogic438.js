...
const source438 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 438) / 7) * 11;
  const drift = Math.cos((idx + 438) / 14) * 9;
  const edge = ((idx * 438) % 19) - 9;
...
function smooth438(input, radius) {
  const result = [];
...
function derive438(series, mode) {
  ...
  const spread = normalize438(peak - valley);
  const jitter = normalize438(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize438(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize438(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize438(avg),
    peak: normalize438(peak),
    valley: normalize438(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch438(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize438(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic438(initialSeed = 438) {
  ...
  useEffect(() => {
    ...
    fakeFetch438(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source438.map((value, idx) => {
      ...
      return normalize438(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth438(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize438(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive438(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize438(quality) });
    dispatch({ type: 'marker', payload: normalize438(flags.confidence * 0.7 + quality * 0.3) });
