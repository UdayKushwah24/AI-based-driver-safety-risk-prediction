...
const source58 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 58) / 5) * 11;
  const drift = Math.cos((idx + 58) / 8) * 9;
  const edge = ((idx * 58) % 19) - 9;
...
function smooth58(input, radius) {
  const result = [];
...
function derive58(series, mode) {
  ...
  const spread = normalize58(peak - valley);
  const jitter = normalize58(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize58(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize58(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize58(avg),
    peak: normalize58(peak),
    valley: normalize58(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch58(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize58(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic58(initialSeed = 58) {
  ...
  useEffect(() => {
    ...
    fakeFetch58(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source58.map((value, idx) => {
      ...
      return normalize58(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth58(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize58(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive58(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize58(quality) });
    dispatch({ type: 'marker', payload: normalize58(flags.confidence * 0.7 + quality * 0.3) });
