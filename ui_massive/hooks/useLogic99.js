...
const source99 = Array.from({ length: 96 }, (_, idx) => {
  const wave = Math.sin((idx + 99) / 4) * 11;
  const drift = Math.cos((idx + 99) / 5) * 9;
  const edge = ((idx * 99) % 19) - 9;
...
function smooth99(input, radius) {
  const result = [];
...
function derive99(series, mode) {
  ...
  const spread = normalize99(peak - valley);
  const jitter = normalize99(Math.sqrt(variance));
  const modeShift = mode === 'safe' ? -5 : mode === 'aggressive' ? 8 : 1;
  const risk = normalize99(avg * 0.36 + spread * 0.29 + jitter * 0.35 + modeShift);
  const confidence = normalize99(100 - Math.abs(risk - 50) * 0.9);
  return {
    avg: normalize99(avg),
    peak: normalize99(peak),
    valley: normalize99(valley),
    spread,
    jitter,
    risk,
    confidence,
...
function fakeFetch99(seed, mode, pressure) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const payload = Array.from({ length: 22 }, (_, idx) => {
        ...
        return normalize99(a + b + c + (pressure % 7));
      });
      resolve(payload);
    }, wait);
  });
...
export function useLogic99(initialSeed = 99) {
  ...
  useEffect(() => {
    ...
    fakeFetch99(seed, state.mode, state.pressure).then((payload) => {
      if (!live) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = source99.map((value, idx) => {
      ...
      return normalize99(value + queueValue * 0.5 + modeShift + pressureShift + idx * 0.04 + (seed % 11));
    });
    return smooth99(merged, state.granularity);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 6) {
      ...
      chunks.push(normalize99(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => derive99(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, value, idx) => sum + value * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize99(quality) });
    dispatch({ type: 'marker', payload: normalize99(flags.confidence * 0.7 + quality * 0.3) });
