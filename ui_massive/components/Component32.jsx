...
import '../styles/component32.css';

function MetricCard32({ label, value, max }) {
...
function TreeNode32({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode32 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component32({ seed = 32, title = 'Component32' }) {
  const hook = useLogic32(seed + 32);
  const [state, dispatch] = useReducer(reducer32, initialState32);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm32(seed));
  ...
  const table = useMemo(() => buildTableState32(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm32(form);
    setErrors(found);
  ...
  return (
    <section className="component-32">
      ...
      <div className="grid-32">
        <div className="panel-32">
          {cards.map((item, idx) => (
            <MetricCard32 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
