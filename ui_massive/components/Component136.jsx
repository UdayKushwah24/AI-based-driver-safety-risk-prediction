...
import '../styles/component136.css';

function MetricCard136({ label, value, max }) {
...
function TreeNode136({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode136 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component136({ seed = 136, title = 'Component136' }) {
  const hook = useLogic136(seed + 136);
  const [state, dispatch] = useReducer(reducer136, initialState136);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm136(seed));
  ...
  const table = useMemo(() => buildTableState136(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm136(form);
    setErrors(found);
  ...
  return (
    <section className="component-136">
      ...
      <div className="grid-136">
        <div className="panel-136">
          {cards.map((item, idx) => (
            <MetricCard136 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
