...
import { useLogic425 } from '../hooks/useLogic425.js';
import { buildNestedTree425, buildTableState425, heavyCompute425, transformDataset425 } from '../utils/compute425.js';
import { buildForecast425, detectAnomalies425, segmentDistribution425 } from '../utils/compute425.js';
import { buildActivityMap425, rankBuckets425, summarizePipeline425, synthesizeFlow425 } from '../pipelines/pipeline425.js';
import '../styles/component425.css';

function MetricCard425({ label, value, max }) {
...
function TreeNode425({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode425 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState425 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer425(state, action) {
...
export default function Component425({ seed = 425, title = 'Component425' }) {
  const hook = useLogic425(seed + 425);
  const [state, dispatch] = useReducer(reducer425, initialState425);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm425(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset425(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState425(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree425(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute425(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast425(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies425(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution425(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow425(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets425(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap425(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline425(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm425(form);
    setErrors(found);
  ...
  return (
    <section className="component-425">
      <div className="header-425">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-425">
      ...
      <div className="grid-425">
        <div className="panel-425">
          {cards.map((item, idx) => (
            <MetricCard425 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
