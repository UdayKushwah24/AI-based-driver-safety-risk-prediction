...
import '../styles/component448.css';

function MetricCard448({ label, value, max }) {
...
function TreeNode448({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode448 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState448 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer448(state, action) {
...
export default function Component448({ seed = 448, title = 'Component448' }) {
  const hook = useLogic448(seed + 448);
  const [state, dispatch] = useReducer(reducer448, initialState448);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm448(seed));
  ...
  const table = useMemo(() => buildTableState448(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm448(form);
    setErrors(found);
  ...
  return (
    <section className="component-448">
      <div className="header-448">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-448">
      ...
      <div className="grid-448">
        <div className="panel-448">
          {cards.map((item, idx) => (
            <MetricCard448 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
