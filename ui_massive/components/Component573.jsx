...
import { useLogic23 } from '../hooks/useLogic23.js';
import { buildNestedTree23, buildTableState23, heavyCompute23, transformDataset23 } from '../utils/compute23.js';
import { buildForecast23, detectAnomalies23, segmentDistribution23 } from '../utils/compute23.js';
import { buildActivityMap23, rankBuckets23, summarizePipeline23, synthesizeFlow23 } from '../pipelines/pipeline23.js';
import '../styles/component573.css';

function MetricCard573({ label, value, max }) {
...
function TreeNode573({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode573 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState573 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer573(state, action) {
...
export default function Component573({ seed = 573, title = 'Component573' }) {
  const hook = useLogic23(seed + 573);
  const [state, dispatch] = useReducer(reducer573, initialState573);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm573(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset23(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState23(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree23(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute23(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast23(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies23(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution23(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow23(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets23(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap23(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline23(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm573(form);
    setErrors(found);
  ...
  return (
    <section className="component-573">
      <div className="header-573">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-573">
      ...
      <div className="grid-573">
        <div className="panel-573">
          {cards.map((item, idx) => (
            <MetricCard573 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
