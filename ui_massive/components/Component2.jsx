...
function MetricCard2({ label, value, max }) {
...
function TreeNode2({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode2 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component2({ seed = 2, title = 'Component2' }) {
  const hook = useLogic2(seed + 2);
  const [state, dispatch] = useReducer(reducer2, initialState2);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm2(seed));
  ...
  const table = useMemo(() => buildTableState2(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm2(form);
    setErrors(found);
    ...
    <section className="component-2">
      ...
      <div className="grid-2">
        <div className="panel-2">
          {cards.map((item, idx) => (
            <MetricCard2 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
