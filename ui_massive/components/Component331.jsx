...
import '../styles/component331.css';

function MetricCard331({ label, value, max }) {
...
function TreeNode331({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode331 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState331 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer331(state, action) {
...
export default function Component331({ seed = 331, title = 'Component331' }) {
  const hook = useLogic331(seed + 331);
  const [state, dispatch] = useReducer(reducer331, initialState331);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm331(seed));
  ...
  const table = useMemo(() => buildTableState331(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm331(form);
    setErrors(found);
  ...
  return (
    <section className="component-331">
      <div className="header-331">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-331">
      ...
      <div className="grid-331">
        <div className="panel-331">
          {cards.map((item, idx) => (
            <MetricCard331 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
