...
import '../styles/component235.css';

function MetricCard235({ label, value, max }) {
...
function TreeNode235({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode235 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState235 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer235(state, action) {
...
export default function Component235({ seed = 235, title = 'Component235' }) {
  const hook = useLogic235(seed + 235);
  const [state, dispatch] = useReducer(reducer235, initialState235);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm235(seed));
  ...
  const table = useMemo(() => buildTableState235(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm235(form);
    setErrors(found);
  ...
  return (
    <section className="component-235">
      <div className="header-235">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-235">
      ...
      <div className="grid-235">
        <div className="panel-235">
          {cards.map((item, idx) => (
            <MetricCard235 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
