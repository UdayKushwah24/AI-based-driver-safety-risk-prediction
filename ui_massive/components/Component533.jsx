...
import '../styles/component533.css';

function MetricCard533({ label, value, max }) {
...
function TreeNode533({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode533 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState533 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer533(state, action) {
...
export default function Component533({ seed = 533, title = 'Component533' }) {
  const hook = useLogic533(seed + 533);
  const [state, dispatch] = useReducer(reducer533, initialState533);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm533(seed));
  ...
  const table = useMemo(() => buildTableState533(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm533(form);
    setErrors(found);
  ...
  return (
    <section className="component-533">
      <div className="header-533">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-533">
      ...
      <div className="grid-533">
        <div className="panel-533">
          {cards.map((item, idx) => (
            <MetricCard533 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
