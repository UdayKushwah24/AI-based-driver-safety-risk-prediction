...
function MetricCard3({ label, value, max }) {
...
function TreeNode3({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode3 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component3({ seed = 3, title = 'Component3' }) {
  const hook = useLogic3(seed + 3);
  const [state, dispatch] = useReducer(reducer3, initialState3);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm3(seed));
  ...
  const table = useMemo(() => buildTableState3(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm3(form);
    setErrors(found);
    ...
    <section className="component-3">
      ...
      <div className="grid-3">
        <div className="panel-3">
          {cards.map((item, idx) => (
            <MetricCard3 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
