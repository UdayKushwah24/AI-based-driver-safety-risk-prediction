...
import '../styles/component52.css';

function MetricCard52({ label, value, max }) {
...
function TreeNode52({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode52 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState52 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer52(state, action) {
...
export default function Component52({ seed = 52, title = 'Component52' }) {
  const hook = useLogic52(seed + 52);
  const [state, dispatch] = useReducer(reducer52, initialState52);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm52(seed));
  ...
  const table = useMemo(() => buildTableState52(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm52(form);
    setErrors(found);
  ...
  return (
    <section className="component-52">
      ...
      <div className="grid-52">
        <div className="panel-52">
          {cards.map((item, idx) => (
            <MetricCard52 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
