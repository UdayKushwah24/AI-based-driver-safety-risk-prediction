...
import '../styles/component495.css';

function MetricCard495({ label, value, max }) {
...
function TreeNode495({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode495 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState495 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer495(state, action) {
...
export default function Component495({ seed = 495, title = 'Component495' }) {
  const hook = useLogic495(seed + 495);
  const [state, dispatch] = useReducer(reducer495, initialState495);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm495(seed));
  ...
  const table = useMemo(() => buildTableState495(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm495(form);
    setErrors(found);
  ...
  return (
    <section className="component-495">
      <div className="header-495">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-495">
      ...
      <div className="grid-495">
        <div className="panel-495">
          {cards.map((item, idx) => (
            <MetricCard495 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
