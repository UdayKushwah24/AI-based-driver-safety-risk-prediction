...
import '../styles/component30.css';

function MetricCard30({ label, value, max }) {
...
function TreeNode30({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode30 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component30({ seed = 30, title = 'Component30' }) {
  const hook = useLogic30(seed + 30);
  const [state, dispatch] = useReducer(reducer30, initialState30);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm30(seed));
  ...
  const table = useMemo(() => buildTableState30(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm30(form);
    setErrors(found);
  ...
  return (
    <section className="component-30">
      ...
      <div className="grid-30">
        <div className="panel-30">
          {cards.map((item, idx) => (
            <MetricCard30 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
