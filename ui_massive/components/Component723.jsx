...
import '../styles/component723.css';

function MetricCard723({ label, value, max }) {
...
function TreeNode723({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode723 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState723 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer723(state, action) {
...
export default function Component723({ seed = 723, title = 'Component723' }) {
  const hook = useLogic173(seed + 723);
  const [state, dispatch] = useReducer(reducer723, initialState723);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm723(seed));
  ...
  const table = useMemo(() => buildTableState173(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm723(form);
    setErrors(found);
  ...
  return (
    <section className="component-723">
      <div className="header-723">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-723">
      ...
      <div className="grid-723">
        <div className="panel-723">
          {cards.map((item, idx) => (
            <MetricCard723 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
