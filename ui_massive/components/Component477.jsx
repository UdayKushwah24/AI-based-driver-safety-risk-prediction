...
import '../styles/component477.css';

function MetricCard477({ label, value, max }) {
...
function TreeNode477({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode477 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState477 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer477(state, action) {
...
export default function Component477({ seed = 477, title = 'Component477' }) {
  const hook = useLogic477(seed + 477);
  const [state, dispatch] = useReducer(reducer477, initialState477);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm477(seed));
  ...
  const table = useMemo(() => buildTableState477(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm477(form);
    setErrors(found);
  ...
  return (
    <section className="component-477">
      <div className="header-477">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-477">
      ...
      <div className="grid-477">
        <div className="panel-477">
          {cards.map((item, idx) => (
            <MetricCard477 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
