...
import '../styles/component29.css';

function MetricCard29({ label, value, max }) {
...
function TreeNode29({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode29 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component29({ seed = 29, title = 'Component29' }) {
  const hook = useLogic29(seed + 29);
  const [state, dispatch] = useReducer(reducer29, initialState29);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm29(seed));
  ...
  const table = useMemo(() => buildTableState29(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm29(form);
    setErrors(found);
  ...
  return (
    <section className="component-29">
      ...
      <div className="grid-29">
        <div className="panel-29">
          {cards.map((item, idx) => (
            <MetricCard29 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
