...
import { useLogic408 } from '../hooks/useLogic408.js';
import { buildNestedTree408, buildTableState408, heavyCompute408, transformDataset408 } from '../utils/compute408.js';
import { buildForecast408, detectAnomalies408, segmentDistribution408 } from '../utils/compute408.js';
import { buildActivityMap408, rankBuckets408, summarizePipeline408, synthesizeFlow408 } from '../pipelines/pipeline408.js';
import '../styles/component958.css';

function MetricCard958({ label, value, max }) {
...
function TreeNode958({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode958 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState958 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer958(state, action) {
...
export default function Component958({ seed = 958, title = 'Component958' }) {
  const hook = useLogic408(seed + 958);
  const [state, dispatch] = useReducer(reducer958, initialState958);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm958(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset408(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState408(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree408(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute408(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast408(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies408(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution408(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow408(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets408(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap408(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline408(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm958(form);
    setErrors(found);
  ...
  return (
    <section className="component-958">
      <div className="header-958">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-958">
      ...
      <div className="grid-958">
        <div className="panel-958">
          {cards.map((item, idx) => (
            <MetricCard958 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
