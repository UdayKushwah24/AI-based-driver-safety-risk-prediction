...
import { useLogic230 } from '../hooks/useLogic230.js';
import { buildNestedTree230, buildTableState230, heavyCompute230, transformDataset230 } from '../utils/compute230.js';
import { buildForecast230, detectAnomalies230, segmentDistribution230 } from '../utils/compute230.js';
import { buildActivityMap230, rankBuckets230, summarizePipeline230, synthesizeFlow230 } from '../pipelines/pipeline230.js';
import '../styles/component780.css';

function MetricCard780({ label, value, max }) {
...
function TreeNode780({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode780 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState780 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer780(state, action) {
...
export default function Component780({ seed = 780, title = 'Component780' }) {
  const hook = useLogic230(seed + 780);
  const [state, dispatch] = useReducer(reducer780, initialState780);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm780(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset230(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState230(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree230(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute230(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast230(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies230(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution230(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow230(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets230(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap230(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline230(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm780(form);
    setErrors(found);
  ...
  return (
    <section className="component-780">
      <div className="header-780">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-780">
      ...
      <div className="grid-780">
        <div className="panel-780">
          {cards.map((item, idx) => (
            <MetricCard780 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
