...
import '../styles/component89.css';

function MetricCard89({ label, value, max }) {
...
function TreeNode89({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode89 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component89({ seed = 89, title = 'Component89' }) {
  const hook = useLogic89(seed + 89);
  const [state, dispatch] = useReducer(reducer89, initialState89);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm89(seed));
  ...
  const table = useMemo(() => buildTableState89(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm89(form);
    setErrors(found);
  ...
  return (
    <section className="component-89">
      ...
      <div className="grid-89">
        <div className="panel-89">
          {cards.map((item, idx) => (
            <MetricCard89 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
