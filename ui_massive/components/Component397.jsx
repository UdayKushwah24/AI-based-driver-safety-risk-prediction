...
import { useLogic397 } from '../hooks/useLogic397.js';
import { buildNestedTree397, buildTableState397, heavyCompute397, transformDataset397 } from '../utils/compute397.js';
import { buildForecast397, detectAnomalies397, segmentDistribution397 } from '../utils/compute397.js';
import { buildActivityMap397, rankBuckets397, summarizePipeline397, synthesizeFlow397 } from '../pipelines/pipeline397.js';
import '../styles/component397.css';

function MetricCard397({ label, value, max }) {
...
function TreeNode397({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode397 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState397 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer397(state, action) {
...
export default function Component397({ seed = 397, title = 'Component397' }) {
  const hook = useLogic397(seed + 397);
  const [state, dispatch] = useReducer(reducer397, initialState397);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm397(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset397(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState397(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree397(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute397(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast397(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies397(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution397(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow397(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets397(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap397(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline397(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm397(form);
    setErrors(found);
  ...
  return (
    <section className="component-397">
      <div className="header-397">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-397">
      ...
      <div className="grid-397">
        <div className="panel-397">
          {cards.map((item, idx) => (
            <MetricCard397 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
