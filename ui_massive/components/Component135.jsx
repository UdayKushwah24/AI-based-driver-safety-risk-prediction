...
import '../styles/component135.css';

function MetricCard135({ label, value, max }) {
...
function TreeNode135({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode135 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState135 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer135(state, action) {
...
export default function Component135({ seed = 135, title = 'Component135' }) {
  const hook = useLogic135(seed + 135);
  const [state, dispatch] = useReducer(reducer135, initialState135);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm135(seed));
  ...
  const table = useMemo(() => buildTableState135(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm135(form);
    setErrors(found);
  ...
  return (
    <section className="component-135">
      <div className="header-135">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-135">
      ...
      <div className="grid-135">
        <div className="panel-135">
          {cards.map((item, idx) => (
            <MetricCard135 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
