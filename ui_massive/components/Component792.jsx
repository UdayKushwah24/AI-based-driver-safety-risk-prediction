...
import '../styles/component792.css';

function MetricCard792({ label, value, max }) {
...
function TreeNode792({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode792 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState792 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer792(state, action) {
...
export default function Component792({ seed = 792, title = 'Component792' }) {
  const hook = useLogic242(seed + 792);
  const [state, dispatch] = useReducer(reducer792, initialState792);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm792(seed));
  ...
  const table = useMemo(() => buildTableState242(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm792(form);
    setErrors(found);
  ...
  return (
    <section className="component-792">
      <div className="header-792">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-792">
      ...
      <div className="grid-792">
        <div className="panel-792">
          {cards.map((item, idx) => (
            <MetricCard792 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
