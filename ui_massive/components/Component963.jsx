...
import { useLogic413 } from '../hooks/useLogic413.js';
import { buildNestedTree413, buildTableState413, heavyCompute413, transformDataset413 } from '../utils/compute413.js';
import { buildForecast413, detectAnomalies413, segmentDistribution413 } from '../utils/compute413.js';
import { buildActivityMap413, rankBuckets413, summarizePipeline413, synthesizeFlow413 } from '../pipelines/pipeline413.js';
import '../styles/component963.css';

function MetricCard963({ label, value, max }) {
...
function TreeNode963({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode963 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState963 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer963(state, action) {
...
export default function Component963({ seed = 963, title = 'Component963' }) {
  const hook = useLogic413(seed + 963);
  const [state, dispatch] = useReducer(reducer963, initialState963);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm963(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset413(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState413(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree413(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute413(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast413(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies413(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution413(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow413(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets413(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap413(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline413(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm963(form);
    setErrors(found);
  ...
  return (
    <section className="component-963">
      <div className="header-963">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-963">
      ...
      <div className="grid-963">
        <div className="panel-963">
          {cards.map((item, idx) => (
            <MetricCard963 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
