...
import '../styles/component485.css';

function MetricCard485({ label, value, max }) {
...
function TreeNode485({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode485 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState485 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer485(state, action) {
...
export default function Component485({ seed = 485, title = 'Component485' }) {
  const hook = useLogic485(seed + 485);
  const [state, dispatch] = useReducer(reducer485, initialState485);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm485(seed));
  ...
  const table = useMemo(() => buildTableState485(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm485(form);
    setErrors(found);
  ...
  return (
    <section className="component-485">
      <div className="header-485">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-485">
      ...
      <div className="grid-485">
        <div className="panel-485">
          {cards.map((item, idx) => (
            <MetricCard485 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
