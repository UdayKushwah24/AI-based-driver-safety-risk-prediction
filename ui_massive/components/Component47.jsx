...
import '../styles/component47.css';

function MetricCard47({ label, value, max }) {
...
function TreeNode47({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode47 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component47({ seed = 47, title = 'Component47' }) {
  const hook = useLogic47(seed + 47);
  const [state, dispatch] = useReducer(reducer47, initialState47);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm47(seed));
  ...
  const table = useMemo(() => buildTableState47(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm47(form);
    setErrors(found);
  ...
  return (
    <section className="component-47">
      ...
      <div className="grid-47">
        <div className="panel-47">
          {cards.map((item, idx) => (
            <MetricCard47 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
