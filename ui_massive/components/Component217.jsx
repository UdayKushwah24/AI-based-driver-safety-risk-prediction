...
import '../styles/component217.css';

function MetricCard217({ label, value, max }) {
...
function TreeNode217({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode217 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState217 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer217(state, action) {
...
export default function Component217({ seed = 217, title = 'Component217' }) {
  const hook = useLogic217(seed + 217);
  const [state, dispatch] = useReducer(reducer217, initialState217);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm217(seed));
  ...
  const table = useMemo(() => buildTableState217(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm217(form);
    setErrors(found);
  ...
  return (
    <section className="component-217">
      <div className="header-217">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-217">
      ...
      <div className="grid-217">
        <div className="panel-217">
          {cards.map((item, idx) => (
            <MetricCard217 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
