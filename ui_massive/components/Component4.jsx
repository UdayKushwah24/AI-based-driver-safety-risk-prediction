...
function MetricCard4({ label, value, max }) {
...
function TreeNode4({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode4 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component4({ seed = 4, title = 'Component4' }) {
  const hook = useLogic4(seed + 4);
  const [state, dispatch] = useReducer(reducer4, initialState4);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm4(seed));
  ...
  const table = useMemo(() => buildTableState4(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm4(form);
    setErrors(found);
    ...
    <section className="component-4">
      ...
      <div className="grid-4">
        <div className="panel-4">
          {cards.map((item, idx) => (
            <MetricCard4 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
