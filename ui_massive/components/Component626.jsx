...
import { useLogic76 } from '../hooks/useLogic76.js';
import { buildNestedTree76, buildTableState76, heavyCompute76, transformDataset76 } from '../utils/compute76.js';
import { buildForecast76, detectAnomalies76, segmentDistribution76 } from '../utils/compute76.js';
import { buildActivityMap76, rankBuckets76, summarizePipeline76, synthesizeFlow76 } from '../pipelines/pipeline76.js';
import '../styles/component626.css';

function MetricCard626({ label, value, max }) {
...
function TreeNode626({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode626 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState626 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer626(state, action) {
...
export default function Component626({ seed = 626, title = 'Component626' }) {
  const hook = useLogic76(seed + 626);
  const [state, dispatch] = useReducer(reducer626, initialState626);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm626(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset76(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState76(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree76(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute76(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast76(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies76(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution76(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow76(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets76(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap76(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline76(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm626(form);
    setErrors(found);
  ...
  return (
    <section className="component-626">
      <div className="header-626">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-626">
      ...
      <div className="grid-626">
        <div className="panel-626">
          {cards.map((item, idx) => (
            <MetricCard626 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
