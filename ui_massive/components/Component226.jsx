...
import '../styles/component226.css';

function MetricCard226({ label, value, max }) {
...
function TreeNode226({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode226 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component226({ seed = 226, title = 'Component226' }) {
  const hook = useLogic226(seed + 226);
  const [state, dispatch] = useReducer(reducer226, initialState226);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm226(seed));
  ...
  const table = useMemo(() => buildTableState226(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm226(form);
    setErrors(found);
  ...
  return (
    <section className="component-226">
      <div className="header-226">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-226">
      ...
      <div className="grid-226">
        <div className="panel-226">
          {cards.map((item, idx) => (
            <MetricCard226 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
