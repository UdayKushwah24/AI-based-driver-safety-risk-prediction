...
import { useLogic200 } from '../hooks/useLogic200.js';
import { buildNestedTree200, buildTableState200, heavyCompute200, transformDataset200 } from '../utils/compute200.js';
import { buildForecast200, detectAnomalies200, segmentDistribution200 } from '../utils/compute200.js';
import { buildActivityMap200, rankBuckets200, summarizePipeline200, synthesizeFlow200 } from '../pipelines/pipeline200.js';
import '../styles/component750.css';

function MetricCard750({ label, value, max }) {
...
function TreeNode750({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode750 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState750 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer750(state, action) {
...
export default function Component750({ seed = 750, title = 'Component750' }) {
  const hook = useLogic200(seed + 750);
  const [state, dispatch] = useReducer(reducer750, initialState750);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm750(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset200(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState200(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree200(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute200(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast200(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies200(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution200(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow200(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets200(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap200(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline200(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm750(form);
    setErrors(found);
  ...
  return (
    <section className="component-750">
      <div className="header-750">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-750">
      ...
      <div className="grid-750">
        <div className="panel-750">
          {cards.map((item, idx) => (
            <MetricCard750 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
