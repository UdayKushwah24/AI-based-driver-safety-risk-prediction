...
import '../styles/component66.css';

function MetricCard66({ label, value, max }) {
...
function TreeNode66({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode66 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component66({ seed = 66, title = 'Component66' }) {
  const hook = useLogic66(seed + 66);
  const [state, dispatch] = useReducer(reducer66, initialState66);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm66(seed));
  ...
  const table = useMemo(() => buildTableState66(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm66(form);
    setErrors(found);
  ...
  return (
    <section className="component-66">
      ...
      <div className="grid-66">
        <div className="panel-66">
          {cards.map((item, idx) => (
            <MetricCard66 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
