...
import '../styles/component24.css';

function MetricCard24({ label, value, max }) {
...
function TreeNode24({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode24 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component24({ seed = 24, title = 'Component24' }) {
  const hook = useLogic24(seed + 24);
  const [state, dispatch] = useReducer(reducer24, initialState24);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm24(seed));
  ...
  const table = useMemo(() => buildTableState24(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm24(form);
    setErrors(found);
  ...
  return (
    <section className="component-24">
      ...
      <div className="grid-24">
        <div className="panel-24">
          {cards.map((item, idx) => (
            <MetricCard24 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
