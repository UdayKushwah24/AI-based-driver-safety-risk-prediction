...
import '../styles/component233.css';

function MetricCard233({ label, value, max }) {
...
function TreeNode233({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode233 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState233 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer233(state, action) {
...
export default function Component233({ seed = 233, title = 'Component233' }) {
  const hook = useLogic233(seed + 233);
  const [state, dispatch] = useReducer(reducer233, initialState233);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm233(seed));
  ...
  const table = useMemo(() => buildTableState233(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm233(form);
    setErrors(found);
  ...
  return (
    <section className="component-233">
      <div className="header-233">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-233">
      ...
      <div className="grid-233">
        <div className="panel-233">
          {cards.map((item, idx) => (
            <MetricCard233 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
