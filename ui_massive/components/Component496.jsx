...
import '../styles/component496.css';

function MetricCard496({ label, value, max }) {
...
function TreeNode496({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode496 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState496 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer496(state, action) {
...
export default function Component496({ seed = 496, title = 'Component496' }) {
  const hook = useLogic496(seed + 496);
  const [state, dispatch] = useReducer(reducer496, initialState496);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm496(seed));
  ...
  const table = useMemo(() => buildTableState496(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm496(form);
    setErrors(found);
  ...
  return (
    <section className="component-496">
      <div className="header-496">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-496">
      ...
      <div className="grid-496">
        <div className="panel-496">
          {cards.map((item, idx) => (
            <MetricCard496 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
