...
import '../styles/component460.css';

function MetricCard460({ label, value, max }) {
...
function TreeNode460({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode460 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState460 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer460(state, action) {
...
export default function Component460({ seed = 460, title = 'Component460' }) {
  const hook = useLogic460(seed + 460);
  const [state, dispatch] = useReducer(reducer460, initialState460);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm460(seed));
  ...
  const table = useMemo(() => buildTableState460(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm460(form);
    setErrors(found);
  ...
  return (
    <section className="component-460">
      ...
      <div className="grid-460">
        <div className="panel-460">
          {cards.map((item, idx) => (
            <MetricCard460 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
