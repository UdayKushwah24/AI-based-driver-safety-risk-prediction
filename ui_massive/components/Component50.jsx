...
import '../styles/component50.css';

function MetricCard50({ label, value, max }) {
...
function TreeNode50({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode50 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component50({ seed = 50, title = 'Component50' }) {
  const hook = useLogic50(seed + 50);
  const [state, dispatch] = useReducer(reducer50, initialState50);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm50(seed));
  ...
  const table = useMemo(() => buildTableState50(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm50(form);
    setErrors(found);
  ...
  return (
    <section className="component-50">
      ...
      <div className="grid-50">
        <div className="panel-50">
          {cards.map((item, idx) => (
            <MetricCard50 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
