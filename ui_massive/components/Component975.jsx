...
import '../styles/component975.css';

function MetricCard975({ label, value, max }) {
...
function TreeNode975({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode975 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState975 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer975(state, action) {
...
export default function Component975({ seed = 975, title = 'Component975' }) {
  const hook = useLogic425(seed + 975);
  const [state, dispatch] = useReducer(reducer975, initialState975);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm975(seed));
  ...
  const table = useMemo(() => buildTableState425(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm975(form);
    setErrors(found);
  ...
  return (
    <section className="component-975">
      <div className="header-975">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-975">
      ...
      <div className="grid-975">
        <div className="panel-975">
          {cards.map((item, idx) => (
            <MetricCard975 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
