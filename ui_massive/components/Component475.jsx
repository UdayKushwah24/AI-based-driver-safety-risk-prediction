...
import '../styles/component475.css';

function MetricCard475({ label, value, max }) {
...
function TreeNode475({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode475 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState475 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer475(state, action) {
...
export default function Component475({ seed = 475, title = 'Component475' }) {
  const hook = useLogic475(seed + 475);
  const [state, dispatch] = useReducer(reducer475, initialState475);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm475(seed));
  ...
  const table = useMemo(() => buildTableState475(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm475(form);
    setErrors(found);
  ...
  return (
    <section className="component-475">
      <div className="header-475">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-475">
      ...
      <div className="grid-475">
        <div className="panel-475">
          {cards.map((item, idx) => (
            <MetricCard475 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
