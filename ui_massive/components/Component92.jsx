...
import '../styles/component92.css';

function MetricCard92({ label, value, max }) {
...
function TreeNode92({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode92 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component92({ seed = 92, title = 'Component92' }) {
  const hook = useLogic92(seed + 92);
  const [state, dispatch] = useReducer(reducer92, initialState92);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm92(seed));
  ...
  const table = useMemo(() => buildTableState92(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm92(form);
    setErrors(found);
  ...
  return (
    <section className="component-92">
      ...
      <div className="grid-92">
        <div className="panel-92">
          {cards.map((item, idx) => (
            <MetricCard92 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
