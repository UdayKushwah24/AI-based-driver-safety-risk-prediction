...
import { useLogic100 } from '../hooks/useLogic100.js';
import { buildNestedTree100, buildTableState100, heavyCompute100, transformDataset100 } from '../utils/compute100.js';
import { buildForecast100, detectAnomalies100, segmentDistribution100 } from '../utils/compute100.js';
import { buildActivityMap100, rankBuckets100, summarizePipeline100, synthesizeFlow100 } from '../pipelines/pipeline100.js';
import '../styles/component650.css';

function MetricCard650({ label, value, max }) {
...
function TreeNode650({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode650 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState650 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer650(state, action) {
...
export default function Component650({ seed = 650, title = 'Component650' }) {
  const hook = useLogic100(seed + 650);
  const [state, dispatch] = useReducer(reducer650, initialState650);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm650(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset100(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState100(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree100(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute100(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast100(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies100(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution100(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow100(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets100(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap100(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline100(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm650(form);
    setErrors(found);
  ...
  return (
    <section className="component-650">
      <div className="header-650">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-650">
      ...
      <div className="grid-650">
        <div className="panel-650">
          {cards.map((item, idx) => (
            <MetricCard650 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
