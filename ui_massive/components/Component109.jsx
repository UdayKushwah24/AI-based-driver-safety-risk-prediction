...
import '../styles/component109.css';

function MetricCard109({ label, value, max }) {
...
function TreeNode109({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode109 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component109({ seed = 109, title = 'Component109' }) {
  const hook = useLogic109(seed + 109);
  const [state, dispatch] = useReducer(reducer109, initialState109);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm109(seed));
  ...
  const table = useMemo(() => buildTableState109(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm109(form);
    setErrors(found);
  ...
  return (
    <section className="component-109">
      ...
      <div className="grid-109">
        <div className="panel-109">
          {cards.map((item, idx) => (
            <MetricCard109 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
