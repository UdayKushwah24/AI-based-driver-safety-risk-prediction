...
import '../styles/component288.css';

function MetricCard288({ label, value, max }) {
...
function TreeNode288({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode288 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState288 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer288(state, action) {
...
export default function Component288({ seed = 288, title = 'Component288' }) {
  const hook = useLogic288(seed + 288);
  const [state, dispatch] = useReducer(reducer288, initialState288);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm288(seed));
  ...
  const table = useMemo(() => buildTableState288(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm288(form);
    setErrors(found);
  ...
  return (
    <section className="component-288">
      <div className="header-288">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-288">
      ...
      <div className="grid-288">
        <div className="panel-288">
          {cards.map((item, idx) => (
            <MetricCard288 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
