...
import '../styles/component94.css';

function MetricCard94({ label, value, max }) {
...
function TreeNode94({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode94 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component94({ seed = 94, title = 'Component94' }) {
  const hook = useLogic94(seed + 94);
  const [state, dispatch] = useReducer(reducer94, initialState94);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm94(seed));
  ...
  const table = useMemo(() => buildTableState94(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm94(form);
    setErrors(found);
  ...
  return (
    <section className="component-94">
      ...
      <div className="grid-94">
        <div className="panel-94">
          {cards.map((item, idx) => (
            <MetricCard94 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
