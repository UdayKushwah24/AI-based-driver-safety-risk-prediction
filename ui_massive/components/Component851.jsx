...
import '../styles/component851.css';

function MetricCard851({ label, value, max }) {
...
function TreeNode851({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode851 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState851 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer851(state, action) {
...
export default function Component851({ seed = 851, title = 'Component851' }) {
  const hook = useLogic301(seed + 851);
  const [state, dispatch] = useReducer(reducer851, initialState851);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm851(seed));
  ...
  const table = useMemo(() => buildTableState301(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm851(form);
    setErrors(found);
  ...
  return (
    <section className="component-851">
      <div className="header-851">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-851">
      ...
      <div className="grid-851">
        <div className="panel-851">
          {cards.map((item, idx) => (
            <MetricCard851 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
