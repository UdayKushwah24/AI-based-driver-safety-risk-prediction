...
import '../styles/component740.css';

function MetricCard740({ label, value, max }) {
...
function TreeNode740({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode740 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState740 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer740(state, action) {
...
export default function Component740({ seed = 740, title = 'Component740' }) {
  const hook = useLogic190(seed + 740);
  const [state, dispatch] = useReducer(reducer740, initialState740);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm740(seed));
  ...
  const table = useMemo(() => buildTableState190(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm740(form);
    setErrors(found);
  ...
  return (
    <section className="component-740">
      <div className="header-740">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-740">
      ...
      <div className="grid-740">
        <div className="panel-740">
          {cards.map((item, idx) => (
            <MetricCard740 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
