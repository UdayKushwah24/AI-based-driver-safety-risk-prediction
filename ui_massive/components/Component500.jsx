...
import '../styles/component500.css';

function MetricCard500({ label, value, max }) {
...
function TreeNode500({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode500 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState500 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer500(state, action) {
...
export default function Component500({ seed = 500, title = 'Component500' }) {
  const hook = useLogic500(seed + 500);
  const [state, dispatch] = useReducer(reducer500, initialState500);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm500(seed));
  ...
  const table = useMemo(() => buildTableState500(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm500(form);
    setErrors(found);
  ...
  return (
    <section className="component-500">
      ...
      <div className="grid-500">
        <div className="panel-500">
          {cards.map((item, idx) => (
            <MetricCard500 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
