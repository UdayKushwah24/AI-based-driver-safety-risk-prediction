...
import '../styles/component116.css';

function MetricCard116({ label, value, max }) {
...
function TreeNode116({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode116 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState116 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer116(state, action) {
...
export default function Component116({ seed = 116, title = 'Component116' }) {
  const hook = useLogic116(seed + 116);
  const [state, dispatch] = useReducer(reducer116, initialState116);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm116(seed));
  ...
  const table = useMemo(() => buildTableState116(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm116(form);
    setErrors(found);
  ...
  return (
    <section className="component-116">
      <div className="header-116">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-116">
      ...
      <div className="grid-116">
        <div className="panel-116">
          {cards.map((item, idx) => (
            <MetricCard116 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
