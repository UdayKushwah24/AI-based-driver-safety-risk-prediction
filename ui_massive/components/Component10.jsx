...
function MetricCard10({ label, value, max }) {
...
function TreeNode10({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode10 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component10({ seed = 10, title = 'Component10' }) {
  const hook = useLogic10(seed + 10);
  const [state, dispatch] = useReducer(reducer10, initialState10);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm10(seed));
  ...
  const table = useMemo(() => buildTableState10(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm10(form);
    setErrors(found);
  ...
  return (
    <section className="component-10">
      ...
      <div className="grid-10">
        <div className="panel-10">
          {cards.map((item, idx) => (
            <MetricCard10 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
