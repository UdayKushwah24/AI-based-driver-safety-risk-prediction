...
import '../styles/component20.css';

function MetricCard20({ label, value, max }) {
...
function TreeNode20({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode20 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component20({ seed = 20, title = 'Component20' }) {
  const hook = useLogic20(seed + 20);
  const [state, dispatch] = useReducer(reducer20, initialState20);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm20(seed));
  ...
  const table = useMemo(() => buildTableState20(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm20(form);
    setErrors(found);
  ...
  return (
    <section className="component-20">
      ...
      <div className="grid-20">
        <div className="panel-20">
          {cards.map((item, idx) => (
            <MetricCard20 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
