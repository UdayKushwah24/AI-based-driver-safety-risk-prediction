...
import '../styles/component548.css';

function MetricCard548({ label, value, max }) {
...
function TreeNode548({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode548 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState548 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer548(state, action) {
...
export default function Component548({ seed = 548, title = 'Component548' }) {
  const hook = useLogic548(seed + 548);
  const [state, dispatch] = useReducer(reducer548, initialState548);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm548(seed));
  ...
  const table = useMemo(() => buildTableState548(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm548(form);
    setErrors(found);
  ...
  return (
    <section className="component-548">
      <div className="header-548">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-548">
      ...
      <div className="grid-548">
        <div className="panel-548">
          {cards.map((item, idx) => (
            <MetricCard548 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
