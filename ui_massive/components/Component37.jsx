...
import '../styles/component37.css';

function MetricCard37({ label, value, max }) {
...
function TreeNode37({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode37 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component37({ seed = 37, title = 'Component37' }) {
  const hook = useLogic37(seed + 37);
  const [state, dispatch] = useReducer(reducer37, initialState37);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm37(seed));
  ...
  const table = useMemo(() => buildTableState37(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm37(form);
    setErrors(found);
  ...
  return (
    <section className="component-37">
      ...
      <div className="grid-37">
        <div className="panel-37">
          {cards.map((item, idx) => (
            <MetricCard37 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
