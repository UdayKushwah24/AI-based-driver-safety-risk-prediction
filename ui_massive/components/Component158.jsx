...
import { useLogic158 } from '../hooks/useLogic158.js';
import { buildNestedTree158, buildTableState158, heavyCompute158, transformDataset158 } from '../utils/compute158.js';
import { buildForecast158, detectAnomalies158, segmentDistribution158 } from '../utils/compute158.js';
import { buildActivityMap158, rankBuckets158, summarizePipeline158, synthesizeFlow158 } from '../pipelines/pipeline158.js';
import '../styles/component158.css';

function MetricCard158({ label, value, max }) {
...
function TreeNode158({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode158 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState158 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer158(state, action) {
...
export default function Component158({ seed = 158, title = 'Component158' }) {
  const hook = useLogic158(seed + 158);
  const [state, dispatch] = useReducer(reducer158, initialState158);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm158(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset158(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState158(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree158(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute158(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast158(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies158(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution158(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow158(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets158(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap158(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline158(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm158(form);
    setErrors(found);
  ...
  return (
    <section className="component-158">
      <div className="header-158">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-158">
      ...
      <div className="grid-158">
        <div className="panel-158">
          {cards.map((item, idx) => (
            <MetricCard158 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
