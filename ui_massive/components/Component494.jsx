...
import '../styles/component494.css';

function MetricCard494({ label, value, max }) {
...
function TreeNode494({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode494 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState494 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer494(state, action) {
...
export default function Component494({ seed = 494, title = 'Component494' }) {
  const hook = useLogic494(seed + 494);
  const [state, dispatch] = useReducer(reducer494, initialState494);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm494(seed));
  ...
  const table = useMemo(() => buildTableState494(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm494(form);
    setErrors(found);
  ...
  return (
    <section className="component-494">
      <div className="header-494">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-494">
      ...
      <div className="grid-494">
        <div className="panel-494">
          {cards.map((item, idx) => (
            <MetricCard494 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
