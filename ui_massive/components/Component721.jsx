...
import '../styles/component721.css';

function MetricCard721({ label, value, max }) {
...
function TreeNode721({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode721 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState721 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer721(state, action) {
...
export default function Component721({ seed = 721, title = 'Component721' }) {
  const hook = useLogic171(seed + 721);
  const [state, dispatch] = useReducer(reducer721, initialState721);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm721(seed));
  ...
  const table = useMemo(() => buildTableState171(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm721(form);
    setErrors(found);
  ...
  return (
    <section className="component-721">
      ...
      <div className="grid-721">
        <div className="panel-721">
          {cards.map((item, idx) => (
            <MetricCard721 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
