...
import { useLogic430 } from '../hooks/useLogic430.js';
import { buildNestedTree430, buildTableState430, heavyCompute430, transformDataset430 } from '../utils/compute430.js';
import { buildForecast430, detectAnomalies430, segmentDistribution430 } from '../utils/compute430.js';
import { buildActivityMap430, rankBuckets430, summarizePipeline430, synthesizeFlow430 } from '../pipelines/pipeline430.js';
import '../styles/component430.css';

function MetricCard430({ label, value, max }) {
...
function TreeNode430({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode430 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState430 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer430(state, action) {
...
export default function Component430({ seed = 430, title = 'Component430' }) {
  const hook = useLogic430(seed + 430);
  const [state, dispatch] = useReducer(reducer430, initialState430);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm430(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset430(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState430(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree430(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute430(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast430(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies430(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution430(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow430(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets430(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap430(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline430(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm430(form);
    setErrors(found);
  ...
  return (
    <section className="component-430">
      <div className="header-430">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-430">
      ...
      <div className="grid-430">
        <div className="panel-430">
          {cards.map((item, idx) => (
            <MetricCard430 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
