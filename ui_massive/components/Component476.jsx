...
import '../styles/component476.css';

function MetricCard476({ label, value, max }) {
...
function TreeNode476({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode476 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState476 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer476(state, action) {
...
export default function Component476({ seed = 476, title = 'Component476' }) {
  const hook = useLogic476(seed + 476);
  const [state, dispatch] = useReducer(reducer476, initialState476);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm476(seed));
  ...
  const table = useMemo(() => buildTableState476(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm476(form);
    setErrors(found);
  ...
  return (
    <section className="component-476">
      <div className="header-476">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-476">
      ...
      <div className="grid-476">
        <div className="panel-476">
          {cards.map((item, idx) => (
            <MetricCard476 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
