...
import { useLogic406 } from '../hooks/useLogic406.js';
import { buildNestedTree406, buildTableState406, heavyCompute406, transformDataset406 } from '../utils/compute406.js';
import { buildForecast406, detectAnomalies406, segmentDistribution406 } from '../utils/compute406.js';
import { buildActivityMap406, rankBuckets406, summarizePipeline406, synthesizeFlow406 } from '../pipelines/pipeline406.js';
import '../styles/component956.css';

function MetricCard956({ label, value, max }) {
...
function TreeNode956({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode956 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState956 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer956(state, action) {
...
export default function Component956({ seed = 956, title = 'Component956' }) {
  const hook = useLogic406(seed + 956);
  const [state, dispatch] = useReducer(reducer956, initialState956);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm956(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset406(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState406(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree406(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute406(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast406(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies406(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution406(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow406(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets406(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap406(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline406(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm956(form);
    setErrors(found);
  ...
  return (
    <section className="component-956">
      <div className="header-956">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-956">
      ...
      <div className="grid-956">
        <div className="panel-956">
          {cards.map((item, idx) => (
            <MetricCard956 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
