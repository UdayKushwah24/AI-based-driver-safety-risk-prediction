...
import '../styles/component364.css';

function MetricCard364({ label, value, max }) {
...
function TreeNode364({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode364 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState364 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer364(state, action) {
...
export default function Component364({ seed = 364, title = 'Component364' }) {
  const hook = useLogic364(seed + 364);
  const [state, dispatch] = useReducer(reducer364, initialState364);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm364(seed));
  ...
  const table = useMemo(() => buildTableState364(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm364(form);
    setErrors(found);
  ...
  return (
    <section className="component-364">
      <div className="header-364">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-364">
      ...
      <div className="grid-364">
        <div className="panel-364">
          {cards.map((item, idx) => (
            <MetricCard364 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
