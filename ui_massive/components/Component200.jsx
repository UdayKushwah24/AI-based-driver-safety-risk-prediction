...
function MetricCard200({ label, value, max }) {
...
function TreeNode200({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode200 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component200({ seed = 200, title = 'Component200' }) {
  const hook = useLogic200(seed + 200);
  const [state, dispatch] = useReducer(reducer200, initialState200);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm200(seed));
  ...
  const table = useMemo(() => buildTableState200(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm200(form);
    setErrors(found);
  ...
  return (
    <section className="component-200">
      <div className="header-200">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-200">
      ...
      <div className="grid-200">
        <div className="panel-200">
          {cards.map((item, idx) => (
            <MetricCard200 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
