...
import { useLogic146 } from '../hooks/useLogic146.js';
import { buildNestedTree146, buildTableState146, heavyCompute146, transformDataset146 } from '../utils/compute146.js';
import { buildForecast146, detectAnomalies146, segmentDistribution146 } from '../utils/compute146.js';
import { buildActivityMap146, rankBuckets146, summarizePipeline146, synthesizeFlow146 } from '../pipelines/pipeline146.js';
import '../styles/component146.css';

function MetricCard146({ label, value, max }) {
...
function TreeNode146({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode146 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState146 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer146(state, action) {
...
export default function Component146({ seed = 146, title = 'Component146' }) {
  const hook = useLogic146(seed + 146);
  const [state, dispatch] = useReducer(reducer146, initialState146);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm146(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset146(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState146(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree146(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute146(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast146(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies146(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution146(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow146(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets146(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap146(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline146(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm146(form);
    setErrors(found);
  ...
  return (
    <section className="component-146">
      <div className="header-146">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-146">
      ...
      <div className="grid-146">
        <div className="panel-146">
          {cards.map((item, idx) => (
            <MetricCard146 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
