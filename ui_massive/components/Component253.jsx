...
import '../styles/component253.css';

function MetricCard253({ label, value, max }) {
...
function TreeNode253({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode253 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState253 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer253(state, action) {
...
export default function Component253({ seed = 253, title = 'Component253' }) {
  const hook = useLogic253(seed + 253);
  const [state, dispatch] = useReducer(reducer253, initialState253);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm253(seed));
  ...
  const table = useMemo(() => buildTableState253(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm253(form);
    setErrors(found);
  ...
  return (
    <section className="component-253">
      <div className="header-253">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-253">
      ...
      <div className="grid-253">
        <div className="panel-253">
          {cards.map((item, idx) => (
            <MetricCard253 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
