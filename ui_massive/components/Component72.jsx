...
import '../styles/component72.css';

function MetricCard72({ label, value, max }) {
...
function TreeNode72({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode72 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component72({ seed = 72, title = 'Component72' }) {
  const hook = useLogic72(seed + 72);
  const [state, dispatch] = useReducer(reducer72, initialState72);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm72(seed));
  ...
  const table = useMemo(() => buildTableState72(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm72(form);
    setErrors(found);
  ...
  return (
    <section className="component-72">
      ...
      <div className="grid-72">
        <div className="panel-72">
          {cards.map((item, idx) => (
            <MetricCard72 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
