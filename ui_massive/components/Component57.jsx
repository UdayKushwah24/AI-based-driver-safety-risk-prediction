...
import '../styles/component57.css';

function MetricCard57({ label, value, max }) {
...
function TreeNode57({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode57 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component57({ seed = 57, title = 'Component57' }) {
  const hook = useLogic57(seed + 57);
  const [state, dispatch] = useReducer(reducer57, initialState57);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm57(seed));
  ...
  const table = useMemo(() => buildTableState57(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm57(form);
    setErrors(found);
  ...
  return (
    <section className="component-57">
      ...
      <div className="grid-57">
        <div className="panel-57">
          {cards.map((item, idx) => (
            <MetricCard57 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
