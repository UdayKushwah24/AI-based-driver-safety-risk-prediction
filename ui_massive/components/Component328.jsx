...
import '../styles/component328.css';

function MetricCard328({ label, value, max }) {
...
function TreeNode328({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode328 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState328 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer328(state, action) {
...
export default function Component328({ seed = 328, title = 'Component328' }) {
  const hook = useLogic328(seed + 328);
  const [state, dispatch] = useReducer(reducer328, initialState328);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm328(seed));
  ...
  const table = useMemo(() => buildTableState328(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm328(form);
    setErrors(found);
  ...
  return (
    <section className="component-328">
      <div className="header-328">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-328">
      ...
      <div className="grid-328">
        <div className="panel-328">
          {cards.map((item, idx) => (
            <MetricCard328 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
