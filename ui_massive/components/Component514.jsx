...
import '../styles/component514.css';

function MetricCard514({ label, value, max }) {
...
function TreeNode514({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode514 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState514 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer514(state, action) {
...
export default function Component514({ seed = 514, title = 'Component514' }) {
  const hook = useLogic514(seed + 514);
  const [state, dispatch] = useReducer(reducer514, initialState514);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm514(seed));
  ...
  const table = useMemo(() => buildTableState514(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm514(form);
    setErrors(found);
  ...
  return (
    <section className="component-514">
      <div className="header-514">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-514">
      ...
      <div className="grid-514">
        <div className="panel-514">
          {cards.map((item, idx) => (
            <MetricCard514 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
