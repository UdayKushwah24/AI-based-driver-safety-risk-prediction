...
import '../styles/component90.css';

function MetricCard90({ label, value, max }) {
...
function TreeNode90({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode90 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component90({ seed = 90, title = 'Component90' }) {
  const hook = useLogic90(seed + 90);
  const [state, dispatch] = useReducer(reducer90, initialState90);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm90(seed));
  ...
  const table = useMemo(() => buildTableState90(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm90(form);
    setErrors(found);
  ...
  return (
    <section className="component-90">
      ...
      <div className="grid-90">
        <div className="panel-90">
          {cards.map((item, idx) => (
            <MetricCard90 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
