...
import '../styles/component112.css';

function MetricCard112({ label, value, max }) {
...
function TreeNode112({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode112 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component112({ seed = 112, title = 'Component112' }) {
  const hook = useLogic112(seed + 112);
  const [state, dispatch] = useReducer(reducer112, initialState112);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm112(seed));
  ...
  const table = useMemo(() => buildTableState112(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm112(form);
    setErrors(found);
  ...
  return (
    <section className="component-112">
      ...
      <div className="grid-112">
        <div className="panel-112">
          {cards.map((item, idx) => (
            <MetricCard112 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
