...
import '../styles/component796.css';

function MetricCard796({ label, value, max }) {
...
function TreeNode796({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode796 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState796 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer796(state, action) {
...
export default function Component796({ seed = 796, title = 'Component796' }) {
  const hook = useLogic246(seed + 796);
  const [state, dispatch] = useReducer(reducer796, initialState796);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm796(seed));
  ...
  const table = useMemo(() => buildTableState246(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm796(form);
    setErrors(found);
  ...
  return (
    <section className="component-796">
      <div className="header-796">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-796">
      ...
      <div className="grid-796">
        <div className="panel-796">
          {cards.map((item, idx) => (
            <MetricCard796 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
