...
import '../styles/component454.css';

function MetricCard454({ label, value, max }) {
...
function TreeNode454({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode454 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState454 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer454(state, action) {
...
export default function Component454({ seed = 454, title = 'Component454' }) {
  const hook = useLogic454(seed + 454);
  const [state, dispatch] = useReducer(reducer454, initialState454);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm454(seed));
  ...
  const table = useMemo(() => buildTableState454(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm454(form);
    setErrors(found);
  ...
  return (
    <section className="component-454">
      <div className="header-454">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-454">
      ...
      <div className="grid-454">
        <div className="panel-454">
          {cards.map((item, idx) => (
            <MetricCard454 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
