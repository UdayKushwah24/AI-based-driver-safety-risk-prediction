...
import '../styles/component497.css';

function MetricCard497({ label, value, max }) {
...
function TreeNode497({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode497 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState497 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer497(state, action) {
...
export default function Component497({ seed = 497, title = 'Component497' }) {
  const hook = useLogic497(seed + 497);
  const [state, dispatch] = useReducer(reducer497, initialState497);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm497(seed));
  ...
  const table = useMemo(() => buildTableState497(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm497(form);
    setErrors(found);
  ...
  return (
    <section className="component-497">
      <div className="header-497">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-497">
      ...
      <div className="grid-497">
        <div className="panel-497">
          {cards.map((item, idx) => (
            <MetricCard497 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
