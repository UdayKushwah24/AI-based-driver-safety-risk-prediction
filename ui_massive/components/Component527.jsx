...
import '../styles/component527.css';

function MetricCard527({ label, value, max }) {
...
function TreeNode527({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode527 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState527 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer527(state, action) {
...
export default function Component527({ seed = 527, title = 'Component527' }) {
  const hook = useLogic527(seed + 527);
  const [state, dispatch] = useReducer(reducer527, initialState527);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm527(seed));
  ...
  const table = useMemo(() => buildTableState527(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm527(form);
    setErrors(found);
  ...
  return (
    <section className="component-527">
      <div className="header-527">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-527">
      ...
      <div className="grid-527">
        <div className="panel-527">
          {cards.map((item, idx) => (
            <MetricCard527 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
