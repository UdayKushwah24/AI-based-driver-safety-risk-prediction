...
import '../styles/component498.css';

function MetricCard498({ label, value, max }) {
...
function TreeNode498({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode498 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState498 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer498(state, action) {
...
export default function Component498({ seed = 498, title = 'Component498' }) {
  const hook = useLogic498(seed + 498);
  const [state, dispatch] = useReducer(reducer498, initialState498);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm498(seed));
  ...
  const table = useMemo(() => buildTableState498(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm498(form);
    setErrors(found);
  ...
  return (
    <section className="component-498">
      <div className="header-498">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-498">
      ...
      <div className="grid-498">
        <div className="panel-498">
          {cards.map((item, idx) => (
            <MetricCard498 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
