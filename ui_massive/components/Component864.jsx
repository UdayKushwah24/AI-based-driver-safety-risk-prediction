...
import '../styles/component864.css';

function MetricCard864({ label, value, max }) {
...
function TreeNode864({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode864 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState864 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer864(state, action) {
...
export default function Component864({ seed = 864, title = 'Component864' }) {
  const hook = useLogic314(seed + 864);
  const [state, dispatch] = useReducer(reducer864, initialState864);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm864(seed));
  ...
  const table = useMemo(() => buildTableState314(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm864(form);
    setErrors(found);
  ...
  return (
    <section className="component-864">
      <div className="header-864">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-864">
      ...
      <div className="grid-864">
        <div className="panel-864">
          {cards.map((item, idx) => (
            <MetricCard864 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
