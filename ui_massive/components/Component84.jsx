...
import '../styles/component84.css';

function MetricCard84({ label, value, max }) {
...
function TreeNode84({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode84 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component84({ seed = 84, title = 'Component84' }) {
  const hook = useLogic84(seed + 84);
  const [state, dispatch] = useReducer(reducer84, initialState84);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm84(seed));
  ...
  const table = useMemo(() => buildTableState84(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm84(form);
    setErrors(found);
  ...
  return (
    <section className="component-84">
      ...
      <div className="grid-84">
        <div className="panel-84">
          {cards.map((item, idx) => (
            <MetricCard84 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
