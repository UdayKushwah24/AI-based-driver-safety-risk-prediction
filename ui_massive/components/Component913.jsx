...
import '../styles/component913.css';

function MetricCard913({ label, value, max }) {
...
function TreeNode913({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode913 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState913 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer913(state, action) {
...
export default function Component913({ seed = 913, title = 'Component913' }) {
  const hook = useLogic363(seed + 913);
  const [state, dispatch] = useReducer(reducer913, initialState913);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm913(seed));
  ...
  const table = useMemo(() => buildTableState363(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm913(form);
    setErrors(found);
  ...
  return (
    <section className="component-913">
      <div className="header-913">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-913">
      ...
      <div className="grid-913">
        <div className="panel-913">
          {cards.map((item, idx) => (
            <MetricCard913 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
