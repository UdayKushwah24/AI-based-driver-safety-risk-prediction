...
function MetricCard12({ label, value, max }) {
...
function TreeNode12({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode12 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component12({ seed = 12, title = 'Component12' }) {
  const hook = useLogic12(seed + 12);
  const [state, dispatch] = useReducer(reducer12, initialState12);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm12(seed));
  ...
  const table = useMemo(() => buildTableState12(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm12(form);
    setErrors(found);
  ...
  return (
    <section className="component-12">
      ...
      <div className="grid-12">
        <div className="panel-12">
          {cards.map((item, idx) => (
            <MetricCard12 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
