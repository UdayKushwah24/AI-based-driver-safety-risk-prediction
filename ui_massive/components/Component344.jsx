...
import '../styles/component344.css';

function MetricCard344({ label, value, max }) {
...
function TreeNode344({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode344 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState344 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer344(state, action) {
...
export default function Component344({ seed = 344, title = 'Component344' }) {
  const hook = useLogic344(seed + 344);
  const [state, dispatch] = useReducer(reducer344, initialState344);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm344(seed));
  ...
  const table = useMemo(() => buildTableState344(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm344(form);
    setErrors(found);
  ...
  return (
    <section className="component-344">
      <div className="header-344">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-344">
      ...
      <div className="grid-344">
        <div className="panel-344">
          {cards.map((item, idx) => (
            <MetricCard344 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
