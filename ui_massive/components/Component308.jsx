...
import '../styles/component308.css';

function MetricCard308({ label, value, max }) {
...
function TreeNode308({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode308 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState308 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer308(state, action) {
...
export default function Component308({ seed = 308, title = 'Component308' }) {
  const hook = useLogic308(seed + 308);
  const [state, dispatch] = useReducer(reducer308, initialState308);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm308(seed));
  ...
  const table = useMemo(() => buildTableState308(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm308(form);
    setErrors(found);
  ...
  return (
    <section className="component-308">
      <div className="header-308">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-308">
      ...
      <div className="grid-308">
        <div className="panel-308">
          {cards.map((item, idx) => (
            <MetricCard308 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
