...
import '../styles/component667.css';

function MetricCard667({ label, value, max }) {
...
function TreeNode667({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode667 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState667 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer667(state, action) {
...
export default function Component667({ seed = 667, title = 'Component667' }) {
  const hook = useLogic117(seed + 667);
  const [state, dispatch] = useReducer(reducer667, initialState667);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm667(seed));
  ...
  const table = useMemo(() => buildTableState117(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm667(form);
    setErrors(found);
  ...
  return (
    <section className="component-667">
      <div className="header-667">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-667">
      ...
      <div className="grid-667">
        <div className="panel-667">
          {cards.map((item, idx) => (
            <MetricCard667 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
