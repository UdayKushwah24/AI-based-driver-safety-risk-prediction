...
import '../styles/component432.css';

function MetricCard432({ label, value, max }) {
...
function TreeNode432({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode432 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState432 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer432(state, action) {
...
export default function Component432({ seed = 432, title = 'Component432' }) {
  const hook = useLogic432(seed + 432);
  const [state, dispatch] = useReducer(reducer432, initialState432);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm432(seed));
  ...
  const table = useMemo(() => buildTableState432(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm432(form);
    setErrors(found);
  ...
  return (
    <section className="component-432">
      <div className="header-432">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-432">
      ...
      <div className="grid-432">
        <div className="panel-432">
          {cards.map((item, idx) => (
            <MetricCard432 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
