...
import { useLogic375 } from '../hooks/useLogic375.js';
import { buildNestedTree375, buildTableState375, heavyCompute375, transformDataset375 } from '../utils/compute375.js';
import { buildForecast375, detectAnomalies375, segmentDistribution375 } from '../utils/compute375.js';
import { buildActivityMap375, rankBuckets375, summarizePipeline375, synthesizeFlow375 } from '../pipelines/pipeline375.js';
import '../styles/component375.css';

function MetricCard375({ label, value, max }) {
...
function TreeNode375({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode375 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState375 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer375(state, action) {
...
export default function Component375({ seed = 375, title = 'Component375' }) {
  const hook = useLogic375(seed + 375);
  const [state, dispatch] = useReducer(reducer375, initialState375);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm375(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset375(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState375(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree375(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute375(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast375(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies375(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution375(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow375(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets375(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap375(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline375(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm375(form);
    setErrors(found);
  ...
  return (
    <section className="component-375">
      <div className="header-375">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-375">
      ...
      <div className="grid-375">
        <div className="panel-375">
          {cards.map((item, idx) => (
            <MetricCard375 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
