...
import { useLogic338 } from '../hooks/useLogic338.js';
import { buildNestedTree338, buildTableState338, heavyCompute338, transformDataset338 } from '../utils/compute338.js';
import { buildForecast338, detectAnomalies338, segmentDistribution338 } from '../utils/compute338.js';
import { buildActivityMap338, rankBuckets338, summarizePipeline338, synthesizeFlow338 } from '../pipelines/pipeline338.js';
import '../styles/component338.css';

function MetricCard338({ label, value, max }) {
...
function TreeNode338({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode338 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState338 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer338(state, action) {
...
export default function Component338({ seed = 338, title = 'Component338' }) {
  const hook = useLogic338(seed + 338);
  const [state, dispatch] = useReducer(reducer338, initialState338);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm338(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset338(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState338(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree338(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute338(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast338(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies338(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution338(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow338(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets338(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap338(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline338(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm338(form);
    setErrors(found);
  ...
  return (
    <section className="component-338">
      <div className="header-338">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-338">
      ...
      <div className="grid-338">
        <div className="panel-338">
          {cards.map((item, idx) => (
            <MetricCard338 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
