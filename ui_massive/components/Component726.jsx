...
import { useLogic176 } from '../hooks/useLogic176.js';
import { buildNestedTree176, buildTableState176, heavyCompute176, transformDataset176 } from '../utils/compute176.js';
import { buildForecast176, detectAnomalies176, segmentDistribution176 } from '../utils/compute176.js';
import { buildActivityMap176, rankBuckets176, summarizePipeline176, synthesizeFlow176 } from '../pipelines/pipeline176.js';
import '../styles/component726.css';

function MetricCard726({ label, value, max }) {
...
function TreeNode726({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode726 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState726 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer726(state, action) {
...
export default function Component726({ seed = 726, title = 'Component726' }) {
  const hook = useLogic176(seed + 726);
  const [state, dispatch] = useReducer(reducer726, initialState726);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm726(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset176(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState176(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree176(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute176(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast176(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies176(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution176(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow176(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets176(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap176(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline176(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm726(form);
    setErrors(found);
  ...
  return (
    <section className="component-726">
      <div className="header-726">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-726">
      ...
      <div className="grid-726">
        <div className="panel-726">
          {cards.map((item, idx) => (
            <MetricCard726 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
