...
import { useLogic404 } from '../hooks/useLogic404.js';
import { buildNestedTree404, buildTableState404, heavyCompute404, transformDataset404 } from '../utils/compute404.js';
import { buildForecast404, detectAnomalies404, segmentDistribution404 } from '../utils/compute404.js';
import { buildActivityMap404, rankBuckets404, summarizePipeline404, synthesizeFlow404 } from '../pipelines/pipeline404.js';
import '../styles/component954.css';

function MetricCard954({ label, value, max }) {
...
function TreeNode954({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode954 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState954 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer954(state, action) {
...
export default function Component954({ seed = 954, title = 'Component954' }) {
  const hook = useLogic404(seed + 954);
  const [state, dispatch] = useReducer(reducer954, initialState954);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm954(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset404(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState404(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree404(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute404(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast404(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies404(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution404(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow404(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets404(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap404(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline404(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm954(form);
    setErrors(found);
  ...
  return (
    <section className="component-954">
      <div className="header-954">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-954">
      ...
      <div className="grid-954">
        <div className="panel-954">
          {cards.map((item, idx) => (
            <MetricCard954 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
