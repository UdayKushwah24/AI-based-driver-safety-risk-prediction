...
import { useLogic301 } from '../hooks/useLogic301.js';
import { buildNestedTree301, buildTableState301, heavyCompute301, transformDataset301 } from '../utils/compute301.js';
import { buildForecast301, detectAnomalies301, segmentDistribution301 } from '../utils/compute301.js';
import { buildActivityMap301, rankBuckets301, summarizePipeline301, synthesizeFlow301 } from '../pipelines/pipeline301.js';
import '../styles/component301.css';

function MetricCard301({ label, value, max }) {
...
function TreeNode301({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode301 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState301 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer301(state, action) {
...
export default function Component301({ seed = 301, title = 'Component301' }) {
  const hook = useLogic301(seed + 301);
  const [state, dispatch] = useReducer(reducer301, initialState301);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm301(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset301(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState301(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree301(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute301(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast301(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies301(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution301(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow301(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets301(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap301(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline301(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm301(form);
    setErrors(found);
  ...
  return (
    <section className="component-301">
      <div className="header-301">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-301">
      ...
      <div className="grid-301">
        <div className="panel-301">
          {cards.map((item, idx) => (
            <MetricCard301 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
