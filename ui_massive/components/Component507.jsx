...
import '../styles/component507.css';

function MetricCard507({ label, value, max }) {
...
function TreeNode507({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode507 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState507 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer507(state, action) {
...
export default function Component507({ seed = 507, title = 'Component507' }) {
  const hook = useLogic507(seed + 507);
  const [state, dispatch] = useReducer(reducer507, initialState507);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm507(seed));
  ...
  const table = useMemo(() => buildTableState507(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm507(form);
    setErrors(found);
  ...
  return (
    <section className="component-507">
      <div className="header-507">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-507">
      ...
      <div className="grid-507">
        <div className="panel-507">
          {cards.map((item, idx) => (
            <MetricCard507 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
