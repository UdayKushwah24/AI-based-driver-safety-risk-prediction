...
import { useLogic101 } from '../hooks/useLogic101.js';
import { buildNestedTree101, buildTableState101, heavyCompute101, transformDataset101 } from '../utils/compute101.js';
import { buildForecast101, detectAnomalies101, segmentDistribution101 } from '../utils/compute101.js';
import { buildActivityMap101, rankBuckets101, summarizePipeline101, synthesizeFlow101 } from '../pipelines/pipeline101.js';
import '../styles/component651.css';

function MetricCard651({ label, value, max }) {
...
function TreeNode651({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode651 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState651 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer651(state, action) {
...
export default function Component651({ seed = 651, title = 'Component651' }) {
  const hook = useLogic101(seed + 651);
  const [state, dispatch] = useReducer(reducer651, initialState651);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm651(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset101(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState101(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree101(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute101(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast101(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies101(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution101(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow101(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets101(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap101(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline101(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm651(form);
    setErrors(found);
  ...
  return (
    <section className="component-651">
      <div className="header-651">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-651">
      ...
      <div className="grid-651">
        <div className="panel-651">
          {cards.map((item, idx) => (
            <MetricCard651 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
