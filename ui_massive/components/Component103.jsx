...
import '../styles/component103.css';

function MetricCard103({ label, value, max }) {
...
function TreeNode103({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode103 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component103({ seed = 103, title = 'Component103' }) {
  const hook = useLogic103(seed + 103);
  const [state, dispatch] = useReducer(reducer103, initialState103);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm103(seed));
  ...
  const table = useMemo(() => buildTableState103(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm103(form);
    setErrors(found);
  ...
  return (
    <section className="component-103">
      ...
      <div className="grid-103">
        <div className="panel-103">
          {cards.map((item, idx) => (
            <MetricCard103 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
