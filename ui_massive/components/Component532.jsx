...
import '../styles/component532.css';

function MetricCard532({ label, value, max }) {
...
function TreeNode532({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode532 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState532 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer532(state, action) {
...
export default function Component532({ seed = 532, title = 'Component532' }) {
  const hook = useLogic532(seed + 532);
  const [state, dispatch] = useReducer(reducer532, initialState532);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm532(seed));
  ...
  const table = useMemo(() => buildTableState532(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm532(form);
    setErrors(found);
  ...
  return (
    <section className="component-532">
      <div className="header-532">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-532">
      ...
      <div className="grid-532">
        <div className="panel-532">
          {cards.map((item, idx) => (
            <MetricCard532 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
