...
import '../styles/component329.css';

function MetricCard329({ label, value, max }) {
...
function TreeNode329({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode329 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState329 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer329(state, action) {
...
export default function Component329({ seed = 329, title = 'Component329' }) {
  const hook = useLogic329(seed + 329);
  const [state, dispatch] = useReducer(reducer329, initialState329);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm329(seed));
  ...
  const table = useMemo(() => buildTableState329(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm329(form);
    setErrors(found);
  ...
  return (
    <section className="component-329">
      <div className="header-329">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-329">
      ...
      <div className="grid-329">
        <div className="panel-329">
          {cards.map((item, idx) => (
            <MetricCard329 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
