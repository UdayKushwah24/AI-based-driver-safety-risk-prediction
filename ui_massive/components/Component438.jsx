...
import { useLogic438 } from '../hooks/useLogic438.js';
import { buildNestedTree438, buildTableState438, heavyCompute438, transformDataset438 } from '../utils/compute438.js';
import { buildForecast438, detectAnomalies438, segmentDistribution438 } from '../utils/compute438.js';
import { buildActivityMap438, rankBuckets438, summarizePipeline438, synthesizeFlow438 } from '../pipelines/pipeline438.js';
import '../styles/component438.css';

function MetricCard438({ label, value, max }) {
...
function TreeNode438({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode438 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState438 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer438(state, action) {
...
export default function Component438({ seed = 438, title = 'Component438' }) {
  const hook = useLogic438(seed + 438);
  const [state, dispatch] = useReducer(reducer438, initialState438);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm438(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset438(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState438(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree438(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute438(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast438(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies438(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution438(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow438(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets438(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap438(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline438(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm438(form);
    setErrors(found);
  ...
  return (
    <section className="component-438">
      <div className="header-438">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-438">
      ...
      <div className="grid-438">
        <div className="panel-438">
          {cards.map((item, idx) => (
            <MetricCard438 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
