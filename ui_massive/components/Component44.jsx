...
import '../styles/component44.css';

function MetricCard44({ label, value, max }) {
...
function TreeNode44({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode44 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component44({ seed = 44, title = 'Component44' }) {
  const hook = useLogic44(seed + 44);
  const [state, dispatch] = useReducer(reducer44, initialState44);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm44(seed));
  ...
  const table = useMemo(() => buildTableState44(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm44(form);
    setErrors(found);
  ...
  return (
    <section className="component-44">
      ...
      <div className="grid-44">
        <div className="panel-44">
          {cards.map((item, idx) => (
            <MetricCard44 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
