...
import '../styles/component991.css';

function MetricCard991({ label, value, max }) {
...
function TreeNode991({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode991 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState991 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer991(state, action) {
...
export default function Component991({ seed = 991, title = 'Component991' }) {
  const hook = useLogic441(seed + 991);
  const [state, dispatch] = useReducer(reducer991, initialState991);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm991(seed));
  ...
  const table = useMemo(() => buildTableState441(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm991(form);
    setErrors(found);
  ...
  return (
    <section className="component-991">
      <div className="header-991">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-991">
      ...
      <div className="grid-991">
        <div className="panel-991">
          {cards.map((item, idx) => (
            <MetricCard991 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
