...
import { useLogic150 } from '../hooks/useLogic150.js';
import { buildNestedTree150, buildTableState150, heavyCompute150, transformDataset150 } from '../utils/compute150.js';
import { buildForecast150, detectAnomalies150, segmentDistribution150 } from '../utils/compute150.js';
import { buildActivityMap150, rankBuckets150, summarizePipeline150, synthesizeFlow150 } from '../pipelines/pipeline150.js';
import '../styles/component700.css';

function MetricCard700({ label, value, max }) {
...
function TreeNode700({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode700 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState700 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer700(state, action) {
...
export default function Component700({ seed = 700, title = 'Component700' }) {
  const hook = useLogic150(seed + 700);
  const [state, dispatch] = useReducer(reducer700, initialState700);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm700(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset150(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState150(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree150(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute150(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast150(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies150(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution150(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow150(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets150(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap150(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline150(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm700(form);
    setErrors(found);
  ...
  return (
    <section className="component-700">
      <div className="header-700">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-700">
      ...
      <div className="grid-700">
        <div className="panel-700">
          {cards.map((item, idx) => (
            <MetricCard700 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
