...
import '../styles/component276.css';

function MetricCard276({ label, value, max }) {
...
function TreeNode276({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode276 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState276 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer276(state, action) {
...
export default function Component276({ seed = 276, title = 'Component276' }) {
  const hook = useLogic276(seed + 276);
  const [state, dispatch] = useReducer(reducer276, initialState276);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm276(seed));
  ...
  const table = useMemo(() => buildTableState276(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm276(form);
    setErrors(found);
  ...
  return (
    <section className="component-276">
      <div className="header-276">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-276">
      ...
      <div className="grid-276">
        <div className="panel-276">
          {cards.map((item, idx) => (
            <MetricCard276 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
