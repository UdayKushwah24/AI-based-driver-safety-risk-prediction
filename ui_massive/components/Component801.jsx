...
import { useLogic251 } from '../hooks/useLogic251.js';
import { buildNestedTree251, buildTableState251, heavyCompute251, transformDataset251 } from '../utils/compute251.js';
import { buildForecast251, detectAnomalies251, segmentDistribution251 } from '../utils/compute251.js';
import { buildActivityMap251, rankBuckets251, summarizePipeline251, synthesizeFlow251 } from '../pipelines/pipeline251.js';
import '../styles/component801.css';

function MetricCard801({ label, value, max }) {
...
function TreeNode801({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode801 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState801 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer801(state, action) {
...
export default function Component801({ seed = 801, title = 'Component801' }) {
  const hook = useLogic251(seed + 801);
  const [state, dispatch] = useReducer(reducer801, initialState801);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm801(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset251(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState251(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree251(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute251(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast251(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies251(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution251(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow251(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets251(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap251(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline251(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm801(form);
    setErrors(found);
  ...
  return (
    <section className="component-801">
      <div className="header-801">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-801">
      ...
      <div className="grid-801">
        <div className="panel-801">
          {cards.map((item, idx) => (
            <MetricCard801 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
