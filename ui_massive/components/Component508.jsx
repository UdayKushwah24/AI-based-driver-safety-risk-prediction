...
import '../styles/component508.css';

function MetricCard508({ label, value, max }) {
...
function TreeNode508({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode508 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState508 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer508(state, action) {
...
export default function Component508({ seed = 508, title = 'Component508' }) {
  const hook = useLogic508(seed + 508);
  const [state, dispatch] = useReducer(reducer508, initialState508);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm508(seed));
  ...
  const table = useMemo(() => buildTableState508(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm508(form);
    setErrors(found);
  ...
  return (
    <section className="component-508">
      <div className="header-508">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-508">
      ...
      <div className="grid-508">
        <div className="panel-508">
          {cards.map((item, idx) => (
            <MetricCard508 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
