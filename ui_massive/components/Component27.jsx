...
import '../styles/component27.css';

function MetricCard27({ label, value, max }) {
...
function TreeNode27({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode27 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component27({ seed = 27, title = 'Component27' }) {
  const hook = useLogic27(seed + 27);
  const [state, dispatch] = useReducer(reducer27, initialState27);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm27(seed));
  ...
  const table = useMemo(() => buildTableState27(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm27(form);
    setErrors(found);
  ...
  return (
    <section className="component-27">
      <div className="header-27">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-27">
      ...
      <div className="grid-27">
        <div className="panel-27">
          {cards.map((item, idx) => (
            <MetricCard27 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
