...
import { useLogic104 } from '../hooks/useLogic104.js';
import { buildNestedTree104, buildTableState104, heavyCompute104, transformDataset104 } from '../utils/compute104.js';
import { buildForecast104, detectAnomalies104, segmentDistribution104 } from '../utils/compute104.js';
import { buildActivityMap104, rankBuckets104, summarizePipeline104, synthesizeFlow104 } from '../pipelines/pipeline104.js';
import '../styles/component654.css';

function MetricCard654({ label, value, max }) {
...
function TreeNode654({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode654 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState654 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer654(state, action) {
...
export default function Component654({ seed = 654, title = 'Component654' }) {
  const hook = useLogic104(seed + 654);
  const [state, dispatch] = useReducer(reducer654, initialState654);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm654(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset104(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState104(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree104(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute104(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast104(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies104(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution104(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow104(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets104(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap104(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline104(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm654(form);
    setErrors(found);
  ...
  return (
    <section className="component-654">
      <div className="header-654">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-654">
      ...
      <div className="grid-654">
        <div className="panel-654">
          {cards.map((item, idx) => (
            <MetricCard654 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
