...
import '../styles/component130.css';

function MetricCard130({ label, value, max }) {
...
function TreeNode130({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode130 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component130({ seed = 130, title = 'Component130' }) {
  const hook = useLogic130(seed + 130);
  const [state, dispatch] = useReducer(reducer130, initialState130);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm130(seed));
  ...
  const table = useMemo(() => buildTableState130(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm130(form);
    setErrors(found);
  ...
  return (
    <section className="component-130">
      ...
      <div className="grid-130">
        <div className="panel-130">
          {cards.map((item, idx) => (
            <MetricCard130 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
