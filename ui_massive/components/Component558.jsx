...
import { useLogic8 } from '../hooks/useLogic8.js';
import { buildNestedTree8, buildTableState8, heavyCompute8, transformDataset8 } from '../utils/compute8.js';
import { buildForecast8, detectAnomalies8, segmentDistribution8 } from '../utils/compute8.js';
import { buildActivityMap8, rankBuckets8, summarizePipeline8, synthesizeFlow8 } from '../pipelines/pipeline8.js';
import '../styles/component558.css';

function MetricCard558({ label, value, max }) {
...
function TreeNode558({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode558 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState558 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer558(state, action) {
...
export default function Component558({ seed = 558, title = 'Component558' }) {
  const hook = useLogic8(seed + 558);
  const [state, dispatch] = useReducer(reducer558, initialState558);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm558(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset8(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState8(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree8(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute8(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast8(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies8(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution8(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow8(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets8(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap8(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline8(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm558(form);
    setErrors(found);
  ...
  return (
    <section className="component-558">
      <div className="header-558">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-558">
      ...
      <div className="grid-558">
        <div className="panel-558">
          {cards.map((item, idx) => (
            <MetricCard558 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
