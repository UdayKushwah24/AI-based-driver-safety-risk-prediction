...
import '../styles/component517.css';

function MetricCard517({ label, value, max }) {
...
function TreeNode517({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode517 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState517 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer517(state, action) {
...
export default function Component517({ seed = 517, title = 'Component517' }) {
  const hook = useLogic517(seed + 517);
  const [state, dispatch] = useReducer(reducer517, initialState517);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm517(seed));
  ...
  const table = useMemo(() => buildTableState517(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm517(form);
    setErrors(found);
  ...
  return (
    <section className="component-517">
      <div className="header-517">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-517">
      ...
      <div className="grid-517">
        <div className="panel-517">
          {cards.map((item, idx) => (
            <MetricCard517 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
