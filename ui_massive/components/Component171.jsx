...
import { useLogic171 } from '../hooks/useLogic171.js';
import { buildNestedTree171, buildTableState171, heavyCompute171, transformDataset171 } from '../utils/compute171.js';
import { buildForecast171, detectAnomalies171, segmentDistribution171 } from '../utils/compute171.js';
import { buildActivityMap171, rankBuckets171, summarizePipeline171, synthesizeFlow171 } from '../pipelines/pipeline171.js';
import '../styles/component171.css';

function MetricCard171({ label, value, max }) {
...
function TreeNode171({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode171 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState171 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer171(state, action) {
...
export default function Component171({ seed = 171, title = 'Component171' }) {
  const hook = useLogic171(seed + 171);
  const [state, dispatch] = useReducer(reducer171, initialState171);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm171(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset171(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState171(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree171(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute171(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast171(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies171(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution171(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow171(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets171(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap171(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline171(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm171(form);
    setErrors(found);
  ...
  return (
    <section className="component-171">
      <div className="header-171">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-171">
      ...
      <div className="grid-171">
        <div className="panel-171">
          {cards.map((item, idx) => (
            <MetricCard171 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
