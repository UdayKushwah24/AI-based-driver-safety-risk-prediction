...
import { useLogic105 } from '../hooks/useLogic105.js';
import { buildNestedTree105, buildTableState105, heavyCompute105, transformDataset105 } from '../utils/compute105.js';
import { buildForecast105, detectAnomalies105, segmentDistribution105 } from '../utils/compute105.js';
import { buildActivityMap105, rankBuckets105, summarizePipeline105, synthesizeFlow105 } from '../pipelines/pipeline105.js';
import '../styles/component655.css';

function MetricCard655({ label, value, max }) {
...
function TreeNode655({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode655 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState655 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer655(state, action) {
...
export default function Component655({ seed = 655, title = 'Component655' }) {
  const hook = useLogic105(seed + 655);
  const [state, dispatch] = useReducer(reducer655, initialState655);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm655(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset105(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState105(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree105(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute105(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast105(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies105(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution105(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow105(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets105(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap105(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline105(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm655(form);
    setErrors(found);
  ...
  return (
    <section className="component-655">
      <div className="header-655">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-655">
      ...
      <div className="grid-655">
        <div className="panel-655">
          {cards.map((item, idx) => (
            <MetricCard655 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
