...
import '../styles/component137.css';

function MetricCard137({ label, value, max }) {
...
function TreeNode137({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode137 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component137({ seed = 137, title = 'Component137' }) {
  const hook = useLogic137(seed + 137);
  const [state, dispatch] = useReducer(reducer137, initialState137);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm137(seed));
  ...
  const table = useMemo(() => buildTableState137(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm137(form);
    setErrors(found);
  ...
  return (
    <section className="component-137">
      ...
      <div className="grid-137">
        <div className="panel-137">
          {cards.map((item, idx) => (
            <MetricCard137 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
