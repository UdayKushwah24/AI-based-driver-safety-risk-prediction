...
import { useLogic296 } from '../hooks/useLogic296.js';
import { buildNestedTree296, buildTableState296, heavyCompute296, transformDataset296 } from '../utils/compute296.js';
import { buildForecast296, detectAnomalies296, segmentDistribution296 } from '../utils/compute296.js';
import { buildActivityMap296, rankBuckets296, summarizePipeline296, synthesizeFlow296 } from '../pipelines/pipeline296.js';
import '../styles/component846.css';

function MetricCard846({ label, value, max }) {
...
function TreeNode846({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode846 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState846 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer846(state, action) {
...
export default function Component846({ seed = 846, title = 'Component846' }) {
  const hook = useLogic296(seed + 846);
  const [state, dispatch] = useReducer(reducer846, initialState846);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm846(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset296(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState296(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree296(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute296(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast296(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies296(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution296(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow296(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets296(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap296(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline296(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm846(form);
    setErrors(found);
  ...
  return (
    <section className="component-846">
      <div className="header-846">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-846">
      ...
      <div className="grid-846">
        <div className="panel-846">
          {cards.map((item, idx) => (
            <MetricCard846 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
