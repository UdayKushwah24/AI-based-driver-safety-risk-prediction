...
import '../styles/component897.css';

function MetricCard897({ label, value, max }) {
...
function TreeNode897({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode897 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState897 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer897(state, action) {
...
export default function Component897({ seed = 897, title = 'Component897' }) {
  const hook = useLogic347(seed + 897);
  const [state, dispatch] = useReducer(reducer897, initialState897);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm897(seed));
  ...
  const table = useMemo(() => buildTableState347(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm897(form);
    setErrors(found);
  ...
  return (
    <section className="component-897">
      <div className="header-897">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-897">
      ...
      <div className="grid-897">
        <div className="panel-897">
          {cards.map((item, idx) => (
            <MetricCard897 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
