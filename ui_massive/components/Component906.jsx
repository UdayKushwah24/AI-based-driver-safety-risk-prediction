...
import { useLogic356 } from '../hooks/useLogic356.js';
import { buildNestedTree356, buildTableState356, heavyCompute356, transformDataset356 } from '../utils/compute356.js';
import { buildForecast356, detectAnomalies356, segmentDistribution356 } from '../utils/compute356.js';
import { buildActivityMap356, rankBuckets356, summarizePipeline356, synthesizeFlow356 } from '../pipelines/pipeline356.js';
import '../styles/component906.css';

function MetricCard906({ label, value, max }) {
...
function TreeNode906({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode906 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState906 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer906(state, action) {
...
export default function Component906({ seed = 906, title = 'Component906' }) {
  const hook = useLogic356(seed + 906);
  const [state, dispatch] = useReducer(reducer906, initialState906);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm906(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset356(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState356(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree356(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute356(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast356(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies356(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution356(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow356(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets356(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap356(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline356(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm906(form);
    setErrors(found);
  ...
  return (
    <section className="component-906">
      <div className="header-906">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-906">
      ...
      <div className="grid-906">
        <div className="panel-906">
          {cards.map((item, idx) => (
            <MetricCard906 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
