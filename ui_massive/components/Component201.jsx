...
import '../styles/component201.css';

function MetricCard201({ label, value, max }) {
...
function TreeNode201({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode201 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState201 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer201(state, action) {
...
export default function Component201({ seed = 201, title = 'Component201' }) {
  const hook = useLogic201(seed + 201);
  const [state, dispatch] = useReducer(reducer201, initialState201);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm201(seed));
  ...
  const table = useMemo(() => buildTableState201(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm201(form);
    setErrors(found);
  ...
  return (
    <section className="component-201">
      <div className="header-201">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-201">
      ...
      <div className="grid-201">
        <div className="panel-201">
          {cards.map((item, idx) => (
            <MetricCard201 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
