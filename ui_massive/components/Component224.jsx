...
import '../styles/component224.css';

function MetricCard224({ label, value, max }) {
...
function TreeNode224({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode224 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState224 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer224(state, action) {
...
export default function Component224({ seed = 224, title = 'Component224' }) {
  const hook = useLogic224(seed + 224);
  const [state, dispatch] = useReducer(reducer224, initialState224);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm224(seed));
  ...
  const table = useMemo(() => buildTableState224(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm224(form);
    setErrors(found);
  ...
  return (
    <section className="component-224">
      <div className="header-224">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-224">
      ...
      <div className="grid-224">
        <div className="panel-224">
          {cards.map((item, idx) => (
            <MetricCard224 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
