...
import { useLogic398 } from '../hooks/useLogic398.js';
import { buildNestedTree398, buildTableState398, heavyCompute398, transformDataset398 } from '../utils/compute398.js';
import { buildForecast398, detectAnomalies398, segmentDistribution398 } from '../utils/compute398.js';
import { buildActivityMap398, rankBuckets398, summarizePipeline398, synthesizeFlow398 } from '../pipelines/pipeline398.js';
import '../styles/component398.css';

function MetricCard398({ label, value, max }) {
...
function TreeNode398({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode398 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState398 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer398(state, action) {
...
export default function Component398({ seed = 398, title = 'Component398' }) {
  const hook = useLogic398(seed + 398);
  const [state, dispatch] = useReducer(reducer398, initialState398);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm398(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset398(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState398(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree398(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute398(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast398(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies398(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution398(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow398(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets398(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap398(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline398(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm398(form);
    setErrors(found);
  ...
  return (
    <section className="component-398">
      <div className="header-398">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-398">
      ...
      <div className="grid-398">
        <div className="panel-398">
          {cards.map((item, idx) => (
            <MetricCard398 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
