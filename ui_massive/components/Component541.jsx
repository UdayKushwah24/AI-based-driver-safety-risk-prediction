...
import '../styles/component541.css';

function MetricCard541({ label, value, max }) {
...
function TreeNode541({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode541 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState541 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer541(state, action) {
...
export default function Component541({ seed = 541, title = 'Component541' }) {
  const hook = useLogic541(seed + 541);
  const [state, dispatch] = useReducer(reducer541, initialState541);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm541(seed));
  ...
  const table = useMemo(() => buildTableState541(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm541(form);
    setErrors(found);
  ...
  return (
    <section className="component-541">
      <div className="header-541">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-541">
      ...
      <div className="grid-541">
        <div className="panel-541">
          {cards.map((item, idx) => (
            <MetricCard541 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
