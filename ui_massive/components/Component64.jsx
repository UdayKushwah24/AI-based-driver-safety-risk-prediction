...
import '../styles/component64.css';

function MetricCard64({ label, value, max }) {
...
function TreeNode64({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode64 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component64({ seed = 64, title = 'Component64' }) {
  const hook = useLogic64(seed + 64);
  const [state, dispatch] = useReducer(reducer64, initialState64);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm64(seed));
  ...
  const table = useMemo(() => buildTableState64(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm64(form);
    setErrors(found);
  ...
  return (
    <section className="component-64">
      ...
      <div className="grid-64">
        <div className="panel-64">
          {cards.map((item, idx) => (
            <MetricCard64 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
