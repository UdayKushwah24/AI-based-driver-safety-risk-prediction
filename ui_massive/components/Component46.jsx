...
import '../styles/component46.css';

function MetricCard46({ label, value, max }) {
...
function TreeNode46({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode46 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component46({ seed = 46, title = 'Component46' }) {
  const hook = useLogic46(seed + 46);
  const [state, dispatch] = useReducer(reducer46, initialState46);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm46(seed));
  ...
  const table = useMemo(() => buildTableState46(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm46(form);
    setErrors(found);
  ...
  return (
    <section className="component-46">
      ...
      <div className="grid-46">
        <div className="panel-46">
          {cards.map((item, idx) => (
            <MetricCard46 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
