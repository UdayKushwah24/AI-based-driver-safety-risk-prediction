...
import '../styles/component132.css';

function MetricCard132({ label, value, max }) {
...
function TreeNode132({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode132 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState132 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer132(state, action) {
...
export default function Component132({ seed = 132, title = 'Component132' }) {
  const hook = useLogic132(seed + 132);
  const [state, dispatch] = useReducer(reducer132, initialState132);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm132(seed));
  ...
  const table = useMemo(() => buildTableState132(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm132(form);
    setErrors(found);
  ...
  return (
    <section className="component-132">
      <div className="header-132">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-132">
      ...
      <div className="grid-132">
        <div className="panel-132">
          {cards.map((item, idx) => (
            <MetricCard132 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
