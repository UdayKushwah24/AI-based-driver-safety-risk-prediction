...
import '../styles/component538.css';

function MetricCard538({ label, value, max }) {
...
function TreeNode538({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode538 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState538 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer538(state, action) {
...
export default function Component538({ seed = 538, title = 'Component538' }) {
  const hook = useLogic538(seed + 538);
  const [state, dispatch] = useReducer(reducer538, initialState538);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm538(seed));
  ...
  const table = useMemo(() => buildTableState538(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm538(form);
    setErrors(found);
  ...
  return (
    <section className="component-538">
      <div className="header-538">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-538">
      ...
      <div className="grid-538">
        <div className="panel-538">
          {cards.map((item, idx) => (
            <MetricCard538 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
