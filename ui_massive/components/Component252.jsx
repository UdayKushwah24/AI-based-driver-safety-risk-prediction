...
import '../styles/component252.css';

function MetricCard252({ label, value, max }) {
...
function TreeNode252({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode252 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState252 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer252(state, action) {
...
export default function Component252({ seed = 252, title = 'Component252' }) {
  const hook = useLogic252(seed + 252);
  const [state, dispatch] = useReducer(reducer252, initialState252);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm252(seed));
  ...
  const table = useMemo(() => buildTableState252(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm252(form);
    setErrors(found);
  ...
  return (
    <section className="component-252">
      <div className="header-252">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-252">
      ...
      <div className="grid-252">
        <div className="panel-252">
          {cards.map((item, idx) => (
            <MetricCard252 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
