...
import '../styles/component436.css';

function MetricCard436({ label, value, max }) {
...
function TreeNode436({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode436 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState436 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer436(state, action) {
...
export default function Component436({ seed = 436, title = 'Component436' }) {
  const hook = useLogic436(seed + 436);
  const [state, dispatch] = useReducer(reducer436, initialState436);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm436(seed));
  ...
  const table = useMemo(() => buildTableState436(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm436(form);
    setErrors(found);
  ...
  return (
    <section className="component-436">
      <div className="header-436">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-436">
      ...
      <div className="grid-436">
        <div className="panel-436">
          {cards.map((item, idx) => (
            <MetricCard436 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
