...
import '../styles/component408.css';

function MetricCard408({ label, value, max }) {
...
function TreeNode408({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode408 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState408 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer408(state, action) {
...
export default function Component408({ seed = 408, title = 'Component408' }) {
  const hook = useLogic408(seed + 408);
  const [state, dispatch] = useReducer(reducer408, initialState408);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm408(seed));
  ...
  const table = useMemo(() => buildTableState408(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm408(form);
    setErrors(found);
  ...
  return (
    <section className="component-408">
      <div className="header-408">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-408">
      ...
      <div className="grid-408">
        <div className="panel-408">
          {cards.map((item, idx) => (
            <MetricCard408 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
