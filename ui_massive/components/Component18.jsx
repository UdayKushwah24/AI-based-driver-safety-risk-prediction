...
import '../styles/component18.css';

function MetricCard18({ label, value, max }) {
...
function TreeNode18({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode18 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component18({ seed = 18, title = 'Component18' }) {
  const hook = useLogic18(seed + 18);
  const [state, dispatch] = useReducer(reducer18, initialState18);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm18(seed));
  ...
  const table = useMemo(() => buildTableState18(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm18(form);
    setErrors(found);
  ...
  return (
    <section className="component-18">
      <div className="header-18">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-18">
      ...
      <div className="grid-18">
        <div className="panel-18">
          {cards.map((item, idx) => (
            <MetricCard18 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
