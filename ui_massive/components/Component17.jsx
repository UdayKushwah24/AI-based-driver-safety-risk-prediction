...
import '../styles/component17.css';

function MetricCard17({ label, value, max }) {
...
function TreeNode17({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode17 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component17({ seed = 17, title = 'Component17' }) {
  const hook = useLogic17(seed + 17);
  const [state, dispatch] = useReducer(reducer17, initialState17);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm17(seed));
  ...
  const table = useMemo(() => buildTableState17(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm17(form);
    setErrors(found);
  ...
  return (
    <section className="component-17">
      ...
      <div className="grid-17">
        <div className="panel-17">
          {cards.map((item, idx) => (
            <MetricCard17 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
