...
import { useLogic294 } from '../hooks/useLogic294.js';
import { buildNestedTree294, buildTableState294, heavyCompute294, transformDataset294 } from '../utils/compute294.js';
import { buildForecast294, detectAnomalies294, segmentDistribution294 } from '../utils/compute294.js';
import { buildActivityMap294, rankBuckets294, summarizePipeline294, synthesizeFlow294 } from '../pipelines/pipeline294.js';
import '../styles/component294.css';

function MetricCard294({ label, value, max }) {
...
function TreeNode294({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode294 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState294 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer294(state, action) {
...
export default function Component294({ seed = 294, title = 'Component294' }) {
  const hook = useLogic294(seed + 294);
  const [state, dispatch] = useReducer(reducer294, initialState294);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm294(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset294(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState294(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree294(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute294(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast294(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies294(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution294(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow294(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets294(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap294(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline294(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm294(form);
    setErrors(found);
  ...
  return (
    <section className="component-294">
      <div className="header-294">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-294">
      ...
      <div className="grid-294">
        <div className="panel-294">
          {cards.map((item, idx) => (
            <MetricCard294 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
