...
import '../styles/component844.css';

function MetricCard844({ label, value, max }) {
...
function TreeNode844({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode844 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState844 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer844(state, action) {
...
export default function Component844({ seed = 844, title = 'Component844' }) {
  const hook = useLogic294(seed + 844);
  const [state, dispatch] = useReducer(reducer844, initialState844);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm844(seed));
  ...
  const table = useMemo(() => buildTableState294(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm844(form);
    setErrors(found);
  ...
  return (
    <section className="component-844">
      <div className="header-844">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-844">
      ...
      <div className="grid-844">
        <div className="panel-844">
          {cards.map((item, idx) => (
            <MetricCard844 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
