...
import '../styles/component365.css';

function MetricCard365({ label, value, max }) {
...
function TreeNode365({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode365 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState365 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer365(state, action) {
...
export default function Component365({ seed = 365, title = 'Component365' }) {
  const hook = useLogic365(seed + 365);
  const [state, dispatch] = useReducer(reducer365, initialState365);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm365(seed));
  ...
  const table = useMemo(() => buildTableState365(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm365(form);
    setErrors(found);
  ...
  return (
    <section className="component-365">
      <div className="header-365">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-365">
      ...
      <div className="grid-365">
        <div className="panel-365">
          {cards.map((item, idx) => (
            <MetricCard365 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
