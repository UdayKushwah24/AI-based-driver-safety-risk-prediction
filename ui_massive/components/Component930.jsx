...
import { useLogic380 } from '../hooks/useLogic380.js';
import { buildNestedTree380, buildTableState380, heavyCompute380, transformDataset380 } from '../utils/compute380.js';
import { buildForecast380, detectAnomalies380, segmentDistribution380 } from '../utils/compute380.js';
import { buildActivityMap380, rankBuckets380, summarizePipeline380, synthesizeFlow380 } from '../pipelines/pipeline380.js';
import '../styles/component930.css';

function MetricCard930({ label, value, max }) {
...
function TreeNode930({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode930 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState930 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer930(state, action) {
...
export default function Component930({ seed = 930, title = 'Component930' }) {
  const hook = useLogic380(seed + 930);
  const [state, dispatch] = useReducer(reducer930, initialState930);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm930(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset380(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState380(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree380(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute380(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast380(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies380(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution380(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow380(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets380(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap380(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline380(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm930(form);
    setErrors(found);
  ...
  return (
    <section className="component-930">
      <div className="header-930">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-930">
      ...
      <div className="grid-930">
        <div className="panel-930">
          {cards.map((item, idx) => (
            <MetricCard930 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
