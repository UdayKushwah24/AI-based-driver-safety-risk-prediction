...
import '../styles/component825.css';

function MetricCard825({ label, value, max }) {
...
function TreeNode825({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode825 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState825 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer825(state, action) {
...
export default function Component825({ seed = 825, title = 'Component825' }) {
  const hook = useLogic275(seed + 825);
  const [state, dispatch] = useReducer(reducer825, initialState825);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm825(seed));
  ...
  const table = useMemo(() => buildTableState275(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm825(form);
    setErrors(found);
  ...
  return (
    <section className="component-825">
      <div className="header-825">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-825">
      ...
      <div className="grid-825">
        <div className="panel-825">
          {cards.map((item, idx) => (
            <MetricCard825 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
