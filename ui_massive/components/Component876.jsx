...
import '../styles/component876.css';

function MetricCard876({ label, value, max }) {
...
function TreeNode876({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode876 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState876 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer876(state, action) {
...
export default function Component876({ seed = 876, title = 'Component876' }) {
  const hook = useLogic326(seed + 876);
  const [state, dispatch] = useReducer(reducer876, initialState876);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm876(seed));
  ...
  const table = useMemo(() => buildTableState326(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm876(form);
    setErrors(found);
  ...
  return (
    <section className="component-876">
      <div className="header-876">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-876">
      ...
      <div className="grid-876">
        <div className="panel-876">
          {cards.map((item, idx) => (
            <MetricCard876 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
