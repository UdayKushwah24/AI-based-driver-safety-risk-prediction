...
import '../styles/component105.css';

function MetricCard105({ label, value, max }) {
...
function TreeNode105({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode105 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component105({ seed = 105, title = 'Component105' }) {
  const hook = useLogic105(seed + 105);
  const [state, dispatch] = useReducer(reducer105, initialState105);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm105(seed));
  ...
  const table = useMemo(() => buildTableState105(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm105(form);
    setErrors(found);
  ...
  return (
    <section className="component-105">
      ...
      <div className="grid-105">
        <div className="panel-105">
          {cards.map((item, idx) => (
            <MetricCard105 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
