...
import '../styles/component756.css';

function MetricCard756({ label, value, max }) {
...
function TreeNode756({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode756 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState756 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer756(state, action) {
...
export default function Component756({ seed = 756, title = 'Component756' }) {
  const hook = useLogic206(seed + 756);
  const [state, dispatch] = useReducer(reducer756, initialState756);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm756(seed));
  ...
  const table = useMemo(() => buildTableState206(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm756(form);
    setErrors(found);
  ...
  return (
    <section className="component-756">
      <div className="header-756">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-756">
      ...
      <div className="grid-756">
        <div className="panel-756">
          {cards.map((item, idx) => (
            <MetricCard756 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
