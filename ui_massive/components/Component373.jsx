...
import { useLogic373 } from '../hooks/useLogic373.js';
import { buildNestedTree373, buildTableState373, heavyCompute373, transformDataset373 } from '../utils/compute373.js';
import { buildForecast373, detectAnomalies373, segmentDistribution373 } from '../utils/compute373.js';
import { buildActivityMap373, rankBuckets373, summarizePipeline373, synthesizeFlow373 } from '../pipelines/pipeline373.js';
import '../styles/component373.css';

function MetricCard373({ label, value, max }) {
...
function TreeNode373({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode373 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState373 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer373(state, action) {
...
export default function Component373({ seed = 373, title = 'Component373' }) {
  const hook = useLogic373(seed + 373);
  const [state, dispatch] = useReducer(reducer373, initialState373);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm373(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset373(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState373(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree373(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute373(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast373(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies373(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution373(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow373(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets373(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap373(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline373(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm373(form);
    setErrors(found);
  ...
  return (
    <section className="component-373">
      <div className="header-373">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-373">
      ...
      <div className="grid-373">
        <div className="panel-373">
          {cards.map((item, idx) => (
            <MetricCard373 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
