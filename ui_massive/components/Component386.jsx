...
import { useLogic386 } from '../hooks/useLogic386.js';
import { buildNestedTree386, buildTableState386, heavyCompute386, transformDataset386 } from '../utils/compute386.js';
import { buildForecast386, detectAnomalies386, segmentDistribution386 } from '../utils/compute386.js';
import { buildActivityMap386, rankBuckets386, summarizePipeline386, synthesizeFlow386 } from '../pipelines/pipeline386.js';
import '../styles/component386.css';

function MetricCard386({ label, value, max }) {
...
function TreeNode386({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode386 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState386 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer386(state, action) {
...
export default function Component386({ seed = 386, title = 'Component386' }) {
  const hook = useLogic386(seed + 386);
  const [state, dispatch] = useReducer(reducer386, initialState386);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm386(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset386(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState386(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree386(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute386(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast386(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies386(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution386(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow386(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets386(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap386(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline386(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm386(form);
    setErrors(found);
  ...
  return (
    <section className="component-386">
      <div className="header-386">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-386">
      ...
      <div className="grid-386">
        <div className="panel-386">
          {cards.map((item, idx) => (
            <MetricCard386 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
