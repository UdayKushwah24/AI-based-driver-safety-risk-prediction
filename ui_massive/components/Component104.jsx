...
import '../styles/component104.css';

function MetricCard104({ label, value, max }) {
...
function TreeNode104({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode104 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component104({ seed = 104, title = 'Component104' }) {
  const hook = useLogic104(seed + 104);
  const [state, dispatch] = useReducer(reducer104, initialState104);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm104(seed));
  ...
  const table = useMemo(() => buildTableState104(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm104(form);
    setErrors(found);
  ...
  return (
    <section className="component-104">
      ...
      <div className="grid-104">
        <div className="panel-104">
          {cards.map((item, idx) => (
            <MetricCard104 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
