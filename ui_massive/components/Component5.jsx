...
function MetricCard5({ label, value, max }) {
...
function TreeNode5({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode5 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component5({ seed = 5, title = 'Component5' }) {
  const hook = useLogic5(seed + 5);
  const [state, dispatch] = useReducer(reducer5, initialState5);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm5(seed));
  ...
  const table = useMemo(() => buildTableState5(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm5(form);
    setErrors(found);
    ...
    <section className="component-5">
      ...
      <div className="grid-5">
        <div className="panel-5">
          {cards.map((item, idx) => (
            <MetricCard5 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
