...
import { useLogic388 } from '../hooks/useLogic388.js';
import { buildNestedTree388, buildTableState388, heavyCompute388, transformDataset388 } from '../utils/compute388.js';
import { buildForecast388, detectAnomalies388, segmentDistribution388 } from '../utils/compute388.js';
import { buildActivityMap388, rankBuckets388, summarizePipeline388, synthesizeFlow388 } from '../pipelines/pipeline388.js';
import '../styles/component938.css';

function MetricCard938({ label, value, max }) {
...
function TreeNode938({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode938 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState938 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer938(state, action) {
...
export default function Component938({ seed = 938, title = 'Component938' }) {
  const hook = useLogic388(seed + 938);
  const [state, dispatch] = useReducer(reducer938, initialState938);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm938(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset388(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState388(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree388(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute388(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast388(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies388(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution388(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow388(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets388(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap388(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline388(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm938(form);
    setErrors(found);
  ...
  return (
    <section className="component-938">
      <div className="header-938">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-938">
      ...
      <div className="grid-938">
        <div className="panel-938">
          {cards.map((item, idx) => (
            <MetricCard938 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
