...
import { useLogic173 } from '../hooks/useLogic173.js';
import { buildNestedTree173, buildTableState173, heavyCompute173, transformDataset173 } from '../utils/compute173.js';
import { buildForecast173, detectAnomalies173, segmentDistribution173 } from '../utils/compute173.js';
import { buildActivityMap173, rankBuckets173, summarizePipeline173, synthesizeFlow173 } from '../pipelines/pipeline173.js';
import '../styles/component173.css';

function MetricCard173({ label, value, max }) {
...
function TreeNode173({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode173 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState173 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer173(state, action) {
...
export default function Component173({ seed = 173, title = 'Component173' }) {
  const hook = useLogic173(seed + 173);
  const [state, dispatch] = useReducer(reducer173, initialState173);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm173(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset173(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState173(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree173(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute173(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast173(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies173(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution173(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow173(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets173(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap173(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline173(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm173(form);
    setErrors(found);
  ...
  return (
    <section className="component-173">
      <div className="header-173">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-173">
      ...
      <div className="grid-173">
        <div className="panel-173">
          {cards.map((item, idx) => (
            <MetricCard173 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
