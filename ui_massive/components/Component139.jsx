...
import '../styles/component139.css';

function MetricCard139({ label, value, max }) {
...
function TreeNode139({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode139 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component139({ seed = 139, title = 'Component139' }) {
  const hook = useLogic139(seed + 139);
  const [state, dispatch] = useReducer(reducer139, initialState139);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm139(seed));
  ...
  const table = useMemo(() => buildTableState139(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm139(form);
    setErrors(found);
  ...
  return (
    <section className="component-139">
      ...
      <div className="grid-139">
        <div className="panel-139">
          {cards.map((item, idx) => (
            <MetricCard139 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
