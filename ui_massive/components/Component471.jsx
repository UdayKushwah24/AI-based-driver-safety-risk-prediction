...
import '../styles/component471.css';

function MetricCard471({ label, value, max }) {
...
function TreeNode471({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode471 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState471 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer471(state, action) {
...
export default function Component471({ seed = 471, title = 'Component471' }) {
  const hook = useLogic471(seed + 471);
  const [state, dispatch] = useReducer(reducer471, initialState471);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm471(seed));
  ...
  const table = useMemo(() => buildTableState471(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm471(form);
    setErrors(found);
  ...
  return (
    <section className="component-471">
      <div className="header-471">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-471">
      ...
      <div className="grid-471">
        <div className="panel-471">
          {cards.map((item, idx) => (
            <MetricCard471 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
