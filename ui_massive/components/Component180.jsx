...
import '../styles/component180.css';

function MetricCard180({ label, value, max }) {
...
function TreeNode180({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode180 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState180 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer180(state, action) {
...
export default function Component180({ seed = 180, title = 'Component180' }) {
  const hook = useLogic180(seed + 180);
  const [state, dispatch] = useReducer(reducer180, initialState180);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm180(seed));
  ...
  const table = useMemo(() => buildTableState180(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm180(form);
    setErrors(found);
  ...
  return (
    <section className="component-180">
      <div className="header-180">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-180">
      ...
      <div className="grid-180">
        <div className="panel-180">
          {cards.map((item, idx) => (
            <MetricCard180 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
