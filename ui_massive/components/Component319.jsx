...
import '../styles/component319.css';

function MetricCard319({ label, value, max }) {
...
function TreeNode319({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode319 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component319({ seed = 319, title = 'Component319' }) {
  const hook = useLogic319(seed + 319);
  const [state, dispatch] = useReducer(reducer319, initialState319);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm319(seed));
  ...
  const table = useMemo(() => buildTableState319(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm319(form);
    setErrors(found);
  ...
  return (
    <section className="component-319">
      <div className="header-319">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-319">
      ...
      <div className="grid-319">
        <div className="panel-319">
          {cards.map((item, idx) => (
            <MetricCard319 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
