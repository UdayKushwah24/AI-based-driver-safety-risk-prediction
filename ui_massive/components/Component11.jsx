...
function MetricCard11({ label, value, max }) {
...
function TreeNode11({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode11 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component11({ seed = 11, title = 'Component11' }) {
  const hook = useLogic11(seed + 11);
  const [state, dispatch] = useReducer(reducer11, initialState11);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm11(seed));
  ...
  const table = useMemo(() => buildTableState11(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm11(form);
    setErrors(found);
    ...
    <section className="component-11">
      ...
      <div className="grid-11">
        <div className="panel-11">
          {cards.map((item, idx) => (
            <MetricCard11 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
