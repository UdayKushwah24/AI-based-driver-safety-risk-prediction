...
import { useLogic46 } from '../hooks/useLogic46.js';
import { buildNestedTree46, buildTableState46, heavyCompute46, transformDataset46 } from '../utils/compute46.js';
import { buildForecast46, detectAnomalies46, segmentDistribution46 } from '../utils/compute46.js';
import { buildActivityMap46, rankBuckets46, summarizePipeline46, synthesizeFlow46 } from '../pipelines/pipeline46.js';
import '../styles/component596.css';

function MetricCard596({ label, value, max }) {
...
function TreeNode596({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode596 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState596 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer596(state, action) {
...
export default function Component596({ seed = 596, title = 'Component596' }) {
  const hook = useLogic46(seed + 596);
  const [state, dispatch] = useReducer(reducer596, initialState596);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm596(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset46(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState46(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree46(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute46(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast46(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies46(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution46(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow46(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets46(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap46(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline46(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm596(form);
    setErrors(found);
  ...
  return (
    <section className="component-596">
      <div className="header-596">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-596">
      ...
      <div className="grid-596">
        <div className="panel-596">
          {cards.map((item, idx) => (
            <MetricCard596 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
