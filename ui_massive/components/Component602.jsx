...
import { useLogic52 } from '../hooks/useLogic52.js';
import { buildNestedTree52, buildTableState52, heavyCompute52, transformDataset52 } from '../utils/compute52.js';
import { buildForecast52, detectAnomalies52, segmentDistribution52 } from '../utils/compute52.js';
import { buildActivityMap52, rankBuckets52, summarizePipeline52, synthesizeFlow52 } from '../pipelines/pipeline52.js';
import '../styles/component602.css';

function MetricCard602({ label, value, max }) {
...
function TreeNode602({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode602 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState602 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer602(state, action) {
...
export default function Component602({ seed = 602, title = 'Component602' }) {
  const hook = useLogic52(seed + 602);
  const [state, dispatch] = useReducer(reducer602, initialState602);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm602(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset52(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState52(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree52(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute52(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast52(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies52(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution52(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow52(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets52(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap52(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline52(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm602(form);
    setErrors(found);
  ...
  return (
    <section className="component-602">
      <div className="header-602">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-602">
      ...
      <div className="grid-602">
        <div className="panel-602">
          {cards.map((item, idx) => (
            <MetricCard602 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
