...
import '../styles/component223.css';

function MetricCard223({ label, value, max }) {
...
function TreeNode223({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode223 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState223 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer223(state, action) {
...
export default function Component223({ seed = 223, title = 'Component223' }) {
  const hook = useLogic223(seed + 223);
  const [state, dispatch] = useReducer(reducer223, initialState223);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm223(seed));
  ...
  const table = useMemo(() => buildTableState223(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm223(form);
    setErrors(found);
  ...
  return (
    <section className="component-223">
      <div className="header-223">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-223">
      ...
      <div className="grid-223">
        <div className="panel-223">
          {cards.map((item, idx) => (
            <MetricCard223 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
