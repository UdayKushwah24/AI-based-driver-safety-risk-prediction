...
import '../styles/component106.css';

function MetricCard106({ label, value, max }) {
...
function TreeNode106({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode106 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component106({ seed = 106, title = 'Component106' }) {
  const hook = useLogic106(seed + 106);
  const [state, dispatch] = useReducer(reducer106, initialState106);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm106(seed));
  ...
  const table = useMemo(() => buildTableState106(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm106(form);
    setErrors(found);
  ...
  return (
    <section className="component-106">
      ...
      <div className="grid-106">
        <div className="panel-106">
          {cards.map((item, idx) => (
            <MetricCard106 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
