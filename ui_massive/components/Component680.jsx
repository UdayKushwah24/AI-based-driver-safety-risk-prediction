...
import { useLogic130 } from '../hooks/useLogic130.js';
import { buildNestedTree130, buildTableState130, heavyCompute130, transformDataset130 } from '../utils/compute130.js';
import { buildForecast130, detectAnomalies130, segmentDistribution130 } from '../utils/compute130.js';
import { buildActivityMap130, rankBuckets130, summarizePipeline130, synthesizeFlow130 } from '../pipelines/pipeline130.js';
import '../styles/component680.css';

function MetricCard680({ label, value, max }) {
...
function TreeNode680({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode680 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState680 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer680(state, action) {
...
export default function Component680({ seed = 680, title = 'Component680' }) {
  const hook = useLogic130(seed + 680);
  const [state, dispatch] = useReducer(reducer680, initialState680);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm680(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset130(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState130(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree130(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute130(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast130(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies130(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution130(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow130(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets130(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap130(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline130(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm680(form);
    setErrors(found);
  ...
  return (
    <section className="component-680">
      <div className="header-680">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-680">
      ...
      <div className="grid-680">
        <div className="panel-680">
          {cards.map((item, idx) => (
            <MetricCard680 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
