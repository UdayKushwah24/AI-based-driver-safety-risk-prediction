...
import { useLogic154 } from '../hooks/useLogic154.js';
import { buildNestedTree154, buildTableState154, heavyCompute154, transformDataset154 } from '../utils/compute154.js';
import { buildForecast154, detectAnomalies154, segmentDistribution154 } from '../utils/compute154.js';
import { buildActivityMap154, rankBuckets154, summarizePipeline154, synthesizeFlow154 } from '../pipelines/pipeline154.js';
import '../styles/component154.css';

function MetricCard154({ label, value, max }) {
...
function TreeNode154({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode154 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState154 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer154(state, action) {
...
export default function Component154({ seed = 154, title = 'Component154' }) {
  const hook = useLogic154(seed + 154);
  const [state, dispatch] = useReducer(reducer154, initialState154);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm154(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset154(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState154(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree154(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute154(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast154(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies154(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution154(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow154(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets154(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap154(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline154(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm154(form);
    setErrors(found);
  ...
  return (
    <section className="component-154">
      <div className="header-154">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-154">
      ...
      <div className="grid-154">
        <div className="panel-154">
          {cards.map((item, idx) => (
            <MetricCard154 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
