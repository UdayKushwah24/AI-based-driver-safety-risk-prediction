...
import '../styles/component971.css';

function MetricCard971({ label, value, max }) {
...
function TreeNode971({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode971 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState971 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer971(state, action) {
...
export default function Component971({ seed = 971, title = 'Component971' }) {
  const hook = useLogic421(seed + 971);
  const [state, dispatch] = useReducer(reducer971, initialState971);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm971(seed));
  ...
  const table = useMemo(() => buildTableState421(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm971(form);
    setErrors(found);
  ...
  return (
    <section className="component-971">
      <div className="header-971">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-971">
      ...
      <div className="grid-971">
        <div className="panel-971">
          {cards.map((item, idx) => (
            <MetricCard971 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
