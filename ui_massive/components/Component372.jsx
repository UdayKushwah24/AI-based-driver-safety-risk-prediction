...
import '../styles/component372.css';

function MetricCard372({ label, value, max }) {
...
function TreeNode372({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode372 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState372 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer372(state, action) {
...
export default function Component372({ seed = 372, title = 'Component372' }) {
  const hook = useLogic372(seed + 372);
  const [state, dispatch] = useReducer(reducer372, initialState372);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm372(seed));
  ...
  const table = useMemo(() => buildTableState372(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm372(form);
    setErrors(found);
  ...
  return (
    <section className="component-372">
      <div className="header-372">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-372">
      ...
      <div className="grid-372">
        <div className="panel-372">
          {cards.map((item, idx) => (
            <MetricCard372 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
