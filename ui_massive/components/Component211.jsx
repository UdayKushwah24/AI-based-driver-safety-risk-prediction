...
import { useLogic211 } from '../hooks/useLogic211.js';
import { buildNestedTree211, buildTableState211, heavyCompute211, transformDataset211 } from '../utils/compute211.js';
import { buildForecast211, detectAnomalies211, segmentDistribution211 } from '../utils/compute211.js';
import { buildActivityMap211, rankBuckets211, summarizePipeline211, synthesizeFlow211 } from '../pipelines/pipeline211.js';
import '../styles/component211.css';

function MetricCard211({ label, value, max }) {
...
function TreeNode211({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode211 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState211 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer211(state, action) {
...
export default function Component211({ seed = 211, title = 'Component211' }) {
  const hook = useLogic211(seed + 211);
  const [state, dispatch] = useReducer(reducer211, initialState211);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm211(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset211(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState211(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree211(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute211(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast211(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies211(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution211(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow211(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets211(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap211(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline211(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm211(form);
    setErrors(found);
  ...
  return (
    <section className="component-211">
      <div className="header-211">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-211">
      ...
      <div className="grid-211">
        <div className="panel-211">
          {cards.map((item, idx) => (
            <MetricCard211 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
