...
import '../styles/component78.css';

function MetricCard78({ label, value, max }) {
...
function TreeNode78({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode78 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState78 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer78(state, action) {
...
export default function Component78({ seed = 78, title = 'Component78' }) {
  const hook = useLogic78(seed + 78);
  const [state, dispatch] = useReducer(reducer78, initialState78);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm78(seed));
  ...
  const table = useMemo(() => buildTableState78(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm78(form);
    setErrors(found);
  ...
  return (
    <section className="component-78">
      <div className="header-78">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-78">
      ...
      <div className="grid-78">
        <div className="panel-78">
          {cards.map((item, idx) => (
            <MetricCard78 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
