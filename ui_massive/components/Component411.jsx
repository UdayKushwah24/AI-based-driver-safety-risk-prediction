...
import { useLogic411 } from '../hooks/useLogic411.js';
import { buildNestedTree411, buildTableState411, heavyCompute411, transformDataset411 } from '../utils/compute411.js';
import { buildForecast411, detectAnomalies411, segmentDistribution411 } from '../utils/compute411.js';
import { buildActivityMap411, rankBuckets411, summarizePipeline411, synthesizeFlow411 } from '../pipelines/pipeline411.js';
import '../styles/component411.css';

function MetricCard411({ label, value, max }) {
...
function TreeNode411({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode411 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState411 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer411(state, action) {
...
export default function Component411({ seed = 411, title = 'Component411' }) {
  const hook = useLogic411(seed + 411);
  const [state, dispatch] = useReducer(reducer411, initialState411);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm411(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset411(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState411(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree411(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute411(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast411(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies411(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution411(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow411(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets411(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap411(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline411(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm411(form);
    setErrors(found);
  ...
  return (
    <section className="component-411">
      <div className="header-411">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-411">
      ...
      <div className="grid-411">
        <div className="panel-411">
          {cards.map((item, idx) => (
            <MetricCard411 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
