...
import '../styles/component856.css';

function MetricCard856({ label, value, max }) {
...
function TreeNode856({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode856 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState856 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer856(state, action) {
...
export default function Component856({ seed = 856, title = 'Component856' }) {
  const hook = useLogic306(seed + 856);
  const [state, dispatch] = useReducer(reducer856, initialState856);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm856(seed));
  ...
  const table = useMemo(() => buildTableState306(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm856(form);
    setErrors(found);
  ...
  return (
    <section className="component-856">
      <div className="header-856">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-856">
      ...
      <div className="grid-856">
        <div className="panel-856">
          {cards.map((item, idx) => (
            <MetricCard856 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
