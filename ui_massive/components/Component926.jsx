...
import { useLogic376 } from '../hooks/useLogic376.js';
import { buildNestedTree376, buildTableState376, heavyCompute376, transformDataset376 } from '../utils/compute376.js';
import { buildForecast376, detectAnomalies376, segmentDistribution376 } from '../utils/compute376.js';
import { buildActivityMap376, rankBuckets376, summarizePipeline376, synthesizeFlow376 } from '../pipelines/pipeline376.js';
import '../styles/component926.css';

function MetricCard926({ label, value, max }) {
...
function TreeNode926({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode926 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState926 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer926(state, action) {
...
export default function Component926({ seed = 926, title = 'Component926' }) {
  const hook = useLogic376(seed + 926);
  const [state, dispatch] = useReducer(reducer926, initialState926);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm926(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset376(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState376(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree376(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute376(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast376(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies376(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution376(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow376(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets376(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap376(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline376(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm926(form);
    setErrors(found);
  ...
  return (
    <section className="component-926">
      <div className="header-926">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-926">
      ...
      <div className="grid-926">
        <div className="panel-926">
          {cards.map((item, idx) => (
            <MetricCard926 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
