...
import { useLogic426 } from '../hooks/useLogic426.js';
import { buildNestedTree426, buildTableState426, heavyCompute426, transformDataset426 } from '../utils/compute426.js';
import { buildForecast426, detectAnomalies426, segmentDistribution426 } from '../utils/compute426.js';
import { buildActivityMap426, rankBuckets426, summarizePipeline426, synthesizeFlow426 } from '../pipelines/pipeline426.js';
import '../styles/component426.css';

function MetricCard426({ label, value, max }) {
...
function TreeNode426({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode426 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState426 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer426(state, action) {
...
export default function Component426({ seed = 426, title = 'Component426' }) {
  const hook = useLogic426(seed + 426);
  const [state, dispatch] = useReducer(reducer426, initialState426);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm426(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset426(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState426(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree426(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute426(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast426(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies426(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution426(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow426(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets426(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap426(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline426(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm426(form);
    setErrors(found);
  ...
  return (
    <section className="component-426">
      <div className="header-426">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-426">
      ...
      <div className="grid-426">
        <div className="panel-426">
          {cards.map((item, idx) => (
            <MetricCard426 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
