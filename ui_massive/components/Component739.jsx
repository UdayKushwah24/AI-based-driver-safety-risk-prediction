...
import '../styles/component739.css';

function MetricCard739({ label, value, max }) {
...
function TreeNode739({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode739 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState739 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer739(state, action) {
...
export default function Component739({ seed = 739, title = 'Component739' }) {
  const hook = useLogic189(seed + 739);
  const [state, dispatch] = useReducer(reducer739, initialState739);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm739(seed));
  ...
  const table = useMemo(() => buildTableState189(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm739(form);
    setErrors(found);
  ...
  return (
    <section className="component-739">
      <div className="header-739">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-739">
      ...
      <div className="grid-739">
        <div className="panel-739">
          {cards.map((item, idx) => (
            <MetricCard739 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
