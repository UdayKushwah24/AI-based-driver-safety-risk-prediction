...
import '../styles/component101.css';

function MetricCard101({ label, value, max }) {
...
function TreeNode101({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode101 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component101({ seed = 101, title = 'Component101' }) {
  const hook = useLogic101(seed + 101);
  const [state, dispatch] = useReducer(reducer101, initialState101);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm101(seed));
  ...
  const table = useMemo(() => buildTableState101(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm101(form);
    setErrors(found);
  ...
  return (
    <section className="component-101">
      ...
      <div className="grid-101">
        <div className="panel-101">
          {cards.map((item, idx) => (
            <MetricCard101 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
