...
import '../styles/component529.css';

function MetricCard529({ label, value, max }) {
...
function TreeNode529({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode529 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState529 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer529(state, action) {
...
export default function Component529({ seed = 529, title = 'Component529' }) {
  const hook = useLogic529(seed + 529);
  const [state, dispatch] = useReducer(reducer529, initialState529);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm529(seed));
  ...
  const table = useMemo(() => buildTableState529(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm529(form);
    setErrors(found);
  ...
  return (
    <section className="component-529">
      <div className="header-529">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-529">
      ...
      <div className="grid-529">
        <div className="panel-529">
          {cards.map((item, idx) => (
            <MetricCard529 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
