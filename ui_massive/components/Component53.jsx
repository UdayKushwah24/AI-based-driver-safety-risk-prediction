...
import '../styles/component53.css';

function MetricCard53({ label, value, max }) {
...
function TreeNode53({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode53 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component53({ seed = 53, title = 'Component53' }) {
  const hook = useLogic53(seed + 53);
  const [state, dispatch] = useReducer(reducer53, initialState53);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm53(seed));
  ...
  const table = useMemo(() => buildTableState53(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm53(form);
    setErrors(found);
  ...
  return (
    <section className="component-53">
      ...
      <div className="grid-53">
        <div className="panel-53">
          {cards.map((item, idx) => (
            <MetricCard53 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
