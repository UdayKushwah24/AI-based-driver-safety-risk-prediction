...
import { useLogic300 } from '../hooks/useLogic300.js';
import { buildNestedTree300, buildTableState300, heavyCompute300, transformDataset300 } from '../utils/compute300.js';
import { buildForecast300, detectAnomalies300, segmentDistribution300 } from '../utils/compute300.js';
import { buildActivityMap300, rankBuckets300, summarizePipeline300, synthesizeFlow300 } from '../pipelines/pipeline300.js';
import '../styles/component850.css';

function MetricCard850({ label, value, max }) {
...
function TreeNode850({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode850 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState850 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer850(state, action) {
...
export default function Component850({ seed = 850, title = 'Component850' }) {
  const hook = useLogic300(seed + 850);
  const [state, dispatch] = useReducer(reducer850, initialState850);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm850(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset300(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState300(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree300(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute300(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast300(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies300(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution300(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow300(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets300(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap300(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline300(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm850(form);
    setErrors(found);
  ...
  return (
    <section className="component-850">
      <div className="header-850">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-850">
      ...
      <div className="grid-850">
        <div className="panel-850">
          {cards.map((item, idx) => (
            <MetricCard850 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
