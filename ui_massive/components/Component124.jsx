...
import '../styles/component124.css';

function MetricCard124({ label, value, max }) {
...
function TreeNode124({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode124 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component124({ seed = 124, title = 'Component124' }) {
  const hook = useLogic124(seed + 124);
  const [state, dispatch] = useReducer(reducer124, initialState124);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm124(seed));
  ...
  const table = useMemo(() => buildTableState124(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm124(form);
    setErrors(found);
  ...
  return (
    <section className="component-124">
      ...
      <div className="grid-124">
        <div className="panel-124">
          {cards.map((item, idx) => (
            <MetricCard124 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
