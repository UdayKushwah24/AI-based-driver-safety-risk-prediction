...
import '../styles/component36.css';

function MetricCard36({ label, value, max }) {
...
function TreeNode36({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode36 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState36 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer36(state, action) {
...
export default function Component36({ seed = 36, title = 'Component36' }) {
  const hook = useLogic36(seed + 36);
  const [state, dispatch] = useReducer(reducer36, initialState36);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm36(seed));
  ...
  const table = useMemo(() => buildTableState36(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm36(form);
    setErrors(found);
  ...
  return (
    <section className="component-36">
      ...
      <div className="grid-36">
        <div className="panel-36">
          {cards.map((item, idx) => (
            <MetricCard36 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
