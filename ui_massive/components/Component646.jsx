...
import { useLogic96 } from '../hooks/useLogic96.js';
import { buildNestedTree96, buildTableState96, heavyCompute96, transformDataset96 } from '../utils/compute96.js';
import { buildForecast96, detectAnomalies96, segmentDistribution96 } from '../utils/compute96.js';
import { buildActivityMap96, rankBuckets96, summarizePipeline96, synthesizeFlow96 } from '../pipelines/pipeline96.js';
import '../styles/component646.css';

function MetricCard646({ label, value, max }) {
...
function TreeNode646({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode646 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState646 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer646(state, action) {
...
export default function Component646({ seed = 646, title = 'Component646' }) {
  const hook = useLogic96(seed + 646);
  const [state, dispatch] = useReducer(reducer646, initialState646);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm646(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset96(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState96(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree96(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute96(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast96(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies96(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution96(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow96(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets96(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap96(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline96(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm646(form);
    setErrors(found);
  ...
  return (
    <section className="component-646">
      <div className="header-646">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-646">
      ...
      <div className="grid-646">
        <div className="panel-646">
          {cards.map((item, idx) => (
            <MetricCard646 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
