...
import '../styles/component464.css';

function MetricCard464({ label, value, max }) {
...
function TreeNode464({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode464 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component464({ seed = 464, title = 'Component464' }) {
  const hook = useLogic464(seed + 464);
  const [state, dispatch] = useReducer(reducer464, initialState464);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm464(seed));
  ...
  const table = useMemo(() => buildTableState464(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm464(form);
    setErrors(found);
  ...
  return (
    <section className="component-464">
      <div className="header-464">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-464">
      ...
      <div className="grid-464">
        <div className="panel-464">
          {cards.map((item, idx) => (
            <MetricCard464 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
