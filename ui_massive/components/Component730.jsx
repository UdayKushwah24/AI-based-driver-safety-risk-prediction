...
import { useLogic180 } from '../hooks/useLogic180.js';
import { buildNestedTree180, buildTableState180, heavyCompute180, transformDataset180 } from '../utils/compute180.js';
import { buildForecast180, detectAnomalies180, segmentDistribution180 } from '../utils/compute180.js';
import { buildActivityMap180, rankBuckets180, summarizePipeline180, synthesizeFlow180 } from '../pipelines/pipeline180.js';
import '../styles/component730.css';

function MetricCard730({ label, value, max }) {
...
function TreeNode730({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode730 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState730 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer730(state, action) {
...
export default function Component730({ seed = 730, title = 'Component730' }) {
  const hook = useLogic180(seed + 730);
  const [state, dispatch] = useReducer(reducer730, initialState730);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm730(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset180(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState180(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree180(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute180(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast180(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies180(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution180(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow180(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets180(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap180(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline180(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm730(form);
    setErrors(found);
  ...
  return (
    <section className="component-730">
      <div className="header-730">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-730">
      ...
      <div className="grid-730">
        <div className="panel-730">
          {cards.map((item, idx) => (
            <MetricCard730 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
