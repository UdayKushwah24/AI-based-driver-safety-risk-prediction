...
import '../styles/component509.css';

function MetricCard509({ label, value, max }) {
...
function TreeNode509({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode509 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState509 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer509(state, action) {
...
export default function Component509({ seed = 509, title = 'Component509' }) {
  const hook = useLogic509(seed + 509);
  const [state, dispatch] = useReducer(reducer509, initialState509);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm509(seed));
  ...
  const table = useMemo(() => buildTableState509(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm509(form);
    setErrors(found);
  ...
  return (
    <section className="component-509">
      <div className="header-509">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-509">
      ...
      <div className="grid-509">
        <div className="panel-509">
          {cards.map((item, idx) => (
            <MetricCard509 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
