...
import '../styles/component404.css';

function MetricCard404({ label, value, max }) {
...
function TreeNode404({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode404 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState404 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer404(state, action) {
...
export default function Component404({ seed = 404, title = 'Component404' }) {
  const hook = useLogic404(seed + 404);
  const [state, dispatch] = useReducer(reducer404, initialState404);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm404(seed));
  ...
  const table = useMemo(() => buildTableState404(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm404(form);
    setErrors(found);
  ...
  return (
    <section className="component-404">
      ...
      <div className="grid-404">
        <div className="panel-404">
          {cards.map((item, idx) => (
            <MetricCard404 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
