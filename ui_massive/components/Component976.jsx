...
import '../styles/component976.css';

function MetricCard976({ label, value, max }) {
...
function TreeNode976({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode976 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState976 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer976(state, action) {
...
export default function Component976({ seed = 976, title = 'Component976' }) {
  const hook = useLogic426(seed + 976);
  const [state, dispatch] = useReducer(reducer976, initialState976);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm976(seed));
  ...
  const table = useMemo(() => buildTableState426(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm976(form);
    setErrors(found);
  ...
  return (
    <section className="component-976">
      <div className="header-976">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-976">
      ...
      <div className="grid-976">
        <div className="panel-976">
          {cards.map((item, idx) => (
            <MetricCard976 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
