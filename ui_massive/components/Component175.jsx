...
import '../styles/component175.css';

function MetricCard175({ label, value, max }) {
...
function TreeNode175({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode175 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState175 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer175(state, action) {
...
export default function Component175({ seed = 175, title = 'Component175' }) {
  const hook = useLogic175(seed + 175);
  const [state, dispatch] = useReducer(reducer175, initialState175);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm175(seed));
  ...
  const table = useMemo(() => buildTableState175(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm175(form);
    setErrors(found);
  ...
  return (
    <section className="component-175">
      <div className="header-175">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-175">
      ...
      <div className="grid-175">
        <div className="panel-175">
          {cards.map((item, idx) => (
            <MetricCard175 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
