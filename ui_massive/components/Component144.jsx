...
import '../styles/component144.css';

function MetricCard144({ label, value, max }) {
...
function TreeNode144({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode144 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState144 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer144(state, action) {
...
export default function Component144({ seed = 144, title = 'Component144' }) {
  const hook = useLogic144(seed + 144);
  const [state, dispatch] = useReducer(reducer144, initialState144);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm144(seed));
  ...
  const table = useMemo(() => buildTableState144(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm144(form);
    setErrors(found);
  ...
  return (
    <section className="component-144">
      <div className="header-144">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-144">
      ...
      <div className="grid-144">
        <div className="panel-144">
          {cards.map((item, idx) => (
            <MetricCard144 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
