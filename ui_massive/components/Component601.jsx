...
import { useLogic51 } from '../hooks/useLogic51.js';
import { buildNestedTree51, buildTableState51, heavyCompute51, transformDataset51 } from '../utils/compute51.js';
import { buildForecast51, detectAnomalies51, segmentDistribution51 } from '../utils/compute51.js';
import { buildActivityMap51, rankBuckets51, summarizePipeline51, synthesizeFlow51 } from '../pipelines/pipeline51.js';
import '../styles/component601.css';

function MetricCard601({ label, value, max }) {
...
function TreeNode601({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode601 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState601 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer601(state, action) {
...
export default function Component601({ seed = 601, title = 'Component601' }) {
  const hook = useLogic51(seed + 601);
  const [state, dispatch] = useReducer(reducer601, initialState601);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm601(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset51(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState51(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree51(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute51(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast51(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies51(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution51(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow51(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets51(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap51(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline51(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm601(form);
    setErrors(found);
  ...
  return (
    <section className="component-601">
      <div className="header-601">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-601">
      ...
      <div className="grid-601">
        <div className="panel-601">
          {cards.map((item, idx) => (
            <MetricCard601 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
