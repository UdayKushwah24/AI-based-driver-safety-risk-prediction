...
import '../styles/component357.css';

function MetricCard357({ label, value, max }) {
...
function TreeNode357({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode357 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState357 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer357(state, action) {
...
export default function Component357({ seed = 357, title = 'Component357' }) {
  const hook = useLogic357(seed + 357);
  const [state, dispatch] = useReducer(reducer357, initialState357);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm357(seed));
  ...
  const table = useMemo(() => buildTableState357(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm357(form);
    setErrors(found);
  ...
  return (
    <section className="component-357">
      <div className="header-357">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-357">
      ...
      <div className="grid-357">
        <div className="panel-357">
          {cards.map((item, idx) => (
            <MetricCard357 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
