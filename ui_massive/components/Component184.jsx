...
import '../styles/component184.css';

function MetricCard184({ label, value, max }) {
...
function TreeNode184({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode184 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState184 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer184(state, action) {
...
export default function Component184({ seed = 184, title = 'Component184' }) {
  const hook = useLogic184(seed + 184);
  const [state, dispatch] = useReducer(reducer184, initialState184);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm184(seed));
  ...
  const table = useMemo(() => buildTableState184(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm184(form);
    setErrors(found);
  ...
  return (
    <section className="component-184">
      <div className="header-184">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-184">
      ...
      <div className="grid-184">
        <div className="panel-184">
          {cards.map((item, idx) => (
            <MetricCard184 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
