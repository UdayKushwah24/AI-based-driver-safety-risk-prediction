...
import '../styles/component71.css';

function MetricCard71({ label, value, max }) {
...
function TreeNode71({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode71 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component71({ seed = 71, title = 'Component71' }) {
  const hook = useLogic71(seed + 71);
  const [state, dispatch] = useReducer(reducer71, initialState71);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm71(seed));
  ...
  const table = useMemo(() => buildTableState71(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm71(form);
    setErrors(found);
  ...
  return (
    <section className="component-71">
      ...
      <div className="grid-71">
        <div className="panel-71">
          {cards.map((item, idx) => (
            <MetricCard71 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
