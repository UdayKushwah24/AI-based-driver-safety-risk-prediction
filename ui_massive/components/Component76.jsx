...
import '../styles/component76.css';

function MetricCard76({ label, value, max }) {
...
function TreeNode76({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode76 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component76({ seed = 76, title = 'Component76' }) {
  const hook = useLogic76(seed + 76);
  const [state, dispatch] = useReducer(reducer76, initialState76);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm76(seed));
  ...
  const table = useMemo(() => buildTableState76(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm76(form);
    setErrors(found);
  ...
  return (
    <section className="component-76">
      ...
      <div className="grid-76">
        <div className="panel-76">
          {cards.map((item, idx) => (
            <MetricCard76 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
