...
import { useLogic10 } from '../hooks/useLogic10.js';
import { buildNestedTree10, buildTableState10, heavyCompute10, transformDataset10 } from '../utils/compute10.js';
import { buildForecast10, detectAnomalies10, segmentDistribution10 } from '../utils/compute10.js';
import { buildActivityMap10, rankBuckets10, summarizePipeline10, synthesizeFlow10 } from '../pipelines/pipeline10.js';
import '../styles/component560.css';

function MetricCard560({ label, value, max }) {
...
function TreeNode560({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode560 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState560 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer560(state, action) {
...
export default function Component560({ seed = 560, title = 'Component560' }) {
  const hook = useLogic10(seed + 560);
  const [state, dispatch] = useReducer(reducer560, initialState560);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm560(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset10(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState10(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree10(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute10(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast10(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies10(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution10(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow10(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets10(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap10(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline10(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm560(form);
    setErrors(found);
  ...
  return (
    <section className="component-560">
      <div className="header-560">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-560">
      ...
      <div className="grid-560">
        <div className="panel-560">
          {cards.map((item, idx) => (
            <MetricCard560 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
