...
import '../styles/component901.css';

function MetricCard901({ label, value, max }) {
...
function TreeNode901({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode901 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState901 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer901(state, action) {
...
export default function Component901({ seed = 901, title = 'Component901' }) {
  const hook = useLogic351(seed + 901);
  const [state, dispatch] = useReducer(reducer901, initialState901);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm901(seed));
  ...
  const table = useMemo(() => buildTableState351(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm901(form);
    setErrors(found);
  ...
  return (
    <section className="component-901">
      <div className="header-901">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-901">
      ...
      <div className="grid-901">
        <div className="panel-901">
          {cards.map((item, idx) => (
            <MetricCard901 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
