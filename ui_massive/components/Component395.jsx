...
import '../styles/component395.css';

function MetricCard395({ label, value, max }) {
...
function TreeNode395({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode395 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState395 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer395(state, action) {
...
export default function Component395({ seed = 395, title = 'Component395' }) {
  const hook = useLogic395(seed + 395);
  const [state, dispatch] = useReducer(reducer395, initialState395);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm395(seed));
  ...
  const table = useMemo(() => buildTableState395(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm395(form);
    setErrors(found);
  ...
  return (
    <section className="component-395">
      <div className="header-395">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-395">
      ...
      <div className="grid-395">
        <div className="panel-395">
          {cards.map((item, idx) => (
            <MetricCard395 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
