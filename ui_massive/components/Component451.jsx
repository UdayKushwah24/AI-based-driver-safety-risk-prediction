...
import '../styles/component451.css';

function MetricCard451({ label, value, max }) {
...
function TreeNode451({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode451 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component451({ seed = 451, title = 'Component451' }) {
  const hook = useLogic451(seed + 451);
  const [state, dispatch] = useReducer(reducer451, initialState451);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm451(seed));
  ...
  const table = useMemo(() => buildTableState451(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm451(form);
    setErrors(found);
  ...
  return (
    <section className="component-451">
      <div className="header-451">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-451">
      ...
      <div className="grid-451">
        <div className="panel-451">
          {cards.map((item, idx) => (
            <MetricCard451 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
