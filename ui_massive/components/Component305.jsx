...
import '../styles/component305.css';

function MetricCard305({ label, value, max }) {
...
function TreeNode305({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode305 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState305 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer305(state, action) {
...
export default function Component305({ seed = 305, title = 'Component305' }) {
  const hook = useLogic305(seed + 305);
  const [state, dispatch] = useReducer(reducer305, initialState305);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm305(seed));
  ...
  const table = useMemo(() => buildTableState305(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm305(form);
    setErrors(found);
  ...
  return (
    <section className="component-305">
      <div className="header-305">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-305">
      ...
      <div className="grid-305">
        <div className="panel-305">
          {cards.map((item, idx) => (
            <MetricCard305 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
