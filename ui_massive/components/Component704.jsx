...
import '../styles/component704.css';

function MetricCard704({ label, value, max }) {
...
function TreeNode704({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode704 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState704 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer704(state, action) {
...
export default function Component704({ seed = 704, title = 'Component704' }) {
  const hook = useLogic154(seed + 704);
  const [state, dispatch] = useReducer(reducer704, initialState704);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm704(seed));
  ...
  const table = useMemo(() => buildTableState154(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm704(form);
    setErrors(found);
  ...
  return (
    <section className="component-704">
      <div className="header-704">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-704">
      ...
      <div className="grid-704">
        <div className="panel-704">
          {cards.map((item, idx) => (
            <MetricCard704 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
