...
import { useLogic6 } from '../hooks/useLogic6.js';
import { buildNestedTree6, buildTableState6, heavyCompute6, transformDataset6 } from '../utils/compute6.js';
import { buildForecast6, detectAnomalies6, segmentDistribution6 } from '../utils/compute6.js';
import { buildActivityMap6, rankBuckets6, summarizePipeline6, synthesizeFlow6 } from '../pipelines/pipeline6.js';
import '../styles/component556.css';

function MetricCard556({ label, value, max }) {
...
function TreeNode556({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode556 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState556 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer556(state, action) {
...
export default function Component556({ seed = 556, title = 'Component556' }) {
  const hook = useLogic6(seed + 556);
  const [state, dispatch] = useReducer(reducer556, initialState556);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm556(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset6(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState6(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree6(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute6(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast6(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies6(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution6(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow6(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets6(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap6(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline6(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm556(form);
    setErrors(found);
  ...
  return (
    <section className="component-556">
      <div className="header-556">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-556">
      ...
      <div className="grid-556">
        <div className="panel-556">
          {cards.map((item, idx) => (
            <MetricCard556 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
