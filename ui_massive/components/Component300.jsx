...
import '../styles/component300.css';

function MetricCard300({ label, value, max }) {
...
function TreeNode300({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode300 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState300 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer300(state, action) {
...
export default function Component300({ seed = 300, title = 'Component300' }) {
  const hook = useLogic300(seed + 300);
  const [state, dispatch] = useReducer(reducer300, initialState300);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm300(seed));
  ...
  const table = useMemo(() => buildTableState300(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm300(form);
    setErrors(found);
  ...
  return (
    <section className="component-300">
      <div className="header-300">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-300">
      ...
      <div className="grid-300">
        <div className="panel-300">
          {cards.map((item, idx) => (
            <MetricCard300 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
