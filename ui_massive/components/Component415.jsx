...
import '../styles/component415.css';

function MetricCard415({ label, value, max }) {
...
function TreeNode415({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode415 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState415 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer415(state, action) {
...
export default function Component415({ seed = 415, title = 'Component415' }) {
  const hook = useLogic415(seed + 415);
  const [state, dispatch] = useReducer(reducer415, initialState415);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm415(seed));
  ...
  const table = useMemo(() => buildTableState415(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm415(form);
    setErrors(found);
  ...
  return (
    <section className="component-415">
      <div className="header-415">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-415">
      ...
      <div className="grid-415">
        <div className="panel-415">
          {cards.map((item, idx) => (
            <MetricCard415 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
