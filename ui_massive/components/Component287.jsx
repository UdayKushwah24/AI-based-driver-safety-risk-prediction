...
import '../styles/component287.css';

function MetricCard287({ label, value, max }) {
...
function TreeNode287({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode287 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState287 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer287(state, action) {
...
export default function Component287({ seed = 287, title = 'Component287' }) {
  const hook = useLogic287(seed + 287);
  const [state, dispatch] = useReducer(reducer287, initialState287);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm287(seed));
  ...
  const table = useMemo(() => buildTableState287(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm287(form);
    setErrors(found);
  ...
  return (
    <section className="component-287">
      <div className="header-287">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-287">
      ...
      <div className="grid-287">
        <div className="panel-287">
          {cards.map((item, idx) => (
            <MetricCard287 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
