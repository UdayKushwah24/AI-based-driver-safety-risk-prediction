...
import '../styles/component35.css';

function MetricCard35({ label, value, max }) {
...
function TreeNode35({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode35 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component35({ seed = 35, title = 'Component35' }) {
  const hook = useLogic35(seed + 35);
  const [state, dispatch] = useReducer(reducer35, initialState35);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm35(seed));
  ...
  const table = useMemo(() => buildTableState35(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm35(form);
    setErrors(found);
  ...
  return (
    <section className="component-35">
      ...
      <div className="grid-35">
        <div className="panel-35">
          {cards.map((item, idx) => (
            <MetricCard35 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
