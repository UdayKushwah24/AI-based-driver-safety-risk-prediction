...
import '../styles/component341.css';

function MetricCard341({ label, value, max }) {
...
function TreeNode341({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode341 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState341 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer341(state, action) {
...
export default function Component341({ seed = 341, title = 'Component341' }) {
  const hook = useLogic341(seed + 341);
  const [state, dispatch] = useReducer(reducer341, initialState341);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm341(seed));
  ...
  const table = useMemo(() => buildTableState341(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm341(form);
    setErrors(found);
  ...
  return (
    <section className="component-341">
      <div className="header-341">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-341">
      ...
      <div className="grid-341">
        <div className="panel-341">
          {cards.map((item, idx) => (
            <MetricCard341 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
