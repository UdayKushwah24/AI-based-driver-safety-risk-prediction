...
import '../styles/component369.css';

function MetricCard369({ label, value, max }) {
...
function TreeNode369({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode369 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState369 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer369(state, action) {
...
export default function Component369({ seed = 369, title = 'Component369' }) {
  const hook = useLogic369(seed + 369);
  const [state, dispatch] = useReducer(reducer369, initialState369);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm369(seed));
  ...
  const table = useMemo(() => buildTableState369(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm369(form);
    setErrors(found);
  ...
  return (
    <section className="component-369">
      <div className="header-369">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-369">
      ...
      <div className="grid-369">
        <div className="panel-369">
          {cards.map((item, idx) => (
            <MetricCard369 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
