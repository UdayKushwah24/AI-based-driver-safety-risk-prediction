...
import '../styles/component752.css';

function MetricCard752({ label, value, max }) {
...
function TreeNode752({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode752 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState752 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer752(state, action) {
...
export default function Component752({ seed = 752, title = 'Component752' }) {
  const hook = useLogic202(seed + 752);
  const [state, dispatch] = useReducer(reducer752, initialState752);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm752(seed));
  ...
  const table = useMemo(() => buildTableState202(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm752(form);
    setErrors(found);
  ...
  return (
    <section className="component-752">
      <div className="header-752">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-752">
      ...
      <div className="grid-752">
        <div className="panel-752">
          {cards.map((item, idx) => (
            <MetricCard752 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
