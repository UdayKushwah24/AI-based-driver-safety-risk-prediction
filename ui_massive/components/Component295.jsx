...
import '../styles/component295.css';

function MetricCard295({ label, value, max }) {
...
function TreeNode295({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode295 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState295 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer295(state, action) {
...
export default function Component295({ seed = 295, title = 'Component295' }) {
  const hook = useLogic295(seed + 295);
  const [state, dispatch] = useReducer(reducer295, initialState295);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm295(seed));
  ...
  const table = useMemo(() => buildTableState295(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm295(form);
    setErrors(found);
  ...
  return (
    <section className="component-295">
      <div className="header-295">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-295">
      ...
      <div className="grid-295">
        <div className="panel-295">
          {cards.map((item, idx) => (
            <MetricCard295 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
