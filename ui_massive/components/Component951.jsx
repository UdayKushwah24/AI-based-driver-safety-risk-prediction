...
import { useLogic401 } from '../hooks/useLogic401.js';
import { buildNestedTree401, buildTableState401, heavyCompute401, transformDataset401 } from '../utils/compute401.js';
import { buildForecast401, detectAnomalies401, segmentDistribution401 } from '../utils/compute401.js';
import { buildActivityMap401, rankBuckets401, summarizePipeline401, synthesizeFlow401 } from '../pipelines/pipeline401.js';
import '../styles/component951.css';

function MetricCard951({ label, value, max }) {
...
function TreeNode951({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode951 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState951 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer951(state, action) {
...
export default function Component951({ seed = 951, title = 'Component951' }) {
  const hook = useLogic401(seed + 951);
  const [state, dispatch] = useReducer(reducer951, initialState951);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm951(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset401(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState401(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree401(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute401(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast401(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies401(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution401(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow401(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets401(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap401(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline401(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm951(form);
    setErrors(found);
  ...
  return (
    <section className="component-951">
      <div className="header-951">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-951">
      ...
      <div className="grid-951">
        <div className="panel-951">
          {cards.map((item, idx) => (
            <MetricCard951 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
