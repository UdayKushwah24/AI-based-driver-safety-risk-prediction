...
import { useLogic50 } from '../hooks/useLogic50.js';
import { buildNestedTree50, buildTableState50, heavyCompute50, transformDataset50 } from '../utils/compute50.js';
import { buildForecast50, detectAnomalies50, segmentDistribution50 } from '../utils/compute50.js';
import { buildActivityMap50, rankBuckets50, summarizePipeline50, synthesizeFlow50 } from '../pipelines/pipeline50.js';
import '../styles/component600.css';

function MetricCard600({ label, value, max }) {
...
function TreeNode600({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode600 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState600 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer600(state, action) {
...
export default function Component600({ seed = 600, title = 'Component600' }) {
  const hook = useLogic50(seed + 600);
  const [state, dispatch] = useReducer(reducer600, initialState600);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm600(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset50(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState50(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree50(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute50(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast50(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies50(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution50(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow50(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets50(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap50(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline50(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm600(form);
    setErrors(found);
  ...
  return (
    <section className="component-600">
      ...
      <div className="grid-600">
        <div className="panel-600">
          {cards.map((item, idx) => (
            <MetricCard600 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
