...
import '../styles/component465.css';

function MetricCard465({ label, value, max }) {
...
function TreeNode465({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode465 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState465 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer465(state, action) {
...
export default function Component465({ seed = 465, title = 'Component465' }) {
  const hook = useLogic465(seed + 465);
  const [state, dispatch] = useReducer(reducer465, initialState465);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm465(seed));
  ...
  const table = useMemo(() => buildTableState465(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm465(form);
    setErrors(found);
  ...
  return (
    <section className="component-465">
      <div className="header-465">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-465">
      ...
      <div className="grid-465">
        <div className="panel-465">
          {cards.map((item, idx) => (
            <MetricCard465 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
