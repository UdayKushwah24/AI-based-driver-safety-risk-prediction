...
import '../styles/component896.css';

function MetricCard896({ label, value, max }) {
...
function TreeNode896({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode896 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState896 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer896(state, action) {
...
export default function Component896({ seed = 896, title = 'Component896' }) {
  const hook = useLogic346(seed + 896);
  const [state, dispatch] = useReducer(reducer896, initialState896);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm896(seed));
  ...
  const table = useMemo(() => buildTableState346(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm896(form);
    setErrors(found);
  ...
  return (
    <section className="component-896">
      <div className="header-896">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-896">
      ...
      <div className="grid-896">
        <div className="panel-896">
          {cards.map((item, idx) => (
            <MetricCard896 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
