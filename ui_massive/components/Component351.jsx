...
import { useLogic351 } from '../hooks/useLogic351.js';
import { buildNestedTree351, buildTableState351, heavyCompute351, transformDataset351 } from '../utils/compute351.js';
import { buildForecast351, detectAnomalies351, segmentDistribution351 } from '../utils/compute351.js';
import { buildActivityMap351, rankBuckets351, summarizePipeline351, synthesizeFlow351 } from '../pipelines/pipeline351.js';
import '../styles/component351.css';

function MetricCard351({ label, value, max }) {
...
function TreeNode351({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode351 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState351 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer351(state, action) {
...
export default function Component351({ seed = 351, title = 'Component351' }) {
  const hook = useLogic351(seed + 351);
  const [state, dispatch] = useReducer(reducer351, initialState351);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm351(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset351(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState351(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree351(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute351(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast351(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies351(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution351(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow351(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets351(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap351(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline351(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm351(form);
    setErrors(found);
  ...
  return (
    <section className="component-351">
      <div className="header-351">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-351">
      ...
      <div className="grid-351">
        <div className="panel-351">
          {cards.map((item, idx) => (
            <MetricCard351 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
