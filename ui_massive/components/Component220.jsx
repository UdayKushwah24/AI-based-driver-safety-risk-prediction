...
import '../styles/component220.css';

function MetricCard220({ label, value, max }) {
...
function TreeNode220({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode220 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState220 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer220(state, action) {
...
export default function Component220({ seed = 220, title = 'Component220' }) {
  const hook = useLogic220(seed + 220);
  const [state, dispatch] = useReducer(reducer220, initialState220);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm220(seed));
  ...
  const table = useMemo(() => buildTableState220(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm220(form);
    setErrors(found);
  ...
  return (
    <section className="component-220">
      <div className="header-220">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-220">
      ...
      <div className="grid-220">
        <div className="panel-220">
          {cards.map((item, idx) => (
            <MetricCard220 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
