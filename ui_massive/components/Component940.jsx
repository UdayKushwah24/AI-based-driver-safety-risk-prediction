...
import '../styles/component940.css';

function MetricCard940({ label, value, max }) {
...
function TreeNode940({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode940 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState940 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer940(state, action) {
...
export default function Component940({ seed = 940, title = 'Component940' }) {
  const hook = useLogic390(seed + 940);
  const [state, dispatch] = useReducer(reducer940, initialState940);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm940(seed));
  ...
  const table = useMemo(() => buildTableState390(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm940(form);
    setErrors(found);
  ...
  return (
    <section className="component-940">
      <div className="header-940">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-940">
      ...
      <div className="grid-940">
        <div className="panel-940">
          {cards.map((item, idx) => (
            <MetricCard940 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
