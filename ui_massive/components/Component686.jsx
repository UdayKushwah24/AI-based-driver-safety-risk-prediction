...
import { useLogic136 } from '../hooks/useLogic136.js';
import { buildNestedTree136, buildTableState136, heavyCompute136, transformDataset136 } from '../utils/compute136.js';
import { buildForecast136, detectAnomalies136, segmentDistribution136 } from '../utils/compute136.js';
import { buildActivityMap136, rankBuckets136, summarizePipeline136, synthesizeFlow136 } from '../pipelines/pipeline136.js';
import '../styles/component686.css';

function MetricCard686({ label, value, max }) {
...
function TreeNode686({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode686 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState686 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer686(state, action) {
...
export default function Component686({ seed = 686, title = 'Component686' }) {
  const hook = useLogic136(seed + 686);
  const [state, dispatch] = useReducer(reducer686, initialState686);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm686(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset136(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState136(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree136(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute136(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast136(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies136(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution136(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow136(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets136(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap136(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline136(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm686(form);
    setErrors(found);
  ...
  return (
    <section className="component-686">
      <div className="header-686">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-686">
      ...
      <div className="grid-686">
        <div className="panel-686">
          {cards.map((item, idx) => (
            <MetricCard686 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
