...
import '../styles/component967.css';

function MetricCard967({ label, value, max }) {
...
function TreeNode967({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode967 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState967 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer967(state, action) {
...
export default function Component967({ seed = 967, title = 'Component967' }) {
  const hook = useLogic417(seed + 967);
  const [state, dispatch] = useReducer(reducer967, initialState967);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm967(seed));
  ...
  const table = useMemo(() => buildTableState417(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm967(form);
    setErrors(found);
  ...
  return (
    <section className="component-967">
      <div className="header-967">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-967">
      ...
      <div className="grid-967">
        <div className="panel-967">
          {cards.map((item, idx) => (
            <MetricCard967 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
