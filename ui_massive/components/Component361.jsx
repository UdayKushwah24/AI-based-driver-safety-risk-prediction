...
import { useLogic361 } from '../hooks/useLogic361.js';
import { buildNestedTree361, buildTableState361, heavyCompute361, transformDataset361 } from '../utils/compute361.js';
import { buildForecast361, detectAnomalies361, segmentDistribution361 } from '../utils/compute361.js';
import { buildActivityMap361, rankBuckets361, summarizePipeline361, synthesizeFlow361 } from '../pipelines/pipeline361.js';
import '../styles/component361.css';

function MetricCard361({ label, value, max }) {
...
function TreeNode361({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode361 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState361 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer361(state, action) {
...
export default function Component361({ seed = 361, title = 'Component361' }) {
  const hook = useLogic361(seed + 361);
  const [state, dispatch] = useReducer(reducer361, initialState361);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm361(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset361(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState361(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree361(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute361(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast361(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies361(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution361(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow361(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets361(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap361(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline361(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm361(form);
    setErrors(found);
  ...
  return (
    <section className="component-361">
      <div className="header-361">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-361">
      ...
      <div className="grid-361">
        <div className="panel-361">
          {cards.map((item, idx) => (
            <MetricCard361 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
