...
import '../styles/component505.css';

function MetricCard505({ label, value, max }) {
...
function TreeNode505({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode505 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState505 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer505(state, action) {
...
export default function Component505({ seed = 505, title = 'Component505' }) {
  const hook = useLogic505(seed + 505);
  const [state, dispatch] = useReducer(reducer505, initialState505);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm505(seed));
  ...
  const table = useMemo(() => buildTableState505(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm505(form);
    setErrors(found);
  ...
  return (
    <section className="component-505">
      <div className="header-505">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-505">
      ...
      <div className="grid-505">
        <div className="panel-505">
          {cards.map((item, idx) => (
            <MetricCard505 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
