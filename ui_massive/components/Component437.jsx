...
import '../styles/component437.css';

function MetricCard437({ label, value, max }) {
...
function TreeNode437({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode437 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState437 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer437(state, action) {
...
export default function Component437({ seed = 437, title = 'Component437' }) {
  const hook = useLogic437(seed + 437);
  const [state, dispatch] = useReducer(reducer437, initialState437);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm437(seed));
  ...
  const table = useMemo(() => buildTableState437(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm437(form);
    setErrors(found);
  ...
  return (
    <section className="component-437">
      <div className="header-437">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-437">
      ...
      <div className="grid-437">
        <div className="panel-437">
          {cards.map((item, idx) => (
            <MetricCard437 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
