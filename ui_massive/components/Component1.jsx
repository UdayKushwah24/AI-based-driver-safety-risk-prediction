...
function MetricCard1({ label, value, max }) {
...
function TreeNode1({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode1 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component1({ seed = 1, title = 'Component1' }) {
  const hook = useLogic1(seed + 1);
  const [state, dispatch] = useReducer(reducer1, initialState1);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm1(seed));
  ...
  const table = useMemo(() => buildTableState1(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm1(form);
    setErrors(found);
    ...
    <section className="component-1">
      ...
      <div className="grid-1">
        <div className="panel-1">
          {cards.map((item, idx) => (
            <MetricCard1 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
