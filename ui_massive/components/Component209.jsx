...
import '../styles/component209.css';

function MetricCard209({ label, value, max }) {
...
function TreeNode209({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode209 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState209 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer209(state, action) {
...
export default function Component209({ seed = 209, title = 'Component209' }) {
  const hook = useLogic209(seed + 209);
  const [state, dispatch] = useReducer(reducer209, initialState209);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm209(seed));
  ...
  const table = useMemo(() => buildTableState209(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm209(form);
    setErrors(found);
  ...
  return (
    <section className="component-209">
      <div className="header-209">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-209">
      ...
      <div className="grid-209">
        <div className="panel-209">
          {cards.map((item, idx) => (
            <MetricCard209 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
