...
import { useLogic374 } from '../hooks/useLogic374.js';
import { buildNestedTree374, buildTableState374, heavyCompute374, transformDataset374 } from '../utils/compute374.js';
import { buildForecast374, detectAnomalies374, segmentDistribution374 } from '../utils/compute374.js';
import { buildActivityMap374, rankBuckets374, summarizePipeline374, synthesizeFlow374 } from '../pipelines/pipeline374.js';
import '../styles/component374.css';

function MetricCard374({ label, value, max }) {
...
function TreeNode374({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode374 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState374 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer374(state, action) {
...
export default function Component374({ seed = 374, title = 'Component374' }) {
  const hook = useLogic374(seed + 374);
  const [state, dispatch] = useReducer(reducer374, initialState374);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm374(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset374(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState374(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree374(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute374(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast374(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies374(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution374(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow374(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets374(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap374(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline374(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm374(form);
    setErrors(found);
  ...
  return (
    <section className="component-374">
      <div className="header-374">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-374">
      ...
      <div className="grid-374">
        <div className="panel-374">
          {cards.map((item, idx) => (
            <MetricCard374 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
