...
import { useLogic405 } from '../hooks/useLogic405.js';
import { buildNestedTree405, buildTableState405, heavyCompute405, transformDataset405 } from '../utils/compute405.js';
import { buildForecast405, detectAnomalies405, segmentDistribution405 } from '../utils/compute405.js';
import { buildActivityMap405, rankBuckets405, summarizePipeline405, synthesizeFlow405 } from '../pipelines/pipeline405.js';
import '../styles/component955.css';

function MetricCard955({ label, value, max }) {
...
function TreeNode955({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode955 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState955 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer955(state, action) {
...
export default function Component955({ seed = 955, title = 'Component955' }) {
  const hook = useLogic405(seed + 955);
  const [state, dispatch] = useReducer(reducer955, initialState955);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm955(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset405(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState405(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree405(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute405(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast405(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies405(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution405(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow405(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets405(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap405(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline405(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm955(form);
    setErrors(found);
  ...
  return (
    <section className="component-955">
      <div className="header-955">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-955">
      ...
      <div className="grid-955">
        <div className="panel-955">
          {cards.map((item, idx) => (
            <MetricCard955 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
