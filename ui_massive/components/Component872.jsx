...
import '../styles/component872.css';

function MetricCard872({ label, value, max }) {
...
function TreeNode872({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode872 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState872 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer872(state, action) {
...
export default function Component872({ seed = 872, title = 'Component872' }) {
  const hook = useLogic322(seed + 872);
  const [state, dispatch] = useReducer(reducer872, initialState872);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm872(seed));
  ...
  const table = useMemo(() => buildTableState322(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm872(form);
    setErrors(found);
  ...
  return (
    <section className="component-872">
      <div className="header-872">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-872">
      ...
      <div className="grid-872">
        <div className="panel-872">
          {cards.map((item, idx) => (
            <MetricCard872 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
