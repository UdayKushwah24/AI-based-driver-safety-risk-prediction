...
import { useLogic341 } from '../hooks/useLogic341.js';
import { buildNestedTree341, buildTableState341, heavyCompute341, transformDataset341 } from '../utils/compute341.js';
import { buildForecast341, detectAnomalies341, segmentDistribution341 } from '../utils/compute341.js';
import { buildActivityMap341, rankBuckets341, summarizePipeline341, synthesizeFlow341 } from '../pipelines/pipeline341.js';
import '../styles/component891.css';

function MetricCard891({ label, value, max }) {
...
function TreeNode891({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode891 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState891 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer891(state, action) {
...
export default function Component891({ seed = 891, title = 'Component891' }) {
  const hook = useLogic341(seed + 891);
  const [state, dispatch] = useReducer(reducer891, initialState891);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm891(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset341(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState341(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree341(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute341(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast341(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies341(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution341(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow341(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets341(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap341(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline341(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm891(form);
    setErrors(found);
  ...
  return (
    <section className="component-891">
      <div className="header-891">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-891">
      ...
      <div className="grid-891">
        <div className="panel-891">
          {cards.map((item, idx) => (
            <MetricCard891 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
