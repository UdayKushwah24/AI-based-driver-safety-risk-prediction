...
import '../styles/component107.css';

function MetricCard107({ label, value, max }) {
...
function TreeNode107({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode107 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component107({ seed = 107, title = 'Component107' }) {
  const hook = useLogic107(seed + 107);
  const [state, dispatch] = useReducer(reducer107, initialState107);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm107(seed));
  ...
  const table = useMemo(() => buildTableState107(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm107(form);
    setErrors(found);
  ...
  return (
    <section className="component-107">
      ...
      <div className="grid-107">
        <div className="panel-107">
          {cards.map((item, idx) => (
            <MetricCard107 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
