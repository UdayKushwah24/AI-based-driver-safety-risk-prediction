...
import '../styles/component214.css';

function MetricCard214({ label, value, max }) {
...
function TreeNode214({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode214 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState214 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer214(state, action) {
...
export default function Component214({ seed = 214, title = 'Component214' }) {
  const hook = useLogic214(seed + 214);
  const [state, dispatch] = useReducer(reducer214, initialState214);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm214(seed));
  ...
  const table = useMemo(() => buildTableState214(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm214(form);
    setErrors(found);
  ...
  return (
    <section className="component-214">
      ...
      <div className="grid-214">
        <div className="panel-214">
          {cards.map((item, idx) => (
            <MetricCard214 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
