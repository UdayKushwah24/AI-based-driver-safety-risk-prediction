...
import '../styles/component444.css';

function MetricCard444({ label, value, max }) {
...
function TreeNode444({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode444 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState444 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer444(state, action) {
...
export default function Component444({ seed = 444, title = 'Component444' }) {
  const hook = useLogic444(seed + 444);
  const [state, dispatch] = useReducer(reducer444, initialState444);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm444(seed));
  ...
  const table = useMemo(() => buildTableState444(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm444(form);
    setErrors(found);
  ...
  return (
    <section className="component-444">
      <div className="header-444">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-444">
      ...
      <div className="grid-444">
        <div className="panel-444">
          {cards.map((item, idx) => (
            <MetricCard444 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
