...
import { useLogic58 } from '../hooks/useLogic58.js';
import { buildNestedTree58, buildTableState58, heavyCompute58, transformDataset58 } from '../utils/compute58.js';
import { buildForecast58, detectAnomalies58, segmentDistribution58 } from '../utils/compute58.js';
import { buildActivityMap58, rankBuckets58, summarizePipeline58, synthesizeFlow58 } from '../pipelines/pipeline58.js';
import '../styles/component608.css';

function MetricCard608({ label, value, max }) {
...
function TreeNode608({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode608 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState608 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer608(state, action) {
...
export default function Component608({ seed = 608, title = 'Component608' }) {
  const hook = useLogic58(seed + 608);
  const [state, dispatch] = useReducer(reducer608, initialState608);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm608(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset58(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState58(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree58(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute58(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast58(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies58(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution58(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow58(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets58(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap58(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline58(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm608(form);
    setErrors(found);
  ...
  return (
    <section className="component-608">
      <div className="header-608">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-608">
      ...
      <div className="grid-608">
        <div className="panel-608">
          {cards.map((item, idx) => (
            <MetricCard608 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
