...
import { useLogic147 } from '../hooks/useLogic147.js';
import { buildNestedTree147, buildTableState147, heavyCompute147, transformDataset147 } from '../utils/compute147.js';
import { buildForecast147, detectAnomalies147, segmentDistribution147 } from '../utils/compute147.js';
import { buildActivityMap147, rankBuckets147, summarizePipeline147, synthesizeFlow147 } from '../pipelines/pipeline147.js';
import '../styles/component147.css';

function MetricCard147({ label, value, max }) {
...
function TreeNode147({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode147 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState147 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer147(state, action) {
...
export default function Component147({ seed = 147, title = 'Component147' }) {
  const hook = useLogic147(seed + 147);
  const [state, dispatch] = useReducer(reducer147, initialState147);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm147(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset147(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState147(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree147(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute147(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast147(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies147(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution147(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow147(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets147(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap147(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline147(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm147(form);
    setErrors(found);
  ...
  return (
    <section className="component-147">
      <div className="header-147">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-147">
      ...
      <div className="grid-147">
        <div className="panel-147">
          {cards.map((item, idx) => (
            <MetricCard147 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
