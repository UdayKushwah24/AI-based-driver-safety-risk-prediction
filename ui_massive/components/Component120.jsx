...
import '../styles/component120.css';

function MetricCard120({ label, value, max }) {
...
function TreeNode120({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode120 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState120 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer120(state, action) {
...
export default function Component120({ seed = 120, title = 'Component120' }) {
  const hook = useLogic120(seed + 120);
  const [state, dispatch] = useReducer(reducer120, initialState120);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm120(seed));
  ...
  const table = useMemo(() => buildTableState120(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm120(form);
    setErrors(found);
  ...
  return (
    <section className="component-120">
      <div className="header-120">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-120">
      ...
      <div className="grid-120">
        <div className="panel-120">
          {cards.map((item, idx) => (
            <MetricCard120 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
