...
import '../styles/component33.css';

function MetricCard33({ label, value, max }) {
...
function TreeNode33({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode33 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component33({ seed = 33, title = 'Component33' }) {
  const hook = useLogic33(seed + 33);
  const [state, dispatch] = useReducer(reducer33, initialState33);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm33(seed));
  ...
  const table = useMemo(() => buildTableState33(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm33(form);
    setErrors(found);
  ...
  return (
    <section className="component-33">
      ...
      <div className="grid-33">
        <div className="panel-33">
          {cards.map((item, idx) => (
            <MetricCard33 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
