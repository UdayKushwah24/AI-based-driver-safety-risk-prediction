...
import { useLogic166 } from '../hooks/useLogic166.js';
import { buildNestedTree166, buildTableState166, heavyCompute166, transformDataset166 } from '../utils/compute166.js';
import { buildForecast166, detectAnomalies166, segmentDistribution166 } from '../utils/compute166.js';
import { buildActivityMap166, rankBuckets166, summarizePipeline166, synthesizeFlow166 } from '../pipelines/pipeline166.js';
import '../styles/component166.css';

function MetricCard166({ label, value, max }) {
...
function TreeNode166({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode166 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState166 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer166(state, action) {
...
export default function Component166({ seed = 166, title = 'Component166' }) {
  const hook = useLogic166(seed + 166);
  const [state, dispatch] = useReducer(reducer166, initialState166);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm166(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset166(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState166(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree166(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute166(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast166(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies166(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution166(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow166(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets166(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap166(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline166(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm166(form);
    setErrors(found);
  ...
  return (
    <section className="component-166">
      <div className="header-166">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-166">
      ...
      <div className="grid-166">
        <div className="panel-166">
          {cards.map((item, idx) => (
            <MetricCard166 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
