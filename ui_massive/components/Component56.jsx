...
import '../styles/component56.css';

function MetricCard56({ label, value, max }) {
...
function TreeNode56({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode56 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component56({ seed = 56, title = 'Component56' }) {
  const hook = useLogic56(seed + 56);
  const [state, dispatch] = useReducer(reducer56, initialState56);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm56(seed));
  ...
  const table = useMemo(() => buildTableState56(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm56(form);
    setErrors(found);
  ...
  return (
    <section className="component-56">
      ...
      <div className="grid-56">
        <div className="panel-56">
          {cards.map((item, idx) => (
            <MetricCard56 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
