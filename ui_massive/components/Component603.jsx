...
import { useLogic53 } from '../hooks/useLogic53.js';
import { buildNestedTree53, buildTableState53, heavyCompute53, transformDataset53 } from '../utils/compute53.js';
import { buildForecast53, detectAnomalies53, segmentDistribution53 } from '../utils/compute53.js';
import { buildActivityMap53, rankBuckets53, summarizePipeline53, synthesizeFlow53 } from '../pipelines/pipeline53.js';
import '../styles/component603.css';

function MetricCard603({ label, value, max }) {
...
function TreeNode603({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode603 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState603 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer603(state, action) {
...
export default function Component603({ seed = 603, title = 'Component603' }) {
  const hook = useLogic53(seed + 603);
  const [state, dispatch] = useReducer(reducer603, initialState603);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm603(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset53(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState53(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree53(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute53(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast53(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies53(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution53(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow53(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets53(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap53(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline53(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm603(form);
    setErrors(found);
  ...
  return (
    <section className="component-603">
      <div className="header-603">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-603">
      ...
      <div className="grid-603">
        <div className="panel-603">
          {cards.map((item, idx) => (
            <MetricCard603 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
