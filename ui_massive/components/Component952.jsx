...
import '../styles/component952.css';

function MetricCard952({ label, value, max }) {
...
function TreeNode952({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode952 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState952 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer952(state, action) {
...
export default function Component952({ seed = 952, title = 'Component952' }) {
  const hook = useLogic402(seed + 952);
  const [state, dispatch] = useReducer(reducer952, initialState952);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm952(seed));
  ...
  const table = useMemo(() => buildTableState402(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm952(form);
    setErrors(found);
  ...
  return (
    <section className="component-952">
      <div className="header-952">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-952">
      ...
      <div className="grid-952">
        <div className="panel-952">
          {cards.map((item, idx) => (
            <MetricCard952 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
