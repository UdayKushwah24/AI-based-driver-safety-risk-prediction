...
import '../styles/component55.css';

function MetricCard55({ label, value, max }) {
...
function TreeNode55({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode55 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component55({ seed = 55, title = 'Component55' }) {
  const hook = useLogic55(seed + 55);
  const [state, dispatch] = useReducer(reducer55, initialState55);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm55(seed));
  ...
  const table = useMemo(() => buildTableState55(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm55(form);
    setErrors(found);
  ...
  return (
    <section className="component-55">
      ...
      <div className="grid-55">
        <div className="panel-55">
          {cards.map((item, idx) => (
            <MetricCard55 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
