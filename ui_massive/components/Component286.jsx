...
import { useLogic286 } from '../hooks/useLogic286.js';
import { buildNestedTree286, buildTableState286, heavyCompute286, transformDataset286 } from '../utils/compute286.js';
import { buildForecast286, detectAnomalies286, segmentDistribution286 } from '../utils/compute286.js';
import { buildActivityMap286, rankBuckets286, summarizePipeline286, synthesizeFlow286 } from '../pipelines/pipeline286.js';
import '../styles/component286.css';

function MetricCard286({ label, value, max }) {
...
function TreeNode286({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode286 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState286 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer286(state, action) {
...
export default function Component286({ seed = 286, title = 'Component286' }) {
  const hook = useLogic286(seed + 286);
  const [state, dispatch] = useReducer(reducer286, initialState286);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm286(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset286(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState286(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree286(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute286(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast286(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies286(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution286(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow286(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets286(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap286(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline286(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm286(form);
    setErrors(found);
  ...
  return (
    <section className="component-286">
      <div className="header-286">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-286">
      ...
      <div className="grid-286">
        <div className="panel-286">
          {cards.map((item, idx) => (
            <MetricCard286 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
