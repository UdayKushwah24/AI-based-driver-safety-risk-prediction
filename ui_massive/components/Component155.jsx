...
import { useLogic155 } from '../hooks/useLogic155.js';
import { buildNestedTree155, buildTableState155, heavyCompute155, transformDataset155 } from '../utils/compute155.js';
import { buildForecast155, detectAnomalies155, segmentDistribution155 } from '../utils/compute155.js';
import { buildActivityMap155, rankBuckets155, summarizePipeline155, synthesizeFlow155 } from '../pipelines/pipeline155.js';
import '../styles/component155.css';

function MetricCard155({ label, value, max }) {
...
function TreeNode155({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode155 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState155 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer155(state, action) {
...
export default function Component155({ seed = 155, title = 'Component155' }) {
  const hook = useLogic155(seed + 155);
  const [state, dispatch] = useReducer(reducer155, initialState155);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm155(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset155(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState155(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree155(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute155(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast155(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies155(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution155(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow155(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets155(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap155(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline155(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm155(form);
    setErrors(found);
  ...
  return (
    <section className="component-155">
      <div className="header-155">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-155">
      ...
      <div className="grid-155">
        <div className="panel-155">
          {cards.map((item, idx) => (
            <MetricCard155 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
