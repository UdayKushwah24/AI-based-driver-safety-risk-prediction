...
import '../styles/component125.css';

function MetricCard125({ label, value, max }) {
...
function TreeNode125({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode125 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState125 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer125(state, action) {
...
export default function Component125({ seed = 125, title = 'Component125' }) {
  const hook = useLogic125(seed + 125);
  const [state, dispatch] = useReducer(reducer125, initialState125);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm125(seed));
  ...
  const table = useMemo(() => buildTableState125(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm125(form);
    setErrors(found);
  ...
  return (
    <section className="component-125">
      <div className="header-125">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-125">
      ...
      <div className="grid-125">
        <div className="panel-125">
          {cards.map((item, idx) => (
            <MetricCard125 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
