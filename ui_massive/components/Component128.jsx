...
import '../styles/component128.css';

function MetricCard128({ label, value, max }) {
...
function TreeNode128({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode128 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component128({ seed = 128, title = 'Component128' }) {
  const hook = useLogic128(seed + 128);
  const [state, dispatch] = useReducer(reducer128, initialState128);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm128(seed));
  ...
  const table = useMemo(() => buildTableState128(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm128(form);
    setErrors(found);
  ...
  return (
    <section className="component-128">
      ...
      <div className="grid-128">
        <div className="panel-128">
          {cards.map((item, idx) => (
            <MetricCard128 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
