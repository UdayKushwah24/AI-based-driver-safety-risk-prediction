...
import '../styles/component74.css';

function MetricCard74({ label, value, max }) {
...
function TreeNode74({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode74 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component74({ seed = 74, title = 'Component74' }) {
  const hook = useLogic74(seed + 74);
  const [state, dispatch] = useReducer(reducer74, initialState74);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm74(seed));
  ...
  const table = useMemo(() => buildTableState74(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm74(form);
    setErrors(found);
  ...
  return (
    <section className="component-74">
      ...
      <div className="grid-74">
        <div className="panel-74">
          {cards.map((item, idx) => (
            <MetricCard74 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
