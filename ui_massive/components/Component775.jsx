...
import '../styles/component775.css';

function MetricCard775({ label, value, max }) {
...
function TreeNode775({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode775 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState775 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer775(state, action) {
...
export default function Component775({ seed = 775, title = 'Component775' }) {
  const hook = useLogic225(seed + 775);
  const [state, dispatch] = useReducer(reducer775, initialState775);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm775(seed));
  ...
  const table = useMemo(() => buildTableState225(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm775(form);
    setErrors(found);
  ...
  return (
    <section className="component-775">
      <div className="header-775">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-775">
      ...
      <div className="grid-775">
        <div className="panel-775">
          {cards.map((item, idx) => (
            <MetricCard775 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
