...
import '../styles/component230.css';

function MetricCard230({ label, value, max }) {
...
function TreeNode230({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode230 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState230 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer230(state, action) {
...
export default function Component230({ seed = 230, title = 'Component230' }) {
  const hook = useLogic230(seed + 230);
  const [state, dispatch] = useReducer(reducer230, initialState230);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm230(seed));
  ...
  const table = useMemo(() => buildTableState230(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm230(form);
    setErrors(found);
  ...
  return (
    <section className="component-230">
      <div className="header-230">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-230">
      ...
      <div className="grid-230">
        <div className="panel-230">
          {cards.map((item, idx) => (
            <MetricCard230 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
