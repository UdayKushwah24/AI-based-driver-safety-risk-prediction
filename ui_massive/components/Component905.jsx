...
import '../styles/component905.css';

function MetricCard905({ label, value, max }) {
...
function TreeNode905({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode905 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState905 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer905(state, action) {
...
export default function Component905({ seed = 905, title = 'Component905' }) {
  const hook = useLogic355(seed + 905);
  const [state, dispatch] = useReducer(reducer905, initialState905);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm905(seed));
  ...
  const table = useMemo(() => buildTableState355(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm905(form);
    setErrors(found);
  ...
  return (
    <section className="component-905">
      <div className="header-905">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-905">
      ...
      <div className="grid-905">
        <div className="panel-905">
          {cards.map((item, idx) => (
            <MetricCard905 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
