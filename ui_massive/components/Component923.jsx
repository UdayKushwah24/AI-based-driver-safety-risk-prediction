...
import '../styles/component923.css';

function MetricCard923({ label, value, max }) {
...
function TreeNode923({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode923 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState923 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer923(state, action) {
...
export default function Component923({ seed = 923, title = 'Component923' }) {
  const hook = useLogic373(seed + 923);
  const [state, dispatch] = useReducer(reducer923, initialState923);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm923(seed));
  ...
  const table = useMemo(() => buildTableState373(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm923(form);
    setErrors(found);
  ...
  return (
    <section className="component-923">
      <div className="header-923">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-923">
      ...
      <div className="grid-923">
        <div className="panel-923">
          {cards.map((item, idx) => (
            <MetricCard923 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
