...
import { useLogic370 } from '../hooks/useLogic370.js';
import { buildNestedTree370, buildTableState370, heavyCompute370, transformDataset370 } from '../utils/compute370.js';
import { buildForecast370, detectAnomalies370, segmentDistribution370 } from '../utils/compute370.js';
import { buildActivityMap370, rankBuckets370, summarizePipeline370, synthesizeFlow370 } from '../pipelines/pipeline370.js';
import '../styles/component370.css';

function MetricCard370({ label, value, max }) {
...
function TreeNode370({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode370 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState370 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer370(state, action) {
...
export default function Component370({ seed = 370, title = 'Component370' }) {
  const hook = useLogic370(seed + 370);
  const [state, dispatch] = useReducer(reducer370, initialState370);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm370(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset370(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState370(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree370(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute370(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast370(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies370(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution370(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow370(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets370(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap370(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline370(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm370(form);
    setErrors(found);
  ...
  return (
    <section className="component-370">
      <div className="header-370">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-370">
      ...
      <div className="grid-370">
        <div className="panel-370">
          {cards.map((item, idx) => (
            <MetricCard370 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
