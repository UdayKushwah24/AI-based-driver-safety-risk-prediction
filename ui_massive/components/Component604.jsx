...
import { useLogic54 } from '../hooks/useLogic54.js';
import { buildNestedTree54, buildTableState54, heavyCompute54, transformDataset54 } from '../utils/compute54.js';
import { buildForecast54, detectAnomalies54, segmentDistribution54 } from '../utils/compute54.js';
import { buildActivityMap54, rankBuckets54, summarizePipeline54, synthesizeFlow54 } from '../pipelines/pipeline54.js';
import '../styles/component604.css';

function MetricCard604({ label, value, max }) {
...
function TreeNode604({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode604 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState604 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer604(state, action) {
...
export default function Component604({ seed = 604, title = 'Component604' }) {
  const hook = useLogic54(seed + 604);
  const [state, dispatch] = useReducer(reducer604, initialState604);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm604(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset54(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState54(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree54(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute54(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast54(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies54(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution54(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow54(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets54(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap54(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline54(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm604(form);
    setErrors(found);
  ...
  return (
    <section className="component-604">
      <div className="header-604">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-604">
      ...
      <div className="grid-604">
        <div className="panel-604">
          {cards.map((item, idx) => (
            <MetricCard604 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
