...
import '../styles/component133.css';

function MetricCard133({ label, value, max }) {
...
function TreeNode133({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode133 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component133({ seed = 133, title = 'Component133' }) {
  const hook = useLogic133(seed + 133);
  const [state, dispatch] = useReducer(reducer133, initialState133);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm133(seed));
  ...
  const table = useMemo(() => buildTableState133(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm133(form);
    setErrors(found);
  ...
  return (
    <section className="component-133">
      ...
      <div className="grid-133">
        <div className="panel-133">
          {cards.map((item, idx) => (
            <MetricCard133 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
