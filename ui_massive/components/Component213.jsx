...
import { useLogic213 } from '../hooks/useLogic213.js';
import { buildNestedTree213, buildTableState213, heavyCompute213, transformDataset213 } from '../utils/compute213.js';
import { buildForecast213, detectAnomalies213, segmentDistribution213 } from '../utils/compute213.js';
import { buildActivityMap213, rankBuckets213, summarizePipeline213, synthesizeFlow213 } from '../pipelines/pipeline213.js';
import '../styles/component213.css';

function MetricCard213({ label, value, max }) {
...
function TreeNode213({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode213 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState213 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer213(state, action) {
...
export default function Component213({ seed = 213, title = 'Component213' }) {
  const hook = useLogic213(seed + 213);
  const [state, dispatch] = useReducer(reducer213, initialState213);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm213(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset213(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState213(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree213(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute213(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast213(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies213(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution213(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow213(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets213(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap213(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline213(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm213(form);
    setErrors(found);
  ...
  return (
    <section className="component-213">
      <div className="header-213">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-213">
      ...
      <div className="grid-213">
        <div className="panel-213">
          {cards.map((item, idx) => (
            <MetricCard213 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
