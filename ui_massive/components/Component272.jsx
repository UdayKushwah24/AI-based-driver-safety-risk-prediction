...
import '../styles/component272.css';

function MetricCard272({ label, value, max }) {
...
function TreeNode272({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode272 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState272 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer272(state, action) {
...
export default function Component272({ seed = 272, title = 'Component272' }) {
  const hook = useLogic272(seed + 272);
  const [state, dispatch] = useReducer(reducer272, initialState272);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm272(seed));
  ...
  const table = useMemo(() => buildTableState272(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm272(form);
    setErrors(found);
  ...
  return (
    <section className="component-272">
      <div className="header-272">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-272">
      ...
      <div className="grid-272">
        <div className="panel-272">
          {cards.map((item, idx) => (
            <MetricCard272 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
