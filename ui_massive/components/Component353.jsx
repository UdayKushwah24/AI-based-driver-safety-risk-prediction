...
import '../styles/component353.css';

function MetricCard353({ label, value, max }) {
...
function TreeNode353({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode353 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState353 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer353(state, action) {
...
export default function Component353({ seed = 353, title = 'Component353' }) {
  const hook = useLogic353(seed + 353);
  const [state, dispatch] = useReducer(reducer353, initialState353);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm353(seed));
  ...
  const table = useMemo(() => buildTableState353(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm353(form);
    setErrors(found);
  ...
  return (
    <section className="component-353">
      <div className="header-353">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-353">
      ...
      <div className="grid-353">
        <div className="panel-353">
          {cards.map((item, idx) => (
            <MetricCard353 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
