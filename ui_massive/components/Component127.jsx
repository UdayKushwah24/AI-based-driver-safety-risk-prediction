...
import '../styles/component127.css';

function MetricCard127({ label, value, max }) {
...
function TreeNode127({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode127 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component127({ seed = 127, title = 'Component127' }) {
  const hook = useLogic127(seed + 127);
  const [state, dispatch] = useReducer(reducer127, initialState127);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm127(seed));
  ...
  const table = useMemo(() => buildTableState127(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm127(form);
    setErrors(found);
  ...
  return (
    <section className="component-127">
      ...
      <div className="grid-127">
        <div className="panel-127">
          {cards.map((item, idx) => (
            <MetricCard127 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
