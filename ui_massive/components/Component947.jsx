...
import '../styles/component947.css';

function MetricCard947({ label, value, max }) {
...
function TreeNode947({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode947 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState947 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer947(state, action) {
...
export default function Component947({ seed = 947, title = 'Component947' }) {
  const hook = useLogic397(seed + 947);
  const [state, dispatch] = useReducer(reducer947, initialState947);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm947(seed));
  ...
  const table = useMemo(() => buildTableState397(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm947(form);
    setErrors(found);
  ...
  return (
    <section className="component-947">
      <div className="header-947">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-947">
      ...
      <div className="grid-947">
        <div className="panel-947">
          {cards.map((item, idx) => (
            <MetricCard947 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
