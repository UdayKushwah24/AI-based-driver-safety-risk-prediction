...
import { useLogic206 } from '../hooks/useLogic206.js';
import { buildNestedTree206, buildTableState206, heavyCompute206, transformDataset206 } from '../utils/compute206.js';
import { buildForecast206, detectAnomalies206, segmentDistribution206 } from '../utils/compute206.js';
import { buildActivityMap206, rankBuckets206, summarizePipeline206, synthesizeFlow206 } from '../pipelines/pipeline206.js';
import '../styles/component206.css';

function MetricCard206({ label, value, max }) {
...
function TreeNode206({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode206 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState206 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer206(state, action) {
...
export default function Component206({ seed = 206, title = 'Component206' }) {
  const hook = useLogic206(seed + 206);
  const [state, dispatch] = useReducer(reducer206, initialState206);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm206(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset206(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState206(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree206(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute206(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast206(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies206(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution206(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow206(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets206(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap206(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline206(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm206(form);
    setErrors(found);
  ...
  return (
    <section className="component-206">
      <div className="header-206">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-206">
      ...
      <div className="grid-206">
        <div className="panel-206">
          {cards.map((item, idx) => (
            <MetricCard206 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
