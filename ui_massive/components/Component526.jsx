...
import '../styles/component526.css';

function MetricCard526({ label, value, max }) {
...
function TreeNode526({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode526 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component526({ seed = 526, title = 'Component526' }) {
  const hook = useLogic526(seed + 526);
  const [state, dispatch] = useReducer(reducer526, initialState526);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm526(seed));
  ...
  const table = useMemo(() => buildTableState526(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm526(form);
    setErrors(found);
  ...
  return (
    <section className="component-526">
      <div className="header-526">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-526">
      ...
      <div className="grid-526">
        <div className="panel-526">
          {cards.map((item, idx) => (
            <MetricCard526 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
