...
import '../styles/component467.css';

function MetricCard467({ label, value, max }) {
...
function TreeNode467({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode467 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState467 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer467(state, action) {
...
export default function Component467({ seed = 467, title = 'Component467' }) {
  const hook = useLogic467(seed + 467);
  const [state, dispatch] = useReducer(reducer467, initialState467);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm467(seed));
  ...
  const table = useMemo(() => buildTableState467(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm467(form);
    setErrors(found);
  ...
  return (
    <section className="component-467">
      <div className="header-467">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-467">
      ...
      <div className="grid-467">
        <div className="panel-467">
          {cards.map((item, idx) => (
            <MetricCard467 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
