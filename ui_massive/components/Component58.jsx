...
import '../styles/component58.css';

function MetricCard58({ label, value, max }) {
...
function TreeNode58({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode58 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component58({ seed = 58, title = 'Component58' }) {
  const hook = useLogic58(seed + 58);
  const [state, dispatch] = useReducer(reducer58, initialState58);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm58(seed));
  ...
  const table = useMemo(() => buildTableState58(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm58(form);
    setErrors(found);
  ...
  return (
    <section className="component-58">
      ...
      <div className="grid-58">
        <div className="panel-58">
          {cards.map((item, idx) => (
            <MetricCard58 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
