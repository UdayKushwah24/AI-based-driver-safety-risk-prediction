...
import '../styles/component482.css';

function MetricCard482({ label, value, max }) {
...
function TreeNode482({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode482 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component482({ seed = 482, title = 'Component482' }) {
  const hook = useLogic482(seed + 482);
  const [state, dispatch] = useReducer(reducer482, initialState482);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm482(seed));
  ...
  const table = useMemo(() => buildTableState482(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm482(form);
    setErrors(found);
  ...
  return (
    <section className="component-482">
      <div className="header-482">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-482">
      ...
      <div className="grid-482">
        <div className="panel-482">
          {cards.map((item, idx) => (
            <MetricCard482 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
