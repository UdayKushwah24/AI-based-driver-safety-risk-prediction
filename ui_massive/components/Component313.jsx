...
import '../styles/component313.css';

function MetricCard313({ label, value, max }) {
...
function TreeNode313({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode313 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState313 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer313(state, action) {
...
export default function Component313({ seed = 313, title = 'Component313' }) {
  const hook = useLogic313(seed + 313);
  const [state, dispatch] = useReducer(reducer313, initialState313);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm313(seed));
  ...
  const table = useMemo(() => buildTableState313(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm313(form);
    setErrors(found);
  ...
  return (
    <section className="component-313">
      <div className="header-313">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-313">
      ...
      <div className="grid-313">
        <div className="panel-313">
          {cards.map((item, idx) => (
            <MetricCard313 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
