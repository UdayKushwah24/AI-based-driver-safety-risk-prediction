...
import '../styles/component51.css';

function MetricCard51({ label, value, max }) {
...
function TreeNode51({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode51 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component51({ seed = 51, title = 'Component51' }) {
  const hook = useLogic51(seed + 51);
  const [state, dispatch] = useReducer(reducer51, initialState51);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm51(seed));
  ...
  const table = useMemo(() => buildTableState51(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm51(form);
    setErrors(found);
  ...
  return (
    <section className="component-51">
      ...
      <div className="grid-51">
        <div className="panel-51">
          {cards.map((item, idx) => (
            <MetricCard51 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
