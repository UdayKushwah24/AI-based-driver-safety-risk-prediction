...
import '../styles/component997.css';

function MetricCard997({ label, value, max }) {
...
function TreeNode997({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode997 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState997 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer997(state, action) {
...
export default function Component997({ seed = 997, title = 'Component997' }) {
  const hook = useLogic447(seed + 997);
  const [state, dispatch] = useReducer(reducer997, initialState997);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm997(seed));
  ...
  const table = useMemo(() => buildTableState447(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm997(form);
    setErrors(found);
  ...
  return (
    <section className="component-997">
      <div className="header-997">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-997">
      ...
      <div className="grid-997">
        <div className="panel-997">
          {cards.map((item, idx) => (
            <MetricCard997 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
