...
import '../styles/component941.css';

function MetricCard941({ label, value, max }) {
...
function TreeNode941({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode941 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState941 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer941(state, action) {
...
export default function Component941({ seed = 941, title = 'Component941' }) {
  const hook = useLogic391(seed + 941);
  const [state, dispatch] = useReducer(reducer941, initialState941);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm941(seed));
  ...
  const table = useMemo(() => buildTableState391(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm941(form);
    setErrors(found);
  ...
  return (
    <section className="component-941">
      <div className="header-941">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-941">
      ...
      <div className="grid-941">
        <div className="panel-941">
          {cards.map((item, idx) => (
            <MetricCard941 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
