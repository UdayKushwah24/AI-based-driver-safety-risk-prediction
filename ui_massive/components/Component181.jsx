...
import '../styles/component181.css';

function MetricCard181({ label, value, max }) {
...
function TreeNode181({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode181 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState181 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer181(state, action) {
...
export default function Component181({ seed = 181, title = 'Component181' }) {
  const hook = useLogic181(seed + 181);
  const [state, dispatch] = useReducer(reducer181, initialState181);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm181(seed));
  ...
  const table = useMemo(() => buildTableState181(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm181(form);
    setErrors(found);
  ...
  return (
    <section className="component-181">
      <div className="header-181">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-181">
      ...
      <div className="grid-181">
        <div className="panel-181">
          {cards.map((item, idx) => (
            <MetricCard181 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
