...
import { useLogic402 } from '../hooks/useLogic402.js';
import { buildNestedTree402, buildTableState402, heavyCompute402, transformDataset402 } from '../utils/compute402.js';
import { buildForecast402, detectAnomalies402, segmentDistribution402 } from '../utils/compute402.js';
import { buildActivityMap402, rankBuckets402, summarizePipeline402, synthesizeFlow402 } from '../pipelines/pipeline402.js';
import '../styles/component402.css';

function MetricCard402({ label, value, max }) {
...
function TreeNode402({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode402 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState402 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer402(state, action) {
...
export default function Component402({ seed = 402, title = 'Component402' }) {
  const hook = useLogic402(seed + 402);
  const [state, dispatch] = useReducer(reducer402, initialState402);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm402(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset402(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState402(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree402(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute402(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast402(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies402(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution402(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow402(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets402(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap402(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline402(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm402(form);
    setErrors(found);
  ...
  return (
    <section className="component-402">
      <div className="header-402">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-402">
      ...
      <div className="grid-402">
        <div className="panel-402">
          {cards.map((item, idx) => (
            <MetricCard402 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
