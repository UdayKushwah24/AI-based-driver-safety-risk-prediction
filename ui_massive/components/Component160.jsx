...
import '../styles/component160.css';

function MetricCard160({ label, value, max }) {
...
function TreeNode160({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode160 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState160 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer160(state, action) {
...
export default function Component160({ seed = 160, title = 'Component160' }) {
  const hook = useLogic160(seed + 160);
  const [state, dispatch] = useReducer(reducer160, initialState160);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm160(seed));
  ...
  const table = useMemo(() => buildTableState160(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm160(form);
    setErrors(found);
  ...
  return (
    <section className="component-160">
      <div className="header-160">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-160">
      ...
      <div className="grid-160">
        <div className="panel-160">
          {cards.map((item, idx) => (
            <MetricCard160 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
