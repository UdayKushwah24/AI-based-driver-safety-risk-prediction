...
import '../styles/component392.css';

function MetricCard392({ label, value, max }) {
...
function TreeNode392({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode392 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState392 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer392(state, action) {
...
export default function Component392({ seed = 392, title = 'Component392' }) {
  const hook = useLogic392(seed + 392);
  const [state, dispatch] = useReducer(reducer392, initialState392);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm392(seed));
  ...
  const table = useMemo(() => buildTableState392(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm392(form);
    setErrors(found);
  ...
  return (
    <section className="component-392">
      <div className="header-392">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-392">
      ...
      <div className="grid-392">
        <div className="panel-392">
          {cards.map((item, idx) => (
            <MetricCard392 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
