...
import '../styles/component504.css';

function MetricCard504({ label, value, max }) {
...
function TreeNode504({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode504 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component504({ seed = 504, title = 'Component504' }) {
  const hook = useLogic504(seed + 504);
  const [state, dispatch] = useReducer(reducer504, initialState504);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm504(seed));
  ...
  const table = useMemo(() => buildTableState504(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm504(form);
    setErrors(found);
  ...
  return (
    <section className="component-504">
      <div className="header-504">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-504">
      ...
      <div className="grid-504">
        <div className="panel-504">
          {cards.map((item, idx) => (
            <MetricCard504 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
