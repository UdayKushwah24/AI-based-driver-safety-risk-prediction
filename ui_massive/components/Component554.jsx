...
import { useLogic4 } from '../hooks/useLogic4.js';
import { buildNestedTree4, buildTableState4, heavyCompute4, transformDataset4 } from '../utils/compute4.js';
import { buildForecast4, detectAnomalies4, segmentDistribution4 } from '../utils/compute4.js';
import { buildActivityMap4, rankBuckets4, summarizePipeline4, synthesizeFlow4 } from '../pipelines/pipeline4.js';
import '../styles/component554.css';

function MetricCard554({ label, value, max }) {
...
function TreeNode554({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode554 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState554 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer554(state, action) {
...
export default function Component554({ seed = 554, title = 'Component554' }) {
  const hook = useLogic4(seed + 554);
  const [state, dispatch] = useReducer(reducer554, initialState554);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm554(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset4(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState4(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree4(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute4(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast4(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies4(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution4(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow4(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets4(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap4(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline4(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm554(form);
    setErrors(found);
  ...
  return (
    <section className="component-554">
      <div className="header-554">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-554">
      ...
      <div className="grid-554">
        <div className="panel-554">
          {cards.map((item, idx) => (
            <MetricCard554 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
