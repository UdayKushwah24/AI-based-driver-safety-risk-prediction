...
import '../styles/component487.css';

function MetricCard487({ label, value, max }) {
...
function TreeNode487({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode487 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState487 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer487(state, action) {
...
export default function Component487({ seed = 487, title = 'Component487' }) {
  const hook = useLogic487(seed + 487);
  const [state, dispatch] = useReducer(reducer487, initialState487);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm487(seed));
  ...
  const table = useMemo(() => buildTableState487(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm487(form);
    setErrors(found);
  ...
  return (
    <section className="component-487">
      <div className="header-487">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-487">
      ...
      <div className="grid-487">
        <div className="panel-487">
          {cards.map((item, idx) => (
            <MetricCard487 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
