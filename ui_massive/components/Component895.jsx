...
import '../styles/component895.css';

function MetricCard895({ label, value, max }) {
...
function TreeNode895({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode895 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState895 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer895(state, action) {
...
export default function Component895({ seed = 895, title = 'Component895' }) {
  const hook = useLogic345(seed + 895);
  const [state, dispatch] = useReducer(reducer895, initialState895);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm895(seed));
  ...
  const table = useMemo(() => buildTableState345(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm895(form);
    setErrors(found);
  ...
  return (
    <section className="component-895">
      <div className="header-895">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-895">
      ...
      <div className="grid-895">
        <div className="panel-895">
          {cards.map((item, idx) => (
            <MetricCard895 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
