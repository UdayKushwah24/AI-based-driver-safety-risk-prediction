...
import { useLogic366 } from '../hooks/useLogic366.js';
import { buildNestedTree366, buildTableState366, heavyCompute366, transformDataset366 } from '../utils/compute366.js';
import { buildForecast366, detectAnomalies366, segmentDistribution366 } from '../utils/compute366.js';
import { buildActivityMap366, rankBuckets366, summarizePipeline366, synthesizeFlow366 } from '../pipelines/pipeline366.js';
import '../styles/component366.css';

function MetricCard366({ label, value, max }) {
...
function TreeNode366({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode366 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState366 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer366(state, action) {
...
export default function Component366({ seed = 366, title = 'Component366' }) {
  const hook = useLogic366(seed + 366);
  const [state, dispatch] = useReducer(reducer366, initialState366);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm366(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset366(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState366(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree366(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute366(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast366(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies366(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution366(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow366(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets366(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap366(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline366(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm366(form);
    setErrors(found);
  ...
  return (
    <section className="component-366">
      <div className="header-366">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-366">
      ...
      <div className="grid-366">
        <div className="panel-366">
          {cards.map((item, idx) => (
            <MetricCard366 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
