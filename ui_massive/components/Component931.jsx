...
import { useLogic381 } from '../hooks/useLogic381.js';
import { buildNestedTree381, buildTableState381, heavyCompute381, transformDataset381 } from '../utils/compute381.js';
import { buildForecast381, detectAnomalies381, segmentDistribution381 } from '../utils/compute381.js';
import { buildActivityMap381, rankBuckets381, summarizePipeline381, synthesizeFlow381 } from '../pipelines/pipeline381.js';
import '../styles/component931.css';

function MetricCard931({ label, value, max }) {
...
function TreeNode931({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode931 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState931 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer931(state, action) {
...
export default function Component931({ seed = 931, title = 'Component931' }) {
  const hook = useLogic381(seed + 931);
  const [state, dispatch] = useReducer(reducer931, initialState931);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm931(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset381(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState381(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree381(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute381(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast381(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies381(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution381(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow381(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets381(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap381(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline381(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm931(form);
    setErrors(found);
  ...
  return (
    <section className="component-931">
      <div className="header-931">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-931">
      ...
      <div className="grid-931">
        <div className="panel-931">
          {cards.map((item, idx) => (
            <MetricCard931 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
