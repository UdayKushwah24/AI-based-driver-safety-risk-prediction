...
import '../styles/component779.css';

function MetricCard779({ label, value, max }) {
...
function TreeNode779({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode779 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState779 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer779(state, action) {
...
export default function Component779({ seed = 779, title = 'Component779' }) {
  const hook = useLogic229(seed + 779);
  const [state, dispatch] = useReducer(reducer779, initialState779);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm779(seed));
  ...
  const table = useMemo(() => buildTableState229(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm779(form);
    setErrors(found);
  ...
  return (
    <section className="component-779">
      <div className="header-779">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-779">
      ...
      <div className="grid-779">
        <div className="panel-779">
          {cards.map((item, idx) => (
            <MetricCard779 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
