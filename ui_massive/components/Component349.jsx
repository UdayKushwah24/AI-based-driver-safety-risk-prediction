...
import '../styles/component349.css';

function MetricCard349({ label, value, max }) {
...
function TreeNode349({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode349 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState349 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer349(state, action) {
...
export default function Component349({ seed = 349, title = 'Component349' }) {
  const hook = useLogic349(seed + 349);
  const [state, dispatch] = useReducer(reducer349, initialState349);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm349(seed));
  ...
  const table = useMemo(() => buildTableState349(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm349(form);
    setErrors(found);
  ...
  return (
    <section className="component-349">
      <div className="header-349">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-349">
      ...
      <div className="grid-349">
        <div className="panel-349">
          {cards.map((item, idx) => (
            <MetricCard349 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
