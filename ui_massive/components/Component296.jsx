...
import '../styles/component296.css';

function MetricCard296({ label, value, max }) {
...
function TreeNode296({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode296 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState296 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer296(state, action) {
...
export default function Component296({ seed = 296, title = 'Component296' }) {
  const hook = useLogic296(seed + 296);
  const [state, dispatch] = useReducer(reducer296, initialState296);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm296(seed));
  ...
  const table = useMemo(() => buildTableState296(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm296(form);
    setErrors(found);
  ...
  return (
    <section className="component-296">
      <div className="header-296">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-296">
      ...
      <div className="grid-296">
        <div className="panel-296">
          {cards.map((item, idx) => (
            <MetricCard296 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
