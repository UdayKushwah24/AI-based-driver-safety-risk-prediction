...
import { useLogic365 } from '../hooks/useLogic365.js';
import { buildNestedTree365, buildTableState365, heavyCompute365, transformDataset365 } from '../utils/compute365.js';
import { buildForecast365, detectAnomalies365, segmentDistribution365 } from '../utils/compute365.js';
import { buildActivityMap365, rankBuckets365, summarizePipeline365, synthesizeFlow365 } from '../pipelines/pipeline365.js';
import '../styles/component915.css';

function MetricCard915({ label, value, max }) {
...
function TreeNode915({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode915 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState915 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer915(state, action) {
...
export default function Component915({ seed = 915, title = 'Component915' }) {
  const hook = useLogic365(seed + 915);
  const [state, dispatch] = useReducer(reducer915, initialState915);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm915(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset365(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState365(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree365(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute365(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast365(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies365(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution365(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow365(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets365(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap365(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline365(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm915(form);
    setErrors(found);
  ...
  return (
    <section className="component-915">
      <div className="header-915">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-915">
      ...
      <div className="grid-915">
        <div className="panel-915">
          {cards.map((item, idx) => (
            <MetricCard915 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
