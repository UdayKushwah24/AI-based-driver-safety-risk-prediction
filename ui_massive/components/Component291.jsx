...
import '../styles/component291.css';

function MetricCard291({ label, value, max }) {
...
function TreeNode291({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode291 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState291 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer291(state, action) {
...
export default function Component291({ seed = 291, title = 'Component291' }) {
  const hook = useLogic291(seed + 291);
  const [state, dispatch] = useReducer(reducer291, initialState291);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm291(seed));
  ...
  const table = useMemo(() => buildTableState291(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm291(form);
    setErrors(found);
  ...
  return (
    <section className="component-291">
      <div className="header-291">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-291">
      ...
      <div className="grid-291">
        <div className="panel-291">
          {cards.map((item, idx) => (
            <MetricCard291 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
