...
import '../styles/component489.css';

function MetricCard489({ label, value, max }) {
...
function TreeNode489({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode489 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState489 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer489(state, action) {
...
export default function Component489({ seed = 489, title = 'Component489' }) {
  const hook = useLogic489(seed + 489);
  const [state, dispatch] = useReducer(reducer489, initialState489);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm489(seed));
  ...
  const table = useMemo(() => buildTableState489(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm489(form);
    setErrors(found);
  ...
  return (
    <section className="component-489">
      <div className="header-489">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-489">
      ...
      <div className="grid-489">
        <div className="panel-489">
          {cards.map((item, idx) => (
            <MetricCard489 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
