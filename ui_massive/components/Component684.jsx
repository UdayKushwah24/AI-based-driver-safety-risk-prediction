...
import '../styles/component684.css';

function MetricCard684({ label, value, max }) {
...
function TreeNode684({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode684 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState684 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer684(state, action) {
...
export default function Component684({ seed = 684, title = 'Component684' }) {
  const hook = useLogic134(seed + 684);
  const [state, dispatch] = useReducer(reducer684, initialState684);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm684(seed));
  ...
  const table = useMemo(() => buildTableState134(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm684(form);
    setErrors(found);
  ...
  return (
    <section className="component-684">
      <div className="header-684">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-684">
      ...
      <div className="grid-684">
        <div className="panel-684">
          {cards.map((item, idx) => (
            <MetricCard684 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
