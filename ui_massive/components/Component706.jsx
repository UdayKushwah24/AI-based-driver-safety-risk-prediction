...
import { useLogic156 } from '../hooks/useLogic156.js';
import { buildNestedTree156, buildTableState156, heavyCompute156, transformDataset156 } from '../utils/compute156.js';
import { buildForecast156, detectAnomalies156, segmentDistribution156 } from '../utils/compute156.js';
import { buildActivityMap156, rankBuckets156, summarizePipeline156, synthesizeFlow156 } from '../pipelines/pipeline156.js';
import '../styles/component706.css';

function MetricCard706({ label, value, max }) {
...
function TreeNode706({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode706 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState706 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer706(state, action) {
...
export default function Component706({ seed = 706, title = 'Component706' }) {
  const hook = useLogic156(seed + 706);
  const [state, dispatch] = useReducer(reducer706, initialState706);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm706(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset156(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState156(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree156(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute156(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast156(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies156(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution156(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow156(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets156(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap156(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline156(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm706(form);
    setErrors(found);
  ...
  return (
    <section className="component-706">
      <div className="header-706">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-706">
      ...
      <div className="grid-706">
        <div className="panel-706">
          {cards.map((item, idx) => (
            <MetricCard706 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
