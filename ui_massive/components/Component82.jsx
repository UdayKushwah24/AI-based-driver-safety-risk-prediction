...
import '../styles/component82.css';

function MetricCard82({ label, value, max }) {
...
function TreeNode82({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode82 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component82({ seed = 82, title = 'Component82' }) {
  const hook = useLogic82(seed + 82);
  const [state, dispatch] = useReducer(reducer82, initialState82);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm82(seed));
  ...
  const table = useMemo(() => buildTableState82(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm82(form);
    setErrors(found);
  ...
  return (
    <section className="component-82">
      ...
      <div className="grid-82">
        <div className="panel-82">
          {cards.map((item, idx) => (
            <MetricCard82 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
