...
import '../styles/component315.css';

function MetricCard315({ label, value, max }) {
...
function TreeNode315({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode315 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState315 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer315(state, action) {
...
export default function Component315({ seed = 315, title = 'Component315' }) {
  const hook = useLogic315(seed + 315);
  const [state, dispatch] = useReducer(reducer315, initialState315);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm315(seed));
  ...
  const table = useMemo(() => buildTableState315(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm315(form);
    setErrors(found);
  ...
  return (
    <section className="component-315">
      <div className="header-315">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-315">
      ...
      <div className="grid-315">
        <div className="panel-315">
          {cards.map((item, idx) => (
            <MetricCard315 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
