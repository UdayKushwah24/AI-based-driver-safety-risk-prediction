...
import { useLogic261 } from '../hooks/useLogic261.js';
import { buildNestedTree261, buildTableState261, heavyCompute261, transformDataset261 } from '../utils/compute261.js';
import { buildForecast261, detectAnomalies261, segmentDistribution261 } from '../utils/compute261.js';
import { buildActivityMap261, rankBuckets261, summarizePipeline261, synthesizeFlow261 } from '../pipelines/pipeline261.js';
import '../styles/component811.css';

function MetricCard811({ label, value, max }) {
...
function TreeNode811({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode811 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState811 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer811(state, action) {
...
export default function Component811({ seed = 811, title = 'Component811' }) {
  const hook = useLogic261(seed + 811);
  const [state, dispatch] = useReducer(reducer811, initialState811);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm811(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset261(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState261(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree261(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute261(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast261(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies261(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution261(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow261(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets261(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap261(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline261(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm811(form);
    setErrors(found);
  ...
  return (
    <section className="component-811">
      <div className="header-811">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-811">
      ...
      <div className="grid-811">
        <div className="panel-811">
          {cards.map((item, idx) => (
            <MetricCard811 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
