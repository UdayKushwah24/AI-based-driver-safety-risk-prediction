...
import '../styles/component469.css';

function MetricCard469({ label, value, max }) {
...
function TreeNode469({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode469 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState469 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer469(state, action) {
...
export default function Component469({ seed = 469, title = 'Component469' }) {
  const hook = useLogic469(seed + 469);
  const [state, dispatch] = useReducer(reducer469, initialState469);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm469(seed));
  ...
  const table = useMemo(() => buildTableState469(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm469(form);
    setErrors(found);
  ...
  return (
    <section className="component-469">
      <div className="header-469">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-469">
      ...
      <div className="grid-469">
        <div className="panel-469">
          {cards.map((item, idx) => (
            <MetricCard469 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
