...
import { useLogic102 } from '../hooks/useLogic102.js';
import { buildNestedTree102, buildTableState102, heavyCompute102, transformDataset102 } from '../utils/compute102.js';
import { buildForecast102, detectAnomalies102, segmentDistribution102 } from '../utils/compute102.js';
import { buildActivityMap102, rankBuckets102, summarizePipeline102, synthesizeFlow102 } from '../pipelines/pipeline102.js';
import '../styles/component102.css';

function MetricCard102({ label, value, max }) {
...
function TreeNode102({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode102 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState102 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer102(state, action) {
...
export default function Component102({ seed = 102, title = 'Component102' }) {
  const hook = useLogic102(seed + 102);
  const [state, dispatch] = useReducer(reducer102, initialState102);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm102(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset102(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState102(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree102(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute102(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast102(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies102(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution102(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow102(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets102(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap102(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline102(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm102(form);
    setErrors(found);
  ...
  return (
    <section className="component-102">
      <div className="header-102">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-102">
      ...
      <div className="grid-102">
        <div className="panel-102">
          {cards.map((item, idx) => (
            <MetricCard102 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
