...
import { useLogic364 } from '../hooks/useLogic364.js';
import { buildNestedTree364, buildTableState364, heavyCompute364, transformDataset364 } from '../utils/compute364.js';
import { buildForecast364, detectAnomalies364, segmentDistribution364 } from '../utils/compute364.js';
import { buildActivityMap364, rankBuckets364, summarizePipeline364, synthesizeFlow364 } from '../pipelines/pipeline364.js';
import '../styles/component914.css';

function MetricCard914({ label, value, max }) {
...
function TreeNode914({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode914 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState914 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer914(state, action) {
...
export default function Component914({ seed = 914, title = 'Component914' }) {
  const hook = useLogic364(seed + 914);
  const [state, dispatch] = useReducer(reducer914, initialState914);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm914(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset364(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState364(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree364(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute364(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast364(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies364(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution364(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow364(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets364(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap364(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline364(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm914(form);
    setErrors(found);
  ...
  return (
    <section className="component-914">
      <div className="header-914">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-914">
      ...
      <div className="grid-914">
        <div className="panel-914">
          {cards.map((item, idx) => (
            <MetricCard914 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
