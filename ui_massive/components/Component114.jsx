...
import '../styles/component114.css';

function MetricCard114({ label, value, max }) {
...
function TreeNode114({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode114 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component114({ seed = 114, title = 'Component114' }) {
  const hook = useLogic114(seed + 114);
  const [state, dispatch] = useReducer(reducer114, initialState114);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm114(seed));
  ...
  const table = useMemo(() => buildTableState114(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm114(form);
    setErrors(found);
  ...
  return (
    <section className="component-114">
      ...
      <div className="grid-114">
        <div className="panel-114">
          {cards.map((item, idx) => (
            <MetricCard114 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
