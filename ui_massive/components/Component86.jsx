...
import '../styles/component86.css';

function MetricCard86({ label, value, max }) {
...
function TreeNode86({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode86 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState86 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer86(state, action) {
...
export default function Component86({ seed = 86, title = 'Component86' }) {
  const hook = useLogic86(seed + 86);
  const [state, dispatch] = useReducer(reducer86, initialState86);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm86(seed));
  ...
  const table = useMemo(() => buildTableState86(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm86(form);
    setErrors(found);
  ...
  return (
    <section className="component-86">
      <div className="header-86">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-86">
      ...
      <div className="grid-86">
        <div className="panel-86">
          {cards.map((item, idx) => (
            <MetricCard86 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
