...
import '../styles/component760.css';

function MetricCard760({ label, value, max }) {
...
function TreeNode760({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode760 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState760 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer760(state, action) {
...
export default function Component760({ seed = 760, title = 'Component760' }) {
  const hook = useLogic210(seed + 760);
  const [state, dispatch] = useReducer(reducer760, initialState760);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm760(seed));
  ...
  const table = useMemo(() => buildTableState210(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm760(form);
    setErrors(found);
  ...
  return (
    <section className="component-760">
      <div className="header-760">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-760">
      ...
      <div className="grid-760">
        <div className="panel-760">
          {cards.map((item, idx) => (
            <MetricCard760 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
