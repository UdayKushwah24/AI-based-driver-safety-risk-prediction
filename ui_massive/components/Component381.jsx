...
import '../styles/component381.css';

function MetricCard381({ label, value, max }) {
...
function TreeNode381({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode381 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState381 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer381(state, action) {
...
export default function Component381({ seed = 381, title = 'Component381' }) {
  const hook = useLogic381(seed + 381);
  const [state, dispatch] = useReducer(reducer381, initialState381);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm381(seed));
  ...
  const table = useMemo(() => buildTableState381(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm381(form);
    setErrors(found);
  ...
  return (
    <section className="component-381">
      <div className="header-381">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-381">
      ...
      <div className="grid-381">
        <div className="panel-381">
          {cards.map((item, idx) => (
            <MetricCard381 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
