...
import { useLogic290 } from '../hooks/useLogic290.js';
import { buildNestedTree290, buildTableState290, heavyCompute290, transformDataset290 } from '../utils/compute290.js';
import { buildForecast290, detectAnomalies290, segmentDistribution290 } from '../utils/compute290.js';
import { buildActivityMap290, rankBuckets290, summarizePipeline290, synthesizeFlow290 } from '../pipelines/pipeline290.js';
import '../styles/component290.css';

function MetricCard290({ label, value, max }) {
...
function TreeNode290({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode290 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState290 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer290(state, action) {
...
export default function Component290({ seed = 290, title = 'Component290' }) {
  const hook = useLogic290(seed + 290);
  const [state, dispatch] = useReducer(reducer290, initialState290);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm290(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset290(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState290(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree290(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute290(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast290(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies290(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution290(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow290(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets290(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap290(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline290(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm290(form);
    setErrors(found);
  ...
  return (
    <section className="component-290">
      <div className="header-290">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-290">
      ...
      <div className="grid-290">
        <div className="panel-290">
          {cards.map((item, idx) => (
            <MetricCard290 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
