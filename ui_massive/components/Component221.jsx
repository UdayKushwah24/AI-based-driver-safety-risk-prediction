...
import { useLogic221 } from '../hooks/useLogic221.js';
import { buildNestedTree221, buildTableState221, heavyCompute221, transformDataset221 } from '../utils/compute221.js';
import { buildForecast221, detectAnomalies221, segmentDistribution221 } from '../utils/compute221.js';
import { buildActivityMap221, rankBuckets221, summarizePipeline221, synthesizeFlow221 } from '../pipelines/pipeline221.js';
import '../styles/component221.css';

function MetricCard221({ label, value, max }) {
...
function TreeNode221({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode221 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState221 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer221(state, action) {
...
export default function Component221({ seed = 221, title = 'Component221' }) {
  const hook = useLogic221(seed + 221);
  const [state, dispatch] = useReducer(reducer221, initialState221);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm221(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset221(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState221(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree221(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute221(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast221(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies221(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution221(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow221(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets221(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap221(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline221(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm221(form);
    setErrors(found);
  ...
  return (
    <section className="component-221">
      <div className="header-221">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-221">
      ...
      <div className="grid-221">
        <div className="panel-221">
          {cards.map((item, idx) => (
            <MetricCard221 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
