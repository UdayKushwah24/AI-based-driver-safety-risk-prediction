...
import '../styles/component920.css';

function MetricCard920({ label, value, max }) {
...
function TreeNode920({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode920 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState920 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer920(state, action) {
...
export default function Component920({ seed = 920, title = 'Component920' }) {
  const hook = useLogic370(seed + 920);
  const [state, dispatch] = useReducer(reducer920, initialState920);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm920(seed));
  ...
  const table = useMemo(() => buildTableState370(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm920(form);
    setErrors(found);
  ...
  return (
    <section className="component-920">
      <div className="header-920">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-920">
      ...
      <div className="grid-920">
        <div className="panel-920">
          {cards.map((item, idx) => (
            <MetricCard920 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
