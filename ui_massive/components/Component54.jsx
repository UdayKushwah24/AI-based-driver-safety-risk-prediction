...
import '../styles/component54.css';

function MetricCard54({ label, value, max }) {
...
function TreeNode54({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode54 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component54({ seed = 54, title = 'Component54' }) {
  const hook = useLogic54(seed + 54);
  const [state, dispatch] = useReducer(reducer54, initialState54);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm54(seed));
  ...
  const table = useMemo(() => buildTableState54(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm54(form);
    setErrors(found);
  ...
  return (
    <section className="component-54">
      ...
      <div className="grid-54">
        <div className="panel-54">
          {cards.map((item, idx) => (
            <MetricCard54 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
