...
import '../styles/component384.css';

function MetricCard384({ label, value, max }) {
...
function TreeNode384({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode384 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState384 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer384(state, action) {
...
export default function Component384({ seed = 384, title = 'Component384' }) {
  const hook = useLogic384(seed + 384);
  const [state, dispatch] = useReducer(reducer384, initialState384);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm384(seed));
  ...
  const table = useMemo(() => buildTableState384(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm384(form);
    setErrors(found);
  ...
  return (
    <section className="component-384">
      <div className="header-384">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-384">
      ...
      <div className="grid-384">
        <div className="panel-384">
          {cards.map((item, idx) => (
            <MetricCard384 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
