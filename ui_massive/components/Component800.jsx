...
import '../styles/component800.css';

function MetricCard800({ label, value, max }) {
...
function TreeNode800({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode800 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState800 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer800(state, action) {
...
export default function Component800({ seed = 800, title = 'Component800' }) {
  const hook = useLogic250(seed + 800);
  const [state, dispatch] = useReducer(reducer800, initialState800);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm800(seed));
  ...
  const table = useMemo(() => buildTableState250(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm800(form);
    setErrors(found);
  ...
  return (
    <section className="component-800">
      <div className="header-800">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-800">
      ...
      <div className="grid-800">
        <div className="panel-800">
          {cards.map((item, idx) => (
            <MetricCard800 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
