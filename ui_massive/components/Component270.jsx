...
import '../styles/component270.css';

function MetricCard270({ label, value, max }) {
...
function TreeNode270({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode270 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component270({ seed = 270, title = 'Component270' }) {
  const hook = useLogic270(seed + 270);
  const [state, dispatch] = useReducer(reducer270, initialState270);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm270(seed));
  ...
  const table = useMemo(() => buildTableState270(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm270(form);
    setErrors(found);
  ...
  return (
    <section className="component-270">
      <div className="header-270">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-270">
      ...
      <div className="grid-270">
        <div className="panel-270">
          {cards.map((item, idx) => (
            <MetricCard270 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
