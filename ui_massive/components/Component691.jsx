...
import '../styles/component691.css';

function MetricCard691({ label, value, max }) {
...
function TreeNode691({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode691 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState691 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer691(state, action) {
...
export default function Component691({ seed = 691, title = 'Component691' }) {
  const hook = useLogic141(seed + 691);
  const [state, dispatch] = useReducer(reducer691, initialState691);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm691(seed));
  ...
  const table = useMemo(() => buildTableState141(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm691(form);
    setErrors(found);
  ...
  return (
    <section className="component-691">
      <div className="header-691">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-691">
      ...
      <div className="grid-691">
        <div className="panel-691">
          {cards.map((item, idx) => (
            <MetricCard691 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
