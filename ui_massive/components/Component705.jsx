...
import '../styles/component705.css';

function MetricCard705({ label, value, max }) {
...
function TreeNode705({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode705 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState705 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer705(state, action) {
...
export default function Component705({ seed = 705, title = 'Component705' }) {
  const hook = useLogic155(seed + 705);
  const [state, dispatch] = useReducer(reducer705, initialState705);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm705(seed));
  ...
  const table = useMemo(() => buildTableState155(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm705(form);
    setErrors(found);
  ...
  return (
    <section className="component-705">
      <div className="header-705">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-705">
      ...
      <div className="grid-705">
        <div className="panel-705">
          {cards.map((item, idx) => (
            <MetricCard705 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
