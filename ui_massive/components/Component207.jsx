...
import '../styles/component207.css';

function MetricCard207({ label, value, max }) {
...
function TreeNode207({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode207 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState207 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer207(state, action) {
...
export default function Component207({ seed = 207, title = 'Component207' }) {
  const hook = useLogic207(seed + 207);
  const [state, dispatch] = useReducer(reducer207, initialState207);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm207(seed));
  ...
  const table = useMemo(() => buildTableState207(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm207(form);
    setErrors(found);
  ...
  return (
    <section className="component-207">
      <div className="header-207">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-207">
      ...
      <div className="grid-207">
        <div className="panel-207">
          {cards.map((item, idx) => (
            <MetricCard207 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
