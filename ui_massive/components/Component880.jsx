...
import '../styles/component880.css';

function MetricCard880({ label, value, max }) {
...
function TreeNode880({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode880 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState880 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer880(state, action) {
...
export default function Component880({ seed = 880, title = 'Component880' }) {
  const hook = useLogic330(seed + 880);
  const [state, dispatch] = useReducer(reducer880, initialState880);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm880(seed));
  ...
  const table = useMemo(() => buildTableState330(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm880(form);
    setErrors(found);
  ...
  return (
    <section className="component-880">
      <div className="header-880">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-880">
      ...
      <div className="grid-880">
        <div className="panel-880">
          {cards.map((item, idx) => (
            <MetricCard880 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
