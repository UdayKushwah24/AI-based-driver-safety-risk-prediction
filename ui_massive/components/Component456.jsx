...
import '../styles/component456.css';

function MetricCard456({ label, value, max }) {
...
function TreeNode456({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode456 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
function reducer456(state, action) {
...
export default function Component456({ seed = 456, title = 'Component456' }) {
  const hook = useLogic456(seed + 456);
  const [state, dispatch] = useReducer(reducer456, initialState456);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm456(seed));
  ...
  const table = useMemo(() => buildTableState456(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm456(form);
    setErrors(found);
  ...
  return (
    <section className="component-456">
      ...
      <div className="grid-456">
        <div className="panel-456">
          {cards.map((item, idx) => (
            <MetricCard456 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
