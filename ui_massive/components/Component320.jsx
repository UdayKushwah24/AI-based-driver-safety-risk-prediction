...
import '../styles/component320.css';

function MetricCard320({ label, value, max }) {
...
function TreeNode320({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode320 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState320 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer320(state, action) {
...
export default function Component320({ seed = 320, title = 'Component320' }) {
  const hook = useLogic320(seed + 320);
  const [state, dispatch] = useReducer(reducer320, initialState320);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm320(seed));
  ...
  const table = useMemo(() => buildTableState320(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm320(form);
    setErrors(found);
  ...
  return (
    <section className="component-320">
      <div className="header-320">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-320">
      ...
      <div className="grid-320">
        <div className="panel-320">
          {cards.map((item, idx) => (
            <MetricCard320 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
