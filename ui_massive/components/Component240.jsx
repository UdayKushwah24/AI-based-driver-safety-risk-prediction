...
import '../styles/component240.css';

function MetricCard240({ label, value, max }) {
...
function TreeNode240({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode240 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState240 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer240(state, action) {
...
export default function Component240({ seed = 240, title = 'Component240' }) {
  const hook = useLogic240(seed + 240);
  const [state, dispatch] = useReducer(reducer240, initialState240);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm240(seed));
  ...
  const table = useMemo(() => buildTableState240(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm240(form);
    setErrors(found);
  ...
  return (
    <section className="component-240">
      <div className="header-240">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-240">
      ...
      <div className="grid-240">
        <div className="panel-240">
          {cards.map((item, idx) => (
            <MetricCard240 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
