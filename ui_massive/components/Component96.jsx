...
import '../styles/component96.css';

function MetricCard96({ label, value, max }) {
...
function TreeNode96({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode96 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component96({ seed = 96, title = 'Component96' }) {
  const hook = useLogic96(seed + 96);
  const [state, dispatch] = useReducer(reducer96, initialState96);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm96(seed));
  ...
  const table = useMemo(() => buildTableState96(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm96(form);
    setErrors(found);
  ...
  return (
    <section className="component-96">
      ...
      <div className="grid-96">
        <div className="panel-96">
          {cards.map((item, idx) => (
            <MetricCard96 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
