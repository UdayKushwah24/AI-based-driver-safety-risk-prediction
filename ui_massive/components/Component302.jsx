...
import { useLogic302 } from '../hooks/useLogic302.js';
import { buildNestedTree302, buildTableState302, heavyCompute302, transformDataset302 } from '../utils/compute302.js';
import { buildForecast302, detectAnomalies302, segmentDistribution302 } from '../utils/compute302.js';
import { buildActivityMap302, rankBuckets302, summarizePipeline302, synthesizeFlow302 } from '../pipelines/pipeline302.js';
import '../styles/component302.css';

function MetricCard302({ label, value, max }) {
...
function TreeNode302({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode302 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState302 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer302(state, action) {
...
export default function Component302({ seed = 302, title = 'Component302' }) {
  const hook = useLogic302(seed + 302);
  const [state, dispatch] = useReducer(reducer302, initialState302);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm302(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset302(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState302(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree302(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute302(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast302(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies302(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution302(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow302(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets302(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap302(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline302(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm302(form);
    setErrors(found);
  ...
  return (
    <section className="component-302">
      <div className="header-302">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-302">
      ...
      <div className="grid-302">
        <div className="panel-302">
          {cards.map((item, idx) => (
            <MetricCard302 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
