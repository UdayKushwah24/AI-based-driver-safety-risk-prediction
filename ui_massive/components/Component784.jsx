...
import '../styles/component784.css';

function MetricCard784({ label, value, max }) {
...
function TreeNode784({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode784 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState784 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer784(state, action) {
...
export default function Component784({ seed = 784, title = 'Component784' }) {
  const hook = useLogic234(seed + 784);
  const [state, dispatch] = useReducer(reducer784, initialState784);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm784(seed));
  ...
  const table = useMemo(() => buildTableState234(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm784(form);
    setErrors(found);
  ...
  return (
    <section className="component-784">
      <div className="header-784">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-784">
      ...
      <div className="grid-784">
        <div className="panel-784">
          {cards.map((item, idx) => (
            <MetricCard784 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
