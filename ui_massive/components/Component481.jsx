...
import '../styles/component481.css';

function MetricCard481({ label, value, max }) {
...
function TreeNode481({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode481 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState481 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer481(state, action) {
...
export default function Component481({ seed = 481, title = 'Component481' }) {
  const hook = useLogic481(seed + 481);
  const [state, dispatch] = useReducer(reducer481, initialState481);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm481(seed));
  ...
  const table = useMemo(() => buildTableState481(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm481(form);
    setErrors(found);
  ...
  return (
    <section className="component-481">
      <div className="header-481">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-481">
      ...
      <div className="grid-481">
        <div className="panel-481">
          {cards.map((item, idx) => (
            <MetricCard481 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
