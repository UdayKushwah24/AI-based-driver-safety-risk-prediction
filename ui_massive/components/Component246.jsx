...
import { useLogic246 } from '../hooks/useLogic246.js';
import { buildNestedTree246, buildTableState246, heavyCompute246, transformDataset246 } from '../utils/compute246.js';
import { buildForecast246, detectAnomalies246, segmentDistribution246 } from '../utils/compute246.js';
import { buildActivityMap246, rankBuckets246, summarizePipeline246, synthesizeFlow246 } from '../pipelines/pipeline246.js';
import '../styles/component246.css';

function MetricCard246({ label, value, max }) {
...
function TreeNode246({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode246 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState246 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer246(state, action) {
...
export default function Component246({ seed = 246, title = 'Component246' }) {
  const hook = useLogic246(seed + 246);
  const [state, dispatch] = useReducer(reducer246, initialState246);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm246(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset246(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState246(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree246(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute246(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast246(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies246(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution246(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow246(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets246(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap246(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline246(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm246(form);
    setErrors(found);
  ...
  return (
    <section className="component-246">
      <div className="header-246">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-246">
      ...
      <div className="grid-246">
        <div className="panel-246">
          {cards.map((item, idx) => (
            <MetricCard246 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
