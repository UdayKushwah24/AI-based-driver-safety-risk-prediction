...
import '../styles/component97.css';

function MetricCard97({ label, value, max }) {
...
function TreeNode97({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode97 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component97({ seed = 97, title = 'Component97' }) {
  const hook = useLogic97(seed + 97);
  const [state, dispatch] = useReducer(reducer97, initialState97);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm97(seed));
  ...
  const table = useMemo(() => buildTableState97(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm97(form);
    setErrors(found);
  ...
  return (
    <section className="component-97">
      ...
      <div className="grid-97">
        <div className="panel-97">
          {cards.map((item, idx) => (
            <MetricCard97 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
