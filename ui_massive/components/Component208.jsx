...
import '../styles/component208.css';

function MetricCard208({ label, value, max }) {
...
function TreeNode208({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode208 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState208 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer208(state, action) {
...
export default function Component208({ seed = 208, title = 'Component208' }) {
  const hook = useLogic208(seed + 208);
  const [state, dispatch] = useReducer(reducer208, initialState208);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm208(seed));
  ...
  const table = useMemo(() => buildTableState208(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm208(form);
    setErrors(found);
  ...
  return (
    <section className="component-208">
      <div className="header-208">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-208">
      ...
      <div className="grid-208">
        <div className="panel-208">
          {cards.map((item, idx) => (
            <MetricCard208 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
