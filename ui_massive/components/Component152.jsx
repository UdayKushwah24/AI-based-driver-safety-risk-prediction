...
import '../styles/component152.css';

function MetricCard152({ label, value, max }) {
...
function TreeNode152({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode152 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState152 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer152(state, action) {
...
export default function Component152({ seed = 152, title = 'Component152' }) {
  const hook = useLogic152(seed + 152);
  const [state, dispatch] = useReducer(reducer152, initialState152);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm152(seed));
  ...
  const table = useMemo(() => buildTableState152(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm152(form);
    setErrors(found);
  ...
  return (
    <section className="component-152">
      <div className="header-152">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-152">
      ...
      <div className="grid-152">
        <div className="panel-152">
          {cards.map((item, idx) => (
            <MetricCard152 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
