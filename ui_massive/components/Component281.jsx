...
import { useLogic281 } from '../hooks/useLogic281.js';
import { buildNestedTree281, buildTableState281, heavyCompute281, transformDataset281 } from '../utils/compute281.js';
import { buildForecast281, detectAnomalies281, segmentDistribution281 } from '../utils/compute281.js';
import { buildActivityMap281, rankBuckets281, summarizePipeline281, synthesizeFlow281 } from '../pipelines/pipeline281.js';
import '../styles/component281.css';

function MetricCard281({ label, value, max }) {
...
function TreeNode281({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode281 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState281 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer281(state, action) {
...
export default function Component281({ seed = 281, title = 'Component281' }) {
  const hook = useLogic281(seed + 281);
  const [state, dispatch] = useReducer(reducer281, initialState281);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm281(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset281(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState281(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree281(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute281(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast281(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies281(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution281(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow281(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets281(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap281(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline281(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm281(form);
    setErrors(found);
  ...
  return (
    <section className="component-281">
      <div className="header-281">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-281">
      ...
      <div className="grid-281">
        <div className="panel-281">
          {cards.map((item, idx) => (
            <MetricCard281 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
