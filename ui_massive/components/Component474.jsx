...
import '../styles/component474.css';

function MetricCard474({ label, value, max }) {
...
function TreeNode474({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode474 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState474 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer474(state, action) {
...
export default function Component474({ seed = 474, title = 'Component474' }) {
  const hook = useLogic474(seed + 474);
  const [state, dispatch] = useReducer(reducer474, initialState474);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm474(seed));
  ...
  const table = useMemo(() => buildTableState474(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm474(form);
    setErrors(found);
  ...
  return (
    <section className="component-474">
      <div className="header-474">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-474">
      ...
      <div className="grid-474">
        <div className="panel-474">
          {cards.map((item, idx) => (
            <MetricCard474 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
