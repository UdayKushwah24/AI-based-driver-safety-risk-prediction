...
import { useLogic56 } from '../hooks/useLogic56.js';
import { buildNestedTree56, buildTableState56, heavyCompute56, transformDataset56 } from '../utils/compute56.js';
import { buildForecast56, detectAnomalies56, segmentDistribution56 } from '../utils/compute56.js';
import { buildActivityMap56, rankBuckets56, summarizePipeline56, synthesizeFlow56 } from '../pipelines/pipeline56.js';
import '../styles/component606.css';

function MetricCard606({ label, value, max }) {
...
function TreeNode606({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode606 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState606 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer606(state, action) {
...
export default function Component606({ seed = 606, title = 'Component606' }) {
  const hook = useLogic56(seed + 606);
  const [state, dispatch] = useReducer(reducer606, initialState606);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm606(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset56(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState56(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree56(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute56(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast56(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies56(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution56(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow56(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets56(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap56(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline56(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm606(form);
    setErrors(found);
  ...
  return (
    <section className="component-606">
      <div className="header-606">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-606">
      ...
      <div className="grid-606">
        <div className="panel-606">
          {cards.map((item, idx) => (
            <MetricCard606 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
