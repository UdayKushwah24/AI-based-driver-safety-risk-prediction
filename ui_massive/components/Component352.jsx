...
import '../styles/component352.css';

function MetricCard352({ label, value, max }) {
...
function TreeNode352({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode352 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState352 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer352(state, action) {
...
export default function Component352({ seed = 352, title = 'Component352' }) {
  const hook = useLogic352(seed + 352);
  const [state, dispatch] = useReducer(reducer352, initialState352);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm352(seed));
  ...
  const table = useMemo(() => buildTableState352(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm352(form);
    setErrors(found);
  ...
  return (
    <section className="component-352">
      <div className="header-352">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-352">
      ...
      <div className="grid-352">
        <div className="panel-352">
          {cards.map((item, idx) => (
            <MetricCard352 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
