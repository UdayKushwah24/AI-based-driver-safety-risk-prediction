...
import '../styles/component88.css';

function MetricCard88({ label, value, max }) {
...
function TreeNode88({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode88 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component88({ seed = 88, title = 'Component88' }) {
  const hook = useLogic88(seed + 88);
  const [state, dispatch] = useReducer(reducer88, initialState88);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm88(seed));
  ...
  const table = useMemo(() => buildTableState88(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm88(form);
    setErrors(found);
  ...
  return (
    <section className="component-88">
      ...
      <div className="grid-88">
        <div className="panel-88">
          {cards.map((item, idx) => (
            <MetricCard88 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
