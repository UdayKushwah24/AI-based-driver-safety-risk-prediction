...
import '../styles/component486.css';

function MetricCard486({ label, value, max }) {
...
function TreeNode486({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode486 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState486 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer486(state, action) {
...
export default function Component486({ seed = 486, title = 'Component486' }) {
  const hook = useLogic486(seed + 486);
  const [state, dispatch] = useReducer(reducer486, initialState486);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm486(seed));
  ...
  const table = useMemo(() => buildTableState486(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm486(form);
    setErrors(found);
  ...
  return (
    <section className="component-486">
      ...
      <div className="grid-486">
        <div className="panel-486">
          {cards.map((item, idx) => (
            <MetricCard486 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
