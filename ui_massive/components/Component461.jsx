...
import '../styles/component461.css';

function MetricCard461({ label, value, max }) {
...
function TreeNode461({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode461 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState461 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer461(state, action) {
...
export default function Component461({ seed = 461, title = 'Component461' }) {
  const hook = useLogic461(seed + 461);
  const [state, dispatch] = useReducer(reducer461, initialState461);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm461(seed));
  ...
  const table = useMemo(() => buildTableState461(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm461(form);
    setErrors(found);
  ...
  return (
    <section className="component-461">
      <div className="header-461">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-461">
      ...
      <div className="grid-461">
        <div className="panel-461">
          {cards.map((item, idx) => (
            <MetricCard461 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
