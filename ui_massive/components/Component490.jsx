...
import '../styles/component490.css';

function MetricCard490({ label, value, max }) {
...
function TreeNode490({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode490 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState490 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer490(state, action) {
...
export default function Component490({ seed = 490, title = 'Component490' }) {
  const hook = useLogic490(seed + 490);
  const [state, dispatch] = useReducer(reducer490, initialState490);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm490(seed));
  ...
  const table = useMemo(() => buildTableState490(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm490(form);
    setErrors(found);
  ...
  return (
    <section className="component-490">
      <div className="header-490">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-490">
      ...
      <div className="grid-490">
        <div className="panel-490">
          {cards.map((item, idx) => (
            <MetricCard490 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
