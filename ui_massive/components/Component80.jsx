...
import '../styles/component80.css';

function MetricCard80({ label, value, max }) {
...
function TreeNode80({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode80 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState80 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer80(state, action) {
...
export default function Component80({ seed = 80, title = 'Component80' }) {
  const hook = useLogic80(seed + 80);
  const [state, dispatch] = useReducer(reducer80, initialState80);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm80(seed));
  ...
  const table = useMemo(() => buildTableState80(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm80(form);
    setErrors(found);
  ...
  return (
    <section className="component-80">
      ...
      <div className="grid-80">
        <div className="panel-80">
          {cards.map((item, idx) => (
            <MetricCard80 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
