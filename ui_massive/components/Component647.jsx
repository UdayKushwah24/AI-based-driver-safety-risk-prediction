...
import { useLogic97 } from '../hooks/useLogic97.js';
import { buildNestedTree97, buildTableState97, heavyCompute97, transformDataset97 } from '../utils/compute97.js';
import { buildForecast97, detectAnomalies97, segmentDistribution97 } from '../utils/compute97.js';
import { buildActivityMap97, rankBuckets97, summarizePipeline97, synthesizeFlow97 } from '../pipelines/pipeline97.js';
import '../styles/component647.css';

function MetricCard647({ label, value, max }) {
...
function TreeNode647({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode647 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState647 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer647(state, action) {
...
export default function Component647({ seed = 647, title = 'Component647' }) {
  const hook = useLogic97(seed + 647);
  const [state, dispatch] = useReducer(reducer647, initialState647);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm647(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset97(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState97(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree97(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute97(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast97(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies97(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution97(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow97(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets97(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap97(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline97(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm647(form);
    setErrors(found);
  ...
  return (
    <section className="component-647">
      <div className="header-647">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-647">
      ...
      <div className="grid-647">
        <div className="panel-647">
          {cards.map((item, idx) => (
            <MetricCard647 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
