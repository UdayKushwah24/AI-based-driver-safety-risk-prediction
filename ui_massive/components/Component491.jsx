...
import '../styles/component491.css';

function MetricCard491({ label, value, max }) {
...
function TreeNode491({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode491 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState491 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer491(state, action) {
...
export default function Component491({ seed = 491, title = 'Component491' }) {
  const hook = useLogic491(seed + 491);
  const [state, dispatch] = useReducer(reducer491, initialState491);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm491(seed));
  ...
  const table = useMemo(() => buildTableState491(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm491(form);
    setErrors(found);
  ...
  return (
    <section className="component-491">
      <div className="header-491">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-491">
      ...
      <div className="grid-491">
        <div className="panel-491">
          {cards.map((item, idx) => (
            <MetricCard491 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
