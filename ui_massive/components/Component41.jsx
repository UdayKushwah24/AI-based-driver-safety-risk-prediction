...
import '../styles/component41.css';

function MetricCard41({ label, value, max }) {
...
function TreeNode41({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode41 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component41({ seed = 41, title = 'Component41' }) {
  const hook = useLogic41(seed + 41);
  const [state, dispatch] = useReducer(reducer41, initialState41);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm41(seed));
  ...
  const table = useMemo(() => buildTableState41(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm41(form);
    setErrors(found);
  ...
  return (
    <section className="component-41">
      ...
      <div className="grid-41">
        <div className="panel-41">
          {cards.map((item, idx) => (
            <MetricCard41 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
