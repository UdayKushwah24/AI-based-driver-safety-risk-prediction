...
import { useLogic30 } from '../hooks/useLogic30.js';
import { buildNestedTree30, buildTableState30, heavyCompute30, transformDataset30 } from '../utils/compute30.js';
import { buildForecast30, detectAnomalies30, segmentDistribution30 } from '../utils/compute30.js';
import { buildActivityMap30, rankBuckets30, summarizePipeline30, synthesizeFlow30 } from '../pipelines/pipeline30.js';
import '../styles/component580.css';

function MetricCard580({ label, value, max }) {
...
function TreeNode580({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode580 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState580 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer580(state, action) {
...
export default function Component580({ seed = 580, title = 'Component580' }) {
  const hook = useLogic30(seed + 580);
  const [state, dispatch] = useReducer(reducer580, initialState580);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm580(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset30(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState30(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree30(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute30(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast30(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies30(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution30(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow30(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets30(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap30(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline30(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm580(form);
    setErrors(found);
  ...
  return (
    <section className="component-580">
      <div className="header-580">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-580">
      ...
      <div className="grid-580">
        <div className="panel-580">
          {cards.map((item, idx) => (
            <MetricCard580 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
