...
import { useLogic38 } from '../hooks/useLogic38.js';
import { buildNestedTree38, buildTableState38, heavyCompute38, transformDataset38 } from '../utils/compute38.js';
import { buildForecast38, detectAnomalies38, segmentDistribution38 } from '../utils/compute38.js';
import { buildActivityMap38, rankBuckets38, summarizePipeline38, synthesizeFlow38 } from '../pipelines/pipeline38.js';
import '../styles/component588.css';

function MetricCard588({ label, value, max }) {
...
function TreeNode588({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode588 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState588 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer588(state, action) {
...
export default function Component588({ seed = 588, title = 'Component588' }) {
  const hook = useLogic38(seed + 588);
  const [state, dispatch] = useReducer(reducer588, initialState588);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm588(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset38(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState38(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree38(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute38(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast38(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies38(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution38(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow38(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets38(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap38(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline38(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm588(form);
    setErrors(found);
  ...
  return (
    <section className="component-588">
      <div className="header-588">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-588">
      ...
      <div className="grid-588">
        <div className="panel-588">
          {cards.map((item, idx) => (
            <MetricCard588 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
