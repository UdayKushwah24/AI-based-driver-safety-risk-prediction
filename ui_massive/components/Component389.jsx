...
import { useLogic389 } from '../hooks/useLogic389.js';
import { buildNestedTree389, buildTableState389, heavyCompute389, transformDataset389 } from '../utils/compute389.js';
import { buildForecast389, detectAnomalies389, segmentDistribution389 } from '../utils/compute389.js';
import { buildActivityMap389, rankBuckets389, summarizePipeline389, synthesizeFlow389 } from '../pipelines/pipeline389.js';
import '../styles/component389.css';

function MetricCard389({ label, value, max }) {
...
function TreeNode389({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode389 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState389 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer389(state, action) {
...
export default function Component389({ seed = 389, title = 'Component389' }) {
  const hook = useLogic389(seed + 389);
  const [state, dispatch] = useReducer(reducer389, initialState389);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm389(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset389(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState389(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree389(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute389(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast389(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies389(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution389(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow389(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets389(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap389(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline389(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm389(form);
    setErrors(found);
  ...
  return (
    <section className="component-389">
      <div className="header-389">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-389">
      ...
      <div className="grid-389">
        <div className="panel-389">
          {cards.map((item, idx) => (
            <MetricCard389 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
