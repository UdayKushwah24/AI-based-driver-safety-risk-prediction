...
import '../styles/component401.css';

function MetricCard401({ label, value, max }) {
...
function TreeNode401({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode401 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
function reducer401(state, action) {
...
export default function Component401({ seed = 401, title = 'Component401' }) {
  const hook = useLogic401(seed + 401);
  const [state, dispatch] = useReducer(reducer401, initialState401);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm401(seed));
  ...
  const table = useMemo(() => buildTableState401(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm401(form);
    setErrors(found);
  ...
  return (
    <section className="component-401">
      ...
      <div className="grid-401">
        <div className="panel-401">
          {cards.map((item, idx) => (
            <MetricCard401 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
