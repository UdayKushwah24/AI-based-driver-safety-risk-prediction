...
import { useLogic379 } from '../hooks/useLogic379.js';
import { buildNestedTree379, buildTableState379, heavyCompute379, transformDataset379 } from '../utils/compute379.js';
import { buildForecast379, detectAnomalies379, segmentDistribution379 } from '../utils/compute379.js';
import { buildActivityMap379, rankBuckets379, summarizePipeline379, synthesizeFlow379 } from '../pipelines/pipeline379.js';
import '../styles/component379.css';

function MetricCard379({ label, value, max }) {
...
function TreeNode379({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode379 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState379 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer379(state, action) {
...
export default function Component379({ seed = 379, title = 'Component379' }) {
  const hook = useLogic379(seed + 379);
  const [state, dispatch] = useReducer(reducer379, initialState379);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm379(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset379(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState379(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree379(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute379(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast379(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies379(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution379(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow379(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets379(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap379(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline379(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm379(form);
    setErrors(found);
  ...
  return (
    <section className="component-379">
      <div className="header-379">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-379">
      ...
      <div className="grid-379">
        <div className="panel-379">
          {cards.map((item, idx) => (
            <MetricCard379 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
