...
import { useLogic172 } from '../hooks/useLogic172.js';
import { buildNestedTree172, buildTableState172, heavyCompute172, transformDataset172 } from '../utils/compute172.js';
import { buildForecast172, detectAnomalies172, segmentDistribution172 } from '../utils/compute172.js';
import { buildActivityMap172, rankBuckets172, summarizePipeline172, synthesizeFlow172 } from '../pipelines/pipeline172.js';
import '../styles/component722.css';

function MetricCard722({ label, value, max }) {
...
function TreeNode722({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode722 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState722 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer722(state, action) {
...
export default function Component722({ seed = 722, title = 'Component722' }) {
  const hook = useLogic172(seed + 722);
  const [state, dispatch] = useReducer(reducer722, initialState722);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm722(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset172(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState172(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree172(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute172(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast172(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies172(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution172(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow172(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets172(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap172(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline172(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm722(form);
    setErrors(found);
  ...
  return (
    <section className="component-722">
      <div className="header-722">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-722">
      ...
      <div className="grid-722">
        <div className="panel-722">
          {cards.map((item, idx) => (
            <MetricCard722 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
