...
import '../styles/component25.css';

function MetricCard25({ label, value, max }) {
...
function TreeNode25({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode25 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component25({ seed = 25, title = 'Component25' }) {
  const hook = useLogic25(seed + 25);
  const [state, dispatch] = useReducer(reducer25, initialState25);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm25(seed));
  ...
  const table = useMemo(() => buildTableState25(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm25(form);
    setErrors(found);
  ...
  return (
    <section className="component-25">
      ...
      <div className="grid-25">
        <div className="panel-25">
          {cards.map((item, idx) => (
            <MetricCard25 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
