...
import '../styles/component413.css';

function MetricCard413({ label, value, max }) {
...
function TreeNode413({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode413 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState413 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer413(state, action) {
...
export default function Component413({ seed = 413, title = 'Component413' }) {
  const hook = useLogic413(seed + 413);
  const [state, dispatch] = useReducer(reducer413, initialState413);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm413(seed));
  ...
  const table = useMemo(() => buildTableState413(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm413(form);
    setErrors(found);
  ...
  return (
    <section className="component-413">
      <div className="header-413">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-413">
      ...
      <div className="grid-413">
        <div className="panel-413">
          {cards.map((item, idx) => (
            <MetricCard413 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
