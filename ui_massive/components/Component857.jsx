...
import '../styles/component857.css';

function MetricCard857({ label, value, max }) {
...
function TreeNode857({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode857 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState857 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer857(state, action) {
...
export default function Component857({ seed = 857, title = 'Component857' }) {
  const hook = useLogic307(seed + 857);
  const [state, dispatch] = useReducer(reducer857, initialState857);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm857(seed));
  ...
  const table = useMemo(() => buildTableState307(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm857(form);
    setErrors(found);
  ...
  return (
    <section className="component-857">
      <div className="header-857">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-857">
      ...
      <div className="grid-857">
        <div className="panel-857">
          {cards.map((item, idx) => (
            <MetricCard857 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
