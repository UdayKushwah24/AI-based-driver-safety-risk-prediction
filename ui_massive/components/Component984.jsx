...
import '../styles/component984.css';

function MetricCard984({ label, value, max }) {
...
function TreeNode984({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode984 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState984 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer984(state, action) {
...
export default function Component984({ seed = 984, title = 'Component984' }) {
  const hook = useLogic434(seed + 984);
  const [state, dispatch] = useReducer(reducer984, initialState984);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm984(seed));
  ...
  const table = useMemo(() => buildTableState434(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm984(form);
    setErrors(found);
  ...
  return (
    <section className="component-984">
      <div className="header-984">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-984">
      ...
      <div className="grid-984">
        <div className="panel-984">
          {cards.map((item, idx) => (
            <MetricCard984 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
