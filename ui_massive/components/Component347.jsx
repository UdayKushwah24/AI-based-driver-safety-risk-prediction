...
import { useLogic347 } from '../hooks/useLogic347.js';
import { buildNestedTree347, buildTableState347, heavyCompute347, transformDataset347 } from '../utils/compute347.js';
import { buildForecast347, detectAnomalies347, segmentDistribution347 } from '../utils/compute347.js';
import { buildActivityMap347, rankBuckets347, summarizePipeline347, synthesizeFlow347 } from '../pipelines/pipeline347.js';
import '../styles/component347.css';

function MetricCard347({ label, value, max }) {
...
function TreeNode347({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode347 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState347 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer347(state, action) {
...
export default function Component347({ seed = 347, title = 'Component347' }) {
  const hook = useLogic347(seed + 347);
  const [state, dispatch] = useReducer(reducer347, initialState347);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm347(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset347(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState347(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree347(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute347(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast347(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies347(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution347(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow347(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets347(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap347(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline347(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm347(form);
    setErrors(found);
  ...
  return (
    <section className="component-347">
      <div className="header-347">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-347">
      ...
      <div className="grid-347">
        <div className="panel-347">
          {cards.map((item, idx) => (
            <MetricCard347 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
