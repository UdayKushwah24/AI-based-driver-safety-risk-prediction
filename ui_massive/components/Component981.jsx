...
import '../styles/component981.css';

function MetricCard981({ label, value, max }) {
...
function TreeNode981({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode981 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState981 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer981(state, action) {
...
export default function Component981({ seed = 981, title = 'Component981' }) {
  const hook = useLogic431(seed + 981);
  const [state, dispatch] = useReducer(reducer981, initialState981);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm981(seed));
  ...
  const table = useMemo(() => buildTableState431(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm981(form);
    setErrors(found);
  ...
  return (
    <section className="component-981">
      <div className="header-981">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-981">
      ...
      <div className="grid-981">
        <div className="panel-981">
          {cards.map((item, idx) => (
            <MetricCard981 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
