...
import { useLogic362 } from '../hooks/useLogic362.js';
import { buildNestedTree362, buildTableState362, heavyCompute362, transformDataset362 } from '../utils/compute362.js';
import { buildForecast362, detectAnomalies362, segmentDistribution362 } from '../utils/compute362.js';
import { buildActivityMap362, rankBuckets362, summarizePipeline362, synthesizeFlow362 } from '../pipelines/pipeline362.js';
import '../styles/component362.css';

function MetricCard362({ label, value, max }) {
...
function TreeNode362({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode362 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState362 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer362(state, action) {
...
export default function Component362({ seed = 362, title = 'Component362' }) {
  const hook = useLogic362(seed + 362);
  const [state, dispatch] = useReducer(reducer362, initialState362);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm362(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset362(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState362(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree362(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute362(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast362(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies362(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution362(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow362(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets362(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap362(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline362(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm362(form);
    setErrors(found);
  ...
  return (
    <section className="component-362">
      <div className="header-362">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-362">
      ...
      <div className="grid-362">
        <div className="panel-362">
          {cards.map((item, idx) => (
            <MetricCard362 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
