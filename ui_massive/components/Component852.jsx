...
import '../styles/component852.css';

function MetricCard852({ label, value, max }) {
...
function TreeNode852({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode852 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState852 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer852(state, action) {
...
export default function Component852({ seed = 852, title = 'Component852' }) {
  const hook = useLogic302(seed + 852);
  const [state, dispatch] = useReducer(reducer852, initialState852);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm852(seed));
  ...
  const table = useMemo(() => buildTableState302(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm852(form);
    setErrors(found);
  ...
  return (
    <section className="component-852">
      <div className="header-852">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-852">
      ...
      <div className="grid-852">
        <div className="panel-852">
          {cards.map((item, idx) => (
            <MetricCard852 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
