...
function MetricCard45({ label, value, max }) {
...
function TreeNode45({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode45 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component45({ seed = 45, title = 'Component45' }) {
  const hook = useLogic45(seed + 45);
  const [state, dispatch] = useReducer(reducer45, initialState45);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm45(seed));
  ...
  const table = useMemo(() => buildTableState45(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm45(form);
    setErrors(found);
  ...
  return (
    <section className="component-45">
      ...
      <div className="grid-45">
        <div className="panel-45">
          {cards.map((item, idx) => (
            <MetricCard45 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
