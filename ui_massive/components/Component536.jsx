...
import '../styles/component536.css';

function MetricCard536({ label, value, max }) {
...
function TreeNode536({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode536 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState536 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer536(state, action) {
...
export default function Component536({ seed = 536, title = 'Component536' }) {
  const hook = useLogic536(seed + 536);
  const [state, dispatch] = useReducer(reducer536, initialState536);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm536(seed));
  ...
  const table = useMemo(() => buildTableState536(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm536(form);
    setErrors(found);
  ...
  return (
    <section className="component-536">
      <div className="header-536">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-536">
      ...
      <div className="grid-536">
        <div className="panel-536">
          {cards.map((item, idx) => (
            <MetricCard536 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
