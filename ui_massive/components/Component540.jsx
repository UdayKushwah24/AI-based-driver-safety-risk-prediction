...
import '../styles/component540.css';

function MetricCard540({ label, value, max }) {
...
function TreeNode540({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode540 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState540 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer540(state, action) {
...
export default function Component540({ seed = 540, title = 'Component540' }) {
  const hook = useLogic540(seed + 540);
  const [state, dispatch] = useReducer(reducer540, initialState540);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm540(seed));
  ...
  const table = useMemo(() => buildTableState540(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm540(form);
    setErrors(found);
  ...
  return (
    <section className="component-540">
      <div className="header-540">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-540">
      ...
      <div className="grid-540">
        <div className="panel-540">
          {cards.map((item, idx) => (
            <MetricCard540 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
