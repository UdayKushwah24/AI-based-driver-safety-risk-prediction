...
import '../styles/component91.css';

function MetricCard91({ label, value, max }) {
...
function TreeNode91({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode91 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component91({ seed = 91, title = 'Component91' }) {
  const hook = useLogic91(seed + 91);
  const [state, dispatch] = useReducer(reducer91, initialState91);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm91(seed));
  ...
  const table = useMemo(() => buildTableState91(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm91(form);
    setErrors(found);
  ...
  return (
    <section className="component-91">
      ...
      <div className="grid-91">
        <div className="panel-91">
          {cards.map((item, idx) => (
            <MetricCard91 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
