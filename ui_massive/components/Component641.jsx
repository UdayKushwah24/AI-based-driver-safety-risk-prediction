...
import { useLogic91 } from '../hooks/useLogic91.js';
import { buildNestedTree91, buildTableState91, heavyCompute91, transformDataset91 } from '../utils/compute91.js';
import { buildForecast91, detectAnomalies91, segmentDistribution91 } from '../utils/compute91.js';
import { buildActivityMap91, rankBuckets91, summarizePipeline91, synthesizeFlow91 } from '../pipelines/pipeline91.js';
import '../styles/component641.css';

function MetricCard641({ label, value, max }) {
...
function TreeNode641({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode641 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState641 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer641(state, action) {
...
export default function Component641({ seed = 641, title = 'Component641' }) {
  const hook = useLogic91(seed + 641);
  const [state, dispatch] = useReducer(reducer641, initialState641);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm641(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset91(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState91(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree91(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute91(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast91(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies91(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution91(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow91(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets91(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap91(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline91(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm641(form);
    setErrors(found);
  ...
  return (
    <section className="component-641">
      <div className="header-641">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-641">
      ...
      <div className="grid-641">
        <div className="panel-641">
          {cards.map((item, idx) => (
            <MetricCard641 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
