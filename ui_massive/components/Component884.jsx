...
import '../styles/component884.css';

function MetricCard884({ label, value, max }) {
...
function TreeNode884({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode884 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState884 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer884(state, action) {
...
export default function Component884({ seed = 884, title = 'Component884' }) {
  const hook = useLogic334(seed + 884);
  const [state, dispatch] = useReducer(reducer884, initialState884);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm884(seed));
  ...
  const table = useMemo(() => buildTableState334(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm884(form);
    setErrors(found);
  ...
  return (
    <section className="component-884">
      <div className="header-884">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-884">
      ...
      <div className="grid-884">
        <div className="panel-884">
          {cards.map((item, idx) => (
            <MetricCard884 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
