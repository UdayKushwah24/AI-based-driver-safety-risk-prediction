...
import '../styles/component440.css';

function MetricCard440({ label, value, max }) {
...
function TreeNode440({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode440 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState440 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer440(state, action) {
...
export default function Component440({ seed = 440, title = 'Component440' }) {
  const hook = useLogic440(seed + 440);
  const [state, dispatch] = useReducer(reducer440, initialState440);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm440(seed));
  ...
  const table = useMemo(() => buildTableState440(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm440(form);
    setErrors(found);
  ...
  return (
    <section className="component-440">
      <div className="header-440">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-440">
      ...
      <div className="grid-440">
        <div className="panel-440">
          {cards.map((item, idx) => (
            <MetricCard440 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
