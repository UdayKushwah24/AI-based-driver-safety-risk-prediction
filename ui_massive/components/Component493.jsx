...
import '../styles/component493.css';

function MetricCard493({ label, value, max }) {
...
function TreeNode493({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode493 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState493 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer493(state, action) {
...
export default function Component493({ seed = 493, title = 'Component493' }) {
  const hook = useLogic493(seed + 493);
  const [state, dispatch] = useReducer(reducer493, initialState493);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm493(seed));
  ...
  const table = useMemo(() => buildTableState493(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm493(form);
    setErrors(found);
  ...
  return (
    <section className="component-493">
      <div className="header-493">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-493">
      ...
      <div className="grid-493">
        <div className="panel-493">
          {cards.map((item, idx) => (
            <MetricCard493 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
