...
import { useLogic298 } from '../hooks/useLogic298.js';
import { buildNestedTree298, buildTableState298, heavyCompute298, transformDataset298 } from '../utils/compute298.js';
import { buildForecast298, detectAnomalies298, segmentDistribution298 } from '../utils/compute298.js';
import { buildActivityMap298, rankBuckets298, summarizePipeline298, synthesizeFlow298 } from '../pipelines/pipeline298.js';
import '../styles/component298.css';

function MetricCard298({ label, value, max }) {
...
function TreeNode298({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode298 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState298 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer298(state, action) {
...
export default function Component298({ seed = 298, title = 'Component298' }) {
  const hook = useLogic298(seed + 298);
  const [state, dispatch] = useReducer(reducer298, initialState298);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm298(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset298(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState298(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree298(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute298(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast298(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies298(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution298(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow298(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets298(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap298(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline298(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm298(form);
    setErrors(found);
  ...
  return (
    <section className="component-298">
      <div className="header-298">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-298">
      ...
      <div className="grid-298">
        <div className="panel-298">
          {cards.map((item, idx) => (
            <MetricCard298 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
