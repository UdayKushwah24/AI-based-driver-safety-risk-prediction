...
import '../styles/component960.css';

function MetricCard960({ label, value, max }) {
...
function TreeNode960({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode960 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState960 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer960(state, action) {
...
export default function Component960({ seed = 960, title = 'Component960' }) {
  const hook = useLogic410(seed + 960);
  const [state, dispatch] = useReducer(reducer960, initialState960);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm960(seed));
  ...
  const table = useMemo(() => buildTableState410(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm960(form);
    setErrors(found);
  ...
  return (
    <section className="component-960">
      <div className="header-960">
        <div>
          <h3>{title}</h3>
      ...
      <div className="grid-960">
        <div className="panel-960">
          {cards.map((item, idx) => (
            <MetricCard960 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
