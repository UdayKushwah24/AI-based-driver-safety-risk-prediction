...
import '../styles/component665.css';

function MetricCard665({ label, value, max }) {
...
function TreeNode665({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode665 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState665 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer665(state, action) {
...
export default function Component665({ seed = 665, title = 'Component665' }) {
  const hook = useLogic115(seed + 665);
  const [state, dispatch] = useReducer(reducer665, initialState665);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm665(seed));
  ...
  const table = useMemo(() => buildTableState115(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm665(form);
    setErrors(found);
  ...
  return (
    <section className="component-665">
      <div className="header-665">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-665">
      ...
      <div className="grid-665">
        <div className="panel-665">
          {cards.map((item, idx) => (
            <MetricCard665 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
