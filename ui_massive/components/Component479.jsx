...
import '../styles/component479.css';

function MetricCard479({ label, value, max }) {
...
function TreeNode479({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode479 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState479 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer479(state, action) {
...
export default function Component479({ seed = 479, title = 'Component479' }) {
  const hook = useLogic479(seed + 479);
  const [state, dispatch] = useReducer(reducer479, initialState479);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm479(seed));
  ...
  const table = useMemo(() => buildTableState479(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm479(form);
    setErrors(found);
  ...
  return (
    <section className="component-479">
      <div className="header-479">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-479">
      ...
      <div className="grid-479">
        <div className="panel-479">
          {cards.map((item, idx) => (
            <MetricCard479 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
