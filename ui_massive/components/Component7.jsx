...
function MetricCard7({ label, value, max }) {
...
function TreeNode7({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode7 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component7({ seed = 7, title = 'Component7' }) {
  const hook = useLogic7(seed + 7);
  const [state, dispatch] = useReducer(reducer7, initialState7);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm7(seed));
  ...
  const table = useMemo(() => buildTableState7(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm7(form);
    setErrors(found);
    ...
    <section className="component-7">
      ...
      <div className="grid-7">
        <div className="panel-7">
          {cards.map((item, idx) => (
            <MetricCard7 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
