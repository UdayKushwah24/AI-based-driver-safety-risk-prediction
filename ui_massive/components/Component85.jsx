...
import '../styles/component85.css';

function MetricCard85({ label, value, max }) {
...
function TreeNode85({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode85 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component85({ seed = 85, title = 'Component85' }) {
  const hook = useLogic85(seed + 85);
  const [state, dispatch] = useReducer(reducer85, initialState85);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm85(seed));
  ...
  const table = useMemo(() => buildTableState85(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm85(form);
    setErrors(found);
  ...
  return (
    <section className="component-85">
      ...
      <div className="grid-85">
        <div className="panel-85">
          {cards.map((item, idx) => (
            <MetricCard85 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
