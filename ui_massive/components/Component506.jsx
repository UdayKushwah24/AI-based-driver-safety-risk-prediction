...
import '../styles/component506.css';

function MetricCard506({ label, value, max }) {
...
function TreeNode506({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode506 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState506 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer506(state, action) {
...
export default function Component506({ seed = 506, title = 'Component506' }) {
  const hook = useLogic506(seed + 506);
  const [state, dispatch] = useReducer(reducer506, initialState506);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm506(seed));
  ...
  const table = useMemo(() => buildTableState506(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm506(form);
    setErrors(found);
  ...
  return (
    <section className="component-506">
      <div className="header-506">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-506">
      ...
      <div className="grid-506">
        <div className="panel-506">
          {cards.map((item, idx) => (
            <MetricCard506 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
