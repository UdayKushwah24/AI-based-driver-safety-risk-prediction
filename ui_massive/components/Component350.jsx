...
import '../styles/component350.css';

function MetricCard350({ label, value, max }) {
...
function TreeNode350({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode350 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState350 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer350(state, action) {
...
export default function Component350({ seed = 350, title = 'Component350' }) {
  const hook = useLogic350(seed + 350);
  const [state, dispatch] = useReducer(reducer350, initialState350);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm350(seed));
  ...
  const table = useMemo(() => buildTableState350(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm350(form);
    setErrors(found);
  ...
  return (
    <section className="component-350">
      ...
      <div className="grid-350">
        <div className="panel-350">
          {cards.map((item, idx) => (
            <MetricCard350 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
