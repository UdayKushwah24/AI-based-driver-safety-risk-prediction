...
import { useLogic276 } from '../hooks/useLogic276.js';
import { buildNestedTree276, buildTableState276, heavyCompute276, transformDataset276 } from '../utils/compute276.js';
import { buildForecast276, detectAnomalies276, segmentDistribution276 } from '../utils/compute276.js';
import { buildActivityMap276, rankBuckets276, summarizePipeline276, synthesizeFlow276 } from '../pipelines/pipeline276.js';
import '../styles/component826.css';

function MetricCard826({ label, value, max }) {
...
function TreeNode826({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode826 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState826 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer826(state, action) {
...
export default function Component826({ seed = 826, title = 'Component826' }) {
  const hook = useLogic276(seed + 826);
  const [state, dispatch] = useReducer(reducer826, initialState826);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm826(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset276(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState276(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree276(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute276(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast276(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies276(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution276(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow276(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets276(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap276(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline276(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm826(form);
    setErrors(found);
  ...
  return (
    <section className="component-826">
      <div className="header-826">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-826">
      ...
      <div className="grid-826">
        <div className="panel-826">
          {cards.map((item, idx) => (
            <MetricCard826 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
