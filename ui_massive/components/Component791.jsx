...
import '../styles/component791.css';

function MetricCard791({ label, value, max }) {
...
function TreeNode791({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode791 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState791 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer791(state, action) {
...
export default function Component791({ seed = 791, title = 'Component791' }) {
  const hook = useLogic241(seed + 791);
  const [state, dispatch] = useReducer(reducer791, initialState791);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm791(seed));
  ...
  const table = useMemo(() => buildTableState241(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm791(form);
    setErrors(found);
  ...
  return (
    <section className="component-791">
      <div className="header-791">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-791">
      ...
      <div className="grid-791">
        <div className="panel-791">
          {cards.map((item, idx) => (
            <MetricCard791 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
