...
import '../styles/component407.css';

function MetricCard407({ label, value, max }) {
...
function TreeNode407({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode407 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component407({ seed = 407, title = 'Component407' }) {
  const hook = useLogic407(seed + 407);
  const [state, dispatch] = useReducer(reducer407, initialState407);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm407(seed));
  ...
  const table = useMemo(() => buildTableState407(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm407(form);
    setErrors(found);
  ...
  return (
    <section className="component-407">
      <div className="header-407">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-407">
      ...
      <div className="grid-407">
        <div className="panel-407">
          {cards.map((item, idx) => (
            <MetricCard407 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
