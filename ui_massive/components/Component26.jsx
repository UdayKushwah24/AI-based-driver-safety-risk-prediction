...
import '../styles/component26.css';

function MetricCard26({ label, value, max }) {
...
function TreeNode26({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode26 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component26({ seed = 26, title = 'Component26' }) {
  const hook = useLogic26(seed + 26);
  const [state, dispatch] = useReducer(reducer26, initialState26);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm26(seed));
  ...
  const table = useMemo(() => buildTableState26(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm26(form);
    setErrors(found);
  ...
  return (
    <section className="component-26">
      ...
      <div className="grid-26">
        <div className="panel-26">
          {cards.map((item, idx) => (
            <MetricCard26 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
