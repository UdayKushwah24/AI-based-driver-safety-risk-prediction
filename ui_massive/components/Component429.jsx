...
import { useLogic429 } from '../hooks/useLogic429.js';
import { buildNestedTree429, buildTableState429, heavyCompute429, transformDataset429 } from '../utils/compute429.js';
import { buildForecast429, detectAnomalies429, segmentDistribution429 } from '../utils/compute429.js';
import { buildActivityMap429, rankBuckets429, summarizePipeline429, synthesizeFlow429 } from '../pipelines/pipeline429.js';
import '../styles/component429.css';

function MetricCard429({ label, value, max }) {
...
function TreeNode429({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode429 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState429 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer429(state, action) {
...
export default function Component429({ seed = 429, title = 'Component429' }) {
  const hook = useLogic429(seed + 429);
  const [state, dispatch] = useReducer(reducer429, initialState429);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm429(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset429(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState429(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree429(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute429(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast429(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies429(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution429(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow429(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets429(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap429(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline429(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm429(form);
    setErrors(found);
  ...
  return (
    <section className="component-429">
      <div className="header-429">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-429">
      ...
      <div className="grid-429">
        <div className="panel-429">
          {cards.map((item, idx) => (
            <MetricCard429 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
