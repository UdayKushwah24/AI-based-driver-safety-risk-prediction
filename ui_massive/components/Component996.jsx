...
import '../styles/component996.css';

function MetricCard996({ label, value, max }) {
...
function TreeNode996({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode996 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState996 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer996(state, action) {
...
export default function Component996({ seed = 996, title = 'Component996' }) {
  const hook = useLogic446(seed + 996);
  const [state, dispatch] = useReducer(reducer996, initialState996);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm996(seed));
  ...
  const table = useMemo(() => buildTableState446(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm996(form);
    setErrors(found);
  ...
  return (
    <section className="component-996">
      <div className="header-996">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-996">
      ...
      <div className="grid-996">
        <div className="panel-996">
          {cards.map((item, idx) => (
            <MetricCard996 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
