...
import '../styles/component537.css';

function MetricCard537({ label, value, max }) {
...
function TreeNode537({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode537 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState537 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer537(state, action) {
...
export default function Component537({ seed = 537, title = 'Component537' }) {
  const hook = useLogic537(seed + 537);
  const [state, dispatch] = useReducer(reducer537, initialState537);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm537(seed));
  ...
  const table = useMemo(() => buildTableState537(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm537(form);
    setErrors(found);
  ...
  return (
    <section className="component-537">
      <div className="header-537">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-537">
      ...
      <div className="grid-537">
        <div className="panel-537">
          {cards.map((item, idx) => (
            <MetricCard537 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
