...
import '../styles/component478.css';

function MetricCard478({ label, value, max }) {
...
function TreeNode478({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode478 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState478 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer478(state, action) {
...
export default function Component478({ seed = 478, title = 'Component478' }) {
  const hook = useLogic478(seed + 478);
  const [state, dispatch] = useReducer(reducer478, initialState478);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm478(seed));
  ...
  const table = useMemo(() => buildTableState478(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm478(form);
    setErrors(found);
  ...
  return (
    <section className="component-478">
      <div className="header-478">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-478">
      ...
      <div className="grid-478">
        <div className="panel-478">
          {cards.map((item, idx) => (
            <MetricCard478 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
