...
import '../styles/component140.css';

function MetricCard140({ label, value, max }) {
...
function TreeNode140({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode140 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState140 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer140(state, action) {
...
export default function Component140({ seed = 140, title = 'Component140' }) {
  const hook = useLogic140(seed + 140);
  const [state, dispatch] = useReducer(reducer140, initialState140);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm140(seed));
  ...
  const table = useMemo(() => buildTableState140(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm140(form);
    setErrors(found);
  ...
  return (
    <section className="component-140">
      ...
      <div className="grid-140">
        <div className="panel-140">
          {cards.map((item, idx) => (
            <MetricCard140 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
