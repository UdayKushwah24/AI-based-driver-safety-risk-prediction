...
import '../styles/component472.css';

function MetricCard472({ label, value, max }) {
...
function TreeNode472({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode472 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState472 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer472(state, action) {
...
export default function Component472({ seed = 472, title = 'Component472' }) {
  const hook = useLogic472(seed + 472);
  const [state, dispatch] = useReducer(reducer472, initialState472);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm472(seed));
  ...
  const table = useMemo(() => buildTableState472(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm472(form);
    setErrors(found);
  ...
  return (
    <section className="component-472">
      <div className="header-472">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-472">
      ...
      <div className="grid-472">
        <div className="panel-472">
          {cards.map((item, idx) => (
            <MetricCard472 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
