...
import '../styles/component360.css';

function MetricCard360({ label, value, max }) {
...
function TreeNode360({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode360 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component360({ seed = 360, title = 'Component360' }) {
  const hook = useLogic360(seed + 360);
  const [state, dispatch] = useReducer(reducer360, initialState360);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm360(seed));
  ...
  const table = useMemo(() => buildTableState360(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm360(form);
    setErrors(found);
  ...
  return (
    <section className="component-360">
      ...
      <div className="grid-360">
        <div className="panel-360">
          {cards.map((item, idx) => (
            <MetricCard360 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
