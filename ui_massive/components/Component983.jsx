...
import '../styles/component983.css';

function MetricCard983({ label, value, max }) {
...
function TreeNode983({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode983 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState983 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer983(state, action) {
...
export default function Component983({ seed = 983, title = 'Component983' }) {
  const hook = useLogic433(seed + 983);
  const [state, dispatch] = useReducer(reducer983, initialState983);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm983(seed));
  ...
  const table = useMemo(() => buildTableState433(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm983(form);
    setErrors(found);
  ...
  return (
    <section className="component-983">
      <div className="header-983">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-983">
      ...
      <div className="grid-983">
        <div className="panel-983">
          {cards.map((item, idx) => (
            <MetricCard983 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
