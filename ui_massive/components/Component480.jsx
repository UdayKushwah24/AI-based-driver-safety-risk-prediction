...
import '../styles/component480.css';

function MetricCard480({ label, value, max }) {
...
function TreeNode480({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode480 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState480 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer480(state, action) {
...
export default function Component480({ seed = 480, title = 'Component480' }) {
  const hook = useLogic480(seed + 480);
  const [state, dispatch] = useReducer(reducer480, initialState480);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm480(seed));
  ...
  const table = useMemo(() => buildTableState480(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm480(form);
    setErrors(found);
  ...
  return (
    <section className="component-480">
      <div className="header-480">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-480">
      ...
      <div className="grid-480">
        <div className="panel-480">
          {cards.map((item, idx) => (
            <MetricCard480 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
