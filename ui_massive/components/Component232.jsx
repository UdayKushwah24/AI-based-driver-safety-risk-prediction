...
import '../styles/component232.css';

function MetricCard232({ label, value, max }) {
...
function TreeNode232({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode232 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState232 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer232(state, action) {
...
export default function Component232({ seed = 232, title = 'Component232' }) {
  const hook = useLogic232(seed + 232);
  const [state, dispatch] = useReducer(reducer232, initialState232);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm232(seed));
  ...
  const table = useMemo(() => buildTableState232(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm232(form);
    setErrors(found);
  ...
  return (
    <section className="component-232">
      <div className="header-232">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-232">
      ...
      <div className="grid-232">
        <div className="panel-232">
          {cards.map((item, idx) => (
            <MetricCard232 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
