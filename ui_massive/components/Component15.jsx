...
import '../styles/component15.css';

function MetricCard15({ label, value, max }) {
...
function TreeNode15({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode15 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component15({ seed = 15, title = 'Component15' }) {
  const hook = useLogic15(seed + 15);
  const [state, dispatch] = useReducer(reducer15, initialState15);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm15(seed));
  ...
  const table = useMemo(() => buildTableState15(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm15(form);
    setErrors(found);
  ...
  return (
    <section className="component-15">
      ...
      <div className="grid-15">
        <div className="panel-15">
          {cards.map((item, idx) => (
            <MetricCard15 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
