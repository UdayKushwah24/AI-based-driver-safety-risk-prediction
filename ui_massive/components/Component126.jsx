...
import '../styles/component126.css';

function MetricCard126({ label, value, max }) {
...
function TreeNode126({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode126 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component126({ seed = 126, title = 'Component126' }) {
  const hook = useLogic126(seed + 126);
  const [state, dispatch] = useReducer(reducer126, initialState126);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm126(seed));
  ...
  const table = useMemo(() => buildTableState126(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm126(form);
    setErrors(found);
  ...
  return (
    <section className="component-126">
      ...
      <div className="grid-126">
        <div className="panel-126">
          {cards.map((item, idx) => (
            <MetricCard126 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
