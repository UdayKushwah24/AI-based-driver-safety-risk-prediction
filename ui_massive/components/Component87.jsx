...
import '../styles/component87.css';

function MetricCard87({ label, value, max }) {
...
function TreeNode87({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode87 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component87({ seed = 87, title = 'Component87' }) {
  const hook = useLogic87(seed + 87);
  const [state, dispatch] = useReducer(reducer87, initialState87);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm87(seed));
  ...
  const table = useMemo(() => buildTableState87(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm87(form);
    setErrors(found);
  ...
  return (
    <section className="component-87">
      ...
      <div className="grid-87">
        <div className="panel-87">
          {cards.map((item, idx) => (
            <MetricCard87 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
