...
import '../styles/component236.css';

function MetricCard236({ label, value, max }) {
...
function TreeNode236({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode236 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState236 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer236(state, action) {
...
export default function Component236({ seed = 236, title = 'Component236' }) {
  const hook = useLogic236(seed + 236);
  const [state, dispatch] = useReducer(reducer236, initialState236);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm236(seed));
  ...
  const table = useMemo(() => buildTableState236(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm236(form);
    setErrors(found);
  ...
  return (
    <section className="component-236">
      <div className="header-236">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-236">
      ...
      <div className="grid-236">
        <div className="panel-236">
          {cards.map((item, idx) => (
            <MetricCard236 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
