...
import '../styles/component964.css';

function MetricCard964({ label, value, max }) {
...
function TreeNode964({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode964 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState964 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer964(state, action) {
...
export default function Component964({ seed = 964, title = 'Component964' }) {
  const hook = useLogic414(seed + 964);
  const [state, dispatch] = useReducer(reducer964, initialState964);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm964(seed));
  ...
  const table = useMemo(() => buildTableState414(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm964(form);
    setErrors(found);
  ...
  return (
    <section className="component-964">
      <div className="header-964">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-964">
      ...
      <div className="grid-964">
        <div className="panel-964">
          {cards.map((item, idx) => (
            <MetricCard964 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
