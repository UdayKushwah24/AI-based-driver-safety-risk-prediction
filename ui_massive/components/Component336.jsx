...
import '../styles/component336.css';

function MetricCard336({ label, value, max }) {
...
function TreeNode336({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode336 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState336 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer336(state, action) {
...
export default function Component336({ seed = 336, title = 'Component336' }) {
  const hook = useLogic336(seed + 336);
  const [state, dispatch] = useReducer(reducer336, initialState336);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm336(seed));
  ...
  const table = useMemo(() => buildTableState336(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm336(form);
    setErrors(found);
  ...
  return (
    <section className="component-336">
      <div className="header-336">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-336">
      ...
      <div className="grid-336">
        <div className="panel-336">
          {cards.map((item, idx) => (
            <MetricCard336 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
