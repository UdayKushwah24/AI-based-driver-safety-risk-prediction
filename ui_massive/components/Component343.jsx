...
import { useLogic343 } from '../hooks/useLogic343.js';
import { buildNestedTree343, buildTableState343, heavyCompute343, transformDataset343 } from '../utils/compute343.js';
import { buildForecast343, detectAnomalies343, segmentDistribution343 } from '../utils/compute343.js';
import { buildActivityMap343, rankBuckets343, summarizePipeline343, synthesizeFlow343 } from '../pipelines/pipeline343.js';
import '../styles/component343.css';

function MetricCard343({ label, value, max }) {
...
function TreeNode343({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode343 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState343 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer343(state, action) {
...
export default function Component343({ seed = 343, title = 'Component343' }) {
  const hook = useLogic343(seed + 343);
  const [state, dispatch] = useReducer(reducer343, initialState343);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm343(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset343(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState343(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree343(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute343(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast343(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies343(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution343(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow343(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets343(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap343(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline343(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm343(form);
    setErrors(found);
  ...
  return (
    <section className="component-343">
      <div className="header-343">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-343">
      ...
      <div className="grid-343">
        <div className="panel-343">
          {cards.map((item, idx) => (
            <MetricCard343 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
