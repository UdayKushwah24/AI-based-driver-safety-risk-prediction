...
import '../styles/component164.css';

function MetricCard164({ label, value, max }) {
...
function TreeNode164({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode164 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState164 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer164(state, action) {
...
export default function Component164({ seed = 164, title = 'Component164' }) {
  const hook = useLogic164(seed + 164);
  const [state, dispatch] = useReducer(reducer164, initialState164);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm164(seed));
  ...
  const table = useMemo(() => buildTableState164(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm164(form);
    setErrors(found);
  ...
  return (
    <section className="component-164">
      <div className="header-164">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-164">
      ...
      <div className="grid-164">
        <div className="panel-164">
          {cards.map((item, idx) => (
            <MetricCard164 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
