...
import '../styles/component245.css';

function MetricCard245({ label, value, max }) {
...
function TreeNode245({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode245 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState245 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer245(state, action) {
...
export default function Component245({ seed = 245, title = 'Component245' }) {
  const hook = useLogic245(seed + 245);
  const [state, dispatch] = useReducer(reducer245, initialState245);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm245(seed));
  ...
  const table = useMemo(() => buildTableState245(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm245(form);
    setErrors(found);
  ...
  return (
    <section className="component-245">
      <div className="header-245">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-245">
      ...
      <div className="grid-245">
        <div className="panel-245">
          {cards.map((item, idx) => (
            <MetricCard245 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
