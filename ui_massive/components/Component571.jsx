...
import { useLogic21 } from '../hooks/useLogic21.js';
import { buildNestedTree21, buildTableState21, heavyCompute21, transformDataset21 } from '../utils/compute21.js';
import { buildForecast21, detectAnomalies21, segmentDistribution21 } from '../utils/compute21.js';
import { buildActivityMap21, rankBuckets21, summarizePipeline21, synthesizeFlow21 } from '../pipelines/pipeline21.js';
import '../styles/component571.css';

function MetricCard571({ label, value, max }) {
...
function TreeNode571({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode571 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState571 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer571(state, action) {
...
export default function Component571({ seed = 571, title = 'Component571' }) {
  const hook = useLogic21(seed + 571);
  const [state, dispatch] = useReducer(reducer571, initialState571);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm571(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset21(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState21(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree21(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute21(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast21(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies21(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution21(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow21(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets21(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap21(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline21(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm571(form);
    setErrors(found);
  ...
  return (
    <section className="component-571">
      <div className="header-571">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-571">
      ...
      <div className="grid-571">
        <div className="panel-571">
          {cards.map((item, idx) => (
            <MetricCard571 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
