...
import '../styles/component387.css';

function MetricCard387({ label, value, max }) {
...
function TreeNode387({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode387 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState387 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer387(state, action) {
...
export default function Component387({ seed = 387, title = 'Component387' }) {
  const hook = useLogic387(seed + 387);
  const [state, dispatch] = useReducer(reducer387, initialState387);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm387(seed));
  ...
  const table = useMemo(() => buildTableState387(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm387(form);
    setErrors(found);
  ...
  return (
    <section className="component-387">
      <div className="header-387">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-387">
      ...
      <div className="grid-387">
        <div className="panel-387">
          {cards.map((item, idx) => (
            <MetricCard387 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
