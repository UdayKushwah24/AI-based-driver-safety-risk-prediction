...
import '../styles/component178.css';

function MetricCard178({ label, value, max }) {
...
function TreeNode178({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode178 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component178({ seed = 178, title = 'Component178' }) {
  const hook = useLogic178(seed + 178);
  const [state, dispatch] = useReducer(reducer178, initialState178);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm178(seed));
  ...
  const table = useMemo(() => buildTableState178(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm178(form);
    setErrors(found);
  ...
  return (
    <section className="component-178">
      <div className="header-178">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-178">
      ...
      <div className="grid-178">
        <div className="panel-178">
          {cards.map((item, idx) => (
            <MetricCard178 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
