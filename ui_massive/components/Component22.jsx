...
import '../styles/component22.css';

function MetricCard22({ label, value, max }) {
...
function TreeNode22({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode22 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component22({ seed = 22, title = 'Component22' }) {
  const hook = useLogic22(seed + 22);
  const [state, dispatch] = useReducer(reducer22, initialState22);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm22(seed));
  ...
  const table = useMemo(() => buildTableState22(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm22(form);
    setErrors(found);
  ...
  return (
    <section className="component-22">
      ...
      <div className="grid-22">
        <div className="panel-22">
          {cards.map((item, idx) => (
            <MetricCard22 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
