...
import { useLogic345 } from '../hooks/useLogic345.js';
import { buildNestedTree345, buildTableState345, heavyCompute345, transformDataset345 } from '../utils/compute345.js';
import { buildForecast345, detectAnomalies345, segmentDistribution345 } from '../utils/compute345.js';
import { buildActivityMap345, rankBuckets345, summarizePipeline345, synthesizeFlow345 } from '../pipelines/pipeline345.js';
import '../styles/component345.css';

function MetricCard345({ label, value, max }) {
...
function TreeNode345({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode345 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState345 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer345(state, action) {
...
export default function Component345({ seed = 345, title = 'Component345' }) {
  const hook = useLogic345(seed + 345);
  const [state, dispatch] = useReducer(reducer345, initialState345);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm345(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset345(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState345(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree345(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute345(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast345(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies345(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution345(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow345(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets345(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap345(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline345(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm345(form);
    setErrors(found);
  ...
  return (
    <section className="component-345">
      <div className="header-345">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-345">
      ...
      <div className="grid-345">
        <div className="panel-345">
          {cards.map((item, idx) => (
            <MetricCard345 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
