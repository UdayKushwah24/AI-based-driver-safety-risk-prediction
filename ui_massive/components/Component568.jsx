...
import { useLogic18 } from '../hooks/useLogic18.js';
import { buildNestedTree18, buildTableState18, heavyCompute18, transformDataset18 } from '../utils/compute18.js';
import { buildForecast18, detectAnomalies18, segmentDistribution18 } from '../utils/compute18.js';
import { buildActivityMap18, rankBuckets18, summarizePipeline18, synthesizeFlow18 } from '../pipelines/pipeline18.js';
import '../styles/component568.css';

function MetricCard568({ label, value, max }) {
...
function TreeNode568({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode568 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState568 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer568(state, action) {
...
export default function Component568({ seed = 568, title = 'Component568' }) {
  const hook = useLogic18(seed + 568);
  const [state, dispatch] = useReducer(reducer568, initialState568);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm568(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset18(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState18(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree18(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute18(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast18(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies18(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution18(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow18(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets18(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap18(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline18(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm568(form);
    setErrors(found);
  ...
  return (
    <section className="component-568">
      <div className="header-568">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-568">
      ...
      <div className="grid-568">
        <div className="panel-568">
          {cards.map((item, idx) => (
            <MetricCard568 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
