...
import '../styles/component145.css';

function MetricCard145({ label, value, max }) {
...
function TreeNode145({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode145 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState145 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer145(state, action) {
...
export default function Component145({ seed = 145, title = 'Component145' }) {
  const hook = useLogic145(seed + 145);
  const [state, dispatch] = useReducer(reducer145, initialState145);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm145(seed));
  ...
  const table = useMemo(() => buildTableState145(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm145(form);
    setErrors(found);
  ...
  return (
    <section className="component-145">
      <div className="header-145">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-145">
      ...
      <div className="grid-145">
        <div className="panel-145">
          {cards.map((item, idx) => (
            <MetricCard145 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
