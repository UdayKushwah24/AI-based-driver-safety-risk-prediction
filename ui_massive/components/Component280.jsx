...
import '../styles/component280.css';

function MetricCard280({ label, value, max }) {
...
function TreeNode280({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode280 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState280 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer280(state, action) {
...
export default function Component280({ seed = 280, title = 'Component280' }) {
  const hook = useLogic280(seed + 280);
  const [state, dispatch] = useReducer(reducer280, initialState280);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm280(seed));
  ...
  const table = useMemo(() => buildTableState280(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm280(form);
    setErrors(found);
  ...
  return (
    <section className="component-280">
      <div className="header-280">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-280">
      ...
      <div className="grid-280">
        <div className="panel-280">
          {cards.map((item, idx) => (
            <MetricCard280 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
