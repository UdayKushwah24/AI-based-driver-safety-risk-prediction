...
import '../styles/component525.css';

function MetricCard525({ label, value, max }) {
...
function TreeNode525({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode525 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState525 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer525(state, action) {
...
export default function Component525({ seed = 525, title = 'Component525' }) {
  const hook = useLogic525(seed + 525);
  const [state, dispatch] = useReducer(reducer525, initialState525);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm525(seed));
  ...
  const table = useMemo(() => buildTableState525(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm525(form);
    setErrors(found);
  ...
  return (
    <section className="component-525">
      <div className="header-525">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-525">
      ...
      <div className="grid-525">
        <div className="panel-525">
          {cards.map((item, idx) => (
            <MetricCard525 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
