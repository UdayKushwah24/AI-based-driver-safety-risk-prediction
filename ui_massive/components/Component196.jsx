...
import '../styles/component196.css';

function MetricCard196({ label, value, max }) {
...
function TreeNode196({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode196 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState196 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer196(state, action) {
...
export default function Component196({ seed = 196, title = 'Component196' }) {
  const hook = useLogic196(seed + 196);
  const [state, dispatch] = useReducer(reducer196, initialState196);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm196(seed));
  ...
  const table = useMemo(() => buildTableState196(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm196(form);
    setErrors(found);
  ...
  return (
    <section className="component-196">
      <div className="header-196">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-196">
      ...
      <div className="grid-196">
        <div className="panel-196">
          {cards.map((item, idx) => (
            <MetricCard196 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
