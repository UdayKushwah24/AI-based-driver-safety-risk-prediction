...
import '../styles/component771.css';

function MetricCard771({ label, value, max }) {
...
function TreeNode771({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode771 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState771 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer771(state, action) {
...
export default function Component771({ seed = 771, title = 'Component771' }) {
  const hook = useLogic221(seed + 771);
  const [state, dispatch] = useReducer(reducer771, initialState771);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm771(seed));
  ...
  const table = useMemo(() => buildTableState221(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm771(form);
    setErrors(found);
  ...
  return (
    <section className="component-771">
      <div className="header-771">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-771">
      ...
      <div className="grid-771">
        <div className="panel-771">
          {cards.map((item, idx) => (
            <MetricCard771 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
