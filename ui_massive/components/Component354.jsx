...
import { useLogic354 } from '../hooks/useLogic354.js';
import { buildNestedTree354, buildTableState354, heavyCompute354, transformDataset354 } from '../utils/compute354.js';
import { buildForecast354, detectAnomalies354, segmentDistribution354 } from '../utils/compute354.js';
import { buildActivityMap354, rankBuckets354, summarizePipeline354, synthesizeFlow354 } from '../pipelines/pipeline354.js';
import '../styles/component354.css';

function MetricCard354({ label, value, max }) {
...
function TreeNode354({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode354 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState354 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer354(state, action) {
...
export default function Component354({ seed = 354, title = 'Component354' }) {
  const hook = useLogic354(seed + 354);
  const [state, dispatch] = useReducer(reducer354, initialState354);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm354(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset354(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState354(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree354(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute354(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast354(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies354(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution354(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow354(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets354(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap354(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline354(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm354(form);
    setErrors(found);
  ...
  return (
    <section className="component-354">
      <div className="header-354">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-354">
      ...
      <div className="grid-354">
        <div className="panel-354">
          {cards.map((item, idx) => (
            <MetricCard354 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
