...
import { useLogic163 } from '../hooks/useLogic163.js';
import { buildNestedTree163, buildTableState163, heavyCompute163, transformDataset163 } from '../utils/compute163.js';
import { buildForecast163, detectAnomalies163, segmentDistribution163 } from '../utils/compute163.js';
import { buildActivityMap163, rankBuckets163, summarizePipeline163, synthesizeFlow163 } from '../pipelines/pipeline163.js';
import '../styles/component713.css';

function MetricCard713({ label, value, max }) {
...
function TreeNode713({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode713 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState713 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer713(state, action) {
...
export default function Component713({ seed = 713, title = 'Component713' }) {
  const hook = useLogic163(seed + 713);
  const [state, dispatch] = useReducer(reducer713, initialState713);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm713(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset163(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState163(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree163(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute163(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast163(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies163(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution163(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow163(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets163(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap163(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline163(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm713(form);
    setErrors(found);
  ...
  return (
    <section className="component-713">
      <div className="header-713">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-713">
      ...
      <div className="grid-713">
        <div className="panel-713">
          {cards.map((item, idx) => (
            <MetricCard713 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
