...
import '../styles/component944.css';

function MetricCard944({ label, value, max }) {
...
function TreeNode944({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode944 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState944 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer944(state, action) {
...
export default function Component944({ seed = 944, title = 'Component944' }) {
  const hook = useLogic394(seed + 944);
  const [state, dispatch] = useReducer(reducer944, initialState944);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm944(seed));
  ...
  const table = useMemo(() => buildTableState394(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm944(form);
    setErrors(found);
  ...
  return (
    <section className="component-944">
      <div className="header-944">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-944">
      ...
      <div className="grid-944">
        <div className="panel-944">
          {cards.map((item, idx) => (
            <MetricCard944 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
