...
import '../styles/component973.css';

function MetricCard973({ label, value, max }) {
...
function TreeNode973({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode973 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState973 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer973(state, action) {
...
export default function Component973({ seed = 973, title = 'Component973' }) {
  const hook = useLogic423(seed + 973);
  const [state, dispatch] = useReducer(reducer973, initialState973);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm973(seed));
  ...
  const table = useMemo(() => buildTableState423(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm973(form);
    setErrors(found);
  ...
  return (
    <section className="component-973">
      <div className="header-973">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-973">
      ...
      <div className="grid-973">
        <div className="panel-973">
          {cards.map((item, idx) => (
            <MetricCard973 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
