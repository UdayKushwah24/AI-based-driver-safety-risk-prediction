...
import '../styles/component14.css';

function MetricCard14({ label, value, max }) {
...
function TreeNode14({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode14 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component14({ seed = 14, title = 'Component14' }) {
  const hook = useLogic14(seed + 14);
  const [state, dispatch] = useReducer(reducer14, initialState14);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm14(seed));
  ...
  const table = useMemo(() => buildTableState14(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm14(form);
    setErrors(found);
  ...
  return (
    <section className="component-14">
      ...
      <div className="grid-14">
        <div className="panel-14">
          {cards.map((item, idx) => (
            <MetricCard14 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
