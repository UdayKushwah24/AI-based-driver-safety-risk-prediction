...
import '../styles/component513.css';

function MetricCard513({ label, value, max }) {
...
function TreeNode513({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode513 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState513 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer513(state, action) {
...
export default function Component513({ seed = 513, title = 'Component513' }) {
  const hook = useLogic513(seed + 513);
  const [state, dispatch] = useReducer(reducer513, initialState513);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm513(seed));
  ...
  const table = useMemo(() => buildTableState513(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm513(form);
    setErrors(found);
  ...
  return (
    <section className="component-513">
      <div className="header-513">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-513">
      ...
      <div className="grid-513">
        <div className="panel-513">
          {cards.map((item, idx) => (
            <MetricCard513 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
