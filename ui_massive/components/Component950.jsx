...
import { useLogic400 } from '../hooks/useLogic400.js';
import { buildNestedTree400, buildTableState400, heavyCompute400, transformDataset400 } from '../utils/compute400.js';
import { buildForecast400, detectAnomalies400, segmentDistribution400 } from '../utils/compute400.js';
import { buildActivityMap400, rankBuckets400, summarizePipeline400, synthesizeFlow400 } from '../pipelines/pipeline400.js';
import '../styles/component950.css';

function MetricCard950({ label, value, max }) {
...
function TreeNode950({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode950 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState950 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer950(state, action) {
...
export default function Component950({ seed = 950, title = 'Component950' }) {
  const hook = useLogic400(seed + 950);
  const [state, dispatch] = useReducer(reducer950, initialState950);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm950(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset400(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState400(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree400(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute400(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast400(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies400(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution400(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow400(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets400(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap400(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline400(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm950(form);
    setErrors(found);
  ...
  return (
    <section className="component-950">
      <div className="header-950">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-950">
      ...
      <div className="grid-950">
        <div className="panel-950">
          {cards.map((item, idx) => (
            <MetricCard950 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
