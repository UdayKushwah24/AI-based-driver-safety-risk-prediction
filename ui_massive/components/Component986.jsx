...
import { useLogic436 } from '../hooks/useLogic436.js';
import { buildNestedTree436, buildTableState436, heavyCompute436, transformDataset436 } from '../utils/compute436.js';
import { buildForecast436, detectAnomalies436, segmentDistribution436 } from '../utils/compute436.js';
import { buildActivityMap436, rankBuckets436, summarizePipeline436, synthesizeFlow436 } from '../pipelines/pipeline436.js';
import '../styles/component986.css';

function MetricCard986({ label, value, max }) {
...
function TreeNode986({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode986 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState986 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer986(state, action) {
...
export default function Component986({ seed = 986, title = 'Component986' }) {
  const hook = useLogic436(seed + 986);
  const [state, dispatch] = useReducer(reducer986, initialState986);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm986(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset436(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState436(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree436(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute436(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast436(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies436(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution436(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow436(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets436(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap436(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline436(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm986(form);
    setErrors(found);
  ...
  return (
    <section className="component-986">
      <div className="header-986">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-986">
      ...
      <div className="grid-986">
        <div className="panel-986">
          {cards.map((item, idx) => (
            <MetricCard986 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
