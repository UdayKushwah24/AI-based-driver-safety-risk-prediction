...
import '../styles/component515.css';

function MetricCard515({ label, value, max }) {
...
function TreeNode515({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode515 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component515({ seed = 515, title = 'Component515' }) {
  const hook = useLogic515(seed + 515);
  const [state, dispatch] = useReducer(reducer515, initialState515);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm515(seed));
  ...
  const table = useMemo(() => buildTableState515(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm515(form);
    setErrors(found);
  ...
  return (
    <section className="component-515">
      <div className="header-515">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-515">
      ...
      <div className="grid-515">
        <div className="panel-515">
          {cards.map((item, idx) => (
            <MetricCard515 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
