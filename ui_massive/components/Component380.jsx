...
import '../styles/component380.css';

function MetricCard380({ label, value, max }) {
...
function TreeNode380({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode380 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState380 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer380(state, action) {
...
export default function Component380({ seed = 380, title = 'Component380' }) {
  const hook = useLogic380(seed + 380);
  const [state, dispatch] = useReducer(reducer380, initialState380);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm380(seed));
  ...
  const table = useMemo(() => buildTableState380(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm380(form);
    setErrors(found);
  ...
  return (
    <section className="component-380">
      <div className="header-380">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-380">
      ...
      <div className="grid-380">
        <div className="panel-380">
          {cards.map((item, idx) => (
            <MetricCard380 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
