...
import '../styles/component356.css';

function MetricCard356({ label, value, max }) {
...
function TreeNode356({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode356 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState356 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer356(state, action) {
...
export default function Component356({ seed = 356, title = 'Component356' }) {
  const hook = useLogic356(seed + 356);
  const [state, dispatch] = useReducer(reducer356, initialState356);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm356(seed));
  ...
  const table = useMemo(() => buildTableState356(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm356(form);
    setErrors(found);
  ...
  return (
    <section className="component-356">
      <div className="header-356">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-356">
      ...
      <div className="grid-356">
        <div className="panel-356">
          {cards.map((item, idx) => (
            <MetricCard356 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
