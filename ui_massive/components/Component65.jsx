...
import '../styles/component65.css';

function MetricCard65({ label, value, max }) {
...
function TreeNode65({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode65 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component65({ seed = 65, title = 'Component65' }) {
  const hook = useLogic65(seed + 65);
  const [state, dispatch] = useReducer(reducer65, initialState65);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm65(seed));
  ...
  const table = useMemo(() => buildTableState65(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm65(form);
    setErrors(found);
  ...
  return (
    <section className="component-65">
      ...
      <div className="grid-65">
        <div className="panel-65">
          {cards.map((item, idx) => (
            <MetricCard65 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
