...
import { useLogic41 } from '../hooks/useLogic41.js';
import { buildNestedTree41, buildTableState41, heavyCompute41, transformDataset41 } from '../utils/compute41.js';
import { buildForecast41, detectAnomalies41, segmentDistribution41 } from '../utils/compute41.js';
import { buildActivityMap41, rankBuckets41, summarizePipeline41, synthesizeFlow41 } from '../pipelines/pipeline41.js';
import '../styles/component591.css';

function MetricCard591({ label, value, max }) {
...
function TreeNode591({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode591 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState591 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer591(state, action) {
...
export default function Component591({ seed = 591, title = 'Component591' }) {
  const hook = useLogic41(seed + 591);
  const [state, dispatch] = useReducer(reducer591, initialState591);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm591(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset41(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState41(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree41(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute41(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast41(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies41(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution41(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow41(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets41(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap41(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline41(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm591(form);
    setErrors(found);
  ...
  return (
    <section className="component-591">
      <div className="header-591">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-591">
      ...
      <div className="grid-591">
        <div className="panel-591">
          {cards.map((item, idx) => (
            <MetricCard591 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
