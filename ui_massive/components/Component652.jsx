...
import '../styles/component652.css';

function MetricCard652({ label, value, max }) {
...
function TreeNode652({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode652 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState652 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer652(state, action) {
...
export default function Component652({ seed = 652, title = 'Component652' }) {
  const hook = useLogic102(seed + 652);
  const [state, dispatch] = useReducer(reducer652, initialState652);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm652(seed));
  ...
  const table = useMemo(() => buildTableState102(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm652(form);
    setErrors(found);
  ...
  return (
    <section className="component-652">
      <div className="header-652">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-652">
      ...
      <div className="grid-652">
        <div className="panel-652">
          {cards.map((item, idx) => (
            <MetricCard652 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
