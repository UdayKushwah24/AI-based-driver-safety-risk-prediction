...
import { useLogic36 } from '../hooks/useLogic36.js';
import { buildNestedTree36, buildTableState36, heavyCompute36, transformDataset36 } from '../utils/compute36.js';
import { buildForecast36, detectAnomalies36, segmentDistribution36 } from '../utils/compute36.js';
import { buildActivityMap36, rankBuckets36, summarizePipeline36, synthesizeFlow36 } from '../pipelines/pipeline36.js';
import '../styles/component586.css';

function MetricCard586({ label, value, max }) {
...
function TreeNode586({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode586 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState586 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer586(state, action) {
...
export default function Component586({ seed = 586, title = 'Component586' }) {
  const hook = useLogic36(seed + 586);
  const [state, dispatch] = useReducer(reducer586, initialState586);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm586(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset36(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState36(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree36(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute36(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast36(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies36(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution36(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow36(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets36(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap36(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline36(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm586(form);
    setErrors(found);
  ...
  return (
    <section className="component-586">
      <div className="header-586">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-586">
      ...
      <div className="grid-586">
        <div className="panel-586">
          {cards.map((item, idx) => (
            <MetricCard586 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
