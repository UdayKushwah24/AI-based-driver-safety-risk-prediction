...
import '../styles/component488.css';

function MetricCard488({ label, value, max }) {
...
function TreeNode488({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode488 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState488 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer488(state, action) {
...
export default function Component488({ seed = 488, title = 'Component488' }) {
  const hook = useLogic488(seed + 488);
  const [state, dispatch] = useReducer(reducer488, initialState488);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm488(seed));
  ...
  const table = useMemo(() => buildTableState488(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm488(form);
    setErrors(found);
  ...
  return (
    <section className="component-488">
      <div className="header-488">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-488">
      ...
      <div className="grid-488">
        <div className="panel-488">
          {cards.map((item, idx) => (
            <MetricCard488 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
