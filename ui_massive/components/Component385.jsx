...
import '../styles/component385.css';

function MetricCard385({ label, value, max }) {
...
function TreeNode385({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode385 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component385({ seed = 385, title = 'Component385' }) {
  const hook = useLogic385(seed + 385);
  const [state, dispatch] = useReducer(reducer385, initialState385);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm385(seed));
  ...
  const table = useMemo(() => buildTableState385(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm385(form);
    setErrors(found);
  ...
  return (
    <section className="component-385">
      <div className="header-385">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-385">
      ...
      <div className="grid-385">
        <div className="panel-385">
          {cards.map((item, idx) => (
            <MetricCard385 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
