...
import { useLogic31 } from '../hooks/useLogic31.js';
import { buildNestedTree31, buildTableState31, heavyCompute31, transformDataset31 } from '../utils/compute31.js';
import { buildForecast31, detectAnomalies31, segmentDistribution31 } from '../utils/compute31.js';
import { buildActivityMap31, rankBuckets31, summarizePipeline31, synthesizeFlow31 } from '../pipelines/pipeline31.js';
import '../styles/component581.css';

function MetricCard581({ label, value, max }) {
...
function TreeNode581({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode581 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState581 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer581(state, action) {
...
export default function Component581({ seed = 581, title = 'Component581' }) {
  const hook = useLogic31(seed + 581);
  const [state, dispatch] = useReducer(reducer581, initialState581);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm581(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset31(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState31(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree31(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute31(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast31(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies31(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution31(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow31(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets31(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap31(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline31(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm581(form);
    setErrors(found);
  ...
  return (
    <section className="component-581">
      <div className="header-581">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-581">
      ...
      <div className="grid-581">
        <div className="panel-581">
          {cards.map((item, idx) => (
            <MetricCard581 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
