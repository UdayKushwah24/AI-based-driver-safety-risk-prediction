...
import { useLogic225 } from '../hooks/useLogic225.js';
import { buildNestedTree225, buildTableState225, heavyCompute225, transformDataset225 } from '../utils/compute225.js';
import { buildForecast225, detectAnomalies225, segmentDistribution225 } from '../utils/compute225.js';
import { buildActivityMap225, rankBuckets225, summarizePipeline225, synthesizeFlow225 } from '../pipelines/pipeline225.js';
import '../styles/component225.css';

function MetricCard225({ label, value, max }) {
...
function TreeNode225({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode225 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState225 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer225(state, action) {
...
export default function Component225({ seed = 225, title = 'Component225' }) {
  const hook = useLogic225(seed + 225);
  const [state, dispatch] = useReducer(reducer225, initialState225);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm225(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset225(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState225(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree225(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute225(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast225(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies225(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution225(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow225(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets225(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap225(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline225(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm225(form);
    setErrors(found);
  ...
  return (
    <section className="component-225">
      <div className="header-225">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-225">
      ...
      <div className="grid-225">
        <div className="panel-225">
          {cards.map((item, idx) => (
            <MetricCard225 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
