...
import { useLogic144 } from '../hooks/useLogic144.js';
import { buildNestedTree144, buildTableState144, heavyCompute144, transformDataset144 } from '../utils/compute144.js';
import { buildForecast144, detectAnomalies144, segmentDistribution144 } from '../utils/compute144.js';
import { buildActivityMap144, rankBuckets144, summarizePipeline144, synthesizeFlow144 } from '../pipelines/pipeline144.js';
import '../styles/component694.css';

function MetricCard694({ label, value, max }) {
...
function TreeNode694({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode694 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState694 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer694(state, action) {
...
export default function Component694({ seed = 694, title = 'Component694' }) {
  const hook = useLogic144(seed + 694);
  const [state, dispatch] = useReducer(reducer694, initialState694);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm694(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset144(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState144(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree144(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute144(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast144(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies144(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution144(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow144(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets144(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap144(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline144(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm694(form);
    setErrors(found);
  ...
  return (
    <section className="component-694">
      <div className="header-694">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-694">
      ...
      <div className="grid-694">
        <div className="panel-694">
          {cards.map((item, idx) => (
            <MetricCard694 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
