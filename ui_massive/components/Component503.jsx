...
import '../styles/component503.css';

function MetricCard503({ label, value, max }) {
...
function TreeNode503({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode503 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState503 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer503(state, action) {
...
export default function Component503({ seed = 503, title = 'Component503' }) {
  const hook = useLogic503(seed + 503);
  const [state, dispatch] = useReducer(reducer503, initialState503);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm503(seed));
  ...
  const table = useMemo(() => buildTableState503(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm503(form);
    setErrors(found);
  ...
  return (
    <section className="component-503">
      <div className="header-503">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-503">
      ...
      <div className="grid-503">
        <div className="panel-503">
          {cards.map((item, idx) => (
            <MetricCard503 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
