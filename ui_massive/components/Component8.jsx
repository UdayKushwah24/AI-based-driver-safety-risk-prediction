...
function MetricCard8({ label, value, max }) {
...
function TreeNode8({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode8 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component8({ seed = 8, title = 'Component8' }) {
  const hook = useLogic8(seed + 8);
  const [state, dispatch] = useReducer(reducer8, initialState8);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm8(seed));
  ...
  const table = useMemo(() => buildTableState8(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm8(form);
    setErrors(found);
    ...
    <section className="component-8">
      ...
      <div className="grid-8">
        <div className="panel-8">
          {cards.map((item, idx) => (
            <MetricCard8 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
