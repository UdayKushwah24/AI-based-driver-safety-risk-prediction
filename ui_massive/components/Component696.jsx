...
import '../styles/component696.css';

function MetricCard696({ label, value, max }) {
...
function TreeNode696({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode696 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState696 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer696(state, action) {
...
export default function Component696({ seed = 696, title = 'Component696' }) {
  const hook = useLogic146(seed + 696);
  const [state, dispatch] = useReducer(reducer696, initialState696);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm696(seed));
  ...
  const table = useMemo(() => buildTableState146(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm696(form);
    setErrors(found);
  ...
  return (
    <section className="component-696">
      <div className="header-696">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-696">
      ...
      <div className="grid-696">
        <div className="panel-696">
          {cards.map((item, idx) => (
            <MetricCard696 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
