...
import '../styles/component860.css';

function MetricCard860({ label, value, max }) {
...
function TreeNode860({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode860 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState860 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer860(state, action) {
...
export default function Component860({ seed = 860, title = 'Component860' }) {
  const hook = useLogic310(seed + 860);
  const [state, dispatch] = useReducer(reducer860, initialState860);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm860(seed));
  ...
  const table = useMemo(() => buildTableState310(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm860(form);
    setErrors(found);
  ...
  return (
    <section className="component-860">
      <div className="header-860">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-860">
      ...
      <div className="grid-860">
        <div className="panel-860">
          {cards.map((item, idx) => (
            <MetricCard860 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
