...
import { useLogic16 } from '../hooks/useLogic16.js';
import { buildNestedTree16, buildTableState16, heavyCompute16, transformDataset16 } from '../utils/compute16.js';
import { buildForecast16, detectAnomalies16, segmentDistribution16 } from '../utils/compute16.js';
import { buildActivityMap16, rankBuckets16, summarizePipeline16, synthesizeFlow16 } from '../pipelines/pipeline16.js';
import '../styles/component566.css';

function MetricCard566({ label, value, max }) {
...
function TreeNode566({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode566 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState566 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer566(state, action) {
...
export default function Component566({ seed = 566, title = 'Component566' }) {
  const hook = useLogic16(seed + 566);
  const [state, dispatch] = useReducer(reducer566, initialState566);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm566(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset16(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState16(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree16(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute16(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast16(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies16(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution16(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow16(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets16(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap16(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline16(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm566(form);
    setErrors(found);
  ...
  return (
    <section className="component-566">
      <div className="header-566">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-566">
      ...
      <div className="grid-566">
        <div className="panel-566">
          {cards.map((item, idx) => (
            <MetricCard566 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
