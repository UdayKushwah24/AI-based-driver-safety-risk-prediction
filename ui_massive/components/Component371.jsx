...
import '../styles/component371.css';

function MetricCard371({ label, value, max }) {
...
function TreeNode371({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode371 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState371 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer371(state, action) {
...
export default function Component371({ seed = 371, title = 'Component371' }) {
  const hook = useLogic371(seed + 371);
  const [state, dispatch] = useReducer(reducer371, initialState371);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm371(seed));
  ...
  const table = useMemo(() => buildTableState371(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm371(form);
    setErrors(found);
  ...
  return (
    <section className="component-371">
      <div className="header-371">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-371">
      ...
      <div className="grid-371">
        <div className="panel-371">
          {cards.map((item, idx) => (
            <MetricCard371 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
