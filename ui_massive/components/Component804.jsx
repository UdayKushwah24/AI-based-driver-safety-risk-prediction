...
import '../styles/component804.css';

function MetricCard804({ label, value, max }) {
...
function TreeNode804({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode804 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState804 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer804(state, action) {
...
export default function Component804({ seed = 804, title = 'Component804' }) {
  const hook = useLogic254(seed + 804);
  const [state, dispatch] = useReducer(reducer804, initialState804);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm804(seed));
  ...
  const table = useMemo(() => buildTableState254(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm804(form);
    setErrors(found);
  ...
  return (
    <section className="component-804">
      <div className="header-804">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-804">
      ...
      <div className="grid-804">
        <div className="panel-804">
          {cards.map((item, idx) => (
            <MetricCard804 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
