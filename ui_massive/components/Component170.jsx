...
import { useLogic170 } from '../hooks/useLogic170.js';
import { buildNestedTree170, buildTableState170, heavyCompute170, transformDataset170 } from '../utils/compute170.js';
import { buildForecast170, detectAnomalies170, segmentDistribution170 } from '../utils/compute170.js';
import { buildActivityMap170, rankBuckets170, summarizePipeline170, synthesizeFlow170 } from '../pipelines/pipeline170.js';
import '../styles/component170.css';

function MetricCard170({ label, value, max }) {
...
function TreeNode170({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode170 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState170 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer170(state, action) {
...
export default function Component170({ seed = 170, title = 'Component170' }) {
  const hook = useLogic170(seed + 170);
  const [state, dispatch] = useReducer(reducer170, initialState170);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm170(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset170(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState170(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree170(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute170(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast170(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies170(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution170(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow170(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets170(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap170(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline170(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm170(form);
    setErrors(found);
  ...
  return (
    <section className="component-170">
      <div className="header-170">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-170">
      ...
      <div className="grid-170">
        <div className="panel-170">
          {cards.map((item, idx) => (
            <MetricCard170 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
