...
import '../styles/component292.css';

function MetricCard292({ label, value, max }) {
...
function TreeNode292({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode292 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState292 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer292(state, action) {
...
export default function Component292({ seed = 292, title = 'Component292' }) {
  const hook = useLogic292(seed + 292);
  const [state, dispatch] = useReducer(reducer292, initialState292);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm292(seed));
  ...
  const table = useMemo(() => buildTableState292(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm292(form);
    setErrors(found);
  ...
  return (
    <section className="component-292">
      <div className="header-292">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-292">
      ...
      <div className="grid-292">
        <div className="panel-292">
          {cards.map((item, idx) => (
            <MetricCard292 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
