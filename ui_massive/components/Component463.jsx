...
import '../styles/component463.css';

function MetricCard463({ label, value, max }) {
...
function TreeNode463({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode463 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState463 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer463(state, action) {
...
export default function Component463({ seed = 463, title = 'Component463' }) {
  const hook = useLogic463(seed + 463);
  const [state, dispatch] = useReducer(reducer463, initialState463);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm463(seed));
  ...
  const table = useMemo(() => buildTableState463(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm463(form);
    setErrors(found);
  ...
  return (
    <section className="component-463">
      <div className="header-463">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-463">
      ...
      <div className="grid-463">
        <div className="panel-463">
          {cards.map((item, idx) => (
            <MetricCard463 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
