...
import '../styles/component763.css';

function MetricCard763({ label, value, max }) {
...
function TreeNode763({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode763 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState763 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer763(state, action) {
...
export default function Component763({ seed = 763, title = 'Component763' }) {
  const hook = useLogic213(seed + 763);
  const [state, dispatch] = useReducer(reducer763, initialState763);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm763(seed));
  ...
  const table = useMemo(() => buildTableState213(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm763(form);
    setErrors(found);
  ...
  return (
    <section className="component-763">
      <div className="header-763">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-763">
      ...
      <div className="grid-763">
        <div className="panel-763">
          {cards.map((item, idx) => (
            <MetricCard763 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
