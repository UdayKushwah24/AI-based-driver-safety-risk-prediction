...
import { useLogic15 } from '../hooks/useLogic15.js';
import { buildNestedTree15, buildTableState15, heavyCompute15, transformDataset15 } from '../utils/compute15.js';
import { buildForecast15, detectAnomalies15, segmentDistribution15 } from '../utils/compute15.js';
import { buildActivityMap15, rankBuckets15, summarizePipeline15, synthesizeFlow15 } from '../pipelines/pipeline15.js';
import '../styles/component565.css';

function MetricCard565({ label, value, max }) {
...
function TreeNode565({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode565 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState565 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer565(state, action) {
...
export default function Component565({ seed = 565, title = 'Component565' }) {
  const hook = useLogic15(seed + 565);
  const [state, dispatch] = useReducer(reducer565, initialState565);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm565(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset15(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState15(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree15(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute15(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast15(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies15(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution15(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow15(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets15(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap15(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline15(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm565(form);
    setErrors(found);
  ...
  return (
    <section className="component-565">
      <div className="header-565">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-565">
      ...
      <div className="grid-565">
        <div className="panel-565">
          {cards.map((item, idx) => (
            <MetricCard565 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
