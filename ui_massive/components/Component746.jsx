...
import { useLogic196 } from '../hooks/useLogic196.js';
import { buildNestedTree196, buildTableState196, heavyCompute196, transformDataset196 } from '../utils/compute196.js';
import { buildForecast196, detectAnomalies196, segmentDistribution196 } from '../utils/compute196.js';
import { buildActivityMap196, rankBuckets196, summarizePipeline196, synthesizeFlow196 } from '../pipelines/pipeline196.js';
import '../styles/component746.css';

function MetricCard746({ label, value, max }) {
...
function TreeNode746({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode746 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState746 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer746(state, action) {
...
export default function Component746({ seed = 746, title = 'Component746' }) {
  const hook = useLogic196(seed + 746);
  const [state, dispatch] = useReducer(reducer746, initialState746);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm746(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset196(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState196(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree196(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute196(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast196(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies196(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution196(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow196(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets196(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap196(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline196(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm746(form);
    setErrors(found);
  ...
  return (
    <section className="component-746">
      <div className="header-746">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-746">
      ...
      <div className="grid-746">
        <div className="panel-746">
          {cards.map((item, idx) => (
            <MetricCard746 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
