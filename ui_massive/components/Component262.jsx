...
import { useLogic262 } from '../hooks/useLogic262.js';
import { buildNestedTree262, buildTableState262, heavyCompute262, transformDataset262 } from '../utils/compute262.js';
import { buildForecast262, detectAnomalies262, segmentDistribution262 } from '../utils/compute262.js';
import { buildActivityMap262, rankBuckets262, summarizePipeline262, synthesizeFlow262 } from '../pipelines/pipeline262.js';
import '../styles/component262.css';

function MetricCard262({ label, value, max }) {
...
function TreeNode262({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode262 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState262 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer262(state, action) {
...
export default function Component262({ seed = 262, title = 'Component262' }) {
  const hook = useLogic262(seed + 262);
  const [state, dispatch] = useReducer(reducer262, initialState262);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm262(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset262(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState262(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree262(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute262(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast262(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies262(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution262(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow262(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets262(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap262(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline262(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm262(form);
    setErrors(found);
  ...
  return (
    <section className="component-262">
      <div className="header-262">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-262">
      ...
      <div className="grid-262">
        <div className="panel-262">
          {cards.map((item, idx) => (
            <MetricCard262 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
