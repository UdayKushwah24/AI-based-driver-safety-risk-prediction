...
import '../styles/component187.css';

function MetricCard187({ label, value, max }) {
...
function TreeNode187({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode187 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState187 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer187(state, action) {
...
export default function Component187({ seed = 187, title = 'Component187' }) {
  const hook = useLogic187(seed + 187);
  const [state, dispatch] = useReducer(reducer187, initialState187);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm187(seed));
  ...
  const table = useMemo(() => buildTableState187(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm187(form);
    setErrors(found);
  ...
  return (
    <section className="component-187">
      <div className="header-187">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-187">
      ...
      <div className="grid-187">
        <div className="panel-187">
          {cards.map((item, idx) => (
            <MetricCard187 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
