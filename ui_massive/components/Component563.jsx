...
import { useLogic13 } from '../hooks/useLogic13.js';
import { buildNestedTree13, buildTableState13, heavyCompute13, transformDataset13 } from '../utils/compute13.js';
import { buildForecast13, detectAnomalies13, segmentDistribution13 } from '../utils/compute13.js';
import { buildActivityMap13, rankBuckets13, summarizePipeline13, synthesizeFlow13 } from '../pipelines/pipeline13.js';
import '../styles/component563.css';

function MetricCard563({ label, value, max }) {
...
function TreeNode563({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode563 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState563 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer563(state, action) {
...
export default function Component563({ seed = 563, title = 'Component563' }) {
  const hook = useLogic13(seed + 563);
  const [state, dispatch] = useReducer(reducer563, initialState563);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm563(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset13(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState13(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree13(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute13(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast13(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies13(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution13(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow13(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets13(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap13(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline13(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm563(form);
    setErrors(found);
  ...
  return (
    <section className="component-563">
      <div className="header-563">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-563">
      ...
      <div className="grid-563">
        <div className="panel-563">
          {cards.map((item, idx) => (
            <MetricCard563 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
