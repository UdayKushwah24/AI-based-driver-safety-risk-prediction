...
import '../styles/component42.css';

function MetricCard42({ label, value, max }) {
...
function TreeNode42({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode42 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component42({ seed = 42, title = 'Component42' }) {
  const hook = useLogic42(seed + 42);
  const [state, dispatch] = useReducer(reducer42, initialState42);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm42(seed));
  ...
  const table = useMemo(() => buildTableState42(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm42(form);
    setErrors(found);
  ...
  return (
    <section className="component-42">
      ...
      <div className="grid-42">
        <div className="panel-42">
          {cards.map((item, idx) => (
            <MetricCard42 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
