...
import '../styles/component176.css';

function MetricCard176({ label, value, max }) {
...
function TreeNode176({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode176 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState176 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer176(state, action) {
...
export default function Component176({ seed = 176, title = 'Component176' }) {
  const hook = useLogic176(seed + 176);
  const [state, dispatch] = useReducer(reducer176, initialState176);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm176(seed));
  ...
  const table = useMemo(() => buildTableState176(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm176(form);
    setErrors(found);
  ...
  return (
    <section className="component-176">
      <div className="header-176">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-176">
      ...
      <div className="grid-176">
        <div className="panel-176">
          {cards.map((item, idx) => (
            <MetricCard176 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
