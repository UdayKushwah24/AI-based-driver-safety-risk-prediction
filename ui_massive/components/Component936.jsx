...
import '../styles/component936.css';

function MetricCard936({ label, value, max }) {
...
function TreeNode936({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode936 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState936 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer936(state, action) {
...
export default function Component936({ seed = 936, title = 'Component936' }) {
  const hook = useLogic386(seed + 936);
  const [state, dispatch] = useReducer(reducer936, initialState936);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm936(seed));
  ...
  const table = useMemo(() => buildTableState386(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm936(form);
    setErrors(found);
  ...
  return (
    <section className="component-936">
      <div className="header-936">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-936">
      ...
      <div className="grid-936">
        <div className="panel-936">
          {cards.map((item, idx) => (
            <MetricCard936 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
