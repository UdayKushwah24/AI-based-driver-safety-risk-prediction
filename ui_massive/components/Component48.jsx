...
import '../styles/component48.css';

function MetricCard48({ label, value, max }) {
...
function TreeNode48({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode48 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component48({ seed = 48, title = 'Component48' }) {
  const hook = useLogic48(seed + 48);
  const [state, dispatch] = useReducer(reducer48, initialState48);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm48(seed));
  ...
  const table = useMemo(() => buildTableState48(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm48(form);
    setErrors(found);
  ...
  return (
    <section className="component-48">
      ...
      <div className="grid-48">
        <div className="panel-48">
          {cards.map((item, idx) => (
            <MetricCard48 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
