...
import '../styles/component501.css';

function MetricCard501({ label, value, max }) {
...
function TreeNode501({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode501 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState501 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer501(state, action) {
...
export default function Component501({ seed = 501, title = 'Component501' }) {
  const hook = useLogic501(seed + 501);
  const [state, dispatch] = useReducer(reducer501, initialState501);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm501(seed));
  ...
  const table = useMemo(() => buildTableState501(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm501(form);
    setErrors(found);
  ...
  return (
    <section className="component-501">
      <div className="header-501">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-501">
      ...
      <div className="grid-501">
        <div className="panel-501">
          {cards.map((item, idx) => (
            <MetricCard501 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
