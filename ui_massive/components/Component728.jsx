...
import { useLogic178 } from '../hooks/useLogic178.js';
import { buildNestedTree178, buildTableState178, heavyCompute178, transformDataset178 } from '../utils/compute178.js';
import { buildForecast178, detectAnomalies178, segmentDistribution178 } from '../utils/compute178.js';
import { buildActivityMap178, rankBuckets178, summarizePipeline178, synthesizeFlow178 } from '../pipelines/pipeline178.js';
import '../styles/component728.css';

function MetricCard728({ label, value, max }) {
...
function TreeNode728({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode728 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState728 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer728(state, action) {
...
export default function Component728({ seed = 728, title = 'Component728' }) {
  const hook = useLogic178(seed + 728);
  const [state, dispatch] = useReducer(reducer728, initialState728);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm728(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset178(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState178(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree178(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute178(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast178(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies178(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution178(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow178(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets178(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap178(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline178(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm728(form);
    setErrors(found);
  ...
  return (
    <section className="component-728">
      <div className="header-728">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-728">
      ...
      <div className="grid-728">
        <div className="panel-728">
          {cards.map((item, idx) => (
            <MetricCard728 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
