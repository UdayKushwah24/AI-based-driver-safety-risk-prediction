...
import '../styles/component961.css';

function MetricCard961({ label, value, max }) {
...
function TreeNode961({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode961 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState961 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer961(state, action) {
...
export default function Component961({ seed = 961, title = 'Component961' }) {
  const hook = useLogic411(seed + 961);
  const [state, dispatch] = useReducer(reducer961, initialState961);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm961(seed));
  ...
  const table = useMemo(() => buildTableState411(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm961(form);
    setErrors(found);
  ...
  return (
    <section className="component-961">
      <div className="header-961">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-961">
      ...
      <div className="grid-961">
        <div className="panel-961">
          {cards.map((item, idx) => (
            <MetricCard961 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
