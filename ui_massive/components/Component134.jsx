...
import { useLogic134 } from '../hooks/useLogic134.js';
import { buildNestedTree134, buildTableState134, heavyCompute134, transformDataset134 } from '../utils/compute134.js';
import { buildForecast134, detectAnomalies134, segmentDistribution134 } from '../utils/compute134.js';
import { buildActivityMap134, rankBuckets134, summarizePipeline134, synthesizeFlow134 } from '../pipelines/pipeline134.js';
import '../styles/component134.css';

function MetricCard134({ label, value, max }) {
...
function TreeNode134({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode134 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState134 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer134(state, action) {
...
export default function Component134({ seed = 134, title = 'Component134' }) {
  const hook = useLogic134(seed + 134);
  const [state, dispatch] = useReducer(reducer134, initialState134);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm134(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset134(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState134(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree134(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute134(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast134(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies134(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution134(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow134(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets134(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap134(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline134(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm134(form);
    setErrors(found);
  ...
  return (
    <section className="component-134">
      <div className="header-134">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-134">
      ...
      <div className="grid-134">
        <div className="panel-134">
          {cards.map((item, idx) => (
            <MetricCard134 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
