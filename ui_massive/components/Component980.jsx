...
import '../styles/component980.css';

function MetricCard980({ label, value, max }) {
...
function TreeNode980({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode980 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState980 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer980(state, action) {
...
export default function Component980({ seed = 980, title = 'Component980' }) {
  const hook = useLogic430(seed + 980);
  const [state, dispatch] = useReducer(reducer980, initialState980);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm980(seed));
  ...
  const table = useMemo(() => buildTableState430(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm980(form);
    setErrors(found);
  ...
  return (
    <section className="component-980">
      <div className="header-980">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-980">
      ...
      <div className="grid-980">
        <div className="panel-980">
          {cards.map((item, idx) => (
            <MetricCard980 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
