...
import '../styles/component60.css';

function MetricCard60({ label, value, max }) {
...
function TreeNode60({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode60 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component60({ seed = 60, title = 'Component60' }) {
  const hook = useLogic60(seed + 60);
  const [state, dispatch] = useReducer(reducer60, initialState60);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm60(seed));
  ...
  const table = useMemo(() => buildTableState60(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm60(form);
    setErrors(found);
  ...
  return (
    <section className="component-60">
      ...
      <div className="grid-60">
        <div className="panel-60">
          {cards.map((item, idx) => (
            <MetricCard60 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
