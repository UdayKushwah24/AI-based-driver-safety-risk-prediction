...
import { useLogic108 } from '../hooks/useLogic108.js';
import { buildNestedTree108, buildTableState108, heavyCompute108, transformDataset108 } from '../utils/compute108.js';
import { buildForecast108, detectAnomalies108, segmentDistribution108 } from '../utils/compute108.js';
import { buildActivityMap108, rankBuckets108, summarizePipeline108, synthesizeFlow108 } from '../pipelines/pipeline108.js';
import '../styles/component658.css';

function MetricCard658({ label, value, max }) {
...
function TreeNode658({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode658 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState658 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer658(state, action) {
...
export default function Component658({ seed = 658, title = 'Component658' }) {
  const hook = useLogic108(seed + 658);
  const [state, dispatch] = useReducer(reducer658, initialState658);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm658(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset108(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState108(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree108(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute108(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast108(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies108(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution108(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow108(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets108(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap108(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline108(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm658(form);
    setErrors(found);
  ...
  return (
    <section className="component-658">
      <div className="header-658">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-658">
      ...
      <div className="grid-658">
        <div className="panel-658">
          {cards.map((item, idx) => (
            <MetricCard658 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
