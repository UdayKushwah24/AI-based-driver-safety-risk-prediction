...
import '../styles/component75.css';

function MetricCard75({ label, value, max }) {
...
function TreeNode75({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode75 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component75({ seed = 75, title = 'Component75' }) {
  const hook = useLogic75(seed + 75);
  const [state, dispatch] = useReducer(reducer75, initialState75);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm75(seed));
  ...
  const table = useMemo(() => buildTableState75(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm75(form);
    setErrors(found);
  ...
  return (
    <section className="component-75">
      ...
      <div className="grid-75">
        <div className="panel-75">
          {cards.map((item, idx) => (
            <MetricCard75 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
