...
import '../styles/component62.css';

function MetricCard62({ label, value, max }) {
...
function TreeNode62({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode62 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component62({ seed = 62, title = 'Component62' }) {
  const hook = useLogic62(seed + 62);
  const [state, dispatch] = useReducer(reducer62, initialState62);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm62(seed));
  ...
  const table = useMemo(() => buildTableState62(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm62(form);
    setErrors(found);
  ...
  return (
    <section className="component-62">
      ...
      <div className="grid-62">
        <div className="panel-62">
          {cards.map((item, idx) => (
            <MetricCard62 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
