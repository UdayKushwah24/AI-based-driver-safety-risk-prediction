...
import '../styles/component303.css';

function MetricCard303({ label, value, max }) {
...
function TreeNode303({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode303 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState303 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer303(state, action) {
...
export default function Component303({ seed = 303, title = 'Component303' }) {
  const hook = useLogic303(seed + 303);
  const [state, dispatch] = useReducer(reducer303, initialState303);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm303(seed));
  ...
  const table = useMemo(() => buildTableState303(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm303(form);
    setErrors(found);
  ...
  return (
    <section className="component-303">
      <div className="header-303">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-303">
      ...
      <div className="grid-303">
        <div className="panel-303">
          {cards.map((item, idx) => (
            <MetricCard303 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
