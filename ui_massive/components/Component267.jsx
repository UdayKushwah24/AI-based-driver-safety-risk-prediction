...
import '../styles/component267.css';

function MetricCard267({ label, value, max }) {
...
function TreeNode267({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode267 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState267 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer267(state, action) {
...
export default function Component267({ seed = 267, title = 'Component267' }) {
  const hook = useLogic267(seed + 267);
  const [state, dispatch] = useReducer(reducer267, initialState267);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm267(seed));
  ...
  const table = useMemo(() => buildTableState267(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm267(form);
    setErrors(found);
  ...
  return (
    <section className="component-267">
      <div className="header-267">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-267">
      ...
      <div className="grid-267">
        <div className="panel-267">
          {cards.map((item, idx) => (
            <MetricCard267 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
