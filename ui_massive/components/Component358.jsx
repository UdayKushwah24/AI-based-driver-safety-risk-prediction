...
import { useLogic358 } from '../hooks/useLogic358.js';
import { buildNestedTree358, buildTableState358, heavyCompute358, transformDataset358 } from '../utils/compute358.js';
import { buildForecast358, detectAnomalies358, segmentDistribution358 } from '../utils/compute358.js';
import { buildActivityMap358, rankBuckets358, summarizePipeline358, synthesizeFlow358 } from '../pipelines/pipeline358.js';
import '../styles/component358.css';

function MetricCard358({ label, value, max }) {
...
function TreeNode358({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode358 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState358 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer358(state, action) {
...
export default function Component358({ seed = 358, title = 'Component358' }) {
  const hook = useLogic358(seed + 358);
  const [state, dispatch] = useReducer(reducer358, initialState358);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm358(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset358(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState358(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree358(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute358(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast358(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies358(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution358(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow358(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets358(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap358(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline358(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm358(form);
    setErrors(found);
  ...
  return (
    <section className="component-358">
      <div className="header-358">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-358">
      ...
      <div className="grid-358">
        <div className="panel-358">
          {cards.map((item, idx) => (
            <MetricCard358 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
