...
import '../styles/component111.css';

function MetricCard111({ label, value, max }) {
...
function TreeNode111({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode111 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component111({ seed = 111, title = 'Component111' }) {
  const hook = useLogic111(seed + 111);
  const [state, dispatch] = useReducer(reducer111, initialState111);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm111(seed));
  ...
  const table = useMemo(() => buildTableState111(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm111(form);
    setErrors(found);
  ...
  return (
    <section className="component-111">
      ...
      <div className="grid-111">
        <div className="panel-111">
          {cards.map((item, idx) => (
            <MetricCard111 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
