...
import '../styles/component285.css';

function MetricCard285({ label, value, max }) {
...
function TreeNode285({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode285 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState285 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer285(state, action) {
...
export default function Component285({ seed = 285, title = 'Component285' }) {
  const hook = useLogic285(seed + 285);
  const [state, dispatch] = useReducer(reducer285, initialState285);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm285(seed));
  ...
  const table = useMemo(() => buildTableState285(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm285(form);
    setErrors(found);
  ...
  return (
    <section className="component-285">
      <div className="header-285">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-285">
      ...
      <div className="grid-285">
        <div className="panel-285">
          {cards.map((item, idx) => (
            <MetricCard285 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
