...
import { useLogic61 } from '../hooks/useLogic61.js';
import { buildNestedTree61, buildTableState61, heavyCompute61, transformDataset61 } from '../utils/compute61.js';
import { buildForecast61, detectAnomalies61, segmentDistribution61 } from '../utils/compute61.js';
import { buildActivityMap61, rankBuckets61, summarizePipeline61, synthesizeFlow61 } from '../pipelines/pipeline61.js';
import '../styles/component611.css';

function MetricCard611({ label, value, max }) {
...
function TreeNode611({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode611 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState611 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer611(state, action) {
...
export default function Component611({ seed = 611, title = 'Component611' }) {
  const hook = useLogic61(seed + 611);
  const [state, dispatch] = useReducer(reducer611, initialState611);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm611(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset61(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState61(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree61(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute61(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast61(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies61(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution61(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow61(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets61(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap61(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline61(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm611(form);
    setErrors(found);
  ...
  return (
    <section className="component-611">
      <div className="header-611">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-611">
      ...
      <div className="grid-611">
        <div className="panel-611">
          {cards.map((item, idx) => (
            <MetricCard611 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
