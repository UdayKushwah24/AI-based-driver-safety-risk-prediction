...
import { useLogic114 } from '../hooks/useLogic114.js';
import { buildNestedTree114, buildTableState114, heavyCompute114, transformDataset114 } from '../utils/compute114.js';
import { buildForecast114, detectAnomalies114, segmentDistribution114 } from '../utils/compute114.js';
import { buildActivityMap114, rankBuckets114, summarizePipeline114, synthesizeFlow114 } from '../pipelines/pipeline114.js';
import '../styles/component664.css';

function MetricCard664({ label, value, max }) {
...
function TreeNode664({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode664 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState664 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer664(state, action) {
...
export default function Component664({ seed = 664, title = 'Component664' }) {
  const hook = useLogic114(seed + 664);
  const [state, dispatch] = useReducer(reducer664, initialState664);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm664(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset114(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState114(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree114(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute114(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast114(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies114(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution114(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow114(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets114(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap114(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline114(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm664(form);
    setErrors(found);
  ...
  return (
    <section className="component-664">
      <div className="header-664">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-664">
      ...
      <div className="grid-664">
        <div className="panel-664">
          {cards.map((item, idx) => (
            <MetricCard664 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
