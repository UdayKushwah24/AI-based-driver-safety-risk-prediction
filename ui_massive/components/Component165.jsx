...
import { useLogic165 } from '../hooks/useLogic165.js';
import { buildNestedTree165, buildTableState165, heavyCompute165, transformDataset165 } from '../utils/compute165.js';
import { buildForecast165, detectAnomalies165, segmentDistribution165 } from '../utils/compute165.js';
import { buildActivityMap165, rankBuckets165, summarizePipeline165, synthesizeFlow165 } from '../pipelines/pipeline165.js';
import '../styles/component165.css';

function MetricCard165({ label, value, max }) {
...
function TreeNode165({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode165 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState165 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer165(state, action) {
...
export default function Component165({ seed = 165, title = 'Component165' }) {
  const hook = useLogic165(seed + 165);
  const [state, dispatch] = useReducer(reducer165, initialState165);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm165(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset165(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState165(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree165(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute165(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast165(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies165(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution165(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow165(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets165(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap165(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline165(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm165(form);
    setErrors(found);
  ...
  return (
    <section className="component-165">
      <div className="header-165">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-165">
      ...
      <div className="grid-165">
        <div className="panel-165">
          {cards.map((item, idx) => (
            <MetricCard165 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
