...
import { useLogic437 } from '../hooks/useLogic437.js';
import { buildNestedTree437, buildTableState437, heavyCompute437, transformDataset437 } from '../utils/compute437.js';
import { buildForecast437, detectAnomalies437, segmentDistribution437 } from '../utils/compute437.js';
import { buildActivityMap437, rankBuckets437, summarizePipeline437, synthesizeFlow437 } from '../pipelines/pipeline437.js';
import '../styles/component987.css';

function MetricCard987({ label, value, max }) {
...
function TreeNode987({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode987 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState987 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer987(state, action) {
...
export default function Component987({ seed = 987, title = 'Component987' }) {
  const hook = useLogic437(seed + 987);
  const [state, dispatch] = useReducer(reducer987, initialState987);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm987(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset437(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState437(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree437(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute437(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast437(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies437(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution437(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow437(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets437(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap437(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline437(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm987(form);
    setErrors(found);
  ...
  return (
    <section className="component-987">
      <div className="header-987">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-987">
      ...
      <div className="grid-987">
        <div className="panel-987">
          {cards.map((item, idx) => (
            <MetricCard987 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
