...
import { useLogic174 } from '../hooks/useLogic174.js';
import { buildNestedTree174, buildTableState174, heavyCompute174, transformDataset174 } from '../utils/compute174.js';
import { buildForecast174, detectAnomalies174, segmentDistribution174 } from '../utils/compute174.js';
import { buildActivityMap174, rankBuckets174, summarizePipeline174, synthesizeFlow174 } from '../pipelines/pipeline174.js';
import '../styles/component174.css';

function MetricCard174({ label, value, max }) {
...
function TreeNode174({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode174 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState174 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer174(state, action) {
...
export default function Component174({ seed = 174, title = 'Component174' }) {
  const hook = useLogic174(seed + 174);
  const [state, dispatch] = useReducer(reducer174, initialState174);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm174(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset174(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState174(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree174(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute174(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast174(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies174(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution174(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow174(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets174(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap174(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline174(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm174(form);
    setErrors(found);
  ...
  return (
    <section className="component-174">
      <div className="header-174">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-174">
      ...
      <div className="grid-174">
        <div className="panel-174">
          {cards.map((item, idx) => (
            <MetricCard174 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
