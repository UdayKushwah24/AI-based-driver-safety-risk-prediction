...
import '../styles/component335.css';

function MetricCard335({ label, value, max }) {
...
function TreeNode335({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode335 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component335({ seed = 335, title = 'Component335' }) {
  const hook = useLogic335(seed + 335);
  const [state, dispatch] = useReducer(reducer335, initialState335);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm335(seed));
  ...
  const table = useMemo(() => buildTableState335(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm335(form);
    setErrors(found);
  ...
  return (
    <section className="component-335">
      <div className="header-335">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-335">
      ...
      <div className="grid-335">
        <div className="panel-335">
          {cards.map((item, idx) => (
            <MetricCard335 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
