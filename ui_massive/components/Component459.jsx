...
import '../styles/component459.css';

function MetricCard459({ label, value, max }) {
...
function TreeNode459({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode459 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState459 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer459(state, action) {
...
export default function Component459({ seed = 459, title = 'Component459' }) {
  const hook = useLogic459(seed + 459);
  const [state, dispatch] = useReducer(reducer459, initialState459);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm459(seed));
  ...
  const table = useMemo(() => buildTableState459(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm459(form);
    setErrors(found);
  ...
  return (
    <section className="component-459">
      <div className="header-459">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-459">
      ...
      <div className="grid-459">
        <div className="panel-459">
          {cards.map((item, idx) => (
            <MetricCard459 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
