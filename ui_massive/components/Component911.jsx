...
import '../styles/component911.css';

function MetricCard911({ label, value, max }) {
...
function TreeNode911({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode911 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState911 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer911(state, action) {
...
export default function Component911({ seed = 911, title = 'Component911' }) {
  const hook = useLogic361(seed + 911);
  const [state, dispatch] = useReducer(reducer911, initialState911);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm911(seed));
  ...
  const table = useMemo(() => buildTableState361(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm911(form);
    setErrors(found);
  ...
  return (
    <section className="component-911">
      <div className="header-911">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-911">
      ...
      <div className="grid-911">
        <div className="panel-911">
          {cards.map((item, idx) => (
            <MetricCard911 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
