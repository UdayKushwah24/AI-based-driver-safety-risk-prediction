...
import '../styles/component729.css';

function MetricCard729({ label, value, max }) {
...
function TreeNode729({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode729 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState729 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer729(state, action) {
...
export default function Component729({ seed = 729, title = 'Component729' }) {
  const hook = useLogic179(seed + 729);
  const [state, dispatch] = useReducer(reducer729, initialState729);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm729(seed));
  ...
  const table = useMemo(() => buildTableState179(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm729(form);
    setErrors(found);
  ...
  return (
    <section className="component-729">
      <div className="header-729">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-729">
      ...
      <div className="grid-729">
        <div className="panel-729">
          {cards.map((item, idx) => (
            <MetricCard729 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
