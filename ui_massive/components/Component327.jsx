...
import '../styles/component327.css';

function MetricCard327({ label, value, max }) {
...
function TreeNode327({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode327 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState327 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer327(state, action) {
...
export default function Component327({ seed = 327, title = 'Component327' }) {
  const hook = useLogic327(seed + 327);
  const [state, dispatch] = useReducer(reducer327, initialState327);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm327(seed));
  ...
  const table = useMemo(() => buildTableState327(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm327(form);
    setErrors(found);
  ...
  return (
    <section className="component-327">
      <div className="header-327">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-327">
      ...
      <div className="grid-327">
        <div className="panel-327">
          {cards.map((item, idx) => (
            <MetricCard327 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
