...
import '../styles/component988.css';

function MetricCard988({ label, value, max }) {
...
function TreeNode988({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode988 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState988 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer988(state, action) {
...
export default function Component988({ seed = 988, title = 'Component988' }) {
  const hook = useLogic438(seed + 988);
  const [state, dispatch] = useReducer(reducer988, initialState988);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm988(seed));
  ...
  const table = useMemo(() => buildTableState438(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm988(form);
    setErrors(found);
  ...
  return (
    <section className="component-988">
      <div className="header-988">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-988">
      ...
      <div className="grid-988">
        <div className="panel-988">
          {cards.map((item, idx) => (
            <MetricCard988 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
