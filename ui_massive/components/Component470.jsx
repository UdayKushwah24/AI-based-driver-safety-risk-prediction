...
import '../styles/component470.css';

function MetricCard470({ label, value, max }) {
...
function TreeNode470({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode470 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState470 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer470(state, action) {
...
export default function Component470({ seed = 470, title = 'Component470' }) {
  const hook = useLogic470(seed + 470);
  const [state, dispatch] = useReducer(reducer470, initialState470);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm470(seed));
  ...
  const table = useMemo(() => buildTableState470(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm470(form);
    setErrors(found);
  ...
  return (
    <section className="component-470">
      <div className="header-470">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-470">
      ...
      <div className="grid-470">
        <div className="panel-470">
          {cards.map((item, idx) => (
            <MetricCard470 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
