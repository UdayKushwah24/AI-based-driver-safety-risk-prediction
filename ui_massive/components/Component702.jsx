...
import { useLogic152 } from '../hooks/useLogic152.js';
import { buildNestedTree152, buildTableState152, heavyCompute152, transformDataset152 } from '../utils/compute152.js';
import { buildForecast152, detectAnomalies152, segmentDistribution152 } from '../utils/compute152.js';
import { buildActivityMap152, rankBuckets152, summarizePipeline152, synthesizeFlow152 } from '../pipelines/pipeline152.js';
import '../styles/component702.css';

function MetricCard702({ label, value, max }) {
...
function TreeNode702({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode702 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState702 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer702(state, action) {
...
export default function Component702({ seed = 702, title = 'Component702' }) {
  const hook = useLogic152(seed + 702);
  const [state, dispatch] = useReducer(reducer702, initialState702);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm702(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset152(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState152(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree152(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute152(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast152(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies152(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution152(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow152(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets152(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap152(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline152(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm702(form);
    setErrors(found);
  ...
  return (
    <section className="component-702">
      <div className="header-702">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-702">
      ...
      <div className="grid-702">
        <div className="panel-702">
          {cards.map((item, idx) => (
            <MetricCard702 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
