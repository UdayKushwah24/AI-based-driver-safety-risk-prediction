...
import '../styles/component28.css';

function MetricCard28({ label, value, max }) {
...
function TreeNode28({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode28 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component28({ seed = 28, title = 'Component28' }) {
  const hook = useLogic28(seed + 28);
  const [state, dispatch] = useReducer(reducer28, initialState28);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm28(seed));
  ...
  const table = useMemo(() => buildTableState28(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm28(form);
    setErrors(found);
  ...
  return (
    <section className="component-28">
      ...
      <div className="grid-28">
        <div className="panel-28">
          {cards.map((item, idx) => (
            <MetricCard28 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
