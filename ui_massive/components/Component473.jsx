...
import '../styles/component473.css';

function MetricCard473({ label, value, max }) {
...
function TreeNode473({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode473 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState473 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer473(state, action) {
...
export default function Component473({ seed = 473, title = 'Component473' }) {
  const hook = useLogic473(seed + 473);
  const [state, dispatch] = useReducer(reducer473, initialState473);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm473(seed));
  ...
  const table = useMemo(() => buildTableState473(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm473(form);
    setErrors(found);
  ...
  return (
    <section className="component-473">
      <div className="header-473">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-473">
      ...
      <div className="grid-473">
        <div className="panel-473">
          {cards.map((item, idx) => (
            <MetricCard473 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
