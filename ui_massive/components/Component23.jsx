...
import '../styles/component23.css';

function MetricCard23({ label, value, max }) {
...
function TreeNode23({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode23 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component23({ seed = 23, title = 'Component23' }) {
  const hook = useLogic23(seed + 23);
  const [state, dispatch] = useReducer(reducer23, initialState23);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm23(seed));
  ...
  const table = useMemo(() => buildTableState23(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm23(form);
    setErrors(found);
    ...
    <section className="component-23">
      ...
      <div className="grid-23">
        <div className="panel-23">
          {cards.map((item, idx) => (
            <MetricCard23 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
