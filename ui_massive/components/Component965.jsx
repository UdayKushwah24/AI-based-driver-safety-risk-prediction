...
import { useLogic415 } from '../hooks/useLogic415.js';
import { buildNestedTree415, buildTableState415, heavyCompute415, transformDataset415 } from '../utils/compute415.js';
import { buildForecast415, detectAnomalies415, segmentDistribution415 } from '../utils/compute415.js';
import { buildActivityMap415, rankBuckets415, summarizePipeline415, synthesizeFlow415 } from '../pipelines/pipeline415.js';
import '../styles/component965.css';

function MetricCard965({ label, value, max }) {
...
function TreeNode965({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode965 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState965 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer965(state, action) {
...
export default function Component965({ seed = 965, title = 'Component965' }) {
  const hook = useLogic415(seed + 965);
  const [state, dispatch] = useReducer(reducer965, initialState965);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm965(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset415(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState415(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree415(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute415(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast415(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies415(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution415(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow415(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets415(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap415(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline415(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm965(form);
    setErrors(found);
  ...
  return (
    <section className="component-965">
      <div className="header-965">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-965">
      ...
      <div className="grid-965">
        <div className="panel-965">
          {cards.map((item, idx) => (
            <MetricCard965 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
