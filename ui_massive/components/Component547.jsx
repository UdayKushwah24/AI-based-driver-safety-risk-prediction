...
import '../styles/component547.css';

function MetricCard547({ label, value, max }) {
...
function TreeNode547({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode547 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState547 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer547(state, action) {
...
export default function Component547({ seed = 547, title = 'Component547' }) {
  const hook = useLogic547(seed + 547);
  const [state, dispatch] = useReducer(reducer547, initialState547);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm547(seed));
  ...
  const table = useMemo(() => buildTableState547(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm547(form);
    setErrors(found);
  ...
  return (
    <section className="component-547">
      <div className="header-547">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-547">
      ...
      <div className="grid-547">
        <div className="panel-547">
          {cards.map((item, idx) => (
            <MetricCard547 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
