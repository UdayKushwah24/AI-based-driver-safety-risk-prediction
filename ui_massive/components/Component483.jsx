...
import '../styles/component483.css';

function MetricCard483({ label, value, max }) {
...
function TreeNode483({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode483 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState483 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer483(state, action) {
...
export default function Component483({ seed = 483, title = 'Component483' }) {
  const hook = useLogic483(seed + 483);
  const [state, dispatch] = useReducer(reducer483, initialState483);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm483(seed));
  ...
  const table = useMemo(() => buildTableState483(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm483(form);
    setErrors(found);
  ...
  return (
    <section className="component-483">
      <div className="header-483">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-483">
      ...
      <div className="grid-483">
        <div className="panel-483">
          {cards.map((item, idx) => (
            <MetricCard483 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
