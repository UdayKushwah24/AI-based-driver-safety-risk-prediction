...
import '../styles/component720.css';

function MetricCard720({ label, value, max }) {
...
function TreeNode720({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode720 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState720 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer720(state, action) {
...
export default function Component720({ seed = 720, title = 'Component720' }) {
  const hook = useLogic170(seed + 720);
  const [state, dispatch] = useReducer(reducer720, initialState720);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm720(seed));
  ...
  const table = useMemo(() => buildTableState170(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm720(form);
    setErrors(found);
  ...
  return (
    <section className="component-720">
      <div className="header-720">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-720">
      ...
      <div className="grid-720">
        <div className="panel-720">
          {cards.map((item, idx) => (
            <MetricCard720 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
