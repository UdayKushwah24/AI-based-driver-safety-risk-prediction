...
import '../styles/component660.css';

function MetricCard660({ label, value, max }) {
...
function TreeNode660({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode660 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState660 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer660(state, action) {
...
export default function Component660({ seed = 660, title = 'Component660' }) {
  const hook = useLogic110(seed + 660);
  const [state, dispatch] = useReducer(reducer660, initialState660);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm660(seed));
  ...
  const table = useMemo(() => buildTableState110(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm660(form);
    setErrors(found);
  ...
  return (
    <section className="component-660">
      <div className="header-660">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-660">
      ...
      <div className="grid-660">
        <div className="panel-660">
          {cards.map((item, idx) => (
            <MetricCard660 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
