...
import '../styles/component118.css';

function MetricCard118({ label, value, max }) {
...
function TreeNode118({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode118 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component118({ seed = 118, title = 'Component118' }) {
  const hook = useLogic118(seed + 118);
  const [state, dispatch] = useReducer(reducer118, initialState118);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm118(seed));
  ...
  const table = useMemo(() => buildTableState118(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm118(form);
    setErrors(found);
  ...
  return (
    <section className="component-118">
      ...
      <div className="grid-118">
        <div className="panel-118">
          {cards.map((item, idx) => (
            <MetricCard118 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
