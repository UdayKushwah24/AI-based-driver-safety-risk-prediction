...
import '../styles/component992.css';

function MetricCard992({ label, value, max }) {
...
function TreeNode992({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode992 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState992 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer992(state, action) {
...
export default function Component992({ seed = 992, title = 'Component992' }) {
  const hook = useLogic442(seed + 992);
  const [state, dispatch] = useReducer(reducer992, initialState992);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm992(seed));
  ...
  const table = useMemo(() => buildTableState442(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm992(form);
    setErrors(found);
  ...
  return (
    <section className="component-992">
      ...
      <div className="grid-992">
        <div className="panel-992">
          {cards.map((item, idx) => (
            <MetricCard992 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
