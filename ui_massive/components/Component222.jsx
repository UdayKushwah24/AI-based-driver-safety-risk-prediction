...
import '../styles/component222.css';

function MetricCard222({ label, value, max }) {
...
function TreeNode222({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode222 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
function reducer222(state, action) {
...
export default function Component222({ seed = 222, title = 'Component222' }) {
  const hook = useLogic222(seed + 222);
  const [state, dispatch] = useReducer(reducer222, initialState222);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm222(seed));
  ...
  const table = useMemo(() => buildTableState222(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm222(form);
    setErrors(found);
  ...
  return (
    <section className="component-222">
      <div className="header-222">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-222">
      ...
      <div className="grid-222">
        <div className="panel-222">
          {cards.map((item, idx) => (
            <MetricCard222 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
