...
import '../styles/component263.css';

function MetricCard263({ label, value, max }) {
...
function TreeNode263({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode263 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component263({ seed = 263, title = 'Component263' }) {
  const hook = useLogic263(seed + 263);
  const [state, dispatch] = useReducer(reducer263, initialState263);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm263(seed));
  ...
  const table = useMemo(() => buildTableState263(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm263(form);
    setErrors(found);
  ...
  return (
    <section className="component-263">
      <div className="header-263">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-263">
      ...
      <div className="grid-263">
        <div className="panel-263">
          {cards.map((item, idx) => (
            <MetricCard263 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
