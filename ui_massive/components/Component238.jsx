...
import { useLogic238 } from '../hooks/useLogic238.js';
import { buildNestedTree238, buildTableState238, heavyCompute238, transformDataset238 } from '../utils/compute238.js';
import { buildForecast238, detectAnomalies238, segmentDistribution238 } from '../utils/compute238.js';
import { buildActivityMap238, rankBuckets238, summarizePipeline238, synthesizeFlow238 } from '../pipelines/pipeline238.js';
import '../styles/component238.css';

function MetricCard238({ label, value, max }) {
...
function TreeNode238({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode238 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState238 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer238(state, action) {
...
export default function Component238({ seed = 238, title = 'Component238' }) {
  const hook = useLogic238(seed + 238);
  const [state, dispatch] = useReducer(reducer238, initialState238);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm238(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset238(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState238(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree238(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute238(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast238(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies238(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution238(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow238(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets238(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap238(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline238(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm238(form);
    setErrors(found);
  ...
  return (
    <section className="component-238">
      <div className="header-238">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-238">
      ...
      <div className="grid-238">
        <div className="panel-238">
          {cards.map((item, idx) => (
            <MetricCard238 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
