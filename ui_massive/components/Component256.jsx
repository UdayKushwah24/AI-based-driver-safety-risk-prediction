...
import '../styles/component256.css';

function MetricCard256({ label, value, max }) {
...
function TreeNode256({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode256 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState256 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer256(state, action) {
...
export default function Component256({ seed = 256, title = 'Component256' }) {
  const hook = useLogic256(seed + 256);
  const [state, dispatch] = useReducer(reducer256, initialState256);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm256(seed));
  ...
  const table = useMemo(() => buildTableState256(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm256(form);
    setErrors(found);
  ...
  return (
    <section className="component-256">
      <div className="header-256">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-256">
      ...
      <div className="grid-256">
        <div className="panel-256">
          {cards.map((item, idx) => (
            <MetricCard256 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
