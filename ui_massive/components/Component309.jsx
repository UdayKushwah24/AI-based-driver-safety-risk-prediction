...
import '../styles/component309.css';

function MetricCard309({ label, value, max }) {
...
function TreeNode309({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode309 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState309 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer309(state, action) {
...
export default function Component309({ seed = 309, title = 'Component309' }) {
  const hook = useLogic309(seed + 309);
  const [state, dispatch] = useReducer(reducer309, initialState309);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm309(seed));
  ...
  const table = useMemo(() => buildTableState309(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm309(form);
    setErrors(found);
  ...
  return (
    <section className="component-309">
      <div className="header-309">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-309">
      ...
      <div className="grid-309">
        <div className="panel-309">
          {cards.map((item, idx) => (
            <MetricCard309 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
