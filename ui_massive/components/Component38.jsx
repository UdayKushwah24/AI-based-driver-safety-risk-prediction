...
import '../styles/component38.css';

function MetricCard38({ label, value, max }) {
...
function TreeNode38({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode38 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component38({ seed = 38, title = 'Component38' }) {
  const hook = useLogic38(seed + 38);
  const [state, dispatch] = useReducer(reducer38, initialState38);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm38(seed));
  ...
  const table = useMemo(() => buildTableState38(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm38(form);
    setErrors(found);
  ...
  return (
    <section className="component-38">
      ...
      <div className="grid-38">
        <div className="panel-38">
          {cards.map((item, idx) => (
            <MetricCard38 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
