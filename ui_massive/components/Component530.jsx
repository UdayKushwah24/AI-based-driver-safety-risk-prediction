...
import '../styles/component530.css';

function MetricCard530({ label, value, max }) {
...
function TreeNode530({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode530 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState530 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer530(state, action) {
...
export default function Component530({ seed = 530, title = 'Component530' }) {
  const hook = useLogic530(seed + 530);
  const [state, dispatch] = useReducer(reducer530, initialState530);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm530(seed));
  ...
  const table = useMemo(() => buildTableState530(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm530(form);
    setErrors(found);
  ...
  return (
    <section className="component-530">
      <div className="header-530">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-530">
      ...
      <div className="grid-530">
        <div className="panel-530">
          {cards.map((item, idx) => (
            <MetricCard530 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
