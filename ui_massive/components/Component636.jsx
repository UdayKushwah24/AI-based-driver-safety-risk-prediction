...
import { useLogic86 } from '../hooks/useLogic86.js';
import { buildNestedTree86, buildTableState86, heavyCompute86, transformDataset86 } from '../utils/compute86.js';
import { buildForecast86, detectAnomalies86, segmentDistribution86 } from '../utils/compute86.js';
import { buildActivityMap86, rankBuckets86, summarizePipeline86, synthesizeFlow86 } from '../pipelines/pipeline86.js';
import '../styles/component636.css';

function MetricCard636({ label, value, max }) {
...
function TreeNode636({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode636 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState636 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer636(state, action) {
...
export default function Component636({ seed = 636, title = 'Component636' }) {
  const hook = useLogic86(seed + 636);
  const [state, dispatch] = useReducer(reducer636, initialState636);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm636(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset86(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState86(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree86(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute86(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast86(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies86(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution86(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow86(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets86(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap86(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline86(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm636(form);
    setErrors(found);
  ...
  return (
    <section className="component-636">
      <div className="header-636">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-636">
      ...
      <div className="grid-636">
        <div className="panel-636">
          {cards.map((item, idx) => (
            <MetricCard636 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
