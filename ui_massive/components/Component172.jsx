...
import '../styles/component172.css';

function MetricCard172({ label, value, max }) {
...
function TreeNode172({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode172 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState172 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer172(state, action) {
...
export default function Component172({ seed = 172, title = 'Component172' }) {
  const hook = useLogic172(seed + 172);
  const [state, dispatch] = useReducer(reducer172, initialState172);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm172(seed));
  ...
  const table = useMemo(() => buildTableState172(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm172(form);
    setErrors(found);
  ...
  return (
    <section className="component-172">
      <div className="header-172">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-172">
      ...
      <div className="grid-172">
        <div className="panel-172">
          {cards.map((item, idx) => (
            <MetricCard172 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
