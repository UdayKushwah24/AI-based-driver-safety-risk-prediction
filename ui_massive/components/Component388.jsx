...
import '../styles/component388.css';

function MetricCard388({ label, value, max }) {
...
function TreeNode388({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode388 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState388 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer388(state, action) {
...
export default function Component388({ seed = 388, title = 'Component388' }) {
  const hook = useLogic388(seed + 388);
  const [state, dispatch] = useReducer(reducer388, initialState388);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm388(seed));
  ...
  const table = useMemo(() => buildTableState388(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm388(form);
    setErrors(found);
  ...
  return (
    <section className="component-388">
      <div className="header-388">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-388">
      ...
      <div className="grid-388">
        <div className="panel-388">
          {cards.map((item, idx) => (
            <MetricCard388 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
