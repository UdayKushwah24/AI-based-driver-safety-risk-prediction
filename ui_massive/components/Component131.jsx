...
import '../styles/component131.css';

function MetricCard131({ label, value, max }) {
...
function TreeNode131({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode131 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component131({ seed = 131, title = 'Component131' }) {
  const hook = useLogic131(seed + 131);
  const [state, dispatch] = useReducer(reducer131, initialState131);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm131(seed));
  ...
  const table = useMemo(() => buildTableState131(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm131(form);
    setErrors(found);
  ...
  return (
    <section className="component-131">
      ...
      <div className="grid-131">
        <div className="panel-131">
          {cards.map((item, idx) => (
            <MetricCard131 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
