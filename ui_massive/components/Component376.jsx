...
import '../styles/component376.css';

function MetricCard376({ label, value, max }) {
...
function TreeNode376({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode376 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState376 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer376(state, action) {
...
export default function Component376({ seed = 376, title = 'Component376' }) {
  const hook = useLogic376(seed + 376);
  const [state, dispatch] = useReducer(reducer376, initialState376);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm376(seed));
  ...
  const table = useMemo(() => buildTableState376(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm376(form);
    setErrors(found);
  ...
  return (
    <section className="component-376">
      <div className="header-376">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-376">
      ...
      <div className="grid-376">
        <div className="panel-376">
          {cards.map((item, idx) => (
            <MetricCard376 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
