...
import { useLogic226 } from '../hooks/useLogic226.js';
import { buildNestedTree226, buildTableState226, heavyCompute226, transformDataset226 } from '../utils/compute226.js';
import { buildForecast226, detectAnomalies226, segmentDistribution226 } from '../utils/compute226.js';
import { buildActivityMap226, rankBuckets226, summarizePipeline226, synthesizeFlow226 } from '../pipelines/pipeline226.js';
import '../styles/component776.css';

function MetricCard776({ label, value, max }) {
...
function TreeNode776({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode776 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState776 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer776(state, action) {
...
export default function Component776({ seed = 776, title = 'Component776' }) {
  const hook = useLogic226(seed + 776);
  const [state, dispatch] = useReducer(reducer776, initialState776);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm776(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset226(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState226(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree226(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute226(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast226(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies226(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution226(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow226(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets226(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap226(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline226(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm776(form);
    setErrors(found);
  ...
  return (
    <section className="component-776">
      <div className="header-776">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-776">
      ...
      <div className="grid-776">
        <div className="panel-776">
          {cards.map((item, idx) => (
            <MetricCard776 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
