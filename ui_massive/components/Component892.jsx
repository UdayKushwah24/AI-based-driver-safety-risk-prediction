...
import '../styles/component892.css';

function MetricCard892({ label, value, max }) {
...
function TreeNode892({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode892 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState892 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer892(state, action) {
...
export default function Component892({ seed = 892, title = 'Component892' }) {
  const hook = useLogic342(seed + 892);
  const [state, dispatch] = useReducer(reducer892, initialState892);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm892(seed));
  ...
  const table = useMemo(() => buildTableState342(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm892(form);
    setErrors(found);
  ...
  return (
    <section className="component-892">
      <div className="header-892">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-892">
      ...
      <div className="grid-892">
        <div className="panel-892">
          {cards.map((item, idx) => (
            <MetricCard892 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
