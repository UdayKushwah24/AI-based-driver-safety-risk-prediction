...
import '../styles/component697.css';

function MetricCard697({ label, value, max }) {
...
function TreeNode697({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode697 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState697 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer697(state, action) {
...
export default function Component697({ seed = 697, title = 'Component697' }) {
  const hook = useLogic147(seed + 697);
  const [state, dispatch] = useReducer(reducer697, initialState697);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm697(seed));
  ...
  const table = useMemo(() => buildTableState147(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm697(form);
    setErrors(found);
  ...
  return (
    <section className="component-697">
      <div className="header-697">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-697">
      ...
      <div className="grid-697">
        <div className="panel-697">
          {cards.map((item, idx) => (
            <MetricCard697 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
