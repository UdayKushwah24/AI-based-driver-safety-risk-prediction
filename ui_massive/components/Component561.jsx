...
import { useLogic11 } from '../hooks/useLogic11.js';
import { buildNestedTree11, buildTableState11, heavyCompute11, transformDataset11 } from '../utils/compute11.js';
import { buildForecast11, detectAnomalies11, segmentDistribution11 } from '../utils/compute11.js';
import { buildActivityMap11, rankBuckets11, summarizePipeline11, synthesizeFlow11 } from '../pipelines/pipeline11.js';
import '../styles/component561.css';

function MetricCard561({ label, value, max }) {
...
function TreeNode561({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode561 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState561 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer561(state, action) {
...
export default function Component561({ seed = 561, title = 'Component561' }) {
  const hook = useLogic11(seed + 561);
  const [state, dispatch] = useReducer(reducer561, initialState561);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm561(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset11(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState11(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree11(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute11(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast11(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies11(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution11(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow11(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets11(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap11(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline11(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm561(form);
    setErrors(found);
  ...
  return (
    <section className="component-561">
      <div className="header-561">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-561">
      ...
      <div className="grid-561">
        <div className="panel-561">
          {cards.map((item, idx) => (
            <MetricCard561 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
