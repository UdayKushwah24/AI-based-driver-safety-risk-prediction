...
import '../styles/component959.css';

function MetricCard959({ label, value, max }) {
...
function TreeNode959({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode959 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState959 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer959(state, action) {
...
export default function Component959({ seed = 959, title = 'Component959' }) {
  const hook = useLogic409(seed + 959);
  const [state, dispatch] = useReducer(reducer959, initialState959);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm959(seed));
  ...
  const table = useMemo(() => buildTableState409(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm959(form);
    setErrors(found);
  ...
  return (
    <section className="component-959">
      <div className="header-959">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-959">
      ...
      <div className="grid-959">
        <div className="panel-959">
          {cards.map((item, idx) => (
            <MetricCard959 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
