...
import '../styles/component427.css';

function MetricCard427({ label, value, max }) {
...
function TreeNode427({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode427 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState427 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer427(state, action) {
...
export default function Component427({ seed = 427, title = 'Component427' }) {
  const hook = useLogic427(seed + 427);
  const [state, dispatch] = useReducer(reducer427, initialState427);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm427(seed));
  ...
  const table = useMemo(() => buildTableState427(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm427(form);
    setErrors(found);
  ...
  return (
    <section className="component-427">
      <div className="header-427">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-427">
      ...
      <div className="grid-427">
        <div className="panel-427">
          {cards.map((item, idx) => (
            <MetricCard427 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
