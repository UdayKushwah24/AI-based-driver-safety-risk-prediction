...
import '../styles/component43.css';

function MetricCard43({ label, value, max }) {
...
function TreeNode43({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode43 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component43({ seed = 43, title = 'Component43' }) {
  const hook = useLogic43(seed + 43);
  const [state, dispatch] = useReducer(reducer43, initialState43);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm43(seed));
  ...
  const table = useMemo(() => buildTableState43(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm43(form);
    setErrors(found);
  ...
  return (
    <section className="component-43">
      ...
      <div className="grid-43">
        <div className="panel-43">
          {cards.map((item, idx) => (
            <MetricCard43 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
