...
import { useLogic433 } from '../hooks/useLogic433.js';
import { buildNestedTree433, buildTableState433, heavyCompute433, transformDataset433 } from '../utils/compute433.js';
import { buildForecast433, detectAnomalies433, segmentDistribution433 } from '../utils/compute433.js';
import { buildActivityMap433, rankBuckets433, summarizePipeline433, synthesizeFlow433 } from '../pipelines/pipeline433.js';
import '../styles/component433.css';

function MetricCard433({ label, value, max }) {
...
function TreeNode433({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode433 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState433 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer433(state, action) {
...
export default function Component433({ seed = 433, title = 'Component433' }) {
  const hook = useLogic433(seed + 433);
  const [state, dispatch] = useReducer(reducer433, initialState433);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm433(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset433(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState433(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree433(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute433(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast433(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies433(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution433(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow433(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets433(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap433(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline433(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm433(form);
    setErrors(found);
  ...
  return (
    <section className="component-433">
      <div className="header-433">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-433">
      ...
      <div className="grid-433">
        <div className="panel-433">
          {cards.map((item, idx) => (
            <MetricCard433 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
