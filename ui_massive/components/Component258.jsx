...
import { useLogic258 } from '../hooks/useLogic258.js';
import { buildNestedTree258, buildTableState258, heavyCompute258, transformDataset258 } from '../utils/compute258.js';
import { buildForecast258, detectAnomalies258, segmentDistribution258 } from '../utils/compute258.js';
import { buildActivityMap258, rankBuckets258, summarizePipeline258, synthesizeFlow258 } from '../pipelines/pipeline258.js';
import '../styles/component258.css';

function MetricCard258({ label, value, max }) {
...
function TreeNode258({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode258 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState258 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer258(state, action) {
...
export default function Component258({ seed = 258, title = 'Component258' }) {
  const hook = useLogic258(seed + 258);
  const [state, dispatch] = useReducer(reducer258, initialState258);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm258(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset258(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState258(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree258(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute258(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast258(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies258(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution258(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow258(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets258(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap258(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline258(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm258(form);
    setErrors(found);
  ...
  return (
    <section className="component-258">
      <div className="header-258">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-258">
      ...
      <div className="grid-258">
        <div className="panel-258">
          {cards.map((item, idx) => (
            <MetricCard258 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
