...
import '../styles/component268.css';

function MetricCard268({ label, value, max }) {
...
function TreeNode268({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode268 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState268 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer268(state, action) {
...
export default function Component268({ seed = 268, title = 'Component268' }) {
  const hook = useLogic268(seed + 268);
  const [state, dispatch] = useReducer(reducer268, initialState268);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm268(seed));
  ...
  const table = useMemo(() => buildTableState268(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm268(form);
    setErrors(found);
  ...
  return (
    <section className="component-268">
      <div className="header-268">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-268">
      ...
      <div className="grid-268">
        <div className="panel-268">
          {cards.map((item, idx) => (
            <MetricCard268 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
