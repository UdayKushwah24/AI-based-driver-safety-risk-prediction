...
import { useLogic330 } from '../hooks/useLogic330.js';
import { buildNestedTree330, buildTableState330, heavyCompute330, transformDataset330 } from '../utils/compute330.js';
import { buildForecast330, detectAnomalies330, segmentDistribution330 } from '../utils/compute330.js';
import { buildActivityMap330, rankBuckets330, summarizePipeline330, synthesizeFlow330 } from '../pipelines/pipeline330.js';
import '../styles/component330.css';

function MetricCard330({ label, value, max }) {
...
function TreeNode330({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode330 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState330 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer330(state, action) {
...
export default function Component330({ seed = 330, title = 'Component330' }) {
  const hook = useLogic330(seed + 330);
  const [state, dispatch] = useReducer(reducer330, initialState330);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm330(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset330(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState330(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree330(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute330(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast330(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies330(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution330(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow330(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets330(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap330(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline330(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm330(form);
    setErrors(found);
  ...
  return (
    <section className="component-330">
      <div className="header-330">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-330">
      ...
      <div className="grid-330">
        <div className="panel-330">
          {cards.map((item, idx) => (
            <MetricCard330 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
