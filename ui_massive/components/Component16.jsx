...
import '../styles/component16.css';

function MetricCard16({ label, value, max }) {
...
function TreeNode16({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode16 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component16({ seed = 16, title = 'Component16' }) {
  const hook = useLogic16(seed + 16);
  const [state, dispatch] = useReducer(reducer16, initialState16);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm16(seed));
  ...
  const table = useMemo(() => buildTableState16(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm16(form);
    setErrors(found);
  ...
  return (
    <section className="component-16">
      ...
      <div className="grid-16">
        <div className="panel-16">
          {cards.map((item, idx) => (
            <MetricCard16 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
