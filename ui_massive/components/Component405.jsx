...
import '../styles/component405.css';

function MetricCard405({ label, value, max }) {
...
function TreeNode405({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode405 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState405 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer405(state, action) {
...
export default function Component405({ seed = 405, title = 'Component405' }) {
  const hook = useLogic405(seed + 405);
  const [state, dispatch] = useReducer(reducer405, initialState405);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm405(seed));
  ...
  const table = useMemo(() => buildTableState405(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm405(form);
    setErrors(found);
  ...
  return (
    <section className="component-405">
      <div className="header-405">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-405">
      ...
      <div className="grid-405">
        <div className="panel-405">
          {cards.map((item, idx) => (
            <MetricCard405 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
