...
import '../styles/component77.css';

function MetricCard77({ label, value, max }) {
...
function TreeNode77({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode77 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component77({ seed = 77, title = 'Component77' }) {
  const hook = useLogic77(seed + 77);
  const [state, dispatch] = useReducer(reducer77, initialState77);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm77(seed));
  ...
  const table = useMemo(() => buildTableState77(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm77(form);
    setErrors(found);
  ...
  return (
    <section className="component-77">
      ...
      <div className="grid-77">
        <div className="panel-77">
          {cards.map((item, idx) => (
            <MetricCard77 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
