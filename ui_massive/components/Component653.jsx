...
import { useLogic103 } from '../hooks/useLogic103.js';
import { buildNestedTree103, buildTableState103, heavyCompute103, transformDataset103 } from '../utils/compute103.js';
import { buildForecast103, detectAnomalies103, segmentDistribution103 } from '../utils/compute103.js';
import { buildActivityMap103, rankBuckets103, summarizePipeline103, synthesizeFlow103 } from '../pipelines/pipeline103.js';
import '../styles/component653.css';

function MetricCard653({ label, value, max }) {
...
function TreeNode653({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode653 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState653 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer653(state, action) {
...
export default function Component653({ seed = 653, title = 'Component653' }) {
  const hook = useLogic103(seed + 653);
  const [state, dispatch] = useReducer(reducer653, initialState653);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm653(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset103(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState103(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree103(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute103(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast103(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies103(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution103(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow103(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets103(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap103(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline103(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm653(form);
    setErrors(found);
  ...
  return (
    <section className="component-653">
      <div className="header-653">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-653">
      ...
      <div className="grid-653">
        <div className="panel-653">
          {cards.map((item, idx) => (
            <MetricCard653 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
