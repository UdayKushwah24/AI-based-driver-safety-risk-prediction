...
import { useLogic252 } from '../hooks/useLogic252.js';
import { buildNestedTree252, buildTableState252, heavyCompute252, transformDataset252 } from '../utils/compute252.js';
import { buildForecast252, detectAnomalies252, segmentDistribution252 } from '../utils/compute252.js';
import { buildActivityMap252, rankBuckets252, summarizePipeline252, synthesizeFlow252 } from '../pipelines/pipeline252.js';
import '../styles/component802.css';

function MetricCard802({ label, value, max }) {
...
function TreeNode802({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode802 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState802 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer802(state, action) {
...
export default function Component802({ seed = 802, title = 'Component802' }) {
  const hook = useLogic252(seed + 802);
  const [state, dispatch] = useReducer(reducer802, initialState802);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm802(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset252(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState252(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree252(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute252(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast252(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies252(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution252(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow252(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets252(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap252(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline252(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm802(form);
    setErrors(found);
  ...
  return (
    <section className="component-802">
      <div className="header-802">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-802">
      ...
      <div className="grid-802">
        <div className="panel-802">
          {cards.map((item, idx) => (
            <MetricCard802 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
