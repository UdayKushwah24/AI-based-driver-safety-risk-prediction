...
import '../styles/component79.css';

function MetricCard79({ label, value, max }) {
...
function TreeNode79({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode79 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component79({ seed = 79, title = 'Component79' }) {
  const hook = useLogic79(seed + 79);
  const [state, dispatch] = useReducer(reducer79, initialState79);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm79(seed));
  ...
  const table = useMemo(() => buildTableState79(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm79(form);
    setErrors(found);
  ...
  return (
    <section className="component-79">
      ...
      <div className="grid-79">
        <div className="panel-79">
          {cards.map((item, idx) => (
            <MetricCard79 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
