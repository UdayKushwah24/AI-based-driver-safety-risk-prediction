...
import '../styles/component239.css';

function MetricCard239({ label, value, max }) {
...
function TreeNode239({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode239 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState239 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer239(state, action) {
...
export default function Component239({ seed = 239, title = 'Component239' }) {
  const hook = useLogic239(seed + 239);
  const [state, dispatch] = useReducer(reducer239, initialState239);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm239(seed));
  ...
  const table = useMemo(() => buildTableState239(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm239(form);
    setErrors(found);
  ...
  return (
    <section className="component-239">
      <div className="header-239">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-239">
      ...
      <div className="grid-239">
        <div className="panel-239">
          {cards.map((item, idx) => (
            <MetricCard239 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
