...
import '../styles/component100.css';

function MetricCard100({ label, value, max }) {
...
function TreeNode100({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode100 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component100({ seed = 100, title = 'Component100' }) {
  const hook = useLogic100(seed + 100);
  const [state, dispatch] = useReducer(reducer100, initialState100);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm100(seed));
  ...
  const table = useMemo(() => buildTableState100(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm100(form);
    setErrors(found);
  ...
  return (
    <section className="component-100">
      ...
      <div className="grid-100">
        <div className="panel-100">
          {cards.map((item, idx) => (
            <MetricCard100 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
