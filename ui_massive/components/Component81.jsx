...
import '../styles/component81.css';

function MetricCard81({ label, value, max }) {
...
function TreeNode81({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode81 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component81({ seed = 81, title = 'Component81' }) {
  const hook = useLogic81(seed + 81);
  const [state, dispatch] = useReducer(reducer81, initialState81);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm81(seed));
  ...
  const table = useMemo(() => buildTableState81(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm81(form);
    setErrors(found);
  ...
  return (
    <section className="component-81">
      ...
      <div className="grid-81">
        <div className="panel-81">
          {cards.map((item, idx) => (
            <MetricCard81 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
