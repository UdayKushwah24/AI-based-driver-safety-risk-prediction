...
import '../styles/component396.css';

function MetricCard396({ label, value, max }) {
...
function TreeNode396({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode396 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState396 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer396(state, action) {
...
export default function Component396({ seed = 396, title = 'Component396' }) {
  const hook = useLogic396(seed + 396);
  const [state, dispatch] = useReducer(reducer396, initialState396);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm396(seed));
  ...
  const table = useMemo(() => buildTableState396(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm396(form);
    setErrors(found);
  ...
  return (
    <section className="component-396">
      <div className="header-396">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-396">
      ...
      <div className="grid-396">
        <div className="panel-396">
          {cards.map((item, idx) => (
            <MetricCard396 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
