...
import '../styles/component95.css';

function MetricCard95({ label, value, max }) {
...
function TreeNode95({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode95 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component95({ seed = 95, title = 'Component95' }) {
  const hook = useLogic95(seed + 95);
  const [state, dispatch] = useReducer(reducer95, initialState95);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm95(seed));
  ...
  const table = useMemo(() => buildTableState95(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm95(form);
    setErrors(found);
  ...
  return (
    <section className="component-95">
      ...
      <div className="grid-95">
        <div className="panel-95">
          {cards.map((item, idx) => (
            <MetricCard95 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
