...
import { useLogic256 } from '../hooks/useLogic256.js';
import { buildNestedTree256, buildTableState256, heavyCompute256, transformDataset256 } from '../utils/compute256.js';
import { buildForecast256, detectAnomalies256, segmentDistribution256 } from '../utils/compute256.js';
import { buildActivityMap256, rankBuckets256, summarizePipeline256, synthesizeFlow256 } from '../pipelines/pipeline256.js';
import '../styles/component806.css';

function MetricCard806({ label, value, max }) {
...
function TreeNode806({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode806 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState806 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer806(state, action) {
...
export default function Component806({ seed = 806, title = 'Component806' }) {
  const hook = useLogic256(seed + 806);
  const [state, dispatch] = useReducer(reducer806, initialState806);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm806(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset256(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState256(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree256(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute256(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast256(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies256(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution256(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow256(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets256(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap256(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline256(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm806(form);
    setErrors(found);
  ...
  return (
    <section className="component-806">
      <div className="header-806">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-806">
      ...
      <div className="grid-806">
        <div className="panel-806">
          {cards.map((item, idx) => (
            <MetricCard806 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
