...
import { useLogic45 } from '../hooks/useLogic45.js';
import { buildNestedTree45, buildTableState45, heavyCompute45, transformDataset45 } from '../utils/compute45.js';
import { buildForecast45, detectAnomalies45, segmentDistribution45 } from '../utils/compute45.js';
import { buildActivityMap45, rankBuckets45, summarizePipeline45, synthesizeFlow45 } from '../pipelines/pipeline45.js';
import '../styles/component595.css';

function MetricCard595({ label, value, max }) {
...
function TreeNode595({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode595 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState595 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer595(state, action) {
...
export default function Component595({ seed = 595, title = 'Component595' }) {
  const hook = useLogic45(seed + 595);
  const [state, dispatch] = useReducer(reducer595, initialState595);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm595(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset45(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState45(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree45(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute45(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast45(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies45(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution45(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow45(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets45(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap45(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline45(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm595(form);
    setErrors(found);
  ...
  return (
    <section className="component-595">
      <div className="header-595">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-595">
      ...
      <div className="grid-595">
        <div className="panel-595">
          {cards.map((item, idx) => (
            <MetricCard595 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
