...
import { useLogic428 } from '../hooks/useLogic428.js';
import { buildNestedTree428, buildTableState428, heavyCompute428, transformDataset428 } from '../utils/compute428.js';
import { buildForecast428, detectAnomalies428, segmentDistribution428 } from '../utils/compute428.js';
import { buildActivityMap428, rankBuckets428, summarizePipeline428, synthesizeFlow428 } from '../pipelines/pipeline428.js';
import '../styles/component978.css';

function MetricCard978({ label, value, max }) {
...
function TreeNode978({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode978 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState978 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer978(state, action) {
...
export default function Component978({ seed = 978, title = 'Component978' }) {
  const hook = useLogic428(seed + 978);
  const [state, dispatch] = useReducer(reducer978, initialState978);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm978(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset428(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState428(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree428(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute428(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast428(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies428(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution428(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow428(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets428(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap428(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline428(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm978(form);
    setErrors(found);
  ...
  return (
    <section className="component-978">
      <div className="header-978">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-978">
      ...
      <div className="grid-978">
        <div className="panel-978">
          {cards.map((item, idx) => (
            <MetricCard978 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
