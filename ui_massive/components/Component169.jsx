...
import '../styles/component169.css';

function MetricCard169({ label, value, max }) {
...
function TreeNode169({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode169 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState169 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer169(state, action) {
...
export default function Component169({ seed = 169, title = 'Component169' }) {
  const hook = useLogic169(seed + 169);
  const [state, dispatch] = useReducer(reducer169, initialState169);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm169(seed));
  ...
  const table = useMemo(() => buildTableState169(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm169(form);
    setErrors(found);
  ...
  return (
    <section className="component-169">
      <div className="header-169">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-169">
      ...
      <div className="grid-169">
        <div className="panel-169">
          {cards.map((item, idx) => (
            <MetricCard169 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
