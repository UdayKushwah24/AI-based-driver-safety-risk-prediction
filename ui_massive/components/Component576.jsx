...
import { useLogic26 } from '../hooks/useLogic26.js';
import { buildNestedTree26, buildTableState26, heavyCompute26, transformDataset26 } from '../utils/compute26.js';
import { buildForecast26, detectAnomalies26, segmentDistribution26 } from '../utils/compute26.js';
import { buildActivityMap26, rankBuckets26, summarizePipeline26, synthesizeFlow26 } from '../pipelines/pipeline26.js';
import '../styles/component576.css';

function MetricCard576({ label, value, max }) {
...
function TreeNode576({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode576 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState576 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer576(state, action) {
...
export default function Component576({ seed = 576, title = 'Component576' }) {
  const hook = useLogic26(seed + 576);
  const [state, dispatch] = useReducer(reducer576, initialState576);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm576(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset26(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState26(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree26(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute26(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast26(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies26(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution26(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow26(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets26(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap26(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline26(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm576(form);
    setErrors(found);
  ...
  return (
    <section className="component-576">
      <div className="header-576">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-576">
      ...
      <div className="grid-576">
        <div className="panel-576">
          {cards.map((item, idx) => (
            <MetricCard576 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
