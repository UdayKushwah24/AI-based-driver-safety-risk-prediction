...
import '../styles/component457.css';

function MetricCard457({ label, value, max }) {
...
function TreeNode457({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode457 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState457 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer457(state, action) {
...
export default function Component457({ seed = 457, title = 'Component457' }) {
  const hook = useLogic457(seed + 457);
  const [state, dispatch] = useReducer(reducer457, initialState457);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm457(seed));
  ...
  const table = useMemo(() => buildTableState457(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm457(form);
    setErrors(found);
  ...
  return (
    <section className="component-457">
      <div className="header-457">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-457">
      ...
      <div className="grid-457">
        <div className="panel-457">
          {cards.map((item, idx) => (
            <MetricCard457 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
