...
import { useLogic14 } from '../hooks/useLogic14.js';
import { buildNestedTree14, buildTableState14, heavyCompute14, transformDataset14 } from '../utils/compute14.js';
import { buildForecast14, detectAnomalies14, segmentDistribution14 } from '../utils/compute14.js';
import { buildActivityMap14, rankBuckets14, summarizePipeline14, synthesizeFlow14 } from '../pipelines/pipeline14.js';
import '../styles/component564.css';

function MetricCard564({ label, value, max }) {
...
function TreeNode564({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode564 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState564 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer564(state, action) {
...
export default function Component564({ seed = 564, title = 'Component564' }) {
  const hook = useLogic14(seed + 564);
  const [state, dispatch] = useReducer(reducer564, initialState564);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm564(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset14(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState14(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree14(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute14(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast14(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies14(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution14(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow14(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets14(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap14(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline14(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm564(form);
    setErrors(found);
  ...
  return (
    <section className="component-564">
      <div className="header-564">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-564">
      ...
      <div className="grid-564">
        <div className="panel-564">
          {cards.map((item, idx) => (
            <MetricCard564 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
