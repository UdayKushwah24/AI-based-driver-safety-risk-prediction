...
import '../styles/component289.css';

function MetricCard289({ label, value, max }) {
...
function TreeNode289({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode289 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState289 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer289(state, action) {
...
export default function Component289({ seed = 289, title = 'Component289' }) {
  const hook = useLogic289(seed + 289);
  const [state, dispatch] = useReducer(reducer289, initialState289);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm289(seed));
  ...
  const table = useMemo(() => buildTableState289(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm289(form);
    setErrors(found);
  ...
  return (
    <section className="component-289">
      <div className="header-289">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-289">
      ...
      <div className="grid-289">
        <div className="panel-289">
          {cards.map((item, idx) => (
            <MetricCard289 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
