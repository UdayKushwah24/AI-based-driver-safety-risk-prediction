...
import '../styles/component761.css';

function MetricCard761({ label, value, max }) {
...
function TreeNode761({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode761 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState761 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer761(state, action) {
...
export default function Component761({ seed = 761, title = 'Component761' }) {
  const hook = useLogic211(seed + 761);
  const [state, dispatch] = useReducer(reducer761, initialState761);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm761(seed));
  ...
  const table = useMemo(() => buildTableState211(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm761(form);
    setErrors(found);
  ...
  return (
    <section className="component-761">
      <div className="header-761">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-761">
      ...
      <div className="grid-761">
        <div className="panel-761">
          {cards.map((item, idx) => (
            <MetricCard761 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
