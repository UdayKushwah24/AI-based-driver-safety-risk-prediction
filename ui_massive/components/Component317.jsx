...
import '../styles/component317.css';

function MetricCard317({ label, value, max }) {
...
function TreeNode317({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode317 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component317({ seed = 317, title = 'Component317' }) {
  const hook = useLogic317(seed + 317);
  const [state, dispatch] = useReducer(reducer317, initialState317);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm317(seed));
  ...
  const table = useMemo(() => buildTableState317(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm317(form);
    setErrors(found);
  ...
  return (
    <section className="component-317">
      <div className="header-317">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-317">
      ...
      <div className="grid-317">
        <div className="panel-317">
          {cards.map((item, idx) => (
            <MetricCard317 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
