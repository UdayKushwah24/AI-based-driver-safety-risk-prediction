...
import '../styles/component337.css';

function MetricCard337({ label, value, max }) {
...
function TreeNode337({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode337 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState337 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer337(state, action) {
...
export default function Component337({ seed = 337, title = 'Component337' }) {
  const hook = useLogic337(seed + 337);
  const [state, dispatch] = useReducer(reducer337, initialState337);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm337(seed));
  ...
  const table = useMemo(() => buildTableState337(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm337(form);
    setErrors(found);
  ...
  return (
    <section className="component-337">
      <div className="header-337">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-337">
      ...
      <div className="grid-337">
        <div className="panel-337">
          {cards.map((item, idx) => (
            <MetricCard337 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
