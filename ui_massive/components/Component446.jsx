...
import { useLogic446 } from '../hooks/useLogic446.js';
import { buildNestedTree446, buildTableState446, heavyCompute446, transformDataset446 } from '../utils/compute446.js';
import { buildForecast446, detectAnomalies446, segmentDistribution446 } from '../utils/compute446.js';
import { buildActivityMap446, rankBuckets446, summarizePipeline446, synthesizeFlow446 } from '../pipelines/pipeline446.js';
import '../styles/component446.css';

function MetricCard446({ label, value, max }) {
...
function TreeNode446({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode446 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState446 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer446(state, action) {
...
export default function Component446({ seed = 446, title = 'Component446' }) {
  const hook = useLogic446(seed + 446);
  const [state, dispatch] = useReducer(reducer446, initialState446);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm446(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset446(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState446(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree446(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute446(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast446(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies446(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution446(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow446(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets446(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap446(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline446(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm446(form);
    setErrors(found);
  ...
  return (
    <section className="component-446">
      <div className="header-446">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-446">
      ...
      <div className="grid-446">
        <div className="panel-446">
          {cards.map((item, idx) => (
            <MetricCard446 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
