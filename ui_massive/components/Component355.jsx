...
import { useLogic355 } from '../hooks/useLogic355.js';
import { buildNestedTree355, buildTableState355, heavyCompute355, transformDataset355 } from '../utils/compute355.js';
import { buildForecast355, detectAnomalies355, segmentDistribution355 } from '../utils/compute355.js';
import { buildActivityMap355, rankBuckets355, summarizePipeline355, synthesizeFlow355 } from '../pipelines/pipeline355.js';
import '../styles/component355.css';

function MetricCard355({ label, value, max }) {
...
function TreeNode355({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode355 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState355 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer355(state, action) {
...
export default function Component355({ seed = 355, title = 'Component355' }) {
  const hook = useLogic355(seed + 355);
  const [state, dispatch] = useReducer(reducer355, initialState355);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm355(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset355(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState355(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree355(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute355(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast355(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies355(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution355(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow355(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets355(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap355(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline355(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm355(form);
    setErrors(found);
  ...
  return (
    <section className="component-355">
      <div className="header-355">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-355">
      ...
      <div className="grid-355">
        <div className="panel-355">
          {cards.map((item, idx) => (
            <MetricCard355 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
