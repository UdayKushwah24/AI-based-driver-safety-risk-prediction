...
import { useLogic326 } from '../hooks/useLogic326.js';
import { buildNestedTree326, buildTableState326, heavyCompute326, transformDataset326 } from '../utils/compute326.js';
import { buildForecast326, detectAnomalies326, segmentDistribution326 } from '../utils/compute326.js';
import { buildActivityMap326, rankBuckets326, summarizePipeline326, synthesizeFlow326 } from '../pipelines/pipeline326.js';
import '../styles/component326.css';

function MetricCard326({ label, value, max }) {
...
function TreeNode326({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode326 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState326 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer326(state, action) {
...
export default function Component326({ seed = 326, title = 'Component326' }) {
  const hook = useLogic326(seed + 326);
  const [state, dispatch] = useReducer(reducer326, initialState326);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm326(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset326(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState326(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree326(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute326(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast326(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies326(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution326(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow326(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets326(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap326(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline326(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm326(form);
    setErrors(found);
  ...
  return (
    <section className="component-326">
      <div className="header-326">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-326">
      ...
      <div className="grid-326">
        <div className="panel-326">
          {cards.map((item, idx) => (
            <MetricCard326 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
