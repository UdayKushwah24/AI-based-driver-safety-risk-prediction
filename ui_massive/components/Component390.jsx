...
import { useLogic390 } from '../hooks/useLogic390.js';
import { buildNestedTree390, buildTableState390, heavyCompute390, transformDataset390 } from '../utils/compute390.js';
import { buildForecast390, detectAnomalies390, segmentDistribution390 } from '../utils/compute390.js';
import { buildActivityMap390, rankBuckets390, summarizePipeline390, synthesizeFlow390 } from '../pipelines/pipeline390.js';
import '../styles/component390.css';

function MetricCard390({ label, value, max }) {
...
function TreeNode390({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode390 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState390 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer390(state, action) {
...
export default function Component390({ seed = 390, title = 'Component390' }) {
  const hook = useLogic390(seed + 390);
  const [state, dispatch] = useReducer(reducer390, initialState390);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm390(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset390(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState390(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree390(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute390(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast390(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies390(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution390(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow390(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets390(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap390(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline390(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm390(form);
    setErrors(found);
  ...
  return (
    <section className="component-390">
      <div className="header-390">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-390">
      ...
      <div className="grid-390">
        <div className="panel-390">
          {cards.map((item, idx) => (
            <MetricCard390 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
