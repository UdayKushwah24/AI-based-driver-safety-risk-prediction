...
import '../styles/component257.css';

function MetricCard257({ label, value, max }) {
...
function TreeNode257({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode257 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState257 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer257(state, action) {
...
export default function Component257({ seed = 257, title = 'Component257' }) {
  const hook = useLogic257(seed + 257);
  const [state, dispatch] = useReducer(reducer257, initialState257);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm257(seed));
  ...
  const table = useMemo(() => buildTableState257(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm257(form);
    setErrors(found);
  ...
  return (
    <section className="component-257">
      <div className="header-257">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-257">
      ...
      <div className="grid-257">
        <div className="panel-257">
          {cards.map((item, idx) => (
            <MetricCard257 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
