...
import { useLogic346 } from '../hooks/useLogic346.js';
import { buildNestedTree346, buildTableState346, heavyCompute346, transformDataset346 } from '../utils/compute346.js';
import { buildForecast346, detectAnomalies346, segmentDistribution346 } from '../utils/compute346.js';
import { buildActivityMap346, rankBuckets346, summarizePipeline346, synthesizeFlow346 } from '../pipelines/pipeline346.js';
import '../styles/component346.css';

function MetricCard346({ label, value, max }) {
...
function TreeNode346({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode346 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState346 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer346(state, action) {
...
export default function Component346({ seed = 346, title = 'Component346' }) {
  const hook = useLogic346(seed + 346);
  const [state, dispatch] = useReducer(reducer346, initialState346);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm346(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset346(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState346(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree346(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute346(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast346(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies346(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution346(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow346(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets346(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap346(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline346(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm346(form);
    setErrors(found);
  ...
  return (
    <section className="component-346">
      <div className="header-346">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-346">
      ...
      <div className="grid-346">
        <div className="panel-346">
          {cards.map((item, idx) => (
            <MetricCard346 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
