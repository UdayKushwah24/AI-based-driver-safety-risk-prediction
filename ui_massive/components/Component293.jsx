...
import '../styles/component293.css';

function MetricCard293({ label, value, max }) {
...
function TreeNode293({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode293 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState293 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer293(state, action) {
...
export default function Component293({ seed = 293, title = 'Component293' }) {
  const hook = useLogic293(seed + 293);
  const [state, dispatch] = useReducer(reducer293, initialState293);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm293(seed));
  ...
  const table = useMemo(() => buildTableState293(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm293(form);
    setErrors(found);
  ...
  return (
    <section className="component-293">
      <div className="header-293">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-293">
      ...
      <div className="grid-293">
        <div className="panel-293">
          {cards.map((item, idx) => (
            <MetricCard293 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
