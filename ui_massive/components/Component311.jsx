...
import '../styles/component311.css';

function MetricCard311({ label, value, max }) {
...
function TreeNode311({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode311 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState311 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer311(state, action) {
...
export default function Component311({ seed = 311, title = 'Component311' }) {
  const hook = useLogic311(seed + 311);
  const [state, dispatch] = useReducer(reducer311, initialState311);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm311(seed));
  ...
  const table = useMemo(() => buildTableState311(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm311(form);
    setErrors(found);
  ...
  return (
    <section className="component-311">
      <div className="header-311">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-311">
      ...
      <div className="grid-311">
        <div className="panel-311">
          {cards.map((item, idx) => (
            <MetricCard311 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
