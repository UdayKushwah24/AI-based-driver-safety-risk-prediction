...
import '../styles/component122.css';

function MetricCard122({ label, value, max }) {
...
function TreeNode122({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode122 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component122({ seed = 122, title = 'Component122' }) {
  const hook = useLogic122(seed + 122);
  const [state, dispatch] = useReducer(reducer122, initialState122);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm122(seed));
  ...
  const table = useMemo(() => buildTableState122(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm122(form);
    setErrors(found);
  ...
  return (
    <section className="component-122">
      ...
      <div className="grid-122">
        <div className="panel-122">
          {cards.map((item, idx) => (
            <MetricCard122 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
