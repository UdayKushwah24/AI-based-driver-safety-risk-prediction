...
import '../styles/component535.css';

function MetricCard535({ label, value, max }) {
...
function TreeNode535({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode535 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState535 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer535(state, action) {
...
export default function Component535({ seed = 535, title = 'Component535' }) {
  const hook = useLogic535(seed + 535);
  const [state, dispatch] = useReducer(reducer535, initialState535);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm535(seed));
  ...
  const table = useMemo(() => buildTableState535(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm535(form);
    setErrors(found);
  ...
  return (
    <section className="component-535">
      <div className="header-535">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-535">
      ...
      <div className="grid-535">
        <div className="panel-535">
          {cards.map((item, idx) => (
            <MetricCard535 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
