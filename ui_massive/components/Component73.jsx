...
import '../styles/component73.css';

function MetricCard73({ label, value, max }) {
...
function TreeNode73({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode73 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component73({ seed = 73, title = 'Component73' }) {
  const hook = useLogic73(seed + 73);
  const [state, dispatch] = useReducer(reducer73, initialState73);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm73(seed));
  ...
  const table = useMemo(() => buildTableState73(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm73(form);
    setErrors(found);
  ...
  return (
    <section className="component-73">
      ...
      <div className="grid-73">
        <div className="panel-73">
          {cards.map((item, idx) => (
            <MetricCard73 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
