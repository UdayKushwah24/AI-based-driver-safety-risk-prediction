...
import '../styles/component188.css';

function MetricCard188({ label, value, max }) {
...
function TreeNode188({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode188 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState188 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer188(state, action) {
...
export default function Component188({ seed = 188, title = 'Component188' }) {
  const hook = useLogic188(seed + 188);
  const [state, dispatch] = useReducer(reducer188, initialState188);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm188(seed));
  ...
  const table = useMemo(() => buildTableState188(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm188(form);
    setErrors(found);
  ...
  return (
    <section className="component-188">
      <div className="header-188">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-188">
      ...
      <div className="grid-188">
        <div className="panel-188">
          {cards.map((item, idx) => (
            <MetricCard188 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
