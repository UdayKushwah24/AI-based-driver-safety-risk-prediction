...
import '../styles/component727.css';

function MetricCard727({ label, value, max }) {
...
function TreeNode727({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode727 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState727 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer727(state, action) {
...
export default function Component727({ seed = 727, title = 'Component727' }) {
  const hook = useLogic177(seed + 727);
  const [state, dispatch] = useReducer(reducer727, initialState727);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm727(seed));
  ...
  const table = useMemo(() => buildTableState177(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm727(form);
    setErrors(found);
  ...
  return (
    <section className="component-727">
      <div className="header-727">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-727">
      ...
      <div className="grid-727">
        <div className="panel-727">
          {cards.map((item, idx) => (
            <MetricCard727 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
