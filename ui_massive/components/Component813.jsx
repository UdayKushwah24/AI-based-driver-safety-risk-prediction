...
import { useLogic263 } from '../hooks/useLogic263.js';
import { buildNestedTree263, buildTableState263, heavyCompute263, transformDataset263 } from '../utils/compute263.js';
import { buildForecast263, detectAnomalies263, segmentDistribution263 } from '../utils/compute263.js';
import { buildActivityMap263, rankBuckets263, summarizePipeline263, synthesizeFlow263 } from '../pipelines/pipeline263.js';
import '../styles/component813.css';

function MetricCard813({ label, value, max }) {
...
function TreeNode813({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode813 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState813 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer813(state, action) {
...
export default function Component813({ seed = 813, title = 'Component813' }) {
  const hook = useLogic263(seed + 813);
  const [state, dispatch] = useReducer(reducer813, initialState813);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm813(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset263(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState263(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree263(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute263(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast263(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies263(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution263(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow263(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets263(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap263(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline263(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm813(form);
    setErrors(found);
  ...
  return (
    <section className="component-813">
      <div className="header-813">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-813">
      ...
      <div className="grid-813">
        <div className="panel-813">
          {cards.map((item, idx) => (
            <MetricCard813 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
