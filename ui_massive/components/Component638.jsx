...
import { useLogic88 } from '../hooks/useLogic88.js';
import { buildNestedTree88, buildTableState88, heavyCompute88, transformDataset88 } from '../utils/compute88.js';
import { buildForecast88, detectAnomalies88, segmentDistribution88 } from '../utils/compute88.js';
import { buildActivityMap88, rankBuckets88, summarizePipeline88, synthesizeFlow88 } from '../pipelines/pipeline88.js';
import '../styles/component638.css';

function MetricCard638({ label, value, max }) {
...
function TreeNode638({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode638 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState638 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer638(state, action) {
...
export default function Component638({ seed = 638, title = 'Component638' }) {
  const hook = useLogic88(seed + 638);
  const [state, dispatch] = useReducer(reducer638, initialState638);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm638(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset88(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState88(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree88(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute88(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast88(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies88(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution88(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow88(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets88(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap88(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline88(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm638(form);
    setErrors(found);
  ...
  return (
    <section className="component-638">
      <div className="header-638">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-638">
      ...
      <div className="grid-638">
        <div className="panel-638">
          {cards.map((item, idx) => (
            <MetricCard638 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
