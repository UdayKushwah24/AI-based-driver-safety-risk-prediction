...
import '../styles/component692.css';

function MetricCard692({ label, value, max }) {
...
function TreeNode692({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode692 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState692 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer692(state, action) {
...
export default function Component692({ seed = 692, title = 'Component692' }) {
  const hook = useLogic142(seed + 692);
  const [state, dispatch] = useReducer(reducer692, initialState692);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm692(seed));
  ...
  const table = useMemo(() => buildTableState142(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm692(form);
    setErrors(found);
  ...
  return (
    <section className="component-692">
      <div className="header-692">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-692">
      ...
      <div className="grid-692">
        <div className="panel-692">
          {cards.map((item, idx) => (
            <MetricCard692 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
