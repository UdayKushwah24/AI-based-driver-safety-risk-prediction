...
import { useLogic306 } from '../hooks/useLogic306.js';
import { buildNestedTree306, buildTableState306, heavyCompute306, transformDataset306 } from '../utils/compute306.js';
import { buildForecast306, detectAnomalies306, segmentDistribution306 } from '../utils/compute306.js';
import { buildActivityMap306, rankBuckets306, summarizePipeline306, synthesizeFlow306 } from '../pipelines/pipeline306.js';
import '../styles/component306.css';

function MetricCard306({ label, value, max }) {
...
function TreeNode306({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode306 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState306 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer306(state, action) {
...
export default function Component306({ seed = 306, title = 'Component306' }) {
  const hook = useLogic306(seed + 306);
  const [state, dispatch] = useReducer(reducer306, initialState306);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm306(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset306(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState306(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree306(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute306(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast306(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies306(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution306(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow306(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets306(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap306(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline306(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm306(form);
    setErrors(found);
  ...
  return (
    <section className="component-306">
      <div className="header-306">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-306">
      ...
      <div className="grid-306">
        <div className="panel-306">
          {cards.map((item, idx) => (
            <MetricCard306 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
