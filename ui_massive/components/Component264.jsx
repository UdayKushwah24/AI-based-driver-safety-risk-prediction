...
import '../styles/component264.css';

function MetricCard264({ label, value, max }) {
...
function TreeNode264({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode264 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState264 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer264(state, action) {
...
export default function Component264({ seed = 264, title = 'Component264' }) {
  const hook = useLogic264(seed + 264);
  const [state, dispatch] = useReducer(reducer264, initialState264);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm264(seed));
  ...
  const table = useMemo(() => buildTableState264(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm264(form);
    setErrors(found);
  ...
  return (
    <section className="component-264">
      <div className="header-264">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-264">
      ...
      <div className="grid-264">
        <div className="panel-264">
          {cards.map((item, idx) => (
            <MetricCard264 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
