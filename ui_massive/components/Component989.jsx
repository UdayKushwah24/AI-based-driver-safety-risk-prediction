...
import '../styles/component989.css';

function MetricCard989({ label, value, max }) {
...
function TreeNode989({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode989 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState989 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer989(state, action) {
...
export default function Component989({ seed = 989, title = 'Component989' }) {
  const hook = useLogic439(seed + 989);
  const [state, dispatch] = useReducer(reducer989, initialState989);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm989(seed));
  ...
  const table = useMemo(() => buildTableState439(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm989(form);
    setErrors(found);
  ...
  return (
    <section className="component-989">
      <div className="header-989">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-989">
      ...
      <div className="grid-989">
        <div className="panel-989">
          {cards.map((item, idx) => (
            <MetricCard989 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
