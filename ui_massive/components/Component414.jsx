...
import { useLogic414 } from '../hooks/useLogic414.js';
import { buildNestedTree414, buildTableState414, heavyCompute414, transformDataset414 } from '../utils/compute414.js';
import { buildForecast414, detectAnomalies414, segmentDistribution414 } from '../utils/compute414.js';
import { buildActivityMap414, rankBuckets414, summarizePipeline414, synthesizeFlow414 } from '../pipelines/pipeline414.js';
import '../styles/component414.css';

function MetricCard414({ label, value, max }) {
...
function TreeNode414({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode414 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState414 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer414(state, action) {
...
export default function Component414({ seed = 414, title = 'Component414' }) {
  const hook = useLogic414(seed + 414);
  const [state, dispatch] = useReducer(reducer414, initialState414);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm414(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset414(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState414(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree414(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute414(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast414(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies414(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution414(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow414(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets414(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap414(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline414(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm414(form);
    setErrors(found);
  ...
  return (
    <section className="component-414">
      <div className="header-414">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-414">
      ...
      <div className="grid-414">
        <div className="panel-414">
          {cards.map((item, idx) => (
            <MetricCard414 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
