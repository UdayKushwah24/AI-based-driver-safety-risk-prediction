...
import '../styles/component348.css';

function MetricCard348({ label, value, max }) {
...
function TreeNode348({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode348 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState348 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer348(state, action) {
...
export default function Component348({ seed = 348, title = 'Component348' }) {
  const hook = useLogic348(seed + 348);
  const [state, dispatch] = useReducer(reducer348, initialState348);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm348(seed));
  ...
  const table = useMemo(() => buildTableState348(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm348(form);
    setErrors(found);
  ...
  return (
    <section className="component-348">
      <div className="header-348">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-348">
      ...
      <div className="grid-348">
        <div className="panel-348">
          {cards.map((item, idx) => (
            <MetricCard348 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
