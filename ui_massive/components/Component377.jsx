...
import '../styles/component377.css';

function MetricCard377({ label, value, max }) {
...
function TreeNode377({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode377 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState377 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer377(state, action) {
...
export default function Component377({ seed = 377, title = 'Component377' }) {
  const hook = useLogic377(seed + 377);
  const [state, dispatch] = useReducer(reducer377, initialState377);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm377(seed));
  ...
  const table = useMemo(() => buildTableState377(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm377(form);
    setErrors(found);
  ...
  return (
    <section className="component-377">
      <div className="header-377">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-377">
      ...
      <div className="grid-377">
        <div className="panel-377">
          {cards.map((item, idx) => (
            <MetricCard377 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
