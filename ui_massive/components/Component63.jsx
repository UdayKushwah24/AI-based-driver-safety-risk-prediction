...
import '../styles/component63.css';

function MetricCard63({ label, value, max }) {
...
function TreeNode63({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode63 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component63({ seed = 63, title = 'Component63' }) {
  const hook = useLogic63(seed + 63);
  const [state, dispatch] = useReducer(reducer63, initialState63);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm63(seed));
  ...
  const table = useMemo(() => buildTableState63(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm63(form);
    setErrors(found);
  ...
  return (
    <section className="component-63">
      ...
      <div className="grid-63">
        <div className="panel-63">
          {cards.map((item, idx) => (
            <MetricCard63 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
