...
import { useLogic396 } from '../hooks/useLogic396.js';
import { buildNestedTree396, buildTableState396, heavyCompute396, transformDataset396 } from '../utils/compute396.js';
import { buildForecast396, detectAnomalies396, segmentDistribution396 } from '../utils/compute396.js';
import { buildActivityMap396, rankBuckets396, summarizePipeline396, synthesizeFlow396 } from '../pipelines/pipeline396.js';
import '../styles/component946.css';

function MetricCard946({ label, value, max }) {
...
function TreeNode946({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode946 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState946 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer946(state, action) {
...
export default function Component946({ seed = 946, title = 'Component946' }) {
  const hook = useLogic396(seed + 946);
  const [state, dispatch] = useReducer(reducer946, initialState946);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm946(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset396(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState396(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree396(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute396(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast396(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies396(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution396(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow396(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets396(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap396(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline396(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm946(form);
    setErrors(found);
  ...
  return (
    <section className="component-946">
      <div className="header-946">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-946">
      ...
      <div className="grid-946">
        <div className="panel-946">
          {cards.map((item, idx) => (
            <MetricCard946 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
