...
import '../styles/component400.css';

function MetricCard400({ label, value, max }) {
...
function TreeNode400({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode400 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState400 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer400(state, action) {
...
export default function Component400({ seed = 400, title = 'Component400' }) {
  const hook = useLogic400(seed + 400);
  const [state, dispatch] = useReducer(reducer400, initialState400);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm400(seed));
  ...
  const table = useMemo(() => buildTableState400(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm400(form);
    setErrors(found);
  ...
  return (
    <section className="component-400">
      <div className="header-400">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-400">
      ...
      <div className="grid-400">
        <div className="panel-400">
          {cards.map((item, idx) => (
            <MetricCard400 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
