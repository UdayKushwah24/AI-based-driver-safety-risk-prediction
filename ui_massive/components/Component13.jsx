...
import '../styles/component13.css';

function MetricCard13({ label, value, max }) {
...
function TreeNode13({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode13 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component13({ seed = 13, title = 'Component13' }) {
  const hook = useLogic13(seed + 13);
  const [state, dispatch] = useReducer(reducer13, initialState13);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm13(seed));
  ...
  const table = useMemo(() => buildTableState13(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm13(form);
    setErrors(found);
  ...
  return (
    <section className="component-13">
      ...
      <div className="grid-13">
        <div className="panel-13">
          {cards.map((item, idx) => (
            <MetricCard13 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
