...
import '../styles/component492.css';

function MetricCard492({ label, value, max }) {
...
function TreeNode492({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode492 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState492 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer492(state, action) {
...
export default function Component492({ seed = 492, title = 'Component492' }) {
  const hook = useLogic492(seed + 492);
  const [state, dispatch] = useReducer(reducer492, initialState492);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm492(seed));
  ...
  const table = useMemo(() => buildTableState492(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm492(form);
    setErrors(found);
  ...
  return (
    <section className="component-492">
      <div className="header-492">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-492">
      ...
      <div className="grid-492">
        <div className="panel-492">
          {cards.map((item, idx) => (
            <MetricCard492 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
