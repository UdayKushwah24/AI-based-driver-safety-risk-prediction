...
import { useLogic352 } from '../hooks/useLogic352.js';
import { buildNestedTree352, buildTableState352, heavyCompute352, transformDataset352 } from '../utils/compute352.js';
import { buildForecast352, detectAnomalies352, segmentDistribution352 } from '../utils/compute352.js';
import { buildActivityMap352, rankBuckets352, summarizePipeline352, synthesizeFlow352 } from '../pipelines/pipeline352.js';
import '../styles/component902.css';

function MetricCard902({ label, value, max }) {
...
function TreeNode902({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode902 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState902 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer902(state, action) {
...
export default function Component902({ seed = 902, title = 'Component902' }) {
  const hook = useLogic352(seed + 902);
  const [state, dispatch] = useReducer(reducer902, initialState902);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm902(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset352(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState352(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree352(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute352(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast352(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies352(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution352(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow352(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets352(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap352(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline352(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm902(form);
    setErrors(found);
  ...
  return (
    <section className="component-902">
      <div className="header-902">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-902">
      ...
      <div className="grid-902">
        <div className="panel-902">
          {cards.map((item, idx) => (
            <MetricCard902 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
