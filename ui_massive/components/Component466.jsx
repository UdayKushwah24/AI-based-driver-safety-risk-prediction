...
import '../styles/component466.css';

function MetricCard466({ label, value, max }) {
...
function TreeNode466({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode466 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState466 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer466(state, action) {
...
export default function Component466({ seed = 466, title = 'Component466' }) {
  const hook = useLogic466(seed + 466);
  const [state, dispatch] = useReducer(reducer466, initialState466);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm466(seed));
  ...
  const table = useMemo(() => buildTableState466(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm466(form);
    setErrors(found);
  ...
  return (
    <section className="component-466">
      <div className="header-466">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-466">
      ...
      <div className="grid-466">
        <div className="panel-466">
          {cards.map((item, idx) => (
            <MetricCard466 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
