...
function MetricCard6({ label, value, max }) {
...
function TreeNode6({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode6 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component6({ seed = 6, title = 'Component6' }) {
  const hook = useLogic6(seed + 6);
  const [state, dispatch] = useReducer(reducer6, initialState6);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm6(seed));
  ...
  const table = useMemo(() => buildTableState6(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm6(form);
    setErrors(found);
    ...
    <section className="component-6">
      ...
      <div className="grid-6">
        <div className="panel-6">
          {cards.map((item, idx) => (
            <MetricCard6 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
