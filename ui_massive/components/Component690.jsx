...
import { useLogic140 } from '../hooks/useLogic140.js';
import { buildNestedTree140, buildTableState140, heavyCompute140, transformDataset140 } from '../utils/compute140.js';
import { buildForecast140, detectAnomalies140, segmentDistribution140 } from '../utils/compute140.js';
import { buildActivityMap140, rankBuckets140, summarizePipeline140, synthesizeFlow140 } from '../pipelines/pipeline140.js';
import '../styles/component690.css';

function MetricCard690({ label, value, max }) {
...
function TreeNode690({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode690 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState690 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer690(state, action) {
...
export default function Component690({ seed = 690, title = 'Component690' }) {
  const hook = useLogic140(seed + 690);
  const [state, dispatch] = useReducer(reducer690, initialState690);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm690(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset140(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState140(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree140(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute140(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast140(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies140(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution140(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow140(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets140(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap140(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline140(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm690(form);
    setErrors(found);
  ...
  return (
    <section className="component-690">
      <div className="header-690">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-690">
      ...
      <div className="grid-690">
        <div className="panel-690">
          {cards.map((item, idx) => (
            <MetricCard690 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
