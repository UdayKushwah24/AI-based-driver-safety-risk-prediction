...
import '../styles/component523.css';

function MetricCard523({ label, value, max }) {
...
function TreeNode523({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode523 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState523 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer523(state, action) {
...
export default function Component523({ seed = 523, title = 'Component523' }) {
  const hook = useLogic523(seed + 523);
  const [state, dispatch] = useReducer(reducer523, initialState523);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm523(seed));
  ...
  const table = useMemo(() => buildTableState523(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm523(form);
    setErrors(found);
  ...
  return (
    <section className="component-523">
      <div className="header-523">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-523">
      ...
      <div className="grid-523">
        <div className="panel-523">
          {cards.map((item, idx) => (
            <MetricCard523 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
