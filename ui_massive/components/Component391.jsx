...
import { useLogic391 } from '../hooks/useLogic391.js';
import { buildNestedTree391, buildTableState391, heavyCompute391, transformDataset391 } from '../utils/compute391.js';
import { buildForecast391, detectAnomalies391, segmentDistribution391 } from '../utils/compute391.js';
import { buildActivityMap391, rankBuckets391, summarizePipeline391, synthesizeFlow391 } from '../pipelines/pipeline391.js';
import '../styles/component391.css';

function MetricCard391({ label, value, max }) {
...
function TreeNode391({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode391 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState391 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer391(state, action) {
...
export default function Component391({ seed = 391, title = 'Component391' }) {
  const hook = useLogic391(seed + 391);
  const [state, dispatch] = useReducer(reducer391, initialState391);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm391(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset391(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState391(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree391(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute391(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast391(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies391(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution391(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow391(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets391(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap391(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline391(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm391(form);
    setErrors(found);
  ...
  return (
    <section className="component-391">
      <div className="header-391">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-391">
      ...
      <div className="grid-391">
        <div className="panel-391">
          {cards.map((item, idx) => (
            <MetricCard391 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
