...
import '../styles/component484.css';

function MetricCard484({ label, value, max }) {
...
function TreeNode484({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode484 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState484 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer484(state, action) {
...
export default function Component484({ seed = 484, title = 'Component484' }) {
  const hook = useLogic484(seed + 484);
  const [state, dispatch] = useReducer(reducer484, initialState484);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm484(seed));
  ...
  const table = useMemo(() => buildTableState484(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm484(form);
    setErrors(found);
  ...
  return (
    <section className="component-484">
      <div className="header-484">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-484">
      ...
      <div className="grid-484">
        <div className="panel-484">
          {cards.map((item, idx) => (
            <MetricCard484 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
