...
import { useLogic37 } from '../hooks/useLogic37.js';
import { buildNestedTree37, buildTableState37, heavyCompute37, transformDataset37 } from '../utils/compute37.js';
import { buildForecast37, detectAnomalies37, segmentDistribution37 } from '../utils/compute37.js';
import { buildActivityMap37, rankBuckets37, summarizePipeline37, synthesizeFlow37 } from '../pipelines/pipeline37.js';
import '../styles/component587.css';

function MetricCard587({ label, value, max }) {
...
function TreeNode587({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode587 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState587 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer587(state, action) {
...
export default function Component587({ seed = 587, title = 'Component587' }) {
  const hook = useLogic37(seed + 587);
  const [state, dispatch] = useReducer(reducer587, initialState587);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm587(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset37(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState37(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree37(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute37(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast37(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies37(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution37(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow37(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets37(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap37(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline37(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm587(form);
    setErrors(found);
  ...
  return (
    <section className="component-587">
      <div className="header-587">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-587">
      ...
      <div className="grid-587">
        <div className="panel-587">
          {cards.map((item, idx) => (
            <MetricCard587 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
