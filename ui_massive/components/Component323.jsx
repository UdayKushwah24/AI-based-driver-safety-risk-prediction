...
import '../styles/component323.css';

function MetricCard323({ label, value, max }) {
...
function TreeNode323({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode323 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState323 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer323(state, action) {
...
export default function Component323({ seed = 323, title = 'Component323' }) {
  const hook = useLogic323(seed + 323);
  const [state, dispatch] = useReducer(reducer323, initialState323);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm323(seed));
  ...
  const table = useMemo(() => buildTableState323(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm323(form);
    setErrors(found);
  ...
  return (
    <section className="component-323">
      <div className="header-323">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-323">
      ...
      <div className="grid-323">
        <div className="panel-323">
          {cards.map((item, idx) => (
            <MetricCard323 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
