...
import '../styles/component21.css';

function MetricCard21({ label, value, max }) {
...
function TreeNode21({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode21 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component21({ seed = 21, title = 'Component21' }) {
  const hook = useLogic21(seed + 21);
  const [state, dispatch] = useReducer(reducer21, initialState21);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm21(seed));
  ...
  const table = useMemo(() => buildTableState21(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm21(form);
    setErrors(found);
  ...
  return (
    <section className="component-21">
      ...
      <div className="grid-21">
        <div className="panel-21">
          {cards.map((item, idx) => (
            <MetricCard21 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
