...
import '../styles/component993.css';

function MetricCard993({ label, value, max }) {
...
function TreeNode993({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode993 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState993 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer993(state, action) {
...
export default function Component993({ seed = 993, title = 'Component993' }) {
  const hook = useLogic443(seed + 993);
  const [state, dispatch] = useReducer(reducer993, initialState993);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm993(seed));
  ...
  const table = useMemo(() => buildTableState443(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm993(form);
    setErrors(found);
  ...
  return (
    <section className="component-993">
      <div className="header-993">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-993">
      ...
      <div className="grid-993">
        <div className="panel-993">
          {cards.map((item, idx) => (
            <MetricCard993 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
