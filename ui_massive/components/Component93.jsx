...
import '../styles/component93.css';

function MetricCard93({ label, value, max }) {
...
function TreeNode93({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode93 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component93({ seed = 93, title = 'Component93' }) {
  const hook = useLogic93(seed + 93);
  const [state, dispatch] = useReducer(reducer93, initialState93);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm93(seed));
  ...
  const table = useMemo(() => buildTableState93(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm93(form);
    setErrors(found);
  ...
  return (
    <section className="component-93">
      ...
      <div className="grid-93">
        <div className="panel-93">
          {cards.map((item, idx) => (
            <MetricCard93 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
