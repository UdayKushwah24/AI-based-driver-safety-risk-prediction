...
import '../styles/component462.css';

function MetricCard462({ label, value, max }) {
...
function TreeNode462({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode462 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component462({ seed = 462, title = 'Component462' }) {
  const hook = useLogic462(seed + 462);
  const [state, dispatch] = useReducer(reducer462, initialState462);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm462(seed));
  ...
  const table = useMemo(() => buildTableState462(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm462(form);
    setErrors(found);
  ...
  return (
    <section className="component-462">
      <div className="header-462">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-462">
      ...
      <div className="grid-462">
        <div className="panel-462">
          {cards.map((item, idx) => (
            <MetricCard462 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
