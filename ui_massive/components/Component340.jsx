...
import '../styles/component340.css';

function MetricCard340({ label, value, max }) {
...
function TreeNode340({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode340 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState340 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer340(state, action) {
...
export default function Component340({ seed = 340, title = 'Component340' }) {
  const hook = useLogic340(seed + 340);
  const [state, dispatch] = useReducer(reducer340, initialState340);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm340(seed));
  ...
  const table = useMemo(() => buildTableState340(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm340(form);
    setErrors(found);
  ...
  return (
    <section className="component-340">
      <div className="header-340">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-340">
      ...
      <div className="grid-340">
        <div className="panel-340">
          {cards.map((item, idx) => (
            <MetricCard340 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
