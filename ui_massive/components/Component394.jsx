...
import { useLogic394 } from '../hooks/useLogic394.js';
import { buildNestedTree394, buildTableState394, heavyCompute394, transformDataset394 } from '../utils/compute394.js';
import { buildForecast394, detectAnomalies394, segmentDistribution394 } from '../utils/compute394.js';
import { buildActivityMap394, rankBuckets394, summarizePipeline394, synthesizeFlow394 } from '../pipelines/pipeline394.js';
import '../styles/component394.css';

function MetricCard394({ label, value, max }) {
...
function TreeNode394({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode394 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState394 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer394(state, action) {
...
export default function Component394({ seed = 394, title = 'Component394' }) {
  const hook = useLogic394(seed + 394);
  const [state, dispatch] = useReducer(reducer394, initialState394);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm394(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset394(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState394(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree394(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute394(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast394(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies394(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution394(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow394(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets394(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap394(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline394(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm394(form);
    setErrors(found);
  ...
  return (
    <section className="component-394">
      <div className="header-394">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-394">
      ...
      <div className="grid-394">
        <div className="panel-394">
          {cards.map((item, idx) => (
            <MetricCard394 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
