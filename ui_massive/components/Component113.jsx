...
import '../styles/component113.css';

function MetricCard113({ label, value, max }) {
...
function TreeNode113({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode113 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState113 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer113(state, action) {
...
export default function Component113({ seed = 113, title = 'Component113' }) {
  const hook = useLogic113(seed + 113);
  const [state, dispatch] = useReducer(reducer113, initialState113);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm113(seed));
  ...
  const table = useMemo(() => buildTableState113(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm113(form);
    setErrors(found);
  ...
  return (
    <section className="component-113">
      <div className="header-113">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-113">
      ...
      <div className="grid-113">
        <div className="panel-113">
          {cards.map((item, idx) => (
            <MetricCard113 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
