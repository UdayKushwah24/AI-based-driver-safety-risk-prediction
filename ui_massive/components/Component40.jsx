...
import '../styles/component40.css';

function MetricCard40({ label, value, max }) {
...
function TreeNode40({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode40 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component40({ seed = 40, title = 'Component40' }) {
  const hook = useLogic40(seed + 40);
  const [state, dispatch] = useReducer(reducer40, initialState40);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm40(seed));
  ...
  const table = useMemo(() => buildTableState40(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm40(form);
    setErrors(found);
  ...
  return (
    <section className="component-40">
      ...
      <div className="grid-40">
        <div className="panel-40">
          {cards.map((item, idx) => (
            <MetricCard40 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
