...
import { useLogic80 } from '../hooks/useLogic80.js';
import { buildNestedTree80, buildTableState80, heavyCompute80, transformDataset80 } from '../utils/compute80.js';
import { buildForecast80, detectAnomalies80, segmentDistribution80 } from '../utils/compute80.js';
import { buildActivityMap80, rankBuckets80, summarizePipeline80, synthesizeFlow80 } from '../pipelines/pipeline80.js';
import '../styles/component630.css';

function MetricCard630({ label, value, max }) {
...
function TreeNode630({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode630 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState630 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer630(state, action) {
...
export default function Component630({ seed = 630, title = 'Component630' }) {
  const hook = useLogic80(seed + 630);
  const [state, dispatch] = useReducer(reducer630, initialState630);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm630(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset80(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState80(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree80(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute80(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast80(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies80(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution80(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow80(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets80(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap80(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline80(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm630(form);
    setErrors(found);
  ...
  return (
    <section className="component-630">
      <div className="header-630">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-630">
      ...
      <div className="grid-630">
        <div className="panel-630">
          {cards.map((item, idx) => (
            <MetricCard630 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
