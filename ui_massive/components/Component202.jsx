...
import { useLogic202 } from '../hooks/useLogic202.js';
import { buildNestedTree202, buildTableState202, heavyCompute202, transformDataset202 } from '../utils/compute202.js';
import { buildForecast202, detectAnomalies202, segmentDistribution202 } from '../utils/compute202.js';
import { buildActivityMap202, rankBuckets202, summarizePipeline202, synthesizeFlow202 } from '../pipelines/pipeline202.js';
import '../styles/component202.css';

function MetricCard202({ label, value, max }) {
...
function TreeNode202({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode202 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState202 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer202(state, action) {
...
export default function Component202({ seed = 202, title = 'Component202' }) {
  const hook = useLogic202(seed + 202);
  const [state, dispatch] = useReducer(reducer202, initialState202);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm202(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset202(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState202(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree202(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute202(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast202(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies202(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution202(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow202(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets202(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap202(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline202(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm202(form);
    setErrors(found);
  ...
  return (
    <section className="component-202">
      <div className="header-202">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-202">
      ...
      <div className="grid-202">
        <div className="panel-202">
          {cards.map((item, idx) => (
            <MetricCard202 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
