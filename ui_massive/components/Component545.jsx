...
import '../styles/component545.css';

function MetricCard545({ label, value, max }) {
...
function TreeNode545({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode545 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState545 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer545(state, action) {
...
export default function Component545({ seed = 545, title = 'Component545' }) {
  const hook = useLogic545(seed + 545);
  const [state, dispatch] = useReducer(reducer545, initialState545);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm545(seed));
  ...
  const table = useMemo(() => buildTableState545(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm545(form);
    setErrors(found);
  ...
  return (
    <section className="component-545">
      <div className="header-545">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-545">
      ...
      <div className="grid-545">
        <div className="panel-545">
          {cards.map((item, idx) => (
            <MetricCard545 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
