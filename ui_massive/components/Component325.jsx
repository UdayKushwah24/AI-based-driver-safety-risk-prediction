...
import { useLogic325 } from '../hooks/useLogic325.js';
import { buildNestedTree325, buildTableState325, heavyCompute325, transformDataset325 } from '../utils/compute325.js';
import { buildForecast325, detectAnomalies325, segmentDistribution325 } from '../utils/compute325.js';
import { buildActivityMap325, rankBuckets325, summarizePipeline325, synthesizeFlow325 } from '../pipelines/pipeline325.js';
import '../styles/component325.css';

function MetricCard325({ label, value, max }) {
...
function TreeNode325({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode325 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState325 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer325(state, action) {
...
export default function Component325({ seed = 325, title = 'Component325' }) {
  const hook = useLogic325(seed + 325);
  const [state, dispatch] = useReducer(reducer325, initialState325);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm325(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset325(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState325(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree325(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute325(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast325(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies325(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution325(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow325(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets325(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap325(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline325(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm325(form);
    setErrors(found);
  ...
  return (
    <section className="component-325">
      <div className="header-325">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-325">
      ...
      <div className="grid-325">
        <div className="panel-325">
          {cards.map((item, idx) => (
            <MetricCard325 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
