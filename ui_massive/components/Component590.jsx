...
import { useLogic40 } from '../hooks/useLogic40.js';
import { buildNestedTree40, buildTableState40, heavyCompute40, transformDataset40 } from '../utils/compute40.js';
import { buildForecast40, detectAnomalies40, segmentDistribution40 } from '../utils/compute40.js';
import { buildActivityMap40, rankBuckets40, summarizePipeline40, synthesizeFlow40 } from '../pipelines/pipeline40.js';
import '../styles/component590.css';

function MetricCard590({ label, value, max }) {
...
function TreeNode590({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode590 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState590 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer590(state, action) {
...
export default function Component590({ seed = 590, title = 'Component590' }) {
  const hook = useLogic40(seed + 590);
  const [state, dispatch] = useReducer(reducer590, initialState590);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm590(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset40(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState40(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree40(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute40(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast40(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies40(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution40(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow40(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets40(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap40(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline40(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm590(form);
    setErrors(found);
  ...
  return (
    <section className="component-590">
      <div className="header-590">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-590">
      ...
      <div className="grid-590">
        <div className="panel-590">
          {cards.map((item, idx) => (
            <MetricCard590 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
