...
import '../styles/component406.css';

function MetricCard406({ label, value, max }) {
...
function TreeNode406({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode406 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component406({ seed = 406, title = 'Component406' }) {
  const hook = useLogic406(seed + 406);
  const [state, dispatch] = useReducer(reducer406, initialState406);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm406(seed));
  ...
  const table = useMemo(() => buildTableState406(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm406(form);
    setErrors(found);
  ...
  return (
    <section className="component-406">
      <div className="header-406">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-406">
      ...
      <div className="grid-406">
        <div className="panel-406">
          {cards.map((item, idx) => (
            <MetricCard406 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
