...
import '../styles/component304.css';

function MetricCard304({ label, value, max }) {
...
function TreeNode304({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode304 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState304 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer304(state, action) {
...
export default function Component304({ seed = 304, title = 'Component304' }) {
  const hook = useLogic304(seed + 304);
  const [state, dispatch] = useReducer(reducer304, initialState304);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm304(seed));
  ...
  const table = useMemo(() => buildTableState304(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm304(form);
    setErrors(found);
  ...
  return (
    <section className="component-304">
      <div className="header-304">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-304">
      ...
      <div className="grid-304">
        <div className="panel-304">
          {cards.map((item, idx) => (
            <MetricCard304 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
