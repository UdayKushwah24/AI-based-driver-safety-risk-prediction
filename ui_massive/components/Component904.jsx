...
import '../styles/component904.css';

function MetricCard904({ label, value, max }) {
...
function TreeNode904({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode904 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState904 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer904(state, action) {
...
export default function Component904({ seed = 904, title = 'Component904' }) {
  const hook = useLogic354(seed + 904);
  const [state, dispatch] = useReducer(reducer904, initialState904);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm904(seed));
  ...
  const table = useMemo(() => buildTableState354(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm904(form);
    setErrors(found);
  ...
  return (
    <section className="component-904">
      <div className="header-904">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-904">
      ...
      <div className="grid-904">
        <div className="panel-904">
          {cards.map((item, idx) => (
            <MetricCard904 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
