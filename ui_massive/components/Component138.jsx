...
import '../styles/component138.css';

function MetricCard138({ label, value, max }) {
...
function TreeNode138({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode138 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component138({ seed = 138, title = 'Component138' }) {
  const hook = useLogic138(seed + 138);
  const [state, dispatch] = useReducer(reducer138, initialState138);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm138(seed));
  ...
  const table = useMemo(() => buildTableState138(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm138(form);
    setErrors(found);
  ...
  return (
    <section className="component-138">
      ...
      <div className="grid-138">
        <div className="panel-138">
          {cards.map((item, idx) => (
            <MetricCard138 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
