...
import '../styles/component297.css';

function MetricCard297({ label, value, max }) {
...
function TreeNode297({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode297 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState297 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer297(state, action) {
...
export default function Component297({ seed = 297, title = 'Component297' }) {
  const hook = useLogic297(seed + 297);
  const [state, dispatch] = useReducer(reducer297, initialState297);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm297(seed));
  ...
  const table = useMemo(() => buildTableState297(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm297(form);
    setErrors(found);
  ...
  return (
    <section className="component-297">
      <div className="header-297">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-297">
      ...
      <div className="grid-297">
        <div className="panel-297">
          {cards.map((item, idx) => (
            <MetricCard297 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
