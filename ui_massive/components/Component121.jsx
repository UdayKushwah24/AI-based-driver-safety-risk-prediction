...
import '../styles/component121.css';

function MetricCard121({ label, value, max }) {
...
function TreeNode121({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode121 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component121({ seed = 121, title = 'Component121' }) {
  const hook = useLogic121(seed + 121);
  const [state, dispatch] = useReducer(reducer121, initialState121);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm121(seed));
  ...
  const table = useMemo(() => buildTableState121(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm121(form);
    setErrors(found);
  ...
  return (
    <section className="component-121">
      ...
      <div className="grid-121">
        <div className="panel-121">
          {cards.map((item, idx) => (
            <MetricCard121 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
