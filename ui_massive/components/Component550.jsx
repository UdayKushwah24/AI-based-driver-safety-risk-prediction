...
import '../styles/component550.css';

function MetricCard550({ label, value, max }) {
...
function TreeNode550({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode550 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState550 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer550(state, action) {
...
export default function Component550({ seed = 550, title = 'Component550' }) {
  const hook = useLogic550(seed + 550);
  const [state, dispatch] = useReducer(reducer550, initialState550);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm550(seed));
  ...
  const table = useMemo(() => buildTableState550(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm550(form);
    setErrors(found);
  ...
  return (
    <section className="component-550">
      <div className="header-550">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-550">
      ...
      <div className="grid-550">
        <div className="panel-550">
          {cards.map((item, idx) => (
            <MetricCard550 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
