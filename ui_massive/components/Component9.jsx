...
function MetricCard9({ label, value, max }) {
...
function TreeNode9({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode9 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component9({ seed = 9, title = 'Component9' }) {
  const hook = useLogic9(seed + 9);
  const [state, dispatch] = useReducer(reducer9, initialState9);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm9(seed));
  ...
  const table = useMemo(() => buildTableState9(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm9(form);
    setErrors(found);
    ...
    <section className="component-9">
      ...
      <div className="grid-9">
        <div className="panel-9">
          {cards.map((item, idx) => (
            <MetricCard9 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
