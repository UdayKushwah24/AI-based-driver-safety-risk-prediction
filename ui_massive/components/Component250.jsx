...
import { useLogic250 } from '../hooks/useLogic250.js';
import { buildNestedTree250, buildTableState250, heavyCompute250, transformDataset250 } from '../utils/compute250.js';
import { buildForecast250, detectAnomalies250, segmentDistribution250 } from '../utils/compute250.js';
import { buildActivityMap250, rankBuckets250, summarizePipeline250, synthesizeFlow250 } from '../pipelines/pipeline250.js';
import '../styles/component250.css';

function MetricCard250({ label, value, max }) {
...
function TreeNode250({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode250 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState250 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer250(state, action) {
...
export default function Component250({ seed = 250, title = 'Component250' }) {
  const hook = useLogic250(seed + 250);
  const [state, dispatch] = useReducer(reducer250, initialState250);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm250(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset250(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState250(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree250(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute250(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast250(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies250(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution250(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow250(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets250(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap250(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline250(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm250(form);
    setErrors(found);
  ...
  return (
    <section className="component-250">
      <div className="header-250">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-250">
      ...
      <div className="grid-250">
        <div className="panel-250">
          {cards.map((item, idx) => (
            <MetricCard250 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
