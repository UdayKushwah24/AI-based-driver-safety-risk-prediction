...
import '../styles/component889.css';

function MetricCard889({ label, value, max }) {
...
function TreeNode889({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode889 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState889 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer889(state, action) {
...
export default function Component889({ seed = 889, title = 'Component889' }) {
  const hook = useLogic339(seed + 889);
  const [state, dispatch] = useReducer(reducer889, initialState889);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm889(seed));
  ...
  const table = useMemo(() => buildTableState339(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm889(form);
    setErrors(found);
  ...
  return (
    <section className="component-889">
      <div className="header-889">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-889">
      ...
      <div className="grid-889">
        <div className="panel-889">
          {cards.map((item, idx) => (
            <MetricCard889 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
