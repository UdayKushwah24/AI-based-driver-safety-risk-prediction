...
import '../styles/component271.css';

function MetricCard271({ label, value, max }) {
...
function TreeNode271({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode271 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState271 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer271(state, action) {
...
export default function Component271({ seed = 271, title = 'Component271' }) {
  const hook = useLogic271(seed + 271);
  const [state, dispatch] = useReducer(reducer271, initialState271);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm271(seed));
  ...
  const table = useMemo(() => buildTableState271(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm271(form);
    setErrors(found);
  ...
  return (
    <section className="component-271">
      <div className="header-271">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-271">
      ...
      <div className="grid-271">
        <div className="panel-271">
          {cards.map((item, idx) => (
            <MetricCard271 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
