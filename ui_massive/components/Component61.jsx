...
import '../styles/component61.css';

function MetricCard61({ label, value, max }) {
...
function TreeNode61({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode61 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component61({ seed = 61, title = 'Component61' }) {
  const hook = useLogic61(seed + 61);
  const [state, dispatch] = useReducer(reducer61, initialState61);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm61(seed));
  ...
  const table = useMemo(() => buildTableState61(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm61(form);
    setErrors(found);
  ...
  return (
    <section className="component-61">
      ...
      <div className="grid-61">
        <div className="panel-61">
          {cards.map((item, idx) => (
            <MetricCard61 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
