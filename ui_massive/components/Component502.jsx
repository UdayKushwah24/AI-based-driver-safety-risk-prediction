...
import '../styles/component502.css';

function MetricCard502({ label, value, max }) {
...
function TreeNode502({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode502 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState502 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer502(state, action) {
...
export default function Component502({ seed = 502, title = 'Component502' }) {
  const hook = useLogic502(seed + 502);
  const [state, dispatch] = useReducer(reducer502, initialState502);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm502(seed));
  ...
  const table = useMemo(() => buildTableState502(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm502(form);
    setErrors(found);
  ...
  return (
    <section className="component-502">
      ...
      <div className="grid-502">
        <div className="panel-502">
          {cards.map((item, idx) => (
            <MetricCard502 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
