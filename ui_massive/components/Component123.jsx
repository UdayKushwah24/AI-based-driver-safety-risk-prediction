...
import '../styles/component123.css';

function MetricCard123({ label, value, max }) {
...
function TreeNode123({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode123 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component123({ seed = 123, title = 'Component123' }) {
  const hook = useLogic123(seed + 123);
  const [state, dispatch] = useReducer(reducer123, initialState123);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm123(seed));
  ...
  const table = useMemo(() => buildTableState123(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm123(form);
    setErrors(found);
  ...
  return (
    <section className="component-123">
      ...
      <div className="grid-123">
        <div className="panel-123">
          {cards.map((item, idx) => (
            <MetricCard123 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
