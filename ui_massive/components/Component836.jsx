...
import '../styles/component836.css';

function MetricCard836({ label, value, max }) {
...
function TreeNode836({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode836 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState836 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer836(state, action) {
...
export default function Component836({ seed = 836, title = 'Component836' }) {
  const hook = useLogic286(seed + 836);
  const [state, dispatch] = useReducer(reducer836, initialState836);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm836(seed));
  ...
  const table = useMemo(() => buildTableState286(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm836(form);
    setErrors(found);
  ...
  return (
    <section className="component-836">
      <div className="header-836">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-836">
      ...
      <div className="grid-836">
        <div className="panel-836">
          {cards.map((item, idx) => (
            <MetricCard836 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
