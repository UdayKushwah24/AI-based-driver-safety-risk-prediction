...
import { useLogic78 } from '../hooks/useLogic78.js';
import { buildNestedTree78, buildTableState78, heavyCompute78, transformDataset78 } from '../utils/compute78.js';
import { buildForecast78, detectAnomalies78, segmentDistribution78 } from '../utils/compute78.js';
import { buildActivityMap78, rankBuckets78, summarizePipeline78, synthesizeFlow78 } from '../pipelines/pipeline78.js';
import '../styles/component628.css';

function MetricCard628({ label, value, max }) {
...
function TreeNode628({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode628 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState628 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer628(state, action) {
...
export default function Component628({ seed = 628, title = 'Component628' }) {
  const hook = useLogic78(seed + 628);
  const [state, dispatch] = useReducer(reducer628, initialState628);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm628(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset78(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState78(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree78(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute78(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast78(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies78(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution78(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow78(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets78(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap78(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline78(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm628(form);
    setErrors(found);
  ...
  return (
    <section className="component-628">
      <div className="header-628">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-628">
      ...
      <div className="grid-628">
        <div className="panel-628">
          {cards.map((item, idx) => (
            <MetricCard628 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
