...
import '../styles/component736.css';

function MetricCard736({ label, value, max }) {
...
function TreeNode736({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode736 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState736 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer736(state, action) {
...
export default function Component736({ seed = 736, title = 'Component736' }) {
  const hook = useLogic186(seed + 736);
  const [state, dispatch] = useReducer(reducer736, initialState736);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm736(seed));
  ...
  const table = useMemo(() => buildTableState186(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm736(form);
    setErrors(found);
  ...
  return (
    <section className="component-736">
      <div className="header-736">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-736">
      ...
      <div className="grid-736">
        <div className="panel-736">
          {cards.map((item, idx) => (
            <MetricCard736 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
