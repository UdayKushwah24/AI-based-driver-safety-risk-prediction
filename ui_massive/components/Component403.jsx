...
import '../styles/component403.css';

function MetricCard403({ label, value, max }) {
...
function TreeNode403({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode403 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState403 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer403(state, action) {
...
export default function Component403({ seed = 403, title = 'Component403' }) {
  const hook = useLogic403(seed + 403);
  const [state, dispatch] = useReducer(reducer403, initialState403);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm403(seed));
  ...
  const table = useMemo(() => buildTableState403(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm403(form);
    setErrors(found);
  ...
  return (
    <section className="component-403">
      <div className="header-403">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-403">
      ...
      <div className="grid-403">
        <div className="panel-403">
          {cards.map((item, idx) => (
            <MetricCard403 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
