...
import '../styles/component279.css';

function MetricCard279({ label, value, max }) {
...
function TreeNode279({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode279 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState279 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer279(state, action) {
...
export default function Component279({ seed = 279, title = 'Component279' }) {
  const hook = useLogic279(seed + 279);
  const [state, dispatch] = useReducer(reducer279, initialState279);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm279(seed));
  ...
  const table = useMemo(() => buildTableState279(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm279(form);
    setErrors(found);
  ...
  return (
    <section className="component-279">
      <div className="header-279">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-279">
      ...
      <div className="grid-279">
        <div className="panel-279">
          {cards.map((item, idx) => (
            <MetricCard279 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
