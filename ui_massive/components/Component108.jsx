...
import '../styles/component108.css';

function MetricCard108({ label, value, max }) {
...
function TreeNode108({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode108 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component108({ seed = 108, title = 'Component108' }) {
  const hook = useLogic108(seed + 108);
  const [state, dispatch] = useReducer(reducer108, initialState108);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm108(seed));
  ...
  const table = useMemo(() => buildTableState108(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm108(form);
    setErrors(found);
  ...
  return (
    <section className="component-108">
      ...
      <div className="grid-108">
        <div className="panel-108">
          {cards.map((item, idx) => (
            <MetricCard108 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
