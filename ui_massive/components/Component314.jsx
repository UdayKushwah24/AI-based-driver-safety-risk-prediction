...
import { useLogic314 } from '../hooks/useLogic314.js';
import { buildNestedTree314, buildTableState314, heavyCompute314, transformDataset314 } from '../utils/compute314.js';
import { buildForecast314, detectAnomalies314, segmentDistribution314 } from '../utils/compute314.js';
import { buildActivityMap314, rankBuckets314, summarizePipeline314, synthesizeFlow314 } from '../pipelines/pipeline314.js';
import '../styles/component314.css';

function MetricCard314({ label, value, max }) {
...
function TreeNode314({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode314 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState314 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer314(state, action) {
...
export default function Component314({ seed = 314, title = 'Component314' }) {
  const hook = useLogic314(seed + 314);
  const [state, dispatch] = useReducer(reducer314, initialState314);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm314(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset314(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState314(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree314(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute314(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast314(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies314(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution314(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow314(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets314(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap314(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline314(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm314(form);
    setErrors(found);
  ...
  return (
    <section className="component-314">
      <div className="header-314">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-314">
      ...
      <div className="grid-314">
        <div className="panel-314">
          {cards.map((item, idx) => (
            <MetricCard314 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
