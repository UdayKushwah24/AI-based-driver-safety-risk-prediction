...
import '../styles/component875.css';

function MetricCard875({ label, value, max }) {
...
function TreeNode875({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode875 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState875 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer875(state, action) {
...
export default function Component875({ seed = 875, title = 'Component875' }) {
  const hook = useLogic325(seed + 875);
  const [state, dispatch] = useReducer(reducer875, initialState875);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm875(seed));
  ...
  const table = useMemo(() => buildTableState325(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm875(form);
    setErrors(found);
  ...
  return (
    <section className="component-875">
      <div className="header-875">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-875">
      ...
      <div className="grid-875">
        <div className="panel-875">
          {cards.map((item, idx) => (
            <MetricCard875 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
