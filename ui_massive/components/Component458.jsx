...
import '../styles/component458.css';

function MetricCard458({ label, value, max }) {
...
function TreeNode458({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode458 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState458 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer458(state, action) {
...
export default function Component458({ seed = 458, title = 'Component458' }) {
  const hook = useLogic458(seed + 458);
  const [state, dispatch] = useReducer(reducer458, initialState458);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm458(seed));
  ...
  const table = useMemo(() => buildTableState458(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm458(form);
    setErrors(found);
  ...
  return (
    <section className="component-458">
      <div className="header-458">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-458">
      ...
      <div className="grid-458">
        <div className="panel-458">
          {cards.map((item, idx) => (
            <MetricCard458 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
