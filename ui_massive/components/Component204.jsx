...
import '../styles/component204.css';

function MetricCard204({ label, value, max }) {
...
function TreeNode204({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode204 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState204 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer204(state, action) {
...
export default function Component204({ seed = 204, title = 'Component204' }) {
  const hook = useLogic204(seed + 204);
  const [state, dispatch] = useReducer(reducer204, initialState204);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm204(seed));
  ...
  const table = useMemo(() => buildTableState204(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm204(form);
    setErrors(found);
  ...
  return (
    <section className="component-204">
      <div className="header-204">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-204">
      ...
      <div className="grid-204">
        <div className="panel-204">
          {cards.map((item, idx) => (
            <MetricCard204 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
