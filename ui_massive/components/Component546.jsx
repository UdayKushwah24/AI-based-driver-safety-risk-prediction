...
import '../styles/component546.css';

function MetricCard546({ label, value, max }) {
...
function TreeNode546({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode546 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState546 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer546(state, action) {
...
export default function Component546({ seed = 546, title = 'Component546' }) {
  const hook = useLogic546(seed + 546);
  const [state, dispatch] = useReducer(reducer546, initialState546);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm546(seed));
  ...
  const table = useMemo(() => buildTableState546(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm546(form);
    setErrors(found);
  ...
  return (
    <section className="component-546">
      <div className="header-546">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-546">
      ...
      <div className="grid-546">
        <div className="panel-546">
          {cards.map((item, idx) => (
            <MetricCard546 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
