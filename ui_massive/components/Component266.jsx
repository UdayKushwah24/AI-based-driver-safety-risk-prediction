...
import { useLogic266 } from '../hooks/useLogic266.js';
import { buildNestedTree266, buildTableState266, heavyCompute266, transformDataset266 } from '../utils/compute266.js';
import { buildForecast266, detectAnomalies266, segmentDistribution266 } from '../utils/compute266.js';
import { buildActivityMap266, rankBuckets266, summarizePipeline266, synthesizeFlow266 } from '../pipelines/pipeline266.js';
import '../styles/component266.css';

function MetricCard266({ label, value, max }) {
...
function TreeNode266({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode266 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState266 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer266(state, action) {
...
export default function Component266({ seed = 266, title = 'Component266' }) {
  const hook = useLogic266(seed + 266);
  const [state, dispatch] = useReducer(reducer266, initialState266);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm266(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset266(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState266(rows, sortBy, sortDir, page, 11), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree266(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute266(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast266(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies266(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution266(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow266(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets266(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap266(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline266(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm266(form);
    setErrors(found);
  ...
  return (
    <section className="component-266">
      <div className="header-266">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-266">
      ...
      <div className="grid-266">
        <div className="panel-266">
          {cards.map((item, idx) => (
            <MetricCard266 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
