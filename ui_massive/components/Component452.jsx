...
import '../styles/component452.css';

function MetricCard452({ label, value, max }) {
...
function TreeNode452({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode452 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState452 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer452(state, action) {
...
export default function Component452({ seed = 452, title = 'Component452' }) {
  const hook = useLogic452(seed + 452);
  const [state, dispatch] = useReducer(reducer452, initialState452);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm452(seed));
  ...
  const table = useMemo(() => buildTableState452(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm452(form);
    setErrors(found);
  ...
  return (
    <section className="component-452">
      <div className="header-452">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-452">
      ...
      <div className="grid-452">
        <div className="panel-452">
          {cards.map((item, idx) => (
            <MetricCard452 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
