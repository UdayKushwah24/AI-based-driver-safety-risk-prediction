...
import '../styles/component511.css';

function MetricCard511({ label, value, max }) {
...
function TreeNode511({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode511 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState511 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer511(state, action) {
...
export default function Component511({ seed = 511, title = 'Component511' }) {
  const hook = useLogic511(seed + 511);
  const [state, dispatch] = useReducer(reducer511, initialState511);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm511(seed));
  ...
  const table = useMemo(() => buildTableState511(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm511(form);
    setErrors(found);
  ...
  return (
    <section className="component-511">
      <div className="header-511">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-511">
      ...
      <div className="grid-511">
        <div className="panel-511">
          {cards.map((item, idx) => (
            <MetricCard511 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
