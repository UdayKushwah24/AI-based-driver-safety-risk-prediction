...
import '../styles/component925.css';

function MetricCard925({ label, value, max }) {
...
function TreeNode925({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode925 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState925 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer925(state, action) {
...
export default function Component925({ seed = 925, title = 'Component925' }) {
  const hook = useLogic375(seed + 925);
  const [state, dispatch] = useReducer(reducer925, initialState925);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm925(seed));
  ...
  const table = useMemo(() => buildTableState375(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm925(form);
    setErrors(found);
  ...
  return (
    <section className="component-925">
      <div className="header-925">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-925">
      ...
      <div className="grid-925">
        <div className="panel-925">
          {cards.map((item, idx) => (
            <MetricCard925 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
