...
import { useLogic106 } from '../hooks/useLogic106.js';
import { buildNestedTree106, buildTableState106, heavyCompute106, transformDataset106 } from '../utils/compute106.js';
import { buildForecast106, detectAnomalies106, segmentDistribution106 } from '../utils/compute106.js';
import { buildActivityMap106, rankBuckets106, summarizePipeline106, synthesizeFlow106 } from '../pipelines/pipeline106.js';
import '../styles/component656.css';

function MetricCard656({ label, value, max }) {
...
function TreeNode656({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode656 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState656 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer656(state, action) {
...
export default function Component656({ seed = 656, title = 'Component656' }) {
  const hook = useLogic106(seed + 656);
  const [state, dispatch] = useReducer(reducer656, initialState656);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm656(seed));
  const [errors, setErrors] = useState({});

  const rows = useMemo(() => transformDataset106(hook.series, query, state.mode, state.bias), [hook.series, query, state.mode, state.bias]);
  const table = useMemo(() => buildTableState106(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  const tree = useMemo(() => buildNestedTree106(rows.slice(0, 28), 4), [rows]);
  const compute = useMemo(() => heavyCompute106(14 + (seed % 5), 2 + (state.bias % 5)), [seed, state.bias]);
  const forecast = useMemo(() => buildForecast106(rows, 24 + (state.bias % 6)), [rows, state.bias]);
  const anomalies = useMemo(() => detectAnomalies106(rows, 68 + (state.bias % 9)), [rows, state.bias]);
  const distribution = useMemo(() => segmentDistribution106(rows), [rows]);
  const flow = useMemo(() => synthesizeFlow106(rows, state.bias), [rows, state.bias]);
  const buckets = useMemo(() => rankBuckets106(flow, 9 + (state.bias % 4)), [flow, state.bias]);
  const activity = useMemo(() => buildActivityMap106(flow, 8, 8), [flow]);
  const pipelineSummary = useMemo(() => summarizePipeline106(flow, buckets), [flow, buckets]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm656(form);
    setErrors(found);
  ...
  return (
    <section className="component-656">
      <div className="header-656">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-656">
      ...
      <div className="grid-656">
        <div className="panel-656">
          {cards.map((item, idx) => (
            <MetricCard656 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
