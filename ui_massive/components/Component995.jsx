...
import '../styles/component995.css';

function MetricCard995({ label, value, max }) {
...
function TreeNode995({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode995 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState995 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer995(state, action) {
...
export default function Component995({ seed = 995, title = 'Component995' }) {
  const hook = useLogic445(seed + 995);
  const [state, dispatch] = useReducer(reducer995, initialState995);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm995(seed));
  ...
  const table = useMemo(() => buildTableState445(rows, sortBy, sortDir, page, 12), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm995(form);
    setErrors(found);
  ...
  return (
    <section className="component-995">
      <div className="header-995">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-995">
      ...
      <div className="grid-995">
        <div className="panel-995">
          {cards.map((item, idx) => (
            <MetricCard995 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
