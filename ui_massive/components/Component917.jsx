...
import '../styles/component917.css';

function MetricCard917({ label, value, max }) {
...
function TreeNode917({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode917 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState917 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer917(state, action) {
...
export default function Component917({ seed = 917, title = 'Component917' }) {
  const hook = useLogic367(seed + 917);
  const [state, dispatch] = useReducer(reducer917, initialState917);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm917(seed));
  ...
  const table = useMemo(() => buildTableState367(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm917(form);
    setErrors(found);
  ...
  return (
    <section className="component-917">
      <div className="header-917">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-917">
      ...
      <div className="grid-917">
        <div className="panel-917">
          {cards.map((item, idx) => (
            <MetricCard917 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
