...
import '../styles/component745.css';

function MetricCard745({ label, value, max }) {
...
function TreeNode745({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode745 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState745 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer745(state, action) {
...
export default function Component745({ seed = 745, title = 'Component745' }) {
  const hook = useLogic195(seed + 745);
  const [state, dispatch] = useReducer(reducer745, initialState745);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm745(seed));
  ...
  const table = useMemo(() => buildTableState195(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm745(form);
    setErrors(found);
  ...
  return (
    <section className="component-745">
      <div className="header-745">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-745">
      ...
      <div className="grid-745">
        <div className="panel-745">
          {cards.map((item, idx) => (
            <MetricCard745 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
