...
import '../styles/component520.css';

function MetricCard520({ label, value, max }) {
...
function TreeNode520({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode520 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState520 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer520(state, action) {
...
export default function Component520({ seed = 520, title = 'Component520' }) {
  const hook = useLogic520(seed + 520);
  const [state, dispatch] = useReducer(reducer520, initialState520);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm520(seed));
  ...
  const table = useMemo(() => buildTableState520(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm520(form);
    setErrors(found);
  ...
  return (
    <section className="component-520">
      <div className="header-520">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-520">
      ...
      <div className="grid-520">
        <div className="panel-520">
          {cards.map((item, idx) => (
            <MetricCard520 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
