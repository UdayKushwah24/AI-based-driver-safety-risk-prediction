...
import '../styles/component260.css';

function MetricCard260({ label, value, max }) {
...
function TreeNode260({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode260 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState260 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer260(state, action) {
...
export default function Component260({ seed = 260, title = 'Component260' }) {
  const hook = useLogic260(seed + 260);
  const [state, dispatch] = useReducer(reducer260, initialState260);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm260(seed));
  ...
  const table = useMemo(() => buildTableState260(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm260(form);
    setErrors(found);
  ...
  return (
    <section className="component-260">
      <div className="header-260">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-260">
      ...
      <div className="grid-260">
        <div className="panel-260">
          {cards.map((item, idx) => (
            <MetricCard260 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
