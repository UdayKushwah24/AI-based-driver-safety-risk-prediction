...
function MetricCard261({ label, value, max }) {
...
function TreeNode261({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode261 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}
...
export default function Component261({ seed = 261, title = 'Component261' }) {
  const hook = useLogic261(seed + 261);
  const [state, dispatch] = useReducer(reducer261, initialState261);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm261(seed));
  ...
  const table = useMemo(() => buildTableState261(rows, sortBy, sortDir, page, 10), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm261(form);
    setErrors(found);
  ...
  return (
    <section className="component-261">
      <div className="header-261">
        <div>
          <h3>{title}</h3>
          <div style={{ fontSize: 12 }}>status {state.status} submissions {state.submissions}</div>
        </div>
        <div className="controls-261">
      ...
      <div className="grid-261">
        <div className="panel-261">
          {cards.map((item, idx) => (
            <MetricCard261 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
