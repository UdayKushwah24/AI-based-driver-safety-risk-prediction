...
import '../styles/component368.css';

function MetricCard368({ label, value, max }) {
...
function TreeNode368({ node, depth, activeId, onPick }) {
  ...
  return (
    <div style={{ marginLeft: depth * 10 }}>
      ...
      {depth < 4 && Array.isArray(node.children) && node.children.map((child) => (
        <TreeNode368 key={child.id} node={child} depth={depth + 1} activeId={activeId} onPick={onPick} />
      ))}
    </div>
  );
}

const initialState368 = {
  mode: 'balanced',
  tab: 'dashboard',
  activeId: 0,
  submissions: 0,
...
function reducer368(state, action) {
...
export default function Component368({ seed = 368, title = 'Component368' }) {
  const hook = useLogic368(seed + 368);
  const [state, dispatch] = useReducer(reducer368, initialState368);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState(createForm368(seed));
  ...
  const table = useMemo(() => buildTableState368(rows, sortBy, sortDir, page, 9), [rows, sortBy, sortDir, page]);
  ...
  function handleSubmit(event) {
    ...
    const found = validateForm368(form);
    setErrors(found);
  ...
  return (
    <section className="component-368">
      ...
      <div className="grid-368">
        <div className="panel-368">
          {cards.map((item, idx) => (
            <MetricCard368 key={item.label + '-' + idx} label={item.label} value={item.value} max={item.max} />
          ))}
