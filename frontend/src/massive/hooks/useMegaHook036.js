...
const baseSeries036 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 36) / 4) * 12;
  const drift = Math.cos((idx + 36) / 8) * 8;
  const offset = ((idx * 36) % 17) - 8;
...
function smooth036(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize036(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags036(values, mode) {
  ...
  const spread = normalize036(peak - valley);
  const jitter = normalize036(Math.sqrt(variance));
  const risk = normalize036(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (36 % 9));
  return {
    avg: normalize036(avg),
    peak: normalize036(peak),
    valley: normalize036(valley),
    spread,
    jitter,
    risk,
    mode,
...
function fakeApi036(seed, mode, budget) {
  return new Promise((resolve) => {
    const delay = 8 + (seed % 6) * 4 + (budget % 4) * 3;
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        const base = Math.sin((idx + seed) / 17) * 9 + Math.cos((idx + seed) / 4) * 6;
        const adjust = mode === 'safe' ? -5 : mode === 'aggressive' ? 7 : 1;
        return normalize036(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook036(initialSeed = 36) {
  ...
  useEffect(() => {
    ...
    fakeApi036(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries036.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize036(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth036(merged, state.window);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 8) {
      ...
      chunks.push(normalize036(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => deriveFlags036(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, n, idx) => sum + n * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize036(quality) });
  }, [trend]);
