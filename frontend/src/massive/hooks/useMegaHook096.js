...
const baseSeries096 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 96) / 8) * 12;
  const drift = Math.cos((idx + 96) / 13) * 8;
  const offset = ((idx * 96) % 17) - 8;
...
function smooth096(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize096(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags096(values, mode) {
  ...
  const spread = normalize096(peak - valley);
  const jitter = normalize096(Math.sqrt(variance));
  const risk = normalize096(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (96 % 9));
  return {
    avg: normalize096(avg),
    peak: normalize096(peak),
    valley: normalize096(valley),
    spread,
    jitter,
    risk,
    mode,
...
function fakeApi096(seed, mode, budget) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        ...
        return normalize096(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook096(initialSeed = 96) {
  ...
  useEffect(() => {
    ...
    fakeApi096(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries096.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize096(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth096(merged, state.window);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 8) {
      ...
      chunks.push(normalize096(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => deriveFlags096(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, n, idx) => sum + n * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize096(quality) });
  }, [trend]);
