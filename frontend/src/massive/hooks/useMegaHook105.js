...
const baseSeries105 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 105) / 3) * 12;
  const drift = Math.cos((idx + 105) / 11) * 8;
  const offset = ((idx * 105) % 17) - 8;
...
function smooth105(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize105(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags105(values, mode) {
  ...
  const risk = normalize105(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (105 % 9));
  return {
...
function fakeApi105(seed, mode, budget) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        ...
        return normalize105(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook105(initialSeed = 105) {
  ...
  useEffect(() => {
    ...
    fakeApi105(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries105.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize105(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth105(merged, state.window);
  ...
  const flags = useMemo(() => deriveFlags105(series, state.mode), [series, state.mode]);
