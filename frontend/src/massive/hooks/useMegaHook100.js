...
const baseSeries100 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 100) / 5) * 12;
  const drift = Math.cos((idx + 100) / 6) * 8;
  const offset = ((idx * 100) % 17) - 8;
...
function smooth100(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize100(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags100(values, mode) {
  ...
  const risk = normalize100(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (100 % 9));
  return {
...
function fakeApi100(seed, mode, budget) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        ...
        return normalize100(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook100(initialSeed = 100) {
  ...
  useEffect(() => {
    ...
    fakeApi100(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries100.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize100(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth100(merged, state.window);
  ...
  const flags = useMemo(() => deriveFlags100(series, state.mode), [series, state.mode]);
