...
const baseSeries025 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 25) / 7) * 12;
  const drift = Math.cos((idx + 25) / 8) * 8;
  const offset = ((idx * 25) % 17) - 8;
...
function smooth025(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize025(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags025(values, mode) {
  ...
  const spread = normalize025(peak - valley);
  const jitter = normalize025(Math.sqrt(variance));
  const risk = normalize025(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (25 % 9));
  return {
    avg: normalize025(avg),
    peak: normalize025(peak),
    valley: normalize025(valley),
    spread,
    jitter,
    risk,
    mode,
...
function fakeApi025(seed, mode, budget) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        ...
        return normalize025(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook025(initialSeed = 25) {
  ...
  useEffect(() => {
    ...
    fakeApi025(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries025.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize025(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth025(merged, state.window);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 8) {
      ...
      chunks.push(normalize025(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => deriveFlags025(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, n, idx) => sum + n * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize025(quality) });
  }, [trend]);
