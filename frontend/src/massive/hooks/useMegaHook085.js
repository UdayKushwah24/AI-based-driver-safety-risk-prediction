...
const baseSeries085 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 85) / 4) * 12;
  const drift = Math.cos((idx + 85) / 13) * 8;
  const offset = ((idx * 85) % 17) - 8;
...
function smooth085(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize085(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags085(values, mode) {
  ...
  const spread = normalize085(peak - valley);
  const jitter = normalize085(Math.sqrt(variance));
  const risk = normalize085(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (85 % 9));
  return {
    avg: normalize085(avg),
    peak: normalize085(peak),
    valley: normalize085(valley),
    spread,
    jitter,
    risk,
    mode,
...
function fakeApi085(seed, mode, budget) {
  return new Promise((resolve) => {
    const delay = 8 + (seed % 6) * 4 + (budget % 4) * 3;
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        const base = Math.sin((idx + seed) / 14) * 9 + Math.cos((idx + seed) / 4) * 6;
        const adjust = mode === 'safe' ? -5 : mode === 'aggressive' ? 7 : 1;
        return normalize085(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook085(initialSeed = 85) {
  ...
  useEffect(() => {
    ...
    fakeApi085(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries085.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize085(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth085(merged, state.window);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 8) {
      ...
      chunks.push(normalize085(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => deriveFlags085(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, n, idx) => sum + n * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize085(quality) });
  }, [trend]);
