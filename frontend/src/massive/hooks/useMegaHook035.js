...
const baseSeries035 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 35) / 3) * 12;
  const drift = Math.cos((idx + 35) / 7) * 8;
  const offset = ((idx * 35) % 17) - 8;
...
function smooth035(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize035(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags035(values, mode) {
  ...
  const spread = normalize035(peak - valley);
  const jitter = normalize035(Math.sqrt(variance));
  const risk = normalize035(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (35 % 9));
  return {
    avg: normalize035(avg),
    peak: normalize035(peak),
    valley: normalize035(valley),
    spread,
    jitter,
    risk,
    mode,
...
function fakeApi035(seed, mode, budget) {
  return new Promise((resolve) => {
    const delay = 8 + (seed % 6) * 4 + (budget % 4) * 3;
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        const base = Math.sin((idx + seed) / 16) * 9 + Math.cos((idx + seed) / 3) * 6;
        const adjust = mode === 'safe' ? -5 : mode === 'aggressive' ? 7 : 1;
        return normalize035(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook035(initialSeed = 35) {
  ...
  useEffect(() => {
    ...
    fakeApi035(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries035.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize035(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth035(merged, state.window);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 8) {
      ...
      chunks.push(normalize035(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => deriveFlags035(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, n, idx) => sum + n * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize035(quality) });
  }, [trend]);
