...
const baseSeries104 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 104) / 9) * 12;
  const drift = Math.cos((idx + 104) / 10) * 8;
  const offset = ((idx * 104) % 17) - 8;
...
function smooth104(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize104(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags104(values, mode) {
  ...
  const risk = normalize104(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (104 % 9));
  return {
...
function fakeApi104(seed, mode, budget) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        ...
        return normalize104(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook104(initialSeed = 104) {
  ...
  useEffect(() => {
    ...
    fakeApi104(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries104.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize104(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth104(merged, state.window);
  ...
  const flags = useMemo(() => deriveFlags104(series, state.mode), [series, state.mode]);
