...
const baseSeries108 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 108) / 6) * 12;
  const drift = Math.cos((idx + 108) / 14) * 8;
  const offset = ((idx * 108) % 17) - 8;
...
function smooth108(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize108(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags108(values, mode) {
  ...
  const risk = normalize108(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (108 % 9));
  return {
...
function fakeApi108(seed, mode, budget) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        ...
        return normalize108(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook108(initialSeed = 108) {
  ...
  useEffect(() => {
    ...
    fakeApi108(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries108.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize108(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth108(merged, state.window);
  ...
  const flags = useMemo(() => deriveFlags108(series, state.mode), [series, state.mode]);
