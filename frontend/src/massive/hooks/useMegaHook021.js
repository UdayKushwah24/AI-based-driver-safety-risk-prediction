...
const baseSeries021 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 21) / 3) * 12;
  const drift = Math.cos((idx + 21) / 15) * 8;
  const offset = ((idx * 21) % 17) - 8;
  return Number((45 + wave + drift + offset).toFixed(3));
});

const initialState021 = {
  mode: 'balanced',
  window: 3,
  tick: 0,
  loading: false,
...
function reducer021(state, action) {
  switch (action.type) {
    case 'mode':
...
function smooth021(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize021(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags021(values, mode) {
  ...
  const spread = normalize021(peak - valley);
  const jitter = normalize021(Math.sqrt(variance));
  const risk = normalize021(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (21 % 9));
  return {
    avg: normalize021(avg),
    peak: normalize021(peak),
    valley: normalize021(valley),
    spread,
    jitter,
    risk,
    mode,
...
function fakeApi021(seed, mode, budget) {
  return new Promise((resolve) => {
    const delay = 8 + (seed % 6) * 4 + (budget % 4) * 3;
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        const base = Math.sin((idx + seed) / 15) * 9 + Math.cos((idx + seed) / 3) * 6;
        const adjust = mode === 'safe' ? -5 : mode === 'aggressive' ? 7 : 1;
        return normalize021(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook021(initialSeed = 21) {
  const [seed, setSeed] = useState(initialSeed);
  const [state, dispatch] = useReducer(reducer021, initialState021);

  useEffect(() => {
    let active = true;
    dispatch({ type: 'loading', payload: true });
    fakeApi021(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries021.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize021(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth021(merged, state.window);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 8) {
      ...
      chunks.push(normalize021(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => deriveFlags021(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, n, idx) => sum + n * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize021(quality) });
  }, [trend]);
