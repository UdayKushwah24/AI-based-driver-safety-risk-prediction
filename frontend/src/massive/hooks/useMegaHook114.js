...
const baseSeries114 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 114) / 5) * 12;
  const drift = Math.cos((idx + 114) / 9) * 8;
  const offset = ((idx * 114) % 17) - 8;
...
function smooth114(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize114(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags114(values, mode) {
  ...
  const risk = normalize114(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (114 % 9));
  return {
...
function fakeApi114(seed, mode, budget) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        ...
        return normalize114(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook114(initialSeed = 114) {
  ...
  useEffect(() => {
    ...
    fakeApi114(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries114.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize114(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth114(merged, state.window);
  ...
  const flags = useMemo(() => deriveFlags114(series, state.mode), [series, state.mode]);
