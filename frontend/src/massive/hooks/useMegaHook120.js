...
const baseSeries120 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 120) / 4) * 12;
  const drift = Math.cos((idx + 120) / 15) * 8;
  const offset = ((idx * 120) % 17) - 8;
...
function smooth120(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize120(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags120(values, mode) {
  ...
  const risk = normalize120(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (120 % 9));
  return {
...
function fakeApi120(seed, mode, budget) {
  return new Promise((resolve) => {
    const delay = 8 + (seed % 6) * 4 + (budget % 4) * 3;
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        const base = Math.sin((idx + seed) / 10) * 9 + Math.cos((idx + seed) / 4) * 6;
        const adjust = mode === 'safe' ? -5 : mode === 'aggressive' ? 7 : 1;
        return normalize120(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook120(initialSeed = 120) {
  ...
  useEffect(() => {
    ...
    fakeApi120(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries120.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize120(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth120(merged, state.window);
  ...
  const flags = useMemo(() => deriveFlags120(series, state.mode), [series, state.mode]);
