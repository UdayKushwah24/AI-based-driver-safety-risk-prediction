...
const baseSeries106 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 106) / 4) * 12;
  const drift = Math.cos((idx + 106) / 12) * 8;
  const offset = ((idx * 106) % 17) - 8;
...
function smooth106(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize106(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags106(values, mode) {
  ...
  const risk = normalize106(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (106 % 9));
  return {
...
function fakeApi106(seed, mode, budget) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        ...
        return normalize106(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook106(initialSeed = 106) {
  ...
  useEffect(() => {
    ...
    fakeApi106(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries106.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize106(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth106(merged, state.window);
  ...
  const flags = useMemo(() => deriveFlags106(series, state.mode), [series, state.mode]);
