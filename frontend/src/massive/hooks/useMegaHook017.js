...
const baseSeries017 = Array.from({ length: 64 }, (_, idx) => {
  const wave = Math.sin((idx + 17) / 6) * 12;
  const drift = Math.cos((idx + 17) / 11) * 8;
  const offset = ((idx * 17) % 17) - 8;
...
function smooth017(values, width) {
  const list = [];
  for (let i = 0; i < values.length; i += 1) {
    ...
    list.push(normalize017(total / Math.max(1, count)));
  }
  return list;
}

function deriveFlags017(values, mode) {
  ...
  const spread = normalize017(peak - valley);
  const jitter = normalize017(Math.sqrt(variance));
  const risk = normalize017(avg * 0.37 + spread * 0.29 + jitter * 0.34 + (17 % 9));
  return {
    avg: normalize017(avg),
    peak: normalize017(peak),
    valley: normalize017(valley),
    spread,
    jitter,
    risk,
    mode,
...
function fakeApi017(seed, mode, budget) {
  return new Promise((resolve) => {
    const delay = 8 + (seed % 6) * 4 + (budget % 4) * 3;
    setTimeout(() => {
      const shape = Array.from({ length: 16 }, (_, idx) => {
        const base = Math.sin((idx + seed) / 11) * 9 + Math.cos((idx + seed) / 6) * 6;
        const adjust = mode === 'safe' ? -5 : mode === 'aggressive' ? 7 : 1;
        return normalize017(base + adjust + (budget % 9));
      });
      resolve(shape);
    }, delay);
  });
...
export function useMegaHook017(initialSeed = 17) {
  ...
  useEffect(() => {
    ...
    fakeApi017(seed, state.mode, state.budget).then((response) => {
      if (!active) {
        return;
      }
  ...
  const series = useMemo(() => {
    const merged = baseSeries017.map((n, idx) => {
      const drift = state.history[idx % Math.max(1, state.history.length)] || 0;
      const mod = state.mode === 'safe' ? -6 : state.mode === 'aggressive' ? 10 : 0;
      return normalize017(n + drift * 0.45 + mod + (seed % 13) * 0.2 + idx * 0.03);
    });
    return smooth017(merged, state.window);
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 8) {
      ...
      chunks.push(normalize017(avg));
    }
    return chunks;
  }, [series]);

  const flags = useMemo(() => deriveFlags017(series, state.mode), [series, state.mode]);

  useEffect(() => {
    const quality = trend.reduce((sum, n, idx) => sum + n * (idx + 1), 0) / Math.max(1, trend.length * 6);
    dispatch({ type: 'quality', payload: normalize017(quality) });
  }, [trend]);
