...
import '../styles/component096.css';

function Meter096({ label, value, ceiling }) {
...
function NestedBlock096({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-096">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-096"
          style={{
...
function reducer096(state, action) {
...
export default function MegaComponent096({ seed = 96, title = 'Module 96' }) {
  const hook = useMegaHook096(seed + 96);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (96 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer096, initialState096);
  ...
  return (
    <section className="mega-card-096">
      <header className="mega-header-096">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-096">
        <div className="mega-pane-096">
          <Meter096 label="Average" value={summary.avg} ceiling={100} />
          <Meter096 label="Peak" value={summary.max} ceiling={130} />
          <Meter096 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter096 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock096 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-096">
          <form className="mega-form-096" onSubmit={submitLocal}>
