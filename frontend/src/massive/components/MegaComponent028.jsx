...
import '../styles/component028.css';

function Meter028({ label, value, ceiling }) {
...
function NestedBlock028({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-028">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-028"
          style={{
...
function reducer028(state, action) {
...
export default function MegaComponent028({ seed = 28, title = 'Module 28' }) {
  const hook = useMegaHook028(seed + 28);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (28 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer028, initialState028);
  ...
  return (
    <section className="mega-card-028">
      <header className="mega-header-028">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-028">
        <div className="mega-pane-028">
          <Meter028 label="Average" value={summary.avg} ceiling={100} />
          <Meter028 label="Peak" value={summary.max} ceiling={130} />
          <Meter028 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter028 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock028 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-028">
          <form className="mega-form-028" onSubmit={submitLocal}>
