...
import { useMegaHook111 } from '../hooks/useMegaHook111.js';
import { buildTableState111, pipelineData111 } from '../utils/transformSet111.js';
import '../styles/component231.css';

function Meter231({ label, value, ceiling }) {
...
function NestedBlock231({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-231">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-231"
          style={{
...
export default function MegaComponent231({ seed = 231, title = 'Module 231' }) {
  const hook = useMegaHook111(seed + 231);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (231 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer231, initialState231);

  const transformed = useMemo(() => pipelineData111(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState111(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-231">
      <header className="mega-header-231">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-231">
        <div className="mega-pane-231">
          <Meter231 label="Average" value={summary.avg} ceiling={100} />
          <Meter231 label="Peak" value={summary.max} ceiling={130} />
          <Meter231 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter231 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock231 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-231">
          <form className="mega-form-231" onSubmit={submitLocal}>
