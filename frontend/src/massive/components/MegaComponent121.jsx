...
function Meter121({ label, value, ceiling }) {
...
function NestedBlock121({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-121">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-121"
          style={{
...
export default function MegaComponent121({ seed = 121, title = 'Module 121' }) {
  const hook = useMegaHook001(seed + 121);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (121 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-121">
      <header className="mega-header-121">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-121">
        <div className="mega-pane-121">
          <Meter121 label="Average" value={summary.avg} ceiling={100} />
          <Meter121 label="Peak" value={summary.max} ceiling={130} />
          <Meter121 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter121 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock121 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-121">
          <form className="mega-form-121" onSubmit={submitLocal}>
