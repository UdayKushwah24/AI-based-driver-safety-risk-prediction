...
import { useMegaHook100 } from '../hooks/useMegaHook100.js';
import { buildTableState100, pipelineData100 } from '../utils/transformSet100.js';
import '../styles/component220.css';

function Meter220({ label, value, ceiling }) {
...
function NestedBlock220({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-220">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-220"
          style={{
...
export default function MegaComponent220({ seed = 220, title = 'Module 220' }) {
  const hook = useMegaHook100(seed + 220);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (220 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer220, initialState220);

  const transformed = useMemo(() => pipelineData100(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState100(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-220">
      <header className="mega-header-220">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-220">
        <div className="mega-pane-220">
          <Meter220 label="Average" value={summary.avg} ceiling={100} />
          <Meter220 label="Peak" value={summary.max} ceiling={130} />
          <Meter220 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter220 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock220 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-220">
          <form className="mega-form-220" onSubmit={submitLocal}>
