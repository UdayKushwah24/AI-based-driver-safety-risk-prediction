...
import '../styles/component099.css';

function Meter099({ label, value, ceiling }) {
...
function NestedBlock099({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-099">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-099"
          style={{
...
function reducer099(state, action) {
...
export default function MegaComponent099({ seed = 99, title = 'Module 99' }) {
  const hook = useMegaHook099(seed + 99);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (99 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer099, initialState099);
  ...
  return (
    <section className="mega-card-099">
      <header className="mega-header-099">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-099">
        <div className="mega-pane-099">
          <Meter099 label="Average" value={summary.avg} ceiling={100} />
          <Meter099 label="Peak" value={summary.max} ceiling={130} />
          <Meter099 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter099 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock099 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-099">
          <form className="mega-form-099" onSubmit={submitLocal}>
