...
import '../styles/component048.css';

function Meter048({ label, value, ceiling }) {
...
function NestedBlock048({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-048">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-048"
          style={{
...
function reducer048(state, action) {
...
export default function MegaComponent048({ seed = 48, title = 'Module 48' }) {
  const hook = useMegaHook048(seed + 48);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (48 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer048, initialState048);
  ...
  return (
    <section className="mega-card-048">
      <header className="mega-header-048">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-048">
        <div className="mega-pane-048">
          <Meter048 label="Average" value={summary.avg} ceiling={100} />
          <Meter048 label="Peak" value={summary.max} ceiling={130} />
          <Meter048 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter048 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock048 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-048">
          <form className="mega-form-048" onSubmit={submitLocal}>
