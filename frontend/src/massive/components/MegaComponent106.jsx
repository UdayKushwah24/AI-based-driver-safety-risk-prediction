...
import { useMegaHook106 } from '../hooks/useMegaHook106.js';
import { buildTableState106, pipelineData106 } from '../utils/transformSet106.js';
import '../styles/component106.css';

function Meter106({ label, value, ceiling }) {
...
function NestedBlock106({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-106">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-106"
          style={{
...
export default function MegaComponent106({ seed = 106, title = 'Module 106' }) {
  const hook = useMegaHook106(seed + 106);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (106 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer106, initialState106);

  const transformed = useMemo(() => pipelineData106(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState106(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-106">
      <header className="mega-header-106">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-106">
        <div className="mega-pane-106">
          <Meter106 label="Average" value={summary.avg} ceiling={100} />
          <Meter106 label="Peak" value={summary.max} ceiling={130} />
          <Meter106 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter106 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock106 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-106">
          <form className="mega-form-106" onSubmit={submitLocal}>
