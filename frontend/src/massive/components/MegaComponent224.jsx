...
import { useMegaHook104 } from '../hooks/useMegaHook104.js';
import { buildTableState104, pipelineData104 } from '../utils/transformSet104.js';
import '../styles/component224.css';

function Meter224({ label, value, ceiling }) {
...
function NestedBlock224({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-224">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-224"
          style={{
...
export default function MegaComponent224({ seed = 224, title = 'Module 224' }) {
  const hook = useMegaHook104(seed + 224);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (224 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer224, initialState224);

  const transformed = useMemo(() => pipelineData104(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState104(transformed, sortBy, sortDir, page, 12), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-224">
      <header className="mega-header-224">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-224">
        <div className="mega-pane-224">
          <Meter224 label="Average" value={summary.avg} ceiling={100} />
          <Meter224 label="Peak" value={summary.max} ceiling={130} />
          <Meter224 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter224 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock224 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-224">
          <form className="mega-form-224" onSubmit={submitLocal}>
