...
import '../styles/component031.css';

function Meter031({ label, value, ceiling }) {
...
function NestedBlock031({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-031">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-031"
          style={{
...
function reducer031(state, action) {
...
export default function MegaComponent031({ seed = 31, title = 'Module 31' }) {
  const hook = useMegaHook031(seed + 31);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (31 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer031, initialState031);
  ...
  return (
    <section className="mega-card-031">
      <header className="mega-header-031">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-031">
        <div className="mega-pane-031">
          <Meter031 label="Average" value={summary.avg} ceiling={100} />
          <Meter031 label="Peak" value={summary.max} ceiling={130} />
          <Meter031 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter031 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock031 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-031">
          <form className="mega-form-031" onSubmit={submitLocal}>
