...
import '../styles/component016.css';

function Meter016({ label, value, ceiling }) {
...
function NestedBlock016({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-016">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-016"
          style={{
...
function reducer016(state, action) {
...
export default function MegaComponent016({ seed = 16, title = 'Module 16' }) {
  const hook = useMegaHook016(seed + 16);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (16 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer016, initialState016);
  ...
  return (
    <section className="mega-card-016">
      <header className="mega-header-016">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-016">
        <div className="mega-pane-016">
          <Meter016 label="Average" value={summary.avg} ceiling={100} />
          <Meter016 label="Peak" value={summary.max} ceiling={130} />
          <Meter016 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter016 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock016 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-016">
          <form className="mega-form-016" onSubmit={submitLocal}>
