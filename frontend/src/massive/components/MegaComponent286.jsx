...
import { useMegaHook046 } from '../hooks/useMegaHook046.js';
import { buildTableState046, pipelineData046 } from '../utils/transformSet046.js';
import '../styles/component286.css';

function Meter286({ label, value, ceiling }) {
...
function NestedBlock286({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-286">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-286"
          style={{
...
export default function MegaComponent286({ seed = 286, title = 'Module 286' }) {
  const hook = useMegaHook046(seed + 286);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (286 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer286, initialState286);

  const transformed = useMemo(() => pipelineData046(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState046(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-286">
      <header className="mega-header-286">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-286">
        <div className="mega-pane-286">
          <Meter286 label="Average" value={summary.avg} ceiling={100} />
          <Meter286 label="Peak" value={summary.max} ceiling={130} />
          <Meter286 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter286 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock286 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-286">
          <form className="mega-form-286" onSubmit={submitLocal}>
