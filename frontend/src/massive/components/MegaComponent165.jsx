...
import { useMegaHook045 } from '../hooks/useMegaHook045.js';
import { buildTableState045, pipelineData045 } from '../utils/transformSet045.js';
import '../styles/component165.css';

function Meter165({ label, value, ceiling }) {
...
function NestedBlock165({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-165">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-165"
          style={{
...
export default function MegaComponent165({ seed = 165, title = 'Module 165' }) {
  const hook = useMegaHook045(seed + 165);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (165 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer165, initialState165);

  const transformed = useMemo(() => pipelineData045(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState045(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-165">
      <header className="mega-header-165">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-165">
        <div className="mega-pane-165">
          <Meter165 label="Average" value={summary.avg} ceiling={100} />
          <Meter165 label="Peak" value={summary.max} ceiling={130} />
          <Meter165 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter165 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock165 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-165">
          <form className="mega-form-165" onSubmit={submitLocal}>
