...
function Meter129({ label, value, ceiling }) {
...
function NestedBlock129({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-129">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-129"
          style={{
...
export default function MegaComponent129({ seed = 129, title = 'Module 129' }) {
  const hook = useMegaHook009(seed + 129);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (129 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-129">
      <header className="mega-header-129">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-129">
        <div className="mega-pane-129">
          <Meter129 label="Average" value={summary.avg} ceiling={100} />
          <Meter129 label="Peak" value={summary.max} ceiling={130} />
          <Meter129 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter129 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock129 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-129">
          <form className="mega-form-129" onSubmit={submitLocal}>
