...
import '../styles/component008.css';

function Meter008({ label, value, ceiling }) {
...
function NestedBlock008({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-008">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-008"
          style={{
...
function reducer008(state, action) {
...
export default function MegaComponent008({ seed = 8, title = 'Module 8' }) {
  const hook = useMegaHook008(seed + 8);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (8 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer008, initialState008);
  ...
  return (
    <section className="mega-card-008">
      <header className="mega-header-008">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-008">
        <div className="mega-pane-008">
          <Meter008 label="Average" value={summary.avg} ceiling={100} />
          <Meter008 label="Peak" value={summary.max} ceiling={130} />
          <Meter008 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter008 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock008 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-008">
          <form className="mega-form-008" onSubmit={submitLocal}>
