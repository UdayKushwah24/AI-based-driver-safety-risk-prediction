...
import '../styles/component037.css';

function Meter037({ label, value, ceiling }) {
...
function NestedBlock037({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-037">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-037"
          style={{
...
function reducer037(state, action) {
...
export default function MegaComponent037({ seed = 37, title = 'Module 37' }) {
  const hook = useMegaHook037(seed + 37);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (37 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer037, initialState037);
  ...
  return (
    <section className="mega-card-037">
      <header className="mega-header-037">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-037">
        <div className="mega-pane-037">
          <Meter037 label="Average" value={summary.avg} ceiling={100} />
          <Meter037 label="Peak" value={summary.max} ceiling={130} />
          <Meter037 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter037 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock037 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-037">
          <form className="mega-form-037" onSubmit={submitLocal}>
