...
function Meter136({ label, value, ceiling }) {
...
function NestedBlock136({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-136">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-136"
          style={{
...
export default function MegaComponent136({ seed = 136, title = 'Module 136' }) {
  const hook = useMegaHook016(seed + 136);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (136 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-136">
      <header className="mega-header-136">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-136">
        <div className="mega-pane-136">
          <Meter136 label="Average" value={summary.avg} ceiling={100} />
          <Meter136 label="Peak" value={summary.max} ceiling={130} />
          <Meter136 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter136 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock136 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-136">
          <form className="mega-form-136" onSubmit={submitLocal}>
