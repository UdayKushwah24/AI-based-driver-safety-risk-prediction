...
import '../styles/component053.css';

function Meter053({ label, value, ceiling }) {
...
function NestedBlock053({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-053">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-053"
          style={{
...
function reducer053(state, action) {
...
export default function MegaComponent053({ seed = 53, title = 'Module 53' }) {
  const hook = useMegaHook053(seed + 53);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (53 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer053, initialState053);
  ...
  return (
    <section className="mega-card-053">
      <header className="mega-header-053">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-053">
        <div className="mega-pane-053">
          <Meter053 label="Average" value={summary.avg} ceiling={100} />
          <Meter053 label="Peak" value={summary.max} ceiling={130} />
          <Meter053 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter053 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock053 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-053">
          <form className="mega-form-053" onSubmit={submitLocal}>
