...
function Meter114({ label, value, ceiling }) {
...
function NestedBlock114({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-114">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-114"
          style={{
...
export default function MegaComponent114({ seed = 114, title = 'Module 114' }) {
  const hook = useMegaHook114(seed + 114);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (114 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-114">
      <header className="mega-header-114">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-114">
        <div className="mega-pane-114">
          <Meter114 label="Average" value={summary.avg} ceiling={100} />
          <Meter114 label="Peak" value={summary.max} ceiling={130} />
          <Meter114 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter114 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock114 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-114">
          <form className="mega-form-114" onSubmit={submitLocal}>
