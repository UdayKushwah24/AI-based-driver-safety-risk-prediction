...
function Meter255({ label, value, ceiling }) {
...
function NestedBlock255({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-255">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-255"
          style={{
...
export default function MegaComponent255({ seed = 255, title = 'Module 255' }) {
  const hook = useMegaHook015(seed + 255);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (255 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-255">
      <header className="mega-header-255">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-255">
        <div className="mega-pane-255">
          <Meter255 label="Average" value={summary.avg} ceiling={100} />
          <Meter255 label="Peak" value={summary.max} ceiling={130} />
          <Meter255 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter255 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock255 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-255">
          <form className="mega-form-255" onSubmit={submitLocal}>
