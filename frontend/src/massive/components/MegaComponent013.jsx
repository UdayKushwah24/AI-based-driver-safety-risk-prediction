...
import '../styles/component013.css';

function Meter013({ label, value, ceiling }) {
...
function NestedBlock013({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-013">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-013"
          style={{
...
export default function MegaComponent013({ seed = 13, title = 'Module 13' }) {
  const hook = useMegaHook013(seed + 13);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (13 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-013">
      <header className="mega-header-013">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-013">
        <div className="mega-pane-013">
          <Meter013 label="Average" value={summary.avg} ceiling={100} />
          <Meter013 label="Peak" value={summary.max} ceiling={130} />
          <Meter013 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter013 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock013 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-013">
          <form className="mega-form-013" onSubmit={submitLocal}>
