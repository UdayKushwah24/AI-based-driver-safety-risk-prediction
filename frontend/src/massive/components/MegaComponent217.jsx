...
import { useMegaHook097 } from '../hooks/useMegaHook097.js';
import { buildTableState097, pipelineData097 } from '../utils/transformSet097.js';
import '../styles/component217.css';

function Meter217({ label, value, ceiling }) {
...
function NestedBlock217({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-217">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-217"
          style={{
...
export default function MegaComponent217({ seed = 217, title = 'Module 217' }) {
  const hook = useMegaHook097(seed + 217);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (217 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer217, initialState217);

  const transformed = useMemo(() => pipelineData097(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState097(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-217">
      <header className="mega-header-217">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-217">
        <div className="mega-pane-217">
          <Meter217 label="Average" value={summary.avg} ceiling={100} />
          <Meter217 label="Peak" value={summary.max} ceiling={130} />
          <Meter217 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter217 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock217 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-217">
          <form className="mega-form-217" onSubmit={submitLocal}>
