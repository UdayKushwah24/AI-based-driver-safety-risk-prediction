...
import '../styles/component041.css';

function Meter041({ label, value, ceiling }) {
...
function NestedBlock041({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-041">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-041"
          style={{
...
export default function MegaComponent041({ seed = 41, title = 'Module 41' }) {
  const hook = useMegaHook041(seed + 41);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (41 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-041">
      <header className="mega-header-041">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-041">
        <div className="mega-pane-041">
          <Meter041 label="Average" value={summary.avg} ceiling={100} />
          <Meter041 label="Peak" value={summary.max} ceiling={130} />
          <Meter041 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter041 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock041 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-041">
          <form className="mega-form-041" onSubmit={submitLocal}>
