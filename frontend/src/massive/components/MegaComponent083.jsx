...
import '../styles/component083.css';

function Meter083({ label, value, ceiling }) {
...
function NestedBlock083({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-083">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-083"
          style={{
...
function reducer083(state, action) {
...
export default function MegaComponent083({ seed = 83, title = 'Module 83' }) {
  const hook = useMegaHook083(seed + 83);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (83 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer083, initialState083);
  ...
  return (
    <section className="mega-card-083">
      <header className="mega-header-083">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-083">
        <div className="mega-pane-083">
          <Meter083 label="Average" value={summary.avg} ceiling={100} />
          <Meter083 label="Peak" value={summary.max} ceiling={130} />
          <Meter083 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter083 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock083 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-083">
          <form className="mega-form-083" onSubmit={submitLocal}>
