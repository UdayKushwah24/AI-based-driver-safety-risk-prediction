...
import { useMegaHook081 } from '../hooks/useMegaHook081.js';
import { buildTableState081, pipelineData081 } from '../utils/transformSet081.js';
import '../styles/component201.css';

function Meter201({ label, value, ceiling }) {
...
function NestedBlock201({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-201">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-201"
          style={{
...
export default function MegaComponent201({ seed = 201, title = 'Module 201' }) {
  const hook = useMegaHook081(seed + 201);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (201 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer201, initialState201);

  const transformed = useMemo(() => pipelineData081(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState081(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-201">
      <header className="mega-header-201">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-201">
        <div className="mega-pane-201">
          <Meter201 label="Average" value={summary.avg} ceiling={100} />
          <Meter201 label="Peak" value={summary.max} ceiling={130} />
          <Meter201 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter201 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock201 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-201">
          <form className="mega-form-201" onSubmit={submitLocal}>
