...
import '../styles/component091.css';

function Meter091({ label, value, ceiling }) {
...
function NestedBlock091({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-091">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-091"
          style={{
...
function reducer091(state, action) {
...
export default function MegaComponent091({ seed = 91, title = 'Module 91' }) {
  const hook = useMegaHook091(seed + 91);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (91 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer091, initialState091);
  ...
  return (
    <section className="mega-card-091">
      <header className="mega-header-091">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-091">
        <div className="mega-pane-091">
          <Meter091 label="Average" value={summary.avg} ceiling={100} />
          <Meter091 label="Peak" value={summary.max} ceiling={130} />
          <Meter091 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter091 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock091 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-091">
          <form className="mega-form-091" onSubmit={submitLocal}>
