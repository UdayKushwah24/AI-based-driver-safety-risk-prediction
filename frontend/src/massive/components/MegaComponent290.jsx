...
import { useMegaHook050 } from '../hooks/useMegaHook050.js';
import { buildTableState050, pipelineData050 } from '../utils/transformSet050.js';
import '../styles/component290.css';

function Meter290({ label, value, ceiling }) {
...
function NestedBlock290({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-290">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-290"
          style={{
...
export default function MegaComponent290({ seed = 290, title = 'Module 290' }) {
  const hook = useMegaHook050(seed + 290);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (290 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer290, initialState290);

  const transformed = useMemo(() => pipelineData050(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState050(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-290">
      <header className="mega-header-290">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-290">
        <div className="mega-pane-290">
          <Meter290 label="Average" value={summary.avg} ceiling={100} />
          <Meter290 label="Peak" value={summary.max} ceiling={130} />
          <Meter290 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter290 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock290 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-290">
          <form className="mega-form-290" onSubmit={submitLocal}>
