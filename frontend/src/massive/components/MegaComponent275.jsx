...
function Meter275({ label, value, ceiling }) {
...
function NestedBlock275({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-275">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-275"
          style={{
...
export default function MegaComponent275({ seed = 275, title = 'Module 275' }) {
  const hook = useMegaHook035(seed + 275);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (275 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-275">
      <header className="mega-header-275">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-275">
        <div className="mega-pane-275">
          <Meter275 label="Average" value={summary.avg} ceiling={100} />
          <Meter275 label="Peak" value={summary.max} ceiling={130} />
          <Meter275 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter275 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock275 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-275">
          <form className="mega-form-275" onSubmit={submitLocal}>
