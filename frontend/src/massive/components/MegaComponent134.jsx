...
import { useMegaHook014 } from '../hooks/useMegaHook014.js';
import { buildTableState014, pipelineData014 } from '../utils/transformSet014.js';
import '../styles/component134.css';

function Meter134({ label, value, ceiling }) {
...
function NestedBlock134({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-134">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-134"
          style={{
...
export default function MegaComponent134({ seed = 134, title = 'Module 134' }) {
  const hook = useMegaHook014(seed + 134);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (134 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer134, initialState134);

  const transformed = useMemo(() => pipelineData014(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState014(transformed, sortBy, sortDir, page, 12), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-134">
      <header className="mega-header-134">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-134">
        <div className="mega-pane-134">
          <Meter134 label="Average" value={summary.avg} ceiling={100} />
          <Meter134 label="Peak" value={summary.max} ceiling={130} />
          <Meter134 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter134 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock134 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-134">
          <form className="mega-form-134" onSubmit={submitLocal}>
