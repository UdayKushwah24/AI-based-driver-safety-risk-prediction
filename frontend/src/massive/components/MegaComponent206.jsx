...
import { useMegaHook086 } from '../hooks/useMegaHook086.js';
import { buildTableState086, pipelineData086 } from '../utils/transformSet086.js';
import '../styles/component206.css';

function Meter206({ label, value, ceiling }) {
...
function NestedBlock206({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-206">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-206"
          style={{
...
export default function MegaComponent206({ seed = 206, title = 'Module 206' }) {
  const hook = useMegaHook086(seed + 206);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (206 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer206, initialState206);

  const transformed = useMemo(() => pipelineData086(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState086(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-206">
      <header className="mega-header-206">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-206">
        <div className="mega-pane-206">
          <Meter206 label="Average" value={summary.avg} ceiling={100} />
          <Meter206 label="Peak" value={summary.max} ceiling={130} />
          <Meter206 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter206 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock206 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-206">
          <form className="mega-form-206" onSubmit={submitLocal}>
