...
function Meter173({ label, value, ceiling }) {
...
function NestedBlock173({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-173">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-173"
          style={{
...
export default function MegaComponent173({ seed = 173, title = 'Module 173' }) {
  const hook = useMegaHook053(seed + 173);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (173 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-173">
      <header className="mega-header-173">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-173">
        <div className="mega-pane-173">
          <Meter173 label="Average" value={summary.avg} ceiling={100} />
          <Meter173 label="Peak" value={summary.max} ceiling={130} />
          <Meter173 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter173 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock173 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-173">
          <form className="mega-form-173" onSubmit={submitLocal}>
