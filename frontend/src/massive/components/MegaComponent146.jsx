...
function Meter146({ label, value, ceiling }) {
...
function NestedBlock146({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-146">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-146"
          style={{
...
export default function MegaComponent146({ seed = 146, title = 'Module 146' }) {
  const hook = useMegaHook026(seed + 146);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (146 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-146">
      <header className="mega-header-146">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-146">
        <div className="mega-pane-146">
          <Meter146 label="Average" value={summary.avg} ceiling={100} />
          <Meter146 label="Peak" value={summary.max} ceiling={130} />
          <Meter146 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter146 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock146 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-146">
          <form className="mega-form-146" onSubmit={submitLocal}>
