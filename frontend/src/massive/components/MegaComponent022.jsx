...
import '../styles/component022.css';

function Meter022({ label, value, ceiling }) {
...
function NestedBlock022({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-022">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-022"
          style={{
...
function reducer022(state, action) {
...
export default function MegaComponent022({ seed = 22, title = 'Module 22' }) {
  const hook = useMegaHook022(seed + 22);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (22 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer022, initialState022);
  ...
  return (
    <section className="mega-card-022">
      <header className="mega-header-022">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-022">
        <div className="mega-pane-022">
          <Meter022 label="Average" value={summary.avg} ceiling={100} />
          <Meter022 label="Peak" value={summary.max} ceiling={130} />
          <Meter022 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter022 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock022 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-022">
          <form className="mega-form-022" onSubmit={submitLocal}>
