...
import '../styles/component015.css';

function Meter015({ label, value, ceiling }) {
...
function NestedBlock015({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-015">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-015"
          style={{
...
function reducer015(state, action) {
...
export default function MegaComponent015({ seed = 15, title = 'Module 15' }) {
  const hook = useMegaHook015(seed + 15);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (15 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer015, initialState015);
  ...
  return (
    <section className="mega-card-015">
      <header className="mega-header-015">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-015">
        <div className="mega-pane-015">
          <Meter015 label="Average" value={summary.avg} ceiling={100} />
          <Meter015 label="Peak" value={summary.max} ceiling={130} />
          <Meter015 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter015 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock015 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-015">
          <form className="mega-form-015" onSubmit={submitLocal}>
