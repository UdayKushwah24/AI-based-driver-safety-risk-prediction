...
import '../styles/component058.css';

function Meter058({ label, value, ceiling }) {
...
function NestedBlock058({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-058">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-058"
          style={{
...
function reducer058(state, action) {
...
export default function MegaComponent058({ seed = 58, title = 'Module 58' }) {
  const hook = useMegaHook058(seed + 58);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (58 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer058, initialState058);
  ...
  return (
    <section className="mega-card-058">
      <header className="mega-header-058">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-058">
        <div className="mega-pane-058">
          <Meter058 label="Average" value={summary.avg} ceiling={100} />
          <Meter058 label="Peak" value={summary.max} ceiling={130} />
          <Meter058 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter058 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock058 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-058">
          <form className="mega-form-058" onSubmit={submitLocal}>
