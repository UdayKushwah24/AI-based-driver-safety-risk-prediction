...
function Meter130({ label, value, ceiling }) {
...
function NestedBlock130({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-130">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-130"
          style={{
...
export default function MegaComponent130({ seed = 130, title = 'Module 130' }) {
  const hook = useMegaHook010(seed + 130);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (130 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-130">
      <header className="mega-header-130">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-130">
        <div className="mega-pane-130">
          <Meter130 label="Average" value={summary.avg} ceiling={100} />
          <Meter130 label="Peak" value={summary.max} ceiling={130} />
          <Meter130 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter130 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock130 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-130">
          <form className="mega-form-130" onSubmit={submitLocal}>
