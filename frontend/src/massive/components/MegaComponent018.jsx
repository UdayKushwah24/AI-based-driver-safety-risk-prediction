...
import '../styles/component018.css';

function Meter018({ label, value, ceiling }) {
...
function NestedBlock018({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-018">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-018"
          style={{
...
export default function MegaComponent018({ seed = 18, title = 'Module 18' }) {
  const hook = useMegaHook018(seed + 18);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (18 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-018">
      <header className="mega-header-018">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-018">
        <div className="mega-pane-018">
          <Meter018 label="Average" value={summary.avg} ceiling={100} />
          <Meter018 label="Peak" value={summary.max} ceiling={130} />
          <Meter018 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter018 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock018 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-018">
          <form className="mega-form-018" onSubmit={submitLocal}>
