...
import '../styles/component075.css';

function Meter075({ label, value, ceiling }) {
...
function NestedBlock075({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-075">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-075"
          style={{
...
function reducer075(state, action) {
...
export default function MegaComponent075({ seed = 75, title = 'Module 75' }) {
  const hook = useMegaHook075(seed + 75);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (75 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer075, initialState075);
  ...
  return (
    <section className="mega-card-075">
      <header className="mega-header-075">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-075">
        <div className="mega-pane-075">
          <Meter075 label="Average" value={summary.avg} ceiling={100} />
          <Meter075 label="Peak" value={summary.max} ceiling={130} />
          <Meter075 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter075 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock075 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-075">
          <form className="mega-form-075" onSubmit={submitLocal}>
