...
import { useMegaHook001 } from '../hooks/useMegaHook001.js';
import { buildTableState001, pipelineData001 } from '../utils/transformSet001.js';
import '../styles/component241.css';

function Meter241({ label, value, ceiling }) {
...
function NestedBlock241({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-241">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-241"
          style={{
...
export default function MegaComponent241({ seed = 241, title = 'Module 241' }) {
  const hook = useMegaHook001(seed + 241);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (241 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer241, initialState241);

  const transformed = useMemo(() => pipelineData001(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState001(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-241">
      <header className="mega-header-241">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-241">
        <div className="mega-pane-241">
          <Meter241 label="Average" value={summary.avg} ceiling={100} />
          <Meter241 label="Peak" value={summary.max} ceiling={130} />
          <Meter241 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter241 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock241 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-241">
          <form className="mega-form-241" onSubmit={submitLocal}>
