...
import { useMegaHook002 } from '../hooks/useMegaHook002.js';
import { buildTableState002, pipelineData002 } from '../utils/transformSet002.js';
import '../styles/component242.css';

function Meter242({ label, value, ceiling }) {
...
function NestedBlock242({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-242">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-242"
          style={{
...
export default function MegaComponent242({ seed = 242, title = 'Module 242' }) {
  const hook = useMegaHook002(seed + 242);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (242 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer242, initialState242);

  const transformed = useMemo(() => pipelineData002(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState002(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-242">
      <header className="mega-header-242">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-242">
        <div className="mega-pane-242">
          <Meter242 label="Average" value={summary.avg} ceiling={100} />
          <Meter242 label="Peak" value={summary.max} ceiling={130} />
          <Meter242 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter242 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock242 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-242">
          <form className="mega-form-242" onSubmit={submitLocal}>
