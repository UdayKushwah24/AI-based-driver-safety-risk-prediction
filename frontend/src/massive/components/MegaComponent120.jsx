...
function Meter120({ label, value, ceiling }) {
...
function NestedBlock120({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-120">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-120"
          style={{
...
export default function MegaComponent120({ seed = 120, title = 'Module 120' }) {
  const hook = useMegaHook120(seed + 120);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (120 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-120">
      <header className="mega-header-120">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-120">
        <div className="mega-pane-120">
          <Meter120 label="Average" value={summary.avg} ceiling={100} />
          <Meter120 label="Peak" value={summary.max} ceiling={130} />
          <Meter120 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter120 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock120 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-120">
          <form className="mega-form-120" onSubmit={submitLocal}>
