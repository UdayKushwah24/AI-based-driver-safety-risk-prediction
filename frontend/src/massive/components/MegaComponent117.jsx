...
function Meter117({ label, value, ceiling }) {
...
function NestedBlock117({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-117">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-117"
          style={{
...
export default function MegaComponent117({ seed = 117, title = 'Module 117' }) {
  const hook = useMegaHook117(seed + 117);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (117 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-117">
      <header className="mega-header-117">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-117">
        <div className="mega-pane-117">
          <Meter117 label="Average" value={summary.avg} ceiling={100} />
          <Meter117 label="Peak" value={summary.max} ceiling={130} />
          <Meter117 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter117 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock117 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-117">
          <form className="mega-form-117" onSubmit={submitLocal}>
