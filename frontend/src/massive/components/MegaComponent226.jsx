...
function Meter226({ label, value, ceiling }) {
...
function NestedBlock226({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-226">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-226"
          style={{
...
export default function MegaComponent226({ seed = 226, title = 'Module 226' }) {
  const hook = useMegaHook106(seed + 226);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (226 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-226">
      <header className="mega-header-226">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-226">
        <div className="mega-pane-226">
          <Meter226 label="Average" value={summary.avg} ceiling={100} />
          <Meter226 label="Peak" value={summary.max} ceiling={130} />
          <Meter226 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter226 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock226 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-226">
          <form className="mega-form-226" onSubmit={submitLocal}>
