...
import '../styles/component004.css';

function Meter004({ label, value, ceiling }) {
...
function NestedBlock004({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-004">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-004"
          style={{
...
function reducer004(state, action) {
...
export default function MegaComponent004({ seed = 4, title = 'Module 4' }) {
  const hook = useMegaHook004(seed + 4);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (4 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer004, initialState004);
  ...
  return (
    <section className="mega-card-004">
      <header className="mega-header-004">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-004">
        <div className="mega-pane-004">
          <Meter004 label="Average" value={summary.avg} ceiling={100} />
          <Meter004 label="Peak" value={summary.max} ceiling={130} />
          <Meter004 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter004 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock004 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-004">
          <form className="mega-form-004" onSubmit={submitLocal}>
