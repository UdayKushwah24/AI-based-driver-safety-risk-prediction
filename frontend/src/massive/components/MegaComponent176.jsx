...
import { useMegaHook056 } from '../hooks/useMegaHook056.js';
import { buildTableState056, pipelineData056 } from '../utils/transformSet056.js';
import '../styles/component176.css';

function Meter176({ label, value, ceiling }) {
...
function NestedBlock176({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-176">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-176"
          style={{
...
export default function MegaComponent176({ seed = 176, title = 'Module 176' }) {
  const hook = useMegaHook056(seed + 176);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (176 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer176, initialState176);

  const transformed = useMemo(() => pipelineData056(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState056(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-176">
      <header className="mega-header-176">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-176">
        <div className="mega-pane-176">
          <Meter176 label="Average" value={summary.avg} ceiling={100} />
          <Meter176 label="Peak" value={summary.max} ceiling={130} />
          <Meter176 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter176 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock176 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-176">
          <form className="mega-form-176" onSubmit={submitLocal}>
