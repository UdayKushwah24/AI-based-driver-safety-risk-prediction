...
import '../styles/component011.css';

function Meter011({ label, value, ceiling }) {
...
function NestedBlock011({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-011">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-011"
          style={{
...
function reducer011(state, action) {
...
export default function MegaComponent011({ seed = 11, title = 'Module 11' }) {
  const hook = useMegaHook011(seed + 11);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (11 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer011, initialState011);
  ...
  return (
    <section className="mega-card-011">
      <header className="mega-header-011">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-011">
        <div className="mega-pane-011">
          <Meter011 label="Average" value={summary.avg} ceiling={100} />
          <Meter011 label="Peak" value={summary.max} ceiling={130} />
          <Meter011 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter011 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock011 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-011">
          <form className="mega-form-011" onSubmit={submitLocal}>
