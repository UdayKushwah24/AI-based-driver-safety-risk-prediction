...
import { useMegaHook088 } from '../hooks/useMegaHook088.js';
import { buildTableState088, pipelineData088 } from '../utils/transformSet088.js';
import '../styles/component208.css';

function Meter208({ label, value, ceiling }) {
...
function NestedBlock208({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-208">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-208"
          style={{
...
export default function MegaComponent208({ seed = 208, title = 'Module 208' }) {
  const hook = useMegaHook088(seed + 208);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (208 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer208, initialState208);

  const transformed = useMemo(() => pipelineData088(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState088(transformed, sortBy, sortDir, page, 11), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-208">
      <header className="mega-header-208">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-208">
        <div className="mega-pane-208">
          <Meter208 label="Average" value={summary.avg} ceiling={100} />
          <Meter208 label="Peak" value={summary.max} ceiling={130} />
          <Meter208 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter208 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock208 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-208">
          <form className="mega-form-208" onSubmit={submitLocal}>
