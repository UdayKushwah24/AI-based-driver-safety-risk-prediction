...
import { useMegaHook112 } from '../hooks/useMegaHook112.js';
import { buildTableState112, pipelineData112 } from '../utils/transformSet112.js';
import '../styles/component232.css';

function Meter232({ label, value, ceiling }) {
...
function NestedBlock232({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-232">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-232"
          style={{
...
export default function MegaComponent232({ seed = 232, title = 'Module 232' }) {
  const hook = useMegaHook112(seed + 232);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (232 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer232, initialState232);

  const transformed = useMemo(() => pipelineData112(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState112(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-232">
      <header className="mega-header-232">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-232">
        <div className="mega-pane-232">
          <Meter232 label="Average" value={summary.avg} ceiling={100} />
          <Meter232 label="Peak" value={summary.max} ceiling={130} />
          <Meter232 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter232 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock232 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-232">
          <form className="mega-form-232" onSubmit={submitLocal}>
