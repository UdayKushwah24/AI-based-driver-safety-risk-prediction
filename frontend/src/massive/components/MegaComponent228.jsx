...
import { useMegaHook108 } from '../hooks/useMegaHook108.js';
import { buildTableState108, pipelineData108 } from '../utils/transformSet108.js';
import '../styles/component228.css';

function Meter228({ label, value, ceiling }) {
...
function NestedBlock228({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-228">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-228"
          style={{
...
export default function MegaComponent228({ seed = 228, title = 'Module 228' }) {
  const hook = useMegaHook108(seed + 228);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (228 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer228, initialState228);

  const transformed = useMemo(() => pipelineData108(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState108(transformed, sortBy, sortDir, page, 11), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-228">
      <header className="mega-header-228">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-228">
        <div className="mega-pane-228">
          <Meter228 label="Average" value={summary.avg} ceiling={100} />
          <Meter228 label="Peak" value={summary.max} ceiling={130} />
          <Meter228 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter228 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock228 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-228">
          <form className="mega-form-228" onSubmit={submitLocal}>
