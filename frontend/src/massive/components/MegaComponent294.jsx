...
import { useMegaHook054 } from '../hooks/useMegaHook054.js';
import { buildTableState054, pipelineData054 } from '../utils/transformSet054.js';
import '../styles/component294.css';

function Meter294({ label, value, ceiling }) {
...
function NestedBlock294({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-294">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-294"
          style={{
...
export default function MegaComponent294({ seed = 294, title = 'Module 294' }) {
  const hook = useMegaHook054(seed + 294);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (294 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer294, initialState294);

  const transformed = useMemo(() => pipelineData054(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState054(transformed, sortBy, sortDir, page, 12), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-294">
      <header className="mega-header-294">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-294">
        <div className="mega-pane-294">
          <Meter294 label="Average" value={summary.avg} ceiling={100} />
          <Meter294 label="Peak" value={summary.max} ceiling={130} />
          <Meter294 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter294 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock294 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-294">
          <form className="mega-form-294" onSubmit={submitLocal}>
