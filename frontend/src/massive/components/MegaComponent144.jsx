...
function Meter144({ label, value, ceiling }) {
...
function NestedBlock144({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-144">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-144"
          style={{
...
export default function MegaComponent144({ seed = 144, title = 'Module 144' }) {
  const hook = useMegaHook024(seed + 144);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (144 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-144">
      <header className="mega-header-144">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-144">
        <div className="mega-pane-144">
          <Meter144 label="Average" value={summary.avg} ceiling={100} />
          <Meter144 label="Peak" value={summary.max} ceiling={130} />
          <Meter144 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter144 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock144 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-144">
          <form className="mega-form-144" onSubmit={submitLocal}>
