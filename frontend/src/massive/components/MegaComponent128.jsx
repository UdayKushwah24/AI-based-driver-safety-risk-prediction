...
function Meter128({ label, value, ceiling }) {
...
function NestedBlock128({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-128">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-128"
          style={{
...
export default function MegaComponent128({ seed = 128, title = 'Module 128' }) {
  const hook = useMegaHook008(seed + 128);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (128 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-128">
      <header className="mega-header-128">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-128">
        <div className="mega-pane-128">
          <Meter128 label="Average" value={summary.avg} ceiling={100} />
          <Meter128 label="Peak" value={summary.max} ceiling={130} />
          <Meter128 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter128 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock128 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-128">
          <form className="mega-form-128" onSubmit={submitLocal}>
