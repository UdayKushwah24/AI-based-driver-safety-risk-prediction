...
import { useMegaHook012 } from '../hooks/useMegaHook012.js';
import { buildTableState012, pipelineData012 } from '../utils/transformSet012.js';
import '../styles/component252.css';

function Meter252({ label, value, ceiling }) {
...
function NestedBlock252({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-252">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-252"
          style={{
...
export default function MegaComponent252({ seed = 252, title = 'Module 252' }) {
  const hook = useMegaHook012(seed + 252);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (252 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer252, initialState252);

  const transformed = useMemo(() => pipelineData012(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState012(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-252">
      <header className="mega-header-252">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-252">
        <div className="mega-pane-252">
          <Meter252 label="Average" value={summary.avg} ceiling={100} />
          <Meter252 label="Peak" value={summary.max} ceiling={130} />
          <Meter252 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter252 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock252 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-252">
          <form className="mega-form-252" onSubmit={submitLocal}>
