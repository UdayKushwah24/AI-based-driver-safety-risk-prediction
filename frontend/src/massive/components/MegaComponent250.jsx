...
import { useMegaHook010 } from '../hooks/useMegaHook010.js';
import { buildTableState010, pipelineData010 } from '../utils/transformSet010.js';
import '../styles/component250.css';

function Meter250({ label, value, ceiling }) {
...
function NestedBlock250({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-250">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-250"
          style={{
...
export default function MegaComponent250({ seed = 250, title = 'Module 250' }) {
  const hook = useMegaHook010(seed + 250);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (250 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer250, initialState250);

  const transformed = useMemo(() => pipelineData010(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState010(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-250">
      <header className="mega-header-250">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-250">
        <div className="mega-pane-250">
          <Meter250 label="Average" value={summary.avg} ceiling={100} />
          <Meter250 label="Peak" value={summary.max} ceiling={130} />
          <Meter250 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter250 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock250 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-250">
          <form className="mega-form-250" onSubmit={submitLocal}>
