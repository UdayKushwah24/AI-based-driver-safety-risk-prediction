...
function Meter280({ label, value, ceiling }) {
...
function NestedBlock280({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-280">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-280"
          style={{
...
export default function MegaComponent280({ seed = 280, title = 'Module 280' }) {
  const hook = useMegaHook040(seed + 280);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (280 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-280">
      <header className="mega-header-280">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-280">
        <div className="mega-pane-280">
          <Meter280 label="Average" value={summary.avg} ceiling={100} />
          <Meter280 label="Peak" value={summary.max} ceiling={130} />
          <Meter280 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter280 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock280 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-280">
          <form className="mega-form-280" onSubmit={submitLocal}>
