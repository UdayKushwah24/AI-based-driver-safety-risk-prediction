...
import { useMegaHook083 } from '../hooks/useMegaHook083.js';
import { buildTableState083, pipelineData083 } from '../utils/transformSet083.js';
import '../styles/component203.css';

function Meter203({ label, value, ceiling }) {
...
function NestedBlock203({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-203">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-203"
          style={{
...
export default function MegaComponent203({ seed = 203, title = 'Module 203' }) {
  const hook = useMegaHook083(seed + 203);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (203 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer203, initialState203);

  const transformed = useMemo(() => pipelineData083(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState083(transformed, sortBy, sortDir, page, 11), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-203">
      <header className="mega-header-203">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-203">
        <div className="mega-pane-203">
          <Meter203 label="Average" value={summary.avg} ceiling={100} />
          <Meter203 label="Peak" value={summary.max} ceiling={130} />
          <Meter203 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter203 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock203 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-203">
          <form className="mega-form-203" onSubmit={submitLocal}>
