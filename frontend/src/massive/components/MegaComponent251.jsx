...
import { useMegaHook011 } from '../hooks/useMegaHook011.js';
import { buildTableState011, pipelineData011 } from '../utils/transformSet011.js';
import '../styles/component251.css';

function Meter251({ label, value, ceiling }) {
...
function NestedBlock251({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-251">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-251"
          style={{
...
export default function MegaComponent251({ seed = 251, title = 'Module 251' }) {
  const hook = useMegaHook011(seed + 251);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (251 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer251, initialState251);

  const transformed = useMemo(() => pipelineData011(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState011(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-251">
      <header className="mega-header-251">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-251">
        <div className="mega-pane-251">
          <Meter251 label="Average" value={summary.avg} ceiling={100} />
          <Meter251 label="Peak" value={summary.max} ceiling={130} />
          <Meter251 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter251 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock251 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-251">
          <form className="mega-form-251" onSubmit={submitLocal}>
