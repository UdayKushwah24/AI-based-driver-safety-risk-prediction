...
import '../styles/component024.css';

function Meter024({ label, value, ceiling }) {
...
function NestedBlock024({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-024">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-024"
          style={{
...
function reducer024(state, action) {
...
export default function MegaComponent024({ seed = 24, title = 'Module 24' }) {
  const hook = useMegaHook024(seed + 24);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (24 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer024, initialState024);
  ...
  return (
    <section className="mega-card-024">
      <header className="mega-header-024">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-024">
        <div className="mega-pane-024">
          <Meter024 label="Average" value={summary.avg} ceiling={100} />
          <Meter024 label="Peak" value={summary.max} ceiling={130} />
          <Meter024 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter024 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock024 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-024">
          <form className="mega-form-024" onSubmit={submitLocal}>
