...
import { useMegaHook076 } from '../hooks/useMegaHook076.js';
import { buildTableState076, pipelineData076 } from '../utils/transformSet076.js';
import '../styles/component196.css';

function Meter196({ label, value, ceiling }) {
...
function NestedBlock196({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-196">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-196"
          style={{
...
export default function MegaComponent196({ seed = 196, title = 'Module 196' }) {
  const hook = useMegaHook076(seed + 196);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (196 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer196, initialState196);

  const transformed = useMemo(() => pipelineData076(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState076(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-196">
      <header className="mega-header-196">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-196">
        <div className="mega-pane-196">
          <Meter196 label="Average" value={summary.avg} ceiling={100} />
          <Meter196 label="Peak" value={summary.max} ceiling={130} />
          <Meter196 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter196 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock196 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-196">
          <form className="mega-form-196" onSubmit={submitLocal}>
