...
import { useMegaHook095 } from '../hooks/useMegaHook095.js';
import { buildTableState095, pipelineData095 } from '../utils/transformSet095.js';
import '../styles/component095.css';

function Meter095({ label, value, ceiling }) {
...
function NestedBlock095({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-095">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-095"
          style={{
...
function reducer095(state, action) {
...
export default function MegaComponent095({ seed = 95, title = 'Module 95' }) {
  const hook = useMegaHook095(seed + 95);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (95 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer095, initialState095);

  const transformed = useMemo(() => pipelineData095(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState095(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-095">
      <header className="mega-header-095">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-095">
        <div className="mega-pane-095">
          <Meter095 label="Average" value={summary.avg} ceiling={100} />
          <Meter095 label="Peak" value={summary.max} ceiling={130} />
          <Meter095 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter095 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock095 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-095">
          <form className="mega-form-095" onSubmit={submitLocal}>
