...
function Meter174({ label, value, ceiling }) {
...
function NestedBlock174({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-174">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-174"
          style={{
...
export default function MegaComponent174({ seed = 174, title = 'Module 174' }) {
  const hook = useMegaHook054(seed + 174);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (174 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-174">
      <header className="mega-header-174">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-174">
        <div className="mega-pane-174">
          <Meter174 label="Average" value={summary.avg} ceiling={100} />
          <Meter174 label="Peak" value={summary.max} ceiling={130} />
          <Meter174 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter174 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock174 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-174">
          <form className="mega-form-174" onSubmit={submitLocal}>
