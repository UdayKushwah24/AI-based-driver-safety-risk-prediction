...
import '../styles/component025.css';

function Meter025({ label, value, ceiling }) {
...
function NestedBlock025({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-025">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-025"
          style={{
...
function reducer025(state, action) {
...
export default function MegaComponent025({ seed = 25, title = 'Module 25' }) {
  const hook = useMegaHook025(seed + 25);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (25 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer025, initialState025);
  ...
  return (
    <section className="mega-card-025">
      <header className="mega-header-025">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-025">
        <div className="mega-pane-025">
          <Meter025 label="Average" value={summary.avg} ceiling={100} />
          <Meter025 label="Peak" value={summary.max} ceiling={130} />
          <Meter025 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter025 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock025 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-025">
          <form className="mega-form-025" onSubmit={submitLocal}>
