...
import '../styles/component014.css';

function Meter014({ label, value, ceiling }) {
...
function NestedBlock014({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-014">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-014"
          style={{
...
export default function MegaComponent014({ seed = 14, title = 'Module 14' }) {
  const hook = useMegaHook014(seed + 14);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (14 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-014">
      <header className="mega-header-014">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-014">
        <div className="mega-pane-014">
          <Meter014 label="Average" value={summary.avg} ceiling={100} />
          <Meter014 label="Peak" value={summary.max} ceiling={130} />
          <Meter014 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter014 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock014 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-014">
          <form className="mega-form-014" onSubmit={submitLocal}>
