...
import { useMegaHook035 } from '../hooks/useMegaHook035.js';
import { buildTableState035, pipelineData035 } from '../utils/transformSet035.js';
import '../styles/component155.css';

function Meter155({ label, value, ceiling }) {
...
function NestedBlock155({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-155">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-155"
          style={{
...
export default function MegaComponent155({ seed = 155, title = 'Module 155' }) {
  const hook = useMegaHook035(seed + 155);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (155 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer155, initialState155);

  const transformed = useMemo(() => pipelineData035(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState035(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-155">
      <header className="mega-header-155">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-155">
        <div className="mega-pane-155">
          <Meter155 label="Average" value={summary.avg} ceiling={100} />
          <Meter155 label="Peak" value={summary.max} ceiling={130} />
          <Meter155 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter155 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock155 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-155">
          <form className="mega-form-155" onSubmit={submitLocal}>
