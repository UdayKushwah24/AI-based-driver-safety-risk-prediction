...
import { useMegaHook040 } from '../hooks/useMegaHook040.js';
import { buildTableState040, pipelineData040 } from '../utils/transformSet040.js';
import '../styles/component160.css';

function Meter160({ label, value, ceiling }) {
...
function NestedBlock160({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-160">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-160"
          style={{
...
export default function MegaComponent160({ seed = 160, title = 'Module 160' }) {
  const hook = useMegaHook040(seed + 160);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (160 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer160, initialState160);

  const transformed = useMemo(() => pipelineData040(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState040(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-160">
      <header className="mega-header-160">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-160">
        <div className="mega-pane-160">
          <Meter160 label="Average" value={summary.avg} ceiling={100} />
          <Meter160 label="Peak" value={summary.max} ceiling={130} />
          <Meter160 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter160 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock160 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-160">
          <form className="mega-form-160" onSubmit={submitLocal}>
