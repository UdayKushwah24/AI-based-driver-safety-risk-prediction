...
function Meter276({ label, value, ceiling }) {
...
function NestedBlock276({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-276">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-276"
          style={{
...
export default function MegaComponent276({ seed = 276, title = 'Module 276' }) {
  const hook = useMegaHook036(seed + 276);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (276 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-276">
      <header className="mega-header-276">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-276">
        <div className="mega-pane-276">
          <Meter276 label="Average" value={summary.avg} ceiling={100} />
          <Meter276 label="Peak" value={summary.max} ceiling={130} />
          <Meter276 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter276 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock276 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-276">
          <form className="mega-form-276" onSubmit={submitLocal}>
