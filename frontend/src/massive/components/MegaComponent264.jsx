...
import { useMegaHook024 } from '../hooks/useMegaHook024.js';
import { buildTableState024, pipelineData024 } from '../utils/transformSet024.js';
import '../styles/component264.css';

function Meter264({ label, value, ceiling }) {
...
function NestedBlock264({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-264">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-264"
          style={{
...
export default function MegaComponent264({ seed = 264, title = 'Module 264' }) {
  const hook = useMegaHook024(seed + 264);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (264 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer264, initialState264);

  const transformed = useMemo(() => pipelineData024(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState024(transformed, sortBy, sortDir, page, 12), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-264">
      <header className="mega-header-264">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-264">
        <div className="mega-pane-264">
          <Meter264 label="Average" value={summary.avg} ceiling={100} />
          <Meter264 label="Peak" value={summary.max} ceiling={130} />
          <Meter264 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter264 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock264 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-264">
          <form className="mega-form-264" onSubmit={submitLocal}>
