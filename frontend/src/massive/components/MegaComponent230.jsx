...
import { useMegaHook110 } from '../hooks/useMegaHook110.js';
import { buildTableState110, pipelineData110 } from '../utils/transformSet110.js';
import '../styles/component230.css';

function Meter230({ label, value, ceiling }) {
...
function NestedBlock230({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-230">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-230"
          style={{
...
export default function MegaComponent230({ seed = 230, title = 'Module 230' }) {
  const hook = useMegaHook110(seed + 230);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (230 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer230, initialState230);

  const transformed = useMemo(() => pipelineData110(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState110(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-230">
      <header className="mega-header-230">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-230">
        <div className="mega-pane-230">
          <Meter230 label="Average" value={summary.avg} ceiling={100} />
          <Meter230 label="Peak" value={summary.max} ceiling={130} />
          <Meter230 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter230 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock230 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-230">
          <form className="mega-form-230" onSubmit={submitLocal}>
