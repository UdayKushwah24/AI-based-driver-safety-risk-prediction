...
function Meter272({ label, value, ceiling }) {
...
function NestedBlock272({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-272">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-272"
          style={{
...
export default function MegaComponent272({ seed = 272, title = 'Module 272' }) {
  const hook = useMegaHook032(seed + 272);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (272 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-272">
      <header className="mega-header-272">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-272">
        <div className="mega-pane-272">
          <Meter272 label="Average" value={summary.avg} ceiling={100} />
          <Meter272 label="Peak" value={summary.max} ceiling={130} />
          <Meter272 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter272 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock272 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-272">
          <form className="mega-form-272" onSubmit={submitLocal}>
