...
import { useMegaHook102 } from '../hooks/useMegaHook102.js';
import { buildTableState102, pipelineData102 } from '../utils/transformSet102.js';
import '../styles/component102.css';

function Meter102({ label, value, ceiling }) {
...
function NestedBlock102({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-102">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-102"
          style={{
...
export default function MegaComponent102({ seed = 102, title = 'Module 102' }) {
  const hook = useMegaHook102(seed + 102);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (102 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer102, initialState102);

  const transformed = useMemo(() => pipelineData102(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState102(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-102">
      <header className="mega-header-102">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-102">
        <div className="mega-pane-102">
          <Meter102 label="Average" value={summary.avg} ceiling={100} />
          <Meter102 label="Peak" value={summary.max} ceiling={130} />
          <Meter102 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter102 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock102 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-102">
          <form className="mega-form-102" onSubmit={submitLocal}>
