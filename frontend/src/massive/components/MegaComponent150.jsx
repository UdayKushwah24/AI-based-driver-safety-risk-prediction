...
import { useMegaHook030 } from '../hooks/useMegaHook030.js';
import { buildTableState030, pipelineData030 } from '../utils/transformSet030.js';
import '../styles/component150.css';

function Meter150({ label, value, ceiling }) {
...
function NestedBlock150({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-150">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-150"
          style={{
...
export default function MegaComponent150({ seed = 150, title = 'Module 150' }) {
  const hook = useMegaHook030(seed + 150);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (150 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer150, initialState150);

  const transformed = useMemo(() => pipelineData030(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState030(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-150">
      <header className="mega-header-150">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-150">
        <div className="mega-pane-150">
          <Meter150 label="Average" value={summary.avg} ceiling={100} />
          <Meter150 label="Peak" value={summary.max} ceiling={130} />
          <Meter150 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter150 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock150 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-150">
          <form className="mega-form-150" onSubmit={submitLocal}>
