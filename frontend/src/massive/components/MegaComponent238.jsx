...
import { useMegaHook118 } from '../hooks/useMegaHook118.js';
import { buildTableState118, pipelineData118 } from '../utils/transformSet118.js';
import '../styles/component238.css';

function Meter238({ label, value, ceiling }) {
...
function NestedBlock238({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-238">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-238"
          style={{
...
export default function MegaComponent238({ seed = 238, title = 'Module 238' }) {
  const hook = useMegaHook118(seed + 238);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (238 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer238, initialState238);

  const transformed = useMemo(() => pipelineData118(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState118(transformed, sortBy, sortDir, page, 11), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-238">
      <header className="mega-header-238">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-238">
        <div className="mega-pane-238">
          <Meter238 label="Average" value={summary.avg} ceiling={100} />
          <Meter238 label="Peak" value={summary.max} ceiling={130} />
          <Meter238 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter238 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock238 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-238">
          <form className="mega-form-238" onSubmit={submitLocal}>
