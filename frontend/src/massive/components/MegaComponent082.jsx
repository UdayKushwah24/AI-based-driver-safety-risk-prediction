...
import '../styles/component082.css';

function Meter082({ label, value, ceiling }) {
...
function NestedBlock082({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-082">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-082"
          style={{
...
function reducer082(state, action) {
...
export default function MegaComponent082({ seed = 82, title = 'Module 82' }) {
  const hook = useMegaHook082(seed + 82);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (82 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer082, initialState082);
  ...
  return (
    <section className="mega-card-082">
      <header className="mega-header-082">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-082">
        <div className="mega-pane-082">
          <Meter082 label="Average" value={summary.avg} ceiling={100} />
          <Meter082 label="Peak" value={summary.max} ceiling={130} />
          <Meter082 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter082 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock082 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-082">
          <form className="mega-form-082" onSubmit={submitLocal}>
