...
import { useMegaHook062 } from '../hooks/useMegaHook062.js';
import { buildTableState062, pipelineData062 } from '../utils/transformSet062.js';
import '../styles/component182.css';

function Meter182({ label, value, ceiling }) {
...
function NestedBlock182({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-182">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-182"
          style={{
...
export default function MegaComponent182({ seed = 182, title = 'Module 182' }) {
  const hook = useMegaHook062(seed + 182);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (182 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer182, initialState182);

  const transformed = useMemo(() => pipelineData062(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState062(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-182">
      <header className="mega-header-182">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-182">
        <div className="mega-pane-182">
          <Meter182 label="Average" value={summary.avg} ceiling={100} />
          <Meter182 label="Peak" value={summary.max} ceiling={130} />
          <Meter182 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter182 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock182 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-182">
          <form className="mega-form-182" onSubmit={submitLocal}>
