...
import '../styles/component072.css';

function Meter072({ label, value, ceiling }) {
...
function NestedBlock072({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-072">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-072"
          style={{
...
function reducer072(state, action) {
...
export default function MegaComponent072({ seed = 72, title = 'Module 72' }) {
  const hook = useMegaHook072(seed + 72);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (72 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer072, initialState072);
  ...
  return (
    <section className="mega-card-072">
      <header className="mega-header-072">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-072">
        <div className="mega-pane-072">
          <Meter072 label="Average" value={summary.avg} ceiling={100} />
          <Meter072 label="Peak" value={summary.max} ceiling={130} />
          <Meter072 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter072 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock072 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-072">
          <form className="mega-form-072" onSubmit={submitLocal}>
