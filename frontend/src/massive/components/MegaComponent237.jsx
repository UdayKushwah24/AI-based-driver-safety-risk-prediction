...
import { useMegaHook117 } from '../hooks/useMegaHook117.js';
import { buildTableState117, pipelineData117 } from '../utils/transformSet117.js';
import '../styles/component237.css';

function Meter237({ label, value, ceiling }) {
...
function NestedBlock237({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-237">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-237"
          style={{
...
export default function MegaComponent237({ seed = 237, title = 'Module 237' }) {
  const hook = useMegaHook117(seed + 237);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (237 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer237, initialState237);

  const transformed = useMemo(() => pipelineData117(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState117(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-237">
      <header className="mega-header-237">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-237">
        <div className="mega-pane-237">
          <Meter237 label="Average" value={summary.avg} ceiling={100} />
          <Meter237 label="Peak" value={summary.max} ceiling={130} />
          <Meter237 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter237 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock237 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-237">
          <form className="mega-form-237" onSubmit={submitLocal}>
