...
import { useMegaHook084 } from '../hooks/useMegaHook084.js';
import { buildTableState084, pipelineData084 } from '../utils/transformSet084.js';
import '../styles/component204.css';

function Meter204({ label, value, ceiling }) {
...
function NestedBlock204({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-204">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-204"
          style={{
...
export default function MegaComponent204({ seed = 204, title = 'Module 204' }) {
  const hook = useMegaHook084(seed + 204);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (204 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer204, initialState204);

  const transformed = useMemo(() => pipelineData084(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState084(transformed, sortBy, sortDir, page, 12), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-204">
      <header className="mega-header-204">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-204">
        <div className="mega-pane-204">
          <Meter204 label="Average" value={summary.avg} ceiling={100} />
          <Meter204 label="Peak" value={summary.max} ceiling={130} />
          <Meter204 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter204 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock204 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-204">
          <form className="mega-form-204" onSubmit={submitLocal}>
