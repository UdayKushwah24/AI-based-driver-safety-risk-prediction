...
function Meter282({ label, value, ceiling }) {
...
function NestedBlock282({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-282">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-282"
          style={{
...
export default function MegaComponent282({ seed = 282, title = 'Module 282' }) {
  const hook = useMegaHook042(seed + 282);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (282 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-282">
      <header className="mega-header-282">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-282">
        <div className="mega-pane-282">
          <Meter282 label="Average" value={summary.avg} ceiling={100} />
          <Meter282 label="Peak" value={summary.max} ceiling={130} />
          <Meter282 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter282 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock282 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-282">
          <form className="mega-form-282" onSubmit={submitLocal}>
