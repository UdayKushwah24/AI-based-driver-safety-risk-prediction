...
function Meter126({ label, value, ceiling }) {
...
function NestedBlock126({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-126">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-126"
          style={{
...
export default function MegaComponent126({ seed = 126, title = 'Module 126' }) {
  const hook = useMegaHook006(seed + 126);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (126 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-126">
      <header className="mega-header-126">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-126">
        <div className="mega-pane-126">
          <Meter126 label="Average" value={summary.avg} ceiling={100} />
          <Meter126 label="Peak" value={summary.max} ceiling={130} />
          <Meter126 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter126 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock126 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-126">
          <form className="mega-form-126" onSubmit={submitLocal}>
