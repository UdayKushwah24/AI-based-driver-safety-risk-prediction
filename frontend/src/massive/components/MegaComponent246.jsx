...
import { useMegaHook006 } from '../hooks/useMegaHook006.js';
import { buildTableState006, pipelineData006 } from '../utils/transformSet006.js';
import '../styles/component246.css';

function Meter246({ label, value, ceiling }) {
...
function NestedBlock246({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-246">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-246"
          style={{
...
export default function MegaComponent246({ seed = 246, title = 'Module 246' }) {
  const hook = useMegaHook006(seed + 246);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (246 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer246, initialState246);

  const transformed = useMemo(() => pipelineData006(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState006(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-246">
      <header className="mega-header-246">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-246">
        <div className="mega-pane-246">
          <Meter246 label="Average" value={summary.avg} ceiling={100} />
          <Meter246 label="Peak" value={summary.max} ceiling={130} />
          <Meter246 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter246 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock246 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-246">
          <form className="mega-form-246" onSubmit={submitLocal}>
