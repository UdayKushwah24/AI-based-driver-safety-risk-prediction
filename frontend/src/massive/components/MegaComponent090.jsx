...
import '../styles/component090.css';

function Meter090({ label, value, ceiling }) {
...
function NestedBlock090({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-090">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-090"
          style={{
...
function reducer090(state, action) {
...
export default function MegaComponent090({ seed = 90, title = 'Module 90' }) {
  const hook = useMegaHook090(seed + 90);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (90 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer090, initialState090);
  ...
  return (
    <section className="mega-card-090">
      <header className="mega-header-090">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-090">
        <div className="mega-pane-090">
          <Meter090 label="Average" value={summary.avg} ceiling={100} />
          <Meter090 label="Peak" value={summary.max} ceiling={130} />
          <Meter090 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter090 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock090 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-090">
          <form className="mega-form-090" onSubmit={submitLocal}>
