...
import '../styles/component051.css';

function Meter051({ label, value, ceiling }) {
...
function NestedBlock051({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-051">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-051"
          style={{
...
function reducer051(state, action) {
...
export default function MegaComponent051({ seed = 51, title = 'Module 51' }) {
  const hook = useMegaHook051(seed + 51);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (51 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer051, initialState051);
  ...
  return (
    <section className="mega-card-051">
      <header className="mega-header-051">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-051">
        <div className="mega-pane-051">
          <Meter051 label="Average" value={summary.avg} ceiling={100} />
          <Meter051 label="Peak" value={summary.max} ceiling={130} />
          <Meter051 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter051 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock051 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-051">
          <form className="mega-form-051" onSubmit={submitLocal}>
