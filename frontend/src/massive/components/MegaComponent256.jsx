...
import { useMegaHook016 } from '../hooks/useMegaHook016.js';
import { buildTableState016, pipelineData016 } from '../utils/transformSet016.js';
import '../styles/component256.css';

function Meter256({ label, value, ceiling }) {
...
function NestedBlock256({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-256">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-256"
          style={{
...
export default function MegaComponent256({ seed = 256, title = 'Module 256' }) {
  const hook = useMegaHook016(seed + 256);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (256 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer256, initialState256);

  const transformed = useMemo(() => pipelineData016(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState016(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-256">
      <header className="mega-header-256">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-256">
        <div className="mega-pane-256">
          <Meter256 label="Average" value={summary.avg} ceiling={100} />
          <Meter256 label="Peak" value={summary.max} ceiling={130} />
          <Meter256 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter256 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock256 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-256">
          <form className="mega-form-256" onSubmit={submitLocal}>
