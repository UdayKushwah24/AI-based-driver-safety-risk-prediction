...
import { useMegaHook113 } from '../hooks/useMegaHook113.js';
import { buildTableState113, pipelineData113 } from '../utils/transformSet113.js';
import '../styles/component233.css';

function Meter233({ label, value, ceiling }) {
...
function NestedBlock233({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-233">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-233"
          style={{
...
export default function MegaComponent233({ seed = 233, title = 'Module 233' }) {
  const hook = useMegaHook113(seed + 233);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (233 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer233, initialState233);

  const transformed = useMemo(() => pipelineData113(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState113(transformed, sortBy, sortDir, page, 11), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-233">
      <header className="mega-header-233">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-233">
        <div className="mega-pane-233">
          <Meter233 label="Average" value={summary.avg} ceiling={100} />
          <Meter233 label="Peak" value={summary.max} ceiling={130} />
          <Meter233 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter233 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock233 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-233">
          <form className="mega-form-233" onSubmit={submitLocal}>
