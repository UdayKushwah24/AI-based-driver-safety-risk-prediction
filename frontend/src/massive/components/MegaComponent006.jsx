...
import '../styles/component006.css';

function Meter006({ label, value, ceiling }) {
...
function NestedBlock006({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-006">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-006"
          style={{
...
function reducer006(state, action) {
...
export default function MegaComponent006({ seed = 6, title = 'Module 6' }) {
  const hook = useMegaHook006(seed + 6);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (6 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer006, initialState006);
  ...
  return (
    <section className="mega-card-006">
      <header className="mega-header-006">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-006">
        <div className="mega-pane-006">
          <Meter006 label="Average" value={summary.avg} ceiling={100} />
          <Meter006 label="Peak" value={summary.max} ceiling={130} />
          <Meter006 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter006 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock006 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-006">
          <form className="mega-form-006" onSubmit={submitLocal}>
