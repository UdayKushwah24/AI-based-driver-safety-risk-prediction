...
import { useMegaHook047 } from '../hooks/useMegaHook047.js';
import { buildTableState047, pipelineData047 } from '../utils/transformSet047.js';
import '../styles/component287.css';

function Meter287({ label, value, ceiling }) {
...
function NestedBlock287({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-287">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-287"
          style={{
...
export default function MegaComponent287({ seed = 287, title = 'Module 287' }) {
  const hook = useMegaHook047(seed + 287);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (287 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer287, initialState287);

  const transformed = useMemo(() => pipelineData047(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState047(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-287">
      <header className="mega-header-287">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-287">
        <div className="mega-pane-287">
          <Meter287 label="Average" value={summary.avg} ceiling={100} />
          <Meter287 label="Peak" value={summary.max} ceiling={130} />
          <Meter287 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter287 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock287 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-287">
          <form className="mega-form-287" onSubmit={submitLocal}>
