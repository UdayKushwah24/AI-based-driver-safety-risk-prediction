...
import { useMegaHook025 } from '../hooks/useMegaHook025.js';
import { buildTableState025, pipelineData025 } from '../utils/transformSet025.js';
import '../styles/component265.css';

function Meter265({ label, value, ceiling }) {
...
function NestedBlock265({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-265">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-265"
          style={{
...
export default function MegaComponent265({ seed = 265, title = 'Module 265' }) {
  const hook = useMegaHook025(seed + 265);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (265 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer265, initialState265);

  const transformed = useMemo(() => pipelineData025(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState025(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-265">
      <header className="mega-header-265">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-265">
        <div className="mega-pane-265">
          <Meter265 label="Average" value={summary.avg} ceiling={100} />
          <Meter265 label="Peak" value={summary.max} ceiling={130} />
          <Meter265 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter265 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock265 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-265">
          <form className="mega-form-265" onSubmit={submitLocal}>
