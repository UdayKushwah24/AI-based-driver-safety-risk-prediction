...
import '../styles/component002.css';

function Meter002({ label, value, ceiling }) {
...
function NestedBlock002({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-002">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-002"
          style={{
...
function reducer002(state, action) {
...
export default function MegaComponent002({ seed = 2, title = 'Module 2' }) {
  const hook = useMegaHook002(seed + 2);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (2 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer002, initialState002);
  ...
  return (
    <section className="mega-card-002">
      <header className="mega-header-002">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-002">
        <div className="mega-pane-002">
          <Meter002 label="Average" value={summary.avg} ceiling={100} />
          <Meter002 label="Peak" value={summary.max} ceiling={130} />
          <Meter002 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter002 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock002 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-002">
          <form className="mega-form-002" onSubmit={submitLocal}>
