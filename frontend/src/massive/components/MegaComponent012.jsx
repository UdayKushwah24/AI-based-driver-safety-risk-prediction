...
import '../styles/component012.css';

function Meter012({ label, value, ceiling }) {
...
function NestedBlock012({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-012">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-012"
          style={{
...
function reducer012(state, action) {
...
export default function MegaComponent012({ seed = 12, title = 'Module 12' }) {
  const hook = useMegaHook012(seed + 12);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (12 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer012, initialState012);
  ...
  return (
    <section className="mega-card-012">
      <header className="mega-header-012">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-012">
        <div className="mega-pane-012">
          <Meter012 label="Average" value={summary.avg} ceiling={100} />
          <Meter012 label="Peak" value={summary.max} ceiling={130} />
          <Meter012 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter012 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock012 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-012">
          <form className="mega-form-012" onSubmit={submitLocal}>
