...
import '../styles/component076.css';

function Meter076({ label, value, ceiling }) {
...
function NestedBlock076({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-076">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-076"
          style={{
...
function reducer076(state, action) {
...
export default function MegaComponent076({ seed = 76, title = 'Module 76' }) {
  const hook = useMegaHook076(seed + 76);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (76 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer076, initialState076);
  ...
  return (
    <section className="mega-card-076">
      <header className="mega-header-076">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-076">
        <div className="mega-pane-076">
          <Meter076 label="Average" value={summary.avg} ceiling={100} />
          <Meter076 label="Peak" value={summary.max} ceiling={130} />
          <Meter076 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter076 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock076 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-076">
          <form className="mega-form-076" onSubmit={submitLocal}>
