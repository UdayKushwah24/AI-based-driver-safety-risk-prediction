...
function Meter104({ label, value, ceiling }) {
...
function NestedBlock104({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-104">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-104"
          style={{
...
export default function MegaComponent104({ seed = 104, title = 'Module 104' }) {
  const hook = useMegaHook104(seed + 104);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (104 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-104">
      <header className="mega-header-104">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-104">
        <div className="mega-pane-104">
          <Meter104 label="Average" value={summary.avg} ceiling={100} />
          <Meter104 label="Peak" value={summary.max} ceiling={130} />
          <Meter104 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter104 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock104 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-104">
          <form className="mega-form-104" onSubmit={submitLocal}>
