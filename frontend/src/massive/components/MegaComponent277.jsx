...
function Meter277({ label, value, ceiling }) {
...
function NestedBlock277({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-277">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-277"
          style={{
...
export default function MegaComponent277({ seed = 277, title = 'Module 277' }) {
  const hook = useMegaHook037(seed + 277);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (277 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-277">
      <header className="mega-header-277">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-277">
        <div className="mega-pane-277">
          <Meter277 label="Average" value={summary.avg} ceiling={100} />
          <Meter277 label="Peak" value={summary.max} ceiling={130} />
          <Meter277 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter277 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock277 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-277">
          <form className="mega-form-277" onSubmit={submitLocal}>
