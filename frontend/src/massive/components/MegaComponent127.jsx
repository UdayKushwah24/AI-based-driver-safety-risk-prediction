...
function Meter127({ label, value, ceiling }) {
...
function NestedBlock127({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-127">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-127"
          style={{
...
export default function MegaComponent127({ seed = 127, title = 'Module 127' }) {
  const hook = useMegaHook007(seed + 127);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (127 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-127">
      <header className="mega-header-127">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-127">
        <div className="mega-pane-127">
          <Meter127 label="Average" value={summary.avg} ceiling={100} />
          <Meter127 label="Peak" value={summary.max} ceiling={130} />
          <Meter127 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter127 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock127 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-127">
          <form className="mega-form-127" onSubmit={submitLocal}>
