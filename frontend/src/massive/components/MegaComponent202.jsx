...
import { useMegaHook082 } from '../hooks/useMegaHook082.js';
import { buildTableState082, pipelineData082 } from '../utils/transformSet082.js';
import '../styles/component202.css';

function Meter202({ label, value, ceiling }) {
...
function NestedBlock202({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-202">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-202"
          style={{
...
export default function MegaComponent202({ seed = 202, title = 'Module 202' }) {
  const hook = useMegaHook082(seed + 202);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (202 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer202, initialState202);

  const transformed = useMemo(() => pipelineData082(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState082(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-202">
      <header className="mega-header-202">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-202">
        <div className="mega-pane-202">
          <Meter202 label="Average" value={summary.avg} ceiling={100} />
          <Meter202 label="Peak" value={summary.max} ceiling={130} />
          <Meter202 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter202 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock202 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-202">
          <form className="mega-form-202" onSubmit={submitLocal}>
