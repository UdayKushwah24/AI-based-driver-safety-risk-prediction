...
import '../styles/component086.css';

function Meter086({ label, value, ceiling }) {
...
function NestedBlock086({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-086">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-086"
          style={{
...
export default function MegaComponent086({ seed = 86, title = 'Module 86' }) {
  const hook = useMegaHook086(seed + 86);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (86 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-086">
      <header className="mega-header-086">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-086">
        <div className="mega-pane-086">
          <Meter086 label="Average" value={summary.avg} ceiling={100} />
          <Meter086 label="Peak" value={summary.max} ceiling={130} />
          <Meter086 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter086 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock086 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-086">
          <form className="mega-form-086" onSubmit={submitLocal}>
