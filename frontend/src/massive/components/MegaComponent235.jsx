...
function Meter235({ label, value, ceiling }) {
...
function NestedBlock235({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-235">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-235"
          style={{
...
export default function MegaComponent235({ seed = 235, title = 'Module 235' }) {
  const hook = useMegaHook115(seed + 235);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (235 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-235">
      <header className="mega-header-235">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-235">
        <div className="mega-pane-235">
          <Meter235 label="Average" value={summary.avg} ceiling={100} />
          <Meter235 label="Peak" value={summary.max} ceiling={130} />
          <Meter235 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter235 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock235 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-235">
          <form className="mega-form-235" onSubmit={submitLocal}>
