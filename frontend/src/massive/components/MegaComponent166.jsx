...
function Meter166({ label, value, ceiling }) {
...
function NestedBlock166({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-166">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-166"
          style={{
...
export default function MegaComponent166({ seed = 166, title = 'Module 166' }) {
  const hook = useMegaHook046(seed + 166);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (166 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-166">
      <header className="mega-header-166">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-166">
        <div className="mega-pane-166">
          <Meter166 label="Average" value={summary.avg} ceiling={100} />
          <Meter166 label="Peak" value={summary.max} ceiling={130} />
          <Meter166 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter166 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock166 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-166">
          <form className="mega-form-166" onSubmit={submitLocal}>
