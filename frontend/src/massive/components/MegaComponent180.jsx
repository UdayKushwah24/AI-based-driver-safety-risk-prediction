...
function Meter180({ label, value, ceiling }) {
...
function NestedBlock180({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-180">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-180"
          style={{
...
export default function MegaComponent180({ seed = 180, title = 'Module 180' }) {
  const hook = useMegaHook060(seed + 180);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (180 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-180">
      <header className="mega-header-180">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-180">
        <div className="mega-pane-180">
          <Meter180 label="Average" value={summary.avg} ceiling={100} />
          <Meter180 label="Peak" value={summary.max} ceiling={130} />
          <Meter180 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter180 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock180 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-180">
          <form className="mega-form-180" onSubmit={submitLocal}>
