...
import { useMegaHook061 } from '../hooks/useMegaHook061.js';
import { buildTableState061, pipelineData061 } from '../utils/transformSet061.js';
import '../styles/component181.css';

function Meter181({ label, value, ceiling }) {
...
function NestedBlock181({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-181">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-181"
          style={{
...
export default function MegaComponent181({ seed = 181, title = 'Module 181' }) {
  const hook = useMegaHook061(seed + 181);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (181 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer181, initialState181);

  const transformed = useMemo(() => pipelineData061(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState061(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-181">
      <header className="mega-header-181">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-181">
        <div className="mega-pane-181">
          <Meter181 label="Average" value={summary.avg} ceiling={100} />
          <Meter181 label="Peak" value={summary.max} ceiling={130} />
          <Meter181 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter181 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock181 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-181">
          <form className="mega-form-181" onSubmit={submitLocal}>
