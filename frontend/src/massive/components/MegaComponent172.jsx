...
function Meter172({ label, value, ceiling }) {
...
function NestedBlock172({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-172">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-172"
          style={{
...
export default function MegaComponent172({ seed = 172, title = 'Module 172' }) {
  const hook = useMegaHook052(seed + 172);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (172 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-172">
      <header className="mega-header-172">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-172">
        <div className="mega-pane-172">
          <Meter172 label="Average" value={summary.avg} ceiling={100} />
          <Meter172 label="Peak" value={summary.max} ceiling={130} />
          <Meter172 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter172 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock172 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-172">
          <form className="mega-form-172" onSubmit={submitLocal}>
