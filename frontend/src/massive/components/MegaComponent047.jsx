...
import '../styles/component047.css';

function Meter047({ label, value, ceiling }) {
...
function NestedBlock047({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-047">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-047"
          style={{
...
export default function MegaComponent047({ seed = 47, title = 'Module 47' }) {
  const hook = useMegaHook047(seed + 47);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (47 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-047">
      <header className="mega-header-047">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-047">
        <div className="mega-pane-047">
          <Meter047 label="Average" value={summary.avg} ceiling={100} />
          <Meter047 label="Peak" value={summary.max} ceiling={130} />
          <Meter047 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter047 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock047 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-047">
          <form className="mega-form-047" onSubmit={submitLocal}>
