...
import { useMegaHook018 } from '../hooks/useMegaHook018.js';
import { buildTableState018, pipelineData018 } from '../utils/transformSet018.js';
import '../styles/component258.css';

function Meter258({ label, value, ceiling }) {
...
function NestedBlock258({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-258">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-258"
          style={{
...
export default function MegaComponent258({ seed = 258, title = 'Module 258' }) {
  const hook = useMegaHook018(seed + 258);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (258 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer258, initialState258);

  const transformed = useMemo(() => pipelineData018(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState018(transformed, sortBy, sortDir, page, 11), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-258">
      <header className="mega-header-258">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-258">
        <div className="mega-pane-258">
          <Meter258 label="Average" value={summary.avg} ceiling={100} />
          <Meter258 label="Peak" value={summary.max} ceiling={130} />
          <Meter258 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter258 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock258 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-258">
          <form className="mega-form-258" onSubmit={submitLocal}>
