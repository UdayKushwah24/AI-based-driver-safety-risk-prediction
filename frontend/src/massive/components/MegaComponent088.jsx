...
import '../styles/component088.css';

function Meter088({ label, value, ceiling }) {
...
function NestedBlock088({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-088">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-088"
          style={{
...
function reducer088(state, action) {
...
export default function MegaComponent088({ seed = 88, title = 'Module 88' }) {
  const hook = useMegaHook088(seed + 88);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (88 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer088, initialState088);
  ...
  return (
    <section className="mega-card-088">
      <header className="mega-header-088">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-088">
        <div className="mega-pane-088">
          <Meter088 label="Average" value={summary.avg} ceiling={100} />
          <Meter088 label="Peak" value={summary.max} ceiling={130} />
          <Meter088 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter088 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock088 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-088">
          <form className="mega-form-088" onSubmit={submitLocal}>
