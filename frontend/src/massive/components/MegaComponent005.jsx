...
import '../styles/component005.css';

function Meter005({ label, value, ceiling }) {
...
function NestedBlock005({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-005">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-005"
          style={{
...
function reducer005(state, action) {
...
export default function MegaComponent005({ seed = 5, title = 'Module 5' }) {
  const hook = useMegaHook005(seed + 5);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (5 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer005, initialState005);
  ...
  return (
    <section className="mega-card-005">
      <header className="mega-header-005">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-005">
        <div className="mega-pane-005">
          <Meter005 label="Average" value={summary.avg} ceiling={100} />
          <Meter005 label="Peak" value={summary.max} ceiling={130} />
          <Meter005 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter005 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock005 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-005">
          <form className="mega-form-005" onSubmit={submitLocal}>
