...
import { useMegaHook041 } from '../hooks/useMegaHook041.js';
import { buildTableState041, pipelineData041 } from '../utils/transformSet041.js';
import '../styles/component281.css';

function Meter281({ label, value, ceiling }) {
...
function NestedBlock281({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-281">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-281"
          style={{
...
export default function MegaComponent281({ seed = 281, title = 'Module 281' }) {
  const hook = useMegaHook041(seed + 281);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (281 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer281, initialState281);

  const transformed = useMemo(() => pipelineData041(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState041(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-281">
      <header className="mega-header-281">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-281">
        <div className="mega-pane-281">
          <Meter281 label="Average" value={summary.avg} ceiling={100} />
          <Meter281 label="Peak" value={summary.max} ceiling={130} />
          <Meter281 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter281 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock281 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-281">
          <form className="mega-form-281" onSubmit={submitLocal}>
