...
import { useMegaHook005 } from '../hooks/useMegaHook005.js';
import { buildTableState005, pipelineData005 } from '../utils/transformSet005.js';
import '../styles/component245.css';

function Meter245({ label, value, ceiling }) {
...
function NestedBlock245({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-245">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-245"
          style={{
...
export default function MegaComponent245({ seed = 245, title = 'Module 245' }) {
  const hook = useMegaHook005(seed + 245);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (245 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer245, initialState245);

  const transformed = useMemo(() => pipelineData005(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState005(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-245">
      <header className="mega-header-245">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-245">
        <div className="mega-pane-245">
          <Meter245 label="Average" value={summary.avg} ceiling={100} />
          <Meter245 label="Peak" value={summary.max} ceiling={130} />
          <Meter245 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter245 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock245 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-245">
          <form className="mega-form-245" onSubmit={submitLocal}>
