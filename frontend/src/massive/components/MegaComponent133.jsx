...
function Meter133({ label, value, ceiling }) {
...
function NestedBlock133({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-133">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-133"
          style={{
...
export default function MegaComponent133({ seed = 133, title = 'Module 133' }) {
  const hook = useMegaHook013(seed + 133);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (133 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-133">
      <header className="mega-header-133">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-133">
        <div className="mega-pane-133">
          <Meter133 label="Average" value={summary.avg} ceiling={100} />
          <Meter133 label="Peak" value={summary.max} ceiling={130} />
          <Meter133 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter133 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock133 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-133">
          <form className="mega-form-133" onSubmit={submitLocal}>
