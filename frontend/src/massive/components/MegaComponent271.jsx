...
function Meter271({ label, value, ceiling }) {
...
function NestedBlock271({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-271">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-271"
          style={{
...
export default function MegaComponent271({ seed = 271, title = 'Module 271' }) {
  const hook = useMegaHook031(seed + 271);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (271 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-271">
      <header className="mega-header-271">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-271">
        <div className="mega-pane-271">
          <Meter271 label="Average" value={summary.avg} ceiling={100} />
          <Meter271 label="Peak" value={summary.max} ceiling={130} />
          <Meter271 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter271 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock271 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-271">
          <form className="mega-form-271" onSubmit={submitLocal}>
