...
import '../styles/component062.css';

function Meter062({ label, value, ceiling }) {
...
function NestedBlock062({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-062">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-062"
          style={{
...
function reducer062(state, action) {
...
export default function MegaComponent062({ seed = 62, title = 'Module 62' }) {
  const hook = useMegaHook062(seed + 62);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (62 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer062, initialState062);
  ...
  return (
    <section className="mega-card-062">
      <header className="mega-header-062">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-062">
        <div className="mega-pane-062">
          <Meter062 label="Average" value={summary.avg} ceiling={100} />
          <Meter062 label="Peak" value={summary.max} ceiling={130} />
          <Meter062 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter062 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock062 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-062">
          <form className="mega-form-062" onSubmit={submitLocal}>
