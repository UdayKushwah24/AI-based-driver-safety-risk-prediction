...
import { useMegaHook058 } from '../hooks/useMegaHook058.js';
import { buildTableState058, pipelineData058 } from '../utils/transformSet058.js';
import '../styles/component298.css';

function Meter298({ label, value, ceiling }) {
...
function NestedBlock298({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-298">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-298"
          style={{
...
export default function MegaComponent298({ seed = 298, title = 'Module 298' }) {
  const hook = useMegaHook058(seed + 298);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (298 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer298, initialState298);

  const transformed = useMemo(() => pipelineData058(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState058(transformed, sortBy, sortDir, page, 11), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-298">
      <header className="mega-header-298">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-298">
        <div className="mega-pane-298">
          <Meter298 label="Average" value={summary.avg} ceiling={100} />
          <Meter298 label="Peak" value={summary.max} ceiling={130} />
          <Meter298 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter298 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock298 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-298">
          <form className="mega-form-298" onSubmit={submitLocal}>
