...
function Meter178({ label, value, ceiling }) {
...
function NestedBlock178({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-178">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-178"
          style={{
...
export default function MegaComponent178({ seed = 178, title = 'Module 178' }) {
  const hook = useMegaHook058(seed + 178);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (178 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-178">
      <header className="mega-header-178">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-178">
        <div className="mega-pane-178">
          <Meter178 label="Average" value={summary.avg} ceiling={100} />
          <Meter178 label="Peak" value={summary.max} ceiling={130} />
          <Meter178 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter178 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock178 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-178">
          <form className="mega-form-178" onSubmit={submitLocal}>
