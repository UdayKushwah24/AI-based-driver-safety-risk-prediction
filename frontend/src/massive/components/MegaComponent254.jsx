...
function Meter254({ label, value, ceiling }) {
...
function NestedBlock254({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-254">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-254"
          style={{
...
export default function MegaComponent254({ seed = 254, title = 'Module 254' }) {
  const hook = useMegaHook014(seed + 254);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (254 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-254">
      <header className="mega-header-254">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-254">
        <div className="mega-pane-254">
          <Meter254 label="Average" value={summary.avg} ceiling={100} />
          <Meter254 label="Peak" value={summary.max} ceiling={130} />
          <Meter254 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter254 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock254 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-254">
          <form className="mega-form-254" onSubmit={submitLocal}>
