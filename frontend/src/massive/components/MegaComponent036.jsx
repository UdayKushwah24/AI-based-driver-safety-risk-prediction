...
import '../styles/component036.css';

function Meter036({ label, value, ceiling }) {
...
function NestedBlock036({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-036">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-036"
          style={{
...
function reducer036(state, action) {
...
export default function MegaComponent036({ seed = 36, title = 'Module 36' }) {
  const hook = useMegaHook036(seed + 36);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (36 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer036, initialState036);
  ...
  return (
    <section className="mega-card-036">
      <header className="mega-header-036">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-036">
        <div className="mega-pane-036">
          <Meter036 label="Average" value={summary.avg} ceiling={100} />
          <Meter036 label="Peak" value={summary.max} ceiling={130} />
          <Meter036 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter036 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock036 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-036">
          <form className="mega-form-036" onSubmit={submitLocal}>
