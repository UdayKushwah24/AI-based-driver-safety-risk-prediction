...
import '../styles/component078.css';

function Meter078({ label, value, ceiling }) {
...
function NestedBlock078({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-078">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-078"
          style={{
...
function reducer078(state, action) {
...
export default function MegaComponent078({ seed = 78, title = 'Module 78' }) {
  const hook = useMegaHook078(seed + 78);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (78 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer078, initialState078);
  ...
  return (
    <section className="mega-card-078">
      <header className="mega-header-078">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-078">
        <div className="mega-pane-078">
          <Meter078 label="Average" value={summary.avg} ceiling={100} />
          <Meter078 label="Peak" value={summary.max} ceiling={130} />
          <Meter078 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter078 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock078 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-078">
          <form className="mega-form-078" onSubmit={submitLocal}>
