...
function Meter288({ label, value, ceiling }) {
...
function NestedBlock288({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-288">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-288"
          style={{
...
export default function MegaComponent288({ seed = 288, title = 'Module 288' }) {
  const hook = useMegaHook048(seed + 288);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (288 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-288">
      <header className="mega-header-288">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-288">
        <div className="mega-pane-288">
          <Meter288 label="Average" value={summary.avg} ceiling={100} />
          <Meter288 label="Peak" value={summary.max} ceiling={130} />
          <Meter288 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter288 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock288 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-288">
          <form className="mega-form-288" onSubmit={submitLocal}>
