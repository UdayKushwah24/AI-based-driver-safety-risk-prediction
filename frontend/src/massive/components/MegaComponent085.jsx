...
import '../styles/component085.css';

function Meter085({ label, value, ceiling }) {
...
function NestedBlock085({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-085">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-085"
          style={{
...
function reducer085(state, action) {
...
export default function MegaComponent085({ seed = 85, title = 'Module 85' }) {
  const hook = useMegaHook085(seed + 85);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (85 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer085, initialState085);
  ...
  return (
    <section className="mega-card-085">
      <header className="mega-header-085">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-085">
        <div className="mega-pane-085">
          <Meter085 label="Average" value={summary.avg} ceiling={100} />
          <Meter085 label="Peak" value={summary.max} ceiling={130} />
          <Meter085 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter085 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock085 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-085">
          <form className="mega-form-085" onSubmit={submitLocal}>
