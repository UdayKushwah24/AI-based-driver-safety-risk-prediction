...
import '../styles/component060.css';

function Meter060({ label, value, ceiling }) {
...
function NestedBlock060({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-060">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-060"
          style={{
...
function reducer060(state, action) {
...
export default function MegaComponent060({ seed = 60, title = 'Module 60' }) {
  const hook = useMegaHook060(seed + 60);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (60 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer060, initialState060);
  ...
  return (
    <section className="mega-card-060">
      <header className="mega-header-060">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-060">
        <div className="mega-pane-060">
          <Meter060 label="Average" value={summary.avg} ceiling={100} />
          <Meter060 label="Peak" value={summary.max} ceiling={130} />
          <Meter060 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter060 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock060 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-060">
          <form className="mega-form-060" onSubmit={submitLocal}>
