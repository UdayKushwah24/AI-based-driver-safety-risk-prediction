...
import '../styles/component010.css';

function Meter010({ label, value, ceiling }) {
...
function NestedBlock010({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-010">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-010"
          style={{
...
function reducer010(state, action) {
...
export default function MegaComponent010({ seed = 10, title = 'Module 10' }) {
  const hook = useMegaHook010(seed + 10);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (10 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer010, initialState010);
  ...
  return (
    <section className="mega-card-010">
      <header className="mega-header-010">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-010">
        <div className="mega-pane-010">
          <Meter010 label="Average" value={summary.avg} ceiling={100} />
          <Meter010 label="Peak" value={summary.max} ceiling={130} />
          <Meter010 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter010 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock010 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-010">
          <form className="mega-form-010" onSubmit={submitLocal}>
