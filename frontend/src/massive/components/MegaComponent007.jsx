...
import '../styles/component007.css';

function Meter007({ label, value, ceiling }) {
...
function NestedBlock007({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-007">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-007"
          style={{
...
function reducer007(state, action) {
...
export default function MegaComponent007({ seed = 7, title = 'Module 7' }) {
  const hook = useMegaHook007(seed + 7);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (7 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer007, initialState007);
  ...
  return (
    <section className="mega-card-007">
      <header className="mega-header-007">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-007">
        <div className="mega-pane-007">
          <Meter007 label="Average" value={summary.avg} ceiling={100} />
          <Meter007 label="Peak" value={summary.max} ceiling={130} />
          <Meter007 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter007 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock007 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-007">
          <form className="mega-form-007" onSubmit={submitLocal}>
