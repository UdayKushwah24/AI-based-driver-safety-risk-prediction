...
import { useMegaHook023 } from '../hooks/useMegaHook023.js';
import { buildTableState023, pipelineData023 } from '../utils/transformSet023.js';
import '../styles/component143.css';

function Meter143({ label, value, ceiling }) {
...
function NestedBlock143({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-143">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-143"
          style={{
...
export default function MegaComponent143({ seed = 143, title = 'Module 143' }) {
  const hook = useMegaHook023(seed + 143);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (143 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer143, initialState143);

  const transformed = useMemo(() => pipelineData023(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState023(transformed, sortBy, sortDir, page, 11), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-143">
      <header className="mega-header-143">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-143">
        <div className="mega-pane-143">
          <Meter143 label="Average" value={summary.avg} ceiling={100} />
          <Meter143 label="Peak" value={summary.max} ceiling={130} />
          <Meter143 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter143 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock143 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-143">
          <form className="mega-form-143" onSubmit={submitLocal}>
