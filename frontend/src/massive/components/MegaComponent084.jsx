...
import '../styles/component084.css';

function Meter084({ label, value, ceiling }) {
...
function NestedBlock084({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-084">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-084"
          style={{
...
function reducer084(state, action) {
...
export default function MegaComponent084({ seed = 84, title = 'Module 84' }) {
  const hook = useMegaHook084(seed + 84);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (84 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer084, initialState084);
  ...
  return (
    <section className="mega-card-084">
      <header className="mega-header-084">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-084">
        <div className="mega-pane-084">
          <Meter084 label="Average" value={summary.avg} ceiling={100} />
          <Meter084 label="Peak" value={summary.max} ceiling={130} />
          <Meter084 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter084 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock084 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-084">
          <form className="mega-form-084" onSubmit={submitLocal}>
