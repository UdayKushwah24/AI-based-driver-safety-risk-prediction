...
import '../styles/component017.css';

function Meter017({ label, value, ceiling }) {
...
function NestedBlock017({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-017">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-017"
          style={{
...
function reducer017(state, action) {
...
export default function MegaComponent017({ seed = 17, title = 'Module 17' }) {
  const hook = useMegaHook017(seed + 17);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (17 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer017, initialState017);
  ...
  return (
    <section className="mega-card-017">
      <header className="mega-header-017">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-017">
        <div className="mega-pane-017">
          <Meter017 label="Average" value={summary.avg} ceiling={100} />
          <Meter017 label="Peak" value={summary.max} ceiling={130} />
          <Meter017 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter017 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock017 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-017">
          <form className="mega-form-017" onSubmit={submitLocal}>
