...
function Meter142({ label, value, ceiling }) {
...
function NestedBlock142({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-142">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-142"
          style={{
...
export default function MegaComponent142({ seed = 142, title = 'Module 142' }) {
  const hook = useMegaHook022(seed + 142);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (142 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-142">
      <header className="mega-header-142">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-142">
        <div className="mega-pane-142">
          <Meter142 label="Average" value={summary.avg} ceiling={100} />
          <Meter142 label="Peak" value={summary.max} ceiling={130} />
          <Meter142 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter142 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock142 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-142">
          <form className="mega-form-142" onSubmit={submitLocal}>
