...
import '../styles/component026.css';

function Meter026({ label, value, ceiling }) {
...
function NestedBlock026({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-026">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-026"
          style={{
...
function reducer026(state, action) {
...
export default function MegaComponent026({ seed = 26, title = 'Module 26' }) {
  const hook = useMegaHook026(seed + 26);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (26 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer026, initialState026);
  ...
  return (
    <section className="mega-card-026">
      <header className="mega-header-026">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-026">
        <div className="mega-pane-026">
          <Meter026 label="Average" value={summary.avg} ceiling={100} />
          <Meter026 label="Peak" value={summary.max} ceiling={130} />
          <Meter026 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter026 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock026 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-026">
          <form className="mega-form-026" onSubmit={submitLocal}>
