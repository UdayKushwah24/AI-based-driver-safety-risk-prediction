...
import { useMegaHook026 } from '../hooks/useMegaHook026.js';
import { buildTableState026, pipelineData026 } from '../utils/transformSet026.js';
import '../styles/component266.css';

function Meter266({ label, value, ceiling }) {
...
function NestedBlock266({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-266">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-266"
          style={{
...
export default function MegaComponent266({ seed = 266, title = 'Module 266' }) {
  const hook = useMegaHook026(seed + 266);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (266 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer266, initialState266);

  const transformed = useMemo(() => pipelineData026(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState026(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-266">
      <header className="mega-header-266">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-266">
        <div className="mega-pane-266">
          <Meter266 label="Average" value={summary.avg} ceiling={100} />
          <Meter266 label="Peak" value={summary.max} ceiling={130} />
          <Meter266 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter266 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock266 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-266">
          <form className="mega-form-266" onSubmit={submitLocal}>
