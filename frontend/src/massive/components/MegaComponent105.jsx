...
function Meter105({ label, value, ceiling }) {
...
function NestedBlock105({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-105">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-105"
          style={{
...
export default function MegaComponent105({ seed = 105, title = 'Module 105' }) {
  const hook = useMegaHook105(seed + 105);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (105 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-105">
      <header className="mega-header-105">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-105">
        <div className="mega-pane-105">
          <Meter105 label="Average" value={summary.avg} ceiling={100} />
          <Meter105 label="Peak" value={summary.max} ceiling={130} />
          <Meter105 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter105 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock105 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-105">
          <form className="mega-form-105" onSubmit={submitLocal}>
