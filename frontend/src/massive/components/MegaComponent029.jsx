...
import '../styles/component029.css';

function Meter029({ label, value, ceiling }) {
...
function NestedBlock029({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-029">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-029"
          style={{
...
function reducer029(state, action) {
...
export default function MegaComponent029({ seed = 29, title = 'Module 29' }) {
  const hook = useMegaHook029(seed + 29);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (29 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer029, initialState029);
  ...
  return (
    <section className="mega-card-029">
      <header className="mega-header-029">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-029">
        <div className="mega-pane-029">
          <Meter029 label="Average" value={summary.avg} ceiling={100} />
          <Meter029 label="Peak" value={summary.max} ceiling={130} />
          <Meter029 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter029 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock029 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-029">
          <form className="mega-form-029" onSubmit={submitLocal}>
