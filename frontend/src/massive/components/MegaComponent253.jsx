...
import { useMegaHook013 } from '../hooks/useMegaHook013.js';
import { buildTableState013, pipelineData013 } from '../utils/transformSet013.js';
import '../styles/component253.css';

function Meter253({ label, value, ceiling }) {
...
function NestedBlock253({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-253">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-253"
          style={{
...
export default function MegaComponent253({ seed = 253, title = 'Module 253' }) {
  const hook = useMegaHook013(seed + 253);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (253 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer253, initialState253);

  const transformed = useMemo(() => pipelineData013(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState013(transformed, sortBy, sortDir, page, 11), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-253">
      <header className="mega-header-253">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-253">
        <div className="mega-pane-253">
          <Meter253 label="Average" value={summary.avg} ceiling={100} />
          <Meter253 label="Peak" value={summary.max} ceiling={130} />
          <Meter253 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter253 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock253 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-253">
          <form className="mega-form-253" onSubmit={submitLocal}>
