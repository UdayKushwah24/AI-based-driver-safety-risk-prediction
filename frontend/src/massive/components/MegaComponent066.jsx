...
import '../styles/component066.css';

function Meter066({ label, value, ceiling }) {
...
function NestedBlock066({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-066">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-066"
          style={{
...
function reducer066(state, action) {
...
export default function MegaComponent066({ seed = 66, title = 'Module 66' }) {
  const hook = useMegaHook066(seed + 66);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (66 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer066, initialState066);
  ...
  return (
    <section className="mega-card-066">
      <header className="mega-header-066">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-066">
        <div className="mega-pane-066">
          <Meter066 label="Average" value={summary.avg} ceiling={100} />
          <Meter066 label="Peak" value={summary.max} ceiling={130} />
          <Meter066 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter066 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock066 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-066">
          <form className="mega-form-066" onSubmit={submitLocal}>
