...
import { useMegaHook067 } from '../hooks/useMegaHook067.js';
import { buildTableState067, pipelineData067 } from '../utils/transformSet067.js';
import '../styles/component187.css';

function Meter187({ label, value, ceiling }) {
...
function NestedBlock187({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-187">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-187"
          style={{
...
export default function MegaComponent187({ seed = 187, title = 'Module 187' }) {
  const hook = useMegaHook067(seed + 187);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (187 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer187, initialState187);

  const transformed = useMemo(() => pipelineData067(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState067(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-187">
      <header className="mega-header-187">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-187">
        <div className="mega-pane-187">
          <Meter187 label="Average" value={summary.avg} ceiling={100} />
          <Meter187 label="Peak" value={summary.max} ceiling={130} />
          <Meter187 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter187 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock187 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-187">
          <form className="mega-form-187" onSubmit={submitLocal}>
