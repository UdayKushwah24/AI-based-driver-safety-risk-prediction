...
import { useMegaHook091 } from '../hooks/useMegaHook091.js';
import { buildTableState091, pipelineData091 } from '../utils/transformSet091.js';
import '../styles/component211.css';

function Meter211({ label, value, ceiling }) {
...
function NestedBlock211({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-211">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-211"
          style={{
...
export default function MegaComponent211({ seed = 211, title = 'Module 211' }) {
  const hook = useMegaHook091(seed + 211);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (211 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer211, initialState211);

  const transformed = useMemo(() => pipelineData091(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState091(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-211">
      <header className="mega-header-211">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-211">
        <div className="mega-pane-211">
          <Meter211 label="Average" value={summary.avg} ceiling={100} />
          <Meter211 label="Peak" value={summary.max} ceiling={130} />
          <Meter211 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter211 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock211 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-211">
          <form className="mega-form-211" onSubmit={submitLocal}>
