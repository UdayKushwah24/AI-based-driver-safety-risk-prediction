...
function Meter108({ label, value, ceiling }) {
...
function NestedBlock108({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-108">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-108"
          style={{
...
export default function MegaComponent108({ seed = 108, title = 'Module 108' }) {
  const hook = useMegaHook108(seed + 108);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (108 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-108">
      <header className="mega-header-108">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-108">
        <div className="mega-pane-108">
          <Meter108 label="Average" value={summary.avg} ceiling={100} />
          <Meter108 label="Peak" value={summary.max} ceiling={130} />
          <Meter108 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter108 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock108 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-108">
          <form className="mega-form-108" onSubmit={submitLocal}>
