...
import { useMegaHook078 } from '../hooks/useMegaHook078.js';
import { buildTableState078, pipelineData078 } from '../utils/transformSet078.js';
import '../styles/component198.css';

function Meter198({ label, value, ceiling }) {
...
function NestedBlock198({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-198">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-198"
          style={{
...
export default function MegaComponent198({ seed = 198, title = 'Module 198' }) {
  const hook = useMegaHook078(seed + 198);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (198 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer198, initialState198);

  const transformed = useMemo(() => pipelineData078(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState078(transformed, sortBy, sortDir, page, 11), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-198">
      <header className="mega-header-198">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-198">
        <div className="mega-pane-198">
          <Meter198 label="Average" value={summary.avg} ceiling={100} />
          <Meter198 label="Peak" value={summary.max} ceiling={130} />
          <Meter198 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter198 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock198 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-198">
          <form className="mega-form-198" onSubmit={submitLocal}>
