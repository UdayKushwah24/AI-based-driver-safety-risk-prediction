...
import { useMegaHook022 } from '../hooks/useMegaHook022.js';
import { buildTableState022, pipelineData022 } from '../utils/transformSet022.js';
import '../styles/component262.css';

function Meter262({ label, value, ceiling }) {
...
function NestedBlock262({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-262">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-262"
          style={{
...
export default function MegaComponent262({ seed = 262, title = 'Module 262' }) {
  const hook = useMegaHook022(seed + 262);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (262 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer262, initialState262);

  const transformed = useMemo(() => pipelineData022(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState022(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-262">
      <header className="mega-header-262">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-262">
        <div className="mega-pane-262">
          <Meter262 label="Average" value={summary.avg} ceiling={100} />
          <Meter262 label="Peak" value={summary.max} ceiling={130} />
          <Meter262 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter262 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock262 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-262">
          <form className="mega-form-262" onSubmit={submitLocal}>
