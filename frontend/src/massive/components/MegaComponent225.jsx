...
import { useMegaHook105 } from '../hooks/useMegaHook105.js';
import { buildTableState105, pipelineData105 } from '../utils/transformSet105.js';
import '../styles/component225.css';

function Meter225({ label, value, ceiling }) {
...
function NestedBlock225({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-225">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-225"
          style={{
...
export default function MegaComponent225({ seed = 225, title = 'Module 225' }) {
  const hook = useMegaHook105(seed + 225);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (225 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer225, initialState225);

  const transformed = useMemo(() => pipelineData105(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState105(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-225">
      <header className="mega-header-225">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-225">
        <div className="mega-pane-225">
          <Meter225 label="Average" value={summary.avg} ceiling={100} />
          <Meter225 label="Peak" value={summary.max} ceiling={130} />
          <Meter225 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter225 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock225 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-225">
          <form className="mega-form-225" onSubmit={submitLocal}>
