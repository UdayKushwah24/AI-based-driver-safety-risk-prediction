...
function Meter100({ label, value, ceiling }) {
...
function NestedBlock100({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-100">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-100"
          style={{
...
export default function MegaComponent100({ seed = 100, title = 'Module 100' }) {
  const hook = useMegaHook100(seed + 100);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (100 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-100">
      <header className="mega-header-100">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-100">
        <div className="mega-pane-100">
          <Meter100 label="Average" value={summary.avg} ceiling={100} />
          <Meter100 label="Peak" value={summary.max} ceiling={130} />
          <Meter100 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter100 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock100 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-100">
          <form className="mega-form-100" onSubmit={submitLocal}>
