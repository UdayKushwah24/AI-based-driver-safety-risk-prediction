...
import { useMegaHook060 } from '../hooks/useMegaHook060.js';
import { buildTableState060, pipelineData060 } from '../utils/transformSet060.js';
import '../styles/component300.css';

function Meter300({ label, value, ceiling }) {
...
function NestedBlock300({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-300">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-300"
          style={{
...
export default function MegaComponent300({ seed = 300, title = 'Module 300' }) {
  const hook = useMegaHook060(seed + 300);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (300 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer300, initialState300);

  const transformed = useMemo(() => pipelineData060(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState060(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-300">
      <header className="mega-header-300">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-300">
        <div className="mega-pane-300">
          <Meter300 label="Average" value={summary.avg} ceiling={100} />
          <Meter300 label="Peak" value={summary.max} ceiling={130} />
          <Meter300 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter300 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock300 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-300">
          <form className="mega-form-300" onSubmit={submitLocal}>
