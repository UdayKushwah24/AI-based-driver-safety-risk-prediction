...
import { useMegaHook003 } from '../hooks/useMegaHook003.js';
import { buildTableState003, pipelineData003 } from '../utils/transformSet003.js';
import '../styles/component003.css';

function Meter003({ label, value, ceiling }) {
...
function NestedBlock003({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-003">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-003"
          style={{
...
function reducer003(state, action) {
...
export default function MegaComponent003({ seed = 3, title = 'Module 3' }) {
  const hook = useMegaHook003(seed + 3);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (3 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer003, initialState003);

  const transformed = useMemo(() => pipelineData003(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState003(transformed, sortBy, sortDir, page, 11), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-003">
      <header className="mega-header-003">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-003">
        <div className="mega-pane-003">
          <Meter003 label="Average" value={summary.avg} ceiling={100} />
          <Meter003 label="Peak" value={summary.max} ceiling={130} />
          <Meter003 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter003 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock003 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-003">
          <form className="mega-form-003" onSubmit={submitLocal}>
