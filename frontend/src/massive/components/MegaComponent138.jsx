...
function Meter138({ label, value, ceiling }) {
...
function NestedBlock138({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-138">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-138"
          style={{
...
export default function MegaComponent138({ seed = 138, title = 'Module 138' }) {
  const hook = useMegaHook018(seed + 138);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (138 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-138">
      <header className="mega-header-138">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-138">
        <div className="mega-pane-138">
          <Meter138 label="Average" value={summary.avg} ceiling={100} />
          <Meter138 label="Peak" value={summary.max} ceiling={130} />
          <Meter138 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter138 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock138 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-138">
          <form className="mega-form-138" onSubmit={submitLocal}>
