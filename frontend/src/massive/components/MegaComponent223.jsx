...
import { useMegaHook103 } from '../hooks/useMegaHook103.js';
import { buildTableState103, pipelineData103 } from '../utils/transformSet103.js';
import '../styles/component223.css';

function Meter223({ label, value, ceiling }) {
...
function NestedBlock223({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-223">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-223"
          style={{
...
export default function MegaComponent223({ seed = 223, title = 'Module 223' }) {
  const hook = useMegaHook103(seed + 223);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (223 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer223, initialState223);

  const transformed = useMemo(() => pipelineData103(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState103(transformed, sortBy, sortDir, page, 11), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-223">
      <header className="mega-header-223">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-223">
        <div className="mega-pane-223">
          <Meter223 label="Average" value={summary.avg} ceiling={100} />
          <Meter223 label="Peak" value={summary.max} ceiling={130} />
          <Meter223 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter223 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock223 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-223">
          <form className="mega-form-223" onSubmit={submitLocal}>
