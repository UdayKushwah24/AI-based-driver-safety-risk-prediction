...
import '../styles/component030.css';

function Meter030({ label, value, ceiling }) {
...
function NestedBlock030({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-030">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-030"
          style={{
...
function reducer030(state, action) {
...
export default function MegaComponent030({ seed = 30, title = 'Module 30' }) {
  const hook = useMegaHook030(seed + 30);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (30 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer030, initialState030);
  ...
  return (
    <section className="mega-card-030">
      <header className="mega-header-030">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-030">
        <div className="mega-pane-030">
          <Meter030 label="Average" value={summary.avg} ceiling={100} />
          <Meter030 label="Peak" value={summary.max} ceiling={130} />
          <Meter030 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter030 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock030 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-030">
          <form className="mega-form-030" onSubmit={submitLocal}>
