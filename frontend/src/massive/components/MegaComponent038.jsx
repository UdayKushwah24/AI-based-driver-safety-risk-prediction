...
import '../styles/component038.css';

function Meter038({ label, value, ceiling }) {
...
function NestedBlock038({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-038">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-038"
          style={{
...
function reducer038(state, action) {
...
export default function MegaComponent038({ seed = 38, title = 'Module 38' }) {
  const hook = useMegaHook038(seed + 38);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (38 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer038, initialState038);
  ...
  return (
    <section className="mega-card-038">
      <header className="mega-header-038">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-038">
        <div className="mega-pane-038">
          <Meter038 label="Average" value={summary.avg} ceiling={100} />
          <Meter038 label="Peak" value={summary.max} ceiling={130} />
          <Meter038 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter038 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock038 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-038">
          <form className="mega-form-038" onSubmit={submitLocal}>
