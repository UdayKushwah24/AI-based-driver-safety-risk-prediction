...
import { useMegaHook029 } from '../hooks/useMegaHook029.js';
import { buildTableState029, pipelineData029 } from '../utils/transformSet029.js';
import '../styles/component269.css';

function Meter269({ label, value, ceiling }) {
...
function NestedBlock269({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-269">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-269"
          style={{
...
export default function MegaComponent269({ seed = 269, title = 'Module 269' }) {
  const hook = useMegaHook029(seed + 269);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (269 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer269, initialState269);

  const transformed = useMemo(() => pipelineData029(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState029(transformed, sortBy, sortDir, page, 12), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-269">
      <header className="mega-header-269">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-269">
        <div className="mega-pane-269">
          <Meter269 label="Average" value={summary.avg} ceiling={100} />
          <Meter269 label="Peak" value={summary.max} ceiling={130} />
          <Meter269 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter269 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock269 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-269">
          <form className="mega-form-269" onSubmit={submitLocal}>
