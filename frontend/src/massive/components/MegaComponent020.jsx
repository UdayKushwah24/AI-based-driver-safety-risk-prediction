...
import '../styles/component020.css';

function Meter020({ label, value, ceiling }) {
...
function NestedBlock020({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-020">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-020"
          style={{
...
function reducer020(state, action) {
...
export default function MegaComponent020({ seed = 20, title = 'Module 20' }) {
  const hook = useMegaHook020(seed + 20);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (20 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer020, initialState020);
  ...
  return (
    <section className="mega-card-020">
      <header className="mega-header-020">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-020">
        <div className="mega-pane-020">
          <Meter020 label="Average" value={summary.avg} ceiling={100} />
          <Meter020 label="Peak" value={summary.max} ceiling={130} />
          <Meter020 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter020 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock020 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-020">
          <form className="mega-form-020" onSubmit={submitLocal}>
