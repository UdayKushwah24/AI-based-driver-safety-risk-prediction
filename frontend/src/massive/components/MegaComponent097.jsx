...
import '../styles/component097.css';

function Meter097({ label, value, ceiling }) {
...
function NestedBlock097({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-097">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-097"
          style={{
...
function reducer097(state, action) {
...
export default function MegaComponent097({ seed = 97, title = 'Module 97' }) {
  const hook = useMegaHook097(seed + 97);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (97 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer097, initialState097);
  ...
  return (
    <section className="mega-card-097">
      <header className="mega-header-097">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-097">
        <div className="mega-pane-097">
          <Meter097 label="Average" value={summary.avg} ceiling={100} />
          <Meter097 label="Peak" value={summary.max} ceiling={130} />
          <Meter097 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter097 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock097 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-097">
          <form className="mega-form-097" onSubmit={submitLocal}>
