...
function Meter141({ label, value, ceiling }) {
...
function NestedBlock141({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-141">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-141"
          style={{
...
export default function MegaComponent141({ seed = 141, title = 'Module 141' }) {
  const hook = useMegaHook021(seed + 141);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (141 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-141">
      <header className="mega-header-141">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-141">
        <div className="mega-pane-141">
          <Meter141 label="Average" value={summary.avg} ceiling={100} />
          <Meter141 label="Peak" value={summary.max} ceiling={130} />
          <Meter141 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter141 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock141 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-141">
          <form className="mega-form-141" onSubmit={submitLocal}>
