...
function Meter260({ label, value, ceiling }) {
...
function NestedBlock260({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-260">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-260"
          style={{
...
export default function MegaComponent260({ seed = 260, title = 'Module 260' }) {
  const hook = useMegaHook020(seed + 260);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (260 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-260">
      <header className="mega-header-260">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-260">
        <div className="mega-pane-260">
          <Meter260 label="Average" value={summary.avg} ceiling={100} />
          <Meter260 label="Peak" value={summary.max} ceiling={130} />
          <Meter260 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter260 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock260 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-260">
          <form className="mega-form-260" onSubmit={submitLocal}>
