...
function Meter296({ label, value, ceiling }) {
...
function NestedBlock296({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-296">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-296"
          style={{
...
export default function MegaComponent296({ seed = 296, title = 'Module 296' }) {
  const hook = useMegaHook056(seed + 296);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (296 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-296">
      <header className="mega-header-296">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-296">
        <div className="mega-pane-296">
          <Meter296 label="Average" value={summary.avg} ceiling={100} />
          <Meter296 label="Peak" value={summary.max} ceiling={130} />
          <Meter296 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter296 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock296 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-296">
          <form className="mega-form-296" onSubmit={submitLocal}>
