...
import '../styles/component093.css';

function Meter093({ label, value, ceiling }) {
...
function NestedBlock093({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-093">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-093"
          style={{
...
function reducer093(state, action) {
...
export default function MegaComponent093({ seed = 93, title = 'Module 93' }) {
  const hook = useMegaHook093(seed + 93);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (93 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer093, initialState093);
  ...
  return (
    <section className="mega-card-093">
      <header className="mega-header-093">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-093">
        <div className="mega-pane-093">
          <Meter093 label="Average" value={summary.avg} ceiling={100} />
          <Meter093 label="Peak" value={summary.max} ceiling={130} />
          <Meter093 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter093 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock093 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-093">
          <form className="mega-form-093" onSubmit={submitLocal}>
