...
import '../styles/component001.css';

function Meter001({ label, value, ceiling }) {
...
function NestedBlock001({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-001">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-001"
          style={{
...
function reducer001(state, action) {
...
export default function MegaComponent001({ seed = 1, title = 'Module 1' }) {
  const hook = useMegaHook001(seed + 1);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (1 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer001, initialState001);
  ...
  return (
    <section className="mega-card-001">
      <header className="mega-header-001">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-001">
        <div className="mega-pane-001">
          <Meter001 label="Average" value={summary.avg} ceiling={100} />
          <Meter001 label="Peak" value={summary.max} ceiling={130} />
          <Meter001 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter001 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock001 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-001">
          <form className="mega-form-001" onSubmit={submitLocal}>
