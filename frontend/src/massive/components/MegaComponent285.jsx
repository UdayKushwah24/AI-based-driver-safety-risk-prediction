...
function Meter285({ label, value, ceiling }) {
...
function NestedBlock285({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-285">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-285"
          style={{
...
export default function MegaComponent285({ seed = 285, title = 'Module 285' }) {
  const hook = useMegaHook045(seed + 285);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (285 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-285">
      <header className="mega-header-285">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-285">
        <div className="mega-pane-285">
          <Meter285 label="Average" value={summary.avg} ceiling={100} />
          <Meter285 label="Peak" value={summary.max} ceiling={130} />
          <Meter285 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter285 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock285 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-285">
          <form className="mega-form-285" onSubmit={submitLocal}>
