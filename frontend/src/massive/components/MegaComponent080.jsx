...
import '../styles/component080.css';

function Meter080({ label, value, ceiling }) {
...
function NestedBlock080({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-080">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-080"
          style={{
...
export default function MegaComponent080({ seed = 80, title = 'Module 80' }) {
  const hook = useMegaHook080(seed + 80);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (80 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-080">
      <header className="mega-header-080">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-080">
        <div className="mega-pane-080">
          <Meter080 label="Average" value={summary.avg} ceiling={100} />
          <Meter080 label="Peak" value={summary.max} ceiling={130} />
          <Meter080 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter080 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock080 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-080">
          <form className="mega-form-080" onSubmit={submitLocal}>
