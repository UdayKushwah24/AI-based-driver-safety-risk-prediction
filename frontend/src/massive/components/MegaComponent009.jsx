...
import '../styles/component009.css';

function Meter009({ label, value, ceiling }) {
...
function NestedBlock009({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-009">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-009"
          style={{
...
function reducer009(state, action) {
...
export default function MegaComponent009({ seed = 9, title = 'Module 9' }) {
  const hook = useMegaHook009(seed + 9);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (9 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer009, initialState009);
  ...
  return (
    <section className="mega-card-009">
      <header className="mega-header-009">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-009">
        <div className="mega-pane-009">
          <Meter009 label="Average" value={summary.avg} ceiling={100} />
          <Meter009 label="Peak" value={summary.max} ceiling={130} />
          <Meter009 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter009 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock009 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-009">
          <form className="mega-form-009" onSubmit={submitLocal}>
