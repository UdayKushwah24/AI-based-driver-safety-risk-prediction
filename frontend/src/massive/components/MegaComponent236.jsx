...
function Meter236({ label, value, ceiling }) {
...
function NestedBlock236({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-236">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-236"
          style={{
...
export default function MegaComponent236({ seed = 236, title = 'Module 236' }) {
  const hook = useMegaHook116(seed + 236);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (236 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-236">
      <header className="mega-header-236">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-236">
        <div className="mega-pane-236">
          <Meter236 label="Average" value={summary.avg} ceiling={100} />
          <Meter236 label="Peak" value={summary.max} ceiling={130} />
          <Meter236 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter236 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock236 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-236">
          <form className="mega-form-236" onSubmit={submitLocal}>
