...
import { useMegaHook064 } from '../hooks/useMegaHook064.js';
import { buildTableState064, pipelineData064 } from '../utils/transformSet064.js';
import '../styles/component184.css';

function Meter184({ label, value, ceiling }) {
...
function NestedBlock184({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-184">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-184"
          style={{
...
export default function MegaComponent184({ seed = 184, title = 'Module 184' }) {
  const hook = useMegaHook064(seed + 184);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (184 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer184, initialState184);

  const transformed = useMemo(() => pipelineData064(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState064(transformed, sortBy, sortDir, page, 12), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-184">
      <header className="mega-header-184">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-184">
        <div className="mega-pane-184">
          <Meter184 label="Average" value={summary.avg} ceiling={100} />
          <Meter184 label="Peak" value={summary.max} ceiling={130} />
          <Meter184 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter184 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock184 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-184">
          <form className="mega-form-184" onSubmit={submitLocal}>
