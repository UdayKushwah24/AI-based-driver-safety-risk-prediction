...
import '../styles/component081.css';

function Meter081({ label, value, ceiling }) {
...
function NestedBlock081({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-081">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-081"
          style={{
...
function reducer081(state, action) {
...
export default function MegaComponent081({ seed = 81, title = 'Module 81' }) {
  const hook = useMegaHook081(seed + 81);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (81 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer081, initialState081);
  ...
  return (
    <section className="mega-card-081">
      <header className="mega-header-081">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-081">
        <div className="mega-pane-081">
          <Meter081 label="Average" value={summary.avg} ceiling={100} />
          <Meter081 label="Peak" value={summary.max} ceiling={130} />
          <Meter081 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter081 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock081 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-081">
          <form className="mega-form-081" onSubmit={submitLocal}>
