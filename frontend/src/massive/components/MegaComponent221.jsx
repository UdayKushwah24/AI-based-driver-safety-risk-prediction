...
import { useMegaHook101 } from '../hooks/useMegaHook101.js';
import { buildTableState101, pipelineData101 } from '../utils/transformSet101.js';
import '../styles/component221.css';

function Meter221({ label, value, ceiling }) {
...
function NestedBlock221({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-221">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-221"
          style={{
...
export default function MegaComponent221({ seed = 221, title = 'Module 221' }) {
  const hook = useMegaHook101(seed + 221);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (221 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer221, initialState221);

  const transformed = useMemo(() => pipelineData101(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState101(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-221">
      <header className="mega-header-221">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-221">
        <div className="mega-pane-221">
          <Meter221 label="Average" value={summary.avg} ceiling={100} />
          <Meter221 label="Peak" value={summary.max} ceiling={130} />
          <Meter221 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter221 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock221 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-221">
          <form className="mega-form-221" onSubmit={submitLocal}>
