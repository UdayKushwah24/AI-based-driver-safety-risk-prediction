...
import { useMegaHook036 } from '../hooks/useMegaHook036.js';
import { buildTableState036, pipelineData036 } from '../utils/transformSet036.js';
import '../styles/component156.css';

function Meter156({ label, value, ceiling }) {
...
function NestedBlock156({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-156">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-156"
          style={{
...
export default function MegaComponent156({ seed = 156, title = 'Module 156' }) {
  const hook = useMegaHook036(seed + 156);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (156 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer156, initialState156);

  const transformed = useMemo(() => pipelineData036(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState036(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-156">
      <header className="mega-header-156">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-156">
        <div className="mega-pane-156">
          <Meter156 label="Average" value={summary.avg} ceiling={100} />
          <Meter156 label="Peak" value={summary.max} ceiling={130} />
          <Meter156 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter156 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock156 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-156">
          <form className="mega-form-156" onSubmit={submitLocal}>
