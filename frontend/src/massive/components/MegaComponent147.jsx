...
import { useMegaHook027 } from '../hooks/useMegaHook027.js';
import { buildTableState027, pipelineData027 } from '../utils/transformSet027.js';
import '../styles/component147.css';

function Meter147({ label, value, ceiling }) {
...
function NestedBlock147({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-147">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-147"
          style={{
...
export default function MegaComponent147({ seed = 147, title = 'Module 147' }) {
  const hook = useMegaHook027(seed + 147);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (147 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer147, initialState147);

  const transformed = useMemo(() => pipelineData027(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState027(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-147">
      <header className="mega-header-147">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-147">
        <div className="mega-pane-147">
          <Meter147 label="Average" value={summary.avg} ceiling={100} />
          <Meter147 label="Peak" value={summary.max} ceiling={130} />
          <Meter147 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter147 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock147 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-147">
          <form className="mega-form-147" onSubmit={submitLocal}>
