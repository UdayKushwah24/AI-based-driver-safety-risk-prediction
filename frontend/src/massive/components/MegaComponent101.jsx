...
function Meter101({ label, value, ceiling }) {
...
function NestedBlock101({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-101">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-101"
          style={{
...
export default function MegaComponent101({ seed = 101, title = 'Module 101' }) {
  const hook = useMegaHook101(seed + 101);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (101 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-101">
      <header className="mega-header-101">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-101">
        <div className="mega-pane-101">
          <Meter101 label="Average" value={summary.avg} ceiling={100} />
          <Meter101 label="Peak" value={summary.max} ceiling={130} />
          <Meter101 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter101 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock101 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-101">
          <form className="mega-form-101" onSubmit={submitLocal}>
