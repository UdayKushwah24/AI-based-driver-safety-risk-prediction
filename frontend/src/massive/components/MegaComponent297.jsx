...
import { useMegaHook057 } from '../hooks/useMegaHook057.js';
import { buildTableState057, pipelineData057 } from '../utils/transformSet057.js';
import '../styles/component297.css';

function Meter297({ label, value, ceiling }) {
...
function NestedBlock297({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-297">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-297"
          style={{
...
export default function MegaComponent297({ seed = 297, title = 'Module 297' }) {
  const hook = useMegaHook057(seed + 297);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (297 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer297, initialState297);

  const transformed = useMemo(() => pipelineData057(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState057(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-297">
      <header className="mega-header-297">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-297">
        <div className="mega-pane-297">
          <Meter297 label="Average" value={summary.avg} ceiling={100} />
          <Meter297 label="Peak" value={summary.max} ceiling={130} />
          <Meter297 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter297 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock297 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-297">
          <form className="mega-form-297" onSubmit={submitLocal}>
