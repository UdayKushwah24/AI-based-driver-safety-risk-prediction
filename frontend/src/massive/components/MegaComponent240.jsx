...
import { useMegaHook120 } from '../hooks/useMegaHook120.js';
import { buildTableState120, pipelineData120 } from '../utils/transformSet120.js';
import '../styles/component240.css';

function Meter240({ label, value, ceiling }) {
...
function NestedBlock240({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-240">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-240"
          style={{
...
export default function MegaComponent240({ seed = 240, title = 'Module 240' }) {
  const hook = useMegaHook120(seed + 240);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (240 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer240, initialState240);

  const transformed = useMemo(() => pipelineData120(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState120(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-240">
      <header className="mega-header-240">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-240">
        <div className="mega-pane-240">
          <Meter240 label="Average" value={summary.avg} ceiling={100} />
          <Meter240 label="Peak" value={summary.max} ceiling={130} />
          <Meter240 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter240 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock240 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-240">
          <form className="mega-form-240" onSubmit={submitLocal}>
