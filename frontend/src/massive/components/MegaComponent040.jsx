...
import '../styles/component040.css';

function Meter040({ label, value, ceiling }) {
...
function NestedBlock040({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-040">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-040"
          style={{
...
function reducer040(state, action) {
...
export default function MegaComponent040({ seed = 40, title = 'Module 40' }) {
  const hook = useMegaHook040(seed + 40);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (40 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer040, initialState040);
  ...
  return (
    <section className="mega-card-040">
      <header className="mega-header-040">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-040">
        <div className="mega-pane-040">
          <Meter040 label="Average" value={summary.avg} ceiling={100} />
          <Meter040 label="Peak" value={summary.max} ceiling={130} />
          <Meter040 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter040 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock040 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-040">
          <form className="mega-form-040" onSubmit={submitLocal}>
