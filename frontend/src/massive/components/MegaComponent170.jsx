...
function Meter170({ label, value, ceiling }) {
...
function NestedBlock170({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-170">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-170"
          style={{
...
export default function MegaComponent170({ seed = 170, title = 'Module 170' }) {
  const hook = useMegaHook050(seed + 170);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (170 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-170">
      <header className="mega-header-170">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-170">
        <div className="mega-pane-170">
          <Meter170 label="Average" value={summary.avg} ceiling={100} />
          <Meter170 label="Peak" value={summary.max} ceiling={130} />
          <Meter170 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter170 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock170 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-170">
          <form className="mega-form-170" onSubmit={submitLocal}>
