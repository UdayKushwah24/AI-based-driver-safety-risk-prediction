...
function Meter270({ label, value, ceiling }) {
...
function NestedBlock270({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-270">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-270"
          style={{
...
export default function MegaComponent270({ seed = 270, title = 'Module 270' }) {
  const hook = useMegaHook030(seed + 270);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (270 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-270">
      <header className="mega-header-270">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-270">
        <div className="mega-pane-270">
          <Meter270 label="Average" value={summary.avg} ceiling={100} />
          <Meter270 label="Peak" value={summary.max} ceiling={130} />
          <Meter270 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter270 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock270 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-270">
          <form className="mega-form-270" onSubmit={submitLocal}>
