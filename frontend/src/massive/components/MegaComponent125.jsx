...
function Meter125({ label, value, ceiling }) {
...
function NestedBlock125({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-125">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-125"
          style={{
...
export default function MegaComponent125({ seed = 125, title = 'Module 125' }) {
  const hook = useMegaHook005(seed + 125);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (125 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-125">
      <header className="mega-header-125">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-125">
        <div className="mega-pane-125">
          <Meter125 label="Average" value={summary.avg} ceiling={100} />
          <Meter125 label="Peak" value={summary.max} ceiling={130} />
          <Meter125 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter125 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock125 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-125">
          <form className="mega-form-125" onSubmit={submitLocal}>
