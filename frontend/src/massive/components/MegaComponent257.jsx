...
import { useMegaHook017 } from '../hooks/useMegaHook017.js';
import { buildTableState017, pipelineData017 } from '../utils/transformSet017.js';
import '../styles/component257.css';

function Meter257({ label, value, ceiling }) {
...
function NestedBlock257({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-257">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-257"
          style={{
...
export default function MegaComponent257({ seed = 257, title = 'Module 257' }) {
  const hook = useMegaHook017(seed + 257);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (257 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer257, initialState257);

  const transformed = useMemo(() => pipelineData017(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState017(transformed, sortBy, sortDir, page, 10), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-257">
      <header className="mega-header-257">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-257">
        <div className="mega-pane-257">
          <Meter257 label="Average" value={summary.avg} ceiling={100} />
          <Meter257 label="Peak" value={summary.max} ceiling={130} />
          <Meter257 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter257 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock257 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-257">
          <form className="mega-form-257" onSubmit={submitLocal}>
