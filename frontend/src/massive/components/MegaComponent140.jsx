...
import { useMegaHook020 } from '../hooks/useMegaHook020.js';
import { buildTableState020, pipelineData020 } from '../utils/transformSet020.js';
import '../styles/component140.css';

function Meter140({ label, value, ceiling }) {
...
function NestedBlock140({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-140">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-140"
          style={{
...
export default function MegaComponent140({ seed = 140, title = 'Module 140' }) {
  const hook = useMegaHook020(seed + 140);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (140 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer140, initialState140);

  const transformed = useMemo(() => pipelineData020(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState020(transformed, sortBy, sortDir, page, 8), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-140">
      <header className="mega-header-140">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-140">
        <div className="mega-pane-140">
          <Meter140 label="Average" value={summary.avg} ceiling={100} />
          <Meter140 label="Peak" value={summary.max} ceiling={130} />
          <Meter140 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter140 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock140 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-140">
          <form className="mega-form-140" onSubmit={submitLocal}>
