...
import '../styles/component077.css';

function Meter077({ label, value, ceiling }) {
...
function NestedBlock077({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-077">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-077"
          style={{
...
function reducer077(state, action) {
...
export default function MegaComponent077({ seed = 77, title = 'Module 77' }) {
  const hook = useMegaHook077(seed + 77);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (77 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer077, initialState077);
  ...
  return (
    <section className="mega-card-077">
      <header className="mega-header-077">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-077">
        <div className="mega-pane-077">
          <Meter077 label="Average" value={summary.avg} ceiling={100} />
          <Meter077 label="Peak" value={summary.max} ceiling={130} />
          <Meter077 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter077 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock077 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-077">
          <form className="mega-form-077" onSubmit={submitLocal}>
