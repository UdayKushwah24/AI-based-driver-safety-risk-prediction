...
import '../styles/component052.css';

function Meter052({ label, value, ceiling }) {
...
function NestedBlock052({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-052">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-052"
          style={{
...
function reducer052(state, action) {
...
export default function MegaComponent052({ seed = 52, title = 'Module 52' }) {
  const hook = useMegaHook052(seed + 52);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (52 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer052, initialState052);
  ...
  return (
    <section className="mega-card-052">
      <header className="mega-header-052">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-052">
        <div className="mega-pane-052">
          <Meter052 label="Average" value={summary.avg} ceiling={100} />
          <Meter052 label="Peak" value={summary.max} ceiling={130} />
          <Meter052 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter052 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock052 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-052">
          <form className="mega-form-052" onSubmit={submitLocal}>
