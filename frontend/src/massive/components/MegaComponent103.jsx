...
function Meter103({ label, value, ceiling }) {
...
function NestedBlock103({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-103">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-103"
          style={{
...
export default function MegaComponent103({ seed = 103, title = 'Module 103' }) {
  const hook = useMegaHook103(seed + 103);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (103 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-103">
      <header className="mega-header-103">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-103">
        <div className="mega-pane-103">
          <Meter103 label="Average" value={summary.avg} ceiling={100} />
          <Meter103 label="Peak" value={summary.max} ceiling={130} />
          <Meter103 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter103 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock103 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-103">
          <form className="mega-form-103" onSubmit={submitLocal}>
