...
import '../styles/component021.css';

function Meter021({ label, value, ceiling }) {
...
function NestedBlock021({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-021">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-021"
          style={{
...
export default function MegaComponent021({ seed = 21, title = 'Module 21' }) {
  const hook = useMegaHook021(seed + 21);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (21 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-021">
      <header className="mega-header-021">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-021">
        <div className="mega-pane-021">
          <Meter021 label="Average" value={summary.avg} ceiling={100} />
          <Meter021 label="Peak" value={summary.max} ceiling={130} />
          <Meter021 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter021 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock021 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-021">
          <form className="mega-form-021" onSubmit={submitLocal}>
