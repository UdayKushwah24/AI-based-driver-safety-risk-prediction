...
import '../styles/component045.css';

function Meter045({ label, value, ceiling }) {
...
function NestedBlock045({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-045">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-045"
          style={{
...
function reducer045(state, action) {
...
export default function MegaComponent045({ seed = 45, title = 'Module 45' }) {
  const hook = useMegaHook045(seed + 45);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (45 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer045, initialState045);
  ...
  return (
    <section className="mega-card-045">
      <header className="mega-header-045">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-045">
        <div className="mega-pane-045">
          <Meter045 label="Average" value={summary.avg} ceiling={100} />
          <Meter045 label="Peak" value={summary.max} ceiling={130} />
          <Meter045 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter045 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock045 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-045">
          <form className="mega-form-045" onSubmit={submitLocal}>
