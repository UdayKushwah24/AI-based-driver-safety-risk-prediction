...
function Meter123({ label, value, ceiling }) {
...
function NestedBlock123({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-123">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-123"
          style={{
...
export default function MegaComponent123({ seed = 123, title = 'Module 123' }) {
  const hook = useMegaHook003(seed + 123);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (123 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-123">
      <header className="mega-header-123">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-123">
        <div className="mega-pane-123">
          <Meter123 label="Average" value={summary.avg} ceiling={100} />
          <Meter123 label="Peak" value={summary.max} ceiling={130} />
          <Meter123 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter123 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock123 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-123">
          <form className="mega-form-123" onSubmit={submitLocal}>
