...
import { useMegaHook004 } from '../hooks/useMegaHook004.js';
import { buildTableState004, pipelineData004 } from '../utils/transformSet004.js';
import '../styles/component244.css';

function Meter244({ label, value, ceiling }) {
...
function NestedBlock244({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-244">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-244"
          style={{
...
export default function MegaComponent244({ seed = 244, title = 'Module 244' }) {
  const hook = useMegaHook004(seed + 244);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (244 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer244, initialState244);

  const transformed = useMemo(() => pipelineData004(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState004(transformed, sortBy, sortDir, page, 12), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-244">
      <header className="mega-header-244">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-244">
        <div className="mega-pane-244">
          <Meter244 label="Average" value={summary.avg} ceiling={100} />
          <Meter244 label="Peak" value={summary.max} ceiling={130} />
          <Meter244 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter244 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock244 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-244">
          <form className="mega-form-244" onSubmit={submitLocal}>
