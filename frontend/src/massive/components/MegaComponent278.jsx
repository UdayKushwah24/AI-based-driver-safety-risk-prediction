...
import { useMegaHook038 } from '../hooks/useMegaHook038.js';
import { buildTableState038, pipelineData038 } from '../utils/transformSet038.js';
import '../styles/component278.css';

function Meter278({ label, value, ceiling }) {
...
function NestedBlock278({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-278">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-278"
          style={{
...
export default function MegaComponent278({ seed = 278, title = 'Module 278' }) {
  const hook = useMegaHook038(seed + 278);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (278 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer278, initialState278);

  const transformed = useMemo(() => pipelineData038(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState038(transformed, sortBy, sortDir, page, 11), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-278">
      <header className="mega-header-278">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-278">
        <div className="mega-pane-278">
          <Meter278 label="Average" value={summary.avg} ceiling={100} />
          <Meter278 label="Peak" value={summary.max} ceiling={130} />
          <Meter278 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter278 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock278 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-278">
          <form className="mega-form-278" onSubmit={submitLocal}>
