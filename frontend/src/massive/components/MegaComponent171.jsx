...
function Meter171({ label, value, ceiling }) {
...
function NestedBlock171({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-171">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-171"
          style={{
...
export default function MegaComponent171({ seed = 171, title = 'Module 171' }) {
  const hook = useMegaHook051(seed + 171);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (171 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-171">
      <header className="mega-header-171">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-171">
        <div className="mega-pane-171">
          <Meter171 label="Average" value={summary.avg} ceiling={100} />
          <Meter171 label="Peak" value={summary.max} ceiling={130} />
          <Meter171 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter171 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock171 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-171">
          <form className="mega-form-171" onSubmit={submitLocal}>
