...
import '../styles/component027.css';

function Meter027({ label, value, ceiling }) {
...
function NestedBlock027({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-027">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-027"
          style={{
...
export default function MegaComponent027({ seed = 27, title = 'Module 27' }) {
  const hook = useMegaHook027(seed + 27);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (27 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-027">
      <header className="mega-header-027">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-027">
        <div className="mega-pane-027">
          <Meter027 label="Average" value={summary.avg} ceiling={100} />
          <Meter027 label="Peak" value={summary.max} ceiling={130} />
          <Meter027 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter027 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock027 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-027">
          <form className="mega-form-027" onSubmit={submitLocal}>
