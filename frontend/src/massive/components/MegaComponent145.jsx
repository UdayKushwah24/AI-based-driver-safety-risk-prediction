...
function Meter145({ label, value, ceiling }) {
...
function NestedBlock145({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-145">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-145"
          style={{
...
export default function MegaComponent145({ seed = 145, title = 'Module 145' }) {
  const hook = useMegaHook025(seed + 145);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (145 % 17), profile: 'standard' });
  ...
  return (
    <section className="mega-card-145">
      <header className="mega-header-145">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-145">
        <div className="mega-pane-145">
          <Meter145 label="Average" value={summary.avg} ceiling={100} />
          <Meter145 label="Peak" value={summary.max} ceiling={130} />
          <Meter145 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter145 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock145 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-145">
          <form className="mega-form-145" onSubmit={submitLocal}>
