...
import { useMegaHook066 } from '../hooks/useMegaHook066.js';
import { buildTableState066, pipelineData066 } from '../utils/transformSet066.js';
import '../styles/component186.css';

function Meter186({ label, value, ceiling }) {
...
function NestedBlock186({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-186">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-186"
          style={{
...
export default function MegaComponent186({ seed = 186, title = 'Module 186' }) {
  const hook = useMegaHook066(seed + 186);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (186 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer186, initialState186);

  const transformed = useMemo(() => pipelineData066(hook.series, query, state.mode), [hook.series, query, state.mode]);
  const table = useMemo(() => buildTableState066(transformed, sortBy, sortDir, page, 9), [transformed, sortBy, sortDir, page]);
  ...
  return (
    <section className="mega-card-186">
      <header className="mega-header-186">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-186">
        <div className="mega-pane-186">
          <Meter186 label="Average" value={summary.avg} ceiling={100} />
          <Meter186 label="Peak" value={summary.max} ceiling={130} />
          <Meter186 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter186 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock186 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-186">
          <form className="mega-form-186" onSubmit={submitLocal}>
