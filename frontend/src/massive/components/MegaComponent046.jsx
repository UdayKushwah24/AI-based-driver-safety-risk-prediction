...
import '../styles/component046.css';

function Meter046({ label, value, ceiling }) {
...
function NestedBlock046({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-046">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-046"
          style={{
...
function reducer046(state, action) {
...
export default function MegaComponent046({ seed = 46, title = 'Module 46' }) {
  const hook = useMegaHook046(seed + 46);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (46 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer046, initialState046);
  ...
  return (
    <section className="mega-card-046">
      <header className="mega-header-046">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-046">
        <div className="mega-pane-046">
          <Meter046 label="Average" value={summary.avg} ceiling={100} />
          <Meter046 label="Peak" value={summary.max} ceiling={130} />
          <Meter046 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter046 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock046 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-046">
          <form className="mega-form-046" onSubmit={submitLocal}>
