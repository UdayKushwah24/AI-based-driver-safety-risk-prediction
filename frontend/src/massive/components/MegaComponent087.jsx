...
import '../styles/component087.css';

function Meter087({ label, value, ceiling }) {
...
function NestedBlock087({ rows, onSelect, activeId }) {
  return (
    <div className="mega-list-087">
      {rows.map((row) => (
        <button
          key={row.id}
          type="button"
          onClick={() => onSelect(row.id)}
          className="mega-item-087"
          style={{
...
function reducer087(state, action) {
...
export default function MegaComponent087({ seed = 87, title = 'Module 87' }) {
  const hook = useMegaHook087(seed + 87);
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState('score');
  const [sortDir, setSortDir] = useState('desc');
  const [page, setPage] = useState(1);
  const [form, setForm] = useState({ vehicle: '', operator: '', speedCap: 78 + (87 % 17), profile: 'standard' });
  const [state, dispatch] = useReducer(reducer087, initialState087);
  ...
  return (
    <section className="mega-card-087">
      <header className="mega-header-087">
        <strong>{title}</strong>
      ...
      <div className="mega-grid-087">
        <div className="mega-pane-087">
          <Meter087 label="Average" value={summary.avg} ceiling={100} />
          <Meter087 label="Peak" value={summary.max} ceiling={130} />
          <Meter087 label="Spread" value={summary.max - summary.min} ceiling={130} />
          <Meter087 label="Quality" value={hook.state.quality} ceiling={130} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter by zone or status" />
          <NestedBlock087 rows={table.pageRows} onSelect={(value) => dispatch({ type: 'active', payload: value })} activeId={state.active} />
        </div>

        <div className="mega-pane-087">
          <form className="mega-form-087" onSubmit={submitLocal}>
