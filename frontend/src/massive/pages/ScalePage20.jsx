...
import { buildTableState020, pipelineData020, simulateRiskApi020 } from '../utils/transformSet020.js';
import MegaComponent153 from '../components/MegaComponent153.jsx';
import MegaComponent154 from '../components/MegaComponent154.jsx';
import MegaComponent155 from '../components/MegaComponent155.jsx';
import MegaComponent156 from '../components/MegaComponent156.jsx';
import MegaComponent157 from '../components/MegaComponent157.jsx';
import MegaComponent158 from '../components/MegaComponent158.jsx';
import MegaComponent159 from '../components/MegaComponent159.jsx';
import MegaComponent160 from '../components/MegaComponent160.jsx';
import '../styles/page20.css';
...
function reducer20(state, action) {
...
function transformChart20(records) {
...
export default function ScalePage20() {
  const [seed, setSeed] = useState(140);
  ...
  const [state, dispatch] = useReducer(reducer20, initialState20);

  useEffect(() => {
    let active = true;
    dispatch({ type: 'loading', payload: true });
    simulateRiskApi020({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData020(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState020(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart20(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate20(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 20);
  ...
  return (
    <div className="scale-page-20">
      <header className="scale-header-20">
        <div>
          <h2>Scale Page 20</h2>
      ...
      <div className="scale-grid-20">
        <MegaComponent153 seed={seed + 1} title="Page 20 Cluster 1" />
        <MegaComponent154 seed={seed + 2} title="Page 20 Cluster 2" />
        <MegaComponent155 seed={seed + 3} title="Page 20 Cluster 3" />
        <MegaComponent156 seed={seed + 4} title="Page 20 Cluster 4" />
      </div>

      <div className="scale-panel-20">
        <form className="scale-form-20" onSubmit={submitForm}>
      ...
      <div className="scale-panel-20">
        ...
        <table className="scale-table-20">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-20" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-20" onClick={(event) => event.stopPropagation()}>
              <MegaComponent157 seed={seed + 9} title="Overlay 1" />
              <MegaComponent158 seed={seed + 10} title="Overlay 2" />
              <MegaComponent159 seed={seed + 11} title="Overlay 3" />
              <MegaComponent160 seed={seed + 12} title="Overlay 4" />
