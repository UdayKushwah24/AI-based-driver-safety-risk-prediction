...
import { buildTableState008, pipelineData008, simulateRiskApi008 } from '../utils/transformSet008.js';
import MegaComponent057 from '../components/MegaComponent057.jsx';
import MegaComponent058 from '../components/MegaComponent058.jsx';
import MegaComponent059 from '../components/MegaComponent059.jsx';
import MegaComponent060 from '../components/MegaComponent060.jsx';
import MegaComponent061 from '../components/MegaComponent061.jsx';
import MegaComponent062 from '../components/MegaComponent062.jsx';
import MegaComponent063 from '../components/MegaComponent063.jsx';
import MegaComponent064 from '../components/MegaComponent064.jsx';
import '../styles/page08.css';

const initialState08 = {
  tab: 'overview',
  modalOpen: false,
  loading: false,
  records: [],
...
function reducer08(state, action) {
...
function transformChart08(records) {
...
export default function ScalePage08() {
  ...
  const [state, dispatch] = useReducer(reducer08, initialState08);

  useEffect(() => {
    let active = true;
    dispatch({ type: 'loading', payload: true });
    simulateRiskApi008({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData008(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState008(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart08(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate08(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 8);
  ...
  return (
    <div className="scale-page-08">
      <header className="scale-header-08">
        <div>
          <h2>Scale Page 8</h2>
      ...
      <div className="scale-grid-08">
        <MegaComponent057 seed={seed + 1} title="Page 8 Cluster 1" />
        <MegaComponent058 seed={seed + 2} title="Page 8 Cluster 2" />
        <MegaComponent059 seed={seed + 3} title="Page 8 Cluster 3" />
        <MegaComponent060 seed={seed + 4} title="Page 8 Cluster 4" />
      </div>

      <div className="scale-panel-08">
        <form className="scale-form-08" onSubmit={submitForm}>
      ...
      <div className="scale-panel-08">
        ...
        <table className="scale-table-08">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-08" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-08" onClick={(event) => event.stopPropagation()}>
              <MegaComponent061 seed={seed + 9} title="Overlay 1" />
              <MegaComponent062 seed={seed + 10} title="Overlay 2" />
              <MegaComponent063 seed={seed + 11} title="Overlay 3" />
              <MegaComponent064 seed={seed + 12} title="Overlay 4" />
