...
import { buildTableState023, pipelineData023, simulateRiskApi023 } from '../utils/transformSet023.js';
import MegaComponent177 from '../components/MegaComponent177.jsx';
import MegaComponent178 from '../components/MegaComponent178.jsx';
import MegaComponent179 from '../components/MegaComponent179.jsx';
import MegaComponent180 from '../components/MegaComponent180.jsx';
import MegaComponent181 from '../components/MegaComponent181.jsx';
import MegaComponent182 from '../components/MegaComponent182.jsx';
import MegaComponent183 from '../components/MegaComponent183.jsx';
import MegaComponent184 from '../components/MegaComponent184.jsx';
import '../styles/page23.css';
...
function reducer23(state, action) {
...
function transformChart23(records) {
...
export default function ScalePage23() {
  const [seed, setSeed] = useState(161);
  ...
  const [state, dispatch] = useReducer(reducer23, initialState23);

  useEffect(() => {
    let active = true;
    dispatch({ type: 'loading', payload: true });
    simulateRiskApi023({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData023(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState023(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart23(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate23(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 23);
  ...
  return (
    <div className="scale-page-23">
      <header className="scale-header-23">
        <div>
          <h2>Scale Page 23</h2>
      ...
      <div className="scale-grid-23">
        <MegaComponent177 seed={seed + 1} title="Page 23 Cluster 1" />
        <MegaComponent178 seed={seed + 2} title="Page 23 Cluster 2" />
        <MegaComponent179 seed={seed + 3} title="Page 23 Cluster 3" />
        <MegaComponent180 seed={seed + 4} title="Page 23 Cluster 4" />
      </div>

      <div className="scale-panel-23">
        <form className="scale-form-23" onSubmit={submitForm}>
      ...
      <div className="scale-panel-23">
        ...
        <table className="scale-table-23">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-23" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-23" onClick={(event) => event.stopPropagation()}>
              <MegaComponent181 seed={seed + 9} title="Overlay 1" />
              <MegaComponent182 seed={seed + 10} title="Overlay 2" />
              <MegaComponent183 seed={seed + 11} title="Overlay 3" />
              <MegaComponent184 seed={seed + 12} title="Overlay 4" />
