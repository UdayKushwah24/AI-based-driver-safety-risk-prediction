...
import { buildTableState005, pipelineData005, simulateRiskApi005 } from '../utils/transformSet005.js';
import MegaComponent033 from '../components/MegaComponent033.jsx';
import MegaComponent034 from '../components/MegaComponent034.jsx';
import MegaComponent035 from '../components/MegaComponent035.jsx';
import MegaComponent036 from '../components/MegaComponent036.jsx';
import MegaComponent037 from '../components/MegaComponent037.jsx';
import MegaComponent038 from '../components/MegaComponent038.jsx';
import MegaComponent039 from '../components/MegaComponent039.jsx';
import MegaComponent040 from '../components/MegaComponent040.jsx';
import '../styles/page05.css';

const initialState05 = {
  tab: 'overview',
  modalOpen: false,
  loading: false,
  records: [],
...
function reducer05(state, action) {
...
function transformChart05(records) {
...
export default function ScalePage05() {
  ...
  const [state, dispatch] = useReducer(reducer05, initialState05);

  useEffect(() => {
    let active = true;
    dispatch({ type: 'loading', payload: true });
    simulateRiskApi005({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData005(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState005(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart05(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate05(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 5);
  ...
  return (
    <div className="scale-page-05">
      <header className="scale-header-05">
        <div>
          <h2>Scale Page 5</h2>
      ...
      <div className="scale-grid-05">
        <MegaComponent033 seed={seed + 1} title="Page 5 Cluster 1" />
        <MegaComponent034 seed={seed + 2} title="Page 5 Cluster 2" />
        <MegaComponent035 seed={seed + 3} title="Page 5 Cluster 3" />
        <MegaComponent036 seed={seed + 4} title="Page 5 Cluster 4" />
      </div>

      <div className="scale-panel-05">
        <form className="scale-form-05" onSubmit={submitForm}>
      ...
      <div className="scale-panel-05">
        ...
        <table className="scale-table-05">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-05" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-05" onClick={(event) => event.stopPropagation()}>
              <MegaComponent037 seed={seed + 9} title="Overlay 1" />
              <MegaComponent038 seed={seed + 10} title="Overlay 2" />
              <MegaComponent039 seed={seed + 11} title="Overlay 3" />
              <MegaComponent040 seed={seed + 12} title="Overlay 4" />
