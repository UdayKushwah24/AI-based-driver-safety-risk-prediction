...
import { buildTableState026, pipelineData026, simulateRiskApi026 } from '../utils/transformSet026.js';
import MegaComponent201 from '../components/MegaComponent201.jsx';
import MegaComponent202 from '../components/MegaComponent202.jsx';
import MegaComponent203 from '../components/MegaComponent203.jsx';
import MegaComponent204 from '../components/MegaComponent204.jsx';
import MegaComponent205 from '../components/MegaComponent205.jsx';
import MegaComponent206 from '../components/MegaComponent206.jsx';
import MegaComponent207 from '../components/MegaComponent207.jsx';
import MegaComponent208 from '../components/MegaComponent208.jsx';
import '../styles/page26.css';

const initialState26 = {
  tab: 'overview',
  modalOpen: false,
  loading: false,
  records: [],
...
function reducer26(state, action) {
...
function transformChart26(records) {
...
export default function ScalePage26() {
  const [seed, setSeed] = useState(182);
  ...
  const [state, dispatch] = useReducer(reducer26, initialState26);

  useEffect(() => {
    let active = true;
    dispatch({ type: 'loading', payload: true });
    simulateRiskApi026({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData026(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState026(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart26(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate26(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 26);
  ...
  return (
    <div className="scale-page-26">
      <header className="scale-header-26">
        <div>
          <h2>Scale Page 26</h2>
      ...
      <div className="scale-grid-26">
        <MegaComponent201 seed={seed + 1} title="Page 26 Cluster 1" />
        <MegaComponent202 seed={seed + 2} title="Page 26 Cluster 2" />
        <MegaComponent203 seed={seed + 3} title="Page 26 Cluster 3" />
        <MegaComponent204 seed={seed + 4} title="Page 26 Cluster 4" />
      </div>

      <div className="scale-panel-26">
        <form className="scale-form-26" onSubmit={submitForm}>
      ...
      <div className="scale-panel-26">
        ...
        <table className="scale-table-26">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-26" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-26" onClick={(event) => event.stopPropagation()}>
              <MegaComponent205 seed={seed + 9} title="Overlay 1" />
              <MegaComponent206 seed={seed + 10} title="Overlay 2" />
              <MegaComponent207 seed={seed + 11} title="Overlay 3" />
              <MegaComponent208 seed={seed + 12} title="Overlay 4" />
