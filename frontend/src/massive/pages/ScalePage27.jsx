...
import { buildTableState027, pipelineData027, simulateRiskApi027 } from '../utils/transformSet027.js';
import MegaComponent209 from '../components/MegaComponent209.jsx';
import MegaComponent210 from '../components/MegaComponent210.jsx';
import MegaComponent211 from '../components/MegaComponent211.jsx';
import MegaComponent212 from '../components/MegaComponent212.jsx';
import MegaComponent213 from '../components/MegaComponent213.jsx';
import MegaComponent214 from '../components/MegaComponent214.jsx';
import MegaComponent215 from '../components/MegaComponent215.jsx';
import MegaComponent216 from '../components/MegaComponent216.jsx';
import '../styles/page27.css';

const initialState27 = {
  tab: 'overview',
  modalOpen: false,
  loading: false,
  records: [],
...
function reducer27(state, action) {
...
function transformChart27(records) {
...
export default function ScalePage27() {
  const [seed, setSeed] = useState(189);
  ...
  const [state, dispatch] = useReducer(reducer27, initialState27);

  useEffect(() => {
    let active = true;
    dispatch({ type: 'loading', payload: true });
    simulateRiskApi027({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData027(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState027(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart27(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate27(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 27);
  ...
  return (
    <div className="scale-page-27">
      <header className="scale-header-27">
        <div>
          <h2>Scale Page 27</h2>
      ...
      <div className="scale-grid-27">
        <MegaComponent209 seed={seed + 1} title="Page 27 Cluster 1" />
        <MegaComponent210 seed={seed + 2} title="Page 27 Cluster 2" />
        <MegaComponent211 seed={seed + 3} title="Page 27 Cluster 3" />
        <MegaComponent212 seed={seed + 4} title="Page 27 Cluster 4" />
      </div>

      <div className="scale-panel-27">
        <form className="scale-form-27" onSubmit={submitForm}>
      ...
      <div className="scale-panel-27">
        ...
        <table className="scale-table-27">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-27" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-27" onClick={(event) => event.stopPropagation()}>
              <MegaComponent213 seed={seed + 9} title="Overlay 1" />
              <MegaComponent214 seed={seed + 10} title="Overlay 2" />
              <MegaComponent215 seed={seed + 11} title="Overlay 3" />
              <MegaComponent216 seed={seed + 12} title="Overlay 4" />
