...
import { buildTableState021, pipelineData021, simulateRiskApi021 } from '../utils/transformSet021.js';
import MegaComponent161 from '../components/MegaComponent161.jsx';
import MegaComponent162 from '../components/MegaComponent162.jsx';
import MegaComponent163 from '../components/MegaComponent163.jsx';
import MegaComponent164 from '../components/MegaComponent164.jsx';
import MegaComponent165 from '../components/MegaComponent165.jsx';
import MegaComponent166 from '../components/MegaComponent166.jsx';
import MegaComponent167 from '../components/MegaComponent167.jsx';
import MegaComponent168 from '../components/MegaComponent168.jsx';
import '../styles/page21.css';

const initialState21 = {
  tab: 'overview',
  modalOpen: false,
  loading: false,
  records: [],
...
function reducer21(state, action) {
...
function transformChart21(records) {
...
export default function ScalePage21() {
  const [seed, setSeed] = useState(147);
  ...
  const [state, dispatch] = useReducer(reducer21, initialState21);

  useEffect(() => {
    let active = true;
    dispatch({ type: 'loading', payload: true });
    simulateRiskApi021({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData021(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState021(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart21(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate21(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 21);
  ...
  return (
    <div className="scale-page-21">
      <header className="scale-header-21">
        <div>
          <h2>Scale Page 21</h2>
      ...
      <div className="scale-grid-21">
        <MegaComponent161 seed={seed + 1} title="Page 21 Cluster 1" />
        <MegaComponent162 seed={seed + 2} title="Page 21 Cluster 2" />
        <MegaComponent163 seed={seed + 3} title="Page 21 Cluster 3" />
        <MegaComponent164 seed={seed + 4} title="Page 21 Cluster 4" />
      </div>

      <div className="scale-panel-21">
        <form className="scale-form-21" onSubmit={submitForm}>
      ...
      <div className="scale-panel-21">
        ...
        <table className="scale-table-21">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-21" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-21" onClick={(event) => event.stopPropagation()}>
              <MegaComponent165 seed={seed + 9} title="Overlay 1" />
              <MegaComponent166 seed={seed + 10} title="Overlay 2" />
              <MegaComponent167 seed={seed + 11} title="Overlay 3" />
              <MegaComponent168 seed={seed + 12} title="Overlay 4" />
