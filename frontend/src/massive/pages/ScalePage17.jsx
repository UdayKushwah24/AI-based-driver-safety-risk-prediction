...
import { buildTableState017, pipelineData017, simulateRiskApi017 } from '../utils/transformSet017.js';
import MegaComponent129 from '../components/MegaComponent129.jsx';
import MegaComponent130 from '../components/MegaComponent130.jsx';
import MegaComponent131 from '../components/MegaComponent131.jsx';
import MegaComponent132 from '../components/MegaComponent132.jsx';
import MegaComponent133 from '../components/MegaComponent133.jsx';
import MegaComponent134 from '../components/MegaComponent134.jsx';
import MegaComponent135 from '../components/MegaComponent135.jsx';
import MegaComponent136 from '../components/MegaComponent136.jsx';
import '../styles/page17.css';
...
function reducer17(state, action) {
...
function transformChart17(records) {
...
export default function ScalePage17() {
  const [seed, setSeed] = useState(119);
  ...
  const [state, dispatch] = useReducer(reducer17, initialState17);

  useEffect(() => {
    let active = true;
    dispatch({ type: 'loading', payload: true });
    simulateRiskApi017({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData017(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState017(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart17(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate17(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 17);
  ...
  return (
    <div className="scale-page-17">
      <header className="scale-header-17">
        <div>
          <h2>Scale Page 17</h2>
      ...
      <div className="scale-grid-17">
        <MegaComponent129 seed={seed + 1} title="Page 17 Cluster 1" />
        <MegaComponent130 seed={seed + 2} title="Page 17 Cluster 2" />
        <MegaComponent131 seed={seed + 3} title="Page 17 Cluster 3" />
        <MegaComponent132 seed={seed + 4} title="Page 17 Cluster 4" />
      </div>

      <div className="scale-panel-17">
        <form className="scale-form-17" onSubmit={submitForm}>
      ...
      <div className="scale-panel-17">
        ...
        <table className="scale-table-17">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-17" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-17" onClick={(event) => event.stopPropagation()}>
              <MegaComponent133 seed={seed + 9} title="Overlay 1" />
              <MegaComponent134 seed={seed + 10} title="Overlay 2" />
              <MegaComponent135 seed={seed + 11} title="Overlay 3" />
              <MegaComponent136 seed={seed + 12} title="Overlay 4" />
