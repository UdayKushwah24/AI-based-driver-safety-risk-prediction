...
import { buildTableState013, pipelineData013, simulateRiskApi013 } from '../utils/transformSet013.js';
import MegaComponent097 from '../components/MegaComponent097.jsx';
import MegaComponent098 from '../components/MegaComponent098.jsx';
import MegaComponent099 from '../components/MegaComponent099.jsx';
import MegaComponent100 from '../components/MegaComponent100.jsx';
import MegaComponent101 from '../components/MegaComponent101.jsx';
import MegaComponent102 from '../components/MegaComponent102.jsx';
import MegaComponent103 from '../components/MegaComponent103.jsx';
import MegaComponent104 from '../components/MegaComponent104.jsx';
...
function transformChart13(records) {
...
export default function ScalePage13() {
  ...
  useEffect(() => {
    ...
    simulateRiskApi013({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData013(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState013(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart13(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate13(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 13);
  ...
  return (
    <div className="scale-page-13">
      <header className="scale-header-13">
        <div>
          <h2>Scale Page 13</h2>
      ...
      <div className="scale-grid-13">
        <MegaComponent097 seed={seed + 1} title="Page 13 Cluster 1" />
        <MegaComponent098 seed={seed + 2} title="Page 13 Cluster 2" />
        <MegaComponent099 seed={seed + 3} title="Page 13 Cluster 3" />
        <MegaComponent100 seed={seed + 4} title="Page 13 Cluster 4" />
      </div>

      <div className="scale-panel-13">
        <form className="scale-form-13" onSubmit={submitForm}>
      ...
      <div className="scale-panel-13">
        ...
        <table className="scale-table-13">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-13" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-13" onClick={(event) => event.stopPropagation()}>
              <MegaComponent101 seed={seed + 9} title="Overlay 1" />
              <MegaComponent102 seed={seed + 10} title="Overlay 2" />
              <MegaComponent103 seed={seed + 11} title="Overlay 3" />
              <MegaComponent104 seed={seed + 12} title="Overlay 4" />
