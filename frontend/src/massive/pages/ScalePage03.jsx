...
import { buildTableState003, pipelineData003, simulateRiskApi003 } from '../utils/transformSet003.js';
import MegaComponent017 from '../components/MegaComponent017.jsx';
import MegaComponent018 from '../components/MegaComponent018.jsx';
import MegaComponent019 from '../components/MegaComponent019.jsx';
import MegaComponent020 from '../components/MegaComponent020.jsx';
import MegaComponent021 from '../components/MegaComponent021.jsx';
import MegaComponent022 from '../components/MegaComponent022.jsx';
import MegaComponent023 from '../components/MegaComponent023.jsx';
import MegaComponent024 from '../components/MegaComponent024.jsx';
import '../styles/page03.css';

const initialState03 = {
  tab: 'overview',
  modalOpen: false,
  loading: false,
  records: [],
...
function reducer03(state, action) {
...
function transformChart03(records) {
...
export default function ScalePage03() {
  ...
  const [state, dispatch] = useReducer(reducer03, initialState03);

  useEffect(() => {
    let active = true;
    dispatch({ type: 'loading', payload: true });
    simulateRiskApi003({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData003(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState003(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart03(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate03(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 3);
  ...
  return (
    <div className="scale-page-03">
      <header className="scale-header-03">
        <div>
          <h2>Scale Page 3</h2>
      ...
      <div className="scale-grid-03">
        <MegaComponent017 seed={seed + 1} title="Page 3 Cluster 1" />
        <MegaComponent018 seed={seed + 2} title="Page 3 Cluster 2" />
        <MegaComponent019 seed={seed + 3} title="Page 3 Cluster 3" />
        <MegaComponent020 seed={seed + 4} title="Page 3 Cluster 4" />
      </div>

      <div className="scale-panel-03">
        <form className="scale-form-03" onSubmit={submitForm}>
      ...
      <div className="scale-panel-03">
        ...
        <table className="scale-table-03">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-03" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-03" onClick={(event) => event.stopPropagation()}>
              <MegaComponent021 seed={seed + 9} title="Overlay 1" />
              <MegaComponent022 seed={seed + 10} title="Overlay 2" />
              <MegaComponent023 seed={seed + 11} title="Overlay 3" />
              <MegaComponent024 seed={seed + 12} title="Overlay 4" />
