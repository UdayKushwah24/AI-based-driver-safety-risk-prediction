...
import { buildTableState028, pipelineData028, simulateRiskApi028 } from '../utils/transformSet028.js';
import MegaComponent217 from '../components/MegaComponent217.jsx';
import MegaComponent218 from '../components/MegaComponent218.jsx';
import MegaComponent219 from '../components/MegaComponent219.jsx';
import MegaComponent220 from '../components/MegaComponent220.jsx';
import MegaComponent221 from '../components/MegaComponent221.jsx';
import MegaComponent222 from '../components/MegaComponent222.jsx';
import MegaComponent223 from '../components/MegaComponent223.jsx';
import MegaComponent224 from '../components/MegaComponent224.jsx';
import '../styles/page28.css';

const initialState28 = {
  tab: 'overview',
  modalOpen: false,
  loading: false,
  records: [],
...
function reducer28(state, action) {
...
function transformChart28(records) {
...
export default function ScalePage28() {
  const [seed, setSeed] = useState(196);
  ...
  const [state, dispatch] = useReducer(reducer28, initialState28);

  useEffect(() => {
    let active = true;
    dispatch({ type: 'loading', payload: true });
    simulateRiskApi028({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData028(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState028(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart28(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate28(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 28);
  ...
  return (
    <div className="scale-page-28">
      <header className="scale-header-28">
        <div>
          <h2>Scale Page 28</h2>
      ...
      <div className="scale-grid-28">
        <MegaComponent217 seed={seed + 1} title="Page 28 Cluster 1" />
        <MegaComponent218 seed={seed + 2} title="Page 28 Cluster 2" />
        <MegaComponent219 seed={seed + 3} title="Page 28 Cluster 3" />
        <MegaComponent220 seed={seed + 4} title="Page 28 Cluster 4" />
      </div>

      <div className="scale-panel-28">
        <form className="scale-form-28" onSubmit={submitForm}>
      ...
      <div className="scale-panel-28">
        ...
        <table className="scale-table-28">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-28" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-28" onClick={(event) => event.stopPropagation()}>
              <MegaComponent221 seed={seed + 9} title="Overlay 1" />
              <MegaComponent222 seed={seed + 10} title="Overlay 2" />
              <MegaComponent223 seed={seed + 11} title="Overlay 3" />
              <MegaComponent224 seed={seed + 12} title="Overlay 4" />
