...
import { buildTableState018, pipelineData018, simulateRiskApi018 } from '../utils/transformSet018.js';
import MegaComponent137 from '../components/MegaComponent137.jsx';
import MegaComponent138 from '../components/MegaComponent138.jsx';
import MegaComponent139 from '../components/MegaComponent139.jsx';
import MegaComponent140 from '../components/MegaComponent140.jsx';
import MegaComponent141 from '../components/MegaComponent141.jsx';
import MegaComponent142 from '../components/MegaComponent142.jsx';
import MegaComponent143 from '../components/MegaComponent143.jsx';
import MegaComponent144 from '../components/MegaComponent144.jsx';
...
function transformChart18(records) {
...
export default function ScalePage18() {
  const [seed, setSeed] = useState(126);
  ...
  useEffect(() => {
    ...
    simulateRiskApi018({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData018(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState018(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart18(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate18(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 18);
  ...
  return (
    <div className="scale-page-18">
      <header className="scale-header-18">
        <div>
          <h2>Scale Page 18</h2>
      ...
      <div className="scale-grid-18">
        <MegaComponent137 seed={seed + 1} title="Page 18 Cluster 1" />
        <MegaComponent138 seed={seed + 2} title="Page 18 Cluster 2" />
        <MegaComponent139 seed={seed + 3} title="Page 18 Cluster 3" />
        <MegaComponent140 seed={seed + 4} title="Page 18 Cluster 4" />
      </div>

      <div className="scale-panel-18">
        <form className="scale-form-18" onSubmit={submitForm}>
      ...
      <div className="scale-panel-18">
        ...
        <table className="scale-table-18">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-18" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-18" onClick={(event) => event.stopPropagation()}>
              <MegaComponent141 seed={seed + 9} title="Overlay 1" />
              <MegaComponent142 seed={seed + 10} title="Overlay 2" />
              <MegaComponent143 seed={seed + 11} title="Overlay 3" />
              <MegaComponent144 seed={seed + 12} title="Overlay 4" />
