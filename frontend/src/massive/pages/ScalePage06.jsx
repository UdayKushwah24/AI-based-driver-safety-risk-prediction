...
import { buildTableState006, pipelineData006, simulateRiskApi006 } from '../utils/transformSet006.js';
import MegaComponent041 from '../components/MegaComponent041.jsx';
import MegaComponent042 from '../components/MegaComponent042.jsx';
import MegaComponent043 from '../components/MegaComponent043.jsx';
import MegaComponent044 from '../components/MegaComponent044.jsx';
import MegaComponent045 from '../components/MegaComponent045.jsx';
import MegaComponent046 from '../components/MegaComponent046.jsx';
import MegaComponent047 from '../components/MegaComponent047.jsx';
import MegaComponent048 from '../components/MegaComponent048.jsx';
...
function reducer06(state, action) {
...
function transformChart06(records) {
...
export default function ScalePage06() {
  ...
  const [state, dispatch] = useReducer(reducer06, initialState06);

  useEffect(() => {
    let active = true;
    dispatch({ type: 'loading', payload: true });
    simulateRiskApi006({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData006(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState006(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart06(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate06(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 6);
  ...
  return (
    <div className="scale-page-06">
      <header className="scale-header-06">
        <div>
          <h2>Scale Page 6</h2>
      ...
      <div className="scale-grid-06">
        <MegaComponent041 seed={seed + 1} title="Page 6 Cluster 1" />
        <MegaComponent042 seed={seed + 2} title="Page 6 Cluster 2" />
        <MegaComponent043 seed={seed + 3} title="Page 6 Cluster 3" />
        <MegaComponent044 seed={seed + 4} title="Page 6 Cluster 4" />
      </div>

      <div className="scale-panel-06">
        <form className="scale-form-06" onSubmit={submitForm}>
      ...
      <div className="scale-panel-06">
        ...
        <table className="scale-table-06">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-06" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-06" onClick={(event) => event.stopPropagation()}>
              <MegaComponent045 seed={seed + 9} title="Overlay 1" />
              <MegaComponent046 seed={seed + 10} title="Overlay 2" />
              <MegaComponent047 seed={seed + 11} title="Overlay 3" />
              <MegaComponent048 seed={seed + 12} title="Overlay 4" />
