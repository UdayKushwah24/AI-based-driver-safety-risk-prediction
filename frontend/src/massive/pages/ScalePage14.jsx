...
import { buildTableState014, pipelineData014, simulateRiskApi014 } from '../utils/transformSet014.js';
import MegaComponent105 from '../components/MegaComponent105.jsx';
import MegaComponent106 from '../components/MegaComponent106.jsx';
import MegaComponent107 from '../components/MegaComponent107.jsx';
import MegaComponent108 from '../components/MegaComponent108.jsx';
import MegaComponent109 from '../components/MegaComponent109.jsx';
import MegaComponent110 from '../components/MegaComponent110.jsx';
import MegaComponent111 from '../components/MegaComponent111.jsx';
import MegaComponent112 from '../components/MegaComponent112.jsx';
...
function reducer14(state, action) {
...
function transformChart14(records) {
...
export default function ScalePage14() {
  const [seed, setSeed] = useState(98);
  ...
  const [state, dispatch] = useReducer(reducer14, initialState14);

  useEffect(() => {
    let active = true;
    dispatch({ type: 'loading', payload: true });
    simulateRiskApi014({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData014(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState014(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart14(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate14(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 14);
  ...
  return (
    <div className="scale-page-14">
      <header className="scale-header-14">
        <div>
          <h2>Scale Page 14</h2>
      ...
      <div className="scale-grid-14">
        <MegaComponent105 seed={seed + 1} title="Page 14 Cluster 1" />
        <MegaComponent106 seed={seed + 2} title="Page 14 Cluster 2" />
        <MegaComponent107 seed={seed + 3} title="Page 14 Cluster 3" />
        <MegaComponent108 seed={seed + 4} title="Page 14 Cluster 4" />
      </div>

      <div className="scale-panel-14">
        <form className="scale-form-14" onSubmit={submitForm}>
      ...
      <div className="scale-panel-14">
        ...
        <table className="scale-table-14">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-14" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-14" onClick={(event) => event.stopPropagation()}>
              <MegaComponent109 seed={seed + 9} title="Overlay 1" />
              <MegaComponent110 seed={seed + 10} title="Overlay 2" />
              <MegaComponent111 seed={seed + 11} title="Overlay 3" />
              <MegaComponent112 seed={seed + 12} title="Overlay 4" />
