...
import { buildTableState025, pipelineData025, simulateRiskApi025 } from '../utils/transformSet025.js';
import MegaComponent193 from '../components/MegaComponent193.jsx';
import MegaComponent194 from '../components/MegaComponent194.jsx';
import MegaComponent195 from '../components/MegaComponent195.jsx';
import MegaComponent196 from '../components/MegaComponent196.jsx';
import MegaComponent197 from '../components/MegaComponent197.jsx';
import MegaComponent198 from '../components/MegaComponent198.jsx';
import MegaComponent199 from '../components/MegaComponent199.jsx';
import MegaComponent200 from '../components/MegaComponent200.jsx';
...
function transformChart25(records) {
...
export default function ScalePage25() {
  const [seed, setSeed] = useState(175);
  ...
  useEffect(() => {
    ...
    simulateRiskApi025({ seed, points: 36, mode: form.mode }).then((payload) => {
      if (!active) return;
  ...
  const rows = useMemo(() => pipelineData025(state.records.map((r) => r.risk), query, form.mode), [state.records, query, form.mode]);
  const table = useMemo(() => buildTableState025(rows, state.sortBy, state.sortDir, state.page, 9), [rows, state.sortBy, state.sortDir, state.page]);
  const chartData = useMemo(() => transformChart25(state.records), [state.records]);
  ...
  function submitForm(event) {
    ...
    const nextErrors = validate25(form);
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length === 0) {
      setSeed((s) => s + Number(form.speed) + Number(form.fatigue) + 25);
  ...
  return (
    <div className="scale-page-25">
      <header className="scale-header-25">
        <div>
          <h2>Scale Page 25</h2>
      ...
      <div className="scale-grid-25">
        <MegaComponent193 seed={seed + 1} title="Page 25 Cluster 1" />
        <MegaComponent194 seed={seed + 2} title="Page 25 Cluster 2" />
        <MegaComponent195 seed={seed + 3} title="Page 25 Cluster 3" />
        <MegaComponent196 seed={seed + 4} title="Page 25 Cluster 4" />
      </div>

      <div className="scale-panel-25">
        <form className="scale-form-25" onSubmit={submitForm}>
      ...
      <div className="scale-panel-25">
        ...
        <table className="scale-table-25">
          <thead>
            <tr>
              <th>ID</th>
              <th>Zone</th>
      ...
      {state.modalOpen && (
        <div className="scale-modal-25" onClick={() => dispatch({ type: 'modal', payload: false })}>
          <div className="scale-modal-body-25" onClick={(event) => event.stopPropagation()}>
              <MegaComponent197 seed={seed + 9} title="Overlay 1" />
              <MegaComponent198 seed={seed + 10} title="Overlay 2" />
              <MegaComponent199 seed={seed + 11} title="Overlay 3" />
              <MegaComponent200 seed={seed + 12} title="Overlay 4" />
